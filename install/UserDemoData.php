...
class UserDemoData {
	...
	*/
	...
	function create_demo_data()
	{
		...
		foreach($dotb_demodata['users'] as $v)
		{
        ...
        if($id == "seed_jim_id") {
            ...
            foreach($arrRoles as $role) {
                if($role['name'] == "Sales Administrator") {
                    ...
                    // because One2MBeanRelationship::add() doesn't call DotbRelationship::addToResaveList()
	...
	function _quick_create_user($name)
	{
		...
		if (!$this->_user->retrieve($name.'_id'))
		{
			$this->_create_seed_user("{$name}_id", $name, $name, $name,
				$dotb_demodata['users'][0]['title'], $dotb_demodata['users'][0]['is_admin'], "seed_jim_id", $dotb_demodata['users'][0]['last_name'].", ".$dotb_demodata['users'][0]['first_name'], $dotb_demodata['users'][0]['email']);
		}
	}
