...
function checkDBSettings($silent=false) {
    ...
    installLog("testing with {$db->dbType}:{$db->variant}");
        ...
        {
            if( $_SESSION['setup_db_dotbsales_password'] != $_SESSION['setup_db_dotbsales_password_retype'] ){
        ...
        if( $_SESSION['setup_db_dotbsales_user'] != '' && !$_SESSION['setup_db_create_dotbsales_user'] ){
            $dbconfig["db_user_name"] = $_SESSION['setup_db_dotbsales_user'];
            $dbconfig["db_password"] = $_SESSION['setup_db_dotbsales_password'];
            ...
            if(!$db->connect($dbconfig, false)) {
                ...
                installLog("Connection made using  host: {$_SESSION['setup_db_host_name']}, usr: {$_SESSION['setup_db_dotbsales_user']}");
                $db->disconnect();
            }
        }
        ...
        if( empty($_SESSION['setup_db_admin_user_name']) ){
            ...
            if(!$db->connect($dbconfig, false)) {
                ...
                if($silent==false && $db_selected && $_SESSION['setup_db_create_database'] && empty($_SESSION['setup_db_drop_tables'])) {
                    ...
                    if($_SESSION['setup_db_create_dotbsales_user'] && $_SESSION['setup_db_dotbsales_user'] != '' && $db_selected) {
                        if($db->userExists($_SESSION['setup_db_dotbsales_user'])) {
...
function copyInputsIntoSession(){
            ...
            if (isset($_REQUEST['goto']) && $_REQUEST['goto'] == 'SilentInstall') {
                ...
                if (isset($_SESSION['setup_db_dotbsales_user']) && !empty($_SESSION['setup_db_dotbsales_user']))
                    $_REQUEST['setup_db_dotbsales_user'] = $_SESSION['setup_db_dotbsales_user'];
                else $_REQUEST['dbUSRData'] = 'same';

                $_REQUEST['setup_db_dotbsales_password'] = $_SESSION['setup_db_dotbsales_password'];
                $_REQUEST['setup_db_dotbsales_password_retype'] = $_SESSION['setup_db_dotbsales_password'];
            }

            //make sure we are creating or using provided user for app db connections
            $_SESSION['setup_db_create_dotbsales_user']  = true;//get_boolean_from_request('setup_db_create_dotbsales_user');
            $db = getInstallDbInstance();
            if( !$db->supports("create_user") ){
             //if the DB doesn't support creating users, make the admin user/password same as connecting user/password
              $_SESSION['setup_db_dotbsales_user']             = $_SESSION['setup_db_admin_user_name'];
              $_SESSION['setup_db_dotbsales_password']         = $_SESSION['setup_db_admin_password'];
              $_SESSION['setup_db_dotbsales_password_retype']  = $_SESSION['setup_db_dotbsales_password'];
              $_SESSION['setup_db_create_dotbsales_user']      = false;
              $_SESSION['setup_db_create_database']             = false;
                ...
                //wants to use for the dotb db user.
                ...
                if(isset($_REQUEST['dbUSRData'])  && !empty($_REQUEST['dbUSRData'])){
                  ...
                  if($_SESSION['dbUSRData'] == 'auto'){
                    //create user automatically
                      $_SESSION['setup_db_create_dotbsales_user']          = true;
                      $_SESSION['setup_db_dotbsales_user']                 = "dotb".create_db_user_creds(5);
                      $_SESSION['setup_db_dotbsales_password']             = create_db_user_creds(10);
                      $_SESSION['setup_db_dotbsales_password_retype']      = $_SESSION['setup_db_dotbsales_password'];
                  }elseif($_SESSION['dbUSRData'] == 'provide'){
                    //use provided user info
                      $_SESSION['setup_db_create_dotbsales_user']          = false;
                      $_SESSION['setup_db_dotbsales_user']                 = $_REQUEST['setup_db_dotbsales_user'];
                      $_SESSION['setup_db_dotbsales_password']             = $_REQUEST['setup_db_dotbsales_password'];
                      $_SESSION['setup_db_dotbsales_password_retype']      = $_REQUEST['setup_db_dotbsales_password_retype'];
                   ...
                   //Use the same login as provided admin user
                      ...
                      $_SESSION['setup_db_dotbsales_user']             = $_SESSION['setup_db_admin_user_name'];
                      $_SESSION['setup_db_dotbsales_password']         = $_SESSION['setup_db_admin_password'];
                      $_SESSION['setup_db_dotbsales_retype']           = $_SESSION['setup_db_admin_password'];
                  }
            }
