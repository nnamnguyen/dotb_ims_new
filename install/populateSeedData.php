...
$last_name_count = count($dotb_demodata['last_name_array']);
$first_name_count = count($dotb_demodata['first_name_array']);
$company_name_count = count($dotb_demodata['company_name_array']);
$street_address_count = count($dotb_demodata['street_address_array']);
$city_array_count = count($dotb_demodata['city_array']);
$tags_array_count = count($dotb_demodata['tags_array']);
...
$large_scale_test = empty($dotb_config['large_scale_test']) ? false : $dotb_config['large_scale_test'];
...
if(!empty($dotb_config['default_user_name']) &&
	!empty($dotb_config['create_default_user']) &&
	$dotb_config['create_default_user'])
{
	$assigned_user_name = $dotb_config['default_user_name'];
}
...
$tag_ids = createTags($dotb_demodata['tags_array']);
...
$accounts_companies_list = $dotb_demodata['company_name_array'];
...
for($i = 0; $i < $number_companies; $i++) {
    ...
    if (count($accounts_companies_list) > 0) {
        ...
        $account_name =
            $dotb_demodata['first_name_array'][array_rand($dotb_demodata['first_name_array'])] . ' ' .
            $dotb_demodata['last_name_array'][array_rand($dotb_demodata['last_name_array'])] . ' ' .
            $dotb_demodata['company_name_suffix_array'][array_rand($dotb_demodata['company_name_suffix_array'])];
    }
    ...
    $account->billing_address_street = $dotb_demodata['street_address_array'][mt_rand(0, $street_address_count-1)];
    $account->billing_address_city = $dotb_demodata['city_array'][mt_rand(0, $city_array_count-1)];
    ...
    $contact = new Contact();
    $contact->first_name = $dotb_demodata['first_name_array'][mt_rand(0, $first_name_max)];
    $contact->last_name = $dotb_demodata['last_name_array'][mt_rand(0, $last_name_max)];
    $contact->assigned_user_id = $account->assigned_user_id;
    $contact->primary_address_street = $dotb_demodata['street_address_array'][mt_rand(0, $street_address_max)];
    $contact->primary_address_city = $dotb_demodata['city_array'][mt_rand(0, $city_array_max)];
    ...
    for($c = 0; $c < $number_cases; $c++) {
        ...
        $case->name = $dotb_demodata['case_seed_names'][mt_rand(0, 4)];
	...
	$bug->name = $dotb_demodata['bug_seed_names'][mt_rand(0,4)];
	...
	$note->name = $dotb_demodata['note_seed_names_and_Descriptions'][$seed_data_index][0];
	$note->description = $dotb_demodata['note_seed_names_and_Descriptions'][$seed_data_index][1];
	...
	$call->name = $dotb_demodata['call_seed_data_names'][mt_rand(0,3)];
...
for($i=0; $i<$number_contacts; $i++) {
	...
	//Create new tasks
	$task = new Task();
	$key = array_rand($dotb_demodata['task_seed_data_names']);
	$task->name = $dotb_demodata['task_seed_data_names'][$key];
	...
	$key = array_rand($dotb_demodata['meeting_seed_data_names']);
	$meeting->name = $dotb_demodata['meeting_seed_data_names'][$key];
	...
	$meeting->description = $dotb_demodata['meeting_seed_data_descriptions'];
	...
	//Create new emails
	$email = new Email();
    $email->id = \Dotbcrm\Dotbcrm\Util\Uuid::uuid1();
    $email->new_with_id = true;
    BeanFactory::registerBean($email);

	$key = array_rand($dotb_demodata['email_seed_data_subjects']);
	$email->name = $dotb_demodata['email_seed_data_subjects'][$key];
	...
	$email->description = $dotb_demodata['email_seed_data_descriptions'];
    $email->description_html = $dotb_demodata['email_seed_data_description_html'];
	$email->status = 'sent';
    $email->state = $i % 2 === 0 ? 'Draft' : 'Archived';
	$email->parent_id = $account_id;
	$email->parent_type = 'Accounts';
    if (key($dotb_demodata['email_seed_data_types']) === null) {
        reset($dotb_demodata['email_seed_data_types']);
    }
    $email->type = current($dotb_demodata['email_seed_data_types']);
    next($dotb_demodata['email_seed_data_types']);
    ...
    if ($email->state === 'Archived') {
        ...
        $from->id = \Dotbcrm\Dotbcrm\Util\Uuid::uuid1();
    ...
    $to->new_with_id = true;
    $to->id = \Dotbcrm\Dotbcrm\Util\Uuid::uuid1();
    ...
    DotbRelationship::addToResaveList($email);
...
{
	$lead = new Lead();
	$lead->account_name = $dotb_demodata['company_name_array'][mt_rand(0,$company_name_count-1)];
	$lead->first_name = $dotb_demodata['first_name_array'][mt_rand(0,$first_name_max)];
	$lead->last_name = $dotb_demodata['last_name_array'][mt_rand(0,$last_name_max)];
    $lead->primary_address_street = $dotb_demodata['street_address_array'][mt_rand(0, $street_address_max)];
    $lead->primary_address_city = $dotb_demodata['city_array'][mt_rand(0, $city_array_max)];
	$lead->lead_source = array_rand($app_list_strings['lead_source_dom']);
	$lead->title = $dotb_demodata['titles'][mt_rand(0,$title_max)];
	...
	// Fill in a bogus address
	$lead->primary_address_state = $dotb_demodata['primary_address_state'];
	...
	if($i % 3 == 1)
	{
		$lead->billing_address_state = $dotb_demodata['billing_address_state']['east'];
		...
		{
			$lead->billing_address_state = $dotb_demodata['billing_address_state']['west'];
			$assigned_user_id = mt_rand(6,8);
			if($assigned_user_id == 6)
			{
	...
	$lead->primary_address_country = $dotb_demodata['primary_address_country'];
	$lead->save();
    $leads[] = $lead->id;
...
foreach($dotb_demodata['manufacturer_seed_data_names'] as $v){
...
$list_order = 1;
foreach($dotb_demodata['shipper_seed_data_names'] as $v){
	$shipper = new Shipper;
	$shipper->name = $v;
...
foreach($dotb_demodata['productcategory_seed_data_names'] as $v){
	$category = new ProductCategory;
	$category->name = $v;
	$category->list_order = "1";
    $key = array_rand($dotb_demodata['users']);
    $category->assigned_user_id = $dotb_demodata['users'][$key]['id'];
	$category->save();
...
$list_order = 1;
foreach($dotb_demodata['producttype_seed_data_names'] as $v){
...
foreach($dotb_demodata['taxrate_seed_data'] as $v){
	$taxrate = new TaxRate;
	$taxrate->name = $v['name'];
...
foreach($dotb_demodata['currency_seed_data'] as $v){
...
echo '.';
$dollar_id = '-99';
foreach($dotb_demodata['producttemplate_seed_data'] as $v){
...
foreach($dotb_demodata['contract_seed_data'] as $v){
...
$categoryIds = array();
foreach ($dotb_demodata['kbcategories_array'] as $name => $v) {
...
$system_config->saveSetting('KBContents', 'languages', $dotb_demodata['kbdocuments_languages'], 'base');

echo '.';

foreach($dotb_demodata['kbdocuments_seed_data'] as $v){
    ...
    $kbdocContent->status = $dotb_demodata['kbdocuments_statuses'][array_rand($dotb_demodata['kbdocuments_statuses'])];
...
///
if(!empty($dotb_demodata['emailtemplates_seed_data'])) {
	foreach($dotb_demodata['emailtemplates_seed_data'] as $v){
...
$GLOBALS['mod_strings']  = $installerStrings;
    ...
    $opportunity_ids = OpportunitiesSeedData::populateSeedData($number_companies*3, $app_list_strings, $accounts, $dotb_demodata['users']);
...
foreach ($dotb_demodata['dataprivacy_seed_data'] as $i => $seedDp) {
    ...
    $randomUser = $i % count($dotb_demodata['users']);
    $randomUser = $dotb_demodata['users'][$randomUser]['id'];
