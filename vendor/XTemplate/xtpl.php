...
jvink@dotbcrm.com - 2015-06-30 - Adding CSRF form token support
...
This code doesn't have a future in Dotb, but there are some legacy
areas that still rely on it.  Refactoring these few areas cuts its impact
on Dotb's main page by 40% (6.14% to 3.72%).
...
r10797 - 2005-12-21 18:10:38 -0800 (Wed, 21 Dec 2005) - wayne - dotb_version and js_custom_version xtpl assignment now in xtpl.php
...
r730 - 2004-09-09 20:14:02 -0700 (Thu, 09 Sep 2004) - dotbjacob - Cleaning up blanks

r462 - 2004-08-25 17:43:37 -0700 (Wed, 25 Aug 2004) - dotbmsi - added an exists method to check if a block exists in a template

r397 - 2004-08-08 02:28:36 -0700 (Sun, 08 Aug 2004) - dotbjacob - Fix: XTemplate changed to use <?php script declarations

r297 - 2004-07-31 15:13:23 -0700 (Sat, 31 Jul 2004) - dotbjacob - Removing default setting of template language arrays.

r295 - 2004-07-31 14:37:38 -0700 (Sat, 31 Jul 2004) - dotbjacob - Adding code to automatically assign the language strings to every template created.

r268 - 2004-07-16 01:21:57 -0700 (Fri, 16 Jul 2004) - dotbjacob - Changing the XTemplate replacement mechanism to allow for '$' in the text being substituted.

r80 - 2004-06-11 16:39:47 -0700 (Fri, 11 Jun 2004) - dotbjacob - Fixing issue with a variable not being an array in some cases.

r78 - 2004-06-11 16:34:17 -0700 (Fri, 11 Jun 2004) - dotbjacob - Removing errors or notices about invalid indexs.

r3 - 2004-05-26 22:30:56 -0700 (Wed, 26 May 2004) - dotbjacob - Moving project to SourceForge.
...
function parse ($bname) {
	...
	$this->assign('DOTB_VERSION', $GLOBALS['js_version_key']);
	$this->assign('JS_CUSTOM_VERSION', $dotb_config['js_custom_version']);
...
function out ($bname) {
	...
	if(isset($focus)){
		...
		if($focus && is_subclass_of($focus, 'DotbBean') && !$focus->ACLAccess($action)){
...
function r_getfile($file) {
     ...
     * @var string
     */
    protected $csrfVar = 'dotb_csrf_form_token';

    /**
     * Wrapper to mimic Smarty to dynamically add CSRF form token by adding
     * `{dotb_csrf_form_token}` to the template file.
     */
