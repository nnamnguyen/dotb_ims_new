...
git clone git@github.com:dotbcrm/multiverse.git
...
cd $workDir
mkdir ../grpc
find /tmp/multiverse/apis/iam -name "*.proto" -print | while read f; do
    protoc -I$GOPATH/src/github.com/grpc-ecosystem/grpc-gateway/third_party/googleapis \
    --proto_path=/tmp/multiverse/apis/iam  --php_out=../grpc --grpc_out=../grpc \
