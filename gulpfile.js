...
gulp.task('karma', function(done) {
    ...
    if (commander.team) {
        ...
        var team = _.findWhere(teams, {name: 'dotbcrm/eng-' + commander.team});
        ...
        tests = _.reduce(team.files, function(memo, value) {
            ...
            if (value.startsWith('dotbcrm/tests/')) {
                value = value.replace(/^dotbcrm\//, '');
            } else {
                // TODO: As soon as most of the teams add their tests to mentionbot, we will remove this
                value = value.replace(/^dotbcrm/, 'tests/unit-js');
            }
...
gulp.task('test:rest', function() {
    ...
    var cmd = commander
        .option('--url <url>', 'Instance URL, ex: http://my.dotb.server/my/dotb/directory')
...
gulp.task('check-license', function(done) {
    ...
    exec(command, {maxBuffer: 2000 * 1024}, function(error, stdout, stderr) {
        if (stderr.length !== 0) {
            ...
            // All files have the exact license specified in `dotbcrm/LICENSE`
            done();
        }
    });
});
...
gulp.task('copy-sucrose', function() {
    gulp.src([
            'node_modules/@dotbcrm/d3-dotb/build/d3-dotb.min.js'
        ])
        .pipe(insert.prepend(';'))
        .pipe(gulp.dest('include/javascript/d3-dotb/'));
    gulp.src([
            'node_modules/@dotbcrm/d3-dotb/LICENSE'
        ])
        .pipe(gulp.dest('include/javascript/d3-dotb/'));
    gulp.src([
            'node_modules/@dotbcrm/sucrose-dotb/build/sucrose.min.js',
            'node_modules/@dotbcrm/sucrose-dotb/LICENSE',
        ])
        .pipe(gulp.dest('include/javascript/sucrose/'));
    gulp.src([
            'node_modules/@dotbcrm/sucrose-dotb/src/less/**/*',
            '!node_modules/@dotbcrm/sucrose-dotb/src/less/**/sucrose*.less',
            '!node_modules/@dotbcrm/sucrose-dotb/src/less/**/tooltip.less',
            '!node_modules/@dotbcrm/sucrose-dotb/src/less/**/variables.less',
        ])
...
gulp.task('bdd:api', function() {
    ...
    commander
        .option('--url <url>', 'URL of Dotb instance under test')
...
gulp.task('bdd:e2e', function() {
    ...
    let command = 'node ./tests/end-to-end/node_modules/@dotbcrm/seedbed/bin/seedbed.js ' +
