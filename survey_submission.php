...
// Admint Attempt //Take survey changes 03-08-2019
$isAdmitAttempt = false;
if (isset($_REQUEST['submitted_by']) && $_REQUEST['submitted_by'] == 'sender') {
    $isAdmitAttempt = true;
}
...
// If email link
    ...
    if ($submitted_by == "sender" && $isAdmitAttempt) {
        ...
        // In case of Module_id not of 36 Char
        if (count($module_id_array) != 2) {
            $module_id = explode('&sub_id', $module_id_array[1])[0];
        }
        // Default case :: 36 char of Module_id
        else {
...
//    $module_id = $module_id_array[1];
        ...
        $subUnqId = $decoded_param;
        $subQ = "SELECT id,target_parent_type,target_parent_id FROM bc_survey_submission WHERE email_survey_sumission_unique_id = '{$subUnqId}'";
        $submissionDetail = $db->fetchByAssoc($db->query($subQ));
        $module_type = $submissionDetail['target_parent_type'];
        $module_id = $submissionDetail['target_parent_id'];
        $submission_id = $submissionDetail['id'];
        $surveyIdQ = "SELECT bc_survey_submission_bc_surveybc_survey_ida FROM bc_survey_submission_bc_survey_c WHERE bc_survey_submission_bc_surveybc_survey_submission_idb = '{$submission_id}'";
        $surveyIdDetail = $db->fetchByAssoc($db->query($surveyIdQ));
        $survey_id = $surveyIdDetail['bc_survey_submission_bc_surveybc_survey_ida'];
    }
...
$format = $dotb_config['default_date_format'] . ' ' . $dotb_config['default_time_format'];
...
$survey_resubmit_request_url = $dotb_config['site_url'] . '/survey_re_submit_request.php?survey_id=';

$dotb_survey_Url = $survey_resubmit_request_url; //create survey submission url
...
$surveyReQURL = $dotb_survey_Url . $encoded_param . '&selected_lang=' . $selected_lang;
$languageShow = 0;
...
elseif (!($requestApproved) && !$isAdmitAttempt && ($userSbmtCount >= $reSubmitCount) && (!$isOpenSurveyLink)) {
    $msg1 = "<div class='success_msg'>$already_sub_msg {$req_msg} {$resubmit_request_msg}</div>";
}
// if user re submission count is reached then make request to resubmit for open ended
elseif (!($requestApproved) && !$isAdmitAttempt && ($userSbmtCount >= $reSubmitCount) && ($survey->allow_redundant_answers == 0 && $isOpenSurveyLink) && ($submission_status == 'Submitted')) {
    $msg1 = "<div class='success_msg'>$already_sub_msg {$req_msg} {$resubmit_request_msg}</div>";
}
...
// if survey is deactivated / deleted by the sender
elseif (!$isAdmitAttempt && isset($isDeletedRecipient) && ($isDeletedRecipient['deleted'] == 1 || (empty($survey_submission->id) && (!$isOpenSurveyLink)))) {
...
// survey is still not submitted then allow to submit or come for re submission then prefill response data
    ...
    $languageShow = 1;
    $survey_trackdatetime_temp = TimeDate::getInstance()->nowDb();
    ...
    $que_piping_array = array(); // que piping seq per qid
    $que_piping_seq_ids = array(); // que piping seq used in which questions
    foreach ($survey->bc_survey_pages_bc_survey->getBeans() as $pages) {
        ...
        foreach ($pages->bc_survey_pages_bc_survey_questions->getBeans() as $survey_questions) {
            ...
            //changes Regarding the Disable Questions not to show in survey Submission form 08-08-2019
            ...
            $questions[$survey_questions->question_sequence]['piping_sequence'] = (isset($survey_questions->piping_sequence)) ? $survey_questions->piping_sequence : 0;
            $questions[$survey_questions->question_sequence]['qpiping_que_ids'] = (isset($survey_questions->qpiping_que_ids)) ? explode(' ', $survey_questions->qpiping_que_ids) : '';
            $que_piping_seq_ids[$survey_questions->piping_sequence] = $survey_questions->id;
            if (!empty($questions[$survey_questions->question_sequence]['qpiping_que_ids'])) {
                foreach ($questions[$survey_questions->question_sequence]['qpiping_que_ids'] as $k => $pseq) {
                    if (!empty(trim($pseq))) {
                        if (!isset($que_piping_array[$pseq])) {
                            $que_piping_array[$pseq] = array();
                        }
                        $que_piping_array[$pseq][] = $survey_questions->piping_sequence;
                    }
                }
            }
            ...
            if ($enable_data_piping == 1 && $sync_type != 'create_records' && !empty($sync_field)) {
                //Changes regarding the survey piping answer for selection type question 29-02-2020
//                if ($survey_questions->question_type == "multiselectlist" || $survey_questions->question_type == "check-box") {
//                    $FieldValue = trim($moduleBeanObj->$sync_field, "^");
...
//                        if (in_array($survey_answers->piping_selection_type_values, $FieldValue)) {
//                            if ($Value == "") {
//                                $Value = $survey_answers->id;
//                            } else {
//                                $Value = $Value . ',' . $survey_answers->id;
//                            }
//                        }
//                        $moduleFieldValue = explode(",", $Value);
//                    }
////                    $moduleFieldValue = $moduleBeanObj->$sync_field;
//                } else if ($survey_questions->question_type == "radio-button" || $survey_questions->question_type == "dropdownlist") {
//                    $FieldValue = $moduleBeanObj->$sync_field;
//                    $survey_questions->load_relationship('bc_survey_answers_bc_survey_questions');
//                    foreach ($survey_questions->bc_survey_answers_bc_survey_questions->getBeans() as $survey_answers) {
//                        if ($survey_answers->piping_selection_type_values == $FieldValue) {
//                            $moduleFieldValue = $survey_answers->id;
//                        }
//                    }
////                    $moduleFieldValue = $moduleBeanObj->$sync_field;
//                } else {
...
//                }
            ...
            foreach ($survey_questions->bc_survey_answers_bc_survey_questions->getBeans() as $survey_answers) {
                if ($questions[$survey_questions->question_sequence]['is_required'] && !isset($survey_answers->answer_name)) {
                    ...
                    if ($survey_questions->disabled_question == 0) {
...
function getMultiselectHTML($skip_logicArrForAll, $queArr = array(), $submittedAns, $survey_theme, $list_lang_detail, $survey_answer_prefillArra = array(), $richtextContent, $survey_id) {
    global $db;
    ...
    $piping_sequence = $queArr['piping_sequence'];
    $piping_wise_answers = $queArr['piping_wise_answers'];
    $html = "";
    switch ($type) {
        case 'multiselectlist':
            $placeholder_label_other = '';
            $survey_answer_prefill = explode(',', $survey_answer_prefill);
            ...
            if ($is_sort == 1) {
                foreach ($options as $ans_id => $answer) {
                    ...
                    if (is_array($submittedAns) && in_array($ans_id, $submittedAns)) {
                        ...
                        $html .= "<option value='{$ans_id}' selected='true' class='{$is_other}'>" . htmlspecialchars_decode($answer) . "</option>";
                    } else if (is_array($survey_answer_prefill) && in_array('^' . $oAnswer->answer_name . '^', $survey_answer_prefill)) {
                        $html .= "<option value='{$ans_id}' selected='true' class='{$is_other}'>" . htmlspecialchars_decode($answer) . "</option>";
                    } else if (!is_array($survey_answer_prefill) && ('^' . $oAnswer->answer_name . '^' == $survey_answer_prefill)) {
                        $html .= "<option value='{$ans_id}' selected='true' class='{$is_other}'>" . htmlspecialchars_decode($answer) . "</option>";
                    } else if (is_array($submittedAnsOther) && !empty($is_other)) {
                        $html .= "<option value='{$ans_id}' selected='true' class='{$is_other}'>" . htmlspecialchars_decode($answer) . "</option>";
                    } else {
            ...
            if ($is_sort == 1) {
                ...
                if ($advancetype == 'Horizontal') {
                    ...
                    if ($is_image_option) {
                        foreach ($options as $ans_id => $answer) {
                            ...
                            $answer = $fullstring;
                            $answer = '<span class="option_label">' . $answer . '</span>';
                ...
                // if vertical
                    ...
                    foreach ($options as $ans_id => $answer) {
                        ...
                        $answer = $fullstring;
                        $answer = '<div class="option_label">' . $answer . '</div>';
                        // check if answer is other type of or not
                        $is_other = '';
                        $oAnswer = BeanFactory::getBean('bc_survey_answers', $ans_id);
                        if ($oAnswer->answer_type == 'other') {
        ...
        case 'boolean':
            ...
            if (!empty($survey_answer_prefill) && $survey_answer_prefill == 1) {
                $checked = 'checked';
            }
            switch ($submittedAns) {
                case '':
                    ...
                    if ($is_image_option) {
                        foreach ($options as $ans_id => $answer) {
                            ...
                            for ($i = 0; $i < $occu; $i++) {
                                ...
                                $piping_seq .= explode('}}', $sub_part)[0] . ' ';
                                $piped_answer = '';
                                if (!empty($piping_wise_answers)) {
                                    $piped_answer = $piping_wise_answers[$current_piping_seq];
                                }
                                $fullstring = str_replace('{{Q' . trim($current_piping_seq) . '}}', ' <span class="que_pipe_' . $current_piping_seq . '"> ' . $piped_answer . '</span>', $fullstring);
                            }
                            $answer = $fullstring;
                            $answer = '<span class="option_label">' . $answer . '</span>';
                            $optionText = "<p>" . htmlspecialchars_decode($answer) . "</p>";
...
//                                $answer = '';
                            ...
                            if (is_array($submittedAns) && in_array($ans_id, $submittedAns)) {
                                if ($is_skipp == 1) {
                                    ...
                                    $html .= " <li class='md-radio' style='display:inline;'><label><img src='" . $oAnswer->radio_image . "'><input type='radio' value='{$ans_id}' name='{$que_id}[]' id='{$que_id}_{$op}' onchange='addOtherField(this); skipp_logic_question(this,$ans_detail); showReset(this); showQuePipe(this,$piping_sequence);' class='{$que_id} md-radiobtn {$is_other}' checked='true'>$optionText<label for='{$que_id}_{$op}'>
                                <span></span>
                                                            <span class='check'></span>
                                                            <span class='box'></span></label></label></li>";
                                } else {
                                    $html .= " <li class='md-radio' style='display:inline;'><label><img src='" . $oAnswer->radio_image . "'><input type='radio' value='{$ans_id}' name='{$que_id}[]' id='{$que_id}_{$op}' class='{$que_id} md-radiobtn {$is_other}' checked='true' onchange='addOtherField(this);showReset(this); showQuePipe(this,$piping_sequence);'>$optionText<label for='{$que_id}_{$op}'>
                                <span></span>
                                                            <span class='check'></span>
                                                            <span class='box'></span></label></label></li>";
                                }
                                ...
                                if ($is_skipp == 1) {
                                    ...
                                    $html .= " <li class='md-radio' style='display:inline;'><label><img src='" . $oAnswer->radio_image . "'><input type='radio' value='{$ans_id}' name='{$que_id}[]' id='{$que_id}_{$op}' onchange='addOtherField(this); skipp_logic_question(this,$ans_detail); showReset(this); showQuePipe(this,$piping_sequence);' class='{$que_id} md-radiobtn {$is_other}'>$optionText<label for='{$que_id}_{$op}'>
                                <span></span>
                                                            <span class='check'></span>
                                                            <span class='box'></span></label></label></li>";
                                } else {
                                    $html .= " <li class='md-radio' style='display:inline;'><label><img src='" . $oAnswer->radio_image . "'><input type='radio' value='{$ans_id}' name='{$que_id}[]' id='{$que_id}_{$op}' class='{$que_id} md-radiobtn {$is_other}' onchange='addOtherField(this);showReset(this); showQuePipe(this,$piping_sequence);'>$optionText<label for='{$que_id}_{$op}'>
                                <span></span>
                                                            <span class='check'></span>
                                                            <span class='box'></span></label></label></li>";
                                }
                        ...
                        foreach ($options as $ans_id => $answer) {
                            ...
                            if (is_array($submittedAns) && in_array($ans_id, $submittedAns)) {
                                if ($is_skipp == 1) {
                                    ...
                                    $html .= " <li class='md-radio' style='display:inline;'><label><input type='radio' value='{$ans_id}' name='{$que_id}[]' id='{$que_id}_{$op}' onchange='addOtherField(this); skipp_logic_question(this,$ans_detail); showReset(this); showQuePipe(this,$piping_sequence);' class='{$que_id} md-radiobtn {$is_other}' checked='true'>" . htmlspecialchars_decode($answer) . "<label for='{$que_id}_{$op}'>
                                <span></span>
                                                            <span class='check'></span>
                                                            <span class='box'></span></label></label></li>";
                                } else {
                                    $html .= " <li class='md-radio' style='display:inline;'><label><input type='radio' value='{$ans_id}' name='{$que_id}[]' id='{$que_id}_{$op}' class='{$que_id} md-radiobtn {$is_other}' checked='true' onchange='addOtherField(this);showReset(this); showQuePipe(this,$piping_sequence);'>" . htmlspecialchars_decode($answer) . "<label for='{$que_id}_{$op}'>
                                <span></span>
                                                            <span class='check'></span>
                                                            <span class='box'></span></label></label></li>";
                                }
                                ...
                                if ($is_skipp == 1) {
                                    ...
                                    $html .= " <li class='md-radio' style='display:inline;'><label><input type='radio' value='{$ans_id}' name='{$que_id}[]' id='{$que_id}_{$op}' onchange='addOtherField(this); skipp_logic_question(this,$ans_detail); showReset(this); showQuePipe(this,$piping_sequence);' class='{$que_id} md-radiobtn {$is_other}'>" . htmlspecialchars_decode($answer) . "<label for='{$que_id}_{$op}'>
                                <span></span>
                                                            <span class='check'></span>
                                                            <span class='box'></span></label></label></li>";
                                } else {
                                    $html .= " <li class='md-radio' style='display:inline;'><label><input type='radio' value='{$ans_id}' name='{$que_id}[]' id='{$que_id}_{$op}' class='{$que_id} md-radiobtn {$is_other}' onchange='addOtherField(this);showReset(this); showQuePipe(this,$piping_sequence);'>" . htmlspecialchars_decode($answer) . "<label for='{$que_id}_{$op}'>
                                <span></span>
                                                            <span class='check'></span>
                                                            <span class='box'></span></label></label></li>";
                                }
                    ...
                    foreach ($options as $ans_id => $answer) {
                        ...
                        if (is_array($submittedAns) && in_array($ans_id, $submittedAns)) {
                            if ($is_skipp == 1) {
                                ...
                                $html .= " <li class='md-radio'><label><input type='radio' value='{$ans_id}' name='{$que_id}[]' id='{$que_id}_{$op}' onchange='addOtherField(this); skipp_logic_question(this,$ans_detail);showReset(this); showQuePipe(this,$piping_sequence);' class='{$que_id} md-radiobtn {$is_other}' checked='true'>" . htmlspecialchars_decode($answer) . "<label for='{$que_id}_{$op}'>
                                <span></span>
                                                            <span class='check'></span>
                                                            <span class='box'></span></label></label></li>";
                            } else {
                                $html .= " <li class='md-radio'><label><input type='radio' value='{$ans_id}' name='{$que_id}[]' id='{$que_id}_{$op}' class='{$que_id} md-radiobtn {$is_other}' checked='true' onchange='addOtherField(this);showReset(this); showQuePipe(this,$piping_sequence);'>" . htmlspecialchars_decode($answer) . "<label for='{$que_id}_{$op}'>
                                <span></span>
                                                            <span class='check'></span>
                                                            <span class='box'></span></label></label></li>";
                            }
                            ...
                            if ($is_skipp == 1) {
                                ...
                                $html .= " <li class='md-radio'><label><input type='radio' value='{$ans_id}' name='{$que_id}[]' id='{$que_id}_{$op}' onchange='addOtherField(this); skipp_logic_question(this,$ans_detail); showReset(this); showQuePipe(this,$piping_sequence);' class='{$que_id} md-radiobtn {$is_other}'>" . htmlspecialchars_decode($answer) . "<label for='{$que_id}_{$op}'>
                                <span></span>
                                                            <span class='check'></span>
                                                            <span class='box'></span></label></label></li>";
                            } else {
                                $html .= " <li class='md-radio'><label><input type='radio' value='{$ans_id}' name='{$que_id}[]' id='{$que_id}_{$op}' class='{$que_id} md-radiobtn {$is_other}' onchange='addOtherField(this);showReset(this); showQuePipe(this,$piping_sequence);'>" . htmlspecialchars_decode($answer) . "<label for='{$que_id}_{$op}'>
                                <span></span>
                                                            <span class='check'></span>
                                                            <span class='box'></span></label></label></li>";
                            }
                            ...
                            foreach ($ans as $ans_id => $answer) {
                                ...
                                if (is_array($submittedAns) && in_array($ans_id, $submittedAns)) {
                                    if ($is_skipp == 1) {
                                        ...
                                        $html .= " <li class='md-radio' style='display:inline;'><label><img src='" . $oAnswer->radio_image . "'><input type='radio' value='{$ans_id}' id='{$que_id}_{$op}' onchange='addOtherField(this); skipp_logic_question(this,$ans_detail); showReset(this); showQuePipe(this,$piping_sequence);' name='{$que_id}[]' class='{$que_id} md-radiobtn {$is_other}' checked='true'>$optionText<label for='{$que_id}_{$op}'>
                                    <span></span>
                                                            <span class='check'></span>
                                                            <span class='box'></span></label></label></li>";
                                    } else {
                                        $html .= " <li class='md-radio' style='display:inline;'><label><img src='" . $oAnswer->radio_image . "'><input type='radio' value='{$ans_id}' id='{$que_id}_{$op}' name='{$que_id}[]' class='{$que_id} md-radiobtn {$is_other}' checked='true' onchange='addOtherField(this);showReset(this); showQuePipe(this,$piping_sequence);'>$optionText<label for='{$que_id}_{$op}'>
                                    <span></span>
                                                            <span class='check'></span>
                                                            <span class='box'></span></label></label></li>";
                                    }
                                    ...
                                    <span></span>
                                        ...
                                        $html .= " <li class='md-radio' style='display:inline;'><label><img src='" . $oAnswer->radio_image . "'><input type='radio' value='{$ans_id}' id='{$que_id}_{$op}' name='{$que_id}[]' class='{$que_id} md-radiobtn {$is_other}' onchange='addOtherField(this);showReset(this); showQuePipe(this,$piping_sequence);'>$optionText<label for='{$que_id}_{$op}'>
                                    <span></span>
                                                            <span class='check'></span>
                                                            <span class='box'></span></label></label></li>";
                                    }
                                    ...
                                    if ($is_skipp == 1) {
                                        ...
                                        $html .= " <li class='md-radio' style='display:inline;'><label><img src='" . $oAnswer->radio_image . "'><input type='radio' value='{$ans_id}' id='{$que_id}_{$op}' name='{$que_id}[]' onchange='addOtherField(this); skipp_logic_question(this,$ans_detail); showReset(this); showQuePipe(this,$piping_sequence);' class='{$que_id} md-radiobtn {$is_other}'>$optionText<label for='{$que_id}_{$op}'>
                                    <span></span>
                                                            <span class='check'></span>
                                                            <span class='box'></span></label></label></li>";
                                    } else {
                            ...
                            foreach ($ans as $ans_id => $answer) {
                                ...
                                if (is_array($submittedAns) && in_array($ans_id, $submittedAns)) {
                                    if ($is_skipp == 1) {
                                        ...
                                        $html .= " <li class='md-radio' style='display:inline;'><label><input type='radio' value='{$ans_id}' id='{$que_id}_{$op}' onchange='addOtherField(this); skipp_logic_question(this,$ans_detail); showReset(this); showQuePipe(this,$piping_sequence);' name='{$que_id}[]' class='{$que_id} md-radiobtn {$is_other}' checked='true'>" . htmlspecialchars_decode($answer) . "<label for='{$que_id}_{$op}'>
                                    <span></span>
                                                            <span class='check'></span>
                                                            <span class='box'></span></label></label></li>";
                                    } else {
                                        $html .= " <li class='md-radio' style='display:inline;'><label><input type='radio' value='{$ans_id}' id='{$que_id}_{$op}' name='{$que_id}[]' class='{$que_id} md-radiobtn {$is_other}' checked='true' onchange='addOtherField(this);showReset(this); showQuePipe(this,$piping_sequence);'>" . htmlspecialchars_decode($answer) . "<label for='{$que_id}_{$op}'>
                                    <span></span>
                                                            <span class='check'></span>
                                                            <span class='box'></span></label></label></li>";
                                    }
                                    ...
                                    if ($is_skipp == 1) {
                                        ...
                                        $html .= " <li class='md-radio' style='display:inline;'><label><input type='radio' value='{$ans_id}' id='{$que_id}_{$op}' name='{$que_id}[]' onchange='addOtherField(this); skipp_logic_question(this,$ans_detail); showReset(this); showQuePipe(this,$piping_sequence);' class='{$que_id} md-radiobtn {$is_other}'>" . htmlspecialchars_decode($answer) . "<label for='{$que_id}_{$op}'>
                                    <span></span>
                                                            <span class='check'></span>
                                                            <span class='box'></span></label></label></li>";
                                    } else {
                                        $html .= " <li class='md-radio' style='display:inline;'><label><input type='radio' value='{$ans_id}' id='{$que_id}_{$op}' name='{$que_id}[]' class='{$que_id} md-radiobtn {$is_other}' onchange='addOtherField(this);showReset(this); showQuePipe(this,$piping_sequence);'>" . htmlspecialchars_decode($answer) . "<label for='{$que_id}_{$op}'>
                                    <span></span>
                                                            <span class='check'></span>
                                                            <span class='box'></span></label></label></li>";
                                    }
        ...
        case 'dropdownlist':
            ...
            if ($is_skipp == 1) {
                ...
                $html .= "<select name='{$que_id}[]' class='form-control required {$que_id}' onchange='addOtherField(this); skipp_logic_question(this,$ans_detail); showQuePipe(this,$piping_sequence);'><option selected='' value='selection_default_value_dropdown'>Select</option>";
            } else {
                $html .= "<select name='{$que_id}[]' class='form-control required {$que_id}' onchange='addOtherField(this); showQuePipe(this,$piping_sequence);'><option selected='' value='selection_default_value_dropdown'>Select</option>";
            }
            ...
            if ($is_sort == 1) {
                ...
                foreach ($options as $ans_id => $answer) {
                    ...
                    if ($oAnswer->answer_type == 'other') {
                        $is_other = 'is_other_option';
                        $otherAnswerName = BeanFactory::getBean('bc_survey_answers', $submittedAns);
                        // Make Other as selected as we got Other answer
                        if ($otherAnswerName->name != "") {
                            if ((is_array($submittedAns) && strtolower($submittedAns[0]) != 'n/a') || !is_array($submittedAns) && !empty($submittedAns) && strtolower($submittedAns) != 'n/a' && !array_key_exists($submittedAns, $other_answer)) {
                                $submittedAns = $ans_id;
                            }
                        }
                    }
        ...
        case 'textbox':
            ...
            if (!is_array($submittedAns) && !empty($submittedAns) && $advancetype == 'Integer') {
                $html .= "<input class='form-control {$que_id} numericField' type='text' name='{$que_id}[]' value='{$submittedAns}' class='{$que_id}' onchange=' showQuePipe(this,$piping_sequence);'>";
            }
            // if float then add class to validate with prefill submitted answer
                ...
                $html .= "<input class='form-control {$que_id} decimalField' type='text' name='{$que_id}[]' value='{$submittedAns}' class='{$que_id}' onchange=' showQuePipe(this,$piping_sequence);'>";
            }
            // if float then add class to validate with prefill submitted answer
            else if (!is_array($submittedAns) && !empty($submittedAns)) {
                $html .= "<input class='form-control {$que_id} ' type='text' name='{$que_id}[]' value='{$submittedAns}' class='{$que_id}' onchange=' showQuePipe(this,$piping_sequence);'>";
            } else if (!is_array($survey_answer_prefill) && !empty($survey_answer_prefill) && $advancetype == 'Integer') {
                $html .= "<input class='form-control {$que_id} numericField' type='text' name='{$que_id}[]' value='{$survey_answer_prefill}' class='{$que_id}' onchange=' showQuePipe(this,$piping_sequence);'>";
            }
            // if float then add class to validate with prefill submitted answer
            else if (!is_array($survey_answer_prefill) && !empty($survey_answer_prefill) && $advancetype == 'Float') {
                $html .= "<input class='form-control {$que_id} decimalField' type='text' name='{$que_id}[]' value='{$survey_answer_prefill}' class='{$que_id}' onchange=' showQuePipe(this,$piping_sequence);'>";
            }
            // if float then add class to validate with prefill submitted answer
            else if (!is_array($survey_answer_prefill) && !empty($survey_answer_prefill)) {
                $html .= "<input class='form-control {$que_id} ' type='text' name='{$que_id}[]' value='{$survey_answer_prefill}' class='{$que_id}' onchange=' showQuePipe(this,$piping_sequence);'>";
            }
            // if interger then add class to validate
            else if (empty($submittedAns) && $advancetype == 'Integer') {
                $html .= "<input class='form-control {$que_id} numericField' type='text' name='{$que_id}[]' class='{$que_id}' onchange=' showQuePipe(this,$piping_sequence);'>";
            }
            // if float then add class to validate
            else if (empty($submittedAns) && $advancetype == 'Float') {
                $html .= "<input class='form-control {$que_id} decimalField' type='text' name='{$que_id}[]' class='{$que_id}' onchange=' showQuePipe(this,$piping_sequence);'>";
            }
            // default textbox
            else {
                $html .= "<input class='form-control {$que_id} ' type='text' name='{$que_id}[]' class='{$que_id}' onchange=' showQuePipe(this,$piping_sequence);'>";
            }
            $html .= "</li></ul></div>";
            return $html;
            break;
        case 'commentbox':
            ...
            if (!is_array($submittedAns) && !empty($submittedAns)) {
                ...
                if (!empty($min) || !empty($max)) {
                    $html .= "<textarea style='height:auto;width:auto;' class='commentbox-class form-control {$que_id}' rows='{$min}' cols='{$max}' name='{$que_id}[]' onchange=' showQuePipe(this,$piping_sequence);'>{$submittedAns}</textarea>";
                }
                // default textarea
                else {
                    $html .= "<textarea class='commentbox-class form-control {$que_id}' rows='4' cols='20' name='{$que_id}[]' onchange=' showQuePipe(this,$piping_sequence);'>{$submittedAns}</textarea>";
                }
            } else if (!is_array($survey_answer_prefill) && !empty($survey_answer_prefill)) {
                // layout with given rows and columns
                if (!empty($min) || !empty($max)) {
                    $html .= "<textarea style='height:auto;width:auto;' class='commentbox-class form-control {$que_id}' rows='{$min}' cols='{$max}' name='{$que_id}[]' onchange=' showQuePipe(this,$piping_sequence);'>{$survey_answer_prefill}</textarea>";
                }
                // default textarea
                else {
                    $html .= "<textarea class='commentbox-class form-control {$que_id}' rows='4' cols='20' name='{$que_id}[]' onchange=' showQuePipe(this,$piping_sequence);'>{$survey_answer_prefill}</textarea>";
                }
            }
            // not submitted answer
                ...
                // layout with given rows and columns
                if (!empty($min) || !empty($max)) {
                    $html .= "<textarea style='height:auto;width:auto;' class='commentbox-class form-control {$que_id}' rows='{$min}' cols='{$max}' name='{$que_id}[]' onchange=' showQuePipe(this,$piping_sequence);'></textarea>";
                }
                // default textarea
                else {
                    $html .= "<textarea class='commentbox-class form-control {$que_id}' rows='4' cols='20' name='{$que_id}[]' onchange=' showQuePipe(this,$piping_sequence);'></textarea>";
                }
            }
            $html .= "</li></ul></div>";
            return $html;
        ...
        case 'rating':
            ...
            for ($i = 1; $i <= $starCount; $i++) {
                ...
                $html .= "<li class='rating {$selected}' style='display: inline;font-size: x-large' onmouseover='highlightStar(this,\"{$que_id}\");'  onmouseout='removeHighlight(\"{$que_id}\");' onclick='addRating(this,\"{$que_id}\");showReset(this); showQuePipe(this,$piping_sequence);'>&#9733;</li>";
            }
            $html .= "</ul>";
            $html .= "</div>";
        ...
        case 'contact-information':
            ...
            break;
        case 'date-time':
            // auto fill date datetime when data piping is enabled
            if (!empty($survey_answer_prefill) && empty($submittedAns)) {
                $survey = BeanFactory::getBean('bc_survey', $survey_id);
                $user = new User();
                $user->retrieve($survey->created_by);
                $timedate = new TimeDate();
                if ($is_datetime == 1) {
                    $submittedAns = $timedate->to_display_date_time($survey_answer_prefill, true, true, $user);
                    $date = date_create($submittedAns);
                    $submittedAns = date_format($date, "Y/m/d H:i");
                } else {
                    $submittedAns = $timedate->to_display_date($survey_answer_prefill, true, true, $user);
                    $date = date_create($submittedAns);
                    $submittedAns = date_format($date, "Y/m/d");
                }
            }
            $html = "<div class='option select-list two-col' id='{$que_id}_div'><ul><li>";
            ...
            if (!is_array($submittedAns) && !empty($submittedAns)) {
                // if is date and time
                if ($is_datetime == 1) {
                    //Allow Future Dates or not
                    if ($allow_future_dates == 1) {
                        $html .= "<input class='form-control setdatetimeAllowfutureDate {$que_id}_datetime' value='{$submittedAns}' autocomplete='off' type='text' name='{$que_id}[]' class='{$que_id}'>";
                    } else {
                        $html .= "<input class='form-control setdatetime {$que_id}_datetime' value='{$submittedAns}' autocomplete='off' type='text' name='{$que_id}[]' class='{$que_id}'>";
                    }
                }
                // only date
                else {
                    //Allow Future Dates or not
                    if ($allow_future_dates == 1) {
                        $html .= "<input class='form-control setdateAllowfutureDate {$que_id}_datetime' autocomplete='off' type='text' value='{$submittedAns}' name='{$que_id}[]' class='{$que_id}'>";
                    } else {
                        $html .= "<input class='form-control setdate {$que_id}_datetime' autocomplete='off' type='text' value='{$submittedAns}' name='{$que_id}[]' class='{$que_id}'>";
                    }
                }
            }
            // not submitted answer
                ...
                if ($is_datetime == 1) {
                    //Allow Future Dates or not
                    if ($allow_future_dates == 1) {
                        $html .= "<input class='form-control setdatetimeAllowfutureDate {$que_id}_datetime' autocomplete='off' type='text' name='{$que_id}[]' class='{$que_id}'>";
                    } else {
                        $html .= "<input class='form-control setdatetime {$que_id}_datetime' autocomplete='off' type='text' name='{$que_id}[]' class='{$que_id}'>";
                    }
                }
                // only date
                else {
                    //Allow Future Dates or not
                    if ($allow_future_dates == 1) {
                        $html .= "<input class='form-control setdateAllowfutureDate {$que_id}_datetime' autocomplete='off' type='text' name='{$que_id}[]' class='{$que_id}'>";
                    } else {
                        $html .= "<input class='form-control setdate {$que_id}_datetime' autocomplete='off' type='text' name='{$que_id}[]' class='{$que_id}'>";
                    }
                }
            }
            $html .= "</li></ul></div>";
        ...
        case 'scale':
            ...
            $html .= '<br/><section style="width:60%;padding-top: 0px !important;" class=' . $que_id . ' onmouseup="showReset(this);showQuePipe(this,' . $piping_sequence . ');"> 
                        <div id="slider"></div>
                    </section>';
            ...
            $html .= "<input type='hidden'  name='{$que_id}[]' class='{$que_id}_scale' id='{$que_id}_hidden' value='{$submittedAns}' onchange='showReset(this);showQuePipe(this,$piping_sequence);'>";
            $html .= "</div>";
            return $html;
            break;
        case 'matrix':
...
//            }
                            ...
                            //Display answer input (RadioButton or Checkbox)
                                ...
                                if (in_array($current_value, $submittedAnsDetail)) {
                                    if ($display_type == 'checkbox') {
                                        ...
                                        $html .= "<td class='matrix-span' style='width:" . $width . "; '><span class='md-radio' style='margin-left:" . $margin_block . "'><input checked type='" . $display_type . "' id='{$que_id}_{$op}' class='{$que_id} md-radio' value='{$row}_{$col}' name='{$que_id}[{$row}][]' onchange='showReset(this);'/><label for='{$que_id}_{$op}'>
                                                            <span></span>
                                                            <span class='check'></span>
                                                            <span class='box'></span>";
                                        $html .= "<span class='mobile_only'> " . $col_details[$j] . "</span>";
                                    ...
                                    if ($display_type == 'checkbox') {
                                        ...
                                        $html .= "<td class='matrix-span' style='width:" . $width . "; '><span class='md-radio' style='margin-left:" . $margin_block . "'><input type='" . $display_type . "' id='{$que_id}_{$op}' class='{$que_id} md-radio' value='{$row}_{$col}' name='{$que_id}[{$row}][]' onchange='showReset(this);'/><label for='{$que_id}_{$op}'>
                                                            <span></span>
                                                            <span class='check'></span>
                                                            <span class='box'></span>";
                                        $html .= "<span class='mobile_only'> " . $col_details[$j] . "</span>";
                        ...
                        // Rows Label
                            ...
                            //Display answer input (RadioButton or Checkbox)
                                ...
                                if ($display_type == 'checkbox') {
                                    ...
                                    $html .= "<td class='matrix-span' style='width:" . $width . "; '><span class='md-radio' style='margin-left:" . $margin_block . "'><div class='matrix-span' style='width:" . $width . "; '><input onclick='showReset(this);' type='" . $display_type . "' id='{$que_id}_{$op}' value='{$row}_{$col}' name='{$que_id}[{$row}][]'/><label for='{$que_id}_{$op}'>
                                                            <span></span>
                                                            <span class='check'></span>
                                                            <span class='box'></span></label></span></div></td>";
                                }
        ...
        case 'doc-attachment':
            ...
            $html = "<div class='option select-list two-col' id='{$que_id}_div'><ul><li style='margin: 0 1% 3px 0; width: 100%;position:relative;'>";
            ...
            if (!empty($file_attched)) {
                $html .= "<div class='file_uploaded'><span class='imgcontent'>" . $file_attched . " &nbsp;&nbsp;<a style='cursor:pointer; color:red;' onclick='removeAttachment(this)'>Remove</a></div></span>"
            ...
            $html .= "<div style='cursor: default;' class='val-msg-upload uploadvalidation'></div>"
        ...
        case 'netpromoterscore':
            ...
            if ($supported_survey_language != null) {
                $html .= "<div class='score_pannel_wrapper language_selected' id='score_pannel_{$que_id}'>
                ...
                foreach ($ans as $ans_id => $answer) {
                    ...
                    if ($answer < 7) {
                        if ($submittedAns == $answer) {
                            if ($is_skipp == 1) {
                                $html .= "<th><div onclick='applyNPSSelectedColor(this); skipp_logic_question(this,$ans_detail);showReset(this); showQuePipe(this,$piping_sequence);' class='score_pannel' id='$ans_id' value='$answer' style='background-color:#a1cbff'>" . $answer . "</div></th>";
                            } else {
                                $html .= "<th><div  onclick='applyNPSSelectedColor(this);showReset(this); showQuePipe(this,$piping_sequence);' class='score_pannel' id='$ans_id' value='$answer' style='background-color:#a1cbff'>" . $answer . "</div></th>";
                            }
                        } else {
                            if ($is_skipp == 1) {
                                $html .= "<th><div onclick='applyNPSSelectedColor(this); skipp_logic_question(this,$ans_detail);showReset(this); showQuePipe(this,$piping_sequence);' class='score_pannel' id='$ans_id' value='$answer' style='background-color:#ff5353'>" . $answer . "</div></th>";
                            } else {
                                $html .= "<th><div onclick='applyNPSSelectedColor(this);showReset(this); showQuePipe(this,$piping_sequence);' class='score_pannel' id='$ans_id' value='$answer' style='background-color:#ff5353'>" . $answer . "</div></th>";
                            }
                        }
                        ...
                        if ($submittedAns == $answer) {
                            ...
                            if ($is_skipp == 1) {
                                $html .= "<th><div onclick='applyNPSSelectedColor(this); skipp_logic_question(this,$ans_detail);showReset(this); showQuePipe(this,$piping_sequence);' class='score_pannel' id='$ans_id' value='$answer' style='background-color:#e9e817'>" . $answer . "</div></th>";
                            } else {
                                $html .= "<th><div  onclick='applyNPSSelectedColor(this);showReset(this); showQuePipe(this,$piping_sequence);' class='score_pannel' id='$ans_id' value='$answer'style='background-color:#e9e817'>" . $answer . "</div></th>";
                            }
                        }
                        ...
                        if ($submittedAns == $answer) {
                            ...
                            if ($is_skipp == 1) {
                                $html .= "<th><div onclick='applyNPSSelectedColor(this); skipp_logic_question(this,$ans_detail);showReset(this); showQuePipe(this,$piping_sequence);' class='score_pannel' id='$ans_id' value='$answer' style='background-color:#92d51a'>" . $answer . "</div></th>";
                            } else {
                                $html .= "<th><div onclick='applyNPSSelectedColor(this);showReset(this); showQuePipe(this,$piping_sequence);'  class='score_pannel' id='$ans_id' value='$answer' style='background-color:#92d51a'>" . $answer . "</div></th>";
                            }
                        }
                    }
                    ...
                    if ($ans_id == $submittedAns['answerId']) {
                        if ($is_skipp == 1) {
                           ...
                           <label for='{$que_id}_{$op}'>
                            ...
                            $html .= "<div  class='md-emojis' onclick='switchEmojis(this,\"{$op}\",\"{$que_id}\");showReset(this);showQuePipe(this,$piping_sequence);'><li class='md-radio' style='display:inline;background-color: #ebebeb;'><label>
                                 <img class='Grey_Emoji' id='Grey_emojis_" . $op . "' src='{$emojisImgesGrey[$op]}' height='40' width='45' style='display:none;' value='Grey_emojis_" . $op . ".png_{$que_id}'>
                                    <img class='Emoji' id='emojis_" . $op . "' src='{$emojisImges[$op]}' height='40' width='45' style='display:inline-block;' value='emojis_" . $op . ".png_{$que_id}'>
                                        <input type='radio' id='{$que_id}_{$op}' value='{$ans_id}' name='{$que_id}[]' class='{$que_id} md-radiobtn ' checked='true' >
                                            <div class='emoji_text'>" . htmlspecialchars_decode($answer) . "</div>
                        ...
                        if ($is_skipp == 1) {
                            $html .= "<div  class='md-emojis' onclick='switchEmojis(this,\"{$op}\",\"{$que_id}\");skipp_logic_question(this,$ans_detail);showReset(this);showQuePipe(this,$piping_sequence);'><li class='md-radio' style='display:inline;background-color: #ebebeb;'><label>
...
if (isset($_REQUEST['btnsend']) && $_REQUEST['btnsend'] != '' && empty($msg1)) {
    $survey_trackdatetime_val = $_REQUEST['survey_trackdatetime_temp'];
    ...
    //Take survey changes 03-08-2019 Submission entry when admin submits the survey
    if (empty($submisstion_id)) {
        $survey_submission = new bc_survey_submission();
        $survey_submission->email_opened = 0;
        $survey_submission->survey_send = 0;
        $survey_submission->name = $moduleBeanObj->name;
        $survey_submission->customer_name = $moduleBeanObj->name;
        $gmtdatetime = TimeDate::getInstance()->nowDb();
        ...
        $survey_submission->parent_type = $moduleBeanObj->module_name;
        $survey_submission->parent_id = $moduleBeanObj->id;
        $survey_submission->target_parent_type = $moduleBeanObj->module_name;
        $survey_submission->target_parent_id = $moduleBeanObj->id;
        $survey_submission->assigned_user_id = $moduleBeanObj->assigned_user_id;
        //  $survey_submission->team_set_id = $team_set_id; // BugFix :: Throws 500 Internal Server Error on Survey Submission for some Teams
        $survey_submission->team_id = '1'; // BugFix :: Throws 500 Internal Server Error on Survey Submission for some Teams $team_id;
        $survey_submission->submission_type = 'Manual';
        $survey_submission->survey_trackdatetime_temp = $survey_trackdatetime_val;
        $survey_submission->survey_trackdatetime = $survey_trackdatetime_val;
...
//                            if ($question_obj->question_type == "multiselectlist" || $question_obj->question_type == "check-box") {
//                                $answer_to_update[$survey_answer_update_module_field_name[$submitted_que]] = $answer_to_update[$survey_answer_update_module_field_name[$submitted_que]] . ',^' . $submitted_ans_obj->piping_selection_type_values . '^';
//                            } else if ($question_obj->question_type == "dropdownlist" || $question_obj->question_type == "radio-button") {
//
//                                $answer_to_update[$survey_answer_update_module_field_name[$submitted_que]] = $answer_to_update[$survey_answer_update_module_field_name[$submitted_que]] . ',' . $submitted_ans_obj->piping_selection_type_values;
//                            } else {
...
//                        } else {
//                            if ($question_obj->question_type == "multiselectlist" || $question_obj->question_type == "check-box") {
//                                //$answer_to_update[$survey_answer_update_module_field_name[$submitted_que]] = '^' . $submitted_ans_obj->piping_selection_type_values . '^';
...
//                    }
                    ...
                    if (array_key_exists($submitted_que, $survey_answer_prefill)) {
                        if (!empty($answer_to_update[$survey_answer_update_module_field_name[$submitted_que]])) {
                            $answer_to_update[$survey_answer_update_module_field_name[$submitted_que]] = $answer_to_update[$survey_answer_update_module_field_name[$submitted_que]] . ',' . '^' . $submitted_ans_obj->answer_name . '^';
                        } else if ($question_obj->question_type == "multiselectlist" || $question_obj->question_type == "check-box") {
                            $answer_to_update[$survey_answer_update_module_field_name[$submitted_que]] = '^' . $submitted_ans_obj->answer_name . '^';
                        } else {
                            $answer_to_update[$survey_answer_update_module_field_name[$submitted_que]] = $submitted_ans_obj->answer_name;
                        }
                    ...
                    if ($question_obj->question_type == 'contact-information') {
                        ...
                        // Added Code To DIsplay Contact Type Question In Dotb Report module. By Biztech *************************************************************//
                        $submitted_ans_obj = new bc_survey_answers();
                        $final_ans_ci = '';
                        ...
                        if (is_array($sub_ans)) {
                            foreach ($sub_ans as $value) {
                                // Added Code To DIsplay Matrix Type Question In Dotb Report module. By Biztech
...
//                    $splitted_values = explode(',', $field_value);
                    $splitted_values = trim($field_value, '^');
                    $splitted_values = explode('^,^', $splitted_values);
                    ...
                    foreach ($find_options_array as $key => $val) {
                        if (in_array($key, $splitted_values)) {
                ...
                // if date field then convert date properly
                    ...
                    $moduleBeanObj->$field_name = $date_only_array[0];
                }
...
//                    $splitted_values = explode(',', $field_value);
                ...
                // if date field then convert date properly
                    ...
                    $date = date_create($field_value);
                    $new_value = date_format($date, "Y-m-d");
//                    $new_date_as_format = $timedate->to_display_date_time($new_value);
//                    echo print_r($new_date_as_format);
//                    $new_date = date_create($new_date_as_format, timezone_open($user->getPreference('timezone', 'global')));
//                    echo print_r($new_date); exit;
//                    $to_db_date_new = $timedate->asDb($new_date);
//                    $date_only_array = explode(' ', $to_db_date_new);
//                    $moduleBeanObjNew->$field_name = $date_only_array[0];
                    $moduleBeanObjNew->$field_name = $new_value;
...
<body style='margin: 0; mso-line-height-rule: exactly;'>
                              ...
                              <td colspan='{$ans_count}' style='margin:0 0 8px 0; padding:5px; font-size:14px; color:#808080; border:1px solid #e5e5e5;background-color: #f3f3f3;'>
                                <table width='100%' cellspacing='0' cellpadding='0' border='1' bgcolor='#fff' style='padding: 0px; margin: 0px; font-family: Calibri,Arial; background-color: #f7fafe; border:1px solid #e5e5e5; border-collapse:collapse; '>
                                    ...
                                    if ($answer != 'is_other_option') {
                                        ...
                                        if ($oQuestionRetrieve->is_image_option == 1) {
                                            if ($submitted_answer != 'N/A') {
                                                $is_image_option_nameQuery = "SELECT radio_image FROM bc_survey_answers WHERE id ='{$key}'";
                                                $is_image_option_nameData = $db->fetchByAssoc($db->query($is_image_option_nameQuery));
                                                $is_image_option = $is_image_option_nameData['radio_image'];
                                                $html .= "<tr>
                                            <td colspan='{$ans_count}' style='margin:0 0 8px 0;padding:5px;font-size:15px;color:#808080;border:1px solid #e5e5e5;background-color:#f3f3f3' width='110'><strong>Answer:</strong></td>
                                                    <td style='margin:0 0 8px 0;padding:5px;font-size:14px;color:#808080;border:1px solid #e5e5e5;background-color:#f3f3f3'><img src='" . $is_image_option . "' style='height:30px;width:30px;'>{$submitted_answer}</td>
                                                        </tr>";
                                            } else {
                                                $html .= "<tr>
                                                    <td colspan='{$ans_count}' style='margin:0 0 8px 0;padding:5px;font-size:15px;color:#808080;border:1px solid #e5e5e5;background-color:#f3f3f3' width='110'><strong>Answer:</strong></td>
        ...
        // $survey_url = $dotb_config['site_url'] . '/survey_re_submit_request.php?survey_id=' . $survey_id . '&ctype=' . $module_type . '&cid=' . $module_id;
        $resubmitHtml = '';
        if ($reSubmitCount > 0) {
...
<html>
    <head>
        ...
        echo $dotb_config['site_url'] . '/custom/include/css/survey_css/' . $survey->survey_theme . '.css';
        ?>" rel="stylesheet">
                        ...
                        $.each(survey_detail, function (pindex, page_data) {
                            $.each(page_data['page_questions'], function (qindex, que_data) {
                                ...
                                if (que_data['que_type'] == 'date-time') {
                                    ...
                                    //for prefilling datetime 15-02-2020
                                    var datetime;
                                    if ($('.setdatetime.' + que_data['que_id'] + '_datetime').val() != "" && typeof $('.setdatetime.' + que_data['que_id'] + '_datetime').val() != "undefined") {
                                        datetime = new Date($('.setdatetime.' + que_data['que_id'] + '_datetime').val());
                                    } else {
                                        datetime = new Date();
                                    }
                                    ...
                                    //for prefilling datetime and allow future date question type 15-02-2020
                                    var datetimeAllowfutureDate;
                                    if ($('.setdatetimeAllowfutureDate.' + que_data['que_id'] + '_datetime').val() != "" && typeof $('.setdatetimeAllowfutureDate.' + que_data['que_id'] + '_datetime').val() != "undefined") {
                                        datetimeAllowfutureDate = new Date($('.setdatetimeAllowfutureDate.' + que_data['que_id'] + '_datetime').val());
                                    } else {
                                        datetimeAllowfutureDate = new Date();
                                    }
                                    ...
                                    $('.setdatetimeAllowfutureDate.' + que_data['que_id'] + '_datetime').click(function (el) {
                                        ...
                                        minYear: minYear,
                                        maxYear: maxYear,
                                        startDate: datetimeAllowfutureDate,
                                    });
                                    ...
                                    //for prefilling date 15-02-2020
                                    var date;
                                    if ($('.setdate.' + que_data['que_id'] + '_datetime').val() != "" && typeof $('.setdate.' + que_data['que_id'] + '_datetime').val() != "undefined") {
                                        date = new Date($('.setdate.' + que_data['que_id'] + '_datetime').val());
                                    } else {
                                        date = new Date();
                                    }
                                    ...
                                    //for prefilling datetime and allow future date question type 15-02-2020
                                    var dateAllowfutureDate;
                                    if ($('.setdateAllowfutureDate.' + que_data['que_id'] + '_datetime').val() != "" && typeof $('.setdateAllowfutureDate.' + que_data['que_id'] + '_datetime').val() != "undefined") {
                                        dateAllowfutureDate = new Date($('.setdateAllowfutureDate.' + que_data['que_id'] + '_datetime').val());
                                    } else {
                                        dateAllowfutureDate = new Date();
                                    }
                            ...
                            $.each(slider_detail, function (qid, slider_data) {
                                ...
                                var slider = $('.' + qid).find("#slider").slider({
                                    slide: function (event, ui) {
                                        ...
                                        var piping_sequence = $(ui["handle"]).parent().parents('.scale').find('[name=current_piping_sequence]').val();
                                        showQuePipe($(ui["handle"]).parent(), piping_sequence);
                                        showReset($(ui.handle));
                                    },
                                    range: "min",
                                    value: answer,
...
//                    var split_selected_id = selected_id.split("_");
            ...
            function skipp_logic_question(el, answers) {
                ...
                if (question_type == "netpromoterscore") {
                    if ($(el).prop('id') == "reloadScale") {
                        var npsQuery = $(el).parent().parent().find('.nps_submission_table').find('div.score_pannel')
                    } else {
                        var npsQuery = $(el).parents('.nps_submission_table').find('div.score_pannel')
                    }
                    $.each(npsQuery, function () {
                ...
                if (question_type == "boolean") {
                    ...
                    if ($(el).prop('id') == "reloadScale") {
                        var npsQuery = $(el).parent().parent().find('input[type=radio]')
                    } else {
                        var npsQuery = $(el).parent().parent().parent().find('input[type=radio]')
                    }
                    $.each(npsQuery, function () {
                        $.each(answers[this.value], function (action, target) {
                            if (action == "show_hide_question") {
                    ...
                    if ($(el).prop('id') == "reloadScale") {
                        var npsQuery = $(el).parent().parent().find('.emojis_class').find('input[type=radio]')
                    } else {
                        var npsQuery = $(el).parents('.emojis_class').find('input[type=radio]')
                    }
                    $.each(npsQuery, function () {
                        $.each(answers[this.value], function (action, target) {
                            if (action == "show_hide_question") {
            ...
            function addOtherField(el) {
                ...
                if ($(el).val() && $(el).val() != 'selection_default_value_dropdown')
                {
            ...
            function validateUploadedFile(evt, fileSize, fileExtension) {
                ...
                if ($.inArray(file_ext, valid_fileext) == '-1') {
                    ...
                    $(evt).parents('.doc-attachment').find('.val-msg-upload').html('<p>You have uploaded Invalid file.</p><div style="display: inline-block;float: right;"><img class="questionImgIcon" style="top:initial" onmouseout="removeHelpTipPopUpDiv();" onmouseover="openFileUploadPopUpSurvey(this,\'' + fileExtension + '\' );" src="custom/include/survey-img/question.png" ></div>');
                ...
                if (file_size_validated) {
                    ...
                    switch (ext.toLowerCase()) {
                        ...
                        default:
                            evt.value = '';
                            $(evt).parents('.doc-attachment').find('.file_uploaded').find('.imgcontent').html('');
                            $(evt).parents('.doc-attachment').find('.val-msg-upload').html('<span>You have uploaded Invalid file.</span><div style="display: inline-block;float: right;"><img class="questionImgIcon" style="top:initial" onmouseout="removeHelpTipPopUpDiv();" onmouseover="openFileUploadPopUpSurvey(this,\'' + fileExtension + '\' );"  src="custom/include/survey-img/question.png" ></div>');
//                            $(evt).parents('.doc-attachment').find('.val-msg-upload').css('display', 'inline-block');
                    ...
                    $(function () {
                        ...
                        var slider = $('.' + question_id).find("#slider").slider({
                            ...
                            create: function (event, ui) {
                                ...
                                $(el).parent().find('.ui-slider-handle').find('.tooltip-score').find('div').remove();
                            },
                            change: function (event, ui) {
                                $('.' + question_id + '_scale').val(ui.value);
                                $(ui.handle).find('.tooltip-score').html('<div>' + ui.value + '</div>');
...
//                            change: function (event, ui) {
//                                $('.' + question_id + '_scale').val("");
//                                $('#hidden').attr('value', "");
//                            }
                        });
                        $(el).parent().parent().find('.ui-slider-handle').find('.tooltip-score').find('div').remove();
                        $('.' + question_id + '_scale').val("");
                        var piping_sequence = $(el).parent().parent().find('.ui-slider-handle').parents('.scale').find('[name=current_piping_sequence]').val();
                        showQuePipe($(el).parent().parent().find('.ui-slider-handle').parents('#slider'), piping_sequence);
                    ...
                    if (parseInt(numberSelected) >= 0 && parseInt(numberSelected) < 7) {
                        ...
                        $(el).parent().parent().find('#previous_nps_selected_id_hidden_' + question_id).val("");
                ...
                var piping_sequence = $(el).parents('.' + question_type).find('[name=current_piping_sequence]').val();
                var que_id = $(el).parents('.' + question_type).find('.questionHiddenField').val();
                showQuePipe($(el).parents('.' + question_type).find('#' + que_id + '_div'), piping_sequence);
            }
            function showReset(el) {
                $(el).parents('.form-body').find('#reloadScale').show();
            }
            function showQuePipe(el, piping_sequence) {
                var que_piping_array = '<?php echo json_encode($que_piping_array); ?>';
                var qpiping = JSON.parse(que_piping_array);
                if (que_piping_array != '[]') {
                    var que_type = $(el).parents('.form-body').attr('class').split(' ')[1];
                    if (que_type == 'textbox' || que_type == 'commentbox') {
                        var current_answer = $(el).val();
                        ...
                        $('.que_pipe_' + piping_sequence).html(current_answer);
                    } else if (que_type == 'radio-button') {
                        var current_answer = $(el).parent().find('.option_label').html();
                        current_answer = current_answer.replace(/&/g, '&amp;');
                        current_answer = current_answer.replace(/</g, '&lt;');
                        current_answer = current_answer.replace(/>/g, '&gt;');
                        if (!current_answer) {
                        ...
                        if ($(el).parent().find('input[type=radio]:checked').length == 0) {
                            current_answer = " ";
                        }
                        $('.que_pipe_' + piping_sequence).html(current_answer);
                    } else if (que_type == 'dropdownlist') {
                        var value = $(el).val();
                        var current_answer = $(el).find('[value=' + value + ']').html();
                        if (!current_answer || value == 'selection_default_value_dropdown') {
                            current_answer = " ";
                        }
                        $('.que_pipe_' + piping_sequence).html(current_answer);
                    } else if (que_type == 'netpromoterscore') {
                        var current_answer = $(el).attr('value');
                        if (!current_answer) {
                            current_answer = " ";
                        }
                        $('.que_pipe_' + piping_sequence).html(current_answer);
                    } else if (que_type == 'emojis') {
                        var current_answer = $(el).find('.emoji_text').html();
                        current_answer = current_answer.replace(/&/g, '&amp;');
                        current_answer = current_answer.replace(/</g, '&lt;');
                        current_answer = current_answer.replace(/>/g, '&gt;');
                        if ($(el).parent().find('input[type=radio]:checked').length == 0) {
                        ...
                        $('.que_pipe_' + piping_sequence).html(current_answer);
                    } else if (que_type == 'scale' || que_type == 'rating') {
                        var que_id = $(el).parents('.' + que_type).find('.questionHiddenField').val();
                        var current_answer = $(el).parents('.' + que_type).find('#' + que_id + '_hidden').val();
                        if (!current_answer) {
                            current_answer = " ";
                        }
                        $('.que_pipe_' + piping_sequence).html(current_answer);
    ...
    <div id="overlay" class="overlay">
        <div id="formProcessLoader" align="center"><img src="themes/default/images/sqsWait.gif" alt="" ><b>Please wait...</b></div>
        ...
        if ($survey->survey_theme == 'theme0') {
            ...
            <div id="dotbcrm">
                <div id="lumia">
                    ...
                    if ($survey->survey_theme == 'theme0') {
                        ?>
                        <div id="header dateCss" style="padding: 80px 0 0 0;">
                        <?php } else { ?>
                            <div id="header">
                            <?php } ?>
                            ...
                            <div class="navbar">
                                <div class="navbar-inner">
                                    <div class="nav-collapse" style="padding:10px">
                                        ...
                                        <img src="<?php echo $company_logo; ?>" alt="DotbCRM" style="height:27px;">
                        ...
                        <?php } ?>
                        <div class="bg"></div>
                        <!-- After survey submitted Language option should not come 13 June 2019 -->
                        <?php if (isset($available_lang) && count($available_lang) != 0 && !isset($_REQUEST['btnsend']) && $languageShow == 1) {
                            ?>
                            ...
                            </div><input type="hidden" value="<?php echo $selected_lang; ?>" id="selectedLanguage"/>
                        <?php } ?>
                        <?php
                        if ($survey->survey_theme == 'theme0') {
                            ?>
                            <div class="main-container datepickerForm">
                            <?php } else { ?>
                                ...
                                <form method="post" name="survey_submisssion" action="" id="survey_submisssion" enctype="multipart/form-data">
                                    ...
                                    if (!isset($_REQUEST['btnsend'])) {
                                        ?>
                                        <input type="hidden" value="<?php echo $survey_trackdatetime_temp; ?>" name="survey_trackdatetime_temp" id="survey_trackdatetime_temp"/>
                                    <?php } ?>
                                        ...
                                        if (isset($msg1) && $msg1 != '') {
                                            ...
                                            <div class="container">
                                                ...
                                                <ul class="bxslider">
                                                    ...
                                                    foreach ($survey_details as $page_sequence => $detail) {
                                                        $queArraylist[$page_sequence] = getSubmittedAnswerByReciever($survey_id, $module_id, $submission_id);
                                                        ?>
                                                        <li>
                                                            <div class="survey-form">
                                                                ...
                                                                foreach ($detail['page_questions'] as $que_sequence => $question) {
                                                                    ...
                                                                    if ($question['disabled_question'] != 1) {
                                                                        ?>
                                                                        <!--  IMP Note :: If wanna Add new class in form-body then add it after que_type class, its being used in sequence manner  -->
                                                                        <div class="form-body <?php echo $question['que_type']; ?>" style="<?php echo $display_qes; ?>;margin-top: 14px;<?php echo $separator_css; ?>">
                                                                            ...
                                                                            if (!empty($answer)) {
                                                                                $getAnsQry = "SELECT answer_name FROM bc_survey_answers WHERE id='$answer' ";
                                                                                $resultQry = $db->query($getAnsQry);
                                                                                if ($resultQry->num_rows != 0) {
                                                                                    $option_label = $db->fetchByAssoc($resultQry)['answer_name'];
                                                                                    // Replace Answer Piping Text with Actual answer submitted
                                                                                    if (in_array($question['que_type'], $QType_allowedforquestionpiping)) {
                                                                                        // Replace Piping string with _blank_
                                                                                        $fullstring = $option_label;
                                                                                        $occu = substr_count($fullstring, "{{Q");
                                                                                        $piping_seq = array();
                                                                                        for ($i = 0; $i < $occu; $i++) {
                                                                                            $sub_part = explode('{{Q', $fullstring)[1];
                                                                                            $current_piping_seq = explode('}}', $sub_part)[0];
                                                                                            $piping_seq .= explode('}}', $sub_part)[0] . ' ';
                                                                                            $fullstring = str_replace('{{Q' . trim($current_piping_seq) . '}}', '<span class="que_pipe_' . $current_piping_seq . '"> ' . $piping_wise_answers[$current_piping_seq] . ' </span>', $fullstring);
                                                                                        }
                                                                                        $option_label = $fullstring;
                                                                                    }
                                                                                } else {
                                                                                    $option_label = $answer;
                                                                                }
                                                                                ...
                                                                                if (in_array($question['que_type'], $QType_allowedforanswerpiping)) {
                                                                                    $piping_wise_answers[$question['piping_sequence']] = $option_label;
                                                                                }
                                                                            }
                                                                            ...
                                                                            if ($question['que_type'] == 'section-header') {
                                                                                ...
                                                                                <h3 class="questions <?php echo $que_class ?>">
                                                                                    ...
                                                                                    if ($question['que_type'] == 'image' || $question['que_type'] == 'video') {
                                                                                        ...
                                                                                        echo $que_no;
                                                                                        ?> .&nbsp; <?php
                                                                                        // Replace Que Title with given answer
                                                                                        if (!empty($answer)) {
                                                                                            // Replace Answer Piping Text with Actual answer submitted
                                                                                    ...
                                                                                    if ($question['que_type'] == 'image' || $question['que_type'] == 'video') {
                                                                                        ...
                                                                                        ?> <div style="display: inline-block;float: right !important; position:absolute;<?php echo $extracss; ?>"><img alt="" class="questionImgIcon" onmouseout="removeHelpTipPopUpDiv();" onmouseover="openHelpTipsPopUpSurvey(this, '<?php echo $question['question_help_comment']; ?>');" src="custom/include/survey-img/question.png" ></div>
                                                                                    <?php } ?>
                                                                                    <?php if ($question['que_type'] == 'scale' || $question['que_type'] == 'rating' || $question['que_type'] == "radio-button" || ($question['que_type'] == "matrix" && $question['advance_type'] == "radio") || $question['que_type'] == "netpromoterscore" || $question['que_type'] == "emojis") { ?>    
                                                                                        <?php if ($question['is_skip_logic'] == 1) { ?>    
                                                                                            <div style="display:none;" class="btn btn-primary" data-queid="<?php echo $question['que_id']; ?>" data-button-clicked="Reset Slider" id="reloadScale" title="Reset Answer" onclick='clearSelection(this, "<?php echo $question['que_id']; ?>", "<?php echo $question['que_type']; ?>");
                                                                                                                                skipp_logic_question(this,<?php echo json_encode($skip_logicArrForAll); ?>);' ><i><img src="custom/include/images/reset.png" alt=""></i></div> 
                                                                                             <?php } else { ?> 
                                                                                            <div style="display:none;" class="btn btn-primary" data-queid="<?php echo $question['que_id']; ?>" data-button-clicked="Reset Slider" id="reloadScale" title="Reset Answer" onclick="clearSelection(this, '<?php echo $question['que_id']; ?>', '<?php echo $question['que_type']; ?>');"><i><img src="custom/include/images/reset.png" alt=""></i></div> 
                                                                                        <?php } ?>
                                                                                            <?php } ?></h3>
                                                                                        <?php
                                                                                        $richtextContent = '';
                                                                                    ...
                                                                                    $question['piping_wise_answers'] = $piping_wise_answers;
                                                                                    $elementHTML = getMultiselectHTML($skip_logicArrForAll, $question, $answer, $survey->survey_theme, $list_lang_detail, $survey_answer_prefill, $richtextContent, $survey_id);
                                                                                    echo $elementHTML;
                                                                                    ?>
                                                                        </div>
                                                                    <?php } ?>
