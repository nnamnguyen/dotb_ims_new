 ...
 */
class SupportPortalVisibility extends DotbVisibility
{
    protected $wherePart = '';
     ...
     *
     * @param DotbQuery $dotbQuery
     * @param array      $options
     *
     * @return DotbQuery
     */
    public function addVisibilityFromQuery(DotbQuery $dotbQuery, $options = array())
    {
        if (empty($_SESSION['type']) || $_SESSION['type'] != 'support_portal') {
            $GLOBALS['log']->error('Not a portal user, but running through the portal visibility class.');
            return $dotbQuery;
        ...
        switch ($this->bean->module_dir) {
            case 'Contacts':
                ...
                if (count($accountIds) || empty($_SESSION['contact_id'])) {
                    $this->bean->load_relationship('accounts');
                    $joins = $this->bean->accounts->buildJoinDotbQuery(
                        $dotbQuery,
            ...
            case 'Notes':
                ...
                if (!empty($accountIds)) {
                    $this->bean->load_relationship('cases');
                    $join = $this->bean->cases->buildJoinDotbQuery(
                        $dotbQuery,
                    ...
                    $caseBean = BeanFactory::newBean('Cases');
                    $caseBean->load_relationship('accounts');
                    $join = $caseBean->accounts->buildJoinDotbQuery(
                        $dotbQuery,
                ...
                $join = $this->bean->bugs->buildJoinDotbQuery(
                    $dotbQuery,
            ...
            case 'Cases':
                $this->bean->load_relationship('accounts');
                $join = $this->bean->accounts->buildJoinDotbQuery(
                    $dotbQuery,
                    array('joinTableAlias' => 'accounts_pv', 'ignoreRole' => true)
                );
