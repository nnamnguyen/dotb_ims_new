...
class DotbACLUsers extends DotbACLStrategy
{
    /**
    ...
    public function checkAccess($module, $view, $context)
    {
        ...
        if ($view == 'export' && (!empty($dotb_config['disable_export'])
        ...
        foreach($field_list as $key => $field) {
            if($myself == true) {
                if($field == 'status' || $field == 'employee_status') {
                    $result[$key] = DotbACL::ACL_READ_ONLY;
                }
            }

            if($is_admin === true) {
                $result[$key] = DotbACL::ACL_READ_WRITE;
            } else {
                if(($field == 'user_hash' || $field == 'password') && !$myself) {
                    $result[$key] = DotbACL::ACL_NO_ACCESS;
