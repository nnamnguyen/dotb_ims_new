 ...
 * @api
 */
class M2MRelationship extends DotbRelationship
{
    ...
    {
        if(!($lhs instanceof DotbBean) || !($rhs instanceof DotbBean)) {
    ...
    {
        $result = $this->getDotbQuery($link, $params)->execute();
    ...
    /**
     * Get DotbQuery for loading relationship records
     *
     * @param Link2 $link
     * @param array $params
     *
     ...
     * @throws \Exception
     */
    protected function getDotbQuery(Link2 $link, array $params = array())
    {
        if ($this->linkIsLHS($link)) {
            $knownKey = $this->def['join_key_lhs'];
            $targetKey = $this->def['join_key_rhs'];
        ...
        $this->buildDotbQueryRoleWhere($query, $rel_table);
    ...
    /**
     * Build a Join using an existing DotbQuery Object
     * @param Link2 $link
     ...
     */
    public function buildJoinDotbQuery(Link2 $link, $dotb_query, $options)
    {
        $linkIsLHS = $link->getSide() == REL_LHS;
        ...
        $relTableJoin = $dotb_query->joinTable($joinTable, array('alias'=>$joinTable_alias, 'joinType' => $join_type, 'linkingTable' => true,))
            ->on()->equalsField("{$startingTable}.{$startingKey}","{$joinTable_alias}.{$startingJoinKey}")
            ->equals("{$joinTable_alias}.deleted","0");

        $targetTableJoin = $dotb_query->joinTable($targetTable, array(
        ...
        $dotb_query->join[$targetTable_alias]->relationshipTableAlias = $joinTable_alias;

        if (empty($options['ignoreRole'])) {
            $this->buildDotbQueryRoleWhere($dotb_query, $joinTable_alias);
        }

        $this->addCustomToDotbQuery($dotb_query, $options, $linkIsLHS, $targetTable_alias);

        return array($joinTable_alias => $relTableJoin, $targetTable_alias => $targetTableJoin);
    }

    protected function addCustomToDotbQuery($dotb_query, $options, $linkIsLHS, $targetTable_alias ) {
        if (!empty($options['includeCustom'])) {
            ...
            if (!empty($bean) && $bean->hasCustomFields()) {
                ...
                $dotb_query->joinTable($table_cstm, array('alias' => $alias_cstm, 'joinType' => "LEFT", 'linkingTable' => true))
