 ...
 * @api
 */
abstract class DotbRelationship
{
    protected $def;
    protected $lhsLink;
    protected $rhsLink;
     ...
     * See Link2->getQuery
     *
     * @deprecated Use DotbRelationship::load() instead
     * @abstract
     *
     ...
     * @return DotbQuery
     */
    public abstract function buildJoinDotbQuery(
        Link2 $link,
        $dotb_query,
        $options
    );
    ...
    /**
     * @param $rowID id of DotbBean to remove from the relationship
     *
     * @return void
     */
     ...
     * @return DotbQuery
     */
    protected function buildDotbQueryRoleWhere(
        $dotb_query,
        $table = "",
        ...
        if (!$ignore_role_filter && !empty($table)) {
            foreach ($this->getRelationshipRoleColumns() as $column => $value) {
                ...
                $field = "$table.$column";
                if (empty($value)) {
                    if (isset($dotb_query->join[$table])) {
                        $dotb_query->join[$table]->on()->isNull($field);
                    } else {
                        $dotb_query->where()->isNull($field);
                    }
                } else {
                    if (isset($dotb_query->join[$table])) { // i.e. Accounts joining Notes
                        $dotb_query->join[$table]->on()->equals($field, $value);
                    } else { // i.e. Notes joining Accounts
                        $dotb_query->where()->equals($field, $value);
                    }
                }
            }
        }
        ...
        if (!empty($this->def['primary_flag_column'])
            ...
            $dotb_query->join[$table]->on()->equals($field, '1');
        }
    ...
    /**
     * @param DotbBean $focus base bean the hooks is triggered from
     * @param DotbBean $related bean being added/removed/updated from relationship
     ...
     * @param DotbBean $relatedBean The related bean
     ...
     * @param DotbBean $relatedBean
     * @return string
     */
    protected function getOptionalWhereClause($optional_array, $whereTable = '', DotbBean $relatedBean = null)
    {
    ...
    /**
     * Adds optional where clause to the dotb query
     *
    ...
    {
        if (DotbBean::inOperation('updating_relationships') || !DotbBean::enterOperation('saving_related')) {
            return;
        }
        ...
        DotbBean::leaveOperation('saving_related');

        //Reset the queue index
        self::$resaveIndex = array();

        DotbBean::clearRecursiveResave();
        return true;
    }
