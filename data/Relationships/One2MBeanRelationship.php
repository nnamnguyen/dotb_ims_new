...
{
    //Type is read in dotbbean to determine query construction
    ...
    public function add($lhs, $rhs, $additionalFields = array())
    {
        ...
        // the save and such as it causes problems with the related() in dotblogic
        ...
        if (!$lhs->inOperation('saving_related')) {
            DotbRelationship::resaveRelatedBeans(false);
        }
    ...
    /**
     * Build a Join Query with a DotbQuery Object
     *
     * @param Link2 $link
    ...
    public function buildJoinDotbQuery(Link2 $link, $dotb_query, $options)
    {
        ...
        $joinTable = $dotb_query->joinTable($targetTable, $joinParams);
        ...
        $this->addCustomToDotbQuery($dotb_query, $options, $linkIsLHS, $jta);

        return $dotb_query->join[$jta];
    }
     ...
     *
     * @param DotbBean $lhs        Left hand side of the relationship
     * @param DotbBean $rhs        Right hand side of the relationship
