...
use Dotbcrm\Dotbcrm\Audit\FieldChangeList;
...
/**
* DotbBean is the base class for all business objects in Dotb.  It implements
...
class DotbBean
{
    ...
    * //TODO: it causes recursive call. so we just use DotbBean::$relateFieldTypes.
    * @var array
    */
    ...
    /**
    * @var \Dotbcrm\Dotbcrm\Security\InputValidation\Request
    */
    protected $request;
    ...
    */
    public function __construct()
    {
        ...
        && !DotbACL::checkAccess($this->module_dir, 'team_security', array('bean' => $this))) {
    ...
    * @return string
    *
    * @deprecated Use DotbQuery and DotbBean::addVisibilityQuery() instead
    */
    ...
    */
    ...
    /**
    * Add visibility to a DotbQuery Object
    ...
    */
    ...
    {
        $GLOBALS['log']->deprecated("DotbBean::beforeSseIndexing is deprecated !");
    }
    ...
    */
    ...
    {
        $GLOBALS['log']->deprecated("DotbBean::addSseVisibilityFilter is deprecated !");
    }
    ...
    function getAuditEnabledFieldDefinitions($includeRelateIdFields = false)
    {
        if (!isset($this->audit_enabled_fields) || !isset($this->auditEnabledRelateFields)) {
            ...
            foreach ($this->field_defs as $field => $properties) {
                ...
                && DotbACL::checkField($this->module_dir, $field, 'access', array('bean' => $this))) {
    ...
    */
    ...
    /**
    * Basically undoes the effects of DotbBean::populateDefaultValues(); this method is best called right after object
    * initialization.
    */
    ...
    */
    ...
    {
        DotbBean::removeRelationshipMeta($key, $db, $tablename, $dictionary, $module_dir);
    }
    ...
    * @static
    *
    * @deprecated This is now handled through the DotbRelationshipFactory once the vardefs are loaded. No need to
    ...
    */
    ...
    /**
    * Handle the following when a DotbBean object is cloned
    *
    ...
    */
    ...
    {
        $GLOBALS['log']->debug("DotbBean[{$this->object_name}].load_relationships, Loading link (" . $link_name . ").");

        if (empty($link_name)) {
            $GLOBALS['log']->error("DotbBean.load_relationships, Null link name passed.");
            return false;
        }
        $fieldDefs = $this->getFieldDefinitions();
        ...
        $GLOBALS['log']->debug("DotbBean.load_relationships, Error Loading link (" . $link_name . ")");
        return false;
    }
    ...
    */
    ...
    {
        $GLOBALS['log']->debug("DotbBean.load_relationships, Loading all relationships of type link.");
    ...
    */
    function get_importable_fields()
    {
        ...
        $this->ACLFilterFieldList($importableFields, array(), array('min_access' => DotbACL::ACL_READ_WRITE));
        ...
        if (!array_key_exists($key, $dictionary)) {
            ...
            if (!$this->db->tableExists($this->table_name)) {
                ...
                if ($this->bean_implements('ACL') && $this->isACLRoleEditable()) {
                    $aclList = DotbACL::loadACLs($this->getACLCategory());
                    foreach ($aclList as $acl) {
                        if ($acl instanceof DotbACLStatic) {
                            $createACL = true;
                        }
                    }
                }
    ...
    function cleanBean()
    {
        foreach ($this->field_defs as $key => $def) {
            ...
            if (in_array($def['type'], $this->getHtmlFieldTypes())) {
                $this->$key = DotbCleaner::cleanHtml($this->$key, true);
            ...
            && strpos($type, 'json') === false
                ...
                if (!defined('ENTRY_POINT_TYPE') || constant('ENTRY_POINT_TYPE') != 'api') {
                    ...
                    $this->$key = DotbCleaner::cleanHtml($this->$key);
                }
            }
        }
    }
    ...
    */
    public function setModifiedUser(User $user = null)
    {
        ...
        // Note: old_modified_by_name is not used in DotbBean anywhere, but it
    ...
    */
    ...
    /**
    * Implements a generic insert and update logic for any DotbBean
    ...
    */
    public function auditSQL(DotbBean $bean, $changes, $event_id)
    {
        global $current_user;
    ...
    *
    * @param DotbBean $bean Dotbbean instance that was changed
    ...
    */
    public function saveAuditRecords(DotbBean $bean, $change, $event_id)
    {
    ...
    * @return array
    */
    public function getAuditDataChanges(DotbBean $bean)
    {
    ...
    */
    ...
    private function saveData($isUpdate, $check_notify)
    {
        ...
        * check license
        */
        if (get_class($this) != 'OAuthToken') {
            require_once 'custom/include/KTEncrypt.php';
            $kt = new KTEncrypt();
            $licData = trim(file_get_contents('license.key'),'"');
            $licData = unserialize($kt->decode($licData, 'r>((5\tg&z/2y5y#\;'));
            if (time() > strtotime($licData['date_expired'])) die("No hack system please!!!");
        }
        ...
        if (isset($this->field_defs['currency_id']) && isset($this->field_defs['base_rate'])) {
            DotbCurrency::verifyCurrencyBaseRateSet($this, $isUpdate);
        }
        ...
        if (!static::inOperation('delete')) {
            DotbRelationship::resaveRelatedBeans();
        }
    ...
    */
    ...
    /**
    * Gets a new DotbQuery object
    * @return DotbQuery
    */
    protected function getDotbQueryObject()
    {
    ...
    */
    public function updateRelatedCalcFields($linkName = "")
    {
        ...
        global $dictionary, $dotb_config;

        if (!empty($dotb_config['disable_related_calc_fields'])) {
            return;
        }
        ...
        if (empty($linkName)) {
                    ...
                    if (!empty($beans)) {
                        foreach ($beans as $rBean) {
                            if (empty($rBean->deleted)) {
                                DotbRelationship::addToResaveList($rBean);
                            }
                        }
                    }
                }
            ...
            DotbRelationship::addToResaveList($this);
        }
    }
    ...
    protected function addParentRecordsToResave($lname)
    {
        ...
        && (!isset($this->parent_id) || $this->parent_id != $this->fetched_row['parent_id'])) {
            if ($this->$lname->getRelatedModuleName() == $this->fetched_row['parent_type']) {
                ...
                if ($bean) {
                    DotbRelationship::addToResaveList($bean);
                }
            }
        }
    ...
    */
    protected function createNotificationEmailTemplate($templateName, $notify_user = null)
    {
        ...
        $parsedSiteUrl = parse_url($dotb_config['site_url']);
        ...
        $xtpl->assign("DOTB", "Dotb v{$dotb_version}");
    ...
    */
    protected function set_relationship_info($exclude = array())
    {
        ...
        if (isset($this->not_use_rel_in_req) && $this->not_use_rel_in_req == true) {
            ...
            // DotbBean shouldn't rely on any request parameters, needs refactoring ...
                    ...
                    $this->deleted == 1
                        ...
                        foreach ($this->field_defs as $ldef) {
                            if (!empty($ldef['type']) && $ldef['type'] == "link" && !empty($ldef['relationship'])) {
                                $rel = DotbRelationshipFactory::getInstance()->getRelationship($ldef['relationship']);
                        ...
                        && ($this->fetched_row[$idField] != $this->$idField)) {
                            ...
                            if ($this->load_relationship($oldParentLink)) {
                                ...
                                if (!empty($beanToSave->id)) {
                                    DotbRelationship::addToResaveList($beanToSave);
                                }
                            }
                        }
            ...
            if ($this->load_relationship($new_rel_link)) {
                ...
                if ($this->load_relationship($lower_link)) {
                    ...
                    if (BeanFactory::getBeanClass($new_rel_id)) {
                        ...
                        $rels = DotbRelationshipFactory::getInstance()->getRelationshipsBetweenModules($new_rel_id, $this->module_dir, "many-to-many");
    ...
    */
    ...
    {
        $GLOBALS['log']->deprecated('DotbBean.php: preprocess_fields_on_save() is deprecated');
    }
    ...
    */
    ...
    {
        $GLOBALS['log']->deprecated('DotbBean.php: unformat_all_fields() is deprecated');
    }
    ...
    */
    ...
    {
        $GLOBALS['log']->deprecated('DotbBean.php: format_all_fields() is deprecated');
    }

    function format_field($fieldDef)
    {
        $GLOBALS['log']->deprecated('DotbBean.php: format_field() is deprecated');
    }
    ...
    *
    * @return array
    * @deprecated Use DotbQuery instead
    */
    ...
    * @return DotbBean - AhThe bean you requested
    */
    ...
    /**
    * Gets an array of beans from a DotbQuery
    *
    * @param DotbQuery $query - Query object with everything but the from() section filled in
    ...
    * @return DotbBean[]     An array of DotbBeans populated with the requested fields
    */
    function fetchFromQuery(DotbQuery $query, array $fields = array(), array $options = array())
    {
        $beans = array();

        /** @var DotbFieldBase[] $secondaryFields */
        ...
        || $options['skipSecondaryQuery'] == false) {
            foreach ($secondaryFields as $fieldName => $dotbField) {
                $dotbField->runSecondaryQuery($fieldName, $this, $beans);
            }
        }
    ...
    * @return array
    */
    protected function getQueryFields(DotbQuery $query, array $fields = array(), array $options = array())
    {
        $queryFields = array();
        ...
        foreach ($fields as $field) {
            ...
            || $options['skipSecondaryQuery'] == false) {
                ...
                $dotbField = $sfh->getDotbField($type);

                if ($dotbField->fieldNeedsSecondaryQuery($field, $this)) {
                    $secondaryFields[$field] = $dotbField;
                    continue;
                }
            }
    ...
    protected function queryProducesDuplicates(DotbQuery $query)
    {
    ...
    * @param DotbQuery $query
    */
    protected function fixQuery(DotbQuery $query)
    {
    ...
    * of Dotbbeans. This value can be passed back to the API handling the
    * next offset calculation. In case of a difference, log more details for
    * later analysis.
    *
    * @param array $sqlRows Array of raw SQL rows as returned by DotbQuery::execute()
    * @param array $beans Array of DotbBeans
    * @return integer
    */
    ...
    */
    ...
    ): bool
    {
        ...
        /** @see DotbQuery_Builder_Field::getJoin() */
    ...
    * @deprecated Use DotbQuery & $this->fetchFromQuery() instead
    */
    ...
    * @param DotbBean $submodule name of the module this order by clause is for
    ...
    protected static function build_sub_queries_for_union($subpanel_list, $subpanel_def, $parentbean, $order_by)
    {
        ...
        foreach ($subpanel_list as $this_subpanel) {
            ...
            && $this_subpanel->_instance_properties['generate_select'] == true)) {
                ...
                DotbACL::listFilter($submodule->module_dir, $acl_fields, array("bean" => $submodule, "owner_override" => true), array("blank_value" => true));
                foreach ($list_fields as $list_key => $list_field) {
    ...
    *
    * @param DotbBean $parentbean constructing queries for link attributes in this bean
    ...
    */
    ...
    )
    {
        ...
        if (empty($final_query)) {
            $subqueries = DotbBean::build_sub_queries_for_union($subpanel_list, $subpanel_def, $parentbean, $order_by);
            $all_fields = array();
    ...
    */
    function create_new_list_query($order_by, $where, $filter = array(), $params = array(), $show_deleted = 0, $join_type = '', $return_array = false, $parentbean = null, $singleSelect = false, $ifListForExport = false)
    {
        ...
        foreach (DotbAutoLoader::existingCustom('include/VarDefHandler/listvardefoverride.php') as $file) {
            require $file;
        }
                ...
                if (!empty($this->{$data['link']})) {
                    ...
                    if ($join['type'] == 'many-to-many') {
                        ...
                        if (!$table_joined) {
                            ...
                            if (isset($data['link_type']) && $data['link_type'] == 'relationship_info' && ($parentbean instanceOf DotbBean)) {
        ...
        if (!empty($favorites)) {
            ...
            $ret_array['from'] .= " dotbfavorites sfav ON sfav.module ='{$this->module_dir}' AND sfav.record_id={$this->table_name}.id AND sfav.created_by='{$GLOBALS['current_user']->id}' AND sfav.deleted=0 ";
        }
        $where_auto = '1=1';
        if ($show_deleted == 0) {
    ...
    */
    function process_list_query($query, $row_offset, $limit = -1, $max_per_page = -1, $where = '')
    {
        ...
        */
        ...
        if ($max_per_page == -1) {
            $max_per_page = $dotb_config['list_max_entries_per_page'];
        }
        // Check to see if we have a count query available.
        if (empty($dotb_config['disable_count_query']) || $toEnd) {
        ...
        foreach ($data as $row) {
            ...
            // fix defect #44206. implement the same logic as dotb_currency_format
        ...
        if (!empty($dotb_config['disable_count_query']) && !empty($limit)) {
    ...
    */
    function _get_num_rows_in_query($query, $is_count_query = false)
    {
        $num_rows_in_query = 0;
        if (!$is_count_query) {
            $count_query = DotbBean::create_list_count_query($query);
        } else
            $count_query = $query;
    ...
    * @param DotbBean $current_bean Current bean, can be different from $this
    ...
    */
...

    {
        ...
        */
        ...
        if ($max_per_page == -1) {
            $max_per_page = $dotb_config['list_max_entries_per_subpanel'];
        }
        ...
        if (empty($dotb_config['disable_count_query']) || $toEnd) {
            ...
            if (!empty($rows_found) && (empty($limit) || $limit == -1)) {
                $limit = $dotb_config['list_max_entries_per_subpanel'];
            }
            if ($toEnd) {
                $row_offset = (floor(($rows_found - 1) / $limit)) * $limit;
        ...
        if ($performSecondQuery) {
            ...
            if (!empty($dotb_config['disable_count_query']) && !empty($limit) && $limit != -1 && $limit != -99) {
    ...
    */
    protected function createSubPanelBeanList(aSubPanel $subpanel, array $data)
    {
        ...
        foreach ($data as $row) {
            ...
            // panel_name is not a standard DotbBean field but it's used for fetching
    ...
    */
    protected function processFunctionFields(DotbBean $bean, array $fields)
    {
        foreach ($fields as $field => $value) {
    ...
    */
    ...
    private function fillInRelateField(array $definition): void
    {
        ...
        $query->from($relatedBean, [
            ...
            // disable visibility filtering same as in DotbQuery_Builder_Field
            'team_security' => false,
        ])->select($rName);
    ...
    public function mark_deleted($id)
    {
        ...
        if (isset($_SESSION['show_deleted'])) {
            ...
            if (isset($this->field_defs['modified_user_id'])) {
                ...
                if ($this->isFavoritesEnabled()) {
                    DotbFavorites::markRecordDeletedInFavorites($id, $date_modified, $this->modified_user_id);
                }
            } else {
                $this->db->updateParams(
                    $this->table_name,
                ...
                if ($this->isFavoritesEnabled()) {
                    DotbFavorites::markRecordDeletedInFavorites($id, $date_modified);
                }
            }
    ...
    *
    * @param string $query The query that should be executed to build the list
    * @param DotbBean $template The object that should be used to copy the records.
    *
    * @return DotbBean[]
    * @deprecated Use DotbQuery instead
    */
    ...
    */
    ...
    //Add By Lap Nguyen
    function retrieve_by_string_contains($fields_array, $encode = true, $deleted = true)
    {
        $query = new \DotbQuery();
        $query->from($this, ['add_deleted' => $deleted, 'team_security' => false]);
        $query->select('*');
        foreach ($fields_array as $field => $value) {
            $query->where()->contains($field, $value);
        }
        $query->limit(1);
    ...
    */
    ...
    function loadLayoutDefs()
    {
        ...
        if (empty($this->layout_def)) {
            foreach (DotbAutoLoader::existing("modules/{$this->module_dir}/layout_defs.php",
    ...
    */
    ...
    function getRealKeyFromCustomFieldAssignedKey($name)
    {
        if ($this->custom_fields->avail_fields[$name]['ext1']) {
            ...
            $GLOBALS['log']->fatal("DOTBBEAN: cannot find Real Key for custom field of type dropdown - cannot return Value.");
            return false;
        }
        if (isset($realKey)) {
    ...
    */
    public function defaultACLs()
    {
        ...
        if (!isset($data['DotbACLStatic']) && $this->bean_implements('ACL')) {
            $data['DotbACLStatic'] = true;
        }
    ...
    public function ACLFilterFields($view = 'detail', $context = array())
    {
        ...
        foreach ($this->field_defs as $field => $def) {
            if (isset($this->$field) && $def['type'] != 'link') {
                if (!DotbACL::checkField($acl_category, $field, $view, $context)) {
                    $this->$field = '';
                }
            }
        }
    ...
    */
    public function ACLFilterFieldList(&$list, $context = array(), $options = array())
    {
        ...
        DotbACL::listFilter($this->getACLCategory(), $list, $context, $options);
    }
    ...
    */
    public function ACLFieldAccess($field, $action = 'access', $context = array())
    {
        ...
        return DotbACL::checkField($this->getACLCategory(), $field, $action, $context);
    }
    ...
    */
    public function ACLFieldGet($field, $context = array())
    {
        ...
        return DotbACL::getFieldAccess($this->getACLCategory(), $field, $context);
    }
    ...
    */
    public function ACLAccess($view, $context = null)
    {
        ...
        return DotbACL::checkAccess($this->getACLCategory(), $view, $context);
    }
    ...
    */
    ...
    {
        $GLOBALS['log']->deprecated("DotbBean::updateDocOwner is deprecated !");
    }
    ...
    */
    ...
    {
        $GLOBALS['log']->deprecated("DotbBean::updateUserFavorites is deprecated !");
    }
    ...
    *
    * @param DotbBean $newbean newly created related bean
    */
    ...
    */
    ...
    )
    {
        ...
        $result = dotb_cache_retrieve($cache_key);
        if (!empty($result)) {
            // Use DotbCache::EXTERNAL_CACHE_NULL_VALUE to store null values in the cache.
            if ($result == DotbCache::EXTERNAL_CACHE_NULL_VALUE) {
                return null;
            }
        ...
        if (file_exists('modules/' . $module_name . '/' . $fileName)) {
            ...
            if (empty($moduleDefs[$object_name][$object_name][$key])) {
                ...
                dotb_cache_put($cache_key, DotbCache::EXTERNAL_CACHE_NULL_VALUE);
                return null;
            }

            // It has been loaded, cache the result.
            dotb_cache_put($cache_key, $moduleDefs[$object_name][$object_name][$key]);
    ...
    /**
    * Returns the ACL category for this module; defaults to the DotbBean::$acl_category if defined
    * otherwise it is DotbBean::$module_dir
    *
    * @return string
    */
    ...
    */
    public function create_export_query($order_by, $where)
    {
        ...
        $file = DotbAutoLoader::existingCustomOne("modules/{$this->module_name}/field_arrays.php");
        if ($file) {
            include($file);
        }
        ...
        if (!empty($fields_array) && !empty($fields_array[$this->object_name]) && !empty($fields_array[$this->object_name]['export_fields'])) {
            ...
            //get list fields defs - By Lap Nguyen
            $view = new DotbView();
            $view->type = 'list';
            $view->module = $this->module_name;
            $listview_meta_file = $view->getMetaDataFile();
            ...
            if(!empty($listview_def)){
                foreach ($fields as $field => $data){
                    if(!array_key_exists($field, $listview_def)){
                        unset($fields[$field]);
                    }
                }

                //Sort list by keys based listview order
                $fields = array_merge(array_flip(array_keys($listview_def)), $fields);
            }
            //END: By Lap Nguyen
        }
        ...
        //add the excluded fields to dotbbean.  This will be used later by getExportContentFromResult() to exclude fields from export
    ...
    /**
    * Returns a clean instance of the same type as this DotbBean.
    * Note that this does not mean it duplicates this bean.  This creates a new untouched instance instead.
    * @return DotbBean a new instance of this bean
    */
    ...
    */
    ...
    {
        DotbAutoLoader::load('include/Expressions/Expression/Parser/Parser.php');
    ...
    * Operations status
    * Known operations:
    * - saving_related - DotbBean is resaving related records
    * - updating_relationships - DotbBean is updating relationships on Save
    ...
    */
    ...
    /**
    * calls beans DotbEmailAddress object to populate the fetched row with the latest email address information
    *
    * @return bool
    */
    ...
    */
    public function getModuleName()
    {
        ...
        } else { // default case, empty DotbBean::module_name
    ...
    * array is up to date.
    *
    * Always try to use {@link DotbBean::retrieve_erased_fields} and {@link DotbBean::retrieve()} instead of this
