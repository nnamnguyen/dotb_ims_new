...
class DotbBeanApiHelper
{
        ...
        if ($hasAccess) {
            ...
            if (isset($bean->deleted) && $bean->deleted == true) {
                ...
                if (isset($bean->date_modified) && !empty($bean->field_defs['date_modified'])) {
                    $field = $sfh->getDotbField($bean->field_defs['date_modified']['type']);
     ...
     * @return array
     */
    public function getBeanAcl(\DotbBean $bean, array $fieldList)
    {
        $acl = array('fields' => (object) array());
        if (\DotbACL::moduleSupportsACL($bean->module_dir)) {
     ...
     * This function
     *
     * @param $bean DotbBean The bean you want populated from the $submittedData array, this function will modify this
     *                        record
     * @param $submittedData array The data that was passed in from the client to update/create this record
     * @param $options array Options to pass in to the populateFromApi function, look at DotbBeanApiHelper:populateFromApi
    ...
    public function populateFromApi(\DotbBean $bean, array $submittedData, array $options = array())
    {
        ...
        if (isset($submittedData['id']) && !empty($bean->id) && $submittedData['id'] != $bean->id) {
            ...
            throw new \DotbApiExceptionInvalidParameter($msg);
        }

        // Some of the DotbFields require ID's, so lets set it up
        if (empty($bean->id)) {
            $bean->id = create_guid();
    ...
    /**
     * This code replicates the behavior in Dotb_Controller::pre_save()
     * @param DotbBean $bean
     * @return boolean
     */
    public function checkNotify($bean)
    {
        ...
        if (!empty($GLOBALS['dotb_config']['exclude_notifications'][$bean->module_dir]) && $GLOBALS['dotb_config']['exclude_notifications'][$bean->module_dir] == true) {
            $check_notify = FALSE;
        } else {
    ...
    protected function checkOptimisticLocking($bean, $timestamp)
    {
        ...
        if(empty($ts_client)) {
            throw new \DotbApiExceptionInvalidParameter("Bad timestamp $timestamp");
        }
        ...
        if($ts_server->ts != $ts_client->ts) {
            ...
            throw new \DotbApiExceptionEditConflict("Edit conflict - $cts, $sts");
        }
    }
}
