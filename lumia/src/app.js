...
DOTB.App = (function() {
     ...
     *
     * `DOTB.App` contains the core instance of the app. All related modules
     * can be found within the `DOTB` namespace.
     *
     ...
     *
     * ```
     * var app = DOTB.App.init({el: '#root', silent: true});
     * ```
     *
     * @class
     * @name App
    ...
    return {
        ...
        init: function(opts) {
             ...
             * @instance
             */
            _app.api = DOTB.Api.getInstance({
                defaultErrorHandler: (opts && opts.defaultErrorHandler) ? opts.defaultErrorHandler : DOTB.App.error.handleHttpError,
        ...
        sync: function(options) {
            ...
            async.waterfall([function(callback) {
                ...
                self.config.dotbLogic = self.config.dotbLogic || {};

                if (serverInfo &&
                    self.config.dotbLogic.enabled &&
                    self.utils.versionCompare(serverInfo.version, self.config.dotbLogic.minServerVersion, ">="))
                {
                    self.fetchDotbLogic(callback);
                }
                else {
                    self.config.dotbLogic.enabled = false;
                    callback();
                }
            }],
                function(err) {
         ...
         * @return {DOTB.HttpRequest} XHR request object.
         * @fires app:logout
         * @memberOf App
         */
        ...
        fetchDotbLogic: function(callback) {
            if (_app.config.dotbLogic.isDynamic) {
                _app.api.call(
                    'read',
                    ...
                    null,
                    {
                        success: function(expressions) {
                            _app.cacheDotbLogicExpressions(expressions);
                            _app.loadDotbLogic(expressions, callback);
                        },
                        error: function(err) {
        ...
        cacheDotbLogicExpressions: function(expressions) {
            _app.cache.set("dotblogic", expressions);
        },

        _loadDotbLogic: function() {
            return _app.cache.get("dotblogic");
        },

        loadDotbLogic: function(expressions, callback) {
            return _app.compileJs(expressions || _app._loadDotbLogic(), callback);
        },
...
const ctxFactory = {
    ...
    getContext: function (attributes) {
        DOTB.App.logger.warn('The function `app.context.getContext()` is deprecated in 7.10. ' +
            'Please use the `new` keyword to create a context.');
        return new Context(attributes);
    }
};

DOTB.App.augment('Bean', require('data/bean'));
DOTB.App.augment('BeanCollection', require('data/bean-collection'));
DOTB.App.augment('MixedBeanCollection', require('data/mixed-bean-collection'));
DOTB.App.augment('error', require('core/error'));
DOTB.App.augment('context', ctxFactory);
DOTB.App.augment('utils', Utils);
DOTB.App.augment('cookie', require('utils/cookie'));
DOTB.App.augment('Controller', Controller);
DOTB.App.augment('events', require('core/events'));
DOTB.App.augment('acl', require('core/acl'));
DOTB.App.augment('metadata', MetadataManager);
DOTB.App.augment('currency', require('utils/currency'));
DOTB.App.augment('date', require('utils/date'));
DOTB.App.augment('math', require('utils/math'));
DOTB.App.augment('plugins', require('core/plugin-manager'));
_.mixin(require('utils/underscore-mixins'));
DOTB.App.augment('user', require('core/user'));
Handlebars.registerHelper(require('view/hbs-helpers'));
DOTB.App.augment('validation', require('data/validation'));
DOTB.App.augment('data', require('data/data-manager'));
DOTB.App.augment('lang', Language);
DOTB.App.augment('template', require('view/template'));
DOTB.App.Router = require('core/router');
DOTB.App.augment('routing', Routing);
const ViewManager = require('view/view-manager');
DOTB.App.augment('view', ViewManager);
ViewManager.Component = require('view/component');
DOTB.App.augment('alert', require('view/alert'));
