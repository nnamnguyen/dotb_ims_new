...
const Alert = {
    ...
    init: function() {
        ...
        if (DOTB.App.config) {
            this.$alerts = $(DOTB.App.config.alertsEl).length ? $(DOTB.App.config.alertsEl) : this.$alerts;

            this.klass = ViewManager[Utils.capitalize(DOTB.App.config.appId) + 'AlertView'] ||
                ViewManager.views[Utils.capitalize(DOTB.App.config.platform) + 'AlertView'] ||
                ViewManager.views.BaseAlertView ||
                ViewManager[Utils.capitalize(DOTB.App.config.platform) + 'AlertView'] ||
                this.klass;
        }
    ...
    _setAutoCloseTimer: function(alert, onAutoClose, autoCloseDelay) {
        ...
        }, this), autoCloseDelay || DOTB.App.config.alertAutoCloseDelay || 5000);
    },
