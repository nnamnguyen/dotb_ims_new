 ...
 * 'page'.
 *
 * Use {@link module:Lumia/View/ViewManager Lumia/View/ViewManager} to
...
const Layout = Component.extend({
    ...
    createComponentFromDef: function(def, context, module) {
        ...
        if (def.context) {
            if (def.context instanceof DOTB.App.Context) {
                context = def.context;
            } else {
        ...
        if (def.view) {
            ...
            DOTB.App.logger.warn('Invalid layout definition:\n' + def.layout);
        }
    },
    ...
    _render: function() {
        if (this._components && this._components.length > 0) {
            ...
            DOTB.App.logger.info("Can't render anything because the layout has no components: " + this.toString() +
