...
/**
 * DotbField widget. A field widget is a low level field widget. Some examples
 * of fields are text boxes, date pickers, drop down menus.
 *
 * ## Creating a DotbField
 * DotBCRM allows for customized "fields" which are visual representations of a type of data (e.g. url would
 * be displayed as a hyperlink).
 *
 * ### Anatomy of a DotbField
 * Field files reside in the **`dotbcrm/clients/base/fields/{field_type}`** folder.
 *
 ...
 * </pre>
 * **`[dotbFieldType].js`** files are optional.
 * Sometimes a DotbField needs to do more than just display a simple input element, other times input elements
 * need additional data such as drop down menu choices. To support advanced functionality, just add your additional
 * controller logic to **`[dotbFieldType].js`** javascript file where dotbFieldType is the type of the DotbField.
 ...
 * Dotb uses Handlebars.js as its client side template of choice. At this time no other templating engines are
 * supported. Sample:
 * ```
 * &lt;span name="{{name}}"&gt;{{value}}&lt;/span&gt;
 * ```
 *
 * These files will be used by the metadata manager to generate metadata for your DotbFields and pass them onto the
 * Dotb JavaScript client.
 *
 * @module View/Field
 * @class
