...
const Component = Backbone.View.extend({
    ...
    /**
     * Constructor for lumia components, currently used to define the
    ...
    initialize: function(options) {
         ...
         * @instance
         */
        this.context = options.context || DOTB.App.controller && DOTB.App.controller.context || new Backbone.Model();
    ...
    render: function() {
        if (this.disposed === true) {
            DOTB.App.logger.error("Unable to render component because it's disposed " + this + '\n');
            return false;
        }
    ...
    unbind: function() {
        ...
        DOTB.App.events.off(null, null, this);
        DOTB.App.events.unregister(this);
    ...
    _super: function(method, args) {
        ...
        if (!method || !_.isString(method)) {
            return DOTB.App.logger.error('tried to call _super without specifying a parent method in ' + this.name);
        }
        ...
        if (this._superStack[method].length > 0) {
            ...
            if (_.contains(this._superStack[method], parent)) {
                return DOTB.App.logger.error('Loop detected calling ' + method + ' from ' + this.name);
            }
        } else {
        ...
        if (!thisProto[method]) {
            return DOTB.App.logger.error('Unable to find method ' + method + ' on class ' + this.name);
        }
        ...
        if (!parent) {
            return DOTB.App.logger.error('Unable to find parent of component ' + this.name);
        }
        if (!parent[method]) {
            return DOTB.App.logger.error('Unable to find method ' + method + ' on parent class of ' + this.name);
        }
