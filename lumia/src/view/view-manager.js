 ...
 * `dotbcrm/clients/<platform>` directory, or using one of the appropriate
...
const ViewManager = {
     ...
     * of putting these controllers in the `dotbcrm/clients/<platform>`
     ...
     *   configurations.
     * @param {Object} [params.context=DOTB.App.controller.context] Context to
    ...
    createView: function(params) {
        ...
        params.context = params.context || DOTB.App.controller.context;
        ...
        params.meta = params.meta || DOTB.App.metadata.getView(params.module, params.name, params.loadModule);
     ...
     * `dotbcrm/clients/<platform>/layouts` in which case the metadata
     ...
     * @param {Object} [params.context=DOTB.App.controller.context]
    ...
    createLayout: function(params) {
        ...
        // name defines which metadata to load
        params.name = params.name || params.type;
        params.meta = params.meta || DOTB.App.metadata.getLayout(params.module, params.name, params.loadModule);
     ...
     * controllers in `dotbcrm/clients/<platform>/fields` or using the
     * `createField` factory.
     *
     ...
     * @param {Object} [params.context=`DOTB.App.controller.context`] The
     *   context.
    ...
    createField: function(params) {
        ...
        params.context = params.context || params.view.context || DOTB.App.controller.context;
     ...
     * `DOTB.App.config.platform` or else 'base'.
     *
    ...
    _getPlatform: function(params) {
        return params.platform || (DOTB.App.config && DOTB.App.config.platform ? DOTB.App.config.platform : 'base');
    },
     ...
     *   Will first attempt to fall back to `DOTB.App.config.platform`, then
     *   'base'.
    ...
    componentHasPlugin: function(params) {
        ...
        if (!params.type || !params.name || !params.plugin) {
            DOTB.App.logger.error("componentHasPlugin requires type, name, and plugin parameters");
            return null;
        }
     ...
     * For example, if `DOTB.App.config.appId == 'portal'`, custom base classes
     * would be:
     ...
     *
     * ```
     * // Assuming DOTB.App.config.appId === 'portal':
    ...
    _getBaseComponent: function(type, name, module, platform, overwrite) {
        ...
        var customBaseClassName = Utils.capitalize(DOTB.App.config.appId) + ucType;
