...
const View = Component.extend({
    ...
    initialize: function(options) {
        ...
        DOTB.App.events.on('app:locale:change', function() {
            this._setLabels();
        }, this);
    },
     ...
     * favor of putting these controllers in the dotbcrm/clients/<platform>
    ...
    _renderHtml: function(ctx, options) {
        if (this.template) {
            try {
                ...
                DOTB.App.logger.error("Failed to render " + this + "\n" + e);
                DOTB.App.error.handleRenderError(this, '_renderHtml');
            }
        }
    },
    ...
    _renderField: function(field, $fieldEl) {
        ...
        try {
            ...
            DOTB.App.logger.error('Failed to render ' + field + ' on ' + this + '\n' + e);
            DOTB.App.error.handleRenderError(this, '_renderField', field);
        }
    },
    ...
    _render: function() {
        if (Acl.hasAccessToModel(this.action, this.model)) {
            ...
            DOTB.App.logger.info('Current user does not have access to this module view. name: ' + this.name +
                ' module: ' + this.module);
            // See Bug56941.
            ...
            if(this.primary){
                DOTB.App.error.handleRenderError(this, 'view_render_denied');
            }
        }
    ...
    getFieldNames: function(module) {
        ...
        var fieldMetadata = DOTB.App.metadata.getModule(module, 'fields');
        if (fieldMetadata) {
