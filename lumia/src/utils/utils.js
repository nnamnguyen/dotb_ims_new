...
const Utils = {
    ...
    extendClass: function(cache, defaultBase, className, controller, platformNamespace) {
        ...
        if (!base) {
            DOTB.App.logger.warn('The "' + controller.extendsFrom + '" component was not found to be used as ' +
    ...
    formatNumber: function(value, round, precision, numberGroupSeparator, decimalSeparator) {
        ...
        if(!_.isNumber(value)) {
            if(!_.isNull(value) && !_.isUndefined(value)) {
                ...
                DOTB.App.logger.warn('formatNumber: invalid variable type ('+typeof(original)+')');
            }
            return original;
        }
    ...
    formatNameModel: function(module, data, format) {
        ...
        var metadata = DOTB.App.metadata.getModule(module) || {fields: {}};
     ...
     * Utils.doWhen('DOTB.ObjectToWaitFor', function(){
     *     // Use the object here
     *     console.log(DOTB.ObjectToWaitFor);
     * });
     *
    ...
    buildUrl: function(url) {
        ...
        if (url.indexOf("http") !== 0 && !_.isEmpty(DOTB.App.config.siteUrl)) {
            // Strip trailing forward slashes from site URL just in case
            url = DOTB.App.config.siteUrl.replace(/\/+$/, "") + "/" + url;
        }
        return url;
    },
    ...
    isSortable: function(module, fieldViewdef) {
        var fieldVardef = DOTB.App.metadata.getModule(module).fields[fieldViewdef.name],
            isSortable = true;
