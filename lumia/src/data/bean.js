...
const Bean = Backbone.Model.extend({
    ...
    initialize: function(attributes) {
        ...
        this.addValidationTask('lumia', _.bind(this._doValidate, this));
    },
     ...
     * @return {DOTB.Api.HttpRequest} The active fetch request.
     * @memberOf Data/Bean
     * @instance
     */
    ...
    abortFetchRequest: function() {
        ...
        if (req) {
            DOTB.App.api.abortRequest(req.uid);
        }
    },
    ...
    get: function(attr) {
        ...
        if (!this.fields || !this.fields[attr] || this.fields[attr].type !== 'collection' ||
            value instanceof DOTB.App.MixedBeanCollection) {
            return value;
        }
    ...
    _setCollectionFieldValues: function (collections, options) {
        _.each(collections, function (records, key) {
            ...
            if (records instanceof DOTB.App.MixedBeanCollection) {
    ...
    _createMixedBeanCollectionField: function (field, models) {
        ...
        if (fieldDef && fieldDef.links) {
            ...
            var collection = DOTB.App.data.createMixedBeanCollection(models || [],
    ...
    getRelatedCollection: function(link) {
        ...
        return DOTB.App.data.createRelatedCollection(this, link);
    },
    ...
    _processValidationErrors: function(errors) {
        var isValid = true;
        if (!_.isEmpty(errors)) {
            DOTB.App.error.handleValidationError(this, errors);
     ...
     *   validate.
     * @return {DOTB.Api.HttpRequest} Returns an HTTP request if
     ...
     *   {@link DOTB.Api#file} for details.
    ...
    copy: function(source, fields, options) {
        var attrs = {};
        var vardefs = DOTB.App.metadata.getModule(this.module).fields;
     ...
     *   {@link DOTB.Api#file} method for details.
     * @return {Object} XHR object.
     * @memberOf Data/Bean
     * @instance
     */
     ...
     * @return {DOTB.Api.HttpRequest} The request to update this bean.
     * @memberOf Data/Bean
     * @instance
     */
    ...
    hasChanged: function(attr) {
        if (this.get(attr) instanceof DOTB.App.MixedBeanCollection) {
    ...
    toJSON: function(options) {
        ...
        _.each(fields, function(fieldName) {
            ...
            if (val instanceof DOTB.App.MixedBeanCollection) {
                _.each(val.getDelta(), (val, linkName) => {
                    json[linkName] = val;
                });
            } else {
                if (_.isObject(val) && _.isFunction(val.toJSON)) {
                    DOTB.App.logger.warn('Calling `toJSON` on object attributes is deprecated in 7.9 and will be ' +
                        'removed in 7.10');
