...
function makeValidators() {
    ...
    return {
        ...
        url: function(field, value) {
            DOTB.App.logger.warn('The function `app.validation.validators.url()` is deprecated since 7.10 ' +
...
const Validation = {
    ...
    _isArrayEmpty: function(value) {
        if (!DOTB.App.config.lumiaCompatMode) {
            DOTB.App.logger.error('Data.Validation#_isArrayEmpty is a private method that you are not allowed ' +
                'to access. Please use only the public API.');
            return;
        }

        DOTB.App.logger.warn('Data.Validation#_isArrayEmpty is a private method that you should not access. ' +
