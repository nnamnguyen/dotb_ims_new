...
const BeanCollection = Backbone.Collection.extend({
    ...
    getDelta: function() {
        ...
        let oppositeLink = DOTB.App.data.getOppositeLink(this.link.bean.module, this.link.name);
        let relationshipFields = DOTB.App.data.getRelationFields(this.module, oppositeLink);
     ...
     * @return {DOTB.HttpRequest} The created fetch request.
     * @memberOf Data/BeanCollection
     * @instance
     */
    fetch: function(options) {
        ...
        // SC-2703. It will be removed when we update our lumia components
     ...
     *
     * @return {DOTB.HttpRequest} The currently active http fetch request.
     * @memberOf Data/BeanCollection
     * @instance
     */
    ...
    abortFetchRequest: function () {
        var activeFetchRequest = this.getFetchRequest();
        if (activeFetchRequest) {
            DOTB.App.api.abortRequest(activeFetchRequest.uid);
            this._activeFetchRequest = null;
        }
    },
    ...
    paginate: function(options) {
        options = options || {};
        var maxSize = options.limit || DOTB.App.config.maxQueryResult;
     ...
     * @return {DOTB.HttpRequest|undefined} Result of {@link DOTB.Api#call},
    ...
    fetchTotal: function(options) {
        ...
        return DOTB.App.api.count(module, data, callbacks, options);
    },
     ...
     * @param {Function} [options.complete] Complete callback.
     * @param {Function} [options.error] Error callback.
     * @return {DOTB.HttpRequest} Result of {@link DOTB.Api#call}.
     * @memberOf Data/BeanCollection
     * @instance
     */
    hasAtLeast: function(amount, options) {
        ...
        var callbacks = _.pick(options, 'success', 'complete', 'error');
        options = _.omit(options, 'success', 'complete', 'error');
        options = DOTB.App.data.parseOptionsForSync(method, this, options);

        return DOTB.App.api[endpoint](method, module, data, options.params, callbacks, options.apiOptions);
    },
    ...
    getPageNumber: function(options) {
        var pageNumber = 1;
        var maxSize = DOTB.App.config.maxQueryResult;
        if(options){
