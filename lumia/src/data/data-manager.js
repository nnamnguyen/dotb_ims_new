...
const getRelationshipName = function(module, link) {
    let vardefs = DOTB.App.metadata.getModule(module);
...
const DataManager = _.extend({
    ...
    init: function() {
        ...
        DOTB.App.events.register(
             ...
             *     DOTB.App.logger.debug('Started operation ' + method + ' on ' + model);
             * });
             * ```
        ...
        DOTB.App.events.register(
             ...
             *  * reference to the model/collection
             *  * options
             *  * request (DOTB.Api.HttpRequest)
             *
             * ```
             * const Events = require('core/events');
             * Events.on('data:sync:complete', function(method, model, options, request) {
             *     DOTB.App.logger.debug("Finished operation " + method + " on " + model);
             * });
             * ```
        ...
        DOTB.App.events.register(
             ...
             *  * reference to the model/collection
             *  * options
             *  * error (DOTB.Api.HttpError)
             *
             * ```
             * const Events = require('core/events');
             * Events.on('data:sync:error', function(method, model, options, error) {
             *     DOTB.App.logger.debug('Operation failed ' + method + ' on ' + model);
             * });
             * ```
        ...
        DOTB.App.events.register(
             ...
             *  * reference to the model/collection
             *  * options
             *  * request {@link DOTB.Api.HttpRequest}
             *
             * ```
             * const Events = require('core/events');
             * DOTB.App.events.on('data:sync:abort', function(method, model, options, request) {
             *     DOTB.App.logger.debug('Operation aborted ' + method + ' on ' + model);
             * });
             * ```
    ...
    declareModel: function(moduleName, module, platform, modelController, collectionController) {
        ...
        platform = platform || DOTB.App.config.platform || 'base';
    ...
    declareModels: function(modules) {
        modules = modules || DOTB.App.metadata.getModules();
    ...
    canHaveMany: function(module, link) {
        var meta = DOTB.App.metadata.getModule(module);
        ...
        var relationship = DOTB.App.metadata.getRelationship(name);
    ...
    getRelateFields: function(parentModuleName, link) {
        ...
        var parentModuleDef = DOTB.App.metadata.getModule(parentModuleName);
        if (!parentModuleDef.fields[link]) {
            DOTB.App.logger.error(`Calling 'getRelateFields' on '${parentModuleName}' with link '${link}' but no fields` +
        ...
        var fields = DOTB.App.metadata.getModule(relatedModule).fields;
    ...
    getRelationshipFields: function(parentModule, link) {
        DOTB.App.logger.warn('DataManager#getRelationshipFields is deprecated since 7.10. It will be removed ' +
            'in a future release. Please use DataManager#getRelationFields instead');
        var ff = null;
        var linkField = DOTB.App.metadata.getModule(parentModule).fields[link];
    ...
    getRelationFields: function(module, link) {
        let moduleFields = DOTB.App.metadata.getModule(module) && DOTB.App.metadata.getModule(module).fields;
        if (!moduleFields) {
            return false;
        }
    ...
    getOppositeLink: function(module, link) {
        ...
        let relatedVardefs = DOTB.App.metadata.getModule(relatedModule);
    ...
    getRelatedModule: function(module, link) {
        ...
        let relationship = DOTB.App.metadata.getRelationship(getRelationshipName(module, link));
        if (!relationship) {
    ...
    getRelatedNameField: function(module, idFieldName) {
        return _.find(DOTB.App.metadata.getModule(module).fields, function(field) {
    ...
    getEditableFields: function(model, fields) {
        ...
        _.each(fields, function(fieldName) {
            ...
            if(model.has(fieldName) && // Model has that field AND
                ...
                //lumia does not know about and needs to be be removed.
     ...
     * the remote server using {@link DOTB.Api}.
     *
     ...
     *   be synced/read.
     * @param {Object} [options] A hash of options.
     * @return {Dotb.HttpRequest|boolean} The sync request, or `false` if you
    ...
    sync: function(method, model, options) {
        DOTB.App.logger.trace('data-sync-' + (options.relate ? 'relate-' : '') + method + ": " + model);
        ...
        if (_.isFunction(options.endpoint)) {
            ...
            if (!model.link.bean.id) {
                DOTB.App.logger.error('Attempted to load linked context with no id on the linked bean: ' +
                    model.link.bean);
                return false;
            }

            request = DOTB.App.api.relationships(
                method,
            ...
            request = DOTB.App.api.collection(
                options.module,
                {
                    id: options.beanId,
            ...
            if (options.query || (model instanceof MixedBeanCollection)) {
                ...
                DOTB.App.logger.error("Unable to sync model with no module");
            }
        }
    ...
    parseOptionsForSync: function(method, model, options) {
        ...
        if ((method == "read") && (model instanceof BeanCollection)) {
            ...
            if (options.limit || (DOTB.App.config && DOTB.App.config.maxQueryResult)) {
                options.params.max_num = options.limit || DOTB.App.config.maxQueryResult;
            }
     ...
     * {@link Core.Events DOTB.App.events}), as well as on the `model`.
     *
    ...
    getSyncErrorCallback: function(method, model, options) {
        return _.bind(function(error) {
            ...
            DOTB.App.error.handleHttpError(error, model, options);
