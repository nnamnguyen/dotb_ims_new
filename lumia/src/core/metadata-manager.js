...
function _setHash(data, isPublic) {
    ...
    DOTB.App.cache.set(_keyPrefix + pKey + 'hash', data._hash);
}

function _cacheMeta(data) {
    DOTB.App.cache.set(_keyPrefix + 'data', data);
}

function _getCachedMeta() {
    return DOTB.App.cache.get(_keyPrefix + 'data');
}
...
function _fetchLabels(metadata, options) {
    ...
    DOTB.App.api.call('read', langStringsUrl, null, {
        success: function(labelsData) {
            ...
            try {
                ...
                DOTB.App.logger.fatal('Failed to parse labels data: ' + ex);
                options.error({
                    code: 'sync_failed',
...
function _initCustomComponents(module, moduleName) {
    var self = this,
        platforms = (DOTB.App.config.platform !== 'base') ? ['base', DOTB.App.config.platform] : ['base'];
...
module.exports = {
            ...
            if (entry.controller) {
                ...
                if (_.isString(controller)) {
                    try {
                        ...
                        DOTB.App.logger.error('Failed to eval view controller for ' + className + ': ' + e + ':\n' + entry.controller);
                    }
                } else {
                    /*
                    ...
                    if (emptyController) {
                        DOTB.App.logger.warn('Instead of creating an empty js controller `' + className +
        ...
        if (options.filter) {
            ...
            _.each(options.filter, function(f) {
                if (f && _.indexOf(filters, f) > -1) {
                    ...
                    DOTB.App.logger.warn('Can\'t filter getModuleNames by ' + f);
                }
            });
            if (_.isEmpty(filter)) {
                filter = undefined;
    ...
    set: function(data, isPublic, reset) {
        ...
        if (data.config) {
            DOTB.App.setConfig(data.config);
        }
        ...
        if (DOTB.App.config.cacheMeta && !isPublic) {
            _cacheMeta(_metadata);
        }

        if (DOTB.App.config.env != 'prod') {
    ...
    getHash: function(isPublic) {
        ...
        return DOTB.App.cache.get(key) || _metadata._hash || '';
    },
    ...
    sync: function(callback, options) {
        ...
        var self = this,
            metadataTypes = options.metadataTypes || (options.getPublic ? DOTB.App.config.publicMetadataTypes : DOTB.App.config.metadataTypes) || [],
            errorCallback = function(error) {
                DOTB.App.logger.debug('Failed fetching metadata');
                if (!options.getPublic) {
                    DOTB.App.error.handleHttpError(error);
                }
        ...
        DOTB.App.api.getMetadata({types: metadataTypes, callbacks: {
            success: function(metadata) {
                var compatible;
                options = options || {};

                if (!_.isEmpty(metadata)) {
                    DOTB.App.logger.debug('Updating metadata');
                    if (_mdLoaded[options.getPublic ? 'public:md' : 'md'] &&
                        self.getHash(options.getPublic) == metadata._hash && !options.forceRefresh
                    ) {
                        DOTB.App.logger.debug('Skipping update as metadata hash matches');
                    ...
                    if (_.isEmpty(metadataTypes) || _.include(metadataTypes, 'server_info') && !options.getPublic) {
                        compatible = DOTB.App.isServerCompatible(metadata.server_info);
    ...
    _loadJSSource: function(metadata, options, callback, errorCallback, self) {
        ...
        async.parallel([
            function(cb) {
                if (loadJS) {
                    Utils.doWhen('DOTB.jssource', function() {
                        self._declareClasses(DOTB.jssource);
                        cb();
                    });
                } else {
                    cb();
    ...
    storageHandler: function() {
        ...
        if (!_syncing && ((this.getHash() != _metadata._hash && _mdLoaded.md) ||
            (
                DOTB.App.api.getUserprefHash() && User.get('_hash') &&
                DOTB.App.api.getUserprefHash() != User.get('_hash')
            )
        )) {
            _syncing = true;
            _mdLoaded.md = false;
            DOTB.App.sync({
                getPublic: !DOTB.App.api.isAuthenticated()
            });
        }
    },
     ...
     */
    clearCache: function() {
        DOTB.App.cache.cut(_keyPrefix + 'public:hash');
        DOTB.App.cache.cut(_keyPrefix + 'hash');
        DOTB.App.cache.cut(_keyPrefix + 'data');
    },
