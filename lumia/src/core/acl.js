...
let Acl = {
    ...
    hasAccess: function(action, module, options) {
        ...
        if (access && field && acls.fields) {
            ...
            let moduleMeta = DOTB.App.metadata.getModule(module);
    ...
    _hasAccess: function(action, acls) {
        if (!DOTB.App.config.lumiaCompatMode) {
            DOTB.App.logger.error('Core.Acl#_hasAccess is a private method that you are not allowed to access. ' +
                'Please use only the public API.');
            return;
        }

        DOTB.App.logger.warn('Core.Acl#_hasAccess is a private method that you should not access. ' +
    ...
    _hasAccessToField: function(action, acls, field) {
        if (!DOTB.App.config.lumiaCompatMode) {
            DOTB.App.logger.error('Core.Acl#_hasAccessToField is a private method that you are not allowed ' +
                'to access. Please use only the public API.');
            return;
        }

        DOTB.App.logger.warn('Core.Acl#_hasAccessToField is a private method that you should not access. ' +
     ...
     */
    clearCache: function() {
        DOTB.App.logger.warn('`Core.Acl#clearCache` is deprecated since 7.10 and will be removed in a future ' +
...
Object.defineProperty(Acl, 'action2permission', {
    ...
    get: function() {
        DOTB.App.logger.warn('`Core.Acl.action2permission` has been made private since 7.10 and you will not be able ' +
...
Object.defineProperty(Acl, '_accessToAny', {
    ...
    get: function() {
        DOTB.App.logger.warn('`Core.Acl#_accessToAny` is deprecated since 7.10 and will be removed in a future ' +
