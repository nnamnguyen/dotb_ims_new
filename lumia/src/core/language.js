...
function get(type, key, module, context) {
    var str,
        bundle = DOTB.App.metadata.getStrings(type);
...
module.exports = {
    ...
    getAppListStrings: function(key) {
        var list = DOTB.App.metadata.getStrings('app_list_strings')[key] || {};
        if (_.isArray(list)) {
            var obj = {};
    ...
    getAppListKeys: function(listName) {
        var keys = [],
            list = DOTB.App.metadata.getStrings('app_list_strings')[listName] || {};
        if (_.isArray(list)) {
            _.each(list, function(tuple) {
                if (tuple.length === 2) {
                    keys.push(tuple[0]);
        ...
        DOTB.App.sync({
            callback: function(err) {
                ...
                if (!err) {
                    ...
                    langHasChanged = !DOTB.App.api.isAuthenticated() && !options.noUserUpdate;
                    DOTB.App.cache.set('langHasChanged', langHasChanged);//persist even after reloads
                }
     ...
     *
     * @param {string} language Language code as defined in Dotb.
     *   (e.g. `en_us`, `pt_PT`)
     */
    updateLanguage: function(language) {
        DOTB.App.cache.set('lang', language);
        User.setPreference('language', language);
        this.setCurrentLanguage(language);
        DOTB.App.trigger('app:locale:change', language);
    },
    ...
    setDirection: function(lang) {
        ...
        if (this.direction !== prevDirection) {
            DOTB.App.trigger('lang:direction:change');
        }
    },

    _get: function(type, key, module, context) {
        if (!DOTB.App.config.lumiaCompatMode) {
            DOTB.App.logger.error('Core.Language#_get is a private method that you are not allowed ' +
                'to access. Please use only the public API.');
            return;
        }

        DOTB.App.logger.warn('Core.Language#_get is a private method that you should not access to. ' +
    ...
    _sanitize: function(str) {
        if (!DOTB.App.config.lumiaCompatMode) {
            DOTB.App.logger.error('Core.Language#_sanitize is a private method that you are not allowed ' +
                'to access. Please use only the public API.');
            return;
        }

        DOTB.App.logger.warn('Core.Language#_sanitize is a private method that you should not access. ' +
            'You will NOT be allowed to access it in the next release. Please update your code to rely on the public ' +
            'API only.');
