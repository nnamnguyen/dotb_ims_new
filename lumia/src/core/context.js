...
const Context = Backbone.Model.extend({
    ...
    loadData: function(options) {
        ...
        var objectToFetch,
            ...
            module = this.get("module"),
            defaultOrdering = (DOTB.App.config.orderByDefaults && module) ? DOTB.App.config.orderByDefaults[module] : null;
        ...
        if (objectToFetch && (objectToFetch instanceof Bean ||
            ...
            DOTB.App.logger.warn("Skipping fetch because model is not Bean, Bean Collection, or it is not defined, module: " + this.get("module"));
        }
    },
    ...
    _shouldFetch: function() {
        if (!DOTB.App.config.lumiaCompatMode) {
            DOTB.App.logger.error('Core.Context#_shouldFetch is a private method that you are not allowed ' +
                'to access. Please use only the public API.');
            return;
        }

        DOTB.App.logger.warn('Core.Context#_shouldFetch is a private method that you should not access. ' +
