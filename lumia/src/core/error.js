...
const ErrorHandler = {
    /**
     * Binds a `window.onerror` callback to log error using lumia's logger.
     */
    init: function() {
        this.enableOnError();
        ...
        Object.defineProperty(this, 'remoteLogging', {
            get: function() {
                DOTB.App.logger.warn('`Core.Error#remoteLogging` property has been deprecated since 7.10.');
                return false;
            },
            configurable: true
        });
    ...
    initialize: function(opts) {
        DOTB.App.logger.warn('The function `Core.Error#initialize` is deprecated in 7.10. ' +
    ...
    handleValidationError: function(error) {
        ...
        _.each(errors, function(fieldError, key) {
            ...
            DOTB.App.logger.debug("validation failed for field `" + key + "`:\n" + errorMsg);
        }, this);
    },
    ...
    handleUnhandledError: function(message, url, line) {
        DOTB.App.logger.fatal(message + ' at ' + url + ' on line ' + line);
    },
    ...
    handleStatusCodesFallback: function(error) {
        DOTB.App.logger.error(error.toString());
    },
        ...
        switch (method) {
            ...
            default:
                ...
                messages = [Language.get('ERR_INTERNAL_ERR_MSG'),
                            Language.get('ERR_CONTACT_TECH_SUPPORT')];
                DOTB.App.logger.error('handleRenderError called for render error caught in ' + method + ', but we have no corresponding handler!');
                break;
        }
    ...
    throwErrorWithCallStack: function(error, skipThrow) {
        DOTB.App.logger.warn('The function `Core.Error#throwErrorWithCallStack` is deprecated in 7.10. ' +
    ...
    _callCustomHandler: function(error, fn, params) {
        if (!DOTB.App.config.lumiaCompatMode) {
            DOTB.App.logger.error('Core.Error#_callCustomHandler is a private method that you are not allowed ' +
                'to access. Please use only the public API.');
            return;
        }

        DOTB.App.logger.warn('Core.Error#_callCustomHandler is a private method that you should not access. ' +
    ...
    _handleFineGrainedError: function(error, alternativeCallback) {
        if (!DOTB.App.config.lumiaCompatMode) {
            DOTB.App.logger.error('Core.Error#_handleFineGrainedError is a private method that you are not allowed ' +
                'to access. Please use only the public API.');
            return;
        }

        DOTB.App.logger.warn('Core.Error#_handleFineGrainedError is a private method that you should not access. ' +
            'You will NOT be allowed to access it in the next release. Please update your code to rely on the public ' +
            'API only.');
