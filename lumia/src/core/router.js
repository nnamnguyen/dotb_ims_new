...
const Router = Backbone.Router.extend({
    ...
    _moduleExists: function(module) {
        if (module && _.isUndefined(DOTB.App.metadata.getModule(module))) {
    ...
    route: function (route, name, callback) {
        if (!name) {
            ...
            DOTB.App.logger.error('Route "' + name + '" is being overridden. This is highly NOT advisable.');
        }
     ...
     * module defined in `DOTB.App.config`.
     */
    index: function() {
        DOTB.App.logger.debug("Route changed to index");
        if (DOTB.App.config.defaultModule) {
            this.navigate(DOTB.App.config.defaultModule, {trigger:true});
        }
    ...
    list: function(module) {
        ...
        DOTB.App.logger.debug("Route changed to list of " + module);
        DOTB.App.controller.loadView({
            module: module,
            layout: "records"
        });
    },
    ...
    layout: function(module, layout) {
        ...
        DOTB.App.logger.debug("Route changed to layout: " + layout + " for " + module);
        DOTB.App.controller.loadView({
            module: module,
            layout: layout
        });
    ...
    create: function(module) {
        ...
        DOTB.App.logger.debug("Route changed: create " + module);
        DOTB.App.controller.loadView({
            module: module,
            create: true,
            layout: "edit"
     ...
     */
    login: function() {
        DOTB.App.logger.debug("Logging in");
        DOTB.App.controller.loadView({
            module: "Login",
            layout: "login",
            create: true
    ...
    logout: function(clear) {
        ...
        clear = (clear === "1");
        DOTB.App.logger.debug("Logging out: " + clear);
        DOTB.App.logout({
            complete() {
                DOTB.App.router.navigate("#");
                if (!DOTB.App.config.externalLogin) {
                    if (clear) {
    ...
    record: function(module, id, action, layout) {
        ...
        var oldCollection = DOTB.App.controller.context.get('collection'),
            oldListCollection = DOTB.App.controller.context.get('listCollection'),
            opts = {
                module: module,
        ...
        DOTB.App.controller.loadView(opts);
    },
});
