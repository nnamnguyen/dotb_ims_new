...
DOTB.util.extend = DOTB.util.extend || (DOTB.App ? DOTB.App.utils.extendFrom : null);

/**
 * Constructs an Expression object given the parameters.
 */
var SEE = DOTB.expressions.Expression = function(context) {
...
DOTB.expressions.NumericConstants = {
	pi: 3.14159265,
	e: 2.718281828459045
}
    ...
    for (i = 0; i < params.length; i++) {
        ...
        if (param instanceof SEE) {
            ...
            if (type != types[i] && types[i] in map) {
                param = new DOTB.expressions[map[types[i]]](param, this.context);
            }
        }
        result.push(param);
    }
...
SEE.prototype.isProperType = function(variable, type) {
	...
	switch(type) {
		...
		case see.NUMERIC_TYPE:
			return (typeof(variable) == 'number' || DOTB.expressions.isNumeric(variable));
			break;
        ...
        case see.TIME_TYPE:
            ...
            if (variable === null || variable === ''
                || (typeof(variable) == 'string' && DOTB.util.DateUtils.guessFormat(variable)))
                return true;
            break;
...
SEE.prototype.getClass = function(exp) {
	for (var i in DOTB.FunctionMap){
		if (typeof DOTB.FunctionMap[i] == "function" && DOTB.FunctionMap[i].prototype
			&& this instanceof DOTB.FunctionMap[i])
			return i;
	}
	return false;
};
...
DOTB.expressions.GenericExpression = function(params) {

};
DOTB.util.extend(DOTB.expressions.GenericExpression, SEE, {
	/**
...
DOTB.util.extend(DOTB.expressions.NumericExpression, SEE, {
	/**
	 * All parameters have to be a number by default.
	 */
	getParameterTypes: function() {
...
DOTB.util.extend(DOTB.expressions.StringExpression, SEE, {
	/**
...
DOTB.util.extend(DOTB.expressions.BooleanExpression, SEE, {
	/**
...
DOTB.util.extend(DOTB.expressions.EnumExpression, SEE, {
	/**
...
DOTB.util.extend(DOTB.expressions.DateExpression, SEE, {
	/**
...
DOTB.util.extend(DOTB.expressions.TimeExpression, SEE, {
	/**
...
DOTB.expressions.RelateExpression = function(params) {

};
DOTB.util.extend(DOTB.expressions.RelateExpression, SEE, {
	/**
	 * All parameters have to be a number by default.
	 */
	getParameterTypes: function() {
...
SEE.TYPE_MAP	= {
		...
		"relate" 	: DOTB.expressions.RelateExpression,
		"generic" 	: DOTB.expressions.GenericExpression
};
 ...
 */
var SEP = DOTB.expressions.ExpressionParser = function() {
	// nothing
};
...
SEP.prototype.evaluate = function(expr, context)
{
	...
	// require and return the appropriate expression object
	return new DOTB.FunctionMap[func](args, context);
}
...
SEP.prototype.toConstant = function(expr) {
	...
	if ( (/^(0[0-9]|1[0-2])\/([0-2][0-9]|3[0-1])\/[0-3][0-9]{3,3}$/).exec(expr) != null ) {
		...
		return new DOTB.expressions.DateExpression([month, day, year]);
	}
 ...
 */
var SEC = DOTB.expressions.ExpressionContext = function()
{
...
{
	return new DOTB.RelatedFieldExpression([
		new DOTB.expressions.StringLiteralExpression( linkField ),
		new DOTB.expressions.StringLiteralExpression( relField )
	]);
}

DOTB.expressions.isNumeric = function(str) {
    if (typeof(str) != 'number' && typeof(str) != 'string') {
        return false;
    }
    str = DOTB.expressions.unFormatNumber(str);
    return (isFinite(str) && !isNaN(parseFloat(str)));

};

DOTB.expressions.unFormatNumber = function(num) {
...
DOTB.expressions.replaceAll = function(haystack, needle, rpl) {
...
DOTB.util.DateUtils = {
	...
	parse : function(date, oldFormat) {
        ...
        if (oldFormat == "user")
		{
			if (DOTB.expressions.userPrefs && DOTB.expressions.userPrefs.datef) {
				oldFormat = DOTB.expressions.userPrefs.datef + " " + DOTB.expressions.userPrefs.timef;
			} else {
				oldFormat = DOTB.util.DateUtils.guessFormat(date);
			}
		}
    ...
    {
        if (!format && DOTB.expressions.userPrefs.datef && DOTB.expressions.userPrefs.timef) {
            if (useTime)
				format = DOTB.expressions.userPrefs.datef + " " + DOTB.expressions.userPrefs.timef;
			else
	...
	getUserTime: function()
	{
		...
		if (DOTB.expressions.userPrefs &&
            typeof(DOTB.expressions.userPrefs.gmt_offset) != 'undefined')
		{
			//Dotbs TZ offset is negative compared with JS's offset, so adding is really subtracting;
			var offset = DOTB.expressions.userPrefs.gmt_offset;
