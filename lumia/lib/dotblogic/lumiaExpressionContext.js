...
DOTB.forms = DOTB.forms || {};
DOTB.forms.animation = DOTB.forms.animation || {};
 ...
 */
var SE = DOTB.expressions;
var App = DOTB.App || App;
/**
 * DotbLogic ExpressionContext for Lumia.
 * @type {DOTB.expressions.LumiaExpressionContext}
 *
 * @param view {App.view.View} Lumia view to attach to
 ...
 * model.
 */
var SEC = SE.LumiaExpressionContext = function (view, model, collection) {
    this.view = view;
...
App.utils.extendFrom(SEC, SE.ExpressionContext, {
    ...
    initialize: function (depsMeta, isCreate) {
        ...
        var updateCollection = _.bind(function(models, trigger) {
            _.each(models, function(model) {
                if (trigger.dependency.testOnLoad || model.isNew()) {
                    ...
                    DOTB.forms.Trigger.fire.apply(trigger, [model]);
        ...
        _.each(depsMeta, function(dep) {
            var newDep = DOTB.forms.Dependency.fromMeta(dep, this);
            if (newDep) {
                ...
                if (scContext.isCreate() || newDep.fireLinkOnlyDependency() || isCreate) {
                    DOTB.forms.Trigger.fire.apply(newDep.trigger);
                    this.view.trigger('dotblogic:initialize');
                }
                ...
                if (newDep.testOnLoad) {
                    scContext.on('list:editrow:fire', function() {
                        ...
                        DOTB.forms.Trigger.fire.apply(this.trigger, arguments);
                        scContext.isOnLoad = null;
                    }, newDep);
                }

                if (this.collection instanceof DOTB.App.data.beanCollection) {
                    ...
                    this.collection.on('sync', function(synced, syncData) {
                        var models;
                        var ids;
                        if (synced instanceof DOTB.App.data.beanCollection) {
        ...
        this.view.on('render', function () {
            _.each(this.dependencies, function (dep) {
                if (scContext.isCreate() || dep.fireLinkOnlyDependency()) {
                    DOTB.forms.Trigger.fire.apply(dep.trigger);
                    this.view.trigger('dotblogic:initialize');
                }
            }, this);
        }, this);
    ...
    getValue : function(varname)
    {
        ...
        if (!def) {
            DOTB.App.logger.warn('Attempted to get a value for field ' + varname
        ...
        if (typeof(value) == "string")
        {
            ...
            result =  value ? new DOTB.expressions.TrueExpression() : new DOTB.expressions.FalseExpression();
        } else {
            result = SEC.parser.toConstant('""');
        }
    ...
    setValue : function(varname, value)
    {
        ...
        if (targetDef.type == 'bool' && _.isString(value)) {
            value = DOTB.expressions.Expression.isTruthy(value);
        }
        ...
        if (_.isString(value) && targetDef.len && value.length > targetDef.len) {
            ...
            this.model.once('change:' + varname, function() {
                var msg = DOTB.App.lang.getAppString('LBL_FIELD_TRIMMED');
                DOTB.forms.markField(varname, self.getElement(varname), msg);
            });
        } else if (DOTB.forms.markedField[varname]) {
            DOTB.forms.unmarkField(varname, this.getElement(varname));
        }
        ...
        if (!this.lockedFields[varname])
        {
            ...
            if (el) {
                DOTB.forms.FlashField($(el).parents('[data-fieldname="' + varname + '"]'), null, varname);
            }
     ...
     * @param {String} ftype The DotbLogic Function
    ...
    /**
     * FixMe: Right now we need to do this so the Rollup DotbLogic doesn't break the saving on creates
     * @param {string} link
     * @return {string}
     * @private
     */
    ...
    _handleRelateExpression : function(link, field){
        ...
        if (field &&
            ...
            var model = this.model;
            DOTB.App.utils.doWhen(function(){return col.page > 0}, function(){
                model.trigger("change:" + link, model);
            });
        }
        return "";
        ...
        if (rField && this.model.get(rField.id_name)){
            ...
            var modelId = this.model.get(rField.id_name),
                model =  relContext.get("model")
                     || DOTB.App.data.createRelatedBean(this.model, this.model.get(rField.id_name), link);
            relContext.set({
                modelId:modelId,
                model : model
            });
    ...
    fireOnLoad : function(dep) {
        ...
        //this.view.model.once("change", DOTB.forms.Trigger.fire, dep.trigger);
    },
    getAppListStrings : function(list) {
        return DOTB.App.lang.getAppListStrings(list);
    },
    parseDate: function(date, type) {
        return DOTB.App.date.parse(date);
    },
    /**
    ...
    setFieldRequired: function(target, required) {
        ...
        required = DOTB.expressions.Expression.isTruthy(required);
    ...
    setAssignedUserName: function(target, username) {
        if (this.model.has('assigned_user_name') && this.model.has('assigned_user_id')) {
            var self = this, options = {}, usersCollection = DOTB.App.data.createBeanCollection('Users');
            ...
            options.success = function(collection) {
                ...
                if (userModel) {
                    ...
                    if (field && field.el) {
                        DOTB.forms.FlashField(field.el, null, target);
                    }
                }
            }

            options.error = function() {
                DOTB.App.alert.show('server-error', {
                    level: 'error',
                    title: DOTB.App.lang.get('ERR_GENERIC_TITLE'),
                    messages: DOTB.App.lang.get('ERR_ASSIGNTO_ACTION')
                });
            }
    ...
    add: function(start, add) {
        return DOTB.App.math.add(start, add, 6, true);
    },
    /**
    ...
    subtract: function(start, subtract) {
        return DOTB.App.math.sub(start, subtract, 6, true);
    },
    /**
    ...
    multiply: function(start, multiply) {
        return DOTB.App.math.mul(start, multiply, 6, true);
    },
    /**
    ...
    divide: function(start, divide) {
        return DOTB.App.math.div(start, divide, 6, true);
    },
    ...
    round: function(start, precision) {
        return DOTB.App.math.round(start, precision, true);
    },

    /**
     * Check the model for collection fields,  If any fields are are found for the given link name, then return it.
     *
     * This will save lumia from having to create a new context if the model will contain the collection as part of
     * it's data set.
     *
     * @param {String} linkName
     * @return {undefined|DOTB.App.data.beanCollection|string}
     */
        ...
        if (!_.isEmpty(collectionFields)) {
            ...
            if (_.size(collectionField) === 1) {
                ...
                if (model.has(cf)) {
                    ...
                    if (!(nestedCollection instanceof DOTB.App.data.beanCollection)) {
 ...
 */
SEC.parser = new DOTB.expressions.ExpressionParser();
 ...
 */
DOTB.forms.Dependency = function(trigger, actions, falseActions, testOnLoad, context)
{
     ...
     * @return {DOTB.forms.Dependency} Dependency object created from metadata
     */
DOTB.forms.Dependency.fromMeta = function(meta, context) {
    ...
    _.each(actions, function(actionDef) {
        if (!actionDef.action || !DOTB.forms[actionDef.action + 'Action'])
            return;
        ...
        if (actionDef.action === 'SetValue' && actionDef.params.target) {
            addAction = DOTB.App.acl.hasAccess('edit', this.model.module, undefined, actionDef.params.target);
        }

        if (addAction) {
            actionObjects.push(new DOTB.forms[actionDef.action + 'Action'](actionDef.params));
        }
    }, context);
    _.each(falseActions, function(actionDef) {
        ...
        if (addAction) {
            falseActionObjects.push(new DOTB.forms[actionDef.action + 'Action'](actionDef.params));
        }

    }, context);

    return new DOTB.forms.Dependency(
        new DOTB.forms.Trigger(triggerFields, condition, context, relatedFields),
     ...
     * @return {boolean}
     */
    DOTB.forms.Dependency.prototype.fireLinkOnlyDependency = function()
    {
        if (!this.testOnLoad) {
            return false;
        }
 ...
 */
DOTB.forms.Dependency.prototype.fire = function(undo)
{
	...
	try {
        ...
        if (actions instanceof DOTB.forms.AbstractAction)
            actions = [actions];
		...
		if (!DOTB.isIE && console && console.log){
			console.log('ERROR: ' + e);
		}
		return;
	}
...
DOTB.forms.Dependency.prototype.getRelatedFields = function() {
    ...
    DOTB.forms.AbstractAction = function (target) {
        this.target = target;
    };

    DOTB.forms.AbstractAction.prototype.exec = function () {

    }

    DOTB.forms.AbstractAction.prototype.setContext = function (context) {
        this.context = context;
    }

    DOTB.forms.AbstractAction.prototype.evalExpression = function (exp, context) {
     ...
     * @return {Boolean}
     */
    DOTB.forms.AbstractAction.prototype.canSetValue = function (context) {
     ...
     * @constructor
     */
    DOTB.forms.Trigger = function (variables, condition, context, relatedFields) {
     ...
     * @private
     */
    DOTB.forms.Trigger.prototype._findRelatedCollection = function(link_name, model)
    {
     ...
     * FIXME: Listeners attached here should be disposed by dotblogic itself.
     * Currently the disposing of listeners relies on context.clear()
     * from lumia.
     */
    DOTB.forms.Trigger.prototype._attachListeners = function() {
        if (!(this.variables instanceof Array)) {
            this.variables = [this.variables];
        }

        for (var i = 0; i < this.variables.length; i++) {
            this.context.addListener(this.variables[i], DOTB.forms.Trigger.fire, this, true);
        }
    ...
    DOTB.forms.Trigger.prototype._attachCollectionListeners = function(model, triggerLink, relModel) {
        ...
        var colChangeCallback = _.bind(function(relModel, value, event) {
            ...
            DOTB.forms.Trigger.fire.call(this, relModel, null, {
                fromRelated: true,
            ...
            if (varContextCollection instanceof Backbone.Collection) {
                ...
                model.once('change:' + varContextCollection, function(parentModel, changedRelatedModel) {
                    ...
                    if (_.isObject(changedRelatedModel) && _.intersection(_.keys(changedRelatedModel.changed), relFields).length > 0) {
                        DOTB.forms.Trigger.fire.call(this, model);
                    }
                }, this);
                return varContextCollection;
            }
     ...
     * the condition to be re-evaluated again.
     */
    DOTB.forms.Trigger.prototype.setDependency = function (dep) {
        this.dependency = dep;
        this._attachListeners();
    }

    DOTB.forms.Trigger.prototype.setContext = function (context) {
        this.context = context;
    }
     ...
     */
    DOTB.forms.Trigger.fire = function (model, value, options) {
        options = options || {};
        // eval the condition
        ...
        if (val == DOTB.expressions.Expression.TRUE) {
            // single dependency
            if (this.dependency instanceof DOTB.forms.Dependency) {
                this.dependency.fire(false);
            }
        } else if (val == DOTB.expressions.Expression.FALSE) {
            // single dependency
            if (this.dependency instanceof DOTB.forms.Dependency) {
    ...
    DOTB.forms.flashInProgress = {};
    DOTB.forms.markedField = {};
    DOTB.forms.exclamationMarkTemplate = Handlebars.compile(
     ...
     */
    DOTB.forms.FlashField = function (field, to_color, key) {
        if (typeof(field) == 'undefined' || (!key && !field.id))
            return;
        key = key || field.id;
        if (DOTB.forms.flashInProgress[key])
            return;

        DOTB.forms.flashInProgress[key] = true;
        ...
        $(field).animate({
            ...
            $(field).animate({
                ...
                delete DOTB.forms.flashInProgress[key];
            });
        });
    };
     ...
     */
    DOTB.forms.markField = function(key, el, text) {
        if (DOTB.forms.markedField[key])
            return;
        ...
        // insert tooltip
        var $tooltip = $(DOTB.forms.exclamationMarkTemplate(text));
        ...
        DOTB.forms.markedField[key] = $tooltip;
    };
     ...
     */
    DOTB.forms.unmarkField = function(key, el) {
        if (!DOTB.forms.markedField[key])
            return;
        ...
        // remove tooltip
        DOTB.forms.markedField[key].remove();

        DOTB.forms.markedField[key] = null;
    };

    SE.plugin = {
        onAttach: function() {
            this.on('init', function() {
                this._slCtx = this.initDotbLogic();
                /**
                 * Reference to the `DotbLogic` plugin context on the view.
                 *
                 * @deprecated Deprecated since 7.9.
                 * @type {DOTB.expressions.ExpressionContext}
                 */
                Object.defineProperty(this, 'slContext', {
                    get: function () {
                        DOTB.App.logger.warn('`View.slContext` has been deprecated since 7.9.');
                        return this._slCtx;
                    },
                    configurable: true,
                    enumerable: true,
        ...
        /**
         * Init DotbLogic on a passed in model or the default one to the view that this plugin is attached to
         *
         * @param {Data.Bean} [model] The Model we want to enable DotbLogic on
         ...
         * @return {DOTB.expressions.LumiaExpressionContext}
         */
        initDotbLogic: function(model, dependencies, isCreate) {
            // set the defaults if they are not already set
            model = model || this.model;
            this._dependencies = dependencies || this.getApplicableDeps();
            isCreate = isCreate || false;
            var slContext = new DOTB.expressions.LumiaExpressionContext(this, model, this.collection);
        ...
        /**
         * Gets the list of dependent/calculated fields with DotbLogic
         * expressions, to be included in the list of fields fetched by the
         * view's context.
         *
         * @protected
         * @return {Array} The list of DotbLogic fields.
         */
        ...
        getApplicableDeps: function() {
            var meta = _.extend({}, this.meta, this.options.meta),
                ...
                modDeps = DOTB.App.metadata.getModule(this.context.get("module"), "dependencies"),
                // FIXME dotbplugin shouldn't depend on info from other plugins...
        ...
        /**
         * Unbinds event listeners bound by LumiaExpressionContext class.
         */
        onDetach: function () {
    ...
    //Register DotbLogic as a plugin to lumia.
    if (DOTB.App && DOTB.App.plugins) {
        DOTB.App.plugins.register('DotbLogic', 'view', SE.plugin);
