...
class DotbMinifyUtils
{
    ...
    public function ConcatenateFiles($from_path){
        ...
        foreach ($js_groupings as $fg) {
            ...
            dotb_file_put_contents_atomic($trgt, $contents);

            // And handle permissions like the way we used to do it
            $func = function_exists('dotb_chmod') ? 'dotb_chmod' : 'chmod';

            // Get a default permission, from config if possible
            if (isset($dotb_config['default_permissions']['file_mode'])) {
                $defaultPerm = $dotb_config['default_permissions']['file_mode'];
            } else {
                $defaultPerm = 0777;
            }
    ...
    protected function create_backup_folder($bu_path){
        ...
        foreach($bu_dir_arr as $bu_dir){
            if(!file_exists($prefix_process_path.'/'.$bu_dir)){
                if(function_exists('dotb_mkdir')){
                    dotb_mkdir($prefix_process_path.'/'.$bu_dir);
                }else{
        ...
        if(isset($from_path) && !empty($from_path)&&isset($to_path) && !empty($to_path)){
            ...
            if(file_exists($from_path) && is_file($from_path)){
                ...
                if(function_exists('dotb_fopen')){
                    $file_handle = dotb_fopen($from_path, 'r');
                }else{
                ...
                if(is_file($min_file)) {
                    ...
                    try {
                        $out = $lic_str . DotbMin::minify(file_get_contents($from_path));
                    } catch (RuntimeException  $e) {
                        $out = file_get_contents($from_path);
                    }
                }

            	if(function_exists('dotb_fopen') && $fh = @dotb_fopen( $to_path, 'w' ) )
			    {
			        fputs( $fh, $out);
			        fclose( $fh );
				} else {
