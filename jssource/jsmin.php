...
class DotbMin {
    ...
    protected function jsParser() {
        ...
        if(!empty($GLOBALS['dotb_config']['uglify'])){
            $descriptorspec = array(
               0 => array("pipe", "r"),  // stdin is a pipe that the child will read from
               1 => array("pipe", "w"),  // stdout is a pipe that the child will write to
            );
            $process = proc_open($GLOBALS['dotb_config']['uglify'], $descriptorspec, $pipes);
    ...
    public static function isMinifyFast() {
        return self::isJSMinExtAvailable()  || !empty($GLOBALS['dotb_config']['uglify']);
    }
}
