...
if (!defined('DOTB_BASE_DIR')) {
    define('DOTB_BASE_DIR', str_replace('\\', '/', dirname(__DIR__)));
}
...
if(isset($_REQUEST['root_directory'])){
    ...
    if(!function_exists('dotb_cached')) {
        if ($argv[1] != '-?') {
            require_once($from.'/./include/utils.php');
            require_once($from.'/./include/utils/file_utils.php');
            require_once($from.'/./include/utils/dotb_file_utils.php');
            require_once($from.'/./include/dir_inc.php');
        }
        if(!function_exists('dotb_cached')) {
            function dotb_cached($dir) { return "cache/$dir"; }
        }
    }
    ...
    <root path> = path of directory to process.  Should be root of dotb instance.
    ...
    *** note that options are mutually exclusive.  You would use -r OR -m OR -c
    ...
    examples: say your patch is located in 'c:/dotb'
