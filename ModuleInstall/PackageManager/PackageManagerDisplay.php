...
class PackageManagerDisplay{
   ...
   /**
    ...
    public static function buildPackageDisplay(
       ...
       $mod_strings = return_module_language($current_language, "Administration");
        ...
        $show_login = false; //hiding install from dotb
   ...
   $str = $ss->fetch('ModuleInstall/PackageManager/tpls/PackageForm.tpl');
    ...
    public static function buildPatchDisplay(
        ...
        $form2 .= "</td><td align='right'><div id='workingStatusDiv' style='display:none;'>".DotbThemeRegistry::current()->getImage("sqsWait","border='0' align='bottom'",null,null,'.gif',"Loading")."</div></td></tr><tr><td colspan='2'>";
    ...
    function buildLoginPanel($mod_strings, $display_cancel){
        ...
        $output .= "<div class='bd'><form><table><tr><td>".$mod_strings['LBL_USERNAME']."</td><td><input type='text' name='login_panel_username' id='login_panel_username' value='".$credentials['username']."'></td><td><a href='http://www.dotbcrm.com/crm/index.php?option=com_registration&task=register' target='blank'>".$mod_strings['LNK_NEW_ACCOUNT']."</a></td>";

        $output .= "</tr><tr><td>".$mod_strings['LBL_PASSWORD']."</td><td><input type='password' name='login_panel_password' id='login_panel_password'></td><td><a href='http://www.dotbcrm.com/crm/component/option,com_registration/Itemid,0/task,lostPassword/' target='blank'>".$mod_strings['LNK_FORGOT_PASS']."</a></td>";
		...
		$output .= "</tr><tr><td colspan='6' valign='top'><b>".$mod_strings['LBL_TERMS_AND_CONDITIONS']."</b><br><textarea readonly cols=80 rows=8>" . $terms['terms'] . '</textarea></td>';
       	$_SESSION['DotbDepot_TermsVersion'] = (!empty($terms['version']) ? $terms['version'] : '');
    ...
    public static function getLicenseDisplay($license_file, $form_action, $next_step, $zipFile, $type, $manifest, $modify_field){
    	global $current_language;
        $mod_strings = return_module_language($current_language, "Administration");
        $contents = dotb_file_get_contents($license_file);
        ...
        $display .= "</td>";
        $display .= "<td>";
        $display .= "<slot><a class=\"listViewTdToolsS1\" id='href_animate' onClick=\"PackageManager.toggleLowerDiv('span_animate_div_$div_id', 'span_license_div_$div_id', 350, 0);\"><span id='span_animate_div_$div_id'<img src='".DotbThemeRegistry::current()->getImageURL('advanced_search.gif')."' width='8' height='8' alt='Advanced' border='0'>&nbsp;Expand</span></a></slot></td>";
        $display .= "</td>";
        $display .= "</tr>";
        $display .= "</table>";
    ...
    protected static function getDisplayScript($install = false, $type = 'module', $releases = null, $types = array(), $isAlive = true)
    {
        ...
        $ss->assign('WAIT_IMAGE', DotbThemeRegistry::current()->getImage("loading","border='0' align='bottom'",null,null,'.gif',"Loading"));

        $ss->assign('dotb_version', $dotb_version);
        $ss->assign('js_custom_version', $dotb_config['js_custom_version']);
		...
		$ss->assign('SHOW_IMG', DotbThemeRegistry::current()->getImage('advanced_search', 'border="0"', 8, 8, '.gif', 'Show'));
		$ss->assign('HIDE_IMG', DotbThemeRegistry::current()->getImage('basic_search', 'border="0"', 8, 8, '.gif', 'Hide'));
