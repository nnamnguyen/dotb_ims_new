...
use Dotbcrm\Dotbcrm\Security\Validator\Constraints\DropdownList as ConstraintsDropdownList;
...
class ModuleScanner{
	...
	private $validExt = array('png', 'gif', 'jpg', 'css', 'js', 'php', 'txt', 'html', 'htm', 'tpl', 'pdf', 'md5', 'xml', 'hbs', 'less', 'wsdl');
	...
	'dotb_touch',
        ...
        // dotb vulnerable functions, need to be lower case
        'getfunctionvalue',
);
    private $methodsBlackList = array('setlevel', 'put' => array('dotbautoloader'), 'unlink' => array('dotbautoloader'));
    ...
    public function __construct()
    {
        ...
        if (!$disableConfigOverride && !empty($GLOBALS['dotb_config']['moduleInstaller'])) {
            $this->config = $GLOBALS['dotb_config']['moduleInstaller'];
        }
     ...
     * @param string $dotbFileAllowed whether should allow to override core files
     **/
    public function scanDir($path, $dotbFileAllowed = true)
    {
        static $startPath = '';
        if (empty($startPath)) {
            $startPath = $path;
        ...
        while ($e = $d->read()) {
            ...
            if (is_dir($next)) {
                ...
                $this->scanDir($next, $dotbFileAllowed);
            } else {
                $issues = $this->scanFile($next, $dotbFileAllowed);
	 ...
	 *
	 */
        public function scanFile($file, $dotbFileAllowed = true)
        {
		$issues = array();
        ...
        if (!$dotbFileAllowed) {
            ...
            if ($this->dotbFileExists($fileNoBase)) {
                $issues[] = translate('ML_OVERRIDE_CORE_FILES');
                $this->issues['file'][$file] = $issues;
                return $issues;
            }
    ...
    /**
     * checks files.md5 file to see if the file is from dotb
     * ONLY WORKS ON FILES
     *
     * @param string $path
     * @return bool
     */
    public function dotbFileExists($path)
    {
        static $md5 = array();
	...
	/**
	 *This function will scan the Manifest for disabled actions specified in $GLOBALS['dotb_config']['moduleInstaller']['disableActions']
	 *if $GLOBALS['dotb_config']['moduleInstaller']['disableRestrictedCopy'] is set to false or not set it will call on scanCopy to ensure that it is not overriding files
	 */
     ...
     * and ensures that there is no official dotb file there.
     * If the file exists it will check
     * against the MD5 file list to see if Dotb Created the file
    ...
    public function scanCopy($from, $to)
    {
        ...
        // if the $to is a file and it is found in dotbFileExists then don't allow overriding it
        if (is_file($to) && $this->dotbFileExists($to)) {
	...
	public function displayIssues($package='Package'){
		global $dotb_version, $dotb_flavor;
...
'</ul></p><br>' . ' <a href="http://www.dotbcrm.com/crm/product_doc.php?module=FailPackageScan&version=' . $dotb_version . '&edtion=' . $dotb_flavor . '" target="_blank">' . translate('ML_PKG_SCAN_GUIDE') . '</a>'.'<br><br>';
	...
	public function lockConfig()
	{
	    if(empty($this->config_hash)) {
	        $this->config_hash = md5(serialize($GLOBALS['dotb_config']));
	    }
	}
	...
	{
	    $config_hash_after = md5(serialize($GLOBALS['dotb_config']));
