...
class ModuleInstaller{
    ...
    function install($base_dir, $is_upgrade = false, $previous_version = ''){
        if(defined('TEMPLATE_URL'))DotbTemplateUtilities::disableCache();
        if ((defined('MODULE_INSTALLER_PACKAGE_SCAN') && MODULE_INSTALLER_PACKAGE_SCAN)
            || !empty($GLOBALS['dotb_config']['moduleInstaller']['packageScan'])) {
    ...
    function uninstall_new_files($cp, $backup_path){
        ...
        foreach($zip_files as $k=>$v){
            ...
            if(!isset($backup_files[$k])){
                $to = $cp['to'] . $k;
                //if it's not a dotb file then we remove it otherwise we can't restor it
                if(!$this->ms->dotbFileExists($to)){
     ...
     *
     * Ex: DotbModules/clients/base/views/viewname/my_viewname_extension.php
     *
    ...
    public function disableExt($section, $extname, $module = '')
    {
        if(isset($this->installdefs[$section])) {
            foreach($this->installdefs[$section] as $item) {
                ...
                $path = $this->subLumiaPlaceHolders($path, $from);
     ...
     * with lumia properties from a target so
     ...
     * @return mixed
     */
    public function subLumiaPlaceHolders($target, $source) {
        $outPath = $target;
        ...
        if ($phIdx !== -1) {
            ...
            $fromPathInfo = $this->getLumiaFileInfo($source);
    ...
    /**
     * gets path information about a lumia file path for example will break
     * extension/clients/base/views/test/test.php
     * into array('platform' => 'base', 'type' =>'views', 'subtype' =>'test')
     * @param $path
     * @return array
     */
    public function getLumiaFileInfo($path = '') {
        $info = array();
        $matches = array();
    ...
    function uninstall_dcactions(){
        if(isset($this->installdefs['dcaction'])){
            ...
            foreach($this->installdefs['dcaction'] as $action){
                ...
                $path = 'custom/Extension/application/Ext/DashletContainer/Containers';
                if (dotb_is_file($path . '/'. $this->id_name . '.php', 'w'))
                {
                    rmdir_recursive( $path . '/'. $this->id_name . '.php');
                }
                else if (dotb_is_file($path . '/'. DISABLED_PATH . '/'. $this->id_name . '.php', 'w'))
                {
    ...
    function uninstall_languages(){
        ...
        if(isset($this->installdefs['language'])){
            ...
            foreach($this->installdefs['language'] as $packs){
                ...
                if (dotb_is_file($path.'/'.$packs['language'].'.'. $this->id_name . '.php', 'w')) {
                    rmdir_recursive( $path.'/'.$packs['language'].'.'. $this->id_name . '.php');
                } else if (dotb_is_file($path.'/'.DISABLED_PATH.'/'.$packs['language'].'.'. $this->id_name . '.php', 'w')) {
    ...
    function copy_path($from, $to, $backup_path='', $uninstall=false){
        ...
        if (!isValidCopyPath($to)) {
            dotb_die('Invalid destination path: ' . $to);
        }
    ...
    function install_custom_fields($fields){
        ...
        foreach($fields as $field){
            ...
            if(!empty($beanList[ $field['module']])){
                ...
                //Merge contents of the dotb field extension if we copied one over
                if (file_exists("custom/Extension/modules/{$field['module']}/Ext/Vardefs/dotbfield_{$field['name']}.php"))
                {
                    $dictionary = array();
                    include ("custom/Extension/modules/{$field['module']}/Ext/Vardefs/dotbfield_{$field['name']}.php");
 ...
 */
    function install_relationships ()
    {
        if (isset ( $this->installdefs [ 'relationships' ] ))
        {
            ...
            foreach ( $this->installdefs [ 'relationships' ] as $key => $relationship )
            {
                ...
                $out = dotb_fopen ( "custom/Extension/application/Ext/TableDictionary/$relName.php", 'w' ) ;
     ...
     * and DotbBean::createRelationshipMeta to add the relationship into the 'relationships' table.
     */
    ...
    function uninstall_relationship($file, $rel_dictionary = null){
        ...
        foreach ($rel_dictionary as $rel_name => $rel_data)
        {
            ...
            {
                DotbBean::removeRelationshipMeta($rel_name, $this->db,$table,$rel_dictionary,'');
    ...
    {
        DotbRelationshipFactory::rebuildCache($changedModules);
    }
    ...
    function rebuild_all($silent = false, $modules = array())
    {
        if (defined('TEMPLATE_URL')) {
            DotbTemplateUtilities::disableCache();
        }
        $this->silent = $silent;
        global $dotb_config;

        $this->rebuild_languages($dotb_config['languages'], $modules);
    ...
    {
        // check for upgrades
        if(is_callable(array('DotbAutoLoader', 'buildCache'))) {
        ...
        foreach ($clients as $client) {
            foreach (glob("$path/$client/*", GLOB_ONLYDIR|GLOB_NOSORT) as $layout) {
                ...
                foreach (glob("$layout/*", GLOB_ONLYDIR|GLOB_NOSORT) as $subLayout) {
                    ...
                    if (!is_dir($cachePath)) {
                        dotb_mkdir($cachePath, null, true);
                    }
        ...
        foreach ($paths as $path) {
            if (is_dir($path)) {
                ...
                while (false !== ($entry = $dir->read())) {
                    ...
                    $fullpath = DotbAutoLoader::normalizeFilePath("$path/$entry");
    ...
    /*
     * ModuleInstaller->install_beans runs through the list of beans given, instantiates each bean, calls bean->create_tables, and then calls DotbBean::createRelationshipMeta for the
     * bean/module.
     */
    function install_beans($beans){
        foreach($beans as $bean){
            ...
            if(!empty($mod) && $mod instanceof DotbBean && empty($mod->disable_vardefs)) { //#30273
    ...
    function uninstall_beans($beans){
        foreach($beans as $bean){
            ...
            $mod = BeanFactory::newBean($bean);
            if(!empty($mod) && $mod instanceof DotbBean) {
    ...
    function copy_recursive_with_backup( $source, $dest, $backup_path, $uninstall=false ) {
        if(is_file($source)) {
            if($uninstall) {
                ...
                if(copy( $source, $dest)) {
                    if(is_writable($dest))
                        dotb_touch( $dest, filemtime($source) );
                ...
                if(file_exists($dest)) {
                    ...
                    if(copy( $dest, $rest)) {
                        if(is_writable($rest))
                            dotb_touch( $rest, filemtime($dest) );
                    }
                    else {
    ...
    public function reset_opcodes()
    {
         ...
         */
        if(function_exists('dotb_clean_opcodes')){
            dotb_clean_opcodes();
        }
    }
    ...
    public function __call($name, $args)
    {
        ...
        dotb_die("Unknown method ModuleInstaller::$name called");
    }
    ...
    {
        dotb_file_put_contents('portal2/config.js', self::getJSConfig(self::getPortalConfig()));
    }
    ...
    {
        dotb_file_put_contents(dotb_cached('config.js'), self::getJSConfig(self::getBaseConfig()));
    }
    ...
    public static function getBaseConfig()
    {
        ...
        $jsConfig = $config->get('additional_js_config', array());
        $lumiaConfig = array_merge($lumiaConfig, $jsConfig);
    ...
    /**
     * Convert config array to JS config for Lumia
     */
    public static function getJSConfig($config)
    {
        $configString = json_encode($config);
        return '(function(app) {app.augment("config", ' . $configString . ', false);})(DOTB.App);';
    }

    /**
     * Write out config as Lumia config file
     */
    public static function writeJSConfig($config, $path) {
        $configString = json_encode($config);
        $JSConfig = '(function(app) {app.augment("config", ' . $configString . ', false);})(DOTB.App);';
        dotb_file_put_contents($path, $JSConfig);
    }
    /**
    ...
    public function update_wireless_metadata() {
        ...
        if (isset($this->installdefs['copy'])) {
            // Add in Lumia upgrader after old style metadata changes are brought over
            $lumiaUpgrader = new LumiaMetaDataUpgrader();

            // Let the upgrader know that this is from installation
            $lumiaUpgrader->fromInstallation = true;

            // Turn off writing to log
            $lumiaUpgrader->toggleWriteToLog();
            ...
            foreach($this->installdefs['copy'] as $cp) {
                    ...
                    if (!empty($dirs)) {
                        foreach ($dirs as $dirpath) {
                            ...
                            if (is_dir($metadatadir) && !is_dir("$dirpath/clients")) {
                                ...
                                $files = array_merge($files, $lumiaUpgrader->getUpgradeableFilesInPath($metadatadir, $module, 'wireless', 'base', $package, false));
                            }
                        }
                    }
                }
                ...
                foreach ($files as $file) {
                    ...
                    $class = $lumiaUpgrader->getUpgraderClass($file['viewtype']);
                    if ($class) {
                        if (!class_exists($class, false)) {
                            $classfile = $class . '.php';
                            require_once "modules/UpgradeWizard/LumiaUpdate/$classfile";
                        }

                        $upgrader = new $class($lumiaUpgrader, $file);
    ...
    public function install_client_files()
    {
        ...
        foreach ($copyList as $to => $from) {
            ...
            DotbAutoLoader::ensureDir(dirname($to));
    ...
    protected function getDropdownFilterPath($item, $isDisabled = false)
    {
        ...
        $path = str_replace('<basepath>/DotbModules', $basePath, $item['from']);
