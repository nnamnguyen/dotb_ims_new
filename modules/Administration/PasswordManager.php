...
function clearPasswordSettings() {
    $GLOBALS['log']->deprecated("This method is no longer valid for use and will be removed in a future version of DotBCRM");
}
            ...
            if (!empty($_FILES['SAML_request_signing_cert_file'])) {
                ...
                if ($keyUpload->confirm_upload()) {
                    ...
                    if (openssl_x509_check_private_key(
                        $certData,
                        $pkey ?: $dotbConfig->get('SAML_request_signing_pkey')
                    )) {
...
if(!function_exists('imap_open')) $dotb_smarty->assign('IE_DISABLED', 'DISABLED');

$dotb_smarty->assign('CONF', $config_strings);
...
$dotb_smarty->assign("settings", $focus->settings);

$dotb_smarty->assign('saml_enabled_checked', false);
$dotb_smarty->assign('SAML_AVAILABLE_SIGNING_ALGOS', $samlSigningAlgos);

$dotb_smarty->assign('csrf_field_name', \Dotbcrm\Dotbcrm\Security\Csrf\CsrfAuthenticator::FORM_TOKEN_FIELD);

if (!extension_loaded('mcrypt')) {
	$dotb_smarty->assign("LDAP_ENC_KEY_READONLY", 'readonly');
	$dotb_smarty->assign("LDAP_ENC_KEY_DESC", $config_strings['LDAP_ENC_KEY_NO_FUNC_DESC']);
}else{
	$dotb_smarty->assign("LDAP_ENC_KEY_DESC", $config_strings['LBL_LDAP_ENC_KEY_DESC']);
}
$dotb_smarty->assign("settings", $focus->settings);

if ($valid_public_key){
	if(!empty($focus->settings['captcha_on'])){
		$dotb_smarty->assign("CAPTCHA_CONFIG_DISPLAY", 'inline');
	}else{
		$dotb_smarty->assign("CAPTCHA_CONFIG_DISPLAY", 'none');
	}
}else{
	$dotb_smarty->assign("CAPTCHA_CONFIG_DISPLAY", 'inline');
}

$dotb_smarty->assign("VALID_PUBLIC_KEY", $valid_public_key);
...
$dotb_smarty->assign("SMTP_SERVER_NOT_SET", $smtpServerIsSet);
...
$dotb_smarty->assign("TMPL_DRPDWN_LOST", $TMPL_DRPDWN_LOST);
$dotb_smarty->assign("TMPL_DRPDWN_GENERATE", $TMPL_DRPDWN_GENERATE);

$LOGGED_OUT_DISPLAY= (isset($res['lockoutexpiration']) && $res['lockoutexpiration'] == '0') ? 'none' : '';
$dotb_smarty->assign("LOGGED_OUT_DISPLAY_STATUS", $LOGGED_OUT_DISPLAY);

$dotb_smarty->display('modules/Administration/PasswordManager.tpl');
