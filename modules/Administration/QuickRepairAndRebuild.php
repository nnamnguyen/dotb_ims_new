...
/**
 * Class for handling repairing of the dotb installation and rebuilding of caches
 */
class RepairAndClear
{
        ...
        foreach ($this->actions as $current_action)
            switch ($current_action) {
                ...
                case 'resetForecasting':
                    ...
                    break;
                case 'rebuildJSCacheFiles':
                    $this->rebuildJSCacheFiles();
                    break;
                case 'collectionLanguagesToOneFiles':
                    $this->clearJsLangFiles();
                    $this->rebuildExtensions();
                    $this->collectionLanguagesToOneFiles();
                    break;
                case 'repairConfigs':
    ...
    function getListModuleHasLanguages()
    {
        $data = array();
    ...
    function deleteExtLang($module, $lang)
    {
        $files = scandir('custom/Extension/modules/' . $module . '/Ext/Language');

        foreach ($files as $file)
            if (is_file('custom/Extension/modules/' . $module . '/Ext/Language/' . $file))
                if (preg_match('/^' . $lang . '/', $file))
                    unlink('custom/Extension/modules/' . $module . '/Ext/Language/' . $file);
    }

    function deleteCustomLang($module, $lang)
    {
        $files = scandir('custom/modules/' . $module . '/language');

        foreach ($files as $file)
            if (is_file('custom/modules/' . $module . '/language/' . $file))
                if (preg_match('/^' . $lang . '/', $file))
                    unlink('custom/modules/' . $module . '/language/' . $file);
    }

    public function collectionLanguagesToOneFiles()
    {
        echo "<h1>Repairing languages...</h1><br/>";
        $modules = $this->getListModuleHasLanguages();
        foreach ($modules as $module) {
            $langVN = return_module_language('vn_vn', $module);
            $langEN = return_module_language('en_us', $module);
            $langVNList = return_mod_list_strings_language('vn_vn', $module);
            $langENList = return_mod_list_strings_language('en_us', $module);

            $langVNr = array_merge($langEN, $langVN);
            ksort($langVNr);
            $langVNListr = array_merge($langENList, $langVNList);
            ksort($langVNListr);

            $langENr = array_merge($langVN, $langEN);
            ksort($langENr);
            $langENListr = array_merge($langVNList, $langENList);
            ksort($langENListr);

            $strVN = "<?php";
            if (count($langVNListr) > 0) $strVN .= "\n\$mod_list_strings=" . var_export($langVNListr, true) . ';';
            if (count($langVNr) > 0) $strVN .= "\n\n\$mod_strings=" . var_export($langVNr, true) . ';';
            file_put_contents('modules/' . $module . '/language/vn_vn.lang.php', $strVN);
            $this->deleteExtLang($module, 'vn_vn');
            $this->deleteCustomLang($module, 'vn_vn');

            $strEN = "<?php";
            if (count($langENListr) > 0) $strEN .= "\n\$mod_list_strings=" . var_export($langENListr, true) . ';';
            if (count($langENr) > 0) $strEN .= "\n\n\$mod_strings=" . var_export($langENr, true) . ';';
            file_put_contents('modules/' . $module . '/language/en_us.lang.php', $strEN);
            $this->deleteExtLang($module, 'en_us');
            $this->deleteCustomLang($module, 'en_us');
        }
        echo "Done!";
    }
    ...
    /**
     * Rebuilds the base Lumia configuration file.
     */
    ...
    public function repairDatabaseSelectModules()
    {
        ...
        if (is_admin($current_user) || is_admin_for_any_module($current_user)) {
            ...
            if ($this->module_list && !in_array($mod_strings['LBL_ALL_MODULES'], $this->module_list)) {
                ...
                //repair DB
                $dm = inDeveloperMode();
                $GLOBALS['dotb_config']['developerMode'] = true;
                $GLOBALS['reload_vardefs'] = true;
                ...
                $GLOBALS['dotb_config']['developerMode'] = $dm;
    ...
    public function clearSmarty()
    {
        ...
        $this->_clearCache(dotb_cached('smarty/templates_c'), '.tpl.php');
    }
    ...
    public function clearXMLfiles()
    {
        ...
        $this->_clearCache(dotb_cached("xml"), '.xml');
    }
    ...
    public function clearDashlets()
    {
        ...
        $this->_clearCache(dotb_cached('dashlets'), '.php');
    }
    ...
    public function clearThemeCache()
    {
        ...
        DotbThemeRegistry::clearAllCaches();

        //Clear Lumia Themes CSS files
        $this->_clearCache(dotb_cached('themes/clients/'), '.css');
    }
    ...
    public function clearTpls()
    {
        ...
        if (!in_array(translate('LBL_ALL_MODULES'), $this->module_list) && !empty($this->module_list)) {
            foreach ($this->module_list as $module_name) {
                $this->_clearCache(dotb_cached('modules/' . $module_name), '.tpl');
            }
        } else
            $this->_clearCache(dotb_cached('modules/'), '.tpl');
    }
    ...
    public function clearVardefs()
    {
        ...
        if (!empty($this->module_list) && is_array($this->module_list) && !in_array(translate('LBL_ALL_MODULES'), $this->module_list)) {
            foreach ($this->module_list as $module_name) {
                $this->_clearCache(dotb_cached('modules/' . $module_name), 'vardefs.php');
            }
        } else
            $this->_clearCache(dotb_cached('modules/'), 'vardefs.php');
    }
    ...
    public function clearJsFiles()
    {
        ...
        if (!in_array(translate('LBL_ALL_MODULES'), $this->module_list) && !empty($this->module_list)) {
            foreach ($this->module_list as $module_name) {
                $this->_clearCache(dotb_cached('modules/' . $module_name), '.js');
            }
        } else {
            $this->_clearCache(dotb_cached('modules/'), '.js');
        }
        $this->_clearCache(dotb_cached('themes/'), '.js');
    }
    ...
    public function clearJsLangFiles()
    {
        ...
        if (!in_array(translate('LBL_ALL_MODULES'), $this->module_list) && !empty($this->module_list)) {
            foreach ($this->module_list as $module_name) {
                $this->_clearCache(dotb_cached('jsLanguage/' . $module_name), '.js');
            }
        } else
            $this->_clearCache(dotb_cached('jsLanguage'), '.js');
    }
    ...
    public function clearLanguageCache()
    {
        ...
        if (!empty($GLOBALS['dotb_config']['languages'])) {
            $languages = $GLOBALS['dotb_config']['languages'];
        } else {
            $languages = array($GLOBALS['current_language'] => $GLOBALS['current_language']);
        }
        foreach (array_keys($languages) as $language) {
            dotb_cache_clear('app_strings.' . $language);
            dotb_cache_clear('app_list_strings.' . $language);
        }
    ...
    public function clearSearchCache()
    {
        ...
        // clear dotb_cache backend for DotbSearchEngine
        DotbSearchEngineMetadataHelper::clearCache();
    ...
    public function clearAdditionalCaches()
    {
        ...
        $this->_clearCache(dotb_cached('include/javascript/'), '.js');
    }
    ...
    public function clearMetadataAPICache()
    {
        ...
        foreach ($this->module_list as $module_name) {
            $this->_clearCache(dotb_cached('modules/' . $module_name . '/clients'), '.php');
        }
    }
