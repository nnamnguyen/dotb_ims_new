...
$hidden_fields .= smarty_function_dotb_csrf_form_token(array(), $smarty);
...
if (((defined('MODULE_INSTALLER_PACKAGE_SCAN') && MODULE_INSTALLER_PACKAGE_SCAN)
    ...
    // package types that should not override dotb core files
    $cantOverrideDotbFilePackTypes = array('langpack');
    $ms->scanPackage($unzip_dir, !in_array($install_type, $cantOverrideDotbFilePackTypes));
	if($ms->hasIssues()){
...
if( $show_files == true ){
	...
	$new_dotb_mod_files = array();

	$cache_html_files = findAllFilesRelative( dotb_cached("layout"), array());
	...
	foreach($new_files as $the_file) {
		...
		array_push($new_dotb_mod_files, $the_file);
	}
    ...
    echo '<div style="text-align: left; cursor: hand; cursor: pointer; text-decoration: underline;'.(($mode == 'Enable' || $mode == 'Disable')?'display:none;':'').'" onclick=\'this.style.display="none"; toggleDisplay("more");\'id="all_text">
        '.DotbThemeRegistry::current()->getImage('advanced_search', '', null, null, ".gif", $mod_strings['LBL_ADVANCED_SEARCH']).$mod_strings['LBL_UW_SHOW_DETAILS'].'</div><div id=\'more\' style=\'display: none\'>
         ...
         .DotbThemeRegistry::current()->getImage('basic_search', '', null, null, ".gif", $mod_strings['LBL_BASIC_SEARCH']).$mod_strings['LBL_UW_HIDE_DETAILS'].'</div><br>';
	...
	echo '<ul>';
	foreach( $new_dotb_mod_files as $the_file ){
		$highlight_start    = "";
		$highlight_end      = "";
		$checked            = "";
		$disabled           = "";
