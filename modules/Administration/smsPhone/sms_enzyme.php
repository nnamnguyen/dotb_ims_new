...
class sms_enzyme {
	...
	function get_custom_phone_field() {
		$this->set_variables();
		$path = "./custom/Extension/modules/{$this->module}/Ext/Vardefs/vardefs.php";
		$result = NULL;
		if (file_exists($path)) {
			include($path);
			$object = $this->module_sing=='aCase' ? 'Case' : $this->module_sing;
			if (isset($dictionary[$object]['fields'])) {
				foreach($dictionary[$object]['fields'] as $key=>$val) {
					if ($val['enableSMS'] == true){
						$result = $key;
						break;
					}
				}
	...
	function get_modules_with_sms() {
		$this->set_variables();
		$arr = array();
		$path = "./custom/Extension/modules/{$this->module}/Ext/Vardefs/";
		$file = "sms_fields_for_" . strtolower($this->module) . ".php";
		if (file_exists($path.$file)) {
			include($path.$file);
			if (isset($dictionary)) $arr = array_keys($dictionary);
		} 
		return $arr;
	}
