...
{
    return DotbAutoLoader::ensureDir("custom/include/language");
}
...
{
    return DotbAutoLoader::ensureDir("custom/modules/$module/language");
}
    ...
    foreach ($languages as $current_lang => $current_lang_name) {
        ...
        foreach ($new_dd_strings as $list_key => $list_val) {
            ...
            if (isset($app_list_strings[$list_key]) && $app_list_strings[$list_key] != $new_dd_strings[$list_key]) {
                ...
                $extFilename = "custom/Extension/application/Ext/Language/$current_lang.dotb_" .
                    $list_key . '.php';
