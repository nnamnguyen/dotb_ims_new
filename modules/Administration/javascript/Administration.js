	...
	RepairXSS : {
		...
		refreshEstimate : function(select) {
			...
			if(selected != "0") {
				...
				AjaxObject.startRequest(callbackRepairXssRefreshEstimate, "&adminAction=refreshEstimate&bean=" + selected + '&csrf_token=' + DOTB.csrf.form_token);
			} else {
		...
		executeRepair : function() {
			if(this.toRepair) {
				...
				AjaxObject.startRequest(callbackRepairXssExecute, "&adminAction=repairXssExecute&bean=" + this.currentRepairObject + "&id=" + beanId + '&csrf_token=' + DOTB.csrf.form_token);
			}
		},
