...
foreach( $GLOBALS['subdirs'] as $subdir ){
    if(!file_exists("$base_upgrade_dir/$subdir")) {
        dotb_mkdir("$base_upgrade_dir/$subdir", 0770, true);
    }
}
        ...
        if ($releaseId != "") {
            ...
            if( empty( $_FILES['upgrade_zip']['tmp_name'] ) ){
                ...
                if(!$upload->confirm_upload() ||
                    ...
                    dotb_die($mod_strings['LBL_UPGRADE_WIZARD_INVALID_PKG']);
            	} else {
...
$csrfToken = smarty_function_dotb_csrf_form_token(array(), $smarty);

// upload link
if(!empty($GLOBALS['dotb_config']['use_common_ml_dir']) && $GLOBALS['dotb_config']['use_common_ml_dir'] && !empty($GLOBALS['dotb_config']['common_ml_dir'])){
	//rrs
	$form = '<form name="move_form" action="index.php?module=Administration&view=module&action=UpgradeWizard" method="post"  ><input type=hidden name="run" value="upload" /><input type=hidden name="load_module_from_dir" id="load_module_from_dir" value="'.$GLOBALS['dotb_config']['common_ml_dir'].'" /><input type=hidden name="upgrade_zip_escaped" value="" />';
    $form .= $csrfToken;
	...
	if ($handle = opendir($GLOBALS['dotb_config']['common_ml_dir'])) {
