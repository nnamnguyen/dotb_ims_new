...
function cleanPreviousInstall($idName, $unzipDir)
{
    ...
    $file = $pvUnzipDir . DIRECTORY_SEPARATOR . constant('DOTBCRM_PRE_UNINSTALL_FILE');
    if (is_file($file)) {
...
if(strpos($install_file,'upload://') === 0){
    ...
    $upload_dir = empty($GLOBALS['dotb_config']['upload_dir']) ? 'upload' : rtrim($GLOBALS['dotb_config']['upload_dir'], '/\\');
...
if (((defined('MODULE_INSTALLER_PACKAGE_SCAN') && MODULE_INSTALLER_PACKAGE_SCAN)
    || !empty($GLOBALS['dotb_config']['moduleInstaller']['packageScan'])) && $install_type != 'patch') {
...
if($install_type == 'patch' || $install_type == 'module')
{
	switch($mode)
 	{
 		case 'Install':
 			$file = "$unzip_dir/" . constant('DOTBCRM_PRE_INSTALL_FILE');
			if(is_file($file))
			{
 			...
 			break;
 		case 'Uninstall':
 			$file = "$unzip_dir/" . constant('DOTBCRM_PRE_UNINSTALL_FILE');
			if(is_file($file))
			{
				print("{$mod_strings['LBL_UW_INCLUDING']}: $file <br>\n");
				include FileLoader::validateFilePath($file);
...
for( $iii = 0; $iii < $copyCount; $iii++ ){
    if( isset($_REQUEST["copy_" . $iii]) && ($_REQUEST["copy_" . $iii] != "") ){
        ...
        $dotb_home_dir = getCwd();
        $dest_file  = clean_path( "$dotb_home_dir/$zip_to_dir/$file_to_copy" );
        if($zip_to_dir != '.')
        ...
        switch( $mode ){
            case "Install":
                ...
                if($install_type=="patch" && is_file($dest_file))
                {
	                ...
	                dotb_touch( $rest_file, filemtime($dest_file) );
                }
            ...
            case "Uninstall":
                if($install_type=="patch" && is_file($rest_file))
                {
	                ...
	                dotb_touch( $dest_file, filemtime($rest_file) );
                }
...
switch( $install_type ){
    case "langpack":
        ...
        if( $mode == "Install" || $mode=="Enable" ){
            $dotb_config['languages'] = $dotb_config['languages'] + array( $newLangName => $newLangDesc );
        }
	        ...
	        $default_dotb_instance_lang = 'en_us';
	        if($current_language == $newLangName){
	        	$_SESSION['authenticated_user_language'] =$default_dotb_instance_lang;
	        	$lang_changed_string = $mod_strings['LBL_CURRENT_LANGUAGE_CHANGE'].$dotb_config['languages'][$default_dotb_instance_lang].'<br/>';
	        }

	        if($dotb_config['default_language'] == $newLangName){
	        	$cfg = new Configurator();
             	$cfg->config['languages'] = $new_langs;
				$cfg->config['default_language'] = $default_dotb_instance_lang;
				$cfg->handleOverride();
	        	$lang_changed_string .= $mod_strings['LBL_DEFAULT_LANGUAGE_CHANGE'].$dotb_config['languages'][$default_dotb_instance_lang].'<br/>';
	        }
        }
        ksort( $dotb_config );
 		...
 		switch($mode)
 		{
 			case 'Install':
				...
				UWrebuild();
 				break;
 			case 'Uninstall':
 				$file = "$unzip_dir/" . constant('DOTBCRM_POST_UNINSTALL_FILE');
 				if(is_file($file)) {
					print("{$mod_strings['LBL_UW_INCLUDING']}: $file <br>\n");
					include FileLoader::validateFilePath($file);
					post_uninstall();
        	...
        	if (!empty($manifest['post_install_url']))
        	{
        		...
        		if (isset($url_conf['type']) && $url_conf['type'] == 'popup')
        		{
        			echo '<script type="text/javascript">window.open("' . $url_conf['url']
        			   . '","' . (empty($url_conf['name']) ? 'dotb_popup' : $url_conf['name']) . '","'
...
<?php
echo smarty_function_dotb_csrf_form_token(array(), $smarty);
echo "<div>";
...
if ($install_type != "module" && $install_type != "langpack"){
    if( sizeof( $files_to_handle ) > 0 ){
        echo '<div style="text-align: left; cursor: hand; cursor: pointer; text-decoration: underline;" onclick=\'this.style.display="none"; toggleDisplay("more");\' id="all_text">' . DotbThemeRegistry::current()->getImage('advanced_search', '', null, null, ".gif", $mod_strings['LBL_ADVANCED_SEARCH']) . ' Show Details</div><div id=\'more\' style=\'display: none\'>
            <div style="text-align: left; cursor: hand; cursor: pointer; text-decoration: underline;" onclick=\'document.getElementById("all_text").style.display=""; toggleDisplay("more");\'>' . DotbThemeRegistry::current()->getImage('basic_search', '', null, null, ".gif", $mod_strings['LBL_BASIC_SEARCH']) . ' Hide Details</div><br>';
