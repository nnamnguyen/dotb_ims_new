...
function executedotblog()
{
    //BEGIN COPY DOTBCRM.LOG
    //Copies the Dotbcrm log to our diagnostic directory
    global $cacheDir, $mod_strings;
	$logger = new DotbLogger();
    if(!copy($logger->getLogFileNameWithPath(), $cacheDir.'/'.$logger->getLogFileName())) {
        $data = array($cacheDir);
        echo string_format($mod_strings['LBL_DIAGNOSTICS_ERROR_DOTBLOG'], $data);
    }
    //END COPY DOTBCRM.LOG

    //UPDATING PROGRESS BAR
    sodUpdateProgressBar(DOTBLOG_WEIGHT);
}
...
function executephpinfo()
{
    ...
    $handle = dotb_fopen($cacheDir."phpinfo.html", "w");
...
function executeconfigphp()
{
    ...
    global $cacheDir;    global $dotb_config;

    $tempPass = $dotb_config['dbconfig']['db_password'];
    $dotb_config['dbconfig']['db_password'] = '********';
    //write config.php to a file
    write_array_to_file("Diagnostic", $dotb_config, $cacheDir."config.php");
    //restore db_password so everything still works
    $dotb_config['dbconfig']['db_password'] = $tempPass;
    //END COPY CONFIG.PHP
...
function executebeanlistbeanfiles()
{
	...
	ob_clean();
	$handle = dotb_fopen($cacheDir."beanFiles.html", "w");
...
{
    //BEGIN DUMP OF DOTB SCHEMA (VARDEFS)

    //END DUMP OF DOTB SCHEMA (VARDEFS)
    //BEGIN UPDATING PROGRESS BAR
    ...
    $versionData = array($dotb_version, $dotb_db_version, $dotb_flavor);
    ...
    $handle = dotb_fopen($cacheDir."vardefschema.html", "w");
...
$dodotblog = ((empty($_POST['dotblog']) || $_POST['dotblog'] == 'off') ? false : true);
...
if($dodotblog) {$totalweight += DOTBLOG_WEIGHT; $totalitems++;}
...
if($dodotblog)
{
  echo $mod_strings['LBL_DIAGNOSTIC_GETDOTBLOG']."<BR>";
  executedotblog();
  echo $mod_strings['LBL_DIAGNOSTIC_DONE']."<BR><BR>";
}
if($dovardefs)
