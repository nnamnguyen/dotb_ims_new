...
use Dotbcrm\Dotbcrm\IdentityProvider\Authentication\Parser\XmlIdpMetadataParser;
...
class AdministrationController extends DotbController
{
    public function action_savetabs()
    {
        ...
        if (!is_admin($current_user)) {
            dotb_die($app_strings['ERR_NOT_ADMIN']);
        }
        ...
        // Add Home back in so that it always appears first in Dotb 7
    ...
    public function action_savelanguages()
    {
        ...
        if (count($dotb_config['languages']) === count($disabled_langs)) {
            dotb_die(translate('LBL_CAN_NOT_DISABLE_ALL_LANG'));
        } else {
            $cfg = $this->getConfigurator();
            if (in_array($dotb_config['default_language'], $disabled_langs)) {
                reset($enabled_langs);
    ...
    public function action_updatewirelessenabledmodules()
    {
        ...
        if (!is_admin($current_user)) dotb_die($app_strings['ERR_NOT_ADMIN']);
        ...
        if (isset($_REQUEST['enabled_modules']) && !empty ($_REQUEST['enabled_modules']))
        {
            ...
            foreach($moduleList as $mod){
                dotb_cache_clear("CONTROLLER_wireless_module_registry_$mod");
            }
            //Users doesn't appear in the normal module list, but its value is cached on login.
            dotb_cache_clear("CONTROLLER_wireless_module_registry_Users");
            dotb_cache_put('wireless_module_registry_keys', array_keys($updated_enabled_modules));
            dotb_cache_reset();
     ...
     *
     * This method is responsible for actually running the DotbSpriteBuilder class to rebuild the sprites.
    ...
    protected function saveFtsConfig($type, array $config)
    {
        ...
        DotbConfig::getInstance()->clearCache();
        return $config;
    }
     ...
     *
     * We want to merge the current $dotb_config settings with those passed in
    ...
    {
        $currentConfig = DotbConfig::getInstance()->get("full_text_engine.{$type}", array());
    ...
    public function action_saveApiPlatforms()
    {
        ...
        dotb_file_put_contents_atomic($file_loc, $out);
    ...
    /**
     * stop dotb workflow and send response
     */
    ...
    {
        $config = (new Config(\DotbConfig::getInstance()))->getSAMLConfig();
