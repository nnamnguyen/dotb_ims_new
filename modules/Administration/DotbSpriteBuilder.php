	...
	public function createSprites() {
		...
		foreach($this->spriteSrc as $name => $dirs)
        {
			...
			if($sp->width() && $sp->height())
            {
				...
				// directory structure
				if(!is_dir(dotb_cached("sprites/$nameSpace")))
                {
					dotb_mkdir(dotb_cached("sprites/$nameSpace"), 0775, true);
                }
