...
<script type="text/javascript" language="Javascript" src="{dotb_getjspath file='modules/UpgradeWizard/upgradeWizard.js'}"></script>
...
{dotb_csrf_form_token}
    ...
    <link rel='stylesheet' type='text/css' href="{dotb_getjspath file='include/javascript/yui/assets/container.css'}" />
        {if $step == 'commit'}
    <link rel='stylesheet' type='text/css' href="{dotb_getjspath file='include/javascript/yui/build/container/assets/container.css'}"/>
    <link rel='stylesheet' type='text/css' href="{dotb_getjspath file='themes/default/css/yui.css'}"/>
       {/if}
    {/if}
		{if $showBack}
...
function updateUpgradeStepTime(ts){
  success = function(r) {
   ...
   postData = 'upgradeStepTime=' + ts + '&module=UpgradeWizard&action=upgradeTimeCounter&to_pdf=1&dotb_body_only=1';
