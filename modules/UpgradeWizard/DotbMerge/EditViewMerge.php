...
/**
 * This is the base class that all other DotbMerge objects extend 
 *
 */
class EditViewMerge{
	/**
	...
	protected function mergeFields() {
		...
		/**
		 * These are fields that were added by dotb
		 */
	...
	protected function getFields(&$panels){
		...
		if($this->scanForMultiPanel){
			require_once('include/DotbFields/Parsers/MetaParser.php');			
	...
	/**
	 * public function that will merge meta data from an original dotb file that shipped with the product, a customized file, and a new file shipped with an upgrade
	 *
	 * @param STRING $module - name of the module's files that are to be merged
	 * @param STRING $original_file - path to the file that originally shipped with dotb
	 * @param STRING $new_file - path to the new file that is shipping with the patch 
