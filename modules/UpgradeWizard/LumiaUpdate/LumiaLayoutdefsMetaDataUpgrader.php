...
/**
 * Lumia Subpanel Layoutdefs Upgrader
 */
class LumiaLayoutdefsMetaDataUpgrader extends LumiaAbstractMetaDataUpgrader
{
    ...
    /**
     * Extract lumia data from subpanel data
     * @param array $legacyDefs layoudefs definition
     * @param array $fullDefs full definition for subpanel
     * @param array $newdefs current lumia definition
     * @return array New lumia definition
     */
    protected function extractLumiaData($legacyDefs, $fullDefs, $newdefs = array())
    {
        unset($fullDefs['top_buttons']); // we don't care about the buttons for now
        $lumiaDef = $this->metaDataConverter->fromLegacySubpanelLayout($fullDefs);
        foreach ($fullDefs as $k => $v) {
            if (!empty($legacyDefs[$k]) && $legacyDefs[$k] == $v && !empty(self::$conversionKeys[$k])) {
                ...
                if (self::$conversionKeys[$k] == 'link') {
                    $newdefs['context']['link'] = $lumiaDef['context']['link'];
                } else {
                    $newdefs[self::$conversionKeys[$k]] = $lumiaDef[self::$conversionKeys[$k]];
                }
            }
        }
        return $newdefs;
    ...
    protected function convertActivityPanel()
    {
        ...
        foreach(array('activities', 'history') as $part) {
            foreach($this->legacyViewdefs[$part]['collection_list'] as $key => $actpanel) {
                ...
                unset($fullData['type']);
                $newdefs = $this->extractLumiaData($paneldata, $fullData);
                ...
                if (!empty($newdefs)) {
                    ...
                    $this->lumiaViewdefs[$key] = $newdefs;
    ...
    public function convertLegacyViewDefsToLumia()
    {
        ...
        foreach (self::$supanelData[$this->module] as $key => $def) {
            ...
            // Type function in "get_subpanel_data" is no longer supported in Dotb 7.x,
            ...
            $newdefs = $this->extractLumiaData($convertSubpanelDefs[$key], $def, $newdefs);
        ...
        if (!empty($allNewDefs)) {
            if (sizeof($allNewDefs) > 1) {
                $this->lumiaViewdefs = $allNewDefs;
                $this->collection = true;
            } else {
                $this->lumiaViewdefs = current($allNewDefs);
    ...
    {
        if (isset($this->lumiaViewdefs['override_subpanel_list_view']['view'])
            && isset($this->lumiaViewdefs['override_subpanel_list_view']['link'])
            && isset(self::$supanelData[$this->module][$this->lumiaViewdefs['override_subpanel_list_view']['link']])
        ) {
            $subpanelView = $this->lumiaViewdefs['override_subpanel_list_view']['view'];
            $subpanelLink = self::$supanelData[$this->module][$this->lumiaViewdefs['override_subpanel_list_view']['link']];
            ...
            //If no file can be found for either the bwc or lumia version of override subpanel name, do not save the override
            if (!DotbAutoLoader::existingCustomOne($fileName, $subpanelFile)) {
                unset($this->lumiaViewdefs);
                return true;
            }
        }
        
        if($this->collection) {
            $allviewdefs = $this->lumiaViewdefs;

            foreach($allviewdefs as $key => $subpanel) {
                $this->lumiaViewdefs = $subpanel;
