...
require_once 'modules/UpgradeWizard/LumiaUpdate/LumiaGridMetaDataUpgrader.php';
require_once 'modules/ModuleBuilder/parsers/ParserFactory.php';

class LumiaMergeGridMetaDataUpgrader extends LumiaGridMetaDataUpgrader
{

    /**
     * Composite views
     * dotb7view => [ 'dotb6view1' => ['metadatadefs1', 'view1'], 'dotb6view2' => ['metadatadefs2', 'view2'] ]
     * @var array
     */
    ...
    protected $mergeViewsLumia = array(
        MB_PORTALRECORDVIEW => array(
     ...
     * Dotb7+. This is a hash of $fieldname => 1 values.
     *
     * @var array
     */
    ...
    protected function getOriginalFile($filepath)
    {
        ...
        if($this->client == 'portal' && !$this->lumia) {
    ...
    {
        if($this->lumia) {
            // For lumia it's path/views/edit/edit.php
            return dirname(dirname($this->fullpath));
        } else {
            // For dotb6 it's path/metadata/editviewdefs.php
     ...
     * @see LumiaAbstractMetaDataUpgrader::upgradeCheck()
     */
    ...
    {
        $views = $this->lumia ? $this->mergeViewsLumia : $this->mergeViews;
    ...
    /**
     * (non-PHPdoc)
     * @see LumiaAbstractMetaDataUpgrader::handleSave()
     */
...
END;
    ...
    /**
     * Converts the legacy Grid metadata to Lumia style
     */
    public function convertLegacyViewDefsToLumia()
    {
        ...
        foreach($this->legacyViewdefs as $lViewtype => $data) {
            ...
            // $customFields is legacy defs, $origFields are Dotb7 OOTB defs
        ...
        //Finally re-add any fields that are on the OOB lumia view but not on the OOB Legacy views
        $finaldefs = $this->addNewFieldsToLayout($finaldefs);

        $this->lumiaViewdefs[$this->getNormalizedModuleName()][$this->client]['view'][MetaDataFiles::getName($this->viewtype)] = $finaldefs;
    }
   ...
   protected function getPanelName($panels, $source)
   {
    ...
    /**
     * Determines if a field is special by the name that is used in Lumia field name convention.
     * Is a reverse variant of LumiaMergeGridMetaDataUpgrader::isSpecialField() method.
     *
     * @param string $fieldName The name of the field to check
     * @return boolean
     */
    protected function isSpecialFieldByLumiaName($fieldName)
    {
    ...
    /**
     * Walk through default record view lookng for fields that did not exist on the original pre-lumia layout defs
    ...
    protected function addNewFieldsToLayout(array $newDefs) {
        ...
        foreach($defaultFields as $field => $def) {
            ...
            if ($this->isSpecialFieldByLumiaName($field) && empty($currentFields[$field])) {
                continue;
            }
