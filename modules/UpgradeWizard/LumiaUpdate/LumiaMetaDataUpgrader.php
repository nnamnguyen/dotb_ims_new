...
require_once 'modules/UpgradeWizard/LumiaUpdate/LumiaSubpanelMetaDataUpgrader.php';
 ...
 *  - All DotbObject templates wireless metadata (will be replaced on upgrade)
 *
...
class LumiaMetaDataUpgrader
{
    ...
    protected $lumiaFilePaths = array(
        'portal' => array(
            'custom'  => 'custom/',
            'history' => 'custom/history/',
            'working' => 'custom/working/',
    ...
    /**
     * List of Lumia*MetaDataUpgrader classes that map to a view type
     *
     * @var array
     */
    ...
    public function setPortalFilesToUpgrade()
    {
        ...
        $this->setLumiaUpgradeFiles('portal');
    }
    ...
    public function getCustomModuleMetadata($metatype = 'base', $customPath = '', $subpanels = false)
    {
        ...
        // - custom/modulebuilder/builds/<PACKAGENAMES>/DotbModules/modules/<PACKAGEKEY>_<MODULENAME>/metadata
        //
        ...
        foreach ($mb->packages as $packagename => $package) {
            ...
            $buildpath = $package->getBuildDir() . '/DotbModules/modules/';
    ...
    protected function setQuickCreateFiles()
    {
        ...
        foreach ($DCActions as $module) {
            ...
            $lumiaMetadataPath = 'modules/' . $module . '/clients/base/menus/quickcreate/quickcreate.php';
            $quickcreatedefsPath = 'modules/' . $module . '/metadata/quickcreatedefs.php';
            if (!file_exists('custom/' . $lumiaMetadataPath) && !file_exists($lumiaMetadataPath) &&
            ...
            $isCustomized =
                file_exists('custom/' . $lumiaMetadataPath) || file_exists('custom/' . $quickcreatedefsPath);
            ...
            $file['isDCEnabled'] = $this->isQuickCreateVisible($lumiaMetadataPath, $module, true);
            $file['order'] = $position++;
            $this->files[] = $file;
        }
        ...
        foreach ($disabled as $module) {
            ...
            $file['isDCEnabled'] = $this->isQuickCreateVisible($lumiaMetadataPath, $module, false);
            $file['order'] = $position++;
            $this->files[] = $file;
        }
    }
     ...
     *
     * @param string $quickCreateFile The Lumia Metadata file
    ...
    protected function setUpgradeFiles($client)
    {
        ...
        if ($isPortal && $skipFiles) {
            $this->logUpgradeStatus("Skipping setUpgradeFiles for portal because the Dotb instance version is newer than 6.5 - version: {$this->upgrade->from_version}");
            return;
        }
    ...
    /**
     * Gets all lumia views that need to be upgraded
     *
     * @param $client
     */
    protected function setLumiaUpgradeFiles($client)
    {
        $this->logUpgradeStatus("Getting $client lumia upgrade files ...");

        // Keep track of how many files were added, for logging
        $count = 0;

        // Hit the legacy paths list to start the ball rolling
        if (!empty($this->lumiaFilePaths[$client]) && is_array($this->lumiaFilePaths[$client])) {
            foreach ($this->lumiaFilePaths[$client] as $type => $path) {
        ...
        $this->logUpgradeStatus("$count $client lumia upgrade files set ...");
    }
    ...
    public function getUpgraderClass($viewtype)
    {
        if (isset($this->upgraderClassMap[$viewtype])) {
            return 'Lumia' . $this->upgraderClassMap[$viewtype] . 'MetaDataUpgrader';
        }
     ...
     * currently living in OOTB modules and in all of the DotbObject templates
    ...
    protected function cleanupLegacyFiles()
    {
        ...
        // And lastly we need to handle DotbObject Templates
        $path = 'include/DotbObjects/templates/';
        $DotbObjectsPaths = glob($path . '*', GLOB_ONLYDIR);
        foreach ($DotbObjectsPaths as $objPath) {
     ...
     * @param bool $deployed
     * @param bool $lumia Is this a lumia view?
     * @param bool $subpanels Is this a subpanel view
     * @return array Array of file params if found, false otherwise
     */
    public function getUpgradeFileParams($file, $module, $client, $type = 'base', $package = null, $deployed = true, $lumia = false, $subpanels = false)
    {
