...
abstract class LumiaAbstractMetaDataUpgrader
{
    /**
     * The upgrader that triggers the upgrade process
     *
     * @var LumiaMetaDataUpgrader
     */
    protected $upgrader;
    ...
    /**
     * Is the legacy view lumia-type view too?
     * @var bool
     */
    protected $lumia;
    ...
    /**
     * The lumia style view defs
     *
     * @var array
     */
    protected $lumiaViewdefs = array();
    ...
    protected $views = array(
        ...
        'baselist'        => MB_LUMIALISTVIEW,
        'baserecordview'  => MB_RECORDVIEW,
        'basefilter'      => MB_BASICSEARCH,
        'basepopuplist'   => MB_LUMIAPOPUPVIEW,
    );
     ...
     */
    abstract function convertLegacyViewDefsToLumia();

    /**
     * Object constructor, simply sets required information into the object
     *
     * @param LumiaMetaDataUpgrader $upgrader The original entry into the upgrade process
     ...
     */
    public function __construct(LumiaMetaDataUpgrader $upgrader, Array $file) {
    ...
    public function upgrade() {
        ...
        // Convert them
        $this->logUpgradeStatus("converting {$this->client}[{$this->type}] legacy viewdefs for {$this->module}:{$this->viewtype} to Dotb 7 format");
        $this->convertLegacyViewDefsToLumia();
    ...
    public function handleSaveArray($name, $path)
    {
        ...
        if (!dotb_mkdir($newDirName, null, true)) {
            $this->logUpgradeStatus("Cannot create '$newDirName'.");
            return false;
        }
        return write_array_to_file($name, $this->lumiaViewdefs, $path);
    }
    ...
    public function isValidField($field)
    {
        ...
        $viewname = $this->views[$this->client . $this->viewtype] == MB_LUMIALISTVIEW ? MB_LISTVIEW :
    ...
    public function save($path, $content) {
        ...
        return dotb_file_put_contents($path, $content);
    }
    ...
    public function getNewFileContents($viewname = null)
    {
        ...
        $client = $this->client == 'wireless' ? 'mobile' : $this->client;
        if(empty($this->lumiaViewdefs[$module][$client]['view'][$viewname])) {
            return '';
        }
        $out  = "<?php\n\$viewdefs['{$module}']['{$client}']['view']['{$viewname}'] = " . var_export($this->lumiaViewdefs[$module][$client]['view'][$viewname], true) . ";\n";
        return $out;
    }
    ...
    /**
     * Returns lumia view defs.
     *
     * @return array
     */
    public function getLumiaViewDefs() {
        return $this->lumiaViewdefs;
    }

    /**
     * Load current Dotb metadata for this module
     * 
     * @return array
     */
    protected function loadDefaultMetadata()
    {
        ...
        // need to get the metadata from the DotbObject type.
        ...
        if (empty($newdefs)) {
            ...
            if (!empty($moduleType)) {
                $this->base_defsfile = 'include/DotbObjects/templates/' . $moduleType . '/clients/' . $client . '/views/' . $viewname . '/' . $viewname . '.php';
                if (file_exists($this->base_defsfile)) {
                    require $this->base_defsfile;
                } else {
                    $this->base_defsfile = 'include/DotbObjects/templates/basic/clients/' . $client . '/views/' . $viewname . '/' . $viewname . '.php';
                    if (file_exists($this->base_defsfile)) {
                        require $this->base_defsfile;
                    } else {
                ...
                if($newdefs && $this->deployed) {
                    ...
                    $data  = <<<END
<?php
/* Generated by DotBCRM Upgrader */
\$viewdefs['{$this->module}']['{$client}']['view']['{$viewname}'] = {$export};
END;
                    dotb_file_put_contents($this->defsfile, $data);
                }
            }
        }
        return $newdefs;
