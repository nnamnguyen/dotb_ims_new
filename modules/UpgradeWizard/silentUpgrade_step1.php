...
function verifyArguments($argv,$usage_regular)
{
	...
	if(is_file("{$cwd}/include/entryPoint.php")) {
    		//this should be a regular dotb install
    		$upgradeType = constant('DOTBCRM_INSTALL');
            ...
            //this should be a regular dotb install
            echo "*******************************************************************************\n";
            echo "*** ERROR: Tried to execute in a non-DotBCRM root directory.\n";
            exit(1);
    }
...
function prepSystemForUpgradeSilent()
{
	...
	foreach($subdirs as $subdir) {
		if(!is_dir($dotb_config['upload_dir']."/upgrades/{$subdir}")) {
			mkdir_recursive($dotb_config['upload_dir']."/upgrades/{$subdir}");
		}
	}
...
$usage_regular =<<<eoq2
Usage: php.exe -f silentUpgrade.php [upgradeZipFile] [logFile] [pathToDotbInstance] [admin-user]
...
Example:
    [path-to-PHP/]php.exe -f silentUpgrade.php [path-to-upgrade-package/]DotbEnt-Upgrade-5.2.0-to-5.5.0.zip [path-to-log-file/]silentupgrade.log  [path-to-dotb-instance/] admin

Arguments:
    upgradeZipFile                       : Upgrade package file.
    logFile                              : Silent Upgarde log file.
    pathToDotbInstance                  : Dotb Instance instance being upgraded.
...
if(!is_file("$zipBasePath/dotb_version.php")) {
    echo "\n$cwd/{$zipBasePath}/dotb_version.php was not extracted\nFAILURE\n";
    exit(1);
}
...
if(is_file("$unzip_dir/manifest.php")) {
	...
	if(!isset($manifest)) {
		...
		copy("$unzip_dir/manifest.php", $dotb_config['upload_dir']."/upgrades/patch/{$zip_from_dir}-manifest.php");
