...
//// Dotb Instance. Three parameters are required to be defined in order to execute this file.
...
///////////////////////////////////////////////////////////////////////////////
////	STANDARD REQUIRED DOTB INCLUDES AND PRESETS
if(!defined('dotbEntry')) define('dotbEntry', true);

$_SESSION = array();
$_SESSION['schema_change'] = 'dotb'; // we force-run all SQL
...
define('DOTBCRM_INSTALL', 'DotbCRM_Install');
$upgradeType = 'DotbCRM_Install';

global $cwd;
$cwd = getcwd(); // default to current, assumed to be in a valid DotBCRM root dir.
...
$unzip_dir = dotb_cached("upgrades/temp");
$install_file = $dotb_config['upload_dir']."/upgrades/patch/".basename($argv[1]);
...
if(version_compare($dotb_version, '6.7.0', '=')) {
...
$ce_to_pro_ent = isset($manifest['name']) && preg_match('/^DotbCE.*?(Pro|Ent|Corp|Ult)$/', $manifest['name']);
...
$new_dotb_version = getUpgradeVersion();
$siv_varset_1 = setSilentUpgradeVar('origVersion', $dotb_version);
$siv_varset_2 = setSilentUpgradeVar('destVersion', $new_dotb_version);
...
if(!$siv_varset_1 || !$siv_varset_2 || !$siv_write){
	logThis("Error with silent upgrade variables: origVersion write success is ({$siv_varset_1}) ".
			...
			"path to cache dir is ({$GLOBALS['dotb_config']['cache_dir']})", $path);
}
...
$configOptions = $dotb_config['dbconfig'];
...
if(empty($errors)) {
		...
		if(!didThisStepRunBefore('commit','post_install')){
			$file = "$unzip_dir/" . constant('DOTBCRM_POST_INSTALL_FILE');
			if(is_file($file)) {
		...
		if($ce_to_pro_ent){
			...
			if(!merge_passwordsetting($dotb_config, $dotb_version)) {
        ...
        if (version_compare($dotb_version, '6.7.0', '<') && isset($dotb_config['default_theme']) && $dotb_config['default_theme'] == 'Dotb5') {
		...
		$dotb_config['default_max_tabs'] = '7';

		logThis('Upgrade the dotb_version', $path);
...
if(empty($errors)) {
		...
		if(!didThisStepRunBefore('commit','upgradeHistory')){
			...
			$new_upgrade->version = $dotb_version;
...
function checkConfigForPermissions(){
	...
	global $dotb_config;
	if(!isset($dotb_config['default_permissions'])){
		$dotb_config['default_permissions'] = array (
				'dir_mode' => 02770,
				'file_mode' => 0660,
				'user' => '',
				'group' => '',
		...
		ksort($dotb_config);
		if(is_writable('config.php') && write_array_to_file("dotb_config", $dotb_config,'config.php')) {
			//writing to the file
		}
	}
}
...
function checkLeadConversionSettings() {
	...
	global $dotb_config;
	if (!isset($dotb_config['lead_conv_activity_opt'])) {
		$dotb_config['lead_conv_activity_opt'] = 'copy';
		ksort($dotb_config);
		if (is_writable('config.php') && write_array_to_file("dotb_config", $dotb_config,'config.php')) {
			//writing to the file
		}
...
function checkResourceSettings(){
	...
	global $dotb_config;
	if(!isset($dotb_config['resource_management'])){
		$dotb_config['resource_management'] =
		array (
 ...
 */
function merge_passwordsetting($dotb_config, $dotb_version)
{
    ...
    $dotb_config = dotbArrayMerge($passwordsetting_defaults, $dotb_config);
    ...
    return write_array_to_file("dotb_config", $dotb_config, "config.php");
}
