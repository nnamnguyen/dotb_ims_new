...
class CliUpgrader extends UpgradeDriver
{
    ...
    /*
     * CLI arguments: Zipfile Logfile Dotbdir Adminuser [Stage]
     */
...
Usage:
php {$argv[0]} -z upgradeZipFile -l logFile -s pathToDotbInstance -u admin-user

Example:
    php [path-to-PHP/]{$argv[0]} -z [path-to-upgrade-package/]DotbEnt-Upgrade-6.5.x-to-7.5.0.zip -l [path-to-log-file/]silentupgrade.log -s path-to-dotb-instance/ -u admin

Arguments:
    -z/--zip upgradeZipFile              : Upgrade package file.
    -l/--log logFile                     : Upgrade log file (by default relative to instance dir)
    -s/--source pathToDotbInstance      : Dotb instance being upgraded.
...
eoq2;
    ...
    protected function verifyArguments()
    {
        ...
        if (!is_file("{$this->context['source_dir']}/include/entryPoint.php") || !is_file(
            ...
            self::argError("{$this->context['source_dir']} is not a DotBCRM directory.");
        }
    ...
    {
        /*
         * Loading DotbSystemInfo from health check only if version < 7.2.2
         * If version 7.2.2 and above DotbSystemInfo also loading in entryPoint.php
         */
        $dotb_version = '9.9.9';
        include "dotb_version.php";
        if (file_exists(dirname(__FILE__).'/DotbSystemInfo.php') && version_compare($dotb_version, '7.2.2', '<')) {
            require_once 'DotbSystemInfo.php';
        }
        
        if (!class_exists('DotbHeartbeatClient')) {
            require_once 'DotbHeartbeatClient.php';
        }
