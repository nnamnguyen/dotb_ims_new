...
switch($run) {
	case 'upload':
		...
		if(isset($_REQUEST['release_id']) && $_REQUEST['release_id'] != ""){
            ...
            global $dotb_config;
            $upload_maxsize_backup = $dotb_config['upload_maxsize'];
            $dotb_config['upload_maxsize'] = 60000000;
            /* End Bug 51722 */
            ...
            $dotb_config['upload_maxsize'] = $upload_maxsize_backup;
            /* End Bug 51722 */
        }
        if($perform){
			...
			if(is_file($manifest_file)) {
				...
				dotb_mkdir("$base_upgrade_dir/$upgrade_zip_type", 0775, true);
...
////	UPLOAD FORM
$form = '';
if(empty($GLOBALS['dotb_config']['disable_uw_upload'])){
$form =<<<eoq
	...
	var callback = {
		 success:function(r) {
		     ...
		     if(file_size.length >0){
		       var msg = DOTB.language.get('UpgradeWizard','LBL_UW_FILE_SIZE');
		       msg1 =DOTB.language.get('UpgradeWizard','LBL_UW_FILE_BIGGER_MSG');
		       msg2 = DOTB.language.get('UpgradeWizard','LBL_BYTES_WEBSERVER_MSG');
