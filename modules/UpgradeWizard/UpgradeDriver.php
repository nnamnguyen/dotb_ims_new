...
abstract class UpgradeDriver
{
     ...
     * source_dir - Dotb source dir
     * new_source_dir - directory where new Dotb source files are stored
     * admin - Admin user
     * log - Log file
    ...
    /**
     * Did we already run Dotb init?
     * @var bool
     */
    public $dotb_initialized = false;
    ...
    protected function loadConfig()
    {
        ...
        $GLOBALS['dotb_config'] = $dotb_config;
        // by-ref so we can modify it
        $this->config =& $GLOBALS['dotb_config'];
    }
    ...
    protected function loadVersion($dir)
    {
        ...
        $dotb_version = $dotb_flavor = null;
        include "$dir/dotb_version.php";
        $dotb_flavor = strtolower($dotb_flavor);
        return array($dotb_version, $dotb_flavor);
    }
    ...
    protected function preflightPHPSettings()
    {
        ...
        if (ini_get('magic_quotes_gpc') || ini_get('magic_quotes_runtime')
            ...
            return $this->error(
                "Magic quotes are deprecated and not supported in DotBCRM. Please read: http://www.php.net/manual/en/security.magicquotes.php",
                true
            );
        }
        return true;
    ...
    /**
     * Check that Dotb directory is writable
     * @return bool
     */
    protected function preflightWriteDotb()
    {
    ...
    /**
     * Check if Dotb files are accessible
     * @return bool
     */
    protected function preflightDotbFiles()
    {
        ...
        if (empty($this->config)) {
            return $this->error('Failed to read Dotb configs.', true);
        }
        return true;
    }
    ...
    protected $preflightChecksBeforeInit = array(
        ...
        "DotbFiles" => true, // check if Dotb dirs are accessible
    );
    ...
    protected $preflightChecksAfterInit = array(
        ...
        "WriteDotb" => true, // check if Dotb directory is writable
        "WriteCustom" => true, // check if Dotb custom directory is writable
        "DB" => true, // check DB permissions
        "DuplicateUpgrade" => true, // check if we already seen this upgrade
    );

    /**
     * Preflight checks for Dotb upgrade
    ...
    {
        // Execute preflight checks before Dotb
        if (!$this->preflight($this->preflightChecksBeforeInit)) {
            return false;
        }
        $this->initDotb();
        // Execute preflight checks after Dotb
    ...
    protected function validateManifest()
    {
        ...
        if (isset($manifest['acceptable_dotb_versions'])) {
            $version_ok = false;
            $matches_empty = true;
            if (!empty($manifest['acceptable_dotb_versions']['exact_matches'])) {
                $matches_empty = false;
            ...
            if (!$version_ok && !empty($manifest['acceptable_dotb_versions']['regex_matches'])) {
                $matches_empty = false;
        ...
        if (!empty($manifest['acceptable_dotb_flavors'])) {
            $flavor_ok = false;
    ...
    protected function loadUser(array $params)
    {
        ...
        // using plain SQL instead of DotbBean::retrieve() as the DB schema
    ...
    /**
     * Initialize Dotb environment
     */
    protected function initDotb()
    {
        if ($this->dotb_initialized) {
            return;
        }

        // BR-385 - This fixes the issues around DotbThemeRegistry fatals.  The cache needs rebuild on stage-post init of dotb
        ...
        $this->log("Initializing DotBCRM environment");
        global $beanFiles, $beanList, $objectList, $timedate, $moduleList, $modInvisList, $dotb_config, $locale,
               $dotb_version, $dotb_flavor, $dotb_build, $dotb_db_version, $dotb_timestamp, $db, $locale,
        ...
        // reset the dotb cache so that the language files won't be loaded from the cache in upgrade wizard
        // otherwise when building $app_strings by using Utils.php::return_application_language(),
        // dotb_cache_retrieve() will return the cached value from the old upgraded-from version.
        dotb_cache_reset();
        ...
        $this->dotb_initialized = true;
        $this->loadStrings();
        $GLOBALS['app_list_strings'] = return_app_list_strings_language($GLOBALS['current_language']);
        $this->log("Done initializing DotBCRM environment");
    }
     ...
     * @param string $dir Dotb directory
     * @param string $stage
     * @return array
     */
        ...
        foreach ($dirs as $dirname) {
            ...
            try {
                foreach (new FilesystemIterator($dirname . $stage, FilesystemIterator::SKIP_DOTS) as $fileInfo) {
                    ...
                    $classname = "DotbUpgrade" . preg_replace(
                            '/^\d+_/',
                            "",
                            $scriptname
                        ); // strip numeric prefix, add DotbUpgrade
    ...
    public function readConfigFiles()
    {
        ...
        $oldConfig = $dotb_config;
        if (is_readable('config_override.php')) {
            include 'config_override.php';
        }
        return array($oldConfig, deepArrayDiff($dotb_config, $oldConfig));
    }
    ...
    public function genConfigs($old, $over, $new)
    {
        ...
        $saveArray = dotbArrayMergeRecursive($old, $diffArray);
        return $saveArray;
    }
    /**
     * Save config.php
...
//        ksort($configs);
//        return rebuildConfigFile($configs, $dotb_version);
echo '123123';
die();
    }
    ...
    public static function getVersion()
    {
        ...
        } elseif (file_exists('dotb_version.php')) {
            if (!defined('dotbEntry')) {
                define('dotbEntry', 'upgrader');
            }
            include 'dotb_version.php';
            $version = $dotb_version;
            $build = $dotb_build;
        }
        return array($version, $build);
    }
