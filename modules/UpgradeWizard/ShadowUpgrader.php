...
php ShadowUpgrader.php -f oldTemplate -t newTemplate -s pathToDotbInstance -l logFile -u admin-user

Example:
    php ShadowUpgrader.php -f /dotb/templates/7.0.0 -t /dotb/templates/7.1.0 -s path-to-dotb-instance/ \
...
eoq2;
    ...
    protected function verifyArguments()
    {
        ...
        if(!is_file("{$this->context['pre_template']}/include/entryPoint.php")) {
            $this->argError("{$this->context['pre_template']} is not a DotBCRM template.");
        }

        if(!is_file("{$this->context['post_template']}/include/entryPoint.php")) {
            $this->argError("{$this->context['post_template']} is not a DotBCRM template.");
        }

        if(!is_file("{$this->context['source_dir']}/config.php")) {
            $this->argError("{$this->context['source_dir']} is not a DotBCRM directory.");
        }
    ...
    public function getManifest()
    {
        ...
        return array(
            ...
            'acceptable_dotb_flavors' => array($this->from_flavor),
            'acceptable_dotb_versions' => array('exact_matches' => array($this->from_version)),
            'type' => 'patch',
            'version' => $to_version,
            'flavor' => $to_flavor,
        );
