...
function commitAjaxRunSql($persistence) {
	...
	if($persistence['schema_change'] == 'dotb') {
...
function commitAjaxPostInstall($persistence) {
	...
	if(!updateVersions($dotb_version)) {
	...
	$file = $persistence['unzip_dir']. "/" . constant('DOTBCRM_POST_INSTALL_FILE');
	if(is_file($file)) {
		include($file);
		ob_start();
		post_install();
...
function preflightCheckJsonPrepSchemaCheck($persistence, $preflight=true) {
    ...
    $origVersion = implodeVersion($dotb_db_version);
...
function preflightCheckJsonFillSchema() {
	...
	if(file_exists($sqlScript)) {
		$contents = dotb_file_get_contents($sqlScript);
