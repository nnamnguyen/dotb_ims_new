...
function getUploadRelativeName($path)
{
    ...
    if(substr($path, 0, 9) == "upload://") {
    	$path = rtrim($GLOBALS['dotb_config']['upload_dir'], "/\\")."/".substr($path, 9);
    }
    return $path;
}
...
function commitCopyNewFiles($unzip_dir, $zip_from_dir, $path='') {
	...
	foreach($newFiles as $file) {
		...
		if((!file_exists($targetFile)) || /* brand new file */
			...
			// handle dotb_version.php
			if(strpos($targetFile, 'dotb_version.php') !== false && !preg_match('/\/portal\/dotb_version\.php$/i', $targetFile)) {
				logThis('Skipping "dotb_version.php" - file copy will occur at end of successful upgrade', $path);
				$_SESSION['dotb_version_file'] = $srcFile;
				continue;
			}
		...
		foreach($modifiedFiles as $file) {
            ...
            if (strpos($srcFile,".svn") === false) {
				...
				if(!is_dir(dirname($targetFile))) {
					...
					// handle dotb_version.php
					if(strpos($targetFile, 'dotb_version.php') !== false && !preg_match('/\/portal\/dotb_version\.php$/i', $targetFile)) {
						logThis('Skipping "dotb_version.php" - file copy will occur at end of successful upgrade', $targetFile);
						$_SESSION['dotb_version_file'] = $srcFile;
						continue;
					}
		...
		foreach($delete_dirs as $del_dir){
			...
			foreach($newFiles as $file) {
				...
				if(!file_exists($targetFile)){
					// handle dotb_version.php
					if(strpos($targetFile, 'dotb_version.php') !== false) {
						logThis('Skipping dotb_version.php - file copy will occur at end of successful upgrade');
						$_SESSION['dotb_version_file'] = $srcFile;
						continue;
					}
...
function deleteChance(){
	...
	if(is_dir('include/DotbObjects/templates/chance')){
		rmdir_recursive('include/DotbObjects/templates/chance');
	 }
	if(is_dir('include/DotbObjects/templates/chance')){
		if(!isset($_SESSION['chance'])){
			$_SESSION['chance'] = '';
		}
		$_SESSION['chance'] = 'include/DotbObjects/templates/chance';
	}
}
...
function upgradeUWFiles($file) {
    $cacheUploadUpgradesTemp = mk_temp_dir(dotb_cached("upgrades/temp"));
    ...
    if(file_exists("$from_dir/include/DotbTheme")) {
        $allFiles[] = findAllFiles("$from_dir/include/DotbTheme", array());
    }
    if(file_exists("$from_dir/include/DotbCache")) {
        $allFiles[] = findAllFiles("$from_dir/include/DotbCache", array());
    }
    ...
    if(file_exists("$from_dir/include/utils/dotb_file_utils.php")) {
        $allFiles[] = "$from_dir/include/utils/dotb_file_utils.php";
    }
...
/**
 * finalizes upgrade by setting upgrade versions in DB (config table) and dotb_version.php
...
function updateVersions($version) {
	...
	logThis('At updateVersions()... updating config table and dotb_version.php.', $path);

	// handle file copy
	if(isset($_SESSION['dotb_version_file']) && !empty($_SESSION['dotb_version_file'])) {
		if(!copy($_SESSION['dotb_version_file'], clean_path(getcwd().'/dotb_version.php'))) {
			logThis('*** ERROR: dotb_version.php could not be copied to destination! Cannot complete upgrade', $path);
			return false;
		} else {
			logThis('dotb_version.php successfully updated!', $path);
		}
	} else {
		logThis('*** ERROR: no dotb_version.php file location found! - cannot complete upgrade...', $path);
		return false;
	}

	$q1 = "DELETE FROM config WHERE category = 'info' AND name = 'dotb_version'";
	$q2 = "INSERT INTO config (category, name, value) VALUES ('info', 'dotb_version', '{$version}')";
...
/**
 * gets a module's lang pack - does not need to be a DotbModule
...
function checkSystemCompliance() {
	...
	if(!defined('DOTBCRM_MIN_MEM')) {
		define('DOTBCRM_MIN_MEM', 40);
	}
	...
	$ret['memory_msg']     = "";
    $memory_limit = '-1';
	$dotbMinMem = constant('DOTBCRM_MIN_MEM');
	...
	if( $memory_limit == "" ){          // memory_limit disabled at compile time, no memory limit
	    ...
	    if( $memory_limit_int < constant('DOTBCRM_MIN_MEM') ){
	        $ret['memory_msg'] = "<b><span class=\"stop\">{$installer_mod_strings['ERR_CHECKSYS_MEM_LIMIT_1']}" . constant('DOTBCRM_MIN_MEM') . "{$installer_mod_strings['ERR_CHECKSYS_MEM_LIMIT_2']}</span></b>";
			$ret['error_found'] = true;
	    } else {
...
function logThis($entry, $path='') {
	global $mod_strings;
	if(file_exists('include/utils/dotb_file_utils.php')){
...
if(!isset($_SESSION['unzip_dir']) || empty($_SESSION['unzip_dir'])) {
        ...
        if(empty($_SESSION['install_file'])){
        	...
        	$redirect_new_wizard = $dotb_config['site_url' ].'/index.php?module=UpgradeWizard&action=index';
...
$uwMain = $upgrade_directories_not_found;
    ...
    logThis ('is DotbConfig there '.file_exists(clean_path($unzip_dir.'/'.$zip_from_dir."/include/DotbObjects/DotbConfig.php")));
	if(file_exists(clean_path($unzip_dir.'/'.$zip_from_dir."/include/DotbObjects/DotbConfig.php"))) {
		$file = clean_path($unzip_dir.'/'.$zip_from_dir."/include/DotbObjects/DotbConfig.php");
...
$uwMain = $upgrade_directories_not_found;
	...
	fileCopy('include/utils/dotb_file_utils.php');
...
function prepSystemForUpgrade() {
	...
	dotb_mkdir($base_tmp_upgrade_dir, 0775, true);
	...
	if($subdirs != null){
		foreach($subdirs as $subdir) {
		    dotb_mkdir("$base_upgrade_dir/$subdir", 0775, true);
		}
	}
	...
	define('DOTBCRM_MIN_UPLOAD_MAX_FILESIZE_BYTES', 6 * 1024 * 1024);  // 6 Megabytes
	$upload_max_filesize = ini_get('upload_max_filesize');
	$upload_max_filesize_bytes = return_bytes($upload_max_filesize);

	if($upload_max_filesize_bytes < constant('DOTBCRM_MIN_UPLOAD_MAX_FILESIZE_BYTES')) {
...
function getImageForType($type) {
    ...
    switch($type) {
        case "full":
            $icon = DotbThemeRegistry::current()->getImage("Upgrade", "",null,null,'.gif',$mod_strings['LBL_UPGRADE']);
            break;
        case "langpack":
            $icon = DotbThemeRegistry::current()->getImage("LanguagePacks", "",null,null,'.gif',$mod_strings['LBL_LANGPACKS']);
            break;
        case "module":
            $icon = DotbThemeRegistry::current()->getImage("ModuleLoader", "",null,null,'.gif',$mod_strings['LBL_MODULELOADER']);
            break;
        case "patch":
            $icon = DotbThemeRegistry::current()->getImage("PatchUpgrades", "",null,null,'.gif',$mod_strings['LBL_PATCHUPGRADES']);
            break;
        case "theme":
            $icon = DotbThemeRegistry::current()->getImage("Themes", "",null,null,'.gif',$mod_strings['LBL_THEMES']);
            break;
        default:
            break;
    }
...
/**
 * Verifies a manifest from a patch or module to be compatible with the current Dotb version and flavor
...
function validate_manifest($manifest) {
    ...
    if(isset($manifest['acceptable_dotb_versions'])) {
        ...
        if(!$matches_empty && !$version_ok) {
            ...
            $mod_strings['ERR_UW_VERSION'].$dotb_version;
        }
    }
    ...
    if(isset($manifest['acceptable_dotb_flavors']) && sizeof($manifest['acceptable_dotb_flavors']) > 0) {
        ...
        if(!$flavor_ok) {
            ...
            $mod_strings['ERR_UW_FLAVOR'].$dotb_flavor."<br />".
            $mod_strings['ERR_UW_FLAVOR_2'].$manifest['acceptable_dotb_flavors'][0];
        }
    }
...
function unlinkUWTempFiles() {
    ...
    if(file_exists($tempDir) && is_dir($tempDir)){
		...
		$cacheFile = dotb_cached("modules/UpgradeWizard/_persistence.php");
...
function resetUwSession() {
	...
	if(isset($_SESSION['dotb_version_file']))
		unset($_SESSION['dotb_version_file']);
    ...
    if(isset($_SESSION['dotbMergeRunResults']))
		unset($_SESSION['dotbMergeRunResults']);
}
...
/**
 * generates an array with all files in the DotBCRM root directory, skipping
 * cache/
...
function set_upgrade_vars(){
	...
	$upgrade_progress_dir = dotb_cached('upgrades/temp');
...
function set_upgrade_progress($currStep,$currState,$currStepSub='',$currStepSubState=''){
	...
	if(file_exists($upgrade_progress_file)){
		...
		if(function_exists('dotb_fopen')){
			dotb_fopen($upgrade_progress_file, 'w+');
		}
		else{
			fopen($upgrade_progress_file, 'w+');
		}
...
function upgradeDashletsForSalesAndMarketing() {
	if(file_exists($cachedfile = dotb_cached('dashlets/dashlets.php'))) {
   	   require($cachedfile);
   	}
    ...
    require_once('include/MyDotb/MyDotb.php');
    ...
    $result = $db->query("SELECT id FROM users where deleted = '0'");
   	...
   	while($row = $db->fetchByAssoc($result)) {
	      ...
	      //Set the user theme to be 'Dotb' theme since this is run for CE flavor conversions
	      $current_user->setPreference('user_theme', 'Dotb', 0, 'global');
...
function upgradeUserPreferences() {
    global $dotb_config, $dotb_version;
    ...
    if(empty($dotb_config['name_formats'])) {
        $dotb_config['name_formats'] = $localeCoreDefaults['name_formats'];
        if(!rebuildConfigFile($dotb_config, $dotb_version)) {
            $errors[] = $uw_strings['ERR_UW_CONFIG_WRITE'];
        }
    }

    $currentDefaultLocaleNameFormat = $dotb_config['default_locale_name_format'];

    if ($localization->isAllowedNameFormat($currentDefaultLocaleNameFormat)) {
        upgradeLocaleNameFormat($currentDefaultLocaleNameFormat);
    } else {
        $dotb_config['default_locale_name_format'] = $localeCoreDefaults['default_locale_name_format'];
        if(!rebuildConfigFile($dotb_config, $dotb_version)) {
            $errors[] = $uw_strings['ERR_UW_CONFIG_WRITE'];
        }
    ...
    $ce_to_pro_or_ent = (isset($_SESSION['upgrade_from_flavor']) && preg_match('/^DotbCE.*?(Pro|Ent|Corp|Ult)$/', $_SESSION['upgrade_from_flavor']));
   	...
   	while($row = $db->fetchByAssoc($result))
    {
        ...
        if(!$current_user->getPreference('calendar_publish_key')) {
          ...
          //If theme is empty or if theme was set to Classic (Dotb5) or if this is a ce to pro/ent flavor upgrade change to RacerX theme
	      if(empty($userTheme) || $userTheme == 'Dotb5' || $ce_to_pro_or_ent)
	      {
            $changed = true;
	 ...
	 * breadcrumb tracking.  It will instead only track visible entries (see trackView() method in DotbView.php).
	...
	if($ce_to_pro_or_ent) {
	   ...
	   if(!empty($results)) {
	       ...
	       $total = $row['total'];
	       if($GLOBALS['dotb_flavor'] == 'PRO' || $GLOBALS['dotb_flavor'] == 'ENT')  {
...
function upgradeLocaleNameFormat($name_format) {
    ...
    $uw_strings = return_module_language($GLOBALS['current_language'], 'UpgradeWizard');
    if(empty($dotb_config['name_formats'])) {
        $dotb_config['name_formats'] = $localeConfigDefaults['name_formats'];
        if(!rebuildConfigFile($dotb_config, $dotb_version)) {
            $errors[] = $uw_strings['ERR_UW_CONFIG_WRITE'];
        }
    }
    if (!in_array($name_format, $dotb_config['name_formats'])) {
        $new_config = dotbArrayMerge($dotb_config['name_formats'], array($name_format=>$name_format));
        $dotb_config['name_formats'] = $new_config;
        if(!rebuildConfigFile($dotb_config, $dotb_version)) {
            $errors[] = $uw_strings['ERR_UW_CONFIG_WRITE'];
            return false;
        }
...
function migrate_dotb_favorite_reports(){
    require_once('modules/DotbFavorites/DotbFavorites.php');

    // Need to repair the RC1 instances that have incorrect GUIDS
    $deleteRows = array();
    $res = $GLOBALS['db']->query("select * from dotbfavorites where module='Reports'");
    while($row = $GLOBALS['db']->fetchByAssoc($res)){
        $expectedId = DotbFavorites::generateGUID('Reports', $row['record_id'], $row['assigned_user_id']);
        if ($row['id'] != $expectedId) {
            $deleteRows[] = $row['id'];
        }
    }
    $GLOBALS['db']->query("delete from dotbfavorites where id in ('" . implode("','",$deleteRows) . "')");
    // End Repair
    ...
    foreach($active_users as $user_id){
        ...
        if(!empty($user_favorites)){
            foreach($user_favorites as $report_id => $bool){
                $fav = new DotbFavorites();
                $record = DotbFavorites::generateGUID('Reports', $report_id, $user_id);
				...
				$searchFields[$module_dir]['favorites_only'] = array(
					...
					'subquery' => 'SELECT dotbfavorites.record_id FROM dotbfavorites
								WHERE dotbfavorites.deleted=0
									and dotbfavorites.module = \''.$module_dir.'\'
 ...
 * added through the DotBCRM 5.5.x upgrade process.  It also adds the values into the team_sets and
 ...
 *
 * @param $bean DotbBean which we are adding team_set_id column to
...
function upgradeTeamColumn($bean, $column_name) {
	...
	//module that does not use the DotbObjects
	if(empty($bean->field_defs['team_set_id']) && $bean->module_dir != 'Trackers'){
		...
		$path = 'include/DotbObjects/implements/team_security/vardefs.php';
		require($path);
 ...
 *  the class DotbFolders does not extend DotbBean and is therefore never picked up by the
...
function upgradeModulesForTeam() {
	...
	function update_iframe_dashlets(){
		require_once(dotb_cached('dashlets/dashlets.php'));
		...
		while ($row = $db->fetchByAssoc($result)) {
			...
			if(!empty($content['dashlets']) && !empty($content['pages'])){
				...
				foreach($originalDashlets as $key => $ds){
				    if(!empty($ds['options']['url']) && stristr($ds['options']['url'],'http://www.dotbcrm.com/crm/product/gopro')){
	...
	function upgradeDocumentTypeFields($path){
		...
		$documentsSql = "UPDATE documents SET doc_type = 'Dotb' WHERE doc_type IS NULL";
		$meetingsSql = "UPDATE meetings SET type = 'Dotb' WHERE type IS NULL";
...
function merge_config_si_settings($write_to_upgrade_log=false, $config_location='', $config_si_location='', $path='')
{
	if(!empty($config_location) && !file_exists($config_location))
	{
		...
		global $argv;
		//We are assuming this is from the silentUpgrade scripts so argv[3] will point to DotBCRM install location
	...
	$modified = false;
	foreach($dotb_config_si as $key=>$value)
	{
        if (!preg_match('/^setup_/', $key)) {
		   ...
		   // Add config_si values to the global dotb_config array so that they
		   // are available on install, since merging SI happens after capturing
		   // dotb_config and only saves to file, not to memory
		   $GLOBALS['dotb_config'][$key] = $dotb_config[$key] = $value;
		   $modified = true;
		}
	}
	...
	if($modified)
	{
	    ...
	    if(!write_array_to_file("dotb_config", $dotb_config, $config_location)) {
	       if($write_to_upgrade_log)
		   {
...
function removeSilentUpgradeVarsCache(){
    ...
    $cacheFileDir = "{$GLOBALS['dotb_config']['cache_dir']}/silentUpgrader";
...
function loadSilentUpgradeVars(){
    ...
    if(empty($silent_upgrade_vars_loaded)){
        $cacheFile = dotb_cached("silentUpgrader/silentUpgradeCache.php");
...
function writeSilentUpgradeVars(){
    ...
    $cacheFileDir = dotb_cached("silentUpgrader");
    $cacheFile = "{$cacheFileDir}/silentUpgradeCache.php";
...
function getUWDirs()
{
    if(!class_exists('UploadStream')) {
        ...
        global $dotb_config;
        return array($dotb_config['upload_dir'] . "/upgrades", $dotb_config['cache_dir'] . "upload/upgrades/temp");
    } else {
        if(!in_array("upload", stream_get_wrappers())) {
            UploadStream::register(); // just in case file was copied, but not run
        }
        return array("upload://upgrades", dotb_cached("upgrades/temp"));
    }
}
...
/*
 * rebuildSprites
 * @param silentUpgrade boolean flag indicating whether or not we should treat running the DotbSpriteBuilder as an upgrade operation
 *
 */
function rebuildSprites($fromUpgrade=true)
{
    require_once('modules/Administration/DotbSpriteBuilder.php');
    $sb = new DotbSpriteBuilder();
    $sb->cssMinify = true;
    ...
    $sb->addDirectory('default', 'themes/default/images/DotbLogic');
...
function repairUpgradeHistoryTable()
{
    ...
    $upload_dir = $dotb_config['cache_dir'].'upload/';
    ...
    while(($row = $GLOBALS['db']->fetchByAssoc($results)))
    {
        ...
        {
            //Update new file location to use the new $dotb_config['upload_dir'] value
            $new_file_location = $dotb_config['upload_dir'] . $matches[1];
...
{
    DotbAutoLoader::load('include/DotbQueue/jobs/DotbJobUpdateOpportunities.php');
    return DotbJobUpdateOpportunities::updateOpportunitiesForForecasting($perJob);
}
