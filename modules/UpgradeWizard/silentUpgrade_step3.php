...
require_once("dotb_version.php"); // provides $dotb_version & $dotb_flavor
...
if(is_dir($GLOBALS['dotb_config']['cache_dir'].'themes')){
    $allModFiles = array();
    $allModFiles = findAllFiles($GLOBALS['dotb_config']['cache_dir'].'themes',$allModFiles);
...
$dotb_version = getSilentUpgradeVar('origVersion');
if (!$dotb_version)
{
    global $silent_upgrade_vars_loaded;
    logThis("Error retrieving silent upgrade var for origVersion: cache dir is {$GLOBALS['dotb_config']['cache_dir']} -- full cache for \$silent_upgrade_vars_loaded is ".var_export($silent_upgrade_vars_loaded, true), $path);
}
...
// this needs to be outside of if($ce_to_pro_ent) because it does not cover second case where $ce_to_pro_ent is 'DotbPro'
...
if(function_exists('unlinkUpgradeFiles')) {
    unlinkUpgradeFiles($dotb_version, $path);
}
...
if (version_compare($dotb_version, '6.7.0', '<')) {
    ...
    ForecastsDefaults::setupForecastSettings(true, $dotb_version, getUpgradeVersion());
...
if (!didThisStepRunBefore('commit','upgradePortalMobileMetadata')) {
    if (version_compare($dotb_version, '6.6.0', '<')) {
        if (file_exists('modules/UpgradeWizard/LumiaUpdate/LumiaMetaDataUpgrader.php')) {
            set_upgrade_progress('commit','in_progress','upgradePortalMobileMetadata','in_progress');
            logThis('Lumia Upgrade: Preparing to upgrade metadata to 6.6.0 compatibility through the silent upgrader ...');
            require_once 'modules/UpgradeWizard/LumiaUpdate/LumiaMetaDataUpgrader.php';

            // Get the lumia metadata upgrader
            logThis('Lumia Upgrade: Instantiating the mobile/portal metadata upgrader ...');
            $smdUpgrader = new LumiaMetaDataUpgrader();

            // Run the upgrader
            logThis('Lumia Upgrade: Beginning the mobile/portal metadata upgrade ...');
            $smdUpgrader->upgrade();
            logThis('Lumia Upgrade: Mobile/portal metadata upgrade complete');

            // Log failures if any
            $failures = $smdUpgrader->getFailures();
            if (!empty($failures)) {
                logThis('Lumia Upgrade: ' . count($failures) . ' metadata files failed to upgrade through the silent upgrader:');
                logThis(print_r($failures, true));
            } else {
                logThis('Lumia Upgrade: Mobile/portal metadata upgrade ran with no failures:');
