...
class Popup_Picker
{
    ...
    */
    function process_page() {
		...
		foreach($focus->get_linked_fields() as $field => $def) {
			if ($focus->load_relationship($field)) {
	        	...
	        	if (in_array($relTable, array_keys($activitiesRels)))
        		{
        			...
        			$$varname = dotbArrayMerge($$varname, $focus->get_linked_beans($field,$activitiesRels[$relTable]));
        		}
        ...
        echo "<A href='javascript:print();' class='utilsLink'>" . DotbThemeRegistry::current()->getImage('print', "border='0' align='absmiddle'", 13, 13, ".gif", $app_strings['LNK_PRINT']) . "</a>&nbsp;<A href='javascript:print();' class='utilsLink'>".$app_strings['LNK_PRINT']."</A>\n";
        echo "</td></tr></table>";
        ...
        foreach($history_list as $activity)
        {
            ...
            if (isset($activity['filename'])) {
                $activity_fields['ATTACHMENT'] = "<a href='index.php?entryPoint=download&id=".$activity['id']."&type=Notes' target='_blank'>".DotbThemeRegistry::current()->getImage("attachment","border='0' align='absmiddle'",null,null,'.gif',$activity['filename'])."</a>";
            }
