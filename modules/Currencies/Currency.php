 ...
 *
 */
class Currency extends DotbBean
{
    // Stored fields
    var $id;
    var $iso4217;
    ...
    {
        parent::__construct();
        global $app_strings, $current_user, $dotb_config, $locale;
    ...
    function convertToBase($amount, $precision = 6) {
        ...
        return DotbCurrency::convertWithRate(str_replace($this->symbol, '', $amount), $this->conversion_rate, 1.0, $precision);
    }
    ...
    function convertFromBase($amount, $precision = 6) {
        $amount = ($amount == null) ? 0 : $amount;
        return DotbCurrency::convertWithRate(str_replace($this->symbol, '', $amount), 1.0, $this->conversion_rate, $precision);
    }
    ...
    {
        global $dotb_config;
        return isset($dotb_config['default_currency_name'])?$dotb_config['default_currency_name']:'';
    }
    ...
    {
        global $dotb_config;
        return isset($dotb_config['default_currency_symbol'])?$dotb_config['default_currency_symbol']:'';
    }
    ...
    {
        global $dotb_config;
        return isset($dotb_config['default_currency_iso4217'])?$dotb_config['default_currency_iso4217']:'';
    }
    ...
    {
        dotb_cache_clear('currency_list');
    ...
    /**
     * Same as DotbBean::mark_deleted except clears api cache.
     * @param $id
     */
...
/**
 * currency_format_number (deprecated, use DotbCurrency class)
 *
...
/**
 * format_number(deprecated, use DotbCurrency class)
 *
 ...
 * apply the system wide Dotb configuration values.  Calls to
 * getPrecendentPreference() method in Localization.php are made that
 * handle this logic.
 *
 * Going forward with Dotb 4.5.0e+ implementations, users of this class should
...
/**
 * (deprecated, use DotbCurrency class)
 *
 * @deprecated
 * @param string $amount
 * @param string $symbol
...
function get_number_seperators($reset_sep = false)
{
    ...
    if ($dec_sep == null)
    {
        $dec_sep = $dotb_config['default_decimal_seperator'];
    ...
    if ($num_grp_sep == null)
    {
        $num_grp_sep = $dotb_config['default_number_grouping_seperator'];
        if (!empty($current_user->id))
        {
