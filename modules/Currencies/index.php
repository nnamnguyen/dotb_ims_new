...
if(is_admin($current_user) && $_REQUEST['module'] != 'DynamicLayout' && !empty($_SESSION['editinplace'])){	
		$header_text = "&nbsp;<a href='index.php?action=index&module=DynamicLayout&from_action=ListView&from_module=".$_REQUEST['module'] ."'>".DotbThemeRegistry::current()->getImage("EditLayout","border='0' align='bottom'",null,null,'.gif',$mod_strings['LBL_EDIT_LAYOUT'])."</a>";
	}
...
if(is_admin($current_user) && $_REQUEST['module'] != 'DynamicLayout' && !empty($_SESSION['editinplace'])){	
		$header_text = "&nbsp;<a href='index.php?action=index&module=DynamicLayout&from_action=EditView&from_module=".$_REQUEST['module'] ."'>".DotbThemeRegistry::current()->getImage("EditLayout","border='0' align='bottom'", null,null,'.gif',$mod_strings['LBL_EDIT_LAYOUT'])."</a>";
}
if ( empty($focus->id) ) {
...
// Load in the full ISO 4217 list, so we can dynamically populate the currency strings
    ...
    $dotb_smarty->assign('JS_ISO4217',$js_iso4217);
	
	if (isset($_REQUEST['return_module'])) $dotb_smarty->assign("RETURN_MODULE", $_REQUEST['return_module']);
	if (isset($_REQUEST['return_action'])) $dotb_smarty->assign("RETURN_ACTION", $_REQUEST['return_action']);
	if (isset($_REQUEST['return_id'])) $dotb_smarty->assign("RETURN_ID", $_REQUEST['return_id']);
	
	$dotb_smarty->assign("JAVASCRIPT", get_set_focus_js());
    $dotb_smarty->assign("THEME", DotbThemeRegistry::current()->__toString());
	$dotb_smarty->assign("ID", $focus->id);
	$dotb_smarty->assign('NAME', $focus->name);
	$dotb_smarty->assign('STATUS', $focus->status);
	$dotb_smarty->assign('ISO4217', $focus->iso4217);
	$dotb_smarty->assign('CONVERSION_RATE', $focus->conversion_rate);
	$dotb_smarty->assign('SYMBOL', $focus->symbol);
	$dotb_smarty->assign('STATUS_OPTIONS', get_select_options_with_id($mod_strings['currency_status_dom'], $focus->status));
	...
	$dotb_smarty->assign('REFRESHMETADATA', $refreshMetadata);
	$dotb_smarty->display("modules/Currencies/EditView.tpl");
