...
class RenameModules
{
    ...
    protected function display()
    {
        ...
        $editImage = DotbThemeRegistry::current()->getImage( 'edit_inline', '');
        $smarty->assign('editImage',$editImage);
        $deleteImage = DotbThemeRegistry::current()->getImage( 'delete_inline', '');
        ...
        if (!is_file(dotb_cached('jsLanguage/') . 'Studio' . '/' . $GLOBALS['current_language'] . '.js')) {
     ...
     * @param  string $beanName  The name of the DotbBean to be renamed.
     * @return void
     */
    ...
    /**
     * Retrieve the subpanel definitions for a given DotbBean object. Unforunately we can't reuse
    ...
    private function getSubpanelDefs($bean)
    {
        ...
        if(isModuleBWC($bean->module_dir)) {
            foreach (DotbAutoLoader::existingCustom('modules/' . $bean->module_dir . '/metadata/subpaneldefs.php') as $file) {
                require FileLoader::validateFilePath($file);
            }

            $defs = DotbAutoLoader::loadExtension('layoutdefs', $bean->module_dir);
            if($defs) {
                require FileLoader::validateFilePath($defs);
            }
        } else {
            // Handle things the new way
            foreach (DotbAutoLoader::existingCustom('modules/' . $bean->module_dir . '/clients/base/layouts/subpanels/subpanels.php') as $file) {
                require FileLoader::validateFilePath($file);
            }
            ...
            if (isset($viewdefs[$bean->module_dir]['base']['layout']['subpanels']['components'])) {
                $layout_defs = $this->getLumiaSubpanelDefsAsLegacy($viewdefs[$bean->module_dir]['base']['layout']['subpanels']['components'], $bean);
            }
        }
    ...
    /**
     * Gets lumia subpanel layout defs in the BWC format
     * 
     * @param Array $components Existing lumia subpanel layout
     * @param DotbBean $bean The bean that the subpanels are being scraped for
     * @return array
     */
    protected function getLumiaSubpanelDefsAsLegacy(Array $components, DotbBean $bean) 
    {
        $return = array();
    ...
    private function renameAllDashlets()
    {
        ...
        include(dotb_cached('dashlets/dashlets.php'));
    ...
    protected function getAllModulesFromRequest()
    {
        ...
        while(isset($params['slot_' . $count])) {
            ...
            $key = (isset($params['key_' . $index]))?DotbCleaner::stripTags($params['key_' . $index]): 'BLANK';
            $value = (isset($params['value_' . $index]))?DotbCleaner::stripTags($params['value_' . $index]): '';
            $svalue = (isset($params['svalue_' . $index]))?DotbCleaner::stripTags($params['svalue_' . $index]): $value;
            if ($key == 'BLANK') {
                $key = '';
            }
