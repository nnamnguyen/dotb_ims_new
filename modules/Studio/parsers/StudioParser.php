...
class StudioParser {
	...
	function parsePositions($str, $output= false) {
		$results = array();
		preg_match_all("'<span[^>]*dotb=[\'\"]+([a-zA-Z\_]*)([0-9]+)([b]*)[\'\"]+[^>]*>(.*?)</span[ ]*dotb=[\'\"]+[a-zA-Z0-9\_]*[\'\"]+>'si", $str, $results, PREG_SET_ORDER);
		if($output){
			return $results;
		}
	...
	function positionCount($str) {
		$result = array ();
		return preg_match_all("'<span[^>]*dotb=[\'\"]+([a-zA-Z\_]*)([0-9]+)([b]*)[\'\"]+[^>]*>(.*?)</span[ ]*dotb=[\'\"]+[a-zA-Z0-9\_]*[\'\"]+>'si", $str, $result, PREG_SET_ORDER)/2;
	}
...
EOQ;
	...
	function generateButtons(){
        ...
        global $mod_strings;
		$imageSave = DotbThemeRegistry::current()->getImage( 'studio_save', '',null,null,'.gif',$mod_strings['LBL_SAVE']);
		$imagePublish = DotbThemeRegistry::current()->getImage( 'studio_publish', '',null,null,'.gif',$mod_strings['LBL_PUBLISH']);
		$imageHistory = DotbThemeRegistry::current()->getImage( 'studio_history', '',null,null,'.gif',$mod_strings['LBL_HISTORY']);
		$imageAddRows = DotbThemeRegistry::current()->getImage('studio_addRows', '',null,null,'.gif',$mod_strings['LBL_ADDROWS']);
		$imageUndo = DotbThemeRegistry::current()->getImage('studio_undo', '',null,null,'.gif',$mod_strings['LBL_UNDO']);
		$imageRedo = DotbThemeRegistry::current()->getImage('studio_redo', '',null,null,'.gif',$mod_strings['LBL_REDO']);
		$imageAddField = DotbThemeRegistry::current()->getImage( 'studio_addField', '',null,null,'.gif',$mod_strings['LBL_ADDFIELD']);
		$buttons = array();
...
EOQ;
	...
	function getFiles($module, $fileId=false){
		if(empty($GLOBALS['studioDefs'][$module])){
            DotbAutoLoader::requireWithCustom('modules/'. $module . '/metadata/studio.php');
		}
		if($fileId){
	...
	function handleSave() {
		...
		for ($i = 0; $i < sizeof($this->positions); $i ++) {
			...
			$explode[0] .= "<span dotb='". $slot[1] . $slot[2]. $slot[3]. "'>";
			$explode[1] = "</span dotb='" .$slot[1] ."'>".$explode[1];
...
"'(href[\ ]*=[\ ]*)([\'])([^\']*)([\'])'si" => "href=\$2javascript:void(0);\$2 alt=\$2\$3\$2", "'(href[\ ]*=[\ ]*)([\"])([^\"]*)([\"])'si" => "href=\$2javascript:void(0)\$2 title=\$2\$3\$2");
	...
	function enableLabelEditor($str) {
        global $mod_strings;
		$image = DotbThemeRegistry::current()->getImage( 'edit_inline', "onclick='studiojs.handleLabelClick(\"$2\", 1);' onmouseover='this.style.cursor=\"default\"'",null,null,'.gif',$mod_strings['LBL_EDIT']);
	...
	function populateRequestFromBuffer($file) {
		$results = array ();
		$temp = dotb_file_get_contents($file);
	...
	function cacheXTPL($file, $cache_file, $preview_file = false) {
		...
		$buffer = dotb_file_get_contents($xtpl);
		...
		$buffer = "<?php\n\$dotb_config['list_max_entries_per_page'] = 1;\n ?>".$buffer;

		$buffer = str_replace($form_string, '', $buffer);
		$buffer = $this->disableInputs($buffer);
		$xtpl_fp_cache = dotb_fopen($xtpl_cache, 'w');
	...
	function parseOldestFile($file){
	 ob_clean();
		require_once('modules/Studio/DotbBackup.php');
		$file = str_replace('custom/working/', '' ,$file);

		$filebk = DotbBackup::oldestBackup($file);
		$oldMatches = array();
		$oldLabels = array();
		// echo $filebk;
		if($filebk){
