...
class OutboundEmailApiHelper extends DotbBeanApiHelper
{
    /**
     * {@inheritdoc}
     *
     ...
     * of any type.
     */
    public function populateFromApi(\DotbBean $bean, array $submittedData, array $options = [])
    {
    ...
    public function formatForApi(DotbBean $bean, array $fieldList = array(), array $options = array())
    {
         ...
         * response object by DotbFieldRelate::apiFormatField(). This overwrites the email address that
         * might have been set on the bean by the logic from OutboundEmail::populateFromRow() and
         * OutboundEmail::populateFromUser() when the bean was first loaded by DotbBean::fetchFromQuery().
         * We want to respond with the data that is already on set on the bean. Unsetting the related bean for the
         * email_address link guarantees that DotbFieldRelate::apiFormatField() will fall back to using the existing
