...
{
    /**
     * This method, just set the date_modified to the value from the db, vs the user formatted value that dotbbean sets
        ...
        if ($event == 'before_save' && $worksheet->draft == 1 && $worksheet->draft_save_type == 'assign_quota') {
            ...
            foreach ($fields as $field) {
                if (($field['type'] == 'currency') && preg_match('#\.[\d]{6}$#', $worksheet->{$field['name']}) === 0) {
                    $worksheet->{$field['name']} = DotbMath::init($worksheet->{$field['name']}, 6)->result();
                }
