...
class ForecastManagerWorksheet extends DotbBean
{
    public $args;
    public $user_id;
    public $version;
        ...
        if (empty($beans) && empty($this->date_modified)) {
            ...
            if (empty($beans) && !empty($this->date_modified)) {
                ...
                if (strtotime($committed_date) < strtotime($this->date_modified)) {
                    ...
                    // we use a direct query since DotbQuery can't do the audit tables...
                    $sql = sprintf(
                    ...
                    while ($row = $this->db->fetchByAssoc($results)) {
                        ...
                        if ($settings['show_worksheet_' . $field] == "1") {
                            ...
                            $diff = DotbMath::init($row['after_value_string'], 6)->sub(
    ...
    protected function recalcUserQuota($userId, $timeperiodId)
    {
        ...
        $newTotal = DotbMath::init($managerAmount, 6)->sub($reporteeTotal)->result();
         ...
         */
        if (DotbMath::init($reporteeTotal, 6)->add($quota->amount)->result() > $quotaAmount ||
    ...
    public function worksheetTotals($userId, $timeperiodId, $useDraftRecords = false)
    {
        ...
        foreach ($results as $row) {
            $return['quota'] = DotbMath::init($return['quota'], 6)->add(
                DotbCurrency::convertWithRate($row['quota'], $row['base_rate'])
            )->result();
            $return['best_case'] = DotbMath::init($return['best_case'], 6)->add(
                DotbCurrency::convertWithRate($row['best_case'], $row['base_rate'])
            )->result();
            $return['best_adjusted'] = DotbMath::init($return['best_adjusted'], 6)->add(
                DotbCurrency::convertWithRate($row['best_case_adjusted'], $row['base_rate'])
            )->result();
            $return['likely_case'] = DotbMath::init($return['likely_case'], 6)->add(
                DotbCurrency::convertWithRate($row['likely_case'], $row['base_rate'])
            )->result();
            $return['likely_adjusted'] = DotbMath::init($return['likely_adjusted'], 6)->add(
                DotbCurrency::convertWithRate($row['likely_case_adjusted'], $row['base_rate'])
            )->result();
            $return['worst_case'] = DotbMath::init($return['worst_case'], 6)->add(
                DotbCurrency::convertWithRate($row['worst_case'], $row['base_rate'])
            )->result();
            $return['worst_adjusted'] = DotbMath::init($return['worst_adjusted'], 6)->add(
                DotbCurrency::convertWithRate($row['worst_case_adjusted'], $row['base_rate'])
            )->result();
            $return['closed_amount'] = DotbMath::init($return['closed_amount'], 6)->add(
                DotbCurrency::convertWithRate($row['closed_amount'], $row['base_rate'])
            )->result();

            $return['included_opp_count'] += $row['opp_count'];
            $return['pipeline_opp_count'] += $row['pipeline_opp_count'];
            $return['pipeline_amount'] = DotbMath::init($return['pipeline_amount'], 6)
    ...
    {
        DotbAutoLoader::load('modules/ActivityStream/Activities/ActivityQueueManager.php');
