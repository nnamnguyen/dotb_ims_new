...
class ForecastWorksheet extends DotbBean
{
     ...
     */
    public function removeMigratedRow(DotbBean $bean)
    {
        $return = false;
    ...
    {
        $returnValue = '';
        $dotb_query = new DotbQuery();
        $dotb_query->select('name');
        $dotb_query->from(BeanFactory::newBean($module))->where()->equals('id', $id);
        $item = $dotb_query->execute();
     ...
     *
     * @param array $fields
     * @param DotbBean|array $seed
     */
    public function copyValues($fields, $seed)
    {
        if ($seed instanceof DotbBean) {
            $seed = $seed->toArray();
        }
    ...
    public function commitWorksheet($user_id, $timeperiod, $chunk_size = 50)
    {
        ...
        $bean_obj->$link_name->buildJoinDotbQuery($sq, array('joinTableAlias' => 'account'));
    ...
    protected function createUpdateForecastWorksheetJob($forecast_by, array $data, $user_id)
    {
        ...
        $job->target = "class::DotbJobUpdateForecastWorksheets";
    ...
    public function createRemoveReassignedJob($data, $user_id)
    {
        ...
        $job->target = 'class::DotbJobRemoveReassignedItems';
    ...
    {
        DotbAutoLoader::load('include/DotbQueue/DotbJobQueue.php');
        return new DotbJobQueue();
    }

    /**
     * Utility function to get a new DotbQuery instance (to aid in test writing)
     * @return DotbQuery
     */
    ...
    public function worksheetTotals($timeperiod_id, $user_id, $forecast_by = null, $useDraftRecords = false)
    {
        ...
        foreach ($results as $row) {
            ...
            $worst_base = DotbCurrency::convertWithRate($row['worst_case'], $row['base_rate']);
            $amount_base = DotbCurrency::convertWithRate($row['likely_case'], $row['base_rate']);
            $best_base = DotbCurrency::convertWithRate($row['best_case'], $row['base_rate']);

            $closed = false;
            if (in_array($row['sales_stage'], $settings['sales_stage_won']) && in_array($row['commit_stage'], $settings['commit_stages_included'])) {
                $return['won_amount'] = DotbMath::init($return['won_amount'], 6)->add($amount_base)->result();
                $return['won_best'] = DotbMath::init($return['won_best'], 6)->add($best_base)->result();
                $return['won_worst'] = DotbMath::init($return['won_worst'], 6)->add($worst_base)->result();
                $return['won_count']++;
                $return['includedClosedCount']++;
                $return['includedClosedAmount'] = DotbMath::init($return['includedClosedAmount'], 6)
                    ->add($amount_base)->result();
                $closed = true;
            } elseif (in_array($row['sales_stage'], $settings['sales_stage_lost'])) {
                $return['lost_amount'] = DotbMath::init($return['lost_amount'], 6)->add($amount_base)->result();
                $return['lost_best'] = DotbMath::init($return['lost_best'], 6)->add($best_base)->result();
                $return['lost_worst'] = DotbMath::init($return['lost_worst'], 6)->add($worst_base)->result();
                $return['lost_count']++;
                $closed = true;
            }
            ...
            if (in_array($row['commit_stage'], $settings['commit_stages_included'])) {
                if (!$closed) {
                    $return['amount'] = DotbMath::init($return['amount'], 6)->add($amount_base)->result();
                    $return['best_case'] = DotbMath::init($return['best_case'], 6)->add($best_base)->result();
                    $return['worst_case'] = DotbMath::init($return['worst_case'], 6)->add($worst_base)->result();
                ...
                if ($closed) {
                    $return['includedClosedBest'] = DotbMath::init($return['includedClosedBest'], 6)
                        ->add($best_base)->result();
                    $return['includedClosedWorst'] = DotbMath::init($return['includedClosedWorst'], 6)
                        ->add($worst_base)->result();
                }
            }

            $return['total_opp_count']++;
            $return['overall_amount'] = DotbMath::init($return['overall_amount'], 6)->add($amount_base)->result();
            $return['overall_best'] = DotbMath::init($return['overall_best'], 6)->add($best_base)->result();
            $return['overall_worst'] = DotbMath::init($return['overall_worst'], 6)->add($worst_base)->result();
        }
