...
class SavedSearch extends DotbBean {
	// Stored fields
	var $id;
	var $date_entered;
	var $date_modified;
	...
	function getForm($module, $inline = true) {
		...
		$dotbSmarty->assign('SEARCH_MODULE', $module);
		$dotbSmarty->assign('MOD', $saved_search_mod_strings);
		$dotbSmarty->assign('DELETE', $app_strings['LBL_DELETE_BUTTON_LABEL']);
        $dotbSmarty->assign('UPDATE', $app_strings['LBL_UPDATE']);
		$dotbSmarty->assign('SAVE', $app_strings['LBL_SAVE_BUTTON_LABEL']);
        ...
        $chooser->args['title'] =  '';
        $dotbSmarty->assign('columnChooser', $chooser->display());

        $dotbSmarty->assign('selectedOrderBy', $this->orderBy);
        if(empty($this->sortOrder)) $this->sortOrder = 'ASC';
        $dotbSmarty->assign('selectedSortOrder', $this->sortOrder);

        $lastSavedView = (empty($_SESSION['LastSavedView'][$module]) ? '' : $_SESSION['LastSavedView'][$module]);
        $dotbSmarty->assign('columnsMeta', $json->encode($this->columns));
        $dotbSmarty->assign('lastSavedView', $lastSavedView);
        $dotbSmarty->assign('SAVED_SEARCHES_OPTIONS', get_select_options_with_id($savedSearchArray, $lastSavedView));

        $json = getJSONobj();

        return $dotbSmarty->fetchCustom('modules/SavedSearch/SavedSearchForm.tpl');
	}
    ...
    function getSelect($module) {
        ...
        $dotbSmarty->assign('SAVED_SEARCHES_OPTIONS', get_select_options_with_id($savedSearchArray, $selectedSearch));

        return $dotbSmarty->fetch('modules/SavedSearch/SavedSearchSelects.tpl');
    }
