<?php
    ...
    class SavedReport extends Basic
    {
        ...
        function save_report(
            ...
            // Added by HP to Custom URL to execute Report
            $this->custom_url       = $_REQUEST['custom_url'];
            $this->is_admin_data    = $_REQUEST['is_admin_data'];
            $this->description      = $_REQUEST['description'];
            if($id == -1 && ($report_type == 'summary' || $report_type == 'detailed_summary'))
                $this->row_number       = 1;
            else
                $this->row_number       = $_REQUEST['row_number'];
            $this->list_of          = encodeMultienumValue($_REQUEST['list_of']);
            $resultArray            = array_filter(array_map("html_entity_decode", $_REQUEST['json_query']));
            if(!empty($resultArray)) $this->replace_str      = $json->encode(($resultArray));
            // End by HP
        ...
        /*The following function has been modified in order to better integrate the Report Lists with the core Dotb functions
        ...
        */
        ...
        function get_list_view_data(){
            ...
            $view = sprintf("%s %s", DotbThemeRegistry::current()->getImage('view_inline','border="0" align="absmiddle"',null,null,'.gif',$mod_strings['LBL_VIEW']), $mod_strings['LBL_VIEW']);

            //logic for showing delete, publish, and unpublish buttons
            $delete_img = DotbThemeRegistry::current()->getImage('delete_inline','border="0" align="absmiddle"',null,null,'.gif',$mod_strings['LBL_DELETE']);
            ...
            $unpublish_img = DotbThemeRegistry::current()->getImage('unpublish_inline','border="0" align="absmiddle"',null,null,'.gif',$mod_strings['LBL_UN_PUBLISH']);
            $publish_img = DotbThemeRegistry::current()->getImage('publish_inline','border="0" align="absmiddle"',null,null,'.gif',$mod_strings['LBL_PUBLISH']);
