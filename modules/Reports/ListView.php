...
require DotbAutoLoader::loadWithMetafiles('Reports', 'listviewdefs');
...
$listViewDefsNewArray = dotbArrayMerge($listViewDefsNewArray, $listViewDefs);
...
if(!empty($_REQUEST['favorite'])) { // handle favorite requests
    ...
    array_push($where_clauses,
        'saved_reports.id IN (
            SELECT dotbfavorites.record_id FROM dotbfavorites
                WHERE dotbfavorites.deleted=0
                    and dotbfavorites.module = \'Reports\'
                    and dotbfavorites.assigned_user_id = \''.$GLOBALS['current_user']->id.'\')');
}
