...
if (isset($_REQUEST['id']) && !isset($_REQUEST['record'])) {
    ...
    //Add By Lap Nguyen - Show data by admin permission
    if($saved_report_seed->is_admin_data == '1' && !$current_user->isAdmin()){
...
if(!empty($args['reporter']->saved_report->id)){
    $star = DotbFavorites::generateStar(DotbFavorites::isUserFavorite('Reports', $args['reporter']->saved_report->id), 'Reports', $args['reporter']->saved_report->id);
}
...
if (isset($_REQUEST['page']) && $_REQUEST['page'] == 'report') {
    ...
    if (!empty($_REQUEST['to_csv'])) {
        if (!hasExportAccess($args)) {
            dotb_die($mod_strings['LBL_NO_EXPORT_ACCESS']);
        }
