...
include DotbAutoLoader::existingCustomOne("modules/Reports/ListView.php");
}
...
function checkACLForEachColInArr ($arr, $full_table_list, $is_owner = 1){
	foreach ($arr as $column) {
		...
		if(!DotbACL::checkField($col_module, $column['name'], 'access', $is_owner?array("owner_override" => true):array())) {
			return false;
		}
	}
	return true;
...
function checkSavedReportACL(&$reporter,&$args) {
	...
	if (!empty($_REQUEST['id'])) {
		...
		if (!checkACLForEachColInArr($display_columns, $full_table_list) ||
			...
			dotb_die($mod_strings['LBL_NO_ACCESS']);
		}
