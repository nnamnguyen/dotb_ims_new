...
class ReportsDotbpdfSummary_combo extends ReportsDotbpdfReports
{
    function display()
    {
        ...
        if ($this->bean->chart_type != 'none') {
            ...
            if ($this->bean->isScheduledReport && empty(DotbConfig::getInstance()->get('schedule_report_with_chart'))) {
                $shouldIncludeImage = false;
            }
            if ($dotbChart->supports_image_export && $shouldIncludeImage) {
                $imageFile = $dotbChart->get_image_cache_file_name($xmlFile, ".".$dotbChart->image_export_type);
                ...
                if (file_exists($imageFile) && getimagesize($imageFile) > 0) {
                    ...
                    if ($dotbChart->print_html_legend_pdf) {
                        $legend = $dotbChart->buildHTMLLegend($xmlFile);
