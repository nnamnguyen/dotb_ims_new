...
class Report
{
    ...
    */
    ...
    // this is a var not in metadata that is set to bypass the dotb_die calls used throughout this class
    ...
    */
    ...
    public function __construct($report_def_str = '', $filters_def_str = '', $panels_def_str = '')
    {
        ...
        if ((!isset($_REQUEST['module']) || $_REQUEST['module'] == 'Reports') && !defined('DOTB_PHPUNIT_RUNNER')) {
            Report::cache_modules_def_js();
        }

        $mod_strings = return_module_language($current_language, 'Reports');

        $this->report_max = (!empty($GLOBALS['dotb_config']['list_report_max_per_page']))
        ? $GLOBALS['dotb_config']['list_report_max_per_page'] : 100;
    ...
    */
        ...
        $row_start_name = 'row_start', $row_end_name = 'row_end', $limit = false)
    {
        ...
        $custom_url = $this->saved_report->custom_url;
        if(empty($custom_url))
            $custom_url = $GLOBALS['db']->getOne("SELECT IFNULL(custom_url, '') custom_url FROM saved_reports WHERE id = '{$this->saved_report_id}'");

        if(!empty($custom_url)){
            include_once($custom_url);
        }
        //END
    ...
    * @return string
    */
    private function addVisibilityFrom(DotbBean $bean, $from, $tableAlias)
    {
    ...
    * @return string
    */
    private function addVisibilityWhere(DotbBean $bean, $where, $tableAlias)
    {
        $options = $this->getVisibilityOptions();
        $options['table_alias'] = $tableAlias;
    ...
    */
    private function applyVisibilityToJoin(DotbBean $bean, string $join, string $tableAlias) : string
    {
    ...
    function create_query($query_name = 'query', $field_list_name = 'select_fields')
    {
        ...
        if($_REQUEST['return_action'] == 'ReportsWizardType'){    //Preview Page
            //array_shift($_REQUEST['json_query']);
            $json_query = $_REQUEST['json_query'];
            $jsonQs     = array_filter(array_map("html_entity_decode", $json_query));
        }elseif(!empty($this->saved_report->replace_str))
            $jsonQs   = json_decode(html_entity_decode($this->saved_report->replace_str));
        else{
            $replace_str = $GLOBALS['db']->getOne("SELECT IFNULL(replace_str, '') replace_str FROM saved_reports WHERE id = '{$this->saved_report_id}'");
            $jsonQs   = json_decode(html_entity_decode($replace_str));
        }

        if(!empty($jsonQs)){
            foreach($jsonQs as $key => $jsonQ){
                $json = json_decode(html_entity_decode($jsonQ),true);
                if($json['query'] == $query_name)
                    $this->$query_name = str_replace($this->parseValue($json['search']), $this->parseValue($json['replace']), $this->$query_name);
            }
        }
        //END:
    ...
    function get_header_row_generic($column_field_name = 'display_columns', $skip_non_group = false, $exporting = false, $force_distinct = false)
    {
        ...
        // Bug 29829 Make sure the header names are distinct labels for dotbpdf writeCellTable()
        if ($force_distinct) {
        ...
        foreach ($this->report_def[$column_field_name] as $display_column) {
            ...
            if ($display_column['type'] == 'currency' && (strpos($display_column['name'], '_usdoll') !== false || !empty($display_column['group_function']))) {
                ...
                if($current_user->getPreference('currency_show_preferred')) {
                    $userCurrency = DotbCurrency::getUserLocaleCurrency();
                    $raw_display = DotbCurrency::convertWithRate($display_column['fields'][$field_name], 1.0, $userCurrency->conversion_rate);
                    $display = DotbCurrency::formatAmountUserLocale($raw_display, $userCurrency->id);
                } else {
                    $raw_display = $display_column['fields'][$field_name];
                    $display = DotbCurrency::formatAmountUserLocale($raw_display, DotbCurrency::getBaseCurrency()->id);
                }
            } else {
                $raw_display = $display;
            }
    ...
    */
    public static function clearCaches($user = null)
    {
        ...
        foreach (glob(dotb_cached('modules').'/modules_def_*'.$md5.'.js') as $file) {
            unlink($file);
        }
    }
    ...
    function cache_modules_def_js()
    {
        ...
        $cacheDefsJs = dotb_cached('modules/modules_def_' . $current_language . '_' . md5($current_user->id) . '.js');
        $cacheFiscalJs = dotb_cached('modules/modules_def_fiscal_' . $current_language . '_' . md5($current_user->id) . '.js');
        ...
        foreach ($files as $file) {
            ...
            if (!file_exists($fileName)) {
                ...
                if (is_writable(dotb_cached('modules/'))) {
    ...
    */
    ...
    protected function handleException($msg, $exit_code=1) {
        if($this->fromApi === false) {
            dotb_die($msg, $exit_code);
        } else {
            throw new DotbApiExceptionNotAuthorized($msg);
        }
    }
    ...
    */
    ...
    function parseValue($data){
        ...
        global $timedate;
        if(strpos($data,'$start_date')!== false || strpos($data,'$end_date')!== false ){
            $filter = preg_replace("/\s+/", ' ', $this->where);
            $parts = explode("AND", $filter);
            for($i = 0; $i < count($parts); $i++){
                if(strpos($parts[$i], ">= '") !== FALSE || strpos($parts[$i], ">='") !== FALSE) $start_date = get_string_between($parts[$i]);
                if(strpos($parts[$i], "<= '") !== FALSE || strpos($parts[$i], "<='") !== FALSE) $end_date   = get_string_between($parts[$i]);
            }
            //Set Start Date
            if(DateTime::createFromFormat('Y-m-d H:i:s', $start_date) !== FALSE)
                $start_date = $timedate->convertToDBDate($timedate->to_display_date($start_date));
            if(DateTime::createFromFormat('Y-m-d', $start_date) !== FALSE)
                $data = str_replace('$start_date', $start_date, $data);

            //Set End Date
            if(DateTime::createFromFormat('Y-m-d H:i:s', $end_date) !== FALSE)
                $end_date = $timedate->convertToDBDate($timedate->to_display_date($end_date));
            if(DateTime::createFromFormat('Y-m-d', $end_date) !== FALSE)
            $data = str_replace('$end_date', $end_date, $data);

        }elseif(strpos($data,'$current_user_id')!== false || strpos($data,'$current_user')!== false ){
             $data = str_replace('$current_user_id', $GLOBALS['current_user']->id, $data);
        }elseif(strpos($data,'$str')!== false){
             $pos  = strpos($data, '$str');
             $str  = str_replace('$','',substr($data, $pos, 5 ));
             $data = str_replace('$'.$str, $this->$str, $data);
        }
