...
{/php}
{* Added by HP to create a col for row number *}
...
function displayGroupCount() {
	for (i in groupCountObject) {
		...
		if (elem != null) {
			elem.innerHTML = trim(elem.innerHTML) + ', ' + DOTB.language.get('app_strings','LBL_REPORT_NEWREPORT_COLUMNS_TAB_COUNT') +' = ' + groupCountObject[i];
		}
	} // for
} // fn
