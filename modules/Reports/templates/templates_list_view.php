<?php
    ...
    function template_pagination(&$args) {
        ...
        global $current_user, $dotb_config, $app_strings;
        ...
        if(!ACLController::checkAccess('Reports', 'export', $is_owner) || $dotb_config['disable_export'] || (!empty($dotb_config['admin_export_only']) && !(is_admin($current_user) || (ACLController::moduleSupportsACL($reporter->module) && ACLAction::getUserAccessLevel($current_user->id,$reporter->module, 'access') == ACL_ALLOW_ENABLED && ACLAction::getUserAccessLevel($current_user->id, $reporter->module, 'admin') == ACL_ALLOW_ADMIN)))){
            // no op
        } else {
            $smarty->assign('exportImagePath', DotbThemeRegistry::current()->getImage('export',"  border='0' align='absmiddle'",null,null,'.gif',translate('LBL_EXPORT')));
            $isExportAccess = true;
        } // else
        $smarty->assign('isExportAccess', $isExportAccess);
        $smarty->assign('start_link_ImagePath', DotbThemeRegistry::current()->getImage("start_off","  border='0' align='absmiddle'",null,null,'.gif',$app_strings['LNK_LIST_START']));
        $smarty->assign('prev_link_ImagePath', DotbThemeRegistry::current()->getImage("previous_off","border='0' align='absmiddle'",null,null,'.gif',$app_strings['LNK_LIST_PREVIOUS']));
        $smarty->assign('end_link_ImagePath', DotbThemeRegistry::current()->getImage("end_off","border='0' align='absmiddle'",null,null,'.gif',$app_strings['LNK_LIST_END']));
        $smarty->assign('next_link_ImagePath', DotbThemeRegistry::current()->getImage("next_off","border='0' align='absmiddle'",null,null,'.gif',$app_strings['LNK_LIST_NEXT']));
        ...
        if($reporter->report_offset > 0){
            ...
            $smarty->assign('start_link_ImagePath', DotbThemeRegistry::current()->getImage("start","  border='0' align='absmiddle'",null,null,'.gif',$app_strings['LNK_LIST_START']));
            $smarty->assign('start_link_onclick' , "onClick=javascript:set_offset(0);");
            $smarty->assign('start_link_disabled', false);
            $smarty->assign('prev_link_ImagePath', DotbThemeRegistry::current()->getImage("previous","border='0' align='absmiddle'",null,null,'.gif',$app_strings['LNK_LIST_PREVIOUS']));
        ...
        if($next < $reporter->total_count){
            ...
            $smarty->assign('end_link_ImagePath', DotbThemeRegistry::current()->getImage("end","  border='0' align='absmiddle'",null,null,'.gif',$app_strings['LNK_LIST_END']));
            $smarty->assign('end_link_disabled', false);
            $smarty->assign('end_link_onclick' , "onClick=javascript:set_offset($end);");

            $smarty->assign('next_link_ImagePath', DotbThemeRegistry::current()->getImage("next"," border='0' align='absmiddle'",null,null,'.gif',$app_strings['LNK_LIST_NEXT']));
    ...
    function getGroupByInfo($groupByArray, $summary_columns_array) {
        ...
        for ($i = 0 ; $i < count($summary_columns_array) ; $i++) {
            ...
            /* Added by HP to fix error None values after edit */
    ...
    function template_list_view_no_results($args) {
        ...
        if($args['reporter']->saved_report->row_number)
            $returnString .= "<td>&nbsp;&nbsp;&nbsp;{$mod_strings['LBL_NO_REPORTS']}</td>";
...
?>
    ...
    function template_list_row(&$column_row,$equal_width=false, $isSummaryComboHeader=false, $divId="") {
        ...
        if($list_type == 'summary') {
            ...
            if ($list_type == 'list' || $list_type == 'summary_combo') {
                ...
                $display_columns = $reporter->report_def['summary_columns'] + $reporter->report_def['display_columns'];
            }
        }
        $field_types = array();
        foreach($display_columns as $display_column) {
        ...
        $report_smarty->assign('count_c',0);
    ...
    <tr height="20" class="<?php echo $row_class; ?>">
        <?php
            ...
            if ($isSummaryComboHeader) {
                echo '<td><span id="img_'.$divId.'"><a href="javascript:expandCollapseComboSummaryDiv(\''.$divId.'\')">'.DotbThemeRegistry::current()->getImage("advanced_search", 'border="0" absmiddle=""', 8, 8, ".gif", $mod_strings['LBL_SHOW']).'"</a></span></td>';
            }
    ...
    <table width="100%" id="query_table" class="contentBox">
        ...
        ?>
                ...
                <tr>
                    <td align=left><span><?php echo $query; ?></span></td>
                </tr>
            <?php
                }
            ?>
