...
<input type="hidden" id="blankimagepath" name="blankimagepath" value="{dotb_getimagepath file='blank.gif'}">
...
{dotb_action_menu id="detail_header_action_menu" buttons=$action_button class="clickMenu fancymenu"}
</td>
</tr>
</table>
...
<div scope="row"><h3>{$mod_strings.LBL_RUNTIME_FILTERS}:<span valign="bottom">&nbsp;{dotb_help text=$mod_strings.LBL_VIEWER_RUNTIME_HELP }</span></h3>
</div>
...
<script type="text/javascript" src="{dotb_getjspath file='include/javascript/reportCriteria.js'}"></script>
<script type="text/javascript" src="{dotb_getjspath file='include/javascript/reportsInlineEdit.js'}"></script>
<!--Add scripts to custom Report result - By Lap Nguyen-->
{dotb_getscript file="modules/Reports/javascript/Blobs.js"}
...
function showFilterString() {
 if(YAHOO.util.Dom.get('filter_results') && YAHOO.util.Dom.get('filter_results_text')) {
    ...
    document.getElementById('filter_results_image').src = 'index.php?entryPoint=getImage&themeName=' + DOTB.themes.theme_name + '&imageName=' + (expand ? 'advanced_search.gif' : 'basic_search.gif');
...
function schedulePOPUP(){
    ...
    var dotbApp = DOTB.App || DOTB.app || app;
    var newModel = dotbApp.data.createBean('ReportSchedules');
    newModel.set({
        report_id : id,
        report_name: name
    });
...
function viewSchedulesPOPUP(){
    ...
    var name = document.getElementById('report_name').value;
    var dotbApp = DOTB.App || DOTB.app || app;
    var filterOptions = new dotbApp.utils.FilterOptions().config({
