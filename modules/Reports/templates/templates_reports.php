...
function reportCriteriaWithResult(&$reporter,&$args) {
	...
	$smarty->assign('dotb_config', $dotb_config);
	...
	$report_edit_access = DotbACL::checkAccess('Reports', 'edit', $context);
	$smarty->assign('report_edit_access', $report_edit_access);
	$report_delete_access = DotbACL::checkAccess('Reports', 'delete', $context);
	$smarty->assign('report_delete_access', $report_delete_access);
	$report_export_access = DotbACL::checkAccess('Reports', 'export', $context);
	$smarty->assign('report_export_access', $report_export_access);
    $schedule_edit_access = DotbACL::checkAccess('ReportSchedules', 'edit');
    $smarty->assign('schedule_edit_access', $schedule_edit_access);
    $schedule_list_access = DotbACL::checkAccess('ReportSchedules', 'list');
    ...
    $smarty->assign('cache_path', dotb_cached(''));
...
EOD
    ...
    $shareButtonCode = 'parent.DOTB.App.bwc.shareRecord("Reports", "$report_id", '.$reportNameEncoded.");";
    $buttons[] = <<<EOD
...
EOD
        ;
    }else{ //Button Export Excel - Add By Lap Nguyen
        $buttons[] = <<<EOD
             <a class="button" style="float: left; margin-left: 5px; margin-right: 5px;" name="exportToExcel" id="exportToExcel" href="javascript:void(0)" title="Export to Excel">Export Excel</a>
EOD;
    }
    ...
    if ($report_delete_access) {
        ...
        <input type="button" class="button"  name="deleteReportButton" id="deleteReportButton" accessKey="{$app_strings['LBL_DELETE_BUTTON_KEY']}" value="{$app_strings['LBL_DELETE_BUTTON_LABEL']}" title="{$app_strings['LBL_DELETE_BUTTON_TITLE']}"
               onclick="if (confirm(DOTB.language.get('app_strings','NTC_DELETE_CONFIRMATION'))){this.form.to_pdf.value='';this.form.to_csv.value='';this.form.is_delete.value='1';this.form.action.value='ReportsWizard';this.form.submit();}">
EOD
        ;
    }
    ...
    if (isset($reporter->report_def['filters_def']) && !isset($reporter->report_def['filters_def']['Filter_1'][0])){
        ...
        $reportFilters = "<span id=\"filter_results\" valign=\"bottom\">&nbsp;<img id=\"filter_results_image\" src=\"".DotbThemeRegistry::current()->getImageURL('basic_search.gif')."\" width=\"8px\" height=\"10px\" onclick=\"showFilterString();\"></span><span id=\"filter_results_text\" style=\"visibility:hidden;\"></span>";
    } // else
...
function template_reports_report(&$reporter,&$args) {
	...
	$tab_panel= new DotbWidgetTabs($tabs,$current_key,'showReportTab');
...
function hasExportAccess($args = array())
{
    ...
    if (// Exports disabled
        ...
        || !DotbACL::checkAccess($args['reporter']->module, 'export', $is_owner?array("owner_override" => true):array())
        // Only admins can export, and the user doesn't have admin rights
        || (
            $dotb_config['admin_export_only']
