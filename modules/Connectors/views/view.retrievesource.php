...
class ViewRetrieveSource extends ViewList {
     ...
     * @param beans The PHP array of filled DotbBean results
    ...
    private function getDisplayColumns($source_instance, $beans) {
		...
		$file = DotbAutoLoader::existingCustomOne("modules/Connectors/connectors/sources/{$dir}/listviewdefs.php");
		if($file) {
		    require $file;
		}
        ...
        $dotb_bean_field_defs = $beans[0]->getFieldDefinitions();
        ...
        if(isset($listViewDefs[$source_id])) {
        	foreach($listViewDefs[$source_id] as $key=>$listDef) {
        		...
        		if(isset($output_list[$check_key])) {
        		   ...
        		   $label = isset($connector_field_defs[$key]['vname']) ? $connector_strings[$connector_field_defs[$key]['vname']] : $dotb_bean_field_defs[$output_list[$check_key]]['vname'];
			...
			foreach($output_list as $key=>$value) {
				...
				} else if(isset($dotb_bean_field_defs[$value]['vname'])){
				  $label = $dotb_bean_field_defs[$value]['vname'];
				} else {
				  $label = $value;
				}
