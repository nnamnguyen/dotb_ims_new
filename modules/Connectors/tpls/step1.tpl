...
<script type="text/javascript" src="{dotb_getjspath file='modules/Connectors/tpls/modify_mapping.js'}"></script>
...
function get_source_details(source, id, spanId){
		go = function() {
			oReturn = function(body, caption, width, theme) {
						return overlib(body, CAPTION, caption, STICKY, MOUSEOFF, 1000, WIDTH, width, CLOSETEXT, ('<img border=0 style="margin-left:2px; margin-right: 2px;" align="right"alt=$mod_strings.LBL_CLOSE src="themes/default/images/close.gif?v='+DOTB.VERSION_MARK+'">'), CLOSETITLE, 'Click to Close', CLOSECLICK, FGCLASS, 'olFgClass', CGCLASS, 'olCgClass', BGCLASS, 'olBgClass', TEXTFONTCLASS, 'olFontClass', CAPTIONFONTCLASS, 'olCapFontClass', CLOSEFONTCLASS, 'olCloseFontClass', REF, spanId, REFC, 'LL', REFX, 13);
					}
			success = function(data) {
						eval(data.responseText);
	
						DOTB.util.additionalDetailsCache[spanId] = new Array();
						DOTB.util.additionalDetailsCache[spanId]['body'] = result['body'];
						DOTB.util.additionalDetailsCache[spanId]['caption'] = result['caption'];
						DOTB.util.additionalDetailsCache[spanId]['width'] = result['width'];
						DOTB.util.additionalDetailsCache[spanId]['theme'] = result['theme'];
						ajaxStatus.hideStatus();
						return oReturn(DOTB.util.additionalDetailsCache[spanId]['body'], DOTB.util.additionalDetailsCache[spanId]['caption'], DOTB.util.additionalDetailsCache[spanId]['width'], DOTB.util.additionalDetailsCache[spanId]['theme']);
					}
	
					if(typeof DOTB.util.additionalDetailsCache[spanId] != 'undefined')
						return oReturn(DOTB.util.additionalDetailsCache[spanId]['body'], DOTB.util.additionalDetailsCache[spanId]['caption'], DOTB.util.additionalDetailsCache[spanId]['width'], DOTB.util.additionalDetailsCache[spanId]['theme']);
	
					if(typeof DOTB.util.additionalDetailsCalls[spanId] != 'undefined') // call already in progress
						return;
					...
					DOTB.util.additionalDetailsCalls[spanId] = YAHOO.util.Connect.asyncRequest('GET', url, {success: success, failure: success});
...
<td width='100%'><IMG height='1' width='1' src={dotb_getjspath file='include/images/blank.gif'}" alt=''></td>
</tr>
</table>
