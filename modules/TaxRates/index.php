...
if(!empty($_REQUEST['record'])) {
    ...
    $focus->value = strtr($focus->value,'.',$dotb_config['default_decimal_seperator']);
    if($result == null)
    {
...
if((is_admin($current_user) || is_admin_for_module($GLOBALS['current_user'],'Quotes')) && $module != 'DynamicLayout' && !empty($_SESSION['editinplace'])){	
		$header_text = "&nbsp;<a href='index.php?action=index&module=DynamicLayout&from_action=ListView&from_module=".htmlspecialchars($module, ENT_QUOTES, 'UTF-8') ."'>".DotbThemeRegistry::current()->getImage("EditLayout","border='0' align='bottom'",null,null,'.gif',$mod_strings['LBL_EDITLAYOUT'])."</a>";
}
...
if ($is_edit) {
		...
		if((is_admin($current_user) || is_admin_for_module($GLOBALS['current_user'],'Quotes')) && $module != 'DynamicLayout' && !empty($_SESSION['editinplace'])){	
		$header_text = "&nbsp;<a href='index.php?action=index&module=DynamicLayout&edit=true&from_action=EditView&from_module=".htmlspecialchars($module, ENT_QUOTES, 'UTF-8') ."'>".DotbThemeRegistry::current()->getImage("EditLayout","border='0'  align='bottom'",null,null,'.gif',$mod_strings['LBL_EDITLAYOUT'])."</a>";
		}
