    ...
    /**
 	...
 	public function display()
 	{
		...
		if((is_admin($current_user) || is_admin_for_module($GLOBALS['current_user'],'Quotes')) && $_REQUEST['module'] != 'DynamicLayout' && !empty($_SESSION['editinplace'])){
            ...
            )), ENT_QUOTES, 'UTF-8') . '">' . DotbThemeRegistry::current()->getImage("EditLayout","border='0' align='bottom'",null,null,'.gif',$mod_strings['LBL_EDITLAYOUT'])."</a>");
		}
		...
		if (!empty($this->bean->id))
		{
			...
			$convert_format = function($value, $prod_currency, $prod_base_rate) use ($quote_currency_id, $quote_base_rate) {
				if ($prod_currency !== $quote_currency_id) {
					$value = DotbCurrency::convertWithRate($value, $prod_base_rate, $quote_base_rate);
				}
				return DotbCurrency::formatAmountUserLocale($value, $quote_currency_id, false);
			};
