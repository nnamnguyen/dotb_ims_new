...
class QuotesDotbpdfStandard extends QuotesDotbpdfQuotes{
    /**
    ...
    private function _initOptions(){
        ...
        $this->itemOptions = array(
         ...
         //                   $mod_strings["LBL_PDF_ITEM_SELECT_DISCOUNT"]=>"8%"
                        ...
                        'stretch' => array(
                            $mod_strings["LBL_PDF_ITEM_LIST_PRICE"] => Dotbpdf::STRETCH_SCALE,
                            $mod_strings["LBL_PDF_ITEM_UNIT_PRICE"] => Dotbpdf::STRETCH_SCALE,
                            $mod_strings["LBL_PDF_ITEM_EXT_PRICE"] => Dotbpdf::STRETCH_SCALE,
                            $mod_strings["LBL_PDF_ITEM_DISCOUNT"] => Dotbpdf::STRETCH_SCALE
                        )
        );
    ...
    function display(){
        ...
        */
            ...
            foreach ($product_bundle_list as $product_bundle) {
                ...
                if (array_key_exists($product_bundle->bundle_stage, $in_total_group_stages)) {
                        ...
                        if (is_array($bundle_list)) {
                            while (list($key, $line_item) = each ($bundle_list)) {
                                ...
                                if ($line_item->object_name == "Product") {
                                    $item[$count][$mod_strings['LBL_PDF_ITEM_QUANTITY']] = format_number_dotbpdf($line_item->quantity, $locale->getPrecision(), $locale->getPrecision());
                                    ...
                                    $item[$count][$mod_strings['LBL_PDF_ITEM_LIST_PRICE']]['value'] = format_number_dotbpdf($line_item->list_usdollar, $locale->getPrecision(), $locale->getPrecision(), array_merge($format_number_array, array('convert' => true)));
                                    $item[$count][$mod_strings['LBL_PDF_ITEM_UNIT_PRICE']]['value'] = format_number_dotbpdf($line_item->discount_usdollar, $locale->getPrecision(), $locale->getPrecision(), array_merge($format_number_array, array('convert' => true)));
                                    $item[$count][$mod_strings['LBL_PDF_ITEM_EXT_PRICE']]['value'] = format_number_dotbpdf($line_item->discount_usdollar * $line_item->quantity, $locale->getPrecision(), $locale->getPrecision(), array_merge($format_number_array, array('convert' => true)));
								    if(format_number($product_bundle->deal_tot, $locale->getPrecision(), $locale->getPrecision())!= 0.00){
		                                ...
		                                $item[$count][$mod_strings['LBL_PDF_ITEM_DISCOUNT']]['value'] = format_number_dotbpdf($line_item->discount_amount, $locale->getPrecision(), $locale->getPrecision(), array_merge($format_number_array, array('convert' => false)));
		                                }
                    ...
                    if($pdf_group_subtotal){
                        ...
                        $total[0]['VALUE']['value'] = format_number_dotbpdf($product_bundle->subtotal, $locale->getPrecision(), $locale->getPrecision(), $format_number_array);
                        $total[0]['VALUE']['options'] = array("align"=>"R");
						$i = 1;
						if(format_number_dotbpdf($product_bundle->deal_tot, $locale->getPrecision(), $locale->getPrecision())!= 0.00){
	                        $total[1]['BLANK'] = ' ';
	                		$total[1]['TITLE'] =  $mod_strings['LBL_PDF_DISCOUNT'];
	                		$total[1]['VALUE']['value'] = format_number_dotbpdf($product_bundle->deal_tot, $locale->getPrecision(), $locale->getPrecision(), $format_number_array);
	                        $total[1]['VALUE']['options'] = array("align"=>"R");

	                        $total[2]['BLANK'] = ' ';
	                		$total[2]['TITLE'] =  $mod_strings['LBL_PDF_NEW_SUB'];
	                		$total[2]['VALUE']['value'] = format_number_dotbpdf($product_bundle->new_sub, $locale->getPrecision(), $locale->getPrecision(), $format_number_array);
	                        $total[2]['VALUE']['options'] = array("align"=>"R");
	                        $i = 3;
						}
                        $total[$i]['BLANK'] = ' ';
                        $total[$i]['TITLE'] = $mod_strings['LBL_PDF_TAX'];
                        $total[$i]['VALUE']['value'] =  format_number_dotbpdf($product_bundle->tax, $locale->getPrecision(), $locale->getPrecision(), $format_number_array);
                        $total[$i]['VALUE']['options'] = array("align"=>"R");
                        $i++;
                        $total[$i]['BLANK'] = ' ';
                        $total[$i]['TITLE'] = $mod_strings['LBL_PDF_SHIPPING'];
                        $total[$i]['VALUE']['value'] =  format_number_dotbpdf($product_bundle->shipping, $locale->getPrecision(), $locale->getPrecision(), $format_number_array);
                        $total[$i]['VALUE']['options'] = array("align"=>"R");
                        $i++;
                        $total[$i]['BLANK'] = ' ';
                        $total[$i]['TITLE'] = $mod_strings['LBL_PDF_TOTAL'];
                        $total[$i]['VALUE']['value'] =  format_number_dotbpdf($product_bundle->total, $locale->getPrecision(), $locale->getPrecision(), $format_number_array);
                        $total[$i]['VALUE']['options'] = array("align"=>"R");
        ...
        if(isset($this->bean->calc_grand_total) && $this->bean->calc_grand_total == 1) {
            ...
            $total[0]['TITLE'] = $mod_strings['LBL_PDF_SUBTOTAL'];
            $total[0]['VALUE']['value'] = format_number_dotbpdf($this->bean->subtotal, $locale->getPrecision(), $locale->getPrecision(), $format_number_array);
            $total[0]['VALUE']['options'] = array("align"=>"R");

			$i = 1;
			if(format_number_dotbpdf($this->bean->deal_tot, $locale->getPrecision(), $locale->getPrecision())!= 0.00){

	            $total[1]['BLANK'] = ' ';
			    $total[1]['TITLE0'] = '';
			    $total[1]['VALUE0'] ='';
			    $total[1]['TITLE'] = $mod_strings['LBL_PDF_DISCOUNT'];
			    $total[1]['VALUE']['value'] = format_number_dotbpdf($this->bean->deal_tot, $locale->getPrecision(), $locale->getPrecision(), $format_number_array);
	            $total[1]['VALUE']['options'] = array("align"=>"R");
			    ...
			    $total[2]['VALUE0'] ='';
			    $total[2]['TITLE'] = $mod_strings['LBL_PDF_NEW_SUB'];
			    $total[2]['VALUE']['value'] = format_number_dotbpdf($this->bean->new_sub, $locale->getPrecision(), $locale->getPrecision(), $format_number_array);
	            $total[2]['VALUE']['options'] = array("align"=>"R");
	            $i = 3;
			}
            $total[$i]['BLANK'] = ' ';
            $total[$i]['TITLE0'] = $mod_strings['LBL_PDF_TAX_RATE'];
            $total[$i]['VALUE0'] = format_number_dotbpdf($this->bean->taxrate_value, $locale->getPrecision(), $locale->getPrecision(), array('percentage' => true));
            $total[$i]['TITLE'] = $mod_strings['LBL_PDF_TAX'];
            $total[$i]['VALUE']['value'] =  format_number_dotbpdf($this->bean->tax, $locale->getPrecision(), $locale->getPrecision(), $format_number_array);
            $total[$i]['VALUE']['options'] = array("align"=>"R");
            $i++;
            $total[$i]['BLANK'] = ' ';
            ...
            $total[$i]['TITLE'] = $mod_strings['LBL_PDF_SHIPPING'];
            $total[$i]['VALUE']['value'] =  format_number_dotbpdf($this->bean->shipping, $locale->getPrecision(), $locale->getPrecision(), $format_number_array);
            $total[$i]['VALUE']['options'] = array("align"=>"R");
            $i++;
            $total[$i]['BLANK'] = ' ';
            $total[$i]['TITLE0'] = '';
            $total[$i]['VALUE0'] ='';
            $total[$i]['TITLE'] = $mod_strings['LBL_PDF_TOTAL'];
            $total[$i]['VALUE']['value'] =  format_number_dotbpdf($this->bean->total, $locale->getPrecision(), $locale->getPrecision(), $format_number_array);
            $total[$i]['VALUE']['options'] = array("align"=>"R");
