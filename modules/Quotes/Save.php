...
// This will ensure that DotbLogic doesn't do weird conversion.
DotbCurrency::verifyCurrencyBaseRateSet($focus, !$focus->new_with_id);
                ...
                foreach ($product->column_fields as $field) {
                    ...
                    if (isset($_POST[$j]) && is_array($_POST[$j]) && isset($_POST[$j][$i])) {
                        ...
                        if (isset($product->field_defs[$field]['type'])) {
                            ...
                            $dotbField = DotbFieldHandler::getDotbField($type);
                            $dotbField->save($product, array($field => $value), $field, $product->field_defs[$field]);
                        } else {
                            $product->$field = $value;
                        }
                    }
...
// we need to resave all the product bundles, so dotblogic works.
