...
class Document extends DotbBean {
	...
	function save($check_notify = false) {
        ...
        if (empty($this->id) || $this->new_with_id)
		{
            ...
            if (!empty($_FILES['filename_file'])) {
                ...
                $createRevision = true;
            } else if ( $isDuplicate && ( empty($this->doc_type) || $this->doc_type == 'Dotb' ) ) {
            ...
            if ( !empty($this->doc_id) && $this->doc_type != 'Dotb' ) {
                $createRevision = true;
            }
	...
	function fill_in_additional_detail_fields() {
        ...
        if (!empty($this->document_revision_id)) {
            ...
            if (!empty ($row['file_ext'])) {
                $img_name = DotbThemeRegistry::current()->getImageURL(strtolower($row['file_ext'])."_image_inline.gif");
		...
		if($this->ACLAccess('DetailView')) {
			if(!empty($this->doc_type) && $this->doc_type != 'Dotb' && !empty($this->doc_url)) {
                $file_url= "<a href='".$this->doc_url."' target='_blank'>".DotbThemeRegistry::current()->getImage($this->doc_type.'_image_inline', 'border="0"',null,null,'.png',$mod_strings['LBL_LIST_VIEW_DOCUMENT'])."</a>";
			} else {
			    $file_url = "<a href='index.php?entryPoint=download&id={$this->document_revision_id}&type=Documents' target='_blank'>".DotbThemeRegistry::current()->getImage($img_name, 'border="0"', null,null,'.gif',$mod_strings['LBL_LIST_VIEW_DOCUMENT'])."</a>";
			}
     ...
     *
     * Override method from DotbBean to handle deleting relationships associated with a Document.  This method will
