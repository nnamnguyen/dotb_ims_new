...
class DocumentsViewExtdoc extends DotbView
{
 	...
 	public function display(){
        ...
        if (isTruthy($isPopup) && !empty($elemBaseName) ) {
         ...
         if (!$validSession || empty($eapmBean))
         {
             ...
             $tpl_file = DotbAutoLoader::existingCustomOne('include/externalAPI/'.$apiName.'/'.$apiName.'Signup.'.$GLOBALS['current_language'].'.tpl');
        ...
        // In order to emulate the list views for the DotbFields, I need to uppercase all of the key names.
        $searchData = array();
        ...
        if ( is_array($searchDataLower) ) {
            foreach ( $searchDataLower as $row ) {
                ...
                if ( $isPopup ) {
                    ...
                    $newRow['DOC_URL'] = "javascript:window.opener.DOTB.field.file.populateFromPopup('"
        ...
        $ss->assign('imgPath',DotbThemeRegistry::current()->getImageURL($apiName.'_image_inline.png'));
