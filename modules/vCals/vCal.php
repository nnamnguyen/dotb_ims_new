...
class vCal extends DotbBean {
	...
	function get_freebusy_lines_cache(&$user_bean)
	{
        ...
        if (empty($dotb_config['freebusy_use_vcal_cache'])) {
	...
	// query and create the FREEBUSY lines for DotBCRM Meetings and Calls and
        // return the string
	function create_dotb_freebusy($user_bean, $start_date_time, $end_date_time)
	{
        $ical_array = array();
		...
		foreach($acts_arr as $act)
		{
            if(empty($act->start_time) || empty($act->end_time)){
                ...
                $errMSG = "ERROR in Vcal::create_dotb_freebusy.  Calendar Activity was not created because of missing start or end time";
                if(!empty($act->dotb_bean->id)){
                    $errMSG .= ", id is ".$act->dotb_bean->id;
                }
                if(!empty($act->dotb_bean->name)){
                    $errMSG .= ", name is: ".$act->dotb_bean->name;
                }
     ...
     * @param $user_focus
     * @param bool  $cached
     * @param DotbDateTime $startDate  optional: if Not cached
     * @param DotbDateTime $endDate    optional: if Not cached
     * @return string freebusy vcal string
     */
    // return a freebusy vcal string
    function get_vcal_freebusy($user_focus, $cached = true, DotbDateTime $startDate = null, DotbDateTime $endDate = null)
    {
        global $locale, $timedate;
        $ical_array = array();
        $ical_array[] = array("BEGIN", "VCALENDAR");
        $ical_array[] = array("VERSION", "2.0");
        $ical_array[] = array("PRODID", "-//DotBCRM//DotBCRM Calendar//EN");
        ...
        if (!$cached && !empty($startDate) && !empty($endDate)) {
            ...
            global $dotb_config;
            if (isset($dotb_config['vcal_time']) && $dotb_config['vcal_time'] != 0 && $dotb_config['vcal_time'] < 13)
            {
                $timeOffset = $dotb_config['vcal_time'];
            }
        ...
        if ($realTimeSearch || (!$cached && $timeOffset != 0)) {
            ...
            $str .= $this->create_dotb_freebusy($user_focus, $start_date_time, $end_date_time);
        } else {
	...
	// static function:
        // cache vcals
        public static function cache_dotb_vcal(&$user_focus)
        {
            self::cache_dotb_vcal_freebusy($user_focus);
        }

	// static function:
    // caches vcal for Activities in Dotb database
    public static function cache_dotb_vcal_freebusy(&$user_focus)
    {
        global $dotb_config;
        ...
        if (static::$backtrace_log_enabled == 'cache' || static::$backtrace_log_enabled == 'all') {
            ...
            $GLOBALS['log']->fatal("VCAL:CACHE - cache_dotb_vcal_freebusy()\n" . $trace);
        }

        // set freebusy members and save
        $arr = array('user_id' => $user_focus->id, 'type' => 'vfb', 'source' => 'dotb');
     ...
     */
    public static function get_ical_event(DotbBean $bean, User $user)
    {
        global $timedate;
        $ical_array = array();
