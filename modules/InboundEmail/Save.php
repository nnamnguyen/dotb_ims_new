...
if( isset($_REQUEST['is_auto_import']) && $_REQUEST['is_auto_import'] == 'on' )
{
    ...
    {
        $groupFolderId = $focus->createAutoImportDotbFolder();
...
$previousTeamAccessCheck = isset($GLOBALS['dotb_config']['disable_team_access_check']) ? $GLOBALS['dotb_config']['disable_team_access_check'] : null;
...
$GLOBALS['dotb_config']['disable_team_access_check'] = $previousTeamAccessCheck;
...
if( !empty($focus->groupfolder_id) )
{
    foreach ($monitor_fields as $singleField)
    {
        ...
        if($current_monitor_fields[$singleField] != $focus->$singleField)
            syncDotbFoldersWithBeanChanges($singleField, $focus);
    }
}
...
/**
 * Certain updates to the IE account need to be reflected in the related DotbFolder since they are
 ...
 * @param DotbBean $focus The InboundEmail bean being saved.
 */
function syncDotbFoldersWithBeanChanges($fieldName, $focus)
{
    $f = new DotbFolder();
