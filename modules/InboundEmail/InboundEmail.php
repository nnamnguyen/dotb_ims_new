...
use Dotbcrm\Dotbcrm\Util\Serialized;
...
class InboundEmail extends DotbBean {
	...
	public function __construct() {
	    $this->InboundEmailCachePath = dotb_cached('modules/InboundEmail');
	    $this->EmailCachePath = dotb_cached('modules/Emails');
	    parent::__construct();
		...
		$this->smarty = new Dotb_Smarty();
		$this->overview = new Overview();
		$this->imagePrefix = "{$GLOBALS['dotb_config']['site_url']}/cache/images/";
	}
	...
	/**
	 * Overrides DotbBean's mark_deleted() to drop the related cache table
	...
	{
		if(DotbAutoLoader::requireWithCustom("modules/InboundEmail/getMessageText.php")) {
			...
			foreach($insert as $overview) {
				...
				foreach($this->overview->fieldDefs as $colDef) {
					...
					switch($colDef['name']) {
                        ...
                        case 'subject':
                            $overview->subject = DotbCleaner::cleanHtml(htmlspecialchars_decode($overview->subject, ENT_QUOTES));
						...
						default:
                            $overview->{$colDef['name']} = DotbCleaner::cleanHtml(from_html($overview->{$colDef['name']}));
	...
	/**
	 * Constructs an IMAP c-client compatible folder path from Dotb proprietary
	 * @param string $mbox "::" delimited IMAP mailbox path, ie, INBOX.saved.stuff
	 * @return string
	 */
	function getImapMboxFromDotbProprietary($mbox) {
		$exMbox = explode("::", $mbox);
	 ...
	 * @param strings userName Name of account, for Dotb purposes
	...
	function savePersonalEmailAccount($userId = '', $userName = '', $forceSave=true) {
		...
		if (DotbConfig::getInstance()->get("disable_user_email_config", false)
	 ...
	 * to a standard string that DotBCRM expects
	 * @param	$arr	an array of email address objects
	 */
	function convertImapToDotbEmailAddress($arr) {
		if(is_array($arr)) {
			$addr = '';
	...
	*/
	...
	function saveAttachmentBinaries($attach, $msgNo, $thisBc, $part, $forDisplay) {
		...
		if((strtolower($part->disposition) == 'inline' && in_array($part->subtype, $this->imageTypes))
		    || ($part->type == 5)) {
		    if(copy($uploadDir.$fileName, dotb_cached("images/{$fileName}.").strtolower($part->subtype))) {
	 ...
	 * to be imported by Dotb without violating the true duplicate-email issues.
	 *
	...
	function importOneEmail($msgNo, $uid, $forDisplay=false, $clean_email=true) {
        ...
        // This leads to the remaining messages not being read because Dotb crashes everytime it tries to read the headers.
        ...
        // place  $dotb_config['mark_emails_seen'] = true;  into  config_override.php  to enable the workaround

        if (!empty($dotb_config['mark_emails_seen'])) {
		...
		if($forDisplay || $dupeCheckResult) {
			...
			if(!empty($header->to)) {
				$email->to_addrs	= $this->convertImapToDotbEmailAddress($header->to);
			}
			$email->from_name		= $this->handleMimeHeaderDecode($header->fromaddress);
			$email->from_addr_name = $email->from_name;
			$email->from_addr		= $this->convertImapToDotbEmailAddress($header->from);
			if(!empty($header->cc)) {
				$email->cc_addrs	= $this->convertImapToDotbEmailAddress($header->cc);
			}
			...
			$email->reply_to_email  = $this->convertImapToDotbEmailAddress($header->reply_to);
			if (!empty($email->reply_to_email)) {
...
eoq;
	...
	function retrieveAllByGroupId($id, $includePersonal=true) {
        ...
        // bug 50536: groupfolder_id cannot be updated to NULL from dotbbean's nullable check ('type' set to ID in the vardef)
	...
	/**
	 * returns the bean name - overrides DotbBean's
	 */
	 ...
	 * @param string $toFolder
	 * @param string $uids UIDs of emails to move, either Dotb GUIDS or IMAP
	 * UIDs
	 */
	...
	function moveEmails($fromIe, $fromFolder, $toIe, $toFolder, $uids, $copy=false) {
		...
		// same I-E server
		if($fromIe == $toIe) {
			$GLOBALS['log']->debug("********* DOTBFOLDER - moveEmails() moving email from I-E to I-E");
		...
		} elseif($toIe == 'folder' && $fromFolder == 'dotb::Emails') {
			$GLOBALS['log']->debug("********* DOTBFOLDER - moveEmails() moving email from DotbFolder to DotbFolder");
			// move from dotb folder to dotb folder
			$dotbFolder = new DotbFolder();
			$exUids = explode($app_strings['LBL_EMAIL_DELIMITER'], $uids);
			foreach($exUids as $id) {
				if($copy) {
					$dotbFolder->copyBean($fromIe, $toFolder, $id, "Emails");
				} else {
					$fromDotbFolder = new DotbFolder();
					$fromDotbFolder->retrieve($fromIe);
					$toDotbFolder = new DotbFolder();
					$toDotbFolder->retrieve($toFolder);
					...
					if ($fromDotbFolder->is_dynamic && $toDotbFolder->is_group) {
						$email->team_id = $toDotbFolder->team_id;
						$email->team_set_id = $toDotbFolder->team_set_id;
                        ...
                        foreach ($attachments as $note) {
                            $note->team_id = $toDotbFolder->team_id;
                            $note->team_set_id = $toDotbFolder->team_set_id;
                            $note->save();
                        }

						if (!$toDotbFolder->checkEmailExistForFolder($id)) {
							$fromDotbFolder->deleteEmailFromAllFolder($id);
							$toDotbFolder->addBean($email);
						}
					} elseif ($fromDotbFolder->is_group && $toDotbFolder->is_dynamic) {
						$fromDotbFolder->deleteEmailFromAllFolder($id);
						$email->assigned_user_id = $current_user->id;
						$email->save();
					} else {
						// If you are moving something from personal folder then delete an entry from all folder
						if (!$fromDotbFolder->is_dynamic && !$fromDotbFolder->is_group) {
							$fromDotbFolder->deleteEmailFromAllFolder($id);
						} // if

						if ($fromDotbFolder->is_dynamic && !$toDotbFolder->is_dynamic && !$toDotbFolder->is_group) {
							// moving email from dynamic folder.  Clean up old relationships and add new one
               				$fromDotbFolder->deleteEmailFromAllFolder($id);
							$toDotbFolder->addBean($email);
						} // if
						if (!$toDotbFolder->checkEmailExistForFolder($id)) {
							if (!$toDotbFolder->is_dynamic) {
								if ($email->team_id != $toDotbFolder->team_id) {
									$email->team_id = $toDotbFolder->team_id;
									$email->team_set_id = $toDotbFolder->team_set_id;
								} // if
								$fromDotbFolder->deleteEmailFromAllFolder($id);
							...
							$dotbFolder->move($fromIe, $toFolder, $id);
						} // else
						$email->save();
					} // else
				}
			...
			$GLOBALS['log']->debug("********* DOTBFOLDER - moveEmails() moving email from I-E to DotbFolder");
			// move to Dotb folder
			$dotbFolder = new DotbFolder();
			$dotbFolder->retrieve($toFolder);
			//Show the import form if we don't have the required info
			if (!isset($_REQUEST['delete'])) {
				$json = getJSONobj();
				if ($dotbFolder->is_group) {
			...
			if(!empty($dotbFolder->id)) {
					...
					if(!empty($msgNo)) {
						...
						if($importStatus) {
                            ...
                            if ($dotbFolder->is_group) {
                                ...
                                foreach ($attachments as $note) {
                                    $note->team_id = $dotbFolder->team_id;
                                    $note->team_set_id = $dotbFolder->team_set_id;
                                    $note->save();
                                }
                            }
						}
				...
				$GLOBALS['log']->error("********* DOTBFOLDER - failed to retrieve folder ID [ {$toFolder} ]");
			}
		} else {
			$GLOBALS['log']->debug("********* DOTBFOLDER - moveEmails() called with no passing criteria");
		}
...
eoq;
        ...
        if (empty($this->email->description_html)) {
            ...
            $description = DotbCleaner::cleanHtml(from_html($this->email->description_html, false), false);
		...
		// meta-metadata
		$meta['is_dotbEmail'] = ($exMbox[0] == 'dotb') ? true : false;

		if(!$meta['is_dotbEmail']) {
			if($this->isAutoImport) {
				$meta['is_dotbEmail'] = true;
			}
		} else {
	...
	function collapseLongMailingList($emails) {
		...
		if(count($ex) > 3) {
			...
			if(!empty($emailsHidden)) {
				$email2 = $emails;
				$emails = "<span onclick='javascript:DOTB.email2.detailView.showFullEmailList(this);' style='cursor:pointer;'>{$emails} [...{$j} {$app_strings['LBL_MORE']}]</span>";
				$emailsHidden = "<span onclick='javascript:DOTB.email2.detailView.showCroppedEmailList(this)' style='cursor:pointer; display:none;'>{$email2}, {$emailsHidden} [ {$app_strings['LBL_LESS']} ]</span>";
			}
...
//			_pp("count elements in sort[sort]: ".count($sorts[$sort]));
	...
	/**
    * Create a dotb folder for this inbound email account
    * if the Enable Auto Import option is selected
    *
    * @return String Id of the dotb folder created.
    */
	function createAutoImportDotbFolder()
	{
	    global $current_user;
	    $guid = create_guid();
	    $GLOBALS['log']->debug("Creating Dotb Folder for IE with id $guid");
	...
	/**
	 * parses Dotb's storage method for imap server service strings
	 * @return string
	 */
     ...
     *
     * Issue reported: https://dotbcrm.atlassian.net/browse/MAR-2297  (SI Bug number: 66022)
     *
    ...
    {
        if (!empty($GLOBALS['dotb_config']['mso_fixup_paragraph_tags'])
            && $GLOBALS['dotb_config']['mso_fixup_paragraph_tags'] === true
	...
	*/
    ...
    public function __construct()
    {
		...
		if(!isset($dictionary['email_cache']) || empty($dictionary['email_cache'])) {
		    include DotbAutoLoader::existingCustomOne('metadata/email_cacheMetaData.php');
		}
