...
class PMSEFieldParser implements PMSEDataParserInterface
{
    ...
    public function setCurrentValueIfCurrency($token)
    {
        ...
        $defCurrency = DotbCurrency::getCurrency($this->evaluatedBean);
        $amount = DotbCurrency::convertAmount((float)$token->expValue, $expCurrency, $defCurrency->id);
        return $amount;
    }
