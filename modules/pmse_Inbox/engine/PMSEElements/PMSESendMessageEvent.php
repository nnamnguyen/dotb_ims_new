...
class PMSESendMessageEvent extends PMSEIntermediateEvent
{
    ...
    public function sendEmail($flowData)
    {
        ...
        $bean = $this->caseFlowHandler->retrieveBean($flowData['cas_dotb_module'], $flowData['cas_dotb_object_id']);
        $addresses = $this->emailHandler->processEmailsFromJson($bean, $json, $flowData);
        $result = $this->emailHandler->sendTemplateEmail($flowData['cas_dotb_module'],
            $flowData['cas_dotb_object_id'], $addresses, $templateId);
