...
class PMSEUserTask extends PMSEEvent
{
    ...
    public function saveBeanData($beanData)
    {
        ...
        if ($bpmI === false) {
            $dotbApiExceptionEditConflict = new DotbApiExceptionEditConflict('Error: Process status complete.');
            PMSELogger::getInstance()->alert($dotbApiExceptionEditConflict->getMessage());
            throw $dotbApiExceptionEditConflict;
        }
        ...
        if (isModuleBWC($beanObject->module_dir) &&
            ...
            } catch (DotbApiExceptionRequestMethodFailure $conflict) {
