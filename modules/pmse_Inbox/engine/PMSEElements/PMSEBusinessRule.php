...
class PMSEBusinessRule extends PMSEScriptTask
{
    ...
    public function run($flowData, $bean = null, $externalAction = '', $arguments = array())
    {
        ...
        $dotbModule = $processDefinitionBean->pro_module;

        $dotbRecord = $flowData['cas_dotb_object_id'];
        ...
        if (is_array($row) && $rst_name != '' && !empty($dotbModule) && !empty($dotbRecord)) {
            ...
            $bean = $this->caseFlowHandler->retrieveBean($dotbModule, $dotbRecord);
            ...
            $global = array();
            if (isset($bean->name)) {
                $logBR .= "Bean: $dotbModule::$dotbRecord {$bean->name}\n";
            } else {
                $logBR .= "Bean: $dotbModule::$dotbRecord\n";
            }
            ...
            $res = $rules->parseRuleSetJSON($dotbModule, $rstSourceDefinition, $row['rst_type']);
            $logBR .= $res['log'];
            ...
            $historyData = $this->retrieveHistoryData($dotbModule);
