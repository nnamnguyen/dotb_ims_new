...
class PMSEExecuter
{
    ...
    public function runEngine(
        ...
        if (is_null($bean)) {
            $bean = BeanFactory::retrieveBean($flowData['cas_dotb_module'], $flowData['cas_dotb_object_id']);
        }
    ...
    public function validateNestedLoopCount($flowData)
    {
        ...
        if (!empty($flowData['cas_dotb_object_id'])) {
            $dotb_id = $flowData['cas_dotb_object_id'];
            ...
            if (empty($this->executedElements[$dotb_id][$element_id])) {
                $this->executedElements[$dotb_id][$element_id] = 1;
            } else {
                $this->executedElements[$dotb_id][$element_id]++;
            }

            // if count exceed the cycle number throw a exception.
            $count = $this->executedElements[$dotb_id][$element_id];
