...
class PMSEUserAssignmentHandler
{
    ...
    public function taskAssignment($flowData)
    {
        ...
        $currentDotbId = $flowData['cas_dotb_object_id'];
        $currentDotbModule = $flowData['cas_dotb_module'];
        ...
        if ($activityType == 'SCRIPTTASK') {
            ...
            $cas_dotb_action = $bpmnElement['act_type'];
            ...
            if ($assign_method == 'static') {
                switch ($assignUser) {
                    case 'owner':
                        $currentUserId = $this->getRecordOwnerId($currentDotbId, $currentDotbModule);
                        break;
                    case 'supervisor':
    ...
    public function reassignRecordToUser($caseData, $userId)
    {
        ...
        $beanObject = $this->retrieveBean($caseBean->cas_dotb_module, $caseBean->cas_dotb_object_id);
     ...
     * @param type $currentDotbId
     * @param type $currentDotbModule
     * @return string
     */
    public function getRecordOwnerId($currentDotbId, $currentDotbModule)
    {
        $bean = $this->retrieveBean($currentDotbModule, $currentDotbId);
