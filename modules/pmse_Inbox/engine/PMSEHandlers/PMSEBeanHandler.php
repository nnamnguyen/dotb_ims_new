...
class PMSEBeanHandler
{
    ...
    {
        global $beanList, $db;
        $moduleName = $flowBean['cas_dotb_module'];
        //$object_id = $flowBean->cas_dotb_object_id;
...
//            require_once ($RelatedModuleDir);
            ...
            if (!isset($beanRelated->id)) {
                $this->logger->info("[][] There is not a data relationship beetween $act_field_module and {$flowBean['cas_dotb_module']}");
                $bean = null;
            } else {
    ...
    public function mergingTemplate ($bean, $template, $component_array, $evaluate)
    {
        ...
        foreach ($component_array as $module_name => $module_array) {
            foreach ($module_array as $field => $field_array) {
                ...
                if ($newBean instanceof DotbBean) {
     ...
     * @param $bean this value is send in cse the date a native Dotb Date
    ...
    private function get_href_link($bean)
    {
        ...
        global $dotb_config;
        $link = "{$dotb_config['site_url']}/index.php?module={$bean->module_dir}&action=DetailView&record={$bean->id}";
