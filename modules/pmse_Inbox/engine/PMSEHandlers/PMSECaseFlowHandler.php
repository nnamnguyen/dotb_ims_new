...
class PMSECaseFlowHandler
{
     ...
     * @codeCoverageIgnore
     */
    public function retrieveDotbQueryObject()
    {
    ...
    public function retrieveFlowData($flowData)
    {
        ...
        $this->getBpmFlow();
        $dotbQueryObject = $this->retrieveDotbQueryObject();
        $dotbQueryObject->select(array(
            'id',
            'cas_id',
            'cas_index',
            'bpmn_type',
        ...
        $dotbQueryObject->from($this->bpmFlow);
        $dotbQueryObject->where()->queryAnd()
            ->addRaw('cas_index=' . $flowData['cas_index'] . ' AND cas_id=' . $flowData['cas_id']);

        $currentFlow = $dotbQueryObject->execute();
    ...
    public function retrieveFollowingElements($flowData)
    {
        ...
        $dotbQueryObject->select($fields);
        $dotbQueryObject->from($bpmnFlowBean);
        $dotbQueryObject->where()
            ->queryAnd()
            ->addRaw($where);

        $flows = $dotbQueryObject->execute();
    ...
    public function setLockedFieldsOnBean(array $flowData)
    {
        ...
        if (isset($flowData['cas_flow_status']) && $flowData['cas_flow_status'] === 'NEW') {
            ...
            $target = BeanFactory::getBean($flowData['cas_dotb_module'], $flowData['cas_dotb_object_id']);
     ...
     * @param string $proId
     */
    public function deleteLockFieldsFromBean(DotbBean $bean, $proId)
    {
        // Get the process definition bean
     ...
     */
    public function addLockedFields(DotbBean $bean, pmse_BpmProcessDefinition $pd)
    {
     ...
     * @param pmse_BpmProcessDefinition $pd
     */
    public function removeLockedFields(DotbBean $bean, pmse_BpmProcessDefinition $pd)
    {
    ...
    public function handleTerminatedFlowRelatedBeans($casId)
    {
        ...
        $q = $this->retrieveDotbQueryObject();

        $fields = [
            'i.cas_status',
            'i.cas_module',
            'i.pro_id',
            'pf.cas_dotb_object_id',
        ];
        ...
        if (!empty($row) && $row['cas_status'] != 'IN PROGRESS') {
            $bean = $this->retrieveBean($row['cas_module'], $row['cas_dotb_object_id']);
            if (isset($bean)) {
    ...
    public function processFlowData($flowData)
    {
        ...
        $preparedFlow['cas_dotb_module'] = $flowData['cas_dotb_module'];
        $preparedFlow['cas_dotb_object_id'] = $flowData['cas_dotb_object_id'];
        $preparedFlow['cas_dotb_action'] = 'None';
    ...
    public function createThread($flowData)
    {
        ...
        $dotbQueryObject->select(array('cas_thread_index'));
        $dotbQueryObject->from($thread);
        $dotbQueryObject->where()
            ->queryAnd()
            ->addRaw('cas_id=' . $flowData['cas_id']);

        $rows = $dotbQueryObject->execute();
     ...
     * case status, and also needs to be transformed to a DotbQuery object
     * @param type $cas_id
     * @param type $status
     * @global type $db
     */
    ...
    public function retrieveFlowIterations($flowData)
    {
        ...
        $this->dotbQueryObject = $this->retrieveDotbQueryObject();
        $this->dotbQueryObject->select(array('id'));
        $this->dotbQueryObject->from($flowBean);
        $this->dotbQueryObject->where()
            ->queryAnd()
