...
class PMSEPreProcessor
{
     ...
     * @codeCoverageIgnore
     */
    public function retrieveDotbQuery()
    {
    ...
    {
        $dotbQuery = $this->retrieveDotbQuery();
        $flowBean = $this->retrieveBean('pmse_BpmFlow');
        $fields = array('cas_id', 'cas_index', 'pro_id');
        $dotbQuery->select($fields);
        $dotbQuery->from($flowBean, array('alias' => 'flow'));
        $dotbQuery->joinTable('pmse_bpm_process_definition', array(
            'alias' => 'definition',
        ))->on()->equalsField('definition.id', 'flow.pro_id');
        $dotbQuery->where()->queryAnd()
            ->addRaw("cas_dotb_object_id = '{$bean->id}' AND cas_dotb_module = '{$bean->module_name}' AND cas_flow_status <> 'CLOSED'");
        $dotbQuery->select->fieldRaw('definition.pro_terminate_variables');
     ...
     * @return boolean
     */
    public function terminateCaseByBeanAndProcess(DotbBean $bean, array $data = array())
    {
     ...
     * @param DotbBean $bean The primary bean
     * @param array $links Array of link names to load for the bean
     * @param boolean $quoted Whether to DB escape the results
     * @return array
     */
    protected function buildLinkedObjectIdList(DotbBean $bean, array $links = array(), $quoted = true)
    {
     ...
     * @return boolean
     */
    private function isNewBean(DotbBean $bean)
    {
     ...
     * @return array
     */
    public function getAllEvents(DotbBean $bean)
    {
        ...
        SELECT
            ...
            flow.cas_user_id, flow.cas_thread, flow.cas_dotb_module,
            flow.cas_dotb_object_id, flow.cas_flow_status
        FROM
            pmse_bpm_related_dependency rd
        LEFT JOIN
            pmse_bpm_flow flow
            ON
                ...
                flow.cas_dotb_object_id IN (?) AND
                flow.deleted = 0 
        WHERE
            rd.deleted = 0 AND rd.pro_status != 'INACTIVE' AND (
                (
    ...
    public function getFlowsByCasId($casId)
    {
        ...
        $select = [
            ...
            'cas_user_id', 'cas_thread', 'cas_flow_status', 'cas_dotb_module',
            'cas_dotb_object_id', 'cas_dotb_action', 'cas_adhoc_type',
    ...
    public function getFlowByCasIdCasIndex($arguments)
    {
        ...
        $q = new DotbQuery();
        $q->select(array('cas_dotb_module', 'cas_dotb_object_id', 'id'));
        $q->from($tmpBean);
     ...
     * @param DotbBean $bean The DotbBean object... could be null
     * @param array $flowData
    ...
    {
        if (is_null($bean) && isset($flowData['cas_dotb_module'], $flowData['cas_dotb_object_id'])) {
            $bean = BeanFactory::getBean($flowData['cas_dotb_module'], $flowData['cas_dotb_object_id']);
        }
