...
class PMSEExpressionValidator extends PMSEBaseValidator implements PMSEValidate
{
    ...
    public function validateParamsRelated($bean, $flowData, $request)
    {
        ...
        if ($request->getExternalAction() == 'EVALUATE_MAIN_MODULE') {
            if (
                $bean->module_name != $flowData['cas_dotb_module']
                || $bean->id != $flowData['cas_dotb_object_id']
            ) {
                $request->invalidate();
            }
        }
    ...
    public function hasValidRelationship($bean, $flowData)
    {
        ...
        $cacheKey = sprintf(
            '%s:%s:%s',
            $flowData['cas_dotb_module'],
            $flowData['cas_dotb_object_id'],
        ...
        $seedBean = BeanFactory::newBean($flowData['cas_dotb_module']);
        ...
        $seedBean->id = $flowData['cas_dotb_object_id'];
