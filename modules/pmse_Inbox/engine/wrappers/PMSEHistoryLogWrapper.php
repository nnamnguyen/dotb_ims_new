...
class PMSEHistoryLogWrapper
{
    ...
    /**
     * Get Module name by id. make the query and return the name of an dotb module
    ...
    private function getActivityModule($caseData)
    {
        ...
        if (empty($activityDefinitionBean) ||
            ...
            $activityDefinitionBean->act_field_module == $caseData['cas_dotb_module']) {
            return PMSEEngineUtils::getModuleLabelFromModuleName($beanList[$caseData['cas_dotb_module']]);
        }

        $relatedModule = $activityDefinitionBean->act_field_module;

        $dotbModuleBean = BeanFactory::getBean($caseData['cas_dotb_module'], $caseData['cas_dotb_object_id']);

        if ($dotbModuleBean == null) {
            return PMSEEngineUtils::getModuleLabelFromModuleName($relatedModule);
        }

        if (!$dotbModuleBean->load_relationship($relatedModule)) {
            return PMSEEngineUtils::getModuleLabelFromModuleName($relatedModule);
        }

        return PMSEEngineUtils::getModuleLabelFromModuleName($dotbModuleBean->$relatedModule->getRelatedModuleName());
    }
    ...
    private function fetchUserType($caseData)
    {
        ...
        if ($caseData['cas_dotb_action'] == 'SCRIPTTASK') {
            $this->setProcessUser($entry);
        } else {
            if ($caseData['bpmn_type'] == 'bpmnActivity' ||
                $caseData['cas_dotb_action'] == 'DetailView' ||
                $caseData['cas_dotb_action'] == 'EditView' ||
                ...
                if (isset($this->currentUser->picture)) {
                    if ($this->currentUser->picture == '' || $this->currentUser->picture == null) {
                        ...
                        $entry['image'] = 'index.php?entryPoint=download&id=' . $this->currentUser->picture . '&type=DotbFieldImage&isTempFile=1';
                    }
                } else {
                    $entry['image'] = 'include/images/default-profile.png';
                }
