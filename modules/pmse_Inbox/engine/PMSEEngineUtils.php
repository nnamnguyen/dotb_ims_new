...
use Dotbcrm\Dotbcrm\Security\Subject\AdvancedWorkflow;
...
class PMSEEngineUtils
{
    ...
    {
        try {
            $dotbUploader = new UploadFile($fileName);
            if ($getFileContents) {
                return $dotbUploader->get_file_contents();
            } else {
                $dotbUploader->confirm_upload();
                $dotbUploader->final_move($fileName . '_' . create_guid());
    ...
    public static function isValidStudioField($def)
    {
        ...
        if (is_null(static::$idmConfig)) {
            static::$idmConfig = new Authentication\Config(\DotbConfig::getInstance());
        }
    ...
    public static function getDateToFE($theDate, $type, $bean = null)
    {
        ...
        if ( $date == null && $bean instanceof DotbBean) {
     ...
     * @return bool|String
     */
    public static function saveAssociatedBean(DotbBean $bean)
    {
        ...
        if ((empty($GLOBALS['dotb_config']['exclude_notifications'][$bean->module_dir]) ||
                $GLOBALS['dotb_config']['exclude_notifications'][$bean->module_dir] != true) &&
            (empty($_REQUEST['__dotb_url']) ||
                substr($_REQUEST['__dotb_url'], -10) != 'MassUpdate') &&
    ...
    public static function fixCurrencyType($currencyObj)
    {
        ...
        if (isset($dotb_config['default_currency_symbol']) &&
            isset($dotb_config['default_currency_iso4217'])
        ) {
            $defaultCurrencyLabel = $dotb_config['default_currency_symbol'] .
                " (" . $dotb_config['default_currency_iso4217'] . ")";
        }
     ...
     * @param DotbBean $bean The bean to use the ID from
    ...
    {
        global $dotb_config, $current_user;

        // Return a disabled = true straight away if that is the case
        if (!empty($dotb_config['disable_export'])) {
            return true;
        }
        ...
        return !empty($dotb_config['admin_export_only']) && $nonAdmin;
    }
     ...
     * @return bool
     */
    public static function hasActiveProcesses(DotbBean $bean)
    {
        $whitelist = DotbCache::instance()->{static::MODULES_WHITELIST_CACHE_KEY};
        if (!is_array($whitelist)) {
            ...
            DotbCache::instance()->{static::MODULES_WHITELIST_CACHE_KEY} = $whitelist;
        }
    ...
    {
        unset(DotbCache::instance()->{static::MODULES_WHITELIST_CACHE_KEY});
    }
    ...
    {
        DotbCache::instance()->{static::getModuleLockedFieldsCacheKey($moduleName)} = true;
    }
    ...
    {
        if (!isset(DotbCache::instance()->{static::getModuleLockedFieldsCacheKey($moduleName)})) {
            ...
            DotbCache::instance()->{static::getModuleLockedFieldsCacheKey($moduleName)} = (bool) $result;
        }

        return DotbCache::instance()->{static::getModuleLockedFieldsCacheKey($moduleName)};
    }
    ...
    {
        unset(DotbCache::instance()->{static::getModuleLockedFieldsCacheKey($moduleName)});
    }
