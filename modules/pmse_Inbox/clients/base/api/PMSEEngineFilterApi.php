...
//                'shortHelp' => 'List of all records in this module',
    ...
    public function filterList(ServiceBase $api, array $args, $acl = 'list')
    {
        ...
        foreach ($result['records'] as $key => $value) {
            ...
            $assignedBean = BeanFactory::getBean($value['cas_dotb_module'], $value['cas_dotb_object_id'], $params);
    ...
    public function filterListSetup(ServiceBase $api, array $args, $acl = 'list')
    {
        ...
        // Create DotbQuery object including PA tables
     ...
     * @return bool
     */
    public static function isSupportedPAFilter($filter, DotbQuery $q) {
     ...
     */
    public static function processFilterPA($filter, $expression, DotbQuery_Builder_Where $where)
    {
        switch ($filter) {
            case 'visibility':
     ...
     * @throws DotbApiExceptionInvalidParameter
     */
    public function addFiltersPA($filters, DotbQuery_Builder_Where $where, DotbQuery $q)
    {
        foreach ($filters as $filterDef) {
            foreach ($filterDef as $filter => $expression) {
                if (!self::isSupportedPAFilter($filter, $q)) {
                    $dotbApiExceptionInvalidParameter = new DotbApiExceptionInvalidParameter(
                        'ERROR_PA_FILTER_UNSUPPORTED_FILTER'
                    );
                    PMSELogger::getInstance()->alert($dotbApiExceptionInvalidParameter->getMessage());
                    throw $dotbApiExceptionInvalidParameter;
                }
     ...
     * @param DotbQuery_Builder_Where $where
     */
    public static function addVisibilityFilter($access, DotbQuery_Builder_Where $where)
    {
        if (!self::$isVisibilityApplied) {
            ...
            $supportedModules = DotbACL::filterModuleList($supportedModules, 'access', true);

            if (!empty($supportedModules)) {
                $where->queryAnd()->in('cas_dotb_module', $supportedModules);
            }
     ...
     * @param DotbQuery_Builder_Where $where
     */
    public static function addInTimeFilter($expression, DotbQuery_Builder_Where $where) {
     ...
     * @param DotbQuery_Builder_Where $where
     */
    public static function addAssignmentMethodFilter($expression, DotbQuery_Builder_Where $where)
    {
        global $current_user;
     ...
     *
     * Notice: PA Processes module have not associated one only record as other modules in DotBCRM, current FilterApi is designed
     ...
     * @param DotbQuery_Builder_Where $where
     */
    public static function addFieldFilterPA($field, $expression, DotbQuery_Builder_Where $where)
    {
     ...
     * @param DotbQuery_Builder_Where $where
     */
    public static function addInvalidFilter(DotbQuery_Builder_Where $where)
    {
        $where->queryAnd()->isNull('id');
    }

    /**
     * Remove visibility filter from DotbQuery object
     * @param DotbQuery_Builder_Where $where
     */
    public static function removeVisibilityFilter(DotbQuery_Builder_Where $where)
    {
    ...
    /**
     * Creates a DotbQuery instance according PA relationships
     * @param DotbBean $seed
     * @param array $options
     * @return DotbQuery
     * @throws DotbQueryException
     */
    protected static function getQueryObjectPA(DotbBean $seed, array $options)
    {
        ...
        $fields[] = array("cas_dotb_module", 'cas_dotb_module');

        $fields[] = array("cas_dotb_object_id", 'cas_dotb_object_id');
        ...
        if (isset($options['order_by'])) {
            foreach ($options['order_by'] as $orderBy) {
                ...
                // Set the values into DotbQuery now
                $q->orderBy($exp, $dir);
            }
        }
    ...
    protected function formatBeans(ServiceBase $api, array $args, $beans, array $options = array())
    {
        ...
        foreach ($beans as $bean) {
            ...
            $assignedBean = BeanFactory::getBean($bean->fetched_row['cas_dotb_module'], $bean->fetched_row['cas_dotb_object_id']);
            ...
            $arr_aux['cas_dotb_module'] = $bean->fetched_row['cas_dotb_module'];
            $arr_aux['cas_dotb_object_id'] = $bean->fetched_row['cas_dotb_object_id'];
