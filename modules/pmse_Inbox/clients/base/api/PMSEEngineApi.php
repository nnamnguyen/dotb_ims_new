...
class PMSEEngineApi extends DotbApi
{
    /**
    ...
    public function updateChangeCaseFlow(ServiceBase $api, array $args)
    {
        ...
        foreach ($args['data'] as $value) {
            if (is_array($value)) {
                ...
                $flow->cas_dotb_module = $flows['rowList'][0]['cas_dotb_module'];
                $flow->cas_dotb_object_id = $flows['rowList'][0]['cas_dotb_object_id'];
                $flow->cas_dotb_action = $flows['rowList'][0]['cas_dotb_action'];
    ...
    public function cancelCase(ServiceBase $api, array $args)
    {
        ...
        try {
            foreach ($args['cas_id'] as $id) {
                ...
                $bean = BeanFactory::retrieveBean($flowBean->cas_dotb_module, $flowBean->cas_dotb_object_id);
                if (is_null($bean)) {
                    throw new Exception($flowBean->cas_dotb_object_id . ' in "' . $flowBean->cas_dotb_module . '" is not a valid record.');
                }
                $auxCasId['cas_id'] = $id;
    ...
    public function getReassignFlows(ServiceBase $api, array $args)
    {
        ...
        foreach ($rows as $key => $row) {
            ...
            if (!empty($args['unattended'])) {
                if (ACLAction::userHasAccess($row["cas_user_id"],$row['cas_dotb_module'], 'access')) {
        ...
        if (!empty($rows)) {
            foreach ($rows as $key => $row) {
                ...
                if ($arrayId !== false) {
                    ...
                    $assignedBean = BeanFactory::getBean($arrayCases[$arrayId]['cas_dotb_module'], $arrayCases[$arrayId]['cas_dotb_object_id'], $params);
                    ...
                    $row['cas_dotb_object_id'] = $arrayCases[$arrayId]['cas_dotb_object_id'];
                    $row['cas_dotb_module'] = $arrayCases[$arrayId]['cas_dotb_module'];
    ...
    private function getUnattendedCasesByFlow()
    {
        ...
        $q->distinct(true);
        $fields = array('cas_id','cas_dotb_module','cas_dotb_object_id','cas_user_id');
        ...
        $q->where()
                ->equals('cas_flow_status', 'FORM')
                ->in('cas_dotb_module', PMSEEngineUtils::getSupportedModules());
    ...
    public function getCaseRecord(ServiceBase $api, array $args)
    {
        ...
        if (!empty($route) && DotbAutoLoader::requireWithCustom($route['file']) && class_exists($route['className'])) {
    ...
    public function getSettingsEngine(ServiceBase $api, array $args)
    {
        ...
        if (isset($args['fields'])) {
            ...
            foreach ($fields as $field) {
                if (isset($dotb_config['pmse_settings_default'][$field])) {
                    $settings[$field] = $dotb_config['pmse_settings_default'][$field];
                }
            }
        } else {
            // else retrieve everything
            $settings = $dotb_config['pmse_settings_default'];
        }
