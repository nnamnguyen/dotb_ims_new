...
//                'shortHelp' => 'Returns the process definition status',
    ...
    public function selectCasesList(ServiceBase $api, array $args)
    {
        ...
        // Now put them into a format that DotbQuery likes
        ...
        $fields[] = array("pf.cas_dotb_module", 'cas_dotb_module');
        $fields[] = array("pf.cas_dotb_object_id", 'cas_dotb_object_id');
        ...
        if (!empty($list)) {
            foreach ($list as $key => $value) {
                ...
                $assignedBean = BeanFactory::getBean($list[$key]['cas_dotb_module'], $list[$key]['cas_dotb_object_id'], $params);
    ...
    protected function createProcessUsersChartData($filter)
    {
        ...
        // creating the dotb query object
        $q = new DotbQuery();
        ...
        foreach ($data_bean as $record) {
            if (isset($record['user_name'])) {
                ...
                // that returns the user name depending the Dotb's configuration
