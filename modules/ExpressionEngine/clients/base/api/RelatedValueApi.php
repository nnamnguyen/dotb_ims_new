 ...
 */
class RelatedValueApi extends DotbApi
{
    /**
                    ...
                    if ($focus->load_relationship($link)) {
                        ...
                        foreach ($relBeans as $bean) {
                            if (isset($bean->$rField) && is_numeric($bean->$rField) &&
                                ...
                                $value = $bean->$rField;
                                if ($isCurrency) {
                                    $value = DotbCurrency::convertWithRate($value, $bean->base_rate);
                                }
                        ...
                        if ($type == "rollupAve") {
                            $ret[$link][$type][$rField] = $count == 0 ? 0 : DotbMath::init($sum)->div($count)->result();
                            $ret[$link][$type][$rField . '_values'] = $values;
                        }
                        if ($type == "rollupMin") {
                        ...
                        foreach ($relBeans as $bean) {
                            if (!empty($bean->$rField) && is_numeric($bean->$rField) &&
                                ...
                                if (in_array($bean->{$rfDef['condition_field']}, $condition_values)) {
                                    ...
                                    $value = $bean->$rField;
                                    if ($isCurrency) {
                                        $value = DotbCurrency::convertWithRate($value, $bean->base_rate, $toRate);
                                    }
                                    $sum = DotbMath::init($sum)->add(
                                        $value
                                    )->result();
                                    $values[$bean->id] = $value;
                                }
     ...
     *
     * @param DotbBean $bean The Bean to which the Field Belongs
     * @param string $field The name of the field
     * @return bool
     */
    protected function isFieldCurrency(DotbBean $bean, $field)
    {
