...
DOTB.expressions.initFormulaBuilder = function() {
	var Dom = YAHOO.util.Dom,
		Connect = YAHOO.util.Connect,
		Msg = YAHOO.DOTB.MessageBox;
 ...
 */
DOTB.expressions.getFunctionList = function()
{
	var typeMap = DOTB.expressions.Expression.TYPE_MAP;
	var funcMap = DOTB.FunctionMap;
	var funcList = [];
     ...
     */
DOTB.expressions.getDisplayFunctionList = function() {
    var functionsArray = DOTB.expressions.getFunctionList();
    var usedClasses = { };
    var ret = [];
    for (var i in functionsArray)
    {
        var fName = functionsArray[i][0];
        //Internal Dotb functions that most users will not find useful
        switch (fName) {
        case "isValidTime":
        case "isAlpha":
        case "doBothExist":
        ...
        if (functionsArray[i][1] == "time")
            continue;
        if (usedClasses[DOTB.FunctionMap[fName].prototype.className])
            continue;
        ...
        usedClasses[DOTB.FunctionMap[fName].prototype.className] = true;
    }
 ...
 */
DOTB.expressions.setReturnTypes = function(t, vMap)
{
	...
	if (t.type == "variable")
	{
		if(typeof(vMap[t.name]) == "undefined")
			...
			t.returnType = DOTB.expressions.Expression.GENERIC_TYPE;
		else
	...
	if (t.type == "function")
	{
		for(var i in t.args)
		{
			DOTB.expressions.setReturnTypes(t.args[i], vMap);
		}
		var fMap = DOTB.FunctionMap;
     ...
     */
DOTB.expressions.validateReturnTypes = function(t)
{
	if (t.type == "function")
	{
		...
		for(var i in t.args)
		{
			DOTB.expressions.validateReturnTypes(t.args[i]);
		}
     ...
     * @param t expression function
     */
DOTB.expressions.validateRelateFunctions = function(t)
{
	var SU = DOTB.util, SE = DOTB.expressions;
    if (t.type == "function")
	{
		//Depth first recursion
		for(var i in t.args)
     ...
     * @param matchType
     */
DOTB.expressions.validateCurrExpression = function(silent, matchType)
{
	try {
		var varTypeMap = {};
		...
		var tokens = new DOTB.expressions.ExpressionParser().tokenize(expression);
		DOTB.expressions.setReturnTypes(tokens, varTypeMap);
		DOTB.expressions.validateReturnTypes(tokens);
		var def = DOTB.expressions.validateRelateFunctions(tokens),
		...
		if (matchType && matchType != returnType) {
			Msg.show({
                type: "alert",
                title: DOTB.language.get("ModuleBuilder", "LBL_FORMULA_INVALID"),
                msg: DOTB.language.get("ModuleBuilder", "LBL_FORMULA_TYPE") + matchType
            });
			return false;
		}
     ...
     */
DOTB.expressions.saveCurrentExpression = function(target, returnType)
{
    var expression = YAHOO.lang.trim(Dom.get('formulaInput').value);
	...
	if (!DOTB.expressions.validateCurrExpression(true, returnType))
		return false;
     ...
     */
DOTB.expressions.GridToolTip = {
	tipCache : { },
	currentHelpFunc : "",
	showFunctionDescription: function(tip, func) {
		var ggt = DOTB.expressions.GridToolTip;
		...
		if (typeof cache[func] == 'string') {
			...
			Connect.asyncRequest(
			    Connect.method, 
			    Connect.url + '&' + DOTB.util.paramsToUrl({
			    	"function": func, 
    ...
    var typeFormatter = function(el, rec, col, data)
	{
		...
		el.innerHTML = '<img src="themes/default/images/DotbLogic/icon_' + out + '_16.png"></img>';
	};
	...
	var fieldsGrid = new YAHOO.widget.ScrollingDataTable('fieldsGrid',
		...
		fieldDS,
	    {height: "200px", MSG_EMPTY: DOTB.language.get('ModuleBuilder','LBL_NO_FIELDS')}
	);
	...
	Dom.get("formulaFieldsSearch").onblur = function() {
		if (this.value == '')
		{
			this.value = DOTB.language.get("ModuleBuilder", "LBL_SEARCH_FIELDS");
			Dom.addClass(this, "empty");
		}
	}
	fieldsGrid.sortColumn(fieldsGrid.getColumn(0))
	fieldsGrid.render();
	DOTB.expressions.fieldGrid = fieldsGrid;

    //Setup the function list
    if (!DOTB.expressions.funcGridData) {
        DOTB.expressions.funcGridData = DOTB.expressions.getDisplayFunctionList();
    }

	var funcDS = new YAHOO.util.LocalDataSource(DOTB.expressions.funcGridData, 
	{
		responseType: YAHOO.util.LocalDataSource.TYPE_JSARRAY,
		responseSchema: 
		{
	...
	var fg = DOTB.expressions.functionsGrid = new YAHOO.widget.ScrollingDataTable('functionsGrid',
		[
		    {key:'name', label: "Functions", width: 200, sortable: true},
		    {key:'type', label: "&nbsp;", width: 20, sortable: true, formatter:typeFormatter}
		],
		funcDS,
	    {height: "200px", MSG_EMPTY: DOTB.language.get('ModuleBuilder','LBL_NO_FUNCS')}
	);
	...
	fg.on("sortedByChange", function(e){
		if(e.newValue)
		    DOTB.expressions.functionsGrid.sortedColumn = e.newValue;
	});

    //If the expression tooltip exists, destroy it to re-initialize
    if(DOTB.expressions.tooltip){
	    DOTB.expressions.tooltip.destroy();
    }
    var funcTip = DOTB.expressions.tooltip = new YAHOO.widget.Tooltip("functionsTooltip", {
	...
	funcTip.contextMouseOverEvent.subscribe(function(context, e){
		...
		this.timer = YAHOO.lang.later(250, this, function(funcName){
			DOTB.expressions.GridToolTip.showFunctionDescription(this, funcName);
		}, row.getData()['name']);
	...
	funcAC.doBeforeLoadData = function( sQuery , oResponse , oPayload ) {
		var fg = DOTB.expressions.functionsGrid;
		fg.initializeTable();
		fg.addRows(oResponse.results);
		fg.sortColumn(fg.sortedColumn.column, fg.sortedColumn.dir);
		fg.render();
    }
	if (!DOTB.expressions.funcionListJSON)
	{
		DOTB.expressions.funcionListJSON =  [];
		for(var i in DOTB.expressions.funcGridData)
		{
			DOTB.expressions.funcionListJSON[i] = {name: DOTB.expressions.funcGridData[i][0], type: DOTB.expressions.funcGridData[i][1]};
		}
	}
    ...
    Dom.get("formulaFuncSearch").onkeyup = function() {
		if (this.value == '') {
			...
			var fg = DOTB.expressions.functionsGrid;
			fg.initializeTable();
			fg.addRows(DOTB.expressions.funcionListJSON);
			fg.sortColumn(fg.sortedColumn.column, fg.sortedColumn.dir);
			fg.render();
		}
	}
	...
	Dom.get("formulaFuncSearch").onblur = function() {
		if (this.value == '')
		{
			this.value = DOTB.language.get("ModuleBuilder", "LBL_SEARCH_FUNCS");
			Dom.addClass(this, "empty");
		}
	}
	fg.render();
    ...
    DOTB.expressions.updateSelRFLink = function(link)
    {
        var win = DOTB.formulaRelFieldWin;
        win.params = {
            module:"ExpressionEngine",
    ...
    DOTB.expressions.updateRollupWizard = function(link, type)
    {
        var win = DOTB.rollupWindow;
        win.params = {
            module:"ExpressionEngine",
            action:"rollupWizard",
            tmodule:ModuleBuilder.module,
    ...
    DOTB.expressions.insertRollup = function(){
        ...
        DOTB.rollupWindow.hide();
    }

    DOTB.expressions.insertRelated = function(){
        $.markItUp({
            target : "#formulaInput",
            closeWith:"related($" + $("#selrf_rmodule").val() + ",\"" + $("#selrf_rfield").val() + "\")"
        });
        DOTB.formulaRelFieldWin.hide()
    }
    ...
    $("#formulaInput").markItUp({
    	...
    	markupSet:  [{
            name:DOTB.language.get("ModuleBuilder", "LBL_RELATED_FIELD"),
            className:'rel_field button',
            beforeInsert:function(){
                if (!DOTB.formulaRelFieldWin)
                    DOTB.formulaRelFieldWin = new YAHOO.DOTB.AsyncPanel('relatedFieldWindow', {
                        width: 400,
                        draggable: true,
                        close: true,
                ...
                DOTB.expressions.updateSelRFLink("");
                win.show();
                win.center();
            }
        },{
            name:DOTB.language.get("ModuleBuilder", "LBL_ROLLUP"),
            className:'rollup button',
            beforeInsert:function(){
                if (!DOTB.rollupWindow)
                    DOTB.rollupWindow = new YAHOO.DOTB.AsyncPanel('rollupWindow', {
                        width: 400,
                        draggable: true,
                        close: true,
                        constraintoviewport: true,
                ...
                win.setBody("loading...");
                win.render(document.body);
                DOTB.expressions.updateRollupWizard("", "");
                win.show();
                win.center();
            }
        }]
    ...
    var displayFunctions = DOTB.expressions.getDisplayFunctionList();
    /**
    ...
    var getFunctionsByType = function(type, search, limit)
    {
        ...
        var ret = [],
            fMap = DOTB.FunctionMap,
        	see = DOTB.expressions.Expression;
    ...
    var showACHelp = function(func)
    {
        ...
        if (DOTB.expressions.fb_ac_help_timer)
           window.clearTimeout(DOTB.expressions.fb_ac_help_timer);

       //Use a 300ms timer before showing/updating the autocomplete
       DOTB.expressions.fb_ac_help_timer = window.setTimeout(do_show, 300);
    };
    ...
    $("#formulaInput").keyup(function(e){
        ...
        {
            if (DOTB.expressions.fb_ac_timer)
                window.clearTimeout(DOTB.expressions.fb_ac_timer);

            //Use a 300ms timer before showing/updating the autocomplete
            DOTB.expressions.fb_ac_timer = window.setTimeout(function(){
    ...
    DOTB.expressions.closeFormulaBuilder = function()
    {
