...
/**
 * Stub class for the future of DotbEvents
 */
class Event{
...
class vBasic{
	...
	/**
	 * Takes in a Dotb Formatted date (Y-m-d H:i:s) and changes it to a VCal Formatted one (Ymd\THis\Z)
...
VEVENT;
	...
	/**
	 * converts a Dotb Event into a vEvent
	 * @param object $event
	 * @return
	 */
...
class vCalendar extends vBasic{
	...
	/**
	 * add a DotbEvent to the vCalendar Object
	 * @param DotbEvent $event
	 */
	...
	function generateVCal(){
		$cal = "BEGIN:VCALENDAR\n";
		if(!isset($this->properties['PRODID']))$this->properties['PRODID'] = '-//DOTBCRM Inc//DOTBCRM Calendar 6.5//EN';
...
class iCalendar {
	...
	/**
	 * Create Dotb meetings from calendar events
	 * @param DotbBean $parent related object to associate with meetings
	 */
	function createDotbEvents($parent = NULL)
	{
				...
				if (!$prev_exists || ($prev_exists && $val->event->sequence > $prev_seq)) {
					...
					if(!empty($emails)) {
					    ...
					    while($inv = $meeting->db->fetchByAssoc($invitees)) {
                            ...
                            if(empty($bean)) {
                                $GLOBALS['log']->info("createDotbEvents: Don't know how to create bean {$inv['bean_module']}");
                                continue;
                            }
                            // Hack - we don't care about this bean's permissions
                            ...
                            if(empty($meeting->$linkname)) {
                                $GLOBALS['log']->info("createDotbEvents: Unknown module $module with link $linkname encountered for id {$inv["bean_id"]}");
                                continue;
                            }
