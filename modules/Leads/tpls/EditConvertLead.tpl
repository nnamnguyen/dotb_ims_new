...
{/if}
<input type='button' name='saveLayout' value='{dotb_translate label="LBL_BTN_SAVE" module="ModuleBuilder"}'
    class='button' onclick='ModuleBuilder.saveConvertLeadLayout();' style="margin-bottom:5px;">
<img class="spacer" src="{dotb_getjspath file='include/images/blank.gif'}" style="width:50px;height:5px"/>
{html_options name="convertSelectNewModule" id="convertSelectNewModule" options=$availableModules}
<input type='button' name='addModule' value='{dotb_translate label="LBL_CONVERT_ADD_MODULE"}'
    class='button' onclick='ModuleBuilder.addConvertLeadLayout();' style="margin-bottom:5px;">

<div id='relGrid'></div>
{if $studio}{dotb_translate label='LBL_CUSTOM_RELATIONSHIPS' module='ModuleBuilder'}</h3>{/if}
<script>
{literal}

function getModuleNameFromLabel(label) {
    var moduleList = DOTB.language.get('app_list_strings', "moduleList");
...
var formatRemoveButton = function(el, rec, col, data) {
    ...
    out = {/literal}"<img alt='{$mod_strings.LBL_EDIT_INLINE}' name='delete_inline' src='{dotb_getimagepath file='delete_inline.gif'}' />";{literal}
    el.innerHTML = out;
...
YAHOO.DOTB.DragDropTable.groups = [];
var grid = ModuleBuilder.convertLayoutGrid = new YAHOO.DOTB.DragDropTable('relGrid',
    [
        {ldelim}key:'module',       label: '{dotb_translate label="LBL_CONVERT_MODULE_NAME"}', hidden: true {rdelim},
        {ldelim}key:'duplicateCheckOnStart', label: '{dotb_translate label="LBL_CONVERT_MODULE_NAME"}', hidden: true {rdelim},
        {ldelim}key:'moduleName',   label: '{dotb_translate label="LBL_CONVERT_MODULE_NAME"}', width: 200,sortable: false {rdelim},
        {ldelim}key:'required',     label: '{dotb_translate label="LBL_CONVERT_REQUIRED"}',    width: 80, sortable: false, formatter:formatCheckbox{rdelim},
        {ldelim}key:'copyData',     label: '{dotb_translate label="LBL_CONVERT_COPY"}',        width: 80, sortable: false, formatter:formatCheckbox{rdelim},
        {ldelim}key:'delete',       label: '{dotb_translate label="LBL_CONVERT_DELETE"}',      width: 60, sortable: false, formatter:formatRemoveButton{rdelim}
    ],{literal}
    ...
    {MSG_EMPTY: DOTB.language.get('ModuleBuilder','LBL_NO_RELS')}
);
