 ...
 function addDropdownElements(){
   ...
   for(i=0; i<=mod_array.length-1; i++){
     if(mod_array[i] != 'Contacts'){
       ...
       opt.text = DOTB.language.get('app_list_strings', "moduleListSingular")[mod_array[i]];
...
{if $bean->aclAccess("save")}
    <input title='{dotb_translate label="LBL_SAVE_BUTTON_LABEL"}' id='SAVE_HEADER'accessKey="{dotb_translate label='LBL_SAVE_BUTTON_KEY}" class="button primary"
        onclick="return check_form('{$form_name}');"
        type="submit" name="button" value="{dotb_translate label='LBL_SAVE_BUTTON_LABEL'}">
{/if}

{if !empty($smarty.request.return_action) && ($smarty.request.return_action == "DetailView" && !empty($record_id))}
    <input title="{dotb_translate label='LBL_CANCEL_BUTTON'}" id="CANCEL_HEADER" accessKey="{dotb_translate label='LBL_CANCEL_BUTTON_KEY'}" class="button"
        ...
        type="submit" name="button" value="{dotb_translate label='LBL_CANCEL_BUTTON_LABEL'}">
{elseif !empty($smarty.request.return_action) && ($smarty.request.return_action == "DetailView" && !empty($smarty.request.return_id))}';
    <input title="{dotb_translate label='LBL_CANCEL_BUTTON_TITLE'}" id="CANCEL_HEADER" accessKey="{dotb_translate label='LBL_CANCEL_BUTTON_KEY'}" class="button"
        onclick="this.form.action.value='DetailView'; this.form.module.value='{$smarty.request.return_module}'; this.form.record.value='{$smarty.request.return_id}';" 
        type="submit" name="button" value="{dotb_translate label='LBL_CANCEL_BUTTON_LABEL'}">
{else}
    <input title="{dotb_translate label='LBL_CANCEL_BUTTON_TITLE'}" id="CANCEL_HEADER" accessKey="{dotb_translate label='LBL_CANCEL_BUTTON_KEY'}" class="button"
