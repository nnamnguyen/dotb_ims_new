...
<tr><td class='mbLBL'>{dotb_translate module="DynamicFields" label="LBL_CALCULATED"}:</td>
    <td style="line-height:1em"><input type="checkbox" name="calculated" id="calculated" value="1" onclick ="ModuleBuilder.toggleCF()"
        ...
        {/if}
		{dotb_help text=$mod_strings.LBL_POPHELP_CALCULATED FIXX=250 FIXY=80}
...
<tr id='formulaRow' {if empty($vardef.formula)}style="display:none"{/if}>
	<td class='mbLBL'>{dotb_translate module="DynamicFields" label="LBL_FORMULA"}:</td>
    <td>
	    ...
	    <input type="button" class="button"  name="editFormula" style="margin-top: -2px"
		      value="{dotb_translate label="LBL_BTN_EDIT_FORMULA"}"
            onclick="ModuleBuilder.moduleLoadFormula(YAHOO.util.Dom.get('formula').value, 'formula', '{$calcFieldType}')"/>
    </td>
</tr>
{/if}
{if $vardef.type != 'address'}
<tr id='depCheckboxRow'><td class='mbLBL'>{dotb_translate module="DynamicFields" label="LBL_DEPENDENT"}:</td>
    <td><input type="checkbox" name="dependent" id="dependent" value="1" onclick ="ModuleBuilder.toggleDF(null, '#popup_form_id .toggleDep')"
        ...
        {dotb_help text=$mod_strings.LBL_POPHELP_DEPENDENT FIXX=250 FIXY=80}
    </td>
</tr>
<tr id='visFormulaRow' {if empty($vardef.dependency)}style="display:none"{/if}><td class='mbLBL'>{dotb_translate module="DynamicFields" label="LBL_VISIBLE_IF"}:</td> 
    <td>
