...
<tr>
	<td class='mbLBL'>{dotb_translate module="DynamicFields" label="LBL_DROP_DOWN_LIST"}:</td>
	<td>
	{if $hideLevel < 5 && empty($vardef.function) && (!isset($vardef.studio.field.options) || isTruthy($vardef.studio.field.options))}
		{html_options name="options" id="options" selected=$selected_dropdown values=$dropdowns output=$dropdowns onChange="ModuleBuilder.dropdownChanged(this.value);"}{if !$uneditable}<br><input type='button' value='{dotb_translate module="DynamicFields" label="LBL_BTN_EDIT"}' class='button' onclick="ModuleBuilder.moduleDropDown(this.form.options.value, this.form.options.value);">&nbsp;<input type='button' value='{dotb_translate module="DynamicFields" label="LBL_BTN_ADD"}' class='button' onclick="ModuleBuilder.moduleDropDown('', this.form.name.value);">{/if}
	{else}
...
<tr id='depTypeRow' class="toggleDep"><td class='mbLBL'>{dotb_translate module="DynamicFields" label="LBL_DEPENDENT"}:</td>
    <td>
        <select id="depTypeSelect" onchange="ModuleBuilder.toggleParent(this.value == 'parent'); ModuleBuilder.toggleDF(this.value == 'formula'); ">
            <option label="{dotb_translate module="ModuleBuilder" label="LBL_NONE"}" value="">{dotb_translate module="ModuleBuilder" label="LBL_NONE"}</option>
            {if !empty($module_dd_fields)}
                <option label="{dotb_translate module="ModuleBuilder" label="LBL_PARENT_DROPDOWN"}" value="parent">{dotb_translate module="ModuleBuilder" label="LBL_PARENT_DROPDOWN"}</option>
            {/if}
            <option label="{dotb_translate module="ModuleBuilder" label="LBL_FORMULA"}" value="formula">{dotb_translate module="ModuleBuilder" label="LBL_FORMULA"}</option>
        </select>
        <script>
...
<tr id='visGridRow' {if empty($vardef.visibility_grid)}style="display:none"{/if} class="toggleDep">
    <td class='mbLBL'>{dotb_translate module="DynamicFields" label="LBL_PARENT_DROPDOWN"}:</td>
	<td>
	...
	{if $hideLevel < 5}
        <button onclick="ModuleBuilder.editVisibilityGrid('visibility_grid', YAHOO.util.Dom.get('parent_dd').value, YAHOO.util.Dom.get('options').value)">
            {dotb_translate module="DynamicFields" label="LBL_EDIT_VIS"}
        </button>
	{/if}
	</td>
</tr>
