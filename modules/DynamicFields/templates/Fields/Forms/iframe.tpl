...
<tr>
	<td class='mbLBL'>{dotb_translate module="DynamicFields" label="LBL_GENERATE_URL"}:</td>
	<td>
	{if $hideLevel < 5}
		<input type='checkbox' id='gencheck' {if $vardef.gen}checked=true{/if} name='genCheck' value="0" onclick="
			if(this.checked) {ldelim}
				 ...
				 addToValidate('popup_form', 'default', 'alphanumeric', true,'{dotb_translate module="DynamicFields" label="COLUMN_TITLE_DEFAULT_VALUE"}' );
			{rdelim} else {ldelim}
...
<tr>
	<td class='mbLBL'>{dotb_translate module="DynamicFields" label="COLUMN_TITLE_FRAME_HEIGHT"}:</td>
	<td>
	{if $hideLevel < 5}
