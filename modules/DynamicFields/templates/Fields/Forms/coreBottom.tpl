...
<tr>
    <td class='mbLBL'>{dotb_translate module="DynamicFields" label="COLUMN_TITLE_FTS"}:</td>
    <td>
        ...
        <img border="0" class="inlineHelpTip" alt="Information" src="themes/Dotb/images/helpInline.png" onclick="return DOTB.util.showHelpTips(this,'{$mod_strings.LBL_POPHELP_FTS_FIELD_CONFIG}','','' );">
    </td>
</tr>
<tr id="ftsFieldBoostRow" {if $fts_field_config_selected < 2}style="display:none"{/if}>
    <td class='mbLBL'>{dotb_translate module="DynamicFields" label="COLUMN_TITLE_FTS_BOOST"}:</td>
    <td>
        <input type="text" name="full_text_search[boost]" id="fts_field_boost" value="{$fts_field_boost_value}" />
        <img border="0" class="inlineHelpTip" alt="Information" src="themes/Dotb/images/helpInline.png" onclick="return DOTB.util.showHelpTips(this,'{$mod_strings.LBL_POPHELP_FTS_FIELD_BOOST}','','' );">
    </td>
</tr>
{/if}
...
<tr ><td class='mbLBL'>{dotb_translate module="DynamicFields" label="COLUMN_TITLE_REQUIRED_OPTION"}:</td><td><input type="checkbox" name="required" value="1" {if !empty($vardef.required)}CHECKED{/if} {if $hideLevel > 5}disabled{/if}/>{if $hideLevel > 5}<input type="hidden" name="required" value="{$vardef.required}">{/if}</td></tr>
{/if}
<tr>
{if !$hideReportable}
<td class='mbLBL'>{dotb_translate module="DynamicFields" label="COLUMN_TITLE_REPORTABLE"}:</td>
<td>
...
<tr><td class='mbLBL'>{dotb_translate module="DynamicFields" label="COLUMN_TITLE_AUDIT"}:</td><td><input id="auditedCheckbox" type="checkbox" name="audited" value="1" {if !empty($vardef.audited) || !empty($vardef.pii) }CHECKED{/if} {if $hideLevel > 5}disabled{/if}/>{if $hideLevel > 5}<input type="hidden" name="audited" value="{$vardef.audited}">{/if}</td></tr>
{if !in_array($vardef.type, array('bool', 'image', 'relate'))}
<tr>
    <td class='mbLBL'>{dotb_translate module="DynamicFields" label="COLUMN_TITLE_PII"}:</td>
    <td>
        ...
        <img border="0" class="inlineHelpTip" alt="Information" src="themes/Dotb/images/helpInline.png" onclick="return DOTB.util.showHelpTips(this,'{$mod_strings.LBL_POPHELP_PII}','','' );">
    </td>
</tr>
{/if}
{/if}
...
{if !$hideImportable}
<tr><td class='mbLBL'>{dotb_translate module="DynamicFields" label="COLUMN_TITLE_IMPORTABLE"}:</td><td>
    {if $hideLevel < 5}
        {html_options name="importable" id="importable" selected=$vardef.importable options=$importable_options}
        {dotb_help text=$mod_strings.LBL_POPHELP_IMPORTABLE FIXX=250 FIXY=80}
    {else}
...
{/if}
{if !$hideDuplicatable}
<tr><td class='mbLBL'>{dotb_translate module="DynamicFields" label="COLUMN_TITLE_DUPLICATE_MERGE"}:</td><td>
{if $hideLevel < 5}
    ...
    {dotb_help text=$mod_strings.LBL_POPHELP_DUPLICATE_MERGE FIXX=250 FIXY=80}
{else}
