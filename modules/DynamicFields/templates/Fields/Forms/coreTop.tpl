...
<table width="100%">
<tr>
	<td class='mbLBL' width='30%' >{dotb_translate module="DynamicFields" label="COLUMN_TITLE_NAME"}:</td>
	<td>
	{if $hideLevel == 0}
	...
	{/if}
        <script>
            {literal}
            addToValidateCallback("popup_form", "name", "callback", true, "{/literal}{dotb_translate module="DynamicFields" label="COLUMN_TITLE_NAME"}{literal}", (function(nameExceptions, existingFields) {
                return function(formName, fieldName, index) {
                    ...
                    if (!isDBName(value)) {
                        validate[formName][index][msgIndex] = "{/literal}{dotb_translate module="DynamicFields" label="ERR_FIELD_NAME_NON_DB_CHARS"}{literal}";
                        return false;
                    }
                    ...
                    for (i = 0; i < nameExceptions.length; i++) {
                        ...
                        if (arrValue == value) {
                            validate[formName][index][msgIndex] = "{/literal}{dotb_translate module="DynamicFields" label="ERR_RESERVED_FIELD_NAME"}{literal}";
                            return false;
                        }
                    }
                    ...
                    for (i = 0; i < existingFields.length; i++) {
                        ...
                        if (arrValue == value) {
                            validate[formName][index][msgIndex] = "{/literal}{dotb_translate module="DynamicFields" label="ERR_FIELD_NAME_ALREADY_EXISTS"}{literal}";
                            return false;
                        }
                    }
                    {/literal}{/if}{literal}
...
<tr>
	<td class='mbLBL'>{dotb_translate module="DynamicFields" label="COLUMN_TITLE_DISPLAY_LABEL"}:</td>
	<td>
...
<tr>
	<td class='mbLBL'>{dotb_translate module="DynamicFields" label="COLUMN_TITLE_HELP_TEXT"}:</td><td>{if $hideLevel < 5 }<input type="text" name="help" value="{$vardef.help}">{else}<input type="hidden" name="help" value="{$vardef.help}">{$vardef.help}{/if}
	</td>
</tr>
<tr>
    <td class='mbLBL'>{dotb_translate module="DynamicFields" label="COLUMN_TITLE_COMMENT_TEXT"}:</td><td>{if $hideLevel < 5 }<input type="text" name="comments" value="{$vardef.comments}">{else}<input type="hidden" name="comment" value="{$vardef.comment}">{$vardef.comment}{/if}
    </td>
</tr>
