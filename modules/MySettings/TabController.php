...
class TabController
{
    ...
    {
        //Administration MySettings are already dotb-cached, and hence only need to retrieve it directly
        $administration = Administration::getSettings('MySettings');
        if (isset($administration->settings) && isset($administration->settings['MySettings_tab'])) {
    ...
    public function get_system_tabs($filter = true)
    {
        ...
        if (isset($administration->settings) && isset($administration->settings['MySettings_tab'])) {
            ...
            if (!empty($trimmed_tabs)) {
                ...
                if ($filter) {
                    $tabs = DotbACL::filterModuleList($tabs, 'access', true);
                }
    ...
    public static function getAllPortalTabs()
    {
        ...
        $browser = new DotbPortalBrowser();
        $browser->loadModules();
        foreach ($browser->modules as $moduleName => $dotbPortalModule) {
            if (!empty($dotbPortalModule->views['list.php'])) {
                $tabs[] = $moduleName;
            }
        }
