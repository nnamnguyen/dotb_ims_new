...
class iCal extends vCal {
    ...
    *
    * @param DotbDateTime $dateTime the dateTime to format
    ...
    *
    * Checks the version of Dotb to see if user timezone adjustments are needed.
    *
    ...
    */
    ...
    {
        global $timedate, $dotb_version;
        $timestamp = ($ts+(date('Z')-$timedate->adjustmentForUserTimeZone()*60));
        return $this->getUtcDateTime(new DotbDateTime("@" . $ts));
    }
    ...
    */
    protected function createDotbIcalTodo($user_bean, $task, $moduleName, $dtstamp)
    {
        global $dotb_config;
        $ical_array = array();
        ...
        $date_arr = array(
             ...
             'min'=>$dueMin,
             'year'=>$dueYear);
        $due_date_time = new DotbDateTime();
        ...
        $ical_array[] = array(
            "URL;VALUE=URI",
            $dotb_config['site_url']."/index.php?module=".$moduleName."&action=DetailView&record=".$task->id
        );
    ...
    */
    protected function createDotbIcal(&$user_bean,&$start_date_time,&$end_date_time, $dtstamp)
    {
        $ical_array = array();
        global $DO_USER_TIME_OFFSET, $dotb_config, $current_user, $timedate;
        ...
        foreach($acts_arr as $act)
        {
            ...
            if (!$hide_calls || ($hide_calls && $event->object_name != "Call"))
            {
                ...
                $ical_array[] = array(
                    "URL;VALUE=URI",
                    $dotb_config['site_url']."/index.php?module=".
        ...
        if (is_array($projectTaskList))
        {
            ...
            {
                $str .= $this->createDotbIcalTodo($user_bean, $projectTask, "ProjectTask", $dtstamp);
            }
        }
        ...
        if ($taskAsVTODO) {
            ...
            if (is_array($taskList))
            {
                ...
                {
                    $str .= $this->createDotbIcalTodo($user_bean, $task, "Tasks", $dtstamp);
                }
            }
        }
    ...
    */
    protected function getDSTRange($current_user, $year)
    {
        ...
        $year_date = DotbDateTime::createFromFormat("Y", $year, new DateTimeZone("UTC"));
    ...
    */
    function getVcalIcal(&$user_focus, $num_months)
    {
        ...
        $ical_array[] = array("X-WR-CALNAME", "$cal_name (DotBCRM)");
        ...
        $str .= $this->createDotbIcal($user_focus,$start_date_time,$end_date_time,$utc_now_time);
