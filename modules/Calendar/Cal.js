	...
	CAL.make_draggable = function (id,type){
		...
		dd.onDragDrop = function(e,id){ 
			...
			var url = "index.php?module=Calendar&action=Reschedule&dotb_body_only=true";
			var data = {
	...
	CAL.make_resizable = function (id,slot){
		...
		resize.on('endResize', function(e){
			...
			if(el){
				...
				ajaxStatus.showStatus(DOTB.language.get('app_strings', 'LBL_SAVING'));						
				var url = "index.php?module=Calendar&action=Resize&dotb_body_only=true";
				var data = {
	...
	CAL.add_item_to_grid = function (item){	
			...
			var item_text = DOTB.language.languages.app_list_strings[item.type +'_status_dom'][item.status];
	...
	CAL.reset_edit_dialog = function (){
		...
		if(typeof DotbWidgetSchedulerSearch.hideCreateForm != 'undefined')
	...
	CAL.GR_update_user = function (user_id){
		var callback = {
			success: function(o){				
				...
				//DotbWidgetScheduler.update_time();
			}
		};	
		var data = {
			"users": user_id
		};
		var url = "index.php?module=Calendar&action=GetGRUsers&dotb_body_only=true";	
		YAHOO.util.Connect.asyncRequest('POST',url,callback,CAL.toURI(data));
	...
	CAL.GR_update_focus = function (module,record){
		if(record == ""){
			...
			var url = 'index.php?module=Calendar&action=GetGR&dotb_body_only=true&type=' + module + '&record=' + record;	
     ...
     * open Lumia record views in a drawer (and allow the user to close)
     *
    ...
    CAL.load_create_form = function (params) {
        ...
        var url = "index.php?module=Calendar&action=QuickEdit&dotb_body_only=true";
        var data = {
	...
	CAL.dialog_save = function(){
								...
								success: function(o){
									...
									if(res.access == 'yes'){
										...
										if(typeof res.limit_error != "undefined"){
											...
											var moduleTitle = DOTB.language.get('app_list_strings', 'moduleListSingular')[module].toLowerCase();
						...
						var url = "index.php?module=Calendar&action=SaveActivity&dotb_body_only=true"; 
	...
	CAL.dialog_remove = function(){
									...
									var url = "index.php?module=Calendar&action=Remove&dotb_body_only=true";									
									YAHOO.util.Connect.asyncRequest('POST',url,callback,CAL.toURI(data));
	...
	CAL.refresh = function () {
		...
		var url = "index.php?module=Calendar&action=getActivities&dotb_body_only=true";									
		YAHOO.util.Connect.asyncRequest('POST', url, callback, CAL.toURI(data));
	...
	CAL.show_additional_details = function (id){
		...
		DOTB.util.getAdditionalDetails(module_name, record, 'div_'+id, true);
		return;
	}
	
	CAL.clear_additional_details = function (id){
		if(typeof DOTB.util.additionalDetailsCache[id] != "undefined")
			DOTB.util.additionalDetailsCache[id] = undefined;		
		if(typeof DOTB.util.additionalDetailsCalls[id] != "undefined")
			DOTB.util.additionalDetailsCalls[id] = undefined;		
	}			
	...
	CAL.update_vcal = function(){
					...
					var callback = {
						success: function(result){ 
							...
							GLOBAL_REGISTRY.freebusy[v] = DotbVCalClient.parseResults(result.responseText, false);
							// parse for current user adjusted vCal
							GLOBAL_REGISTRY.freebusy_adjusted[v] = DotbVCalClient.parseResults(result.responseText, true);
							DotbWidgetScheduler.update_time();
						}												
					};
