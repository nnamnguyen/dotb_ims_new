...
class CalendarDisplay {
	...
	public function display(){
		...
		$ss->assign('items_draggable',DotbConfig::getInstance()->get('calendar.items_draggable',true));
		$ss->assign('items_resizable',DotbConfig::getInstance()->get('calendar.items_resizable',true));
		...
		$ss->assign('editview_width',DotbConfig::getInstance()->get('calendar.editview_width',800));
		$ss->assign('editview_height',DotbConfig::getInstance()->get('calendar.editview_height',600));

		$ss->assign('a_str',json_encode($cal->items));

		$ss->assign('dotb_body_only',(isset($_REQUEST['to_pdf']) && $_REQUEST['to_pdf'] || isset($_REQUEST['dotb_body_only']) && $_REQUEST['dotb_body_only']));
		...
		if($_REQUEST['module'] == "Calendar"){
			...
			$ss->assign("settings",DotbAutoLoader::existingCustomOne("modules/Calendar/tpls/settings.tpl"));
		}

		$ss->assign("form",DotbAutoLoader::existingCustomOne("modules/Calendar/tpls/form.tpl"));

		if($this->cal->enable_repeat){
			$ss->assign("repeat", DotbAutoLoader::existingCustomOne("modules/Calendar/tpls/repeat.tpl"));
		...
		echo $ss->fetch(DotbAutoLoader::existingCustomOne("modules/Calendar/tpls/main.tpl"));
	...
	protected function load_settings_template($ss)
	{
		...
		if(is_null($displayTimeslots)) {
			$displayTimeslots = DotbConfig::getInstance()->get('calendar.display_timeslots', true);
		}
	...
	public function get_date_info($view, $date_time){
		...
		if($view == 'month'){
			...
			if($view == 'week' || $view == 'shared') {
				...
				dotb_die("echo_date_info: date not supported");
			}
		return $str;
	}
	...
	protected function get_next_calendar(){
		...
		if($_REQUEST['module'] == "Calendar"){
			...
			$str .= "<a href='#' onclick='return DOTB.myDotb.retrieveDashlet(\"".$this->dashlet_id."\", \"index.php?module=Home&action=DynamicAction&DynamicAction=displayDashlet&dotb_body_only=1&".$this->cal->get_neighbor_date_str("next")."&id=".$this->dashlet_id."\")'>";
		}
			$str .= $cal_strings["LBL_NEXT_".strtoupper($this->cal->view)];

		$str .= "&nbsp;&nbsp;".DotbThemeRegistry::current()->getImage("calendar_next", 'align="absmiddle" border="0"' ,null,null,'.gif', '') . "</a>"; //setting alt tag blank on purpose for 508 compliance
		return $str;
	}
	...
	protected function get_previous_calendar(){
		...
		if($_REQUEST['module'] == "Calendar"){
			...
			$str .= "<a href='#' onclick='return DOTB.myDotb.retrieveDashlet(\"".$this->dashlet_id."\", \"index.php?module=Home&action=DynamicAction&DynamicAction=displayDashlet&dotb_body_only=1&".$this->cal->get_neighbor_date_str("previous")."&id=".$this->dashlet_id."\")'>";
		}
		$str .= DotbThemeRegistry::current()->getImage('calendar_previous','align="absmiddle" border="0"', null, null, '.gif', ''); //setting alt tag blank on purpose for 508 compliance
	...
	public function display_calendar_header($controls = true){
		...
		if($controls){
			...
			$ss->assign('cal_img',DotbThemeRegistry::current()->getImageURL("jscalendar.gif",false));
		}
		...
		echo $ss->fetch(DotbAutoLoader::existingCustomOne("modules/Calendar/tpls/header.tpl"));
	}
	...
	public function display_calendar_footer(){
		...
		echo $ss->fetch(DotbAutoLoader::existingCustomOne("modules/Calendar/tpls/footer.tpl"));
	}
	...
	public function display_shared_html(){
			...
			$ss->assign("MOD",$cal_strings);
			$ss->assign("UP",DotbThemeRegistry::current()->getImage('uparrow_big', 'border="0" style="margin-bottom: 1px;"', null, null, '.gif', $app_strings['LBL_SORT']));
			$ss->assign("DOWN",DotbThemeRegistry::current()->getImage('downarrow_big', 'border="0" style="margin-top: 1px;"', null, null, '.gif', $app_strings['LBL_SORT']));
