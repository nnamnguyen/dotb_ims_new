...
class CalendarUtils
{
	 ...
	 * @param DotbDateTime $date
	 * @return DotbDateTime $date
	 */
	static function get_first_day_of_week(DotbDateTime $date){
     ...
     * @return array
     */
    public static function get_time_data(DotbBean $bean)
    {
        $arr = array();
        ...
        $dtmStart = DotbDateTime::createFromFormat(
            $GLOBALS['timedate']->get_date_time_format(),
            $userStartDate,
            new DateTimeZone('UTC')
        );
        $dtmEnd = DotbDateTime::createFromFormat(
            $GLOBALS['timedate']->get_date_time_format(),
            $userEndDate,
            new DateTimeZone('UTC')
        );
	 ...
	 * @return array
	 */
	static function getBeanDataArray(DotbBean $bean)
	{
	 ...
	 * @return array
	 */
	 static function getRepeatData(DotbBean $bean, $editAllRecurrences = false, $dateStart = false)
	 {
	 	if ($bean->module_dir == "Meetings" || $bean->module_dir == "Calls") {
            ...
            $date = DotbDateTime::createFromFormat($GLOBALS['timedate']->get_date_time_format(), $dateStart);
            if (empty($date)) {
	...
	static function buildRecurringSequence($date_start, $params)
	{
        ...
        /**
		 * @var DotbDateTime $start Recurrence start date.
		 */
		$start = DotbDateTime::createFromFormat($GLOBALS['timedate']->get_date_time_format(),$date_start);
        $current = clone $start;

        /**
		 * @var DotbDateTime $end Recurrence end date. Used if recurrence ends by date.
         * To Make the RepeatUntil Date Inclusive, we need to Add 1 Day to End
		 */
		if (!empty($params['until'])) {
			$end = DotbDateTime::createFromFormat($GLOBALS['timedate']->get_date_format(), $until);
     ...
     * @return array
     */
    public static function saveRecurring(DotbBean $bean, $timeArray)
    {
		...
		vCal::cache_dotb_vcal($GLOBALS['current_user']);
		return $arr;
	}
     ...
     *
     * {@link DotbBean::mark_deleted()} is not used because it runs slowly. The before_delete and after_delete logic
     * hooks are triggered, but the before_relationship_delete and after_relationship_delete logic hooks are not
     * triggered.
     *
     * @param DotbBean $bean
     */
    public static function markRepeatDeleted(DotbBean $bean)
    {
     ...
     * @param string $beanId
     */
    static function correctRecurrences(DotbBean $bean, $beanId)
    {
        global $db;
     ...
     * @param DotbBean|Call|Meeting $bean
     * @return array
     */
    public static function getInvitees(\DotbBean $bean)
    {
     ...
     *
     * @param Call|Meeting|DotbBean $event
     * @return string[]
     * @throws Exception
     */
    public static function buildInvitesList(\DotbBean $event)
    {
