...
class Calendar {
	...
	public function __construct($view = "day", $time_arr = array()){
		...
		if(is_null($this->show_tasks))
			$this->show_tasks = DotbConfig::getInstance()->get('calendar.show_tasks_by_default',true);		
		$this->show_calls = $current_user->getPreference('show_calls');
		if(is_null($this->show_calls))
			$this->show_calls = DotbConfig::getInstance()->get('calendar.show_calls_by_default',true);
			
		$this->enable_repeat = DotbConfig::getInstance()->get('calendar.enable_repeat',true);	
		...
		if(is_null($this->day_start_time))
			$this->day_start_time = DotbConfig::getInstance()->get('calendar.default_day_start',"08:00");
		$this->day_end_time = $current_user->getPreference('day_end_time');
		if(is_null($this->day_end_time))
			$this->day_end_time = DotbConfig::getInstance()->get('calendar.default_day_end',"19:00");
			
		if($this->view == "day"){
			$this->time_step = DotbConfig::getInstance()->get('calendar.day_timestep',15);
		}else if($this->view == "week" || $this->view == "shared"){
			$this->time_step = DotbConfig::getInstance()->get('calendar.week_timestep',30);
		}else if($this->view == "month"){
			$this->time_step = DotbConfig::getInstance()->get('calendar.month_timestep',60);
		}else{
			$this->time_step = 60;
		}
	 ...
	 * @param string $type
	 */	
	public function add_activities($user,$type='dotb'){
    ...
    protected function generateActivityItem($act, $user_id)
    {
        ...
        $item['module_name'] = $act->dotb_bean->module_dir;
        $item['type'] = strtolower($act->dotb_bean->object_name);
        $item['assigned_user_id'] = $act->dotb_bean->assigned_user_id;
        $item['record'] = $act->dotb_bean->id;
        $item['name'] = $act->dotb_bean->name;

        if (isset($act->dotb_bean->duration_hours)) {
            $item['duration_hours'] = $act->dotb_bean->duration_hours;
            $item['duration_minutes'] = $act->dotb_bean->duration_minutes;
        }

        $item['detail'] = 0;
        $item['edit'] = 0;

        if ($act->dotb_bean->ACLAccess('DetailView')) {
            $item['detail'] = 1;
        }
        if ($act->dotb_bean->ACLAccess('Save')) {
            $item['edit'] = 1;
        }

        if (empty($act->dotb_bean->id)) {
            $item['detail'] = 0;
            $item['edit'] = 0;
        }

        if (!empty($act->dotb_bean->repeat_parent_id)) {
            $item['repeat_parent_id'] = $act->dotb_bean->repeat_parent_id;
        }

        if ($item['detail'] == 1) {
            if (isset($field_list[$item['module_name']])) {
                foreach ($field_list[$item['module_name']] as $field) {
                    if (!isset($item[$field]) && isset($act->dotb_bean->$field)) {
                        $item[$field] = $act->dotb_bean->$field;
                        if (empty($item[$field])) {
                            $item[$field] = "";
                        }
                    }
        ...
        if (!empty($act->dotb_bean->parent_type) && !empty($act->dotb_bean->parent_id)) {
            $focus = BeanFactory::getBean($act->dotb_bean->parent_type, $act->dotb_bean->parent_id);
        ...
        $item = array_merge($item, CalendarUtils::get_time_data($act->dotb_bean));
        return $item;
    }
     ...
     * @return array
     */
    protected function getActivities($user, $type = 'dotb')
    {
