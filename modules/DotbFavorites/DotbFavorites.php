<?php
    ...
    class DotbFavorites extends Basic
    {
        public $new_schema = true;
        public $module_dir = 'DotbFavorites';
        public $object_name = 'DotbFavorites';
        public $table_name = 'dotbfavorites';
        ...
        {
            return '<div class="star"><div class="'. ($on ? 'on': 'off') .'" onclick="var self=this; parent.DOTB.App.api.favorite(\''.$module. '\',  \''.$record. '\', $(self).hasClass(\'off\'), { success: function() {$(self).toggleClass(\'on off\');} });">&nbsp;</div></div>';
        }
        ...
        {
            $id = DotbFavorites::generateGUID($module, $record, $user_id);

            $focus = BeanFactory::getBean('DotbFavorites', $id);
        ...
        {
            if ( empty($user) )
                $where = " dotbfavorites.assigned_user_id = '{$GLOBALS['current_user']->id}' ";
            else
                $where = " dotbfavorites.assigned_user_id = '{$user->id}' ";

            if ( !empty($module) ) {
                if ( is_array($module) ) {
                    $where .= " AND dotbfavorites.module IN ('" . implode("','",$module) . "')";
                }
                else {
                    $where .= " AND dotbfavorites.module = '$module' ";
                }
            }
            $focus = BeanFactory::newBean('DotbFavorites');
        ...
        public static function getFavoritesByModuleByRecord($module, $id)
        {
            ...
            if ( !empty($module) ) {
                if ( is_array($module) ) {
                    $where .= " dotbfavorites.module IN ('" . implode("','",$module) . "')";
                }
                else {
                    $where .= " dotbfavorites.module = '$module' ";
                }
            }

            $where .= " AND dotbfavorites.record_id = '{$id}'";
        ...
        public static function getUserIdsForFavoriteRecordByModuleRecord($module, $id) {
            global $db;
            $query = "SELECT assigned_user_id FROM dotbfavorites WHERE module = '$module' AND record_id = '$id' AND deleted = 0";
        ...
        /**
         * Add a Favorites block to the DotbQuery Object to fetch favorites for a specific [default to current] user
         * @param DotbQuery $dotb_query
         * @param bool $joinTo
         * @param string $join_type
         * @param bool|guid $user_id
         * @return string
         */
        public function addToDotbQuery(DotbQuery $dotb_query, $options = array()) {
            $alias = '';
            ...
            if(!$joinTo) {
                if(is_array($dotb_query->from)) {
                    list($bean, $alias) = $dotb_query->from;
                }
                else {
                    $bean = $dotb_query->from;
                    $alias = $bean->getTableName();
                }
            } else {
                $alias = $joinTo->joinName();
                $bean = $dotb_query->getTableBean($alias);
            }
            ...
            $dotb_query->joinTable(self::getTableName(), array('alias'=>$sfAlias, 'joinType'=>$joinType, 'linkingTable' => true))
         ...
         * {@inheritdoc}
         *
         * Attempt to subscribe the user to the favorited bean after restoring the DotbFavorite bean.
         *
         * @param $id
         */
         ...
         * @return bool|string
         */
        protected function subscribeUserToRecord(User $user, DotbBean $bean)
        {
