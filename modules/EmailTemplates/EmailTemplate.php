 ...
 */
class EmailTemplate extends DotbBean {
	// Stored fields
	var $id;
	var $date_entered;
	var $date_modified;
	...
	function fill_in_additional_detail_fields() {
        ...
        {
            global $dotb_config;
            $this->body = strip_tags(html_entity_decode($this->body_html, ENT_COMPAT, $dotb_config['default_charset']));
        }
...
//function all string that match the pattern {.} , also catches the list of found strings.
    ...
    function parse_tracker_urls($template_text_array,$url_template,$tracked_urls,$removeme_url_template) {
        global $beanFiles,$beanList, $app_list_strings,$dotb_config;
                    ...
                    if($count > 0)
                    {
                        for ($i=($count -1); $i>=0; $i--) {
                            ...
                            if(!empty($tracker_url) && !empty($template_text) && !empty($matches[0][$i][0]) && !empty($tracked_urls[$matches[0][$i][0]])){
                                ...
                                $template_text=str_replace($dotb_config['site_url'].'/'.$dotb_config['site_url'],$dotb_config['site_url'],$template_text);
                            }
                        }
                    }
                }
