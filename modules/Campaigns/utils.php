...
function log_campaign_activity($identifier, $activity, $update=true, $clicked_url_key=null) {
    ...
    if($identifier == 'BANNER' && isset($clicked_url_key)  && !empty($clicked_url_key))
    {
        ...
        if(isset($dotb_config['campaign_banner_id_generation'])  && $dotb_config['campaign_banner_id_generation'] != 'md5'){
        ...
        if ($row==null) {
            ...
            elseif ($row){
                ...
                if (!empty($row['target_id']) && !empty($row['target_type'])) {
                    ...
                    $primeEmail = $dotbEmailAddress->getPrimaryAddress(null, $row['target_id'], $row['target_type']);
                    if (!empty($primeEmail)) {
