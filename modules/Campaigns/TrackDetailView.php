...
if (isset($_REQUEST['offset']) or isset($_REQUEST['record'])) {
	$result = $detailView->processDotbBean("CAMPAIGN", $focus, $offset);
	if($result == null) {
...
if(isset($focus->campaign_type) && $focus->campaign_type == "NewsLetter"){
    ...
    if (!empty($focus->budget)) {
        $smarty->assign("BUDGET", DotbCurrency::formatAmountUserLocale($focus->budget, $focus->currency_id));
    }
    if (!empty($focus->actual_cost)) {
        $smarty->assign("ACTUAL_COST", DotbCurrency::formatAmountUserLocale($focus->actual_cost, $focus->currency_id));
    }
    if (!empty($focus->expected_cost)) {
        $smarty->assign("EXPECTED_COST", DotbCurrency::formatAmountUserLocale($focus->expected_cost, $focus->currency_id));
    }
    if (!empty($focus->expected_revenue)) {
        $smarty->assign("EXPECTED_REVENUE", DotbCurrency::formatAmountUserLocale($focus->expected_revenue, $focus->currency_id));
    }
    ...
    $smarty->assign("TRACKER_URL", $dotb_config['site_url'] . '/campaign_tracker.php?track=' . $focus->tracker_key);
    ...
    if (is_admin($current_user) && $request_module != 'DynamicLayout' && !empty($_SESSION['editinplace'])) {
        ...
        $smarty->assign("ADMIN_EDIT","<a href='index.php?action=index&module=DynamicLayout&from_action=". urlencode($request_action) ."&from_module=". urlencode($request_module) ."&record=". urlencode($request_record) . "'>".DotbThemeRegistry::current()->getImage("EditLayout","border='0' align='bottom'",null,null,'.gif',$mod_strings['LBL_EDIT_LAYOUT'])."</a>");
    }
...
$chart= new campaign_charts();
    ...
    if(empty($latest_marketing_id) ||  $latest_marketing_id === 'all'){
        $smarty->assign("MY_CHART", $chart->campaign_response_by_activity_type($app_list_strings['campainglog_activity_type_dom'],$app_list_strings['campainglog_target_type_dom'],$focus->id,dotb_cached("xml/$cache_file_name"),true));
    }else{
        $smarty->assign("MY_CHART", $chart->campaign_response_by_activity_type($app_list_strings['campainglog_activity_type_dom'],$app_list_strings['campainglog_target_type_dom'],$focus->id,dotb_cached("xml/$cache_file_name"),true,$latest_marketing_id));
    }
