...
class campaign_charts {
	...
	*/
	...
	function campaign_response_by_activity_type($datay= array(),$targets=array(),$campaign_id, $cache_file_name='a_file', $refresh=false, $marketing_id='') {
		...
		$xmlFile = $dotbChart->getXMLFileName($campaign_id);
		...
		if (!file_exists($xmlFile) || $refresh == true) {
            ...
            $dotbChart->group_by = array('activity_type', 'target_type');

            if ($camp_data) {
                $dotbChart->setData($camp_data);
            } else {
                $dotbChart->setData(array());
            }
		...
		$width = '100%';
		$return = '';
		$return .= $dotbChart->display($campaign_id, $xmlFile, $width, '480',"");
	...
	function campaign_response_roi($datay= array(),$targets=array(),$campaign_id, $cache_file_name='a_file', $refresh=false,$marketing_id='',$is_dashlet=false,$dashlet_id='') {
		global $app_strings,$mod_strings, $current_module_strings, $charset, $lang, $app_list_strings, $current_language,$dotb_config;
...
//            if (empty($opp_data['opp_count'])) $opp_data['opp_count']=0;
        ...
        $dotbChart->currency_symbol = $currency_symbol;

        // campaign charts should not convert currency values
        // see DotbChart::convertCurrency()
        $dotbChart->div = 1;

		if ($not_empty)
	 		$dotbChart->setData($opp_data1);
		else
			$dotbChart->setData(array());
        ...
        $dotbChart->setDisplayProperty('xDataType', 'ordinal');
        $dotbChart->setDisplayProperty('yDataType', 'currency');

    	if (!$is_dashlet){
			$xmlFile = $dotbChart->getXMLFileName('roi_details_chart');
			$dotbChart->saveXMLFile($xmlFile, $dotbChart->generateXML());
			$return .= $dotbChart->display('roi_details_chart', $xmlFile, $width, '480');
		}
		else{
			$xmlFile = $dotbChart->getXMLFileName($dashlet_id);
			$dotbChart->saveXMLFile($xmlFile, $dotbChart->generateXML());
			$return .= $dotbChart->display($dashlet_id, $xmlFile, $width, '480');
		}
