...
{ global $mod_strings;
    ...
    if($num == $total){
        //red
        return DotbThemeRegistry::current()->getImage('red_camp', "align='absmiddle'", null, null, ".gif", $mod_strings['LBL_INVALID']);


    }elseif($num == 0){
        //if health number is zero, then all checks passed, set green image
        //green
       return DotbThemeRegistry::current()->getImage('green_camp', "align='absmiddle'", null, null, ".gif", $mod_strings['LBL_VALID']);


    }else{
        //if health number is between total and num params, then some checks failed but not all, set yellow image
        //yellow
        return DotbThemeRegistry::current()->getImage('yellow_camp', "align='absmiddle'", null, null, ".gif", $mod_strings['LBL_ALERT']);
