...
global $currentModule;
    ...
    if(isset($_REQUEST['from'])){
        ...
        $confirm_msg = "var ajaxWizStatus = new DOTB.ajaxStatusClass(); ";
    ...
    $ss->display(DotbAutoLoader::existingCustomOne('modules/Campaigns/WizardHome.html'));
...
*/
    ...
    $ss->assign("MOD", $mod_strings);
    $ss->assign("APP", $app_strings);
    $ss->display(DotbAutoLoader::existingCustomOne('modules/Campaigns/tpls/WizardHomeStart.tpl'));
}
...
function create_campaign_summary  ($focus){
    ...
    if (!empty($focus->budget)) {
        $focus->budget = DotbCurrency::formatAmountUserLocale($focus->budget, $focus->currency_id);
    }
    if (!empty($focus->actual_cost)) {
        $focus->actual_cost = DotbCurrency::formatAmountUserLocale($focus->actual_cost, $focus->currency_id);
    }
    if (!empty($focus->expected_cost)) {
        $focus->expected_cost = DotbCurrency::formatAmountUserLocale($focus->expected_cost, $focus->currency_id);
    }
    if (!empty($focus->expected_revenue)) {
        $focus->expected_revenue = DotbCurrency::formatAmountUserLocale($focus->expected_revenue, $focus->currency_id);
    }
            ...
            foreach($pl_lists as $pl_id){
             ...
             if( $colorclass== "class='evenListRowS1'"){
              ...
              if(isset($pl_focus->id) && !empty($pl_focus->id)){
                ...
                $pl_tbl  .= DotbThemeRegistry::current()->getImage('edit_inline', 'border=0', null, null, ".gif", $mod_strings['LBL_EDIT_INLINE']) . "</a>&nbsp;";
                ...
                $pl_tbl  .= DotbThemeRegistry::current()->getImage('view_inline', 'border=0', null, null, ".gif", $mod_strings['LBL_VIEW_INLINE'])."</a></td>";
