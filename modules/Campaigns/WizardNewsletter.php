...
if (!empty($focus->budget)) {
    $focus->budget = DotbCurrency::formatAmountUserLocale($focus->budget, $focus->currency_id, false);
}
if (!empty($focus->actual_cost)) {
    $focus->actual_cost = DotbCurrency::formatAmountUserLocale($focus->actual_cost, $focus->currency_id, false);
}
if (!empty($focus->expected_cost)) {
    $focus->expected_cost = DotbCurrency::formatAmountUserLocale($focus->expected_cost, $focus->currency_id, false);
}
if (!empty($focus->expected_revenue)) {
    $focus->expected_revenue = DotbCurrency::formatAmountUserLocale($focus->expected_revenue, $focus->currency_id, false);
}
...
if(is_admin($current_user) && $_REQUEST['module'] != 'DynamicLayout' && !empty($_SESSION['editinplace'])){
    ...
    $ss->assign("ADMIN_EDIT","<a href='index.php?action=index&module=DynamicLayout&from_action=".$_REQUEST['action'] ."&from_module=".$_REQUEST['module'] ."&record=".$record. "'>".DotbThemeRegistry::current()->getImage("EditLayout","border='0' align='bottom'",null,null,'.gif',$mod_strings['LBL_EDIT_LAYOUT'])."</a>");
...
//create the html to create tracker table
    foreach($trkr_lists as $trkr_id){
      ...
      if(isset($ct_focus->tracker_name) && !empty($ct_focus->tracker_name)){
            ...
            $trkr_html .= DotbThemeRegistry::current()->getImage('delete_inline', "border='0'  align='absmiddle'", 12, 12, ".gif", $mod_strings['LBL_DELETE'])."</a></td></tr></table></div>";
 ...
 //this is not a newlsetter campaign, so fill in target list table
    ...
    if(count($prospect_lists)>0){
        ...
        //retrieve prospect list
             ...
             $trgt_html .= DotbThemeRegistry::current()->getImage('delete_inline', "border='0' align='absmiddle'", 12, 12, ".gif", $mod_strings['LBL_DELETE'])."</a></td></tr></table></div>";
...
$ss->display(DotbAutoLoader::existingCustomOne('modules/Campaigns/tpls/WizardNewsletter.tpl'));

function create_common_wizard_steps()
{
    global $mod_strings;
    $steps = array();
    $steps[$mod_strings['LBL_NAVIGATION_MENU_GEN1']]          = DotbAutoLoader::existingCustomOne('modules/Campaigns/tpls/WizardCampaignHeader.tpl');
    $steps[$mod_strings['LBL_NAVIGATION_MENU_GEN2']]          = DotbAutoLoader::existingCustomOne('modules/Campaigns/tpls/WizardCampaignBudget.tpl');
    $steps[$mod_strings['LBL_NAVIGATION_MENU_TRACKERS']]      = DotbAutoLoader::existingCustomOne('modules/Campaigns/tpls/WizardCampaignTracker.tpl');
    return $steps;
}
...
function create_newsletter_steps(){
    ...
    $steps[$mod_strings['LBL_NAVIGATION_MENU_SUBSCRIPTIONS']] = DotbAutoLoader::existingCustomOne('modules/Campaigns/tpls/WizardCampaignTargetList.tpl');
    return  $steps;
}
...
function create_campaign_steps(){
    global $mod_strings;
    $steps = create_common_wizard_steps();
    $steps[$mod_strings['LBL_TARGET_LISTS']]                   = DotbAutoLoader::existingCustomOne('modules/Campaigns/tpls/WizardCampaignTargetListForNonNewsLetter.tpl');
    return  $steps;
}
