...
use Dotbcrm\Dotbcrm\Security\Subject\WebToLead;

require_once('include/formbase.php');



global $app_strings, $app_list_strings, $dotb_config, $timedate, $current_user;

$mod_strings = return_module_language($dotb_config['default_language'], 'Leads');
...
if (isset($_POST['campaign_id']) && !empty($_POST['campaign_id'])) {
	    ...
	    if(isset($camp_data) && $camp_data != null ){
            ...
            // Bug #52563 : Web to Lead form redirects to Dotb when duplicate detected
            ...
            $optOut = !empty($GLOBALS['dotb_config']['new_email_addresses_opted_out']);
            ...
            if ($redirect_url !== null) {
			    ...
			    if(headers_sent() || strlen($redirect_url) > 2083){
    				echo '<html ' . get_language_header() . '><head><title>DotBCRM</title></head><body>';
 ...
 * @param bool $optOut
 */
function _setDefaultEmailProperties(DotbBean $lead, $emailField, $optOut = false)
{
