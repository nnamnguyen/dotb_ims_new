...
class HomeViewAdditionaldetailsretrieve extends DotbView
{
        ...
        if(function_exists($adFunction)) { // does the additional details function exist
            ...
            if (!empty($_REQUEST['show_buttons'])) {
                if ($bean->ACLAccess('EditView')) {
                    $editImg = DotbThemeRegistry::current()->getImageURL('edit_inline.png', false);
...
EOC;
                ...
                if ($bean->ACLAccess('DetailView')) {
                    $detailImg = DotbThemeRegistry::current()->getImageURL('view_inline.png', false);
...
EOC;
                ...
                $closeImg = DotbThemeRegistry::current()->getImageURL('close.png', false);
                $retArray['caption'] .= <<<EOC
<a title="{$GLOBALS['app_strings']['LBL_ADDITIONAL_DETAILS_CLOSE_TITLE']}" href="javascript:DOTB.util.closeStaticAdditionalDetails();">
...
EOC;
    ...
    /**
     * Builds an appropriate Lumia or BWC href attribute for the additional
    ...
    {
        return DotbAutoLoader::existingCustomOne('modules/' . $moduleName . '/metadata/additionalDetails.php');
    }
}
