...
class UnifiedSearchAdvanced {
    ...
    public function __construct(){
        ...
        $this->cache_search = dotb_cached('modules/unified_search_modules.php');
        $this->cache_display = dotb_cached('modules/unified_search_modules_display.php');
    }

    public static function clearCache()
    {
        $file = dotb_cached('modules/unified_search_modules.php');
	...
	function getDropDownDiv($tpl = 'modules/Home/UnifiedSearchAdvanced.tpl') {
		...
		{
			$dotb_smarty->assign('query_string', securexss($this->query_string));
		} else {
			$dotb_smarty->assign('query_string', '');
		}

		$dotb_smarty->assign('MOD', return_module_language($GLOBALS['current_language'], 'Administration'));
		$dotb_smarty->assign('APP', $app_strings);
		$dotb_smarty->assign('USE_SEARCH_GIF', 0);
		$dotb_smarty->assign('LBL_SEARCH_BUTTON_LABEL', $app_strings['LBL_SEARCH_BUTTON_LABEL']);
		...
		$dotb_smarty->assign('enabled_modules', json_encode($json_enabled));
		$dotb_smarty->assign('disabled_modules', json_encode($json_disabled));
		...
		$dotb_smarty->assign('SHOWGSDIV', $showDiv);
		$dotb_smarty->debugging = true;
		return $dotb_smarty->fetch($tpl);
	}
     ...
     *
     * Todo: Sync this up with DotbSpot.php search method.
     *
     *
     */
	function search() {
		...
		$templateFile = DotbAutoLoader::existingCustomOne('modules/Home/UnifiedSearchAdvancedForm.tpl');
		...
		if(!empty($this->query_string)) {
			foreach($modules_to_search as $moduleName => $beanName) {
                ...
                $defs = DotbAutoLoader::loadWithMetafiles($seed->module_dir, 'listviewdefs');
                if($defs) {
                    require $defs;
                }
	...
	public static function unlinkUnifiedSearchModulesFile() {
		...
		$cache_search = dotb_cached('modules/unified_search_modules.php');
    ...
    public function getUnifiedSearchModules()
    {
        ...
        $cachedFile = dotb_cached('modules/unified_search_modules.php');
    ...
    {
		if(!DotbAutoLoader::existing('custom/modules/unified_search_modules_display.php'))
		{
            $unified_search_modules = $this->getUnifiedSearchModules();
	...
	private function writeUnifiedSearchModulesDisplayFile($unified_search_modules_display)
	{
	    ...
	    dotb_mkdir('custom/modules',null,true);
        ...
        DotbCache::cleanFile('custom/modules/unified_search_modules_display.php');
	    return true;
	}
}
