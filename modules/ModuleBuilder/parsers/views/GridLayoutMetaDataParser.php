...
class GridLayoutMetaDataParser extends AbstractMetaDataParser implements MetaDataParserInterface
{
     ...
     * 
     * This is used by the lumia grid parser but is defined here since it is set
    ...
    public function __construct($view, $moduleName, $packageName = '', $client = '', array $params = array())
    {
        ...
        if (!$this->hasViewVariable($viewdefs, $view)) {
            dotb_die ( get_class ( $this ) . ": incorrect view variable for $view" ) ;
        }
        ...
        if ($this->getMaxColumns () < 1)
            dotb_die ( get_class ( $this ) . ": maxColumns=" . $this->getMaxColumns () . " - must be greater than 0!" ) ;
        ...
        // Setup the fieldset member fields. Used by lumia.
        $this->setFieldsetMemberFields();
    }

    /**
     * Sets the fields that are part of fieldsets. Since this is used by lumia
    ...
    public function validateMetaData($viewdefs) {
        if (!isset($viewdefs['templateMeta'])) {
            dotb_die(get_class($this) . ': missing templateMeta section in layout definition (case sensitive)');
        }

        if (!isset($viewdefs['panels'])) {
            dotb_die(get_class($this) . ': missing panels section in layout definition (case sensitive)');
        }
    }
    ...
    *
    * NOTE: At the moment this is a Lumia parser function only. It is declared
