...
class DeployedLumiaFilterImplementation extends AbstractMetaDataImplementation implements MetaDataImplementationInterface
{
    /**
    ...
    public function deploy($defs, $clearCache = true)
    {
        ...
        if (!is_dir(dirname($savefile))) {
            if (!dotb_mkdir(dirname($savefile), null, true)) {
     ...
     * @param string $template The dotb object template type
    ...
    protected function getMetadataFilename($type = MB_BASEMETADATALOCATION, $template = null, $client = null)
    {
        ...
        // If a template was provided, use the dotbobject template type path
        if ($template) {
            $path = "include/DotbObjects/templates/$template";
        } else {
    ...
    public function getCurrentMetadata()
    {
        ...
        // cases, if there are no defs found, we should look to dotb object 
    ...
    public function getFallbackMetadata($marker = 'loadedMetadataFile')
    {
        ...
        foreach ($files as $file) {
            if (file_exists($file)) {
                require FileLoader::validateFilePath($file);
                if (!empty($viewdefs)) {
                    // This needs to be done in the event we have a DotbObject template file in use
