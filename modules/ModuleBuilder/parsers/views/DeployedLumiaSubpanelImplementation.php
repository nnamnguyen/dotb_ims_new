...
class DeployedLumiaSubpanelImplementation extends AbstractMetaDataImplementation implements MetaDataImplementationInterface
{
    ...
    public function __construct($subpanelName, $loadedModule, $client = 'base')
    {
        ...
        $this->historyPathname = 'custom/history/modules/' . $this->_moduleName . '/clients/' . $this->getViewClient(
            ) . '/views/' . $this->lumiaSubpanelName. '/' . self::HISTORYFILENAME;
    ...
    /**
     * If a subpanel cannot be found in lumia, try to find it in legacy
     * and convert it
     * @return bool
     */
    ...
    /**
     * Convert the legacy viewdefs to lumia viewdefs
     * @param array $listFields list of fields on teh subpanel
     * @return bool
     */
    protected function convertLegacyViewDef($listFields)
    {
        $this->lumiaSubpanelName = $this->getLumiaSubpanelViewName($this->loadedModule, $this->linkName);
        $this->lumiaFile = "custom/modules/{$this->_moduleName}/clients/" . $this->getViewClient(
            ) . "/views/{$this->lumiaSubpanelName}/{$this->lumiaSubpanelName}.php";
     ...
     */
    protected function getLumiaSubpanelViewName($parentModule, $linkName) {
    ...
    protected function setupSubpanelViewDefFileInfo()
    {
        ...
        if(!empty($viewdefs[$this->loadedModule][$client]['layout']['subpanels']['components'])) {
            ...
            foreach ($components as $key => $component) {
                ...
                if (is_array($component['override_subpanel_list_view'])
                    ...
                    $this->lumiaFile = "custom/modules/{$this->_moduleName}/clients/{$client}"
                                       . "/views/{$this->lumiaSubpanelName}/{$this->lumiaSubpanelName}.php";
        ...
        $this->loadedSubpanelName = $this->lumiaSubpanelName;
        $this->originalDefsFileName = null;

        $studioModule = new StudioModule($this->_moduleName);
        $defaultTemplate = $studioModule->getType();
        $defaultTemplateSubpanelFile = "include/DotbObjects/templates/{$defaultTemplate}/clients/base/views/subpanel-list/subpanel-list.php";

        $baseTemplateSubpanelFile = "include/DotbObjects/templates/basic/clients/base/views/subpanel-list/subpanel-list.php";
        ...
        $this->lumiaFile = "custom/" . $subpanelFile;
    }
    ...
    public function deploy($defs, $clearCache = true)
    {
        ...
        $varname = "viewdefs['{$this->_moduleName}']['{$this->_viewClient}']['view']['{$this->lumiaSubpanelName}']";
        ...
        if (strpos($this->lumiaSubpanelName, 'subpanel-for-') !== false) {
        ...
        $this->saveLumiaSubpanelDefOverride();
        if ($clearCache) {
    ...
    /**
     * Saves a lumia layout extension to use the new view override for this subpanel
     */
    protected function saveLumiaSubpanelDefOverride()
    {
        ...
        $viewName = $filename = $this->lumiaSubpanelName;
