...
class DeployedMetaDataImplementation extends AbstractMetaDataImplementation implements MetaDataImplementationInterface
{
    ...
    public function __construct($view, $moduleName, $client = '', array $params = array())
	{
		...
		{
			dotb_die ( get_class ( $this ) . ": Modulename $moduleName is not a Deployed Module" ) ;
		}
		// END ASSERTIONS
		...
		if ($loaded === null)
		{
			switch ( $view )
			{
                ...
                case MB_WIRELESSLISTVIEW:
                    $_viewtype = 'mobile';
                if (in_array($view,
                    array(MB_RECORDVIEW, MB_LUMIAPOPUPVIEW, MB_LUMIADUPECHECKVIEW, MB_LUMIALISTVIEW))) {
                    $_viewtype = 'base';
                }
					...
					// If we're missing a wireless view, we can create it easily from a template, sourced from DotbObjects
					// First, need to identify which DotbObject template would be the best to use
					$type = $module->getType () ;
					$this->_sourceFilename = $this->getFileName ( $view, $moduleName, MB_CUSTOMMETADATALOCATION ) ;

					// Recurse through the various DotbObjects templates to get
        		...
        		case MB_DASHLET:
	        		...
	        		elseif(file_exists(
	        			...
	        			$loaded = $this->_loadFromFile ( "include/DotbObjects/templates/$type/metadata/".basename ( $this->_sourceFilename ) ) ;
	        			$needSave = true;
	        		}
	        		if ($loaded === null)
						throw new Exception( get_class ( $this ) . ": cannot create dashlet view for module $moduleName - definitions for $view are missing in the DotbObject template for type $type" ) ;
        		...
        		case MB_POPUPSEARCH:
					...
					global $current_language;
					$mod = return_module_language($current_language , $moduleName);
					$loadedForWrite = $this->_loadFromPopupFile (  "include/DotbObjects/templates/$type/metadata/".basename ( $this->_sourceFilename )  , $mod, $view, true);
        			if ($loadedForWrite === null)
						throw new Exception( get_class ( $this ) . ": cannot create popup view for module $moduleName - definitions for $view are missing in the DotbObject template for type $type" ) ;
					...
					$loaded = $this->_loadFromPopupFile (  "include/DotbObjects/templates/$type/metadata/".basename ( $this->_sourceFilename )  , $mod, $view);
					$this->_mergeFielddefs ( $fielddefs , $loaded ) ;
        			break;
				default:
    ...
    /**
     * Gets viewdefs from a DotbObjects template when the expected metadata file
     * is not found.
     *
    ...
    protected function getDefsFromTemplate($module, $client = 'base')
    {
        ...
        foreach ($clients as $client) {
            foreach ($types as $type) {
                ...
                $path = "include/DotbObjects/templates/$type/clients/$client/views/$file";
                $return['file'] = $path;
