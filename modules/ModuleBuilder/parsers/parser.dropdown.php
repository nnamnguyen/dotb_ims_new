...
class ParserDropDown extends ModuleBuilderParser
{
    ...
    public function saveDropDown($params, $finalize = true)
    {
        ...
        if (is_array($temp)) {
            foreach ($temp as $item) {
                $key = DotbCleaner::stripTags(from_html($item[0]), false);
                $dropdown[$key] = empty($key) ? '' : DotbCleaner::stripTags(from_html($item[1]), false);
            }
        }
    ...
    protected function getExtensionFilePath($dropdownName, $lang)
    {
        ...
        if (DotbAutoLoader::ensureDir($dirName)) {
            $fileName = "$dirName/$lang.dotb_$dropdownName.php";
