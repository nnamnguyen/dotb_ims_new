...
class AbstractRelationships
{
    ...
    */
    ...
    protected function getDeployedRelationships()
    {
        ...
        $relationships = DotbRelationshipFactory::getInstance()->getRelationshipDefs();
     ...
     * @param string $installDefPrefix  Pathname prefix for the installdefs, for example for ModuleBuilder use "<basepath>/DotbModules"
     ...
     * @param $installDefPrefix             Pathname prefix for the installdefs, for example for ModuleBuilder use "<basepath>/DotbModules"
    ...
    protected function saveLabels($basepath, $installDefPrefix, $relationshipName, $labelDefinitions)
    {
        ...
        foreach ($moduleLabels as $module => $labels) {
            ...
            dotb_file_put_contents($filename, $out);

            foreach ($dotb_config['languages'] as $lk => $lv) {
     ...
     * @return bool
     */
    protected function hasLumiaLayoutDefs($moduleName, $client = 'base')
    {
     ...
     */
    public function saveLumiaSubpanelDefinitions($basepath , $installDefPrefix , $relationshipName , $subpanelDefinitions, $client='base')
    {
        ...
        foreach ( $subpanelDefinitions as $moduleName => $definitions )
        {
            if(!$this->hasLumiaLayoutDefs($moduleName, $client)) {
                continue;
            }
            ...
            if (!is_dir($layoutPath)) {
                dotb_mkdir($layoutPath, null, true);
            }


            foreach ( $definitions as $definition ) {
                $override_array = array();
                // we currently do not support collections in lumia
            ...
            // with /DotbModules as a path. Otherwise, use the path that the defs
            // are being saved to in this request.
            $fromPath = $fileName;
            if (strpos($installDefPrefix, '/DotbModules') !== false) {
    ...
    protected function saveClientFiles ($basepath, $installDefPrefix, $relationshipName, $moduleClientFileList)
    {
        ...
        foreach ($moduleClientFileList as $moduleName => $clientFileList) {
            ...
            foreach ($clientFileList as $fileName => $contents) {
                ...
                DotbAutoLoader::ensureDir(dirname($from));
                dotb_file_put_contents($from, $contents);
            }
        }
    ...
    public function saveLumiaMobileSubpanelDefinitions($basepath , $installDefPrefix , $relationshipName , $subpanelDefinitions, $client='mobile')
    {
        return $this->saveLumiaSubpanelDefinitions($basepath , $installDefPrefix , $relationshipName , $subpanelDefinitions, $client);
    }
