...
class ParserModifyPortalConfig extends ModuleBuilderParser
{
    ...
    private function savePortalSettings(array $portalConfig)
    {
        ...
        foreach ($portalConfig as $fieldKey => $fieldValue) {
            if (is_string($fieldValue)) {
                $fieldValue = DotbCleaner::stripTags($fieldValue);
            }
    ...
    public function removeOAuthForPortalUser()
    {
        ...
        $portalUserName = "DotbCustomerSupportPortalUser";
    ...
    public function getPortalUser()
    {
        ...
        if (!$id) {
            ...
            $user->title = 'Dotb Customer Support Portal User';
    ...
    public function createOAuthForPortalUser()
    {
        ...
        if (!$clientBean) {
            ...
            $newKey->description = 'This OAuth key is automatically created by the OAuth2.0 system to enable logins to the serf-service portal system in Dotb.';
            $newKey->save();
        }
    }
