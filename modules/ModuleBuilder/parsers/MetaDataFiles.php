...
class MetaDataFiles
{
    ...
    public static $names = array(
        ...
        MB_LUMIAPOPUPVIEW       => 'selection-list',
        MB_LUMIADUPECHECKVIEW   => 'dupecheck-list',
        ...
        MB_LUMIAQUOTEDATAGROUPLIST => 'quote-data-group-list',
    ...
    /**
     * Listing of module name placeholders in DotbObject templates metadata
     *
     * @var array
     * @static
     */
    ...
    public static $excludedClientFilePaths = array(
        'include/DotbObjects/templates/basic/clients/base/views/' => array(
            'edit',
            'detail',
        ),
    );
    ...
    /**
     * Gets the listing of DotbObject template placeholders
     *
     * @static
     * @return array
     */
     ...
     * fallback allows for the use of the more standard naming for lumia stuff
    ...
    public static function getFile($view, $module, array $params = array())
    {
        ...
        if (!empty($params['client'])) {
            $file = new MetaDataFileLumia($file, $params['client']);
        } else {
    ...
    /**
     * Gets a metadata file path for a module from its DotbObject template type
     *
     * @static
     * @param  string $module    The name of the module to get metadata for
     * @param  string $deftype   The def type to get (list, detail, edit, search)
     ...
     * @return string
     */
    public static function getDotbObjectFileName($module, $deftype, $client = '', $component = self::COMPONENTVIEW)
    {
        $filename =  self::getDotbObjectFileDir($module, $client, $component) . '/' . $deftype . '.php';
    ...
    /**
     * Gets a metadata directory path for a module from its DotbObject template type
     *
     * @static
     * @param string $module The name of the module to get metadata for
     * @param string $client The client making this request
     ...
     * @return string
     */
    public static function getDotbObjectFileDir($module, $client = '', $component = self::COMPONENTVIEW, $seed = null)
    {
        $sm = new StudioModule($module);

        $dirname = 'include/DotbObjects/templates/' . $sm->getType();
        if (!empty($client)) {
     ...
     * @return string
     */
    public static function getDotbExtensionFileDir($module = '', $client = 'base', $component = self::COMPONENTVIEW)
    {
        if (empty($module)) {
    ...
    /**
     * Gets DotbObjects type metadata for a module and cleans the defs up by
     * replacing variables with correct values based on the module
     *
     * @static
     * @param DotbBean|string $module Either a been or a string name of a module
    ...
    public static function clearModuleClientCache($modules = array(), $type = '', $platforms = array())
    {
        ...
        foreach ($modules as $module) {
            ...
            $clientDirs = glob(dotb_cached('modules/'.$module.'/clients/'));
    ...
    public static function getModuleClientCache($platforms, $type, $module, MetaDataContextInterface $context = null)
    {
        ...
        $clientCache = array();
        $cacheFile = dotb_cached('modules/' . $module . '/clients/' . $platforms[0] . '/' . $type . '.php');
    ...
    public static function buildModuleClientCache(
        ...
        foreach ($modules as $module) {
            ...
            if ($noCache) {
                ...
                $basePath = dotb_cached('modules/'.$module.'/clients/'.$platforms[0]);
                if ($context != null && $context->getHash()) {
                    $basePath .= '/' . $context->getHash();
                }
                dotb_mkdir($basePath,null,true);
    
                $output = "<?php\n\$clientCache['".$module."']['".$platforms[0]."']['".$type."'] = ".var_export($moduleResults,true).";\n\n";
                dotb_file_put_contents_atomic($basePath.'/'.$type.'.php', $output);
            }
        }
    }
    ...
    public static function getClientFiles( $platforms, $type, $module = null, MetaDataContextInterface $context = null, $bean = null)
    {
        ...
        if ($module == '') {
            foreach ($platforms as $platform) {
                ...
                $checkPaths[self::getDotbExtensionFileDir('', $platform, $type)] = array('platform' => $platform, 'template' => false);
            }
        } else {
            foreach ($platforms as $platform) {
                // These are sorted in order of priority.
                ...
                $baseTemplateDir = 'include/DotbObjects/templates/basic/clients/'.$platform.'/'.$type.'s';
                $nonBaseTemplateDir = self::getDotbObjectFileDir($module, $platform, $type, $bean);
                ...
                $checkPaths[self::getDotbExtensionFileDir($module, $platform, $type)] = array('platform' => $platform, 'template' => false);
            }
        }
    ...
    public static function getClientFileList($checkPaths, MetaDataContextInterface $context = null)
    {
        ...
        foreach ($checkPaths as $path => $pathInfo) {
            ...
            $dirsInPath = DotbAutoLoader::getDirFiles($path, true, null, true);
            $fileList = array();
            ...
            foreach ($dirsInPath as $fullSubPath) {
                ...
                $filesInDir = DotbAutoLoader::getDirFiles($fullSubPath, false, null, true);
                ...
                case 'php':
                    ...
                    if ($fileInfo['template']) {
                        ...
                        if($fileInfo['subPath'] != 'subpanels') {
                            $extensionName = "lumia{$type}{$fileInfo['platform']}{$fileInfo['subPath']}";
                            $extFile = DotbAutoLoader::loadExtension($extensionName, $module);
                            if ($extFile) {
    ...
    public static function loadSingleClientMetadata($type, $name, $platform = 'base')
    {
        ...
        foreach ($fileList as $file => $platform) {
            if (DotbAutoLoader::existing($file)) {
                require $file;
    ...
    /**
     * Used to remove the dotbcrm license header from component files that are going to be rolled into a JSON response
     * @param string $text
     *
     * @return string
     */
