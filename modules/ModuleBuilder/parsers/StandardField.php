...
class StandardField extends DynamicField
{
    ...
    protected function loadCustomDef($field){
    	global $beanList;
    	if (!empty($beanList[$this->module]) && is_file("custom/Extension/modules/{$this->module}/Ext/Vardefs/dotbfield_$field.php"))
    	{
            $bean_name = get_valid_bean_name($this->module);
            $dictionary = array($bean_name => array("fields" => array($field => array())));
            include FileLoader::validateFilePath("$this->base_path/dotbfield_$field.php");
