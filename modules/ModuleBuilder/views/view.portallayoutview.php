...
class ViewPortalLayoutView extends ViewLayoutView
{
    ...
    public function display($preview = false)
	{
		...
		$mod_strings = dotbArrayMerge($editModStrings, $mod_strings);
		...
		foreach($images as $image=>$file) {
			$smarty->assign($image,DotbThemeRegistry::current()->getImage($file, ''
,null,null,'.gif',$file));
		}
		$smarty->assign('icon_delete',DotbThemeRegistry::current()->getImage('icon_Delete','',48,48,'.gif',$mod_strings['LBL_MB_DELETE'] ));

		$buttons = array();
		$buttons[] = array(
		  'id'=>'saveBtn',
		  'image'=>DotbThemeRegistry::current()->getImage($images['icon_save'],'',null,null,'.gif',$mod_strings['LBL_BTN_SAVE']),
		...
		$buttons[] = array(
		  'id'=>'publishBtn',
		  'image'=>DotbThemeRegistry::current()->getImage($images['icon_publish'],'',null,null,'.gif',$mod_strings['LBL_BTN_PUBLISH']),
		...
		$smarty->assign('language',$this->parser->getLanguage());	// for dotb_translate in the smarty template
