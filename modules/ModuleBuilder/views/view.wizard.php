...
class ModuleBuilderViewWizard extends DotbView
{
    ...
    public function display()
    {
        ...
        if (isset ($_REQUEST ['MB'])) {
            ...
            if (isset ($_REQUEST ['portal'])) {
                $this->processDotbPortal($this->ajax);
            } else {
    ...
    public function processStudio($ajax)
    {
...
//                $this->actions = "<input class=\"button\" type=\"button\" id=\"exportBtn\" name=\"exportBtn\" onclick=\"ModuleBuilder.getContent('module=ModuleBuilder&action=exportcustomizations');\" value=\"" . translate('LBL_BTN_EXPORT') . '">';
    ...
    )
    {
        if (!isset ($_REQUEST ['view_package'])) {
            dotb_die("no ModuleBuilder package set");
        }
    ...
    public function processDotbPortal(
        $ajax
    )
    {
        $this->ajax->addCrumb(translate('LBL_DOTBPORTAL'), 'ModuleBuilder.main("dotbportal")');

        if (isset ($this->editModule)) {
            $module = StudioModuleFactory::getStudioModule($this->editModule);
            $this->generateDotbPortalViewButtons();
            $this->title = $module->name;
            $this->question = translate('LBL_QUESTION_DOTB_PORTAL');
            ...
            // DotbPortal Layouts Page
            $this->generateDotbPortalModuleButtons();
            $this->question = translate('LBL_QUESTION_MODULE1');
            $this->title = translate('LBL_DOTBPORTAL');
            $this->help = 'portalLayoutHelp';
        } else {
            //Main DotbPortal Page
            $this->generateDotbPortalMainButtons();
    ...
    public function generateStudioModuleButtons()
    {
        ...
        $nodes = $sb->getNodes();
        $this->buttons = array();
        $icons = json_decode(file_get_contents('custom/include/module_icon.json'), true);
        ...
        foreach ($nodes as $module) {
            $this->buttons [$module['module']] = array(
                ...
                'label' => $module['name'],
                'icon' => $icons[$module['module']]['src']
            );
        }
    }

    public function generateDotbPortalViewButtons()
    {
        $module = new DotbPortalModule ($this->editModule);
    ...
    public function generateDotbPortalModuleButtons()
    {
        $sb = new DotbPortalBrowser ();
        $nodes = $sb->getNodes();
        $this->buttons = array();
    ...
    public function generateDotbPortalMainButtons()
    {
        $sb = new DotbPortalBrowser ();
        $nodes = $sb->getNodes();
