...
class ViewLayoutView extends DotbView
{
    ...
    function display ($preview = false)
    {
        ...
        {
            $smarty->assign ( $image, DotbThemeRegistry::current()->getImage($file,'',null,null,'.gif',$file) ) ;
        }
		...
		$smarty->assign ( 'language', $parser->getLanguage() ) ; // for dotb_translate in the smarty template
        ...
        $ajax->addSection(
            'center',
            $translatedViewType,
            $smarty->fetch(DotbAutoLoader::existingCustomOne('modules/ModuleBuilder/tpls/layoutView.tpl'))
        );
        if ($preview) {
    ...
    protected function getButtons($history, $disableLayout, $params)
    {
        ...
        } elseif (!empty($GLOBALS['dotb_config']['roleBasedViews'])
