...
class ViewModulefield extends DotbView
{
    /**
     * @var FieldViewer
     */
		...
		*/
        ...
        if(empty($_REQUEST['view_package']) || $_REQUEST['view_package'] == 'studio') {
            ...
            if (empty($vardef['name']) || $isNew) {
                ...
                $action = 'saveDotbField'; // tyoung - for OOB fields we currently only support modifying the label
            ...
            foreach($module->field_defs as $field => $def)
            {
                ...
                if (!empty($def['type']) && ($def['type'] == "enum" || $def['type'] == "parent_type") && $field != $vardef['name'])
                {
    ...
    /**
     * fetchTemplate
     * This function overrides fetchTemplate from DotbView.  For view.modulefield.php we go through the FieldViewer
     * class to fetch the display contents.
     *
     * @param string $template the file to fetch
     * @return string contents from calling the fetch method on the FieldViewer Dotb_Smarty instance
     */
