...
define('MB_TEMPLATES', 'include/DotbObjects/templates');
define('MB_IMPLEMENTS', 'include/DotbObjects/implements');
...
class MBModule
{
    ...
    /**
     * The metadata converter object, used to keep the peace between BWC and Lumia
     * formatted metadata
     * 
     ...
     * Bug 56675
     *
     * Copies the clients directory from the dotb object this module is based on.
     ...
     *
     * Adapted for Dotb7 to include base clients as well.
     */
     ...
     * - <MODULE_NAME>_dotb.php Bean controller that shouldn't be customized
    ...
    function createClasses($path)
    {
        ...
        //Generate <MODULE_NAME>_dotb.php
        $fp = dotb_fopen($path . '/' . $class['name'] . '_dotb.php', 'w');
        fwrite($fp, $smarty->fetch('modules/ModuleBuilder/tpls/MBModule/Class.tpl'));
        fclose($fp);
        //Generate vardefs.php
        $fp = dotb_fopen($path . '/vardefs.php', 'w');
        ...
        if (!file_exists($path . '/' . $class['name'] . '.php')) {
            $fp = dotb_fopen($path . '/' . $class['name'] . '.php', 'w');
        ...
        if (!empty($this->config['studio'])) {
            $fp = dotb_fopen($path . '/metadata/studio.php', 'w');
    ...
    {
        $name = $this->key_name ;
        $installDefs [ 'copy' ] [] = array ( 'from' => '<basepath>/DotbModules/modules/' . $name , 'to' => 'modules/' . $name ) ;
    ...
    public function populateFromPost(Request $request = null)
    {
        ...
        if ($label) {
            ...
            $label = DotbCleaner::cleanHtml($label, false);
    ...
    function createIcon ()
    {
        ...
        copy ( "include/DotbObjects/templates/$template/icons/$template.gif", "$icondir/" . $this->key_name . ".gif" ) ;
        if (file_exists("include/DotbObjects/templates/$template/icons/Create$template.gif"))
        	copy ( "include/DotbObjects/templates/$template/icons/Create$template.gif", "$icondir/Create" . $this->key_name . ".gif" ) ;
        if (file_exists("include/DotbObjects/templates/$template/icons/{$template}_bar_32.png"))
        	copy ( "include/DotbObjects/templates/$template/icons/{$template}_bar_32.png", "$icondir/icon_{$this->key_name}_bar_32.png" ) ;
    }
    ...
    /**
     * Gets the dotb object template type for this module
     *
     * @return string
     */
