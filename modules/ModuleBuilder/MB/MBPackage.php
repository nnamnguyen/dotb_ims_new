...
use Dotbcrm\Dotbcrm\Security\InputValidation\Exception\ViolationException;
...
function getManifest($version_specific = false, $for_export = false){
    ...
    if (isset($dotb_flavor)) {
        if (self::$compatibilityMap[$dotb_flavor]) {
            $flavors = self::$compatibilityMap[$dotb_flavor];
        } else {
            $flavors = array($dotb_flavor);
        }
    } else {
        $flavors = array();
    }

    $version = (!empty($version_specific))?$GLOBALS['dotb_version']:'';

    // Build an array and use var_export to build this file
    $manifest = array(
        'built_in_version' => $GLOBALS['dotb_version'],
        'acceptable_dotb_versions' => array($version),
        'acceptable_dotb_flavors' => $flavors,
        'readme' => $this->readme,
        'key' => $this->key,
        'author' => $this->author,
...
/*
    ...
    $header
    \$manifest = array (
         'acceptable_dotb_versions' =>
          array (
            $version
          ),
          'acceptable_dotb_flavors' =>
          array(
            $flavor
          ),
          'readme'=>'$this->readme',
...
function buildInstall($path){
            ...
            if(substr($e, 0, 1) != '.' && is_dir($lang_path)){
                ...
                while($g = $f->read()){
                    if(substr($g, 0, 1) != '.' && is_file($lang_path.'/'. $g)){
                        ...
                        'from'=> '<basepath>/DotbModules/language/'.$e . '/'. $g,
                        'to_module'=> $e,
                        'language'=> $lang
                        );
                    }
    ...
    function save(){
        ...
        if(mkdir_recursive($path)){
            ...
            dotb_file_put_contents_atomic($path .'/manifest.php', $this->getManifest());
        }
    }
    ...
    function populateFromPost(){
        ...
        if (count($violations) > 0) {
            ...
            $softFail = $dotbConfig->get('validation.soft_fail', true);
            if (!$softFail) {
    ...
    function customBuildInstall($modules, $path, $extensions = array()){
        ...
        foreach (self::$appExtensions as $type => $spec) {
            $include_path = $path . '/DotbModules/' . $spec['dir'];
            $it = $this->getDirectoryIterator($include_path);
            foreach ($it as $file) {
                $subPathName = $it->getSubPathname();
                $def = array(
                    'from'=> '<basepath>/DotbModules/' . $spec['dir'] . '/' . $subPathName,
    ...
    private function getLanguageManifestForModule($module, &$installdefs)
    {
        ...
        foreach(scandir($lang_path) as $langFile)
        {
	        if(substr($langFile, 0, 1) != '.' && is_file($lang_path . DIRECTORY_SEPARATOR . $langFile)){
	            ...
	            $installdefs['language'][] = array(
	                'from'=> '<basepath>/DotbModules/modules/' . $module . '/language/'. $langFile,
	                'to_module'=> $module,
	                'language'=>$lang
	            );
	        }
    ...
    public function getCustomClientMetadata($module, &$installdefs)
    {
        ...
        foreach ($it as $file) {
            $installdefs['copy'][] = array(
                'from' => str_replace('custom/modules', '<basepath>/DotbModules/modules', $file),
                'to' => $file->getPathname(),
            );
        }
    }
    	...
    	foreach(scandir($meta_path) as $meta_file)
    	{
    		if(substr($meta_file, 0, 1) != '.' && is_file($meta_path . '/' . $meta_file)){
    			if($meta_file == 'listviewdefs.php'){
    				$installdefs['copy'][] = array(
                                'from'=> '<basepath>/DotbModules/modules/'. $module . '/metadata/'. $meta_file,
    ...
    function exportCustom($modules, $export=true, $clean = true){
        ...
        foreach ($modules as $module) {
            $pathmod = "$path/DotbModules/modules/$module";
            ...
            foreach ($extensions as $file) {
                ...
                dotb_mkdir($path . $trimmedPath, NULL, true);
        ...
        if (!is_dir($path)) {
            dotb_die('Build directory has not been created');
        }

        $manifest = $this->getManifest(true) . $this->customBuildInstall($modules, $path);
        dotb_file_put_contents($path .'/manifest.php', $manifest);
    ...
    private function convertLangFilesToExtensions($langDir)
    {
        if (is_dir($langDir))
        {
            foreach(scandir($langDir) as $langFile)
            {
                ...
                $out .= "\n?>\n";
                dotb_file_put_contents("$langDir/$langFile", $out);
            }
        }
    }
        ...
        foreach (self::$appExtensions as $spec) {
            ...
            foreach ($it as $file) {
                ...
                if (count($values) > 0) {
                    ...
                    $destination = $path . '/DotbModules/' . $spec['dir'] . '/' . $subPathName;
                    mkdir_recursive(dirname($destination));
                    dotb_file_put_contents($destination, $contents);
                }
            }
        }
    }
    ...
    public function getCustomModules()
    {
        ...
        foreach ($modules as $module) {
            ...
            if (in_array($module, $modulesWithCustomDropdowns) &&
                DotbAutoLoader::existingCustomOne("modules/{$module}/metadata/studio.php")) {
    ...
    protected function getModuleCustomizations($module)
    {
        ...
        $result = array();
        if (!DotbAutoLoader::existingCustomOne("modules/{$module}/metadata/studio.php")) {
            return $result;
        }
    ...
    public function exportProject()
    {
        ...
        if(file_exists($this->getPackageDir())){
            if(mkdir_recursive($tmppath)){
                ...
                $fp = dotb_fopen($tmppath .'/manifest.php', 'w');
                fwrite($fp, $manifest);
                fclose($fp);
                if(file_exists('modules/ModuleBuilder/MB/LICENSE.txt')){
                ...
                $readmefp = dotb_fopen($tmppath.'/README.txt','w');
