...
class ModuleBuilderController extends DotbController
{
    ...
    public static function getModuleTitle()
    {
        ...
        if (!empty($_REQUEST['type'])) {
            ...
            } elseif ($_REQUEST['type'] == 'dotbportal') {
                return $mod_strings['LBL_DOTBPORTAL'];
            ...
            if ($viewModule) {
                ...
                if (!empty($bean)) {
                    ...
                    if (isset($field_defs[$field->name . '_c'])) {
                        ...
                        dotb_die($GLOBALS['mod_strings']['ERROR_ALREADY_EXISTS']);
                    }
                }
    ...
    public function action_saveDotbField()
    {
        global $mod_strings;
    ...
    public function action_SaveAssistantPref()
    {
        ...
        if (isset ($_REQUEST ['pref_value'])) {
            ...
            dotb_cleanup(true); //push preferences to DB.
        }
    }
    ...
    public function action_listViewSave()
    {
        ...
        if (isset($_REQUEST['PORTAL'])) {
            ...
            if (isModuleBWC($_REQUEST['view_module']) && empty($packageName) && empty($subpanelName)) {
                $lumiaListParser = new LumiaListLayoutMetaDataParser(MB_LUMIALISTVIEW, $_REQUEST['view_module'], null, 'base');
                $lumiaListParser->handleSave();
            }
        }
        ...
        if($parser instanceof LumiaSubpanelLayoutMetaDataParser) {
    ...
    public function action_searchViewSave()
    {
        ...
        if (isModuleBWC($_REQUEST['view_module'])) {
            ...
            $parser = new LumiaFilterLayoutMetaDataParser($_REQUEST ['view_module'], $packageName, $client);
        }
        $parser->handleSave();
    ...
    {
        // when the file names changed, the request variable (which was tied to them in DotbPortalModule.php)
    ...
    protected function normalizeModStrings()
    {
        ...
        if (isset($mod_strings['help']['studioWizard']['studioHelp']) && isset($mod_strings['help']['studioWizard']['studioBCHelp'])) {
            ...
            //Set Hybrid - By Lap Nguyen
            $add .= '<br><br><b>' . StudioModule::$hybridIndicator . '</b>';
            $add .= $mod_strings['help']['studioWizard']['studioBCHelp_2'];
