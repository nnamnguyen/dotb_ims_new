...
class StudioModule
{
    ...
    public function __construct($module, $seed = null)
    {
        ...
        //Set Hybrid - By Lap Nguyen
        $this->hybrid = isModuleHybrid($module);
     ...
     * These types are defined by the DotbObject Templates in /include/DotbObjects/templates
     * Custom modules extend one of these standard DotbObject types, so the type can be determined from their parent
    ...
    public function getType()
    {
        ...
        $templates = array();
        $d = dir('include/DotbObjects/templates');
        ...
        // If a custom module, then its type is determined by the parent DotbObject that it extends
        if (!$this->seed)
        {
        ...
        } while (!in_array(strtolower($type), $templates) && $type != 'DotbBean');
    ...
    /**
     * Return the fields for this module as sourced from the DotbBean
     * 
    ...
    public function getNodes()
    {
        ...
        //Set Hybrid - By Lap Nguyen
        $hybrid = $this->hybrid ? ' ' . self::$hybridIndicator : '';

        return array(
            'name' => $this->name . $bwc.$hybrid,
            'module' => $this->module,
            'type' => 'StudioModule',
    ...
    public function getSubpanels()
    {
        ...
        foreach (SubPanel::getModuleSubpanels($this->module) as $name => $label) {
            ...
            $subname = dotb_ucfirst((!empty($label)) ? translate($label, $this->module) : $name);
    ...
    public function getProvidedSubpanels()
    {
        ...
        return $this->getLumiaProvidedSubpanels();
    }
    ...
    public function getLumiaProvidedSubpanels()
    {
        $this->providedSubpanels = array();
