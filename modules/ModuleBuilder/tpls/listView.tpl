...
<tr>
    <td colspan=3>
    <span class='mbLBL'>{dotb_translate label='LBL_SUBPANEL_TITLE'}</span>
...
<li name="width={$value.width}%" id='subslot{$modCounter}' class='draggable' data-name="{$key}">
    <table width='100%'>
        <tr>
            ...
            {else}
            {if !empty($value.label)}{dotb_translate label=$value.label module=$language}{elseif !empty($value.vname)}{dotb_translate label=$value.vname module=$language}{else}{$key}{/if}
            {/if}
            </td>
            <td></td>
            <td align="right" class="editIcon">
                ...
                <img src="{dotb_getimagepath file='edit_inline.gif'}" style="cursor: pointer;"
