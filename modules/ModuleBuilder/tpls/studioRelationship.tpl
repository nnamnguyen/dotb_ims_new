	...
	<tr><td colspan=2>
		<table class="edit view">
			...
			<tr>
				...
				{dotb_translate label='LBL_MODULE'}:
				</td>
				<td>
					{capture name="module_label"}{dotb_translate label=$module_key}{/capture}
			...
			<tr>
                {if $rel.relationship_type == 'many-to-many' || $rel.relationship_type == 'many-to-one'}
                    <td align="right" scope="row">{dotb_translate label="LBL_REL_LABEL"}:</td>
            ...
            <tr>
                {if $rel.relationship_type == 'many-to-many' || $rel.relationship_type == 'many-to-one'}
                {capture name="lhs_module"}{dotb_translate label=$rel.lhs_module}{/capture}
                ...
                <td></td>
                {if $rel.relationship_type != 'many-to-one'}
                {capture name="rhs_module"}{dotb_translate label=$rel.rhs_module}{/capture}
...
ModuleBuilder.deleteRel = function()
{
	...
	YAHOO.DOTB.MessageBox.show(
	{
		...
		width: 300,
	    {/literal}
	    msg:'<b>{dotb_translate label="LBL_CONFIRM_RELATIONSHIP_DELETE"}</b>' + 
	       "<div style='height:1em;'>&nbsp;</div><p><input type='checkbox' onclick='YAHOO.util.Dom.get(\"rel_remove_tables\").value = this.checked ? \"\" : true;' />" +
		   "&nbsp;{dotb_translate label="ML_LBL_DO_NOT_REMOVE_TABLES" module="Administration"}</p>",
	    {literal}
	    fn: function(confirm) {
		    if (confirm == 'yes') {
