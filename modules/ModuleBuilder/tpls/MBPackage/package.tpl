...
<table class='mbTable' >
	...
	<tr>	
		<td></td><td id='readme'>{dotb_getimage name="advanced_search" ext=".gif" alt=$mod_strings.LBL_HIDEOPTIONS other_attributes='border="0" id="options" '}<a style="text-decoration:none" href="javascript: void(0)" onclick ="toggleEl('readmefield');"><b>{$mod_strings.LBL_ADD_README}</b></a></td>
	</tr>
	<tr>
		<td height='100%'>&nbsp;</td><td>&nbsp;</td>
	</tr>
			...
			<table>
				<tr id="package_modules">
					...
					<td align='center'>
						...
						<tr>
						<td  align='center'>{dotb_image name="NewModule" width=32 height=32}</td>
						</tr>
						<tr><td>
					...
					{assign var='imgurl' value=$type|cat:'_32'}
						...
						<table id= "existing_module" onclick="ModuleBuilder.viewModule('{$package->name}', '{$name}')" class='wizardButton' onmousedown="ModuleBuilder.buttonDown(this);return false;" onmouseout="ModuleBuilder.buttonOut(this);">
						  <tr>
						      <td  align='center'>{dotb_image name=$imgurl width=32 height=32}</td>
...
function toggleEl(){
		...
		if(display=='table-row' || display=='inline-block'){
			document.getElementById('options').src = "{/literal}{dotb_getimagepath file='advanced_search.gif'}{literal}";
			document.getElementById('readmefield').style.display ='none';
		}else if (display=='none'){
			document.getElementById('options').src = "{/literal}{dotb_getimagepath file='basic_search.gif'}{literal}";
