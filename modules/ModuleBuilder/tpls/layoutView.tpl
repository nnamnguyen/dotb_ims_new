...
{if $disable_layout}
<span class='required'>
{dotb_translate label="LBL_SYNC_TO_DETAILVIEW_NOTICE" module="ModuleBuilder"}
</span>
{/if}
<table id='layoutEditorButtons' cellspacing='2'>
	...
	<td><input type="checkbox" {if $syncDetailEditViews}checked="true"{/if} id="syncCheckbox" onclick="document.forms.prepareForSave.sync_detail_and_edit.value=this.checked">
	   {dotb_translate label="LBL_SYNC_TO_DETAILVIEW" module="ModuleBuilder"}&nbsp;{dotb_help text=$mod.LBL_SYNC_TO_DETAILVIEW_HELP}
	</input></td>
	{/if}
    </tr>
</table>
...
<div id='toolbox' style='float:left; overflow-y:auto; overflow-x:hidden';>
    ...
    <div id='delete'>
    {dotb_image name=Delete width=48 height=48}
    </div>
    ...
    {foreach from=$available_fields item='col' key='id'}
        ...
        <div class='le_field' data-name="{$field}" id='{$idCount}'>
            {if ! $fromModuleBuilder && ($col.name != '(filler)')}
                ...
                {dotb_getimage name="edit_inline" ext=".gif" other_attributes=$otherAttributes}
            {/if}
            ...
            {if isset($field_defs.$field.calculated) && $field_defs.$field.calculated}
                {dotb_getimage name="DotbLogic/icon_calculated" alt=$mod_strings.LBL_CALCULATED ext=".png" other_attributes='class="right_icon" '}
            {/if}
            {if isset($field_defs.$field.dependency) && $field_defs.$field.dependency}
                {dotb_getimage name="DotbLogic/icon_dependent" ext=".png" alt=$mod_strings.LBL_DEPENDANT other_attributes='class="right_icon" '}
            {/if}
            ...
            {if !empty($translate) && !empty($col.label)}
                ...
                {if $from_mb}
                    ...
                    {/if}
                {else}
                    {dotb_translate label=$newLabel module=$language}
                {/if}
 			{else}
            ...
            <span class='field_fieldset_fields' id='fieldset_{$idCount}'>
                {foreach from=$col.fieldset_fields item='fsfield'}
                    ...
                    {if !empty($translate) && !empty($fsfield.label)}
                        {dotb_translate label=$fslabel module=$language}
                    {else}
...
{foreach from=$layout item='panel' key='panelid'}
    ...
    <div class='le_panel' id='{$idCount}'>
        ...
        <div class='panel_label' id='le_panellabel_{$idCount}'>
          <span class='panel_name' id='le_panelname_{$idCount}'>
			...
			{elseif !empty($translate)}
			    {dotb_translate label=$panelid|upper module=$language}
			{else}
			    {$panelid}
			{/if}</span>
        ...
        &nbsp;&nbsp;{dotb_translate label="LBL_TABDEF_TYPE" module="ModuleBuilder"}&nbsp;{dotb_help text=$mod.LBL_TABDEF_TYPE_OPTION_HELP}:
        {if $idCounter == 1}
        ...
        <select id="le_paneltype_select_{$idCount}" onchange="document.forms.prepareForSave.tabDefs_{$panelid}_newTab.value=this.value; showHideBox(this.value, {$idCount}, '{$panelid}', '{$firstpanelid}', {$firstpanelidcount});"
                title="{dotb_translate label="LBL_TABDEF_TYPE_HELP" module="ModuleBuilder"}">
          <option value="0" {if $tabDefs[$panel_upper].newTab == false}selected="selected"{/if}>{dotb_translate label="LBL_TABDEF_TYPE_OPTION_PANEL" module="ModuleBuilder"}</option>
          {if $disable_tabs != true}
          <option value="1" {if $tabDefs[$panel_upper].newTab == true}selected="selected"{/if}>{dotb_translate label="LBL_TABDEF_TYPE_OPTION_TAB" module="ModuleBuilder"}</option>
          {/if}
        </select>
        </span>
        ...
        &nbsp;{dotb_translate label="LBL_TABDEF_COLLAPSE" module="ModuleBuilder"}{dotb_translate label="LBL_QUESTION_MARK"}
        <input type="checkbox" title="{dotb_translate label="LBL_TABDEF_COLLAPSE_HELP" module="ModuleBuilder"}" {if $tabDefs[$panel_upper].panelDefault == "collapsed"}checked="checked"{/if}
        ...
        {foreach from=$panel item='row' key='rid'}
            ...
            {foreach from=$row item='col' key='cid'}
                ...
                <div class='le_field' data-name="{$col.name}" id='{$idCount}'>
                    ...
                    {if !empty($translate) && !empty($col.label)}
                        {dotb_translate label=$label module=$language}
                    {else}
		                {if !empty($current_mod_strings[$label])}
		                    {$current_mod_strings[$label]}
...
<!-- BEGIN DOTBCRM flav=ent ONLY -->
{if $fromPortal}
    <input type='hidden' name='PORTAL' value='1'>
{/if}
<!-- END DOTBCRM flav=ent ONLY -->
...
var editPanelProperties = function (panelId, view) {
	...
	var params = "module=ModuleBuilder&action=editProperty&view_module=" + encodeURIComponent(ModuleBuilder.module) 
                ...
                + "&title_label=" + encodeURIComponent(DOTB.language.get("ModuleBuilder", "LBL_LABEL_TITLE")) + "&value_label=" + encodeURIComponent(value_label);
...
var editFieldProperties = function (idCount, label) {ldelim}
	...
	var title_label = '{dotb_translate label="LBL_LABEL_TITLE" module="ModuleBuilder"}';
	var title_tabindex = '{dotb_translate label="LBL_TAB_ORDER" module="ModuleBuilder"}';
...
<div id="copy-from-contents" style="display: none">
    <label for="copy-from-options">{dotb_translate label="LBL_COPY_FROM" module="ModuleBuilder"}:&nbsp;</label>
