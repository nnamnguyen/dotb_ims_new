...
{/literal}
<div id='depGrid'></div>
{if $empty}{dotb_translate label='LBL_NO_RELS' module='ModuleBuilder'}</h3>{/if}
...
<table cellspacing="0" cellpadding="0" border="0" width="100%" class="list view">
	<tr class="list header">
		...
		<th width="25%">{dotb_translate label="LBL_REL_NAME"}</th>
		<th width="55%">{dotb_translate label="LBL_TRIGGER"}</th>
		<th width="25%">{dotb_translate label="LBL_ACTIONS"}</th><th>&nbsp;</th><th>&nbsp;</th>
	</tr>
	{foreach from=$dependencies item='dep' key='depName'}
		...
		<tr class="{$_rowColor}ListRowS1">
			...
			<td width="10px"><img border="0" alt=$mod_strings.LBL_DELETE src="themes/Dotb/images/delete_inline.gif"/></td>
		</tr>
		<tr style="display:none" id="{$dep.name}actions">
    		...
    		<table cellspacing="0" class="action view list" border="0" cellpadding="0"/>
    			...
    			<td width="10px"><img border="0" alt=$mod_strings.LBL_EDIT src="themes/default/images/edit_inline.gif"/></td>
    			<td width="10px"><img border="0" alt=$mod_strings.LBL_DELETE src="themes/Dotb/images/delete_inline.gif"/></td></tr>
    			{/foreach}
...
{dotb_translate label="LBL_REL_NAME"}:<input id="newDepName" name="newDepName"/>
{dotb_translate label="LBL_TRIGGER"}:
...
'<table cellspacing="0" cellpadding="0" border="0" width="100%" class="list view">',
	'<tr class="list header">',
		'<th>{dotb_translate label="LBL_REL_NAME"}</th>',
		'<th>{dotb_translate label="LBL_CONDITION"}</th>',
		'<th>{dotb_translate label="LBL_ACTIONS"}</th><th/><th/>',
...
var grid = new Ext.grid.GridPanel({
    store: depstore,
     columns: [
        {id:'name', header: DOTB.language.get('ModuleBuilder','LBL_REL_NAME'), width: 120, sortable: true, dataIndex: 'name'},
        {header: DOTB.language.get('ModuleBuilder','LBL_CONDITION'), width: 200, sortable: false, dataIndex: 'condition'},
        {header: DOTB.language.get('ModuleBuilder','LBL_ACTIONS'), width: 200, sortable: false, dataIndex: 'actions', renderer:function(a){
