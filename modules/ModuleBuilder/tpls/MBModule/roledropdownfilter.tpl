...
<div>
    <link rel="stylesheet" type="text/css"
          href="{dotb_getjspath file='modules/ModuleBuilder/tpls/ListEditor.css'}"></link>
    <link rel="stylesheet" type="text/css"
          href="{dotb_getjspath file='modules/ModuleBuilder/tpls/MBModule/dropdown.css'}"></link>
...
{dotb_csrf_form_token}
        ...
        <table>
            <tr>
                <td colspan='2'>
                    <input id="saveBtn" type='button' class='button'
                           value='{dotb_translate label='LBL_SAVE_BUTTON_LABEL'}'>
                    <input type='button' class='button' onclick='SimpleList.undo()'
                           value='{dotb_translate label='LBL_BTN_UNDO'}'>
                    <input type='button' class='button' onclick='SimpleList.redo()'
                           value='{dotb_translate label='LBL_BTN_REDO'}'>
                    <input type='button' class='button' name='cancel' value='{dotb_translate label='LBL_BTN_CANCEL'}'
                           onclick='ModuleBuilder.tabPanel.get("activeTab").close()'>
                </td>
                <td style="text-align: right">
                    <label>{dotb_translate label='LBL_ROLE'}
            ...
            <tr>
                <td colspan="3">
                    <span class='mbLBLL'>{dotb_translate label='LBL_DROPDOWN_TITLE_NAME'}:&nbsp;</span>
                    {if $name }
            ...
            <tr>
                <td colspan="3" class='mbLBLL'>
                    {dotb_translate label='LBL_DROPDOWN_LANGUAGE'}:&nbsp;
                    {html_options name='dropdown_lang' options=$available_languages selected=$selected_lang}
                </td>
            </tr>
            <tr>
                <td colspan="3" style='padding-top:10px;' class='mbLBLL'>{dotb_translate label='LBL_DROPDOWN_ITEMS'}:
                </td>
            </tr>
            <tr>
                <td colspan="2"><b>{dotb_translate label='LBL_DROPDOWN_ITEM_NAME'}</b><span
                            class='fieldValue'>[{dotb_translate label='LBL_DROPDOWN_ITEM_LABEL'}]<span>
                </td>
                <td style="text-align: right">
                    <input type='button' class='button' value='{dotb_translate label='LBL_BTN_SORT_ASCENDING'}'
                           onclick='SimpleList.sortAscending()'>
                    <input type='button' class='button' value='{dotb_translate label='LBL_BTN_SORT_DESCENDING'}'
                           onclick='SimpleList.sortDescending()'>
                    <input type="button" class="button" id="select-all" value="{dotb_translate label="LBL_LISTVIEW_OPTION_ENTIRE"}">
                    <input type="button" class="button" id="select-none" value="{dotb_translate label="LBL_LISTVIEW_NONE"}">
                </td>
            </tr>
            <tr>
                <td colspan='3'>
                    <ul id="ul1" class="listContainer">
                        {foreach from=$role_options key='id' item='checked'}
                            {if isset($options[$id])}
                                {if ($options[$id] === "")}
                                    {capture assign=id}{dotb_translate label='LBL_BLANK'}{/capture}
...
(function(){
        $("#saveBtn").on("click", function(e) {
            ...
            if ($("#ul1").children().not(".deleted").length == 0) {
                YAHOO.DOTB.MessageBox.show({
                    msg: DOTB.language.get('ModuleBuilder', 'LBL_DROPDOWN_LIST_EMPTY'),
                    width: 300,
                    type: "alert"
                });
                return false;
        ...
        addToValidate('dropdown_form', 'dropdown_name', 'DBName', false, DOTB.language.get("ModuleBuilder", "LBL_JS_VALIDATE_NAME"));
        addToValidate('dropdown_form', 'drop_value', 'varchar', false, DOTB.language.get("ModuleBuilder", "LBL_JS_VALIDATE_LABEL"));
