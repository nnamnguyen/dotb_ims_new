...
<script type="text/javascript" src="{dotb_getjspath file='cache/javascript/dotbcrm2.min.js'}"></script>
...
<div class="left_list">
    <div class="dd_title">
        <div id="ddd_delete">
            <div>{dotb_image name=Delete width=48 height=48 id="ddd_delete"}</div>
        </div><br/>
        {dotb_translate label="LBL_AVAILABLE_OPTIONS" module="ModuleBuilder"}
    </div>
...
<div style="max-height: 510px; overflow-y: auto; overflow-x: hidden">
    <div class="dd_help" style="width:600px">
        {dotb_translate label="LBL_DEPENDENT_DROPDOWN_HELP" module="ModuleBuilder"}
    </div>
<table ><tr>
...
<div style="position: absolute;right: 10px;bottom: 10px;">
    <button onclick="ModuleBuilder.visGridWindow.hide();">
        {dotb_translate label="LBL_BTN_CANCEL" module="ModuleBuilder"}
    </button>
    <button onclick="$('#visibility_grid').val($.toJSON(DOTB.ddd.getMapping()));ModuleBuilder.visGridWindow.hide();">
    {dotb_translate label="LBL_BTN_SAVE" module="ModuleBuilder"}
    </button>
</div>
{literal}
<script type="text/javascript">
DOTB.ddd = {};
DOTB.util.doWhen("typeof($) != 'undefined'", function()
{
    ...
    $.widget("ui.dotbdddlist", $.extend({}, $.ui.sortable.prototype, {
    ...
    DOTB.ddd.childTable =  $( "#childTable" ).dotbdddlist({
        ...
        remove: function(event, ui) {
            ...
            if (DOTB.ddd.oldPos[0])
                DOTB.ddd.oldPos.after(ui.item.clone());
            else {
                DOTB.ddd.childTable.children().first().before(ui.item.clone());
            }
        },
        stop : function(){
