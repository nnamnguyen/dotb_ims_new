...
if (typeof(ModuleBuilder) == 'undefined') {
	ModuleBuilder = {
	    ...
	    init: function(){
			...
			Connect.url = 'index.php?to_pdf=1&dotb_body_only=1';
			Connect.method = 'POST';
			...
			tp.addTab(new YAHOO.widget.Tab({ 
				label: DOTB.language.get('ModuleBuilder', 'LBL_SECTION_MAIN'),
				scroll : true,
				content : "<div> </div>",
				id : "center",
				active : true
			...
			var mp = ModuleBuilder.mainPanel = new YAHOO.widget.Layout('mblayout', {
				...
				units: [//ModuleBuilder.tree, ModuleBuilder.tabPanel,
				{
					...
					id: 'help',
					header: DOTB.language.get('ModuleBuilder', 'LBL_SECTION_HELP'),
					position:'right',
					body: 'mbhelp',
					scroll: true,
					width: 250,
			...
			if (ModuleBuilder.mode == 'mb') {
				mp.getUnitByPosition('left').header.firstChild.innerHTML = DOTB.language.get('ModuleBuilder', 'LBL_SECTION_PACKAGES');
				...
				mp.getUnitByPosition('left').header.firstChild.innerHTML = DOTB.language.get('ModuleBuilder', 'LBL_SECTION_MODULES');
				mbContent = mbContent ? mbContent :'module=ModuleBuilder&action=wizard';
			}
			else if (ModuleBuilder.mode == 'dotbportal') {
				mp.getUnitByPosition('left').header.firstChild.innerHTML =DOTB.language.get('ModuleBuilder', 'LBL_SECTION_PORTAL');
				mbContent = mbContent ? mbContent : 'module=ModuleBuilder&action=wizard&portal=1';
			}
			else if (ModuleBuilder.mode == 'dropdowns') {
				mp.getUnitByPosition('left').header.firstChild.innerHTML = DOTB.language.get('ModuleBuilder', 'LBL_SECTION_DROPDOWNS');
			...
			if (DOTB.themes.tempHideLeftCol) DOTB.themes.tempHideLeftCol();
		...
		layoutValidation: {
			...
			popup: function(){
				ModuleBuilder.layoutValidation.popup_window = new YAHOO.widget.SimpleDialog("emptyLayout", {
					...
					modal: true,
					fixedcenter: true,
					text: DOTB.language.get('ModuleBuilder', 'ERROR_MINIMUM_FIELDS'),
					bodyStyle: "padding:5px",
					buttons: [{
						text: DOTB.language.get('ModuleBuilder', 'LBL_BTN_CLOSE'),
						isDefault:true,
						handler: function(){
		...
		history: {
			...
			browse: function(module, layout, subpanel){
				...
				if (!ModuleBuilder.history.popup_window) {
					ModuleBuilder.history.popup_window = new YAHOO.DOTB.AsyncPanel('histWindow', {
						width: "400px",
						draggable: true,
						close: true,
						constraintoviewport: true,
				...
				ModuleBuilder.history.popup_window.setHeader( module_str + ' : ' + DOTB.language.get('ModuleBuilder', 'LBL_' + layout.toUpperCase()) + DOTB.language.get('ModuleBuilder', 'LBL_HISTORY_TITLE'));
			...
			preview: function(module, layout, id, subpanel) {
				...
				if (!prevPanel) {
					...
					prevPanel = new YAHOO.DOTB.ClosableTab({
						dataSrc: Connect.url + "&" + ModuleBuilder.paramsToUrl(ModuleBuilder.history.params),
						label: DOTB.language.get("ModuleBuilder", "LBL_MB_PREVIEW"),
						id: 'preview:' + id,
						scroll: true,
						cacheData: true,
						active :true
			...
			onSaveClick: function(){
				...
				if (!saveBtn) {
					...
					if (mbButtons) {
						...
						alert(DOTB.language.get('ModuleBuilder', 'LBL_NO_SAVE_ACTION'));
					}
				}
				if (saveBtn) {
			...
			popup: function(){
                if(false == YAHOO.lang.isObject(ModuleBuilder.state.popup_window) || ModuleBuilder.state.popup_window.id != 'confirmUnsaved'){
                    ModuleBuilder.state.popup_window = new YAHOO.widget.SimpleDialog("confirmUnsaved", {
                     ...
                     modal: true,
                     fixedcenter: true,
                     text: DOTB.language.get('ModuleBuilder', 'LBL_CONFIRM_DONT_SAVE'),
                     bodyStyle: "padding:5px",
                     buttons: [{
                        text: DOTB.language.get('ModuleBuilder', 'LBL_BTN_DONT_SAVE'),
                        handler: ModuleBuilder.state.onDontSaveClick
                     }, {
                        text: DOTB.language.get('ModuleBuilder', 'LBL_BTN_CANCEL'),
                        isDefault:true,
                        handler: function(){
                            ModuleBuilder.state.popup_window.hide()
                        ...
                        text: DOTB.language.get('ModuleBuilder', 'LBL_BTN_SAVE_CHANGES'),
                        handler: ModuleBuilder.state.onSaveClick
                        }]
                    });
                    ModuleBuilder.state.popup_window.setHeader(DOTB.language.get('ModuleBuilder', 'LBL_CONFIRM_DONT_SAVE_TITLE'));
                }
		...
		updateContent: function(o){
			...
			ajaxStatus.flashStatus(DOTB.language.get('app_strings', 'LBL_REQUEST_PROCESSED'), 2000);
			...
			var ajaxResponse = YAHOO.lang.JSON.parse((o.responseText));
				...
				if (!(ModuleBuilder.isResend && ModuleBuilder.isLogoutRedirect(o.responseText))) {
					YAHOO.DOTB.MessageBox.show({
	                    title: DOTB.language.get('ModuleBuilder', 'ERROR_GENERIC_TITLE'),
	                    msg: o.responseText,
	                    width: 500
	                });
				}
			...
			for (var maj in ajaxResponse) {
				...
				if (name == 'mbwest') { //refresh package_tree!
					...
					if (!comp) {
						...
						comp = new YAHOO.DOTB.ClosableTab({
						...
						if (!ModuleBuilder.scriptTest) {
							DOTB.util.evalScript(ajaxResponse[maj].content);
						}
					} else {
		...
		checkForErrors: function(o){
			if (DOTB.util.isLoginPage(o.responseText))
				return true;
			if (o.responseText.substr(0, 1) == "<") {
				...
				if (ModuleBuilder.isLogoutRedirect(o.responseText)) {
					...
					parent.DOTB.App.bwc.login(null, ModuleBuilder.resendRequest);
					return false;
				}
		...
		failed: function(o){
            if(!ModuleBuilder.state.hideFailedMesage){
                ajaxStatus.flashStatus(DOTB.language.get('ModuleBuilder', 'LBL_AJAX_FAILED_DATA'), 2000);
            }
		},
		//Wizard Functions
				...
				if (fieldsetElems[i].id != 'undefined') {
					...
					if (fieldElem) {
						...
						fieldElem.ondblclick = function() {
							...
							YAHOO.DOTB.MessageBox.show({
			                    title: $('#le_label_' + this.id).text().replace(" **", "") + " " + DOTB.language.get("ModuleBuilder", "LBL_COMBO_FIELD_CONTAINS"),
		...
		helpSetup: function(group, def, panel){
			...
			if ( ! ModuleBuilder.AllHelpLang ) ModuleBuilder.AllHelpLang = DOTB.language.get('ModuleBuilder', 'help');
		...
		helpToggle: function(name){
			...
			if (ModuleBuilder.helpLang != null && typeof(ModuleBuilder.helpLang[name]) != 'undefined') {
				...
				if (isRecordView && appendComboFieldIndicator) {
					...
					newHelpText += "<br><br>" + DOTB.language.get('ModuleBuilder', 'LBL_INDICATES_COMBO_FIELD');
				}
		...
		packageDelete: function(MBpackage){
			ajaxStatus.showStatus(DOTB.language.get('ModuleBuilder', 'LBL_AJAX_DELETING'));
			if (confirm(DOTB.language.get('ModuleBuilder', 'LBL_JS_REMOVE_PACKAGE'))) {
		...
		packagePublish: function(form){
			if (check_form(form)) {
				ajaxStatus.showStatus(DOTB.language.get('ModuleBuilder', 'LBL_AJAX_BUILDPROGRESS'));
		...
		packageDeploy: function(form, deployed){
            var confirmed = true;
            if (deployed){
    			confirmed = confirm(DOTB.language.get('ModuleBuilder', 'LBL_JS_DEPLOY_PACKAGE'));
            }
	        if (confirmed && check_form(form)) {
				ajaxStatus.showStatus(DOTB.language.get('ModuleBuilder', 'LBL_AJAX_DEPLOYPROGRESS'));
		...
		deployComplete: function(o){
			...
			if (!resp.match(/^\s*(\s*(Table already exists : [\w_]*)(<br>)*\s*)*complete$/m))
			{
					...
					alert(DOTB.language.get("ModuleBuilder", "LBL_DEPLOY_FAILED"));
			}
		...
		moduleDelete: function(MBpackage, module){
			ajaxStatus.showStatus(DOTB.language.get('ModuleBuilder', 'LBL_AJAX_DELETING'));
			if (confirm(DOTB.language.get('ModuleBuilder', 'LBL_JS_REMOVE_MODULE'))) {
		...
		moduleLoadRelationship2: function(name, resetLabel, checkLanguage) {
			...
			if (!panel) {
				panel = new YAHOO.DOTB.ClosableTab({
					label: DOTB.language.get("ModuleBuilder", "LBL_RELATIONSHIP_EDIT"),
					id: 'relEditor',
					scroll: true,
					cacheData: true,
					active :true
		...
		autoSetLayout: function(){
            ...
            if (DOTB.util.isTouchScreen()) {
                width = parent.DOTB.App.controller.layout.$el.width() - 40;
                height = parent.DOTB.App.controller.layout.$el.height() - 60;
            }
        ...
        resendRequest: function() {
            ...
            if (method == "POST") {
                postFields['csrf_token'] = DOTB.csrf.form_token;
            }
            ...
            Connect.asyncRequest(
                ...
                url,
                ModuleBuilder.requestElements.callbacks,
                DOTB.util.paramsToUrl(postFields)
            );
		},
		asyncRequest : function (params, callback, showLoading) {
            ...
            if (typeof(showLoading) == 'undefined' || showLoading == true) {
                ajaxStatus.showStatus(DOTB.language.get('app_strings', 'LBL_LOADING_PAGE'));
            }
		...
		,moduleLoadFormula: function(formula, targetId, returnType){
            ...
            if (!ModuleBuilder.formulaEditorWindow)
                ModuleBuilder.formulaEditorWindow = new YAHOO.DOTB.AsyncPanel('formulaBuilderWindow', {
                    width: 512,
                    draggable: true,
                    close: true,
                    constraintoviewport: true,
            ...
            win.setHeader(DOTB.language.get("ModuleBuilder", "LBL_FORMULA_BUILDER"));
            win.setBody("loading...");
            ...
            win.load('', "POST", function ()
            {
                ...
                DOTB.util.evalScript(ModuleBuilder.formulaEditorWindow.body.innerHTML);
        ...
        editVisibilityGrid: function(targetId, parent_options, child_options){
            ...
            if (!ModuleBuilder.visGridWindow)
                ModuleBuilder.visGridWindow = new YAHOO.DOTB.AsyncPanel('visGridWindow', {
                    draggable: true,
                    close: true,
                    constraintoviewport: true,
                    fixedcenter: false,
            ...
            win.setHeader(DOTB.language.get("ModuleBuilder", "LBL_VISIBILITY_EDITOR"));
            win.setBody("loading...");
            win.render(document.body);
            win.params = {
                module:"ModuleBuilder",
            ...
            win.load('', 'POST', function() {
                DOTB.util.evalScript(win.body.innerHTML);
        ...
        getCopyLayoutDialog: function() {
            ...
            var dialog = new YAHOO.widget.SimpleDialog("copy-from-dialog", {
                ...
                draggable: false,
                buttons: [{
                    text: DOTB.language.get("ModuleBuilder", "LBL_BTN_COPY"),
                    handler: function() {
                        var role = $("input[name=role]").val();
            ...
            dialog.setHeader(DOTB.language.get("ModuleBuilder", "LBL_HEADER_COPY_FROM_LAYOUT"));
