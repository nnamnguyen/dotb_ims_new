...
if(typeof(SimpleList) == 'undefined'){
    ...
    init: function(editImage, deleteImage) {
        ...
        for (i=0;i<SimpleList.ul_list.length;i++){
            if ( typeof SimpleList.ul_list[i] != "number" && SimpleList.ul_list[i] == "" ) {
                SimpleList.ul_list[i] = DOTB.language.get('ModuleBuilder', 'LBL_BLANK');
            }
    ...
    isBlank : function(value){
    	return value == DOTB.language.get('ModuleBuilder', 'LBL_BLANK')
    ...
    addToList : function(event, form){
        ...
        if ((!SimpleList.isBlank(YAHOO.lang.escapeHTML(dropName.value)) &&
            ...
            !SimpleList.isBlank(YAHOO.lang.escapeHTML(dropValue.value)))) {
            alert(DOTB.language.get('ModuleBuilder', 'LBL_DROPDOWN_BLANK_WARNING'));
            return;
        }
        ...
        for (i=0;i<items.length;i=i+1) {
            if ((SimpleList.isBlank(items[i].id) &&
                ...
                alert(DOTB.language.get('ModuleBuilder', 'LBL_DROPDOWN_KEY_EXISTS'));
                return;
            }
        }
        ...
        if (YAHOO.lang.escapeHTML(dropName.value) == '' || !YAHOO.lang.escapeHTML(dropName.value)) {
            liObj.id = DOTB.language.get('ModuleBuilder', 'LBL_BLANK');
        } else {
        ...
        if (dropValue.value == '') {
            html += '[' + DOTB.language.get('ModuleBuilder', 'LBL_BLANK') + ']';
        } else {
    ...
    handleSave:function(){
        ...
        if(hasDeletedItem) {
        	if(!confirm(DOTB.language.get('ModuleBuilder', 'LBL_CONFIRM_SAVE_DROPDOWN')))
        		return false;
    	}
