...
Studio2 = {
	...
	*/
	...
	getImageElement:function(default_toggle){
		...
		if(!default_toggle)
			img.src = 'index.php?entryPoint=getImage&themeName='+DOTB.themes.theme_name+'&imageName=minus_inline.gif';
		else
			img.src = 'index.php?entryPoint=getImage&themeName='+DOTB.themes.theme_name+'&imageName=plus_inline.gif';
	...
	newField: function(){ // TODO: use properties to set field contents
		...
		newField.innerHTML = '<span>'+DOTB.language.get('ModuleBuilder', 'LBL_FILLER')+'</span>' + // the displayed label
	...
	newRow: function(titleRequired) {
		...
		if (titleRequired) {
	        ...
	        child.appendChild(document.createTextNode(DOTB.language.get('ModuleBuilder', 'LBL_NEW_ROW') ) );
	...
	newPanel: function() {
        ...
        child.className = 'panel_name';
        child.appendChild(document.createTextNode(DOTB.language.get('ModuleBuilder', 'LBL_NEW_PANEL') ) );
		...
		var img = document.createElement('img');
		img.src='index.php?entryPoint=getImage&themeName='+DOTB.themes.theme_name+'&imageName=edit_inline.gif';
		img.className = 'le_edit';
		img.style.cursor="pointer;";
		var editModule = document.getElementById('prepareForSave').view_module.value;
		var editString = 'module=ModuleBuilder&action=editProperty&view_module='+editModule+'&view='+view+'&id_label=le_panelname_'+newPanel.id+'&name_label=label_'+panelLabel+'&title_label='+DOTB.language.get('ModuleBuilder', 'LBL_LABEL_TITLE') ;
	...
	establishLocation: function(element) {
		...
		if (location == null) {
			alert(DOTB.language.get('ModuleBuilder', 'LBL_BADLY_FORMED_DOCUMENT'));
			die();
		}
		return location;
	},
...
//		field.style.width = newWidth+'px';
	...
	handlePublish: function() {
		...
		ModuleBuilder.submitForm('prepareForSave');
		ajaxStatus.flashStatus(DOTB.language.get('ModuleBuilder','LBL_DEPLOYE_COMPLETE'),5000);
	},
    ...
    checkRequiredFields : function(){
		...
		if (missing.length > 0)	
		{
		    var msg = DOTB.language.get("ModuleBuilder", "ERROR_REQUIRED_FIELDS");
	...
	checkCalcFields: function(view, error) {
	    ...
	    if (cfs.length > 0) 
	    {
	        var msg = DOTB.language.get("ModuleBuilder", error) + "\n";
