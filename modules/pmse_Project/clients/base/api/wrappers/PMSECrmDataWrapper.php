...
class PMSECrmDataWrapper implements PMSEObservable
{
    ...
    public function __construct()
    {
        ...
        $this->dotbQueryObject = new DotbQuery();
     ...
     * @return object
     */
    public function getDotbQueryObject()
    {
        if (!isset($this->dotbQueryObject) || empty($this->dotbQueryObject)) {
            $this->dotbQueryObject = new DotbQuery();
        }
        return $this->dotbQueryObject;
    }
     ...
     * @codeCoverageIgnore
     */
    public function setDotbQueryObject($dotbQueryObject)
    {
        $this->dotbQueryObject = $dotbQueryObject;
    }
...
//    }
    ...
    {
        return DotbRelationshipFactory::getInstance()->getRelationshipDef($relationName);
    }
...
//        $result['from_address'] = $bean->from_address;
    ...
    /**
     * Retrieve list of modules installed in DotBCRM
     * @param string $filter
     * @return object
     */
    ...
    public function retrieveActivities($filter = '')
    {
        ...
        $this->dotbQueryObject->select($fields);
        $this->dotbQueryObject->from($activityBean, array('alias' => 'a'));
...
//                $select = array('pmse_bpmn_activity.*');
                ...
                $this->dotbQueryObject->joinTable('pmse_bpm_activity_definition', array('alias' => 'b'))
                    ->on()->equalsField('b.id', 'a.id');
                $this->dotbQueryObject->joinTable('pmse_bpm_dynamic_forms', array('alias' => 'c'))
...
//                );
                ...
                // $this->dotbQueryObject->joinRaw("INNER JOIN pmse_project d ON (d.id=a.prj_id)", array('alias' => 'd'));
                // $this->dotbQueryObject->joinRaw("INNER JOIN pmse_project d ON (d.id=c.prj_id)", array('alias' => 'd'));
                break;
        }
        $this->dotbQueryObject->where()->queryAnd()
            ->addRaw($where);
        // TODO add debug log here
        $activityList = $this->dotbQueryObject->execute();
...
//        $activityList = $activityList['rowList'];
     ...
     * @return object
     * @codeCoverageIgnore
     * @deprecated since version branch dotb7bwc
...
//            $update_evendef1 = "update bpm_event_definition set evn_module = '$res->newModule', evn_criteria = '', evn_params = '' where evn_id in (select evn_id from bpmn_event where evn_behavior = 'CATCH' and prj_id = $projId and evn_type = 'INTERMEDIATE');";
    ...
    public function retrieveBusinessRules($filter)
    {
        ...
        if ($projectBean->retrieve($filter)) {
            ...
            $this->dotbQueryObject->select(array('id', 'name'));
            $this->dotbQueryObject->from($activityDefinitionBean, array('alias' => 'a'));
            $this->dotbQueryObject->joinTable('pmse_bpmn_activity', array('alias' => 'b'))
                ->on()->equalsField('b.id', 'a.id');
            $this->dotbQueryObject->where()->queryAnd()
                ->addRaw(
                    "b.prj_id='" . $projectBean->id . "' AND b.act_script_type = 'BUSINESS_RULE'"
                );
            $this->dotbQueryObject->select->fieldRaw('b.name');
            $rows = $this->dotbQueryObject->execute();
...
//        $res->success = true;
    ...
    /**
     * Method that returns the user roles as Dotb
     * @return object
     */
    ...
    public function validateReclaimCase($casID, $casIndex)
    {
        ...
        $this->dotbQueryObject->select(array('a.cas_id'));
        $this->dotbQueryObject->from($caseBean, array('alias' => 'a'));
        $this->dotbQueryObject->joinTable('pmse_bpm_flow', array('joinType' => 'LEFT', 'alias' => 'b'))
