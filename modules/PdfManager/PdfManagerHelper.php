...
class PdfManagerHelper
{
    ...
    public static function getBannnedFieldsAndLinks()
    {
        ...
        foreach(DotbAutoLoader::existingCustom('modules/PdfManager/metadata/pdfmanagermodulesdefs.php') as $file) {
            include $file;
        }
     ...
     * @return bool
     */
    protected static function hasOneRelationship(DotbBean $bean, $fieldName)
    {
    ...
    public static function parseBeanFields($module_instance, $recursive = FALSE)
    {
        ...
        foreach ($module_instance->toArray() as $name => $value) {
            ...
            if (isset($module_instance->field_defs[$name]['type']) &&
                ...
                $fields_module[$name] = format_number_dotbpdf($module_instance->$name, $locale->getPrecision(), $locale->getPrecision(), $format_number_array);
            } elseif (
                isset($module_instance->field_defs[$name]['type']) &&
                ...
                $fields_module[$name] = $GLOBALS['dotb_config']['upload_dir']."/".$value;
