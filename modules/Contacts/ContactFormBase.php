...
function getWideFormBody($prefix, $mod='',$formname='',  $contact = '', $portal = true){
	...
	$dotbEmailAddress = BeanFactory::newBean('EmailAddresses');
	$dotbEmailAddress->handleLegacyRetrieve($contact);
...
$form .= $dotbEmailAddress->getEmailAddressWidgetEditView($contact->id, $_REQUEST['action']=='ConvertLead'?'Leads':'Contacts', false, 'include/DotbEmailAddress/templates/forWideFormBodyView.tpl');

$dotbfield = new DotbFieldText('Text');
$description_text = $dotbfield->getClassicEditView('description', $contact->description, $prefix, true);
...
EOQ;
	...
	$javascript->setDotbBean($this->getContact());
...
function handleSave($prefix, $redirect=true, $useRequired=false){
	...
	if (!empty($record) && !empty($dupCheck)) {
		...
		if(isset($duplicateContacts)){
            ...
            {
                echo "<script>DOTB.ajaxUI.loadContent('index.php?$location');</script>";
            }
            else {
...
function handleRedirect($return_id){
    ...
    if(!empty($_REQUEST['ajax_load'])){
        echo "<script>DOTB.ajaxUI.loadContent('$redirect_url');</script>\n";
    }
    else {
