        ...
        $('.thumbnail').each(function () {
            ...
            $(data_page).each(function () {
                                        ...
                                        if (start != null && end != null && step != null && step <= allowed_step_value) {
                                            ...
                                            if ($('[name="step_' + que_section + '"]').parent().find('.error-step-msg').length == 0) {
                                                if (allowed_step_value == 1) {
                                                    $('[name="step_' + que_section + '"]').parent().append('<div class="error-step-msg" style="color:red;font-size:11px;">You need to have step value set to 1.</div>');
                                                } else {
                                                    $('[name="step_' + que_section + '"]').parent().append('<div class="error-step-msg" style="color:red;font-size:11px;">You need to set step value less than ' + allowed_step_value + '.</div>');
                                                }
                                            }
                                        } else {
                                            if (allowed_step_value == 1) {
                                                $('[name="step_' + que_section + '"]').parent().find('.error-step-msg').html('You need to have step value set to 1.');
                                            } else {
                                                $('[name="step_' + que_section + '"]').parent().find('.error-step-msg').html('You need to set step value less than ' + allowed_step_value + '.');
                                            }
                                        }
                                                ...
                                                if (this.value.trim() != '') {
                                                    ...
                                                    if (is_image_option) {
                                                        if (typeof self.radio_image_content == "object" && typeof self.radio_image_content[count] != "undefined" && self.radio_image_content[count] != '' && typeof self.radio_image_content[count][op_count] != "undefined" && self.radio_image_content[count][op_count] != '') {
                                                            ...
                                                            answer_detail[op_count]['radio_image'] = radio_image;
                                                        } else if ($(this).parent().find('.spanRadioUploadError').find('.uploadedImage').attr('src')) {
                                                            //Wile Creating Template Images were not saved in Radiobutton And checkbox Type Question by Kairvi 1 June 2019
                ...
                if (typeof page_title != "undefined" && page_title.trim() != "") {
                    ...
                    $('#' + page_id).find('#txt_page_title').parent().find('input').val("");//show placeholder if black value is given ::kairvi
                }
            });
        });
