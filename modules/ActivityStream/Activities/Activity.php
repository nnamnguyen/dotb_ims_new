...
class Activity extends Basic
{
     ...
     *
     * Override DotbBean's constructor so that we can create a comment bean as
     ...
     */
    protected function getChangedFieldsForUser(User $user, DotbBean $bean)
    {
        $fields = array();
     ...
     * @param  DotbBean $bean
     */
    public function processRecord(DotbBean $bean)
    {
     ...
     */
    protected function handleUserToBeanRelationship(DotbBean $bean, User $user, $isADeleteActivity = false)
    {
    ...
    public function processDataWithHtmlPurifier($activityType, $data = array())
    {
        if ($activityType === 'post' && !empty($data['value'])) {
            $data['value'] = DotbCleaner::cleanHtml($data['value']);
        }
    ...
    public static function restoreToPreviousState()
    {
        ...
        if (!is_null($state)) {
            ...
            static::$enabled = $GLOBALS['dotb_config']['activity_streams_enabled'];
        }
    }
}
