...
class ActivityQueueManager
{
    ...
    public static $moduleBlacklist = array('OAuthTokens', 'SchedulersJobs',
        ...
        'Quotes', //Quotes should not allow admin to enable activities until Quotes are converted to lumia
    );
     ...
     * @param  array     $args
     */
    public function eventDispatcher(DotbBean $bean, $event, $args)
    {
    ...
    public static function isEnabledForModule($moduleName)
    {
        ...
        return ($bean instanceof DotbBean) && $bean->isActivityEnabled() && $bean->is_AuditEnabled();
    }
    ...
    {
        if (DotbBean::inOperation('saving_related')) {
            return false;
        }
     ...
     */
    protected function createOrUpdate(DotbBean $bean, array $args, Activity $act)
    {
        if ($bean->deleted) {
            return false;
        }
    ...
    protected function link(array $args, Activity $act)
    {
        ...
        // @see DotbRelationship::getCustomLogicArguments() as well
        if (!$lhs || !$rhs) {
     ...
     */
    public static function getBeanAttributes(DotbBean $bean)
    {
        return array(
     ...
     * @return void
     */
    protected function addRecordSubscriptions($args, DotbBean $bean)
    {
     ...
     * @return string
     */
    protected static function getDisplayName(DotbBean $bean)
    {
        ...
        $field = DotbFieldHandler::getDotbField($fieldDef['type']);
