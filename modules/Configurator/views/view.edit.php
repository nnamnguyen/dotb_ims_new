...
class ConfiguratorViewEdit extends ViewEdit
{
	...
	{
	    global $current_user, $mod_strings, $app_strings, $app_list_strings, $dotb_config, $locale;
        ...
        if (!empty($configurator->config['logger']['file']['suffix'])) {
            $this->ss->assign('filename_suffix', get_select_options_with_id(  DotbLogger::$filename_suffix,$configurator->config['logger']['file']['suffix']));
        } else {
            $this->ss->assign('filename_suffix', get_select_options_with_id(  DotbLogger::$filename_suffix,''));
        }
