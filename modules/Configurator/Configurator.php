...
/**
 * Configurator class around `$dotb_config`
 */
class Configurator
{
    /**
	...
	var $logger = NULL;
	var $previous_dotb_override_config_array = array();
	var $useAuthenticationClass = false;

    /**
     * List of allowed undefined `$dotb_config` keys to be set
     * @var array
     */
    ...
    /**
     * Populate $dotb_config from POST. If no list of allowed keys is passed
     * in then no filtering happens and basically exposes every available
     * $dotb_config settings to be changed if already present or if allowed
     * as an undefined key.
     */
    public function populateFromPost()
    {
        ...
        foreach ($_POST as $key => $value) {
            ...
            // around `$dotb_config` in the first place. Keeping it in here
            ...
            if (isset($this->config[$key]) || in_array($key, $this->allowUndefined)) {
                ...
                // underscores.
                if ($dotbConfig->get(str_replace('_', '.', $key)) !== null) {
	...
	function handleOverride()
	{
		...
		$this->previous_dotb_override_config_array = $overrideArray;
		$diffArray = deepArrayDiff($this->config, $dotb_config);
		$overrideArray = dotbArrayMergeRecursive($overrideArray, $diffArray);
		...
		$GLOBALS['dotb_config'] = $this->config;
        ...
        if( isset($overrideArray['enable_action_menu']) &&
                ( !isset($this->previous_dotb_override_config_array['enable_action_menu']) ||
                    $overrideArray['enable_action_menu'] != $this->previous_dotb_override_config_array['enable_action_menu'] )
        ) {
		...
		foreach($overrideArray as $key => $val) {
            if (in_array($key, $this->allowUndefined) || isset ($dotb_config[$key])) {
			...
			$overideString .= override_value_to_string_recursive2('dotb_config', $key, $val, true, $oldConfig);
		}
    ...
    //bug #27947 , if previous $dotb_config['stack_trace_errors'] is true and now we disable it , we should clear all the cache.
    function clearCache()
    {
        global $dotb_config, $dotb_version;
        ...
        foreach($currentConfigArray as $key => $val) {
            if (in_array($key, $this->allowUndefined) || isset ($dotb_config[$key])) {
                if (empty($val) ) {
                    if(!empty($this->previous_dotb_override_config_array['stack_trace_errors']) && $key == 'stack_trace_errors'){
        ...
        if ($dotb_config['activity_streams_enabled']) {
            Activity::enable();
        } else {
            Activity::disable();
        }
	...
	protected function readOverride() {
		...
		return array($old_config, deepArrayDiff($dotb_config, $old_config));
	}
	function saveOverride($override) {
        ...
        $result = dotb_file_put_contents_atomic('config_override.php', $override);
        if ($result === false) {
	...
	function restoreConfig() {
		$this->readOverride();
		$this->overrideClearDuplicates('dotb_config', '[a-zA-Z0-9\_]+');
		$this->saveOverride();
		ob_clean();
	...
	function saveImages() {
		...
		if (!empty ($_POST['quotes_logo'])) {
			...
			rmdir_recursive(dotb_cached('smarty/templates_c'));
		}
	}
	...
	function saveCompanyLogo($path)
    {
    	...
    	$logo = create_custom_directory(DotbThemeRegistry::current()->getDefaultImagePath(). '/company_logo.png');
        copy($path, $logo);
        dotb_cache_clear('company_logo_attributes');
        DotbThemeRegistry::clearAllCaches();
        DotbThemeRegistry::current()->clearImageCache('company_logo.png');
        $this->updateMetadataCache(array(MetaDataManager::MM_LOGOURL));
	}
	/**
	 * @params : none
	 * @return : An array of logger configuration properties including log size, file extensions etc. See DotbLogger for more details.
	...
	function parseLoggerSettings(){
		if (file_exists('log4php.properties')) {
			...
			$GLOBALS['dotb_config'] = $this->config; //load the rest of the dotb_config settings.
			//$logger = new DotbLogger(); //this will create the log file.
