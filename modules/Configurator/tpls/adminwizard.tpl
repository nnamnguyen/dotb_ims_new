...
<div id="welcome" class="screen">
        ...
        <td>
            ...
            <tr>
              ...
              <p>{$MOD.LBL_WIZARD_WELCOME}</p>
				<div class="userWizWelcome"><img src='include/images/dotb_wizard_welcome.jpg' alt='{$MOD.LBL_WELCOME}' border='0' width='765px' height='325px'></div>
                </td>
            </tr>
            </table>
            </div>
    ...
    <div class="nav-buttons">
        ...
        <input title="{$MOD.LBL_WIZARD_NEXT_BUTTON}"
            ...
            onclick="DotbWizard.changeScreen('system',false);" id="next_tab_system" />
    </div>
</div>
...
<div id="system" class="screen">
    ...
    <tr>
        <td>
            ...
            <tr>
                <td scope="row" width='15%' nowrap>{$MOD.SYSTEM_NAME_WIZARD}&nbsp;{dotb_help text=$MOD.SYSTEM_NAME_HELP}</td>
                <td width='35%'>
                    <input type='text' name='system_name' value='{$settings.system_name}' />
                </td>
            </tr>
            <tr>
                <td scope="row" width='12%' nowrap>{$MOD.NEW_LOGO}&nbsp;{dotb_help text=$MOD.NEW_LOGO_HELP_NO_SPACE}
                </td>
                <td  width='35%'>
            ...
            <tr>
                <td scope="row" width='12%' nowrap>{$MOD.CURRENT_LOGO}&nbsp;{dotb_help text=$MOD.CURRENT_LOGO_HELP}</td>
                <td width="35%">
...
<div id="locale" class="screen">
    ...
    <tr>
        <td>
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                    <th align="left" scope="row" colspan="4"><h2>{dotb_translate module='Administration' label='LBL_LOCALE_TITLE'}</h2></th>
                </tr>
                <tr>
                    <td align="left" scope="row" colspan="4"><i>{$MOD.LBL_WIZARD_LOCALE_DESC}</i></td>
                </tr>
                <tr>
                    <td nowrap="nowrap" scope="row" width="200">{dotb_translate module='Administration' label='LBL_LOCALE_DEFAULT_DATE_FORMAT'}: </td>
                    <td>
                        {html_options name='default_date_format' selected=$config.default_date_format options=$config.date_formats}
                    </td>
                    <td nowrap="nowrap" scope="row" width="200">{dotb_translate module='Administration' label='LBL_LOCALE_DEFAULT_TIME_FORMAT'}: </td>
                    <td>
                        {html_options name='default_time_format' selected=$config.default_time_format options=$config.time_formats}
                    </td>
                </tr>
                <tr>
                    <td nowrap="nowrap" scope="row">{dotb_translate module='Administration' label='LBL_LOCALE_DEFAULT_LANGUAGE'}: </td>
                    <td>
                ...
                <tr>
                    <td nowrap="nowrap" scope="row" width="200">{dotb_translate module='Administration' label='LBL_LOCALE_DEFAULT_CURRENCY_NAME'}: </td>
                    <td>
                        <input type='text' size='15' name='default_currency_name' value='{$config.default_currency_name}' >
                    </td>
                    <td nowrap="nowrap" scope="row" width="200">{dotb_translate module='Administration' label='LBL_LOCALE_DEFAULT_CURRENCY_SYMBOL'}: </td>
                    <td>
                        <input type='text' size='4' name='default_currency_symbol'  value='{$config.default_currency_symbol}' >
                    </td>
                </tr>
                <tr>
                    <td nowrap="nowrap" scope="row" width="200">{dotb_translate module='Administration' label='LBL_LOCALE_DEFAULT_CURRENCY_ISO4217'}: </td>
                    <td>
                        <input type='text' size='4' name='default_currency_iso4217' value='{$config.default_currency_iso4217}'>
                    </td>
                    <td nowrap="nowrap" scope="row">{dotb_translate module='Administration' label='LBL_LOCALE_DEFAULT_NUMBER_GROUPING_SEP'}: </td>
                    <td>
                        <input type='text' size='3' maxlength='1' name='default_number_grouping_seperator' value='{$config.default_number_grouping_seperator}'>
                    </td>
                </tr>
                <tr>
                    <td nowrap="nowrap" scope="row">{dotb_translate module='Administration' label='LBL_LOCALE_DEFAULT_DECIMAL_SEP'}: </td>
                    <td>
                ...
                <tr>
                    <td nowrap="nowrap" scope="row" valign="top">{dotb_translate module='Administration' label='LBL_LOCALE_DEFAULT_NAME_FORMAT'}: </td>
                    <td>
    ...
    <div class="nav-buttons">
        <input title="{$MOD.LBL_WIZARD_BACK_BUTTON}"
            class="button" type="button" name="next_tab1" value="  {$MOD.LBL_WIZARD_BACK_BUTTON}  "
            onclick="DotbWizard.changeScreen('system',true);" id="previous_tab_system" />&nbsp;
        <input title="{$MOD.LBL_WIZARD_NEXT_BUTTON}"
            class="button primary" type="button" name="next_tab1" value="  {$MOD.LBL_WIZARD_NEXT_BUTTON}  "
            onclick="DotbWizard.changeScreen('smtp',false); changeEmailScreenDisplay('{$mail_smtptype}'); document.getElementById('AdminWizard').mail_smtptype.value = 'gmail';" id="next_tab_smtp" />
    </div>
</div>
                        ...
                        <table width="100%" cellpadding="0" cellspacing="0">
                            ...
                            <tr>
                                <td width="20%" scope="row">
                                   ...
                                   {dotb_help text=$MOD.LBL_ALLOW_DEFAULT_SELECTION_HELP }
                                    </span>
                                </td>
                                <td width="30%">
                                     <slot>
...
{dotb_csrf_form_token}
        ...
        {dotb_getimage name="sqsWait" ext=".gif" alt=$mod_strings.LBL_LOADING other_attributes='id="loading_img_company" style="display:none" '}
    </form>
</div>
...
DotbWizard.changeScreen('{/literal}{$START_PAGE|escape:'html':'UTF-8'}{literal}');
    ...
    if( !server.value || !user.value || !pass.value || !port.value)
    {
        ...
        if (server.value == "smtp.gmail.com" && !isValidEmail(user.value)) {
            addToValidate("AdminWizard", 'mail_smtpuser', 'email', false, 
              DOTB.language.get('Configurator','LBL_GMAIL_SMTPUSER'));
        }
        else if (server.value == "plus.smtp.mail.yahoo.com" && !isValidEmail(user.value)) {
            addToValidate("AdminWizard", 'mail_smtpuser', 'email', false, 
              DOTB.language.get('Configurator','LBL_YAHOOMAIL_SMTPUSER'));
        }
...
function uploadCheck(quotes){
    ...
    var callback = {
        upload:function(r) {
            ...
            switch(file_type['data']){
                case 'other':
                    alert(DOTB.language.get('Configurator','LBL_ALERT_JPG_IMAGE'));
                    document.getElementById('my_file_' + forQuotes).value='';
                    break;
                case 'size':
                    alert(DOTB.language.get('Configurator','LBL_ALERT_SIZE_RATIO'));
                    ...
                    break;
                case 'file_error':
                    alert(DOTB.language.get('Configurator','ERR_ALERT_FILE_UPLOAD'));
                    document.getElementById('my_file_' + forQuotes).value='';
                    break;
                //File good
                case 'ok':
                ...
                default:
                   alert(DOTB.language.get('Configurator','LBL_ALERT_TYPE_IMAGE'));
                   document.getElementById('my_file_' + forQuotes).value='';
            }
        },
        failure:function(r){
    ...
    postData = '&entryPoint=UploadFileCheck&forQuotes=false&csrf_token=' + DOTB.csrf.form_token;
...
function sendTestEmail()
{
    ...
    YAHOO.util.Connect.asyncRequest("POST", "index.php?action=EmailUIAjax&module=Emails&emailUIAction=testOutbound&to_pdf=true&dotb_body_only=true", callbackOutboundTest, postDataString);
}
