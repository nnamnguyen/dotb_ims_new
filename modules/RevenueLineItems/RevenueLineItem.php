...
class RevenueLineItem extends DotbBean
{
    ...
    /**
     * Bean specific logic for when DotbFieldCurrency_id::save() is called to make sure we can update the base_rate
     *
     * @return bool
     */
    ...
    protected function setDiscountPrice()
    {
        if (
            ...
            $this->discount_price = DotbMath::init($this->likely_case)->div($quantity)->result();
        }
    }

    
    /**
     * Override the current DotbBean functionality to make sure that when this method is called that it will also
    ...
    protected function mapFieldsFromProductTemplate()
    {
        if (!empty($this->product_template_id)
            ...
            $this->list_price = DotbCurrency::convertAmount($pt->list_price, $pt->currency_id, $this->currency_id);
            $this->cost_price = DotbCurrency::convertAmount($pt->cost_price, $pt->currency_id, $this->currency_id);
            $this->discount_price = DotbCurrency::convertAmount($pt->discount_price, $pt->currency_id, $this->currency_id); // discount_price = unit price on the front end...
