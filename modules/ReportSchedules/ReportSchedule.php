...
class ReportSchedule extends Basic
{
    ...
    /**
     * {@inheritDoc}
     * @see DotbBean::save($check_notify)
     */
     ...
     * {@inheritDoc}
     * @see DotbBean::fetchFromQuery($query, $fields, $options)
     */
    public function fetchFromQuery(DotbQuery $query, array $fields = array(), array $options = array())
    {
     ...
     * @return String
     *
     * TODO Remove once advanced scheduler is converted to lumia
     */
...
QUERY;
     ...
     * be converted to using DotbBeans, which would make this obsolete
     * @param $row
    ...
    INNER JOIN (
        ...
        INNER JOIN (
            SELECT
                ...
                job_group
            FROM job_queue
            WHERE target = 'class::DotbJobSendScheduledReport'
            GROUP BY job_group
        ) last
