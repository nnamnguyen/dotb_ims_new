        ...
        foreach ($metadata->components as $component_key => $component) {
            foreach ($component->rows as $row_key => $row) {
                foreach ($row as $item_key => $item) {
                    ...
                    // check if this user has access to the module & access module:Reports - by Lap Nguyen
                    $accessReport = true;
                    $module       = $item->context->module;
                    if($item->view->type == 'saved-reports-chart'){
                        $accessReport = DotbACL::checkAccess('Reports', 'access');
                        $module = $GLOBALS['db']->getOne("SELECT module FROM saved_reports WHERE id = '{$item->view->saved_report_id}'");
                    }
                    //End - By Lap Nguyen

                    if ( (isset($module) && !DotbACL::checkAccess($module, 'access')) || !$accessReport ) {
