...
global $selector_meta_array;
	...
	//end expression object
		...
		dotb_die("You need a meta filter name to access this popup");
	}
...
if (!is_file(dotb_cached('jsLanguage/') . $exp_object->module_dir . '/' . $GLOBALS['current_language'] . '.js')) {
...
$javascript_language_files .= getVersionedScript("cache/jsLanguage/{$exp_object->module_dir}/{$GLOBALS['current_language']}.js",  $GLOBALS['dotb_config']['js_lang_version']);
...
//end if to show time components
	...
	$javascript->setDotbBean($temp_module);
