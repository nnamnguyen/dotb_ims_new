...
class EmailsApiHelper extends DotbBeanApiHelper
{
    /**
     * {@inheritdoc}
     *
     ...
     * persisting data on {@link Email::outbound_email_id} when DotBCRM was not used to send the email or mutating the
     * sender-of-record of an archived email.
     *
     * {@inheritdoc}
     *
     * @throws DotbApiExceptionInvalidParameter Thrown if `state` is submitted and the state transition is not
     * supported.
     * @throws DotbApiExceptionNotAuthorized Thrown if `from` is submitted and the email is a draft.
     * @throws DotbApiExceptionNotFound Thrown if `outbound_email_id` is submitted and the record cannot be found or is
    ...
    public function populateFromApi(DotbBean $bean, array $submittedData, array $options = array())
    {
        ...
        if ($bean->state === Email::STATE_DRAFT) {
            if (isset($submittedData['from'])) {
                throw new DotbApiExceptionNotAuthorized('Not allowed to edit field from when saving a draft');
            }
