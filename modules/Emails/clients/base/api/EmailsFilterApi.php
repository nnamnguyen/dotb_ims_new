...
class EmailsFilterApi extends FilterApi
{
    ...
    /**
     * This function adds a from, to, cc, or bcc filter to the dotb query based on the value of `$field`.
     *
     * <code>
     * array(
     *     'filter' => array(
     ...
     */
    protected static function addParticipantFilter(DotbQuery $q, DotbQuery_Builder_Where $where, $filter, $field)
    {
        if (!is_array($filter)) {
            throw new DotbApiExceptionInvalidParameter(static::MACRO_FROM . ' requires an array');
        }
