...
class MailApi extends ModuleApi
{
    ...
    protected function handleMail(ServiceBase $api, array $args)
    {
        ...
        try {
            ...
            throw new DotbApiExceptionRequestMethodFailure($eMessage, null, 'Emails');
        }
    ...
    protected function invalidParameter($message, $msgArgs = null)
    {
        ...
        throw new DotbApiExceptionInvalidParameter($message, $msgArgs, 'Emails');
    }
    ...
    public function validateEmailAddresses(ServiceBase $api, array $args)
    {
        ...
        if (!is_array($args)) {
            throw new DotbApiExceptionInvalidParameter('Invalid argument: cannot validate');
        }
        if (empty($args)) {
            throw new DotbApiExceptionMissingParameter('Missing email address(es) to validate');
        }
        $emailAddresses = $args;
        foreach ($emailAddresses as $emailAddress) {
            $validatedEmailAddresses[$emailAddress] = DotbEmailAddress::isValidEmail($emailAddress);
        }
