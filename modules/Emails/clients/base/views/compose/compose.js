 ...
 * @alias DOTB.App.view.views.BaseEmailsComposeView
...
({
    ...
    ATTACH_TYPE_DOTB_DOCUMENT: 'document',
    ...
    /**
     * Open the drawer with the DotbDocuments attachment selection list layout. The callback should take the data
    ...
    /**
     * Fetches the selected DotbDocument using its ID and triggers an "add" event to add the attachment to the
     * attachments field.
     *
     * @param {Data.Bean} model
     */
    documentDrawerCallback: function(model) {
        if (model) {
            var dotbDocument = app.data.createBean('Documents', { id: model.id });
            dotbDocument.fetch({
                success: _.bind(function(model) {
                    ...
                    this.context.trigger('attachment:add', {
                        ...
                        type: this.ATTACH_TYPE_DOTB_DOCUMENT
                    });
                }, this),
                error: _.bind(function(model, error) {
                    this._showServerError(error);
