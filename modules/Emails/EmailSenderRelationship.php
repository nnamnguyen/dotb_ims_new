...
class EmailSenderRelationship extends EmailRecipientRelationship
{
    ...
    public function add($lhs, $rhs, $additionalFields = array())
    {
        ...
        if ($lhs->state === Email::STATE_DRAFT) {
            ...
            if (!$isRhsParentCurrentUser) {
                throw new DotbApiExceptionNotAuthorized('Only the current user can be added as the sender of a draft');
            }
        }
    ...
    public function remove($lhs, $rhs)
    {
        if (!$this->allowRemove &&
            ...
            throw new DotbApiExceptionNotAuthorized("Cannot remove from {$this->name} when the email is a draft");
        }
        ...
        if ($lhs->state === Email::STATE_DRAFT) {
            if (empty($lhs->outbound_email_id)) {
                if (!empty($rhs->email_address_id)) {
                    throw new DotbApiExceptionInvalidParameter($noConfig);
                }
            } else {
                ...
                if ($oe) {
                    if (empty($rhs->email_address_id)) {
                        ...
                        throw new DotbApiExceptionInvalidParameter($noMatch);
                    }
                } elseif (!empty($rhs->email_address_id)) {
                    throw new DotbApiExceptionInvalidParameter($failure);
                }
            }
        }
