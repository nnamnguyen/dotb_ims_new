...
if (isset($_REQUEST['offset']) or isset($_REQUEST['record'])) {
	$result = $detailView->processDotbBean("EMAIL", $focus, $offset);
	if($result == null) {
...
if ($focus->type=='campaign' and !empty($_REQUEST['parent_id']) and !empty($_REQUEST['parent_module'])) {
    ...
    if (!empty($row)) {
        ...
        if (count($macro_values) > 0) {
            ...
            if (!empty($macro_values['dotb_to_email_address'])) {
                $focus->to_addrs=$macro_values['dotb_to_email_address'];
            }
        }
    }
}
...
require_once('include/DotbSmarty/plugins/function.dotb_action_menu.php');
$action_button_sent_email = smarty_function_dotb_action_menu(array(
    'id' => 'detail_header_action_menu',
...
if(is_admin($GLOBALS['current_user']) && $_REQUEST['module'] != 'DynamicLayout' && !empty($_SESSION['editinplace'])){
	$xtpl->assign("ADMIN_EDIT","<a href='index.php?action=index&module=DynamicLayout&from_action=".$_REQUEST['action'] ."&from_module=".$_REQUEST['module'] ."&record=".$_REQUEST['record']. "'>".DotbThemeRegistry::current()->getImage("EditLayout","border='0' align='bottom'",null,null,'.gif',$mod_strings['LBL_EDIT_LAYOUT'])."</a>");
}
...
$xtpl->assign('DESCRIPTION_HTML', DotbCleaner::cleanHtml(from_html($focus->description_html, false), false));
