 ...
 if (isset($_REQUEST['emailUIAction'])) {
  switch($_REQUEST['emailUIAction']) {
    ...
    case "composeEmail":
        ...
        if (isset($_REQUEST['dotbEmail']) && $_REQUEST['dotbEmail'] == 'true' && !empty($uid)) {
            ...
            if (empty($email->description_html)) {
                ...
                $ret['description'] = $ie->getHTMLDisplay(DotbCleaner::cleanHtml($email->description_html));
    ...
    case "getQuickCreateForm":
        ...
        if(!empty($qcModule)) {
            ...
            if(isset($_REQUEST['dotbEmail']) && !empty($_REQUEST['dotbEmail'])) {
                $ie->email->retrieve($uid); // uid is a dotb GUID in this case
            } else {
                $ie->retrieve($ieId);
                $ie->mailbox = $mailbox;
                $ie->setEmailForDisplay($uid);
    ...
    case 'saveQuickCreate':
        ...
        if(isset($_REQUEST['dotbEmail']) && $_REQUEST['dotbEmail'] == 'true' && isset($_REQUEST['uid']) && !empty($_REQUEST['uid'])) {
        ...
        if (isset($ie->email->id)) {
            ...
            if ($controller->bean->module_dir == 'Cases') {
	            if($controller->bean->load_relationship('contacts')) {
	            	...
	            	$emailAddress = DotbEmailAddress::_cleanAddress($emailAddressWithName);
    ...
    case "markEmail":
        ...
        isset($_REQUEST['folder']) && !empty($_REQUEST['folder']) &&
		isset($_REQUEST['ieId']) && (!empty($_REQUEST['ieId']) || (empty($_REQUEST['ieId']) && strpos($_REQUEST['folder'], 'dotb::') !== false))
        ) {
        	...
        	$ret = array();
        	if(strpos($_REQUEST['folder'], 'dotb::') !== false && ($_REQUEST['type'] == 'deleted') && !ACLController::checkAccess('Emails', 'delete')) {
        		$ret['status'] = false;
    ...
    case "getSingleMessageFromDotb":
        $GLOBALS['log']->debug("********** EMAIL 2.0 - Asynchronous - at: getSingleMessageFromDotb");
        if(isset($_REQUEST['uid']) && !empty($_REQUEST['uid'])) {
            ...
            if($email->status == 'draft' || $email->type == 'draft') {
				...
				$teamSetField = new EmailDotbFieldTeamsetCollection($ie->email, $ie->email->field_defs, "get_non_private_teams_array", 'composeEmailForm');
...
eoq;
    ...
    case "getMultipleMessagesFromDotb":
        $GLOBALS['log']->debug("********** EMAIL 2.0 - Asynchronous - at: getMultipleMessagesFromDotb");
        if(isset($_REQUEST['uid']) && !empty($_REQUEST['uid'])) {
    ...
    case "getMessageListDotbFolders":
        $GLOBALS['log']->debug("********** EMAIL 2.0 - Asynchronous - at: getMessageListDotbFoldersJSON");
        if(isset($_REQUEST['ieId']) && !empty($_REQUEST['ieId'])) {
            // user view preferences
            $email->et->saveListView($_REQUEST['ieId'], "DOTB.{$_REQUEST['mbox']}");
            if(isset($_REQUEST['start']) && isset($_REQUEST['limit'])) {
	            $page = ceil($_REQUEST['start'] / $_REQUEST['limit']) + 1;
	        } else {
	        	$page = 1;
    ...
    case "refreshDotbFolders":
        $GLOBALS['log']->debug("********** EMAIL 2.0 - Asynchronous - at: refreshDotbFolders");
    ...
    case "renameFolder":
        ...
        && isset($_REQUEST['newFolderName']) && !empty($_REQUEST['newFolderName'])) {
            ...
            if(is_guid($_REQUEST['folderId'])) {
                ...
                echo $mod_strings['LBL_NOT_DOTB_FOLDER'];
            }
        }
    case "moveFolder":
    ...
    case "getGroupFolder":
			...
			$teamSetField = new EmailDotbFieldTeamsetCollection($email->et->folder, $ie->field_defs, "get_non_private_teams_array", 'EditViewGroupFolder');
			$sqs_objects = $teamSetField->createQuickSearchCode(true);
			$code = $teamSetField->get_code();
