	...
	if(!empty($contact)) {
    	...
    	if($contact->object_name == 'Opportunity' || $contact->object_name == 'aCase'){
    	    if(!empty($contact->account_id)) {
    		    ...
    		    if(!empty($account)){
        			$dotbEmailAddress->handleLegacyRetrieve($account);
...
$xtpl->assign('THEME', DotbThemeRegistry::current()->__toString());
	...
	for($i = 0;$i < count($notes_list);$i++) {
		...
		$attachments .= "
			<div id='noteDiv{$the_note->id}'>
				" . DotbThemeRegistry::current()->getImage('delete_inline', "onclick='deletePriorAttachment(\"{$the_note->id}\");' value='{$the_note->id}'", null, null, ".gif", $mod_strings['LBL_DELETE_INLINE']) . "&nbsp;";
