...
class ArchivedEmailsLink extends Link2
{
    ...
    public function buildJoinDotbQuery($dotb_query, $options = array())
    {
        ...
        if (!empty($options['reverse'])) {
            return $this->joinEmails($dotb_query, $fromAlias, $jta);
        } else {
            $dotb_query->joinTable('emails', $joinParams);
            return $this->joinEmails($dotb_query, $fromAlias, $jta);
        }
    }
    ...
    /**
     * Builds DotbQuery main join for archived emails
     *
     * @param DotbQuery $query The query
     ...
     */
    protected function joinEmails(DotbQuery $query, $fromAlias, $alias)
    {
