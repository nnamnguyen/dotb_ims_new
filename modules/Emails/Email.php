...
class Email extends DotbBean {
	...
	{
	    global $current_user;
	    $this->cachePath = dotb_cached('modules/Emails');
		parent::__construct();
		$this->team_id = 1; // make the item globally accessible

		$this->emailAddress = BeanFactory::newBean('EmailAddresses');

		$this->imagePrefix = rtrim($GLOBALS['dotb_config']['site_url'], "/")."/cache/images/";
	}
	...
	public function email2saveAttachment()
	{
        ...
        if($upload->final_move($guid)) {
			dotb_mkdir(dotb_cached("$email_uploads/"));
			copy("upload://$guid", dotb_cached("$email_uploads/$guid"));
			return array(
					'guid' => $guid,
	...
	function safeAttachmentName($filename) {
		...
		foreach($dotb_config['upload_badext'] as $badExt) {
	...
	function decodeDuringSend($htmlData) {
	    $htmlData = str_replace("dotbLessThan", "&lt;", $htmlData);
	    $htmlData = str_replace("dotbGreaterThan", "&gt;", $htmlData);
		return $htmlData;
	}
	...
	function email2Send($request) {
            ...
            if (!empty($request['attachments'])) {
                ...
                foreach ($exAttachments as $file) {
                    ...
                    if (!empty($file)) {
                        ...
                        if ((($this->type == 'draft') && !empty($this->id)) || (isset($request['saveToDotb']) && $request['saveToDotb'] == 1)) {
                            $note                 = new Note();
                            $note->id             = create_guid();
                        ...
                        if (isset($validNote) && $validNote === true) {
                            $attachment = AttachmentPeer::attachmentFromDotbBean($note);
                            if (!is_null($mailer)) {
                ...
                foreach ($exDocs as $docId) {
                    ...
                    if (!empty($docId)) {
                        ...
                        if ((($this->type == 'draft') && !empty($this->id)) || (isset($request['saveToDotb']) && $request['saveToDotb'] == 1)) {
                            ...
                            $dest = "upload://{$note->id}";
                            if (!file_exists($fileLocation) || (!copy($fileLocation, $dest))) {
                                $GLOBALS['log']->debug("EMAIL 2.0: could not copy DotbDocument revision file $fileLocation => $dest");
                            }
                            $note->save();
                        }

                        $attachment = AttachmentPeer::attachmentFromDotbBean($documentRevision);
                        //print_r($attachment);
                        if (!is_null($mailer)) {
                            $mailer->addAttachment($attachment);
                        }
		...
		if(	$forceSave ||
			$this->type == 'draft' ||
            $this->type == 'archived' ||
			(isset($request['saveToDotb']) && $request['saveToDotb'] == 1)) {
			...
			///////////////////////////////////////////////////////////////////
			////	LINK EMAIL TO DOTBBEANS BASED ON EMAIL ADDY
    ...
    function _arrayToDelimitedString($arr)
    {
        ...
        $separator = (isset($GLOBALS['dotb_config']['email_address_separator']) &&
                        !empty($GLOBALS['dotb_config']['email_address_separator'])) ?
                     $GLOBALS['dotb_config']['email_address_separator'] :
                     ',';
	...
	function save($check_notify = false) {
		if($this->isDuplicate) {
            ...
            // Overrides DotbBean behavior to use Global as the default team.
            ...
            if ($this->state === static::STATE_DRAFT) {
                ...
                if (!empty($this->date_start) && !empty($this->time_start)) {
                    ...
                    // SI Bug #39503: DotbBean is not setting date_sent when seconds are missing.
    ...
    {
        return DotbCleaner::cleanHtml($content, $encoded);
    }
     ...
     *
     * @param DotbBean $bean
     * @throws DotbException if the "from" relationship could not be loaded.
     */
    protected function setSender(DotbBean $bean)
    {
        if (!$this->load_relationship('from')) {
            throw new DotbException('Could not find a relationship named: from');
        }
	...
	function saveTempNoteAttachments($filename,$fileLocation, $mimeType, $uploadId = null)
	{
        ...
        if (!empty($uploadId)) {
            ...
            if(!file_exists($fileLocation) || (!copy($fileLocation, $noteFile))) {
                $GLOBALS['log']->fatal("EMAIL 2.0: could not copy DotbDocument revision file $fileLocation => $noteFile");
            }
        }
	    $tmpNote->save();
        return $tmpNote;
    ...
    {
        // cn: bug 11915, return DotbBean's retrieve() call bean instead of $this
     ...
     */
    function loadAdditionalEmailData(DotbBean $emailBean = null)
    {
	...
	function getUserEditorPreference() {
		...
		if(!isset($dotb_config['email_default_editor'])) {
			$dotb_config = $current_user->setDefaultsInConfig();
		}

		$userEditor = $current_user->getPreference('email_editor_option');
		$systemEditor = $dotb_config['email_default_editor'];
     ...
     * @see SI Bug 22504
     * @see SI Bug 49024
     * @param DotbBean $bean A bean from the related module.
     * @return bool|string Name of an Emails relationship link or false.
     */
    public function findEmailsLink(DotbBean $bean)
    {
            ...
            if (is_array($this->saved_attachments)) {
                foreach ($this->saved_attachments as $note) {
                    ...
                    if ($note->embed_flag == true) {
                        $cid = $filename;
                        $attachment = AttachmentPeer::embeddedImageFromDotbBean($note, $cid);
                    } else {
                        $attachment = AttachmentPeer::attachmentFromDotbBean($note);
                    }
    ...
    public function create_new_list_query(
        ...
        LoggerManager::getLogger()->deprecated('Email::create_new_list_query() has been deprecated. Use DotbQuery ' .
    ...
    public function fill_in_additional_list_fields()
    {
        ...
        $this->attachment_image = ($row !=null) ? DotbThemeRegistry::current()->getImage('attachment',"","","") : "";

		if ($row !=null) {
			$this->attachment_image = DotbThemeRegistry::current()->getImage('attachment',"","","",'.gif',translate('LBL_ATTACHMENT', 'Emails'));
		}
	...
	function get_list_view_data() {
		...
		if(!empty($this->parent_type)) {
			...
			switch($this->intent) {
				case 'support':
					$email_fields['CREATE_RELATED'] = '<a href="index.php?module=Cases&action=EditView&inbound_email_id='.$this->id.'" >' . DotbThemeRegistry::current()->getImage('CreateCases', 'border="0"', null, null, ".gif", $mod_strings['LBL_CREATE_CASES']).$mod_strings['LBL_CREATE_CASE'].'</a>';
				break;

				case 'sales':
					$email_fields['CREATE_RELATED'] = '<a href="index.php?module=Leads&action=EditView&inbound_email_id='.$this->id.'" >'.DotbThemeRegistry::current()->getImage('CreateLeads', 'border="0"', null, null, ".gif", $mod_strings['LBL_CREATE_LEADS']).$mod_strings['LBL_CREATE_LEAD'].'</a>';
				break;

				case 'contact':
					$email_fields['CREATE_RELATED'] = '<a href="index.php?module=Contacts&action=EditView&inbound_email_id='.$this->id.'" >'.DotbThemeRegistry::current()->getImage('CreateContacts', 'border="0"', null, null, ".gif", $mod_strings['LBL_CREATE_CONTACTS']).$mod_strings['LBL_CREATE_CONTACT'].'</a>';
				break;

				case 'bug':
					$email_fields['CREATE_RELATED'] = '<a href="index.php?module=Bugs&action=EditView&inbound_email_id='.$this->id.'" >'.DotbThemeRegistry::current()->getImage('CreateBugs', 'border="0"', null, null, ".gif", $mod_strings['LBL_CREATE_BUGS']).$mod_strings['LBL_CREATE_BUG'].'</a>';
				break;

				case 'task':
					$email_fields['CREATE_RELATED'] = '<a href="index.php?module=Tasks&action=EditView&inbound_email_id='.$this->id.'" >'.DotbThemeRegistry::current()->getImage('CreateTasks', 'border="0"', null, null, ".gif", $mod_strings['LBL_CREATE_TASKS']).$mod_strings['LBL_CREATE_TASK'].'</a>';
				break;
    ...
    function quickCreateForm() {
        ...
        return $mod_strings['LBL_QUICK_CREATE']."&nbsp;<a id='$this->id' onclick='return quick_create_overlib(\"{$this->id}\", \"".DotbThemeRegistry::current()->__toString()."\", this);' href=\"#\" >".DotbThemeRegistry::current()->getImage("advanced_search","border='0' align='absmiddle'", null,null,'.gif',$mod_strings['LBL_QUICK_CREATE'])."</a>";
    }
    ...
    function searchImportedEmails($sort = '', $direction='')
    {
		...
		while($a = $this->db->fetchByAssoc($rs)) {
			...
			$temp['site_url'] = $dotb_config['site_url'];
			$temp['seen'] = ($a['status'] == 'unread') ? 0 : 1;
			$temp['type'] = $a['type'];
			$temp['mbox'] = 'dotb::Emails';
	...
	function distributionForm($where) {
		...
		$teamSetField = new EmailDotbFieldTeamsetCollection($this, $this->field_defs, '', 'Distribute');
		...
		$out .= '
			...
			<span id="showUsersDiv" style="position:relative;">
				<a href="#" id="showUsers" onClick="javascript:showUserSelect();">
					'.DotbThemeRegistry::current()->getImage('Users', '', null, null, ".gif", $mod_strings['LBL_USERS']).'</a>&nbsp;
				<a href="#" id="showUsers" onClick="javascript:showUserSelect();">
					<span style="display:none;" id="checkMark">'.DotbThemeRegistry::current()->getImage('check_inline', 'border="0"', null, null, ".gif", $mod_strings['LBL_CHECK_INLINE']).'</span>
				</a>
				...
				<table cellpadding="0" cellspacing="0" border="0" class="list view">
					<tr height="20">
						<td  colspan="'.$colspan.'" id="hiddenhead" onClick="hideUserSelect();" onMouseOver="this.style.border = \'outset red 1px\';" onMouseOut="this.style.border = \'inset white 0px\';this.style.borderBottom = \'inset red 1px\';">
							<a href="#" onClick="javascript:hideUserSelect();">'.DotbThemeRegistry::current()->getImage('close', 'border="0"', null, null, ".gif", $mod_strings['LBL_CLOSE']).'</a>
...
//<td valign="middle" height="30"  colspan="'.$colspan.'" id="hiddenhead" onClick="hideUserSelect();" onMouseOver="this.style.border = \'outset red 1px\';" onMouseOut="this.style.border = \'inset white 0px\';this.style.borderBottom = \'inset red 1px\';">
    ...
    public function fillPrimaryParentFields($table)
    {
        ...
        public function cid2Link($noteId, $noteType)
        {
			...
			$imgfilename = dotb_cached("images/")."$noteId.".strtolower($subtype);
			$note = BeanFactory::getBean('Notes', $noteId);
     ...
     *
     * @todo Consider moving to DotbBean to allow other models to set fields to NULL
     * @deprecated The BWC Emails UI is no longer being used.
    ...
    public function toDatabaseSearchDateTime($userSearchDateTime) {
        ...
        $dotbDateTime = new DotbDateTime($userSearchDateTime);
        $dotbDateTime->setTimezone($tz);
        $dbSearchDateTime = $timedate->asDb($dotbDateTime);
    ...
    {
        if ($this->state !== static::STATE_DRAFT) {
            throw new DotbException("Cannot send an email with state: {$this->state}");
        }
     ...
     * @param string $role Can be "to", "cc", or "bcc".
     * @return array
     * @throws DotbException
     */
    protected function getParticipants($role)
    {
        if (!$this->load_relationship($role)) {
            throw new DotbException("Cannot get participants for link: {$role}");
        }
        ...
        $q = new DotbQuery();
        $q->from($seed);
        // Must add the fields to select before calling DotbBean::fetchFromQuery() so that the email_address_used
        // field is in DotbQuery::$joinLinkToKey. Otherwise, the joins that DotbQuery::joinSubpanel() adds will be
        // replaced by new joins when the email_address_used field is added in DotbBean::fetchFromQuery(), and the
        ...
        // DotbBean::fetchFromQuery() will add all of the fields in the vardefs if we don't.
    ...
    public function getOutboundEmailDropdown()
    {
        ...
        if ($error) {
            throw new DotbApiExceptionNotAuthorized($error, null, $this->getModuleName());
        }
