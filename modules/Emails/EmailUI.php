...
class EmailUI {
	var $db;
	var $folder; // place holder for DotbFolder object
	...
	public function __construct() {
		...
		$this->folder = new DotbFolder();
		$this->userCacheDir = dotb_cached("modules/Emails/{$current_user->id}");
	...
	function displayEmailFrame() {
		...
		$this->smarty->assign('dotb_config', $dotb_config);
		$this->smarty->assign('is_admin', $current_user->is_admin);
		$this->smarty->assign('dotb_version', $dotb_version);
		$this->smarty->assign('dotb_flavor', $dotb_flavor);
		...
		$this->smarty->assign('disable_account_config',
			DotbConfig::getInstance()->get("disable_user_email_config", false)
		...
		$signatureButtons = $current_user->getSignatureButtons('DOTB.email2.settings.createSignature', !empty($defsigID));
		if (!empty($defsigID)) {
			$signatureButtons = $signatureButtons . '<span name="delete_sig" id="delete_sig" style="visibility:inherit;"><input class="button" onclick="javascript:DOTB.email2.settings.deleteSignature();" value="'.$app_strings['LBL_EMAIL_DELETE'].'" type="button" tabindex="392">&nbsp;
					</span>';
		} else {
			$signatureButtons = $signatureButtons . '<span name="delete_sig" id="delete_sig" style="visibility:hidden;"><input class="button" onclick="javascript:DOTB.email2.settings.deleteSignature();" value="'.$app_strings['LBL_EMAIL_DELETE'].'" type="button" tabindex="392">&nbsp;
					</span>';
		}
        ...
        $jsPath = getJSPath("include/javascript/dotbwidgets/DotbYUIWidgets.js");
...
eoq;
    ...
    protected function _loadQuickCreateModules()
    {
        ...
        foreach(DotbAutoLoader::existingCustom('modules/Emails/metadata/qcmodulesdefs.php') as $file) {
            include $file;
        }

        foreach($QCModules as $module) {
            $seed = BeanFactory::newBean($module);
            if ( ( $seed instanceOf DotbBean ) && $seed->ACLAccess('edit') ) {
     ...
     * result is typically passed into the js call DOTB.quickCompose.init which initalizes the quick compose
     * UI.
     *
     ...
     * @param DotbBean $bean Optional - the parent object bean with data
	...
	function _generateComposeConfigData($type = "email_compose_light" )
	{
        ...
        $teamSetField
            = new EmailDotbFieldTeamsetCollection($emailsBean, $emailsBean->field_defs, '', 'composeEmailForm');
	...
	function retrieveTeamInfoForSettingsUI() {
        ...
        $ie1->disable_row_level_security = true;
		$ie1->team_set_id = '';
		$teamSetField = new EmailDotbFieldTeamsetCollection($ie1, $ie1->field_defs, '', 'Distribute');
	...
	function getUserPrefsJS() {
		...
		if(empty($emailSettings)) {
			...
			$emailSettings['defaultOutboundCharset'] = $GLOBALS['dotb_config']['default_email_charset'];
	...
	/**
	 * Creates a new Dotb folder
	 * @param string $nodeLabel New dotb folder name
	...
	function getMailboxNodes() {
		...
		if($current_user->hasPersonalEmail()) {
			...
			foreach($personals as $k => $personalAccount) {
				if(in_array($personalAccount->id, $showFolders)) {
					...
					$cacheRoot = dotb_cached("modules/Emails/{$personalAccount->id}");
		...
		foreach($beans as $k => $groupAccount) {
			if(in_array($groupAccount->id, $showFolders)) {
				// check for cache value
				$cacheRoot = dotb_cached("modules/Emails/{$groupAccount->id}");
				$this->preflightEmailCache($cacheRoot);
		...
		// DotbFolder nodes
		/* DotbFolders are built at onload when the UI renders */
	...
	function buildTreeNode($key, $label, $mbox, $ieId, $nodePath, $isGroup, $ie) {
        ...
        //$node->set_property('href', " DOTB.email2.listView.populateListFrame(YAHOO.namespace('frameFolders').selectednode, '{$ieId}', 'false');");
			...
			if (isset($meta['attachments'])) {
				...
				if (sizeof($actualAttachmentInfo) > 0) {
					foreach($actualAttachmentInfo as $key => $value) {
						...
						$attachmentFilePath = dotb_cached("modules/Emails/{$ie->id}/attachments/{$attachmentid}");
	...
	function getQuickCreateForm($vars, $email, $addToAddressBookButton=false)
	{
		...
		if(in_array($beanName, $people)) {
            ...
            if(isset($_REQUEST['dotbEmail']) && !empty($_REQUEST['dotbEmail']))
            {
		...
		$EditView->ss = new Dotb_Smarty();
		//MFH BUG#20283 - checks for custom quickcreate fields
		$EditView->setup($module, $focus, DotbAutoLoader::loadWithMetafiles($module, 'editviewdefs'));
		$EditView->process();
		$EditView->render();
		...
		if($addToAddressBookButton) {
			$EditView->defs['templateMeta']['form']['buttons']['email2saveAddToAddressBook'] = array(
				...
				'customCode' => '<input type="button" class="button" value="   '.$app_strings['LBL_EMAIL_ADDRESS_BOOK_SAVE_AND_ADD']
				              . '   " onclick="DOTB.email2.detailView.saveQuickCreate(true);" />'
			);
		}

		//Get the module language for javascript
	    if(!is_file(dotb_cached('jsLanguage/') . "{$module}/{$GLOBALS['current_language']}.js")) {
            jsLanguage::createModuleStringsCache($module, $GLOBALS['current_language']);
        }
		$jsLanguage = getVersionedScript("cache/jsLanguage/{$module}/{$GLOBALS['current_language']}.js", $GLOBALS['dotb_config']['js_lang_version']);
	    	...
	    	if(empty($admin->settings['portal_on']) || !$admin->settings['portal_on']) {
			   ...
			   $jsLanguage .= <<<EOQ
			    <script language="javascript">
				   addToValidateComparison('form_EmailQCView_Contacts', 'portal_password', 'varchar', false, DOTB.language.get('app_strings', 'ERR_SQS_NO_MATCH_FIELD') + DOTB.language.get('Contacts', 'LBL_PORTAL_PASSWORD'), 'portal_password1');
		           addToValidateVerified('form_EmailQCView_Contacts', 'portal_name_verified', 'bool', false, DOTB.language.get('app_strings', 'ERR_EXISTING_PORTAL_USERNAME'));
...
EOQ;
		...
		$meta['html'] = str_replace("src='".getVersionedPath('include/DotbEmailAddress/DotbEmailAddress.js')."'", '', $meta['html']);
    ...
    function getImportForm($vars, $email, $formName = 'ImportEditView') {
		...
		$teamSetField = new EmailDotbFieldTeamsetCollection($email, $email->field_defs, '', $formName);
    ...
    function getDetailViewForEmail2($emailId) {
		...
		$smarty->assign("DOTB_VERSION", $GLOBALS['dotb_version']);
		$smarty->assign("JS_CUSTOM_VERSION", $GLOBALS['dotb_config']['js_custom_version']);
	 ...
	 * @param string $ieId
	 * @param string $folder IMAP folder structure or DotbFolder GUID
	...
	function markEmails($type, $ieId, $folder, $uids) {
		...
		if(strpos($folder, 'dotb::') !== false) {
            // Collect message IDs for deleting mails from server
            $messageUIDs = array();
            // dealing with a dotb email object, uids are GUIDs
...
function setLastRobin($ie, $lastRobin) {
    global $dotb_config;
    $cacheFolderPath = dotb_cached("modules/Emails/{$ie->id}/folders");
...
eoq;
	...
	function clearInboundAccountCache($ieId) {
		global $dotb_config;
		$cacheRoot = dotb_cached("modules/Emails/{$ieId}");
    ...
    {
        return dotb_cached("modules/Emails/{$ieId}/{$type}/{$filename}");
    }
...
eoq;
	...
	function generateExpandableAddrs($str) {
        ...
        if(!empty($matchs[0]) && $totalCount > $defaultNum) {
            ...
            $str = htmlentities($frontStr) . '<a class="utilsLink" onclick="javascript: DOTB.email2.detailView.displayAllAddrs(this);">...['.$mod_strings['LBL_EMAIL_DETAIL_VIEW_SHOW'].$hiddenCount.$mod_strings['LBL_EMAIL_DETAIL_VIEW_MORE'].']</a><span style="display: none;">' .htmlentities($backStr).'</span>';
        }
