...
class EmailRecipientRelationship extends One2MBeanRelationship
{
    ...
    {
        if ($lhs->isArchived()) {
            throw new DotbApiExceptionNotAuthorized("Cannot add to {$this->name} when the email is archived");
        }
    ...
    public function remove($lhs, $rhs)
    {
        if ($lhs->isArchived() && !$rhs->deleted && !$lhs->deleted) {
            throw new DotbApiExceptionNotAuthorized("Cannot remove from {$this->name} when the email is archived");
        }
    ...
    protected function updateFields($lhs, $rhs, $additionalFields)
    {
        ...
        // the bean is saved if `DotbRelationship::resaveRelatedBeans` reloads the bean and we happened to have an
        // older instance already in memory.
        BeanFactory::registerBean($rhs);
        DotbRelationship::addToResaveList($rhs);
    }

    /**
     * Changes the bean's `parent_type` to Users if it is Employees and the employee is actually a user.
     *
     * @param DotbBean $bean The EmailParticipants bean.
     */
    protected function fixParentModule(DotbBean $bean)
    {
     ...
     * @throws DotbApiExceptionNotAuthorized
     */
    protected function assertParentModule(DotbBean $bean)
    {
     ...
     *
     * @param DotbBean $lhs The Emails bean.
     * @param DotbBean $rhs The EmailParticipants bean.
     */
    protected function setEmailAddress(DotbBean $lhs, DotbBean $rhs)
    {
     ...
     */
    private function getParent(DotbBean $bean)
    {
     ...
     *
     * @param DotbBean $bean
     * @param DotbBean $emailAddress
     * @return bool
     */
    private function addEmailAddressToRecord(DotbBean $bean, DotbBean $emailAddress)
    {
