	   ...
	   <td colspan="2" >
			...
			<tr>
				<td> <input title="{$mod_strings.LBL_ADD_INBOUND_ACCOUNT}"
	                        type='button' 
	                        class="button"
	                        onClick='DOTB.email2.accounts.showEditInboundAccountDialogue();'
								...
								<tr>
                				    <td style="padding-bottom: 5px">
                					   <input id="outbound_email_add_button" title="{$app_strings.LBL_EMAIL_FOLDERS_ADD}" type='button' 
                					   	class="button" onClick='DOTB.email2.accounts.showAddSmtp();' name="button" value="{$mod_strings.LBL_ADD_OUTBOUND_ACCOUNT}">
                					</td>
                				</tr>
								{/if}
                            </table>
