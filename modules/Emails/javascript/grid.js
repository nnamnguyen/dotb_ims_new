	...
	e2Grid = {
		init : function() {
			...
			if (Ck.get("EmailGridWidths")) {
				...
				for (var i=0; i < widths.length; i++) {
					Ck.setSub("EmailGridWidths", i + "", widths[i], {expires: DOTB.email2.nextYear});
				}
			}
			...
			function repliedIcon(cell, record, column, value) {
				if(value != "") {
					cell.innerHTML = "<img src='index.php?entryPoint=getImage&themeName="+DOTB.themes.theme_name+"&imageName=export.gif' class='image' border='0' width='10' align='absmiddle'>";
				}
			}
	        function attachIcon(cell, record, column, value) {
				if(value == "1") {
					cell.innerHTML = "<img src='index.php?entryPoint=getImage&themeName="+DOTB.themes.theme_name+"&imageName=attachment.gif' class='image' border='0' width='10' align='absmiddle'>";
				}
			}
			...
			var colModel =
				[
					{
						label: "<h2><img src='index.php?entryPoint=getImage&themeName="+DOTB.themes.theme_name+"&imageName=attachment.gif' class='image' border='0' width='10' align='absmiddle'></h2>",
						width: 10,
						sortable: false,
						fixed: true,
						resizeable: true,
					...
					{
						label: "<img src='index.php?entryPoint=getImage&themeName="+DOTB.themes.theme_name+"&imageName=export.gif' class='image' border='0' width='10' align='absmiddle'>",
						width: widths[1],
						sortable: true,
						fixed: true,
						resizeable: true,
			...
			if(lazyLoadFolder != null) {
				...
				//Check if the folder is a Dotb Folder
				var test = new String(lazyLoadFolder.folder);
				if(test.match(/DOTB\./)) {
					params['emailUIAction'] = 'getMessageListDotbFolders';
					params['mbox'] = test.substr(6);
				}
			}

			// create the Grid
			var grid = DOTB.email2.grid = new YAHOO.DOTB.SelectionGrid('emailGrid', colModel, dataModel, {
				MSG_EMPTY: DOTB.language.get("Emails", "LBL_EMPTY_FOLDER"),
				dynamicData: true,
				paginator: new YAHOO.widget.Paginator({
					rowsPerPage:parseInt(DOTB.email2.userPrefs.emailSettings.showNumInList),
                ...
                initialRequest:DOTB.util.paramsToUrl(params),
				width:  "800px",
				height: "400px"
			});
			...
			grid.set("generateRequest", function(oState, oSelf) {
	            ...
	            // Build the request
	            var ret =
		            DOTB.util.paramsToUrl(oSelf.params) +
		            "&sort=" + sort +
	                "&dir=" + dir +
	                "&start=" + startIndex +
			...
			var resize = grid.resizeGrid = function () {
				DOTB.email2.grid.set("width",  DOTB.email2.grid.get("element").parentNode.clientWidth + "px");
				DOTB.email2.grid.set("height", (DOTB.email2.grid.get("element").parentNode.clientHeight - 57) + "px");
			}
			...
			grid.on("columnResizeEvent", function(o) {
				...
				var colSet = DOTB.email2.grid.getColumnSet().flat;
				for (var i=0; i < colSet.length; i++) {
					if (o.column == colSet[i]) {
						//Store it in the cookie
						Ck.setSub("EmailGridWidths", i + "", o.width, {expires: DOTB.email2.nextYear});
					}
				}
				//this.resizeGrid();
			}, null, grid);
			...
			grid.on("rowClickEvent", DOTB.email2.listView.handleClick);
			grid.on("rowDblclickEvent", DOTB.email2.listView.getEmail);
			grid.render();
			DOTB.email2.listViewLayout.on("render", resize);
			resize();

			//Setup the default load parameters
			DOTB.email2.grid.params = params;

			grid.on('postRenderEvent', DOTB.email2.listView.setEmailListStyles);
...
function initRowDD() {
	...
	YAHOO.extend(sg.DDRow, YAHOO.util.DDProxy, {
	    ...
	    onDragOver: function(ev, id) {
	    	var node = DOTB.email2.tree.getNodeByElement(Dom.get(id));
	    ...
	    endDrag: function() {
	    	...
	    	if (this.targetNode) {
	    		DOTB.email2.folders.handleDrop(this.rows, this.targetNode);
	    	}
	    	DOTB.email2.folders.unhighliteAll();
	    	this.rows = null;
	    }
...
function AddressSearchGridInit() {
    function moduleIcon(elCell, oRecord, oColumn, oData) {
    	elCell.innerHTML = "<img src='index.php?entryPoint=getImage&themeName="+DOTB.themes.theme_name+"&imageName=" + oData + ".gif' class='image' border='0' width='16' align='absmiddle'>";
    };
    function selectionCheckBox(elCell, oRecord, oColumn, oData) {
        elCell.innerHTML =  '<input type="checkbox" onclick="DOTB.email2.addressBook.grid.toggleSelectCheckbox(\'' + oRecord.getId() + '\', this.checked);">';
    };
    var checkHeader = '<input type="checkbox" ';
    if (DOTB.email2.util.isIe()) {
        checkHeader += 'style="top:-5px" ';
    }
    checkHeader += 'onclick="DOTB.email2.addressBook.grid.toggleSelectAll(this.checked);">';
    var colModel =
	    [{
	    	label: checkHeader,
            width: 30,
    ...
    DOTB.email2.addressBook.addressBookDataModel = dataModel;

    var grid = DOTB.email2.addressBook.grid = new YAHOO.widget.ScrollingDataTable("addrSearchGrid", colModel, dataModel, {
    	MSG_EMPTY: "&nbsp;", //DOTB.language.get("Emails", "LBL_EMPTY_FOLDER"),
		dynamicData: true,
		paginator: new YAHOO.widget.Paginator({
			rowsPerPage: 25,
		...
		initialRequest:DOTB.util.paramsToUrl(dataModel.params),
		width:  "560px",
		height: "250px"
    });
	//Override Paging request construction
	grid.set("generateRequest", function(oState, oSelf) {
        ...
        // Build the request
        var ret =
            DOTB.util.paramsToUrl(oSelf.getDataSource().params) +
	...
	grid.clickToggleSelect= function(args) {
		...
		if(targetElement.type == null || targetElement.type != 'checkbox') {
			DOTB.email2.addressBook.grid.toggleSelect(args.target.id);
		}
	}
	...
	grid.reSelectRowsOnRender = function (){
        ...
        for (var i = 0; i < rows.length; i++)
        {
            ...
            var alreadyAdded = DOTB.email2.addressBook.doesEmailAdddressExistInResultTable(emailAddress);
            if(alreadyAdded)
            {
            ...
            else
            {
                ...
                DOTB.email2.addressBook.grid.unselectRow(rows[i]);
            }
        }
	}
    ...
    grid.toggleSelectCheckbox = function(id,checked){
        var row = DOTB.email2.addressBook.grid.getRecord(id);
    ...
    grid.toggleSelect = function(id, checked) {
        ...
        if (!checked)
        {
            DOTB.email2.addressBook.grid.selectRow(row);
            SE.addressBook.insertContactRowToResultTable(id,null)
        } else
        {
            DOTB.email2.addressBook.grid.unselectRow(row);
    ...
    grid.toggleSelectAll = function(checked) {
        rows = DOTB.email2.addressBook.grid.getRecordSet().getRecords();
        for (var i = 0; i < rows.length; i++) {
...
function AddressSearchResultsGridInit()
{
    ...
    var ColumnDefs = [{key:'type',label:app_strings.LBL_EMAIL_ADDRESS_BOOK_ADRRESS_TYPE, width: 60, sortable: true, editor: new YAHOO.widget.RadioCellEditor({radioOptions:typeDdOptions,disableBtns:true})},
	 ...
	 var gridResults = DOTB.email2.addressBook.gridResults = new YAHOO.widget.ScrollingDataTable("addrSearchResultGrid", ColumnDefs, myDataSource, {
