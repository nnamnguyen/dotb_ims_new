...
SE.addressBook = {
    ...
    */
    ...
    composeTo : function(type, waited) {
        var activePanel = DOTB.email2.innerLayout.get("activeTab").get("id")
    ...
    searchContacts : function() {
        ...
        this.grid.getDataSource().sendRequest(DOTB.util.paramsToUrl(this.addressBookDataModel.params),  this.grid.onDataReturnInitializeTable, this.grid);
    },
    ...
    selectContactsDialogue : function(destId) {
        if(!this.contactsDialogue) {
        	...
        	var body = DOTB.util.getAndRemove("contactsDialogueHTML");
        	...
        	var tabContent = DOTB.util.getAndRemove("searchForm");
        	...
        	YAHOO.util.Event.addListener(addListenerFields,"keydown", function(e){
        		if (e.keyCode == 13) {
        			...
        			DOTB.email2.addressBook.searchContacts();
        		}
        	});
        ...
        if (typeof(DOTB.email2.innerLayout) == 'undefined')
            var idx = 0;
        else
        {
            var activePanel = DOTB.email2.innerLayout.get("activeTab").get("id");
	    ...
	    else
	    {
    		...
    		SE.addressBook.grid.getDataSource().sendRequest(DOTB.util.paramsToUrl(SE.addressBook.addressBookDataModel.params),  SE.addressBook.grid.onDataReturnInitializeTable, SE.addressBook.grid);
	    }

	    //Remove any lingering rows in the result set table if the module was closed.
	    SE.addressBook.gridResults.deleteRows(0, DOTB.email2.addressBook.gridResults.getRecordSet().getLength());
	    //Repopulate
    ...
    */
    clearAllEmailAddressFieldsFromResultTable: function () {
        DOTB.email2.addressBook.gridResults.deleteRows(0, DOTB.email2.addressBook.gridResults.getRecordSet().getLength());
        //Unhighlight any rows currently selected if the emails were cleared.
        DOTB.email2.addressBook.grid.toggleSelectAll(false);
        DOTB.email2.addressBook.grid.reSelectRowsOnRender();
    },
    /**
    ...
    populateResulstTableEmailAddresses: function () {
        ...
        for(var k=0;k<emailFields.length;k++)
        {
    		...
    		for (var i=0; i<formatedEmails.length; i++)
    		{
                ...
                DOTB.email2.addressBook.gridResults.addRow({'type':addressType,'name':t_name,'email_address': t_emailAddr,
    ...
    */
    insertContactToResultTable : function(event,address_type) {
        ...
        var rows = DOTB.email2.addressBook.grid.getRecordSet().getRecords();
        for (var i = 0; i < rows.length; i++)
        {
			if (typeof(rows[i]) != "undefined" && rows[i].getData().checked )
			{
                ...
                DOTB.email2.addressBook.grid.selectRow(rows[i]);
                rows[i].setData("selected",true);
			}
        }
        var checkBoxes = DOTB.email2.addressBook.grid.get("element").getElementsByTagName('input');
    ...
    */
    insertContactRowToResultTable : function(rowId, addressType) {
        ...
        DOTB.email2.addressBook.gridResults.addRow({'type':addressType,'name':name,'email_address': data.email,'display_email_address': ea,'bean_id': data.bean_id,'idx' : SE.addressBook.idx});
    },
    /**
    ...
    */
    removeRowFromGridResults : function(rowId,emailAddress)
    {
        ...
        for (var i=0; i < contacts.length; i++)
        {
            ...
            if(data.email_address == emailAddress)
            {
                DOTB.email2.addressBook.gridResults.deleteRow(rec.getId());
                break;
            }
        }

       DOTB.email2.addressBook.toggleSearchRowIcon(rowId,true);
    },
    /**
...
SE.composeLayout = {
    ...
    */
    ...
    _0_yui : function() {
        ...
        var composeTab = new YAHOO.DOTB.ClosableTab({
        ...
        */
        _initComposeOptionTabs : function(idx)
        {
	        ...
	        var tab = new YAHOO.widget.Tab({
				...
				scroll : true,
				content : DOTB.util.getAndRemove("divAttachments" + idx).innerHTML,
	        ...
	        tab = new YAHOO.widget.Tab({
				...
				scroll : true,
				content : DOTB.util.getAndRemove("divOptions" + idx).innerHTML,
				id : "divOptions" + idx,
				active : false
			});
    ...
    resizeEditorSetSignature : function(idx,setSignature)
    {
        ...
        if(setSignature) {
            setTimeout("DOTB.email2.composeLayout.setSignature("+idx+");",250);
        }
    ...
    c1_composeEmail : function(isReplyForward, retry) {
        ...
        if  (!DOTB.util.isTouchScreen() && (typeof(tinyMCE) == 'undefined' || typeof(tinyMCE.settings) == 'undefined')){
    ...
    c0_replyForwardEmail : function(ieId, uid, mbox, type) {
        ...
        if(mbox == 'dotb::Emails') {
            SE.composeLayout.replyForwardObj.dotbEmail = true;
        }
    ...
    applyEmailTemplate : function (idx, id) {
		...
		if(id == '' || id == "0") {
			YAHOO.DOTB.MessageBox.show({
	           title:box_title,
	           msg: box_none_msg,
	           type: 'confirm',
	           fn: function(btn){
	           		if(btn=='no'){return;};
	           		DOTB.email2.composeLayout.processNoneResult(idx, id);},
	           modal:true,
	           scope:this
	       });
	       return;
		...
		YAHOO.DOTB.MessageBox.show({
           ...
           fn: function(btn){
           		if(btn=='no'){return;};
           		DOTB.email2.composeLayout.processResult(idx, id);},
           modal:true,
           scope:this
       });
    },
    ...
    processNoneResult : function(idx, id) {
        ...
        setTimeout("DOTB.email2.composeLayout.setSignature("+idx+");",500);
    },

	processResult : function(idx , id){
        var api = parent.DOTB.App.api;
    ...
    appendEmailTemplateJSON : function() {
        ...
        if(json_objects['email_template_object']['fields']['text_only'] == 1)
        {
        	...
        	DOTB.email2.composeLayout.renderTinyMCEToolBar(idx, 1);
        }
        else
        {
        	...
        	DOTB.email2.composeLayout.renderTinyMCEToolBar(idx, 0);
        }
    ...
    setSignature : function(idx)
    {
        ...
        else
        {
            ...
            else if (DOTB.email2.userPrefs.signatures.signature_prepend == 'true') // Prepend
            {
            	//bug 48285
                var newHtml = html;
    ...
    addDocumentField : function(idx) {
        ...
        documentCup.innerHTML = "<input type='hidden' name='document" + idx + index + "' id='document" + idx + index + "' />" +
                ...
                "<img src='index.php?entryPoint=getImage&themeName=" + DOTB.themes.theme_name + "&imageName=id-ff-select.png' ></button>" +
                // remove button
                "<button class='button lastChild' type='button' name='documentRemove" + idx + index + "' id='documentRemove" + idx + index + "'" +
                 ...
                 "<img src='index.php?entryPoint=getImage&themeName=" + DOTB.themes.theme_name + "&imageName=id-ff-clear.png' ></button>" +
                "<br/>";
    ...
    /**
     * Adds a DotbDocument to an outbound email.  Action occurs in a popup window displaying a ListView from the Documents module
    ...
    addTemplateAttachmentField : function(idx) {
        ...
        var out = "<div id='templateAttachmentCup" + idx + index + "'>" +
				...
				"<img src='index.php?entryPoint=getImage&themeName=" + DOTB.themes.theme_name + "&imageName=minus.gif' " +
					"style='cursor:pointer' align='absmiddle' onclick='DOTB.email2.composeLayout.deleteTemplateAttachmentField(\"" +
					idx + "\",\"" + index + "\");'/>" +
				// file icon
				"<img src='index.php?entryPoint=getImage&themeName=" + DOTB.themes.theme_name + "&imageName=attachment.gif' " + "align='absmiddle' />" +
    ...
    sendEmail : function(idx, isDraft) {
        ...
        if( typeof(DOTB.email2.composeLayout.outboundAccountErrors[obAccountID]) != 'undefined' && !isDraft )
        {
            DOTB.showMessageBox(app_strings.LBL_EMAIL_ERROR_DESC, DOTB.email2.composeLayout.outboundAccountErrors[obAccountID], 'alert');
            return false;
        }
        ...
        if (!DOTB.collection.prototype.validateTemSet(composeOptionsFormName, 'team_name')) {
        ...
        DOTB.showMessageBox(title, app_strings.LBL_EMAIL_ONE_MOMENT);
        html = html.replace(/&lt;/ig, "dotbLessThan");
        html = html.replace(/&gt;/ig, "dotbGreaterThan");
		...
		var teamIdsArray = DOTB.collection.prototype.getTeamIdsfromUI(composeOptionsFormName, 'team_name');
		var selTeamIdsArray = DOTB.collection.prototype.getSelectedTeamIdsFromUI(composeOptionsFormName, 'team_name');
		form.teamIds.value = teamIdsArray.join(",");
        form.selectedTeam.value = selTeamIdsArray.join(",");
        form.primaryteam.value = DOTB.collection.prototype.getPrimaryTeamidsFromUI(composeOptionsFormName, 'team_name');
    ...
    */
    ...
    /**
     * Takes an array of objects that contain the filename and GUID of a Note (attachment or Dotb Document) and applies the values to the compose screen.  Valid use-cases are applying an EmailTemplate or resuming a Draft Email.
     */
    ...
    setComposeOptions : function(idx) {
        ...
        if (addressFrom.options.length <= 0) {
	        ...
	        for (id = 0 ; id < fromAccountOpts.length ; id++) {
	              ...
	              if(key == DOTB.default_inbound_accnt_id)
	              	is_default = true;
	              var opt = new Option(display, key);
	              addressFrom.options.add(opt);
    ...
    /**
     * After compose screen is rendered, async call to get email body from Dotb
     */
    replyForwardEmailStage2 : function() {
        ...
        var dotbEmail = (SE.composeLayout.replyForwardObj.dotbEmail) ? '&dotbEmail=true' : "";
        ...
        var sendType = type;
        AjaxObject.startRequest(callbackReplyForward, urlStandard + "&composeType=" + type + dotbEmail);
    },
...
SE.util = {
    ...
    generateMboxPath : function(str) {
        ...
        if(ex.length > 1) {
            ...
            /* we have a Dotb folder GUID - do nothing */
            return str;
        }
    ...
    /**
     * returns a DOTB GUID by navigating the DOM tree a few moves backwards
