	...
	se.e2Layout = {
    	getInnerLayout : function(rows) {
        	...
        	se.complexLayout.on("render", function(){
        		var height = DOTB.email2.innerLayout.get("element").clientHeight - 30;
				DOTB.email2.innerLayout.get("activeTab").get("contentEl").parentNode.style.height = height + "px";
				DOTB.email2.listViewLayout.set("height", height);
				DOTB.email2.listViewLayout.render();
        	});
        ...
        init : function(){
            ...
            var foldersImg = '<img src="index.php?entryPoint=getImage&themeName='+DOTB.themes.theme_name+'&imageName=icon_email_folder.gif" align=absmiddle border=0 alt="'+app_strings.LBL_EMAIL_FOLDERS+'">';
            ...
            var tabContent = DOTB.util.getAndRemove("searchTab");
            var searchImg = '<img src="index.php?entryPoint=getImage&themeName='+DOTB.themes.theme_name+'&imageName=Search.gif" align=absmiddle border=0 alt="'+app_strings.LBL_SEARCH+'">';
            ...
            searchTab.on("activeChange", function(o){ 
            	if (o.newValue) 
            	{
	               ...
	               YAHOO.util.Event.addListener('data_parent_type_search', 'change',function(){ 
	                   DOTB.email2.composeLayout.enableQuickSearchRelate(null,parentSearchArgs) });
	               ...
	               for(var i=0; i < addKeyPressFields.length;i++)
	               {
    	               YAHOO.util.Event.addListener(window.document.forms['advancedSearchForm'].elements[addKeyPressFields[i]],"keydown", function(e){
                    		if (e.keyCode == 13) {
                    			...
                    			DOTB.email2.search.searchAdvanced();
                    		}
            	       });
	               }
            ...
            var resizeTabBody = function() {
            	var height = DOTB.email2.leftTabs.get("element").clientHeight - 30;
				DOTB.email2.leftTabs.get("activeTab").get("contentEl").parentNode.style.height = height + "px";
            }
            resizeTabBody();
        ...
        setPreviewPanel: function(rows) {
        	if (rows) {
            	DOTB.email2.listViewLayout.getUnitByPosition("right").set("width", 0);
            	DOTB.email2.listViewLayout.getUnitByPosition("bottom").set("height", 250);
            	Dom.get("listRight").innerHTML = "";
            	Dom.get("listBottom").innerHTML = "<div id='_blank' />";
            } else {
            	DOTB.email2.listViewLayout.getUnitByPosition("bottom").set("height", 0);
            	DOTB.email2.listViewLayout.getUnitByPosition("right").set("width", 250);
