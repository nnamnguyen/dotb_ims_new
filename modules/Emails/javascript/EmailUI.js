...
SE.accounts = {
    ...
    deleteOutbound : function(obi) {
        ...
        if(obi.match(/^(add|line|sendmail)+/)) {
        	...
        	DOTB.showMessageBox(app_strings.LBL_EMAIL_DELETING_OUTBOUND, app_strings.LBL_EMAIL_ONE_MOMENT);
    ...
    _setupInboundAccountTable: function()
    {
    	...
    	if(!this.inboundAccountsSettingsTable)
    	{
    		  this.customImageFormatter = function(elLiner, oRecord, oColumn, oData) {
   					...
   					if(!is_group)
   					{
    		  			if(oColumn.key == 'edit' && !DOTB.email2.disableAccountEdit)
    		  			{
    		  				clckEvent = "DOTB.email2.accounts.getIeAccount('"+ oRecord.getData('id') +"')";
    		  				imgSrc = 'index.php?entryPoint=getImage&amp;themeName=Dotb&amp;imageName='+oColumn.key+'_inline.gif';
    		  			}
    		  			else if(oColumn.key == 'delete')
    		  			{
    		  				clckEvent = "DOTB.email2.accounts.deleteIeAccount('"+ oRecord.getData('id') +"','" + oRecord.getData('group_id') +"')";
    		  				imgSrc = 'index.php?entryPoint=getImage&amp;themeName=Dotb&amp;imageName='+oColumn.key+'_inline.gif';
    		  			}
			...
			this.inboundAccountsSettingsTable.subscribe("checkboxClickEvent", function(oArgs){
	            ...
	            var oColumn = this.getColumn(elCheckbox);
	          	if(oColumn.key == 'is_active')
	          	{
	            	...
	            	if(isGroupFolder)
	            	    DOTB.email2.folders.updateSubscriptions();
	            	else
    	            	DOTB.email2.folders.setFolderSelection();
    		...
    		this.inboundAccountsSettingsTable.subscribe("radioClickEvent", function(oArgs){
	            ...
	            var oColumn = this.getColumn(elRadio);
	          	if(oColumn.key == 'is_default')
	          	{
	          		...
	          		{
						DOTB.default_inbound_accnt_id = t_id; //Set in the global space for access during compose
    	...
    	if(!this.outboundAccountsSettingsTable)
    	{
	        this.obImageFormatter = function(elLiner, oRecord, oColumn, oData) {
   					...
   					if(isEditable)
   					{
    		  			if(oColumn.key == 'edit' && !DOTB.email2.disableAccountEdit)
    		  			{
    		  				clckEvent = "DOTB.email2.accounts.editOutbound('"+ oRecord.getData('id') +"')";
    		  				imgSrc = 'index.php?entryPoint=getImage&amp;themeName=Dotb&amp;imageName='+oColumn.key+'_inline.gif';
    		  			}
    		  			else if(oColumn.key == 'delete' && type == 'user')
    		  			{
    		  				clckEvent = "DOTB.email2.accounts.deleteOutbound('"+ oRecord.getData('id')+"')";
    		  				imgSrc = 'index.php?entryPoint=getImage&amp;themeName=Dotb&amp;imageName='+oColumn.key+'_inline.gif';
    		  			}
    		  			if(imgSrc != '')
	            		    elLiner.innerHTML = '<img onclick="'+clckEvent+'" src="'+imgSrc+'" align="absmiddle" border="0"/>';
			...
			this.messageDisplay = function(elLiner, oRecord, oColumn, oData) {
                    ...
                    if(DOTB.email2.composeLayout.outboundAccountErrors == null)
    		  			    DOTB.email2.composeLayout.outboundAccountErrors = {};
    		        ...
    		        var id = oRecord.getData('id');
   					...
   					if(message != '')
   					{
    		  			...
    		  			DOTB.email2.composeLayout.outboundAccountErrors[id] = message;
   					}
   					else
   					{
   					    if(typeof(DOTB.email2.composeLayout.outboundAccountErrors[id]) != 'undefined' )
    		  			    delete DOTB.email2.composeLayout.outboundAccountErrors[id];
   					}
	        };
    ...
    showEditInboundAccountDialogue : function(clear) {
        ...
        DOTB.util.setEmailPasswordDisplay('email_password', clear == false);
    },
    ...
    */
    ...
    showAddSmtp : function() {
        ...
        if ( !DOTB.smtpButtonGroup ) {
            DOTB.smtpButtonGroup = new YAHOO.widget.ButtonGroup("smtpButtonGroup");
            DOTB.smtpButtonGroup.subscribe('checkedButtonChange', function(e)
            {
                DOTB.email2.accounts.changeEmailScreenDisplay(e.newValue.get('value'));
        ...
        DOTB.email2.accounts.changeEmailScreenDisplay('other');
    ...
    changeEmailScreenDisplay : function(smtptype, isSystemAccount)
    {
        ...
        DOTB.email2.accounts.smtp_authenticate_field_display();
        DOTB.email2.accounts.smtp_setDefaultSMTPPort()
    },
    ...
    */
    fillInboundGmailDefaults: function () {
        ...
        document.forms['ieAccount'].elements['protocol'].value = "imap";
        DOTB.email2.accounts.setPortDefault();
        DOTB.util.setEmailPasswordDisplay('email_password', false);
    },
    /**
    ...
    checkOutBoundSelection: function() {
    	...
    	var errorAccounts = DOTB.email2.composeLayout.outboundAccountErrors;
    ...
    rebuildMailerOptions : function() {
        ...
        //Get the available dotb mailers
 ...
 // handle SMTP selection
    ...
    checkOutboundSettings: function() {
        ...
        if(isError) {
            DOTB.showMessageBox(mod_strings.ERR_MISSING_REQUIRED_FIELDS, errorMessage, 'alert');
            return false;
        } else {
            return true;
        }
    ...
    testOutboundSettings: function() {
    	...
    	if (trim(fromAddress) == "") {
            ...
            errorMessage += app_strings.LBL_EMAIL_SETTINGS_FROM_TO_EMAIL_ADDR + "<br/>";
            DOTB.showMessageBox(mod_strings.ERR_INVALID_REQUIRED_FIELDS, errorMessage, 'alert');
            return false;
        }

        //Hide the dialogue and show an in progress indicator.
        SE.accounts.testOutboundDialog.hide();
        DOTB.showMessageBox(app_strings.LBL_EMAIL_PERFORMING_TASK, app_strings.LBL_EMAIL_ONE_MOMENT, 'plain');
    ...
    saveIeAccount : function() {
            ...
            'validateSMTPCreds':true}) )
        {
            ...
            DOTB.showMessageBox(app_strings.LBL_EMAIL_IE_SAVE, app_strings.LBL_EMAIL_ONE_MOMENT);
    ...
    getIeAccount : function(ieId) {
        ...
        DOTB.showMessageBox(app_strings.LBL_EMAIL_SETTINGS_RETRIEVING_ACCOUNT, app_strings.LBL_EMAIL_ONE_MOMENT);
    ...
    setupDDTarget : function(node) {
    	...
    	if (node.data.origText != DOTB.language.get("Emails", "LNK_MY_INBOX") &&
			node.data.origText != DOTB.language.get("Emails", "LNK_MY_DRAFTS") &&
			node.data.origText != DOTB.language.get("Emails", "LNK_SENT_EMAIL_LIST")) {

    		node.ddTarget = new DOTB.email2.folders.folderDD("ygtvcontentel" + num);
    	}
    	else if (node.data.origText == DOTB.language.get("Emails", "LNK_MY_INBOX")){
    ...
    rebuildFolderList : function() {
        DOTB.showMessageBox(app_strings.LBL_EMAIL_REBUILDING_FOLDERS, app_strings.LBL_EMAIL_ONE_MOMENT);
...
SE.contextMenus = {
     ...
     * @param string folder
     */
    _archiveToDotb : function(uids, ieId, folder) {
        var ser = '';
     ...
     * Archives from context menu
     */
    archiveToDotb : function(menuItem) {
        ...
        SE.contextMenus._archiveToDotb(uids, data.ieId, data.mbox);
    },
    ...
    /**
     * generic call API to apply a flag to emails on the server and on dotb
    ...
    markEmail : function(type) {
        ...
        if(type == 'read' || type == 'deleted') {
           ...
           //bug# 40257 - adding if condition to check the ieId (Id of a dotb mail box) , which would be null for search email results
            if(ieId){
    ...
    showEmailDetailViewInPopup : function(ieId,uid, folder) {
        DOTB.showMessageBox(app_strings.LBL_EMAIL_RETRIEVING_RECORD, app_strings.LBL_EMAIL_ONE_MOMENT);
...
SE.contextMenus.dv = {
    archiveToDotb : function(contextMenuId) {

        SE.contextMenus._archiveToDotb(uids, ieId, folder);
    },
...
SE.detailView = {
    consumeMetaDetail: function(ret) {
        ...
        var displayTemplate = new YAHOO.DOTB.Template(SE.templates['displayOneEmail']);
    ...
    emailDelete : function(ieId, uid, mbox) {
       DOTB.showMessageBox(app_strings.LBL_EMAIL_DELETING_MESSAGE, app_strings.LBL_EMAIL_ONE_MOMENT);
    ...
    getEmailPreview : function() {
    	var row = DOTB.email2.listView.currentRow;
    	var data = row.getData();
	    if (data && !(DOTB.email2.contextMenus.emailListContextMenu.cfg.getProperty("visible") && data.type =='draft')) {
	       var setRead = (data['seen'] == 0) ? true : false;
		   DOTB.email2.listView.markRead(DOTB.email2.listView.currentRowIndex, row);
		   DOTB.email2.detailView.populateDetailView(data['uid'], data['mbox'], data['ieId'], setRead, DOTB.email2.previewLayout);
	    }
    },

    /**
     * Imports one email into Dotb
     */
    ...
    populateDetailView : function(uid, mbox, ieId, setRead, destination) {
    	DOTB.email2.util.clearHiddenFieldValues('emailUIForm');
        ...
        if(destination == SE.innerLayout) {
        	...
        	targetDiv = new YAHOO.DOTB.ClosableTab({
	        		label: loadingSprite,
					scroll : true,
					content : "",
					active : true
    ...
    {
    	if(mboxStr.substring(0,7) == 'dotb::') {
            // display an email from Dotb
            document.getElementById('emailUIAction').value = 'getSingleMessageFromDotb';
        } else {
    ...
    populateDetailViewMultiple : function(uids, mbox, ieId, setRead) {
        DOTB.showMessageBox(app_strings.LBL_EMAIL_RETRIEVING_MESSAGE, app_strings.LBL_EMAIL_ONE_MOMENT);
        SE.util.clearHiddenFieldValues('emailUIForm');
        ...
        if(mboxStr.substring(0,7) == 'dotb::') {
            // display an email from Dotb
            document.getElementById('emailUIAction').value = 'getMultipleMessagesFromDotb';
        } else {
            // display an email from an email server
    ...
    quickCreate : function(module, ieId, uid, mailbox) {
        ...
        if(ieId == null || ieId == "null" || mailbox == 'dotb::Emails') {
            get += "&dotbEmail=true";
        }
    ...
    saveQuickCreate : function(action) {
        ...
        if (check_form('form_EmailQCView_' + qcd.qcmodule)) {
	        ...
	        var accountType = '&dotbEmail=true';
	        if (qcd.ieId != 'null' && qcd.mbox != 'dotb::Emails') {
	        ...
	        YAHOO.util.Connect.setForm(formObject);
	        DOTB.showMessageBox('Saving', app_strings.LBL_EMAIL_ONE_MOMENT);
    ...
    viewPrintable : function(ieId, uid, mailbox) {
    	if(mailbox == 'dotb::Emails') {
            // display an email from Dotb
            var emailUIAction = '&emailUIAction=getSingleMessageFromDotb';
        } else {
            // display an email from an email server
...
SE.folders = {
    ...
    _createFolderId : function(node) {
        ...
        if(node.data.ieId) {
            /* we have a local Dotb folder */
            if(node.data.ieId == 'folder') {
                ret = "dotb::" + node.data.id; // FYI: folder_id is also stored in mbox field
    ...
    checkEmailAccountsSilent : function(showOverlay) {
        ...
        if(AjaxObject.currentRequestObject.conn == null) {
            if(showOverlay) {
                DOTB.showMessageBox(app_strings.LBL_EMAIL_CHECKING_NEW,
    ...
    startEmailAccountCheck : function() {
       ...
       if(!AjaxObject.requestInProgress()) {
            DOTB.showMessageBox(app_strings.LBL_EMAIL_ONE_MOMENT, app_strings.LBL_EMAIL_CHECKING_NEW, 'progress');
     ...
     */
     startEmailCheckOneAccount : function(ieId, synch) {
            ...
            DOTB.showMessageBox(app_strings.LBL_EMAIL_CHECKING_NEW, app_strings.LBL_EMAIL_CHECKING_DESC, 'progress');
    ...
    emptyTrash : function() {
        ...
        DOTB.showMessageBox(app_strings.LBL_EMAIL_EMPTYING_TRASH, app_strings.LBL_EMAIL_ONE_MOMENT);
    ...
    clearCacheFiles : function(ieId) {
        SE.contextMenus.frameFoldersContextMenu.hide();
        DOTB.showMessageBox(app_strings.LBL_EMAIL_CLEARING_CACHE_FILES, app_strings.LBL_EMAIL_ONE_MOMENT);
    ...
    /**
     * Recursively removes nodes from the TreeView of type Dotb (data.ieId = 'folder')
     */
    removeDotbFolders : function() {
        var tree = SE.tree;
    ...
    rebuildFolders : function(silent) {
      if (!silent) DOTB.showMessageBox(app_strings.LBL_EMAIL_REBUILDING_FOLDERS, app_strings.LBL_EMAIL_ONE_MOMENT);
    ...
    /**
     * Updates TreeView with Dotb Folders
     */
    setDotbFolders : function(delay) {
        this.removeDotbFolders();
		AjaxObject.startRequest(callbackRefreshDotbFolders, urlStandard + "&emailUIAction=refreshDotbFolders");
    },

    /**
     * Takes async data object and creates the dotb folders in TreeView
     */
    setDotbFoldersEnd : function(o) {
        var root = SE.tree.getRoot();
        addChildNodes(root, {nodes: o});
        SE.accounts.renderTree();
        //If nothing is loaded in the grid, load "My Inbox"
        if (SE.grid.params.ieId == "undefined") {
        	SE.listView.populateListFrameDotbFolder({data: o[0]}, o[0].id, false);
        }
    },
    ...
    folderAdd : function() {
        ...
        if(node != null && node.data) {
            DOTB.showMessageBox(app_strings.LBL_EMAIL_FOLDERS_ADD_DIALOG_TITLE,
    ...
    folderAddRegisterEnter : function() {
    	this.enterKeyListener = new YAHOO.util.KeyListener(YAHOO.util.Dom.get("dotb-message-prompt"),
    ...
    folderAddXmlCall : function(name) {
        ...
        if(type == 'imap') {
            ...
            if(SE.folders.isUniqueFolderName(name)) {
                ...
                AjaxObject.startRequest(callbackFolderSave, urlStandard + '&emailUIAction=saveNewFolder&folderType=dotb&' + post);
            } else {
    ...
    /**
     * Removes either an IMAP folder or a Dotb Folder
     */
        ...
        if(confirm(app_strings.LBL_EMAIL_FOLDERS_DELETE_CONFIRM)) {
            ...
            if(parentNode != null && parentNode.data) {
                if(parentNode.data.mbox == 'INBOX' || parentNode.data.id == 'Home') {
                    DOTB.showMessageBox(app_strings.LBL_EMAIL_ERROR_GENERAL_TITLE, app_strings.LBL_EMAIL_FOLDERS_CHANGE_HOME, 'alert');
                    return;
                }
                ...
                if(parentNode.data.ieId != 'folder') {
                    ...
                    // delete a dotb folder
                    post = "&folderType=dotb&folder_id=" + parentNode.data.id;
                }
                DOTB.showMessageBox("Deleting folder", app_strings.LBL_EMAIL_ONE_MOMENT);
    ...
    folderRename : function() {
        ...
        if(node != null) {
			...
			DOTB.showMessageBox(app_strings.LBL_EMAIL_FOLDERS_RENAME_DIALOG_TITLE + " - " + node.data.text,
     ...
     * @param string spacer Defined in app_strings, visual separator b/t Dotb and Remote folders
     */
    ...
    */
    handleClick : function(o) {
    	var node = o.node;
        //If the click was on a dotb folder
    	if (node.data.ieId == "folder") {
            SE.listView.populateListFrameDotbFolder(node, node.id, false);
        }
        else {
    ...
    */
    handleRightClick : function(e) {
		...
		//Get the Tree Node
		var node = DOTB.email2.tree.getNodeByElement(YAHOO.util.Event.getTarget(e));
		var menu = DOTB.email2.contextMenus.frameFoldersContextMenu;
    ...
    */
    ...
    submitFolderRename : function(newName) {
        ...
        if(SE.folders.isUniqueFolderName(newName)) {
            DOTB.showMessageBox(app_strings.LBL_EMAIL_MENU_RENAMING_FOLDER, app_strings.LBL_EMAIL_ONE_MOMENT);
            if (node.data.ieId == "folder") {
                //Dotb Folder
    ...
    synchronizeAccounts : function() {
        if(confirm(app_strings.LBL_EMAIL_SETTINGS_FULL_SYNC_WARN)) {
            DOTB.showMessageBoxModal(app_strings.LBL_EMAIL_SETTINGS_FULL_SYNC, app_strings.LBL_EMAIL_ONE_MOMENT + "<br>&nbsp;<br>" + app_strings.LBL_EMAIL_COFFEE_BREAK);
            AjaxObject.startRequest(callbackFullSync, urlStandard + '&emailUIAction=synchronizeEmail');
        }
    },

    /**
     * Updates user's folder subscriptsion (Dotb only)
    ...
    updateSubscriptions : function() {
        ...
        var group_folders = DOTB.email2.folders.retrieveGroupFolderSubscriptions();
    ...
    /**
     * Updates user's group folder subscriptsion (Dotb only)
...
SE.listView = {
    ...
    /**
     * Like populateListFrame(), but specifically for DotbFolders since the API is radically different
     */
    populateListFrameDotbFolder : function(node, folderId, forceRefresh) {
        SE.innerLayout.selectTab(0);
        Dom.get('_blank').innerHTML = "";
        SE.grid.params['emailUIAction'] = 'getMessageListDotbFolders';
    ...
    /**
     * Moves email(s) from a folder to another, from IMAP/POP3 to Dotb and vice-versa
    ...
    moveEmails : function(sourceIeId, sourceFolder, destinationIeId, destinationFolder, emailUids, selectedRows) {
        if(destinationIeId != 'folder' && sourceIeId != destinationIeId) {
            DOTB.showMessageBox(app_strings.LBL_EMAIL_ERROR_MOVE_TITLE, app_strings.LBL_EMAIL_ERROR_MOVE);
        } else {
            ...
            if (destinationIeId == 'folder' && sourceFolder != 'dotb::Emails') {
     ...
     */
    handleClick : function(o) {
        DOTB.email2.grid.clearTextSelection();

        var el = DOTB.email2.grid.getSelectedRows();

        //Load an email preview only if a single record is selected.  For multiple selections do nothing.
    	if ( el.length == 1)
    	{
    	   var rowId = el[0];
    	   DOTB.email2.listView.currentRow = DOTB.email2.grid.getRecord(rowId);
    	   DOTB.email2.listView.currentRowIndex = rowId;
    	   clearTimeout(DOTB.email2.detailView.previewTimer);
    	   DOTB.email2.detailView.previewTimer = setTimeout("DOTB.email2.detailView.getEmailPreview();", 500);
    	}
    },
    ...
    refreshGrid : function() {
        SE.grid.getDataSource().sendRequest(
            DOTB.util.paramsToUrl(SE.grid.params),
...
SE.settings = {
    ...
    createSignature : function(record, the_user_id) {
        var URL = "index.php?module=Users&action=PopupSignature&dotb_body_only=true";
        if(record != "") {
            URL += "&record="+record;
        }
        if(the_user_id != "") {
    ...
    deleteSignature : function() {
        if(confirm(app_strings.LBL_EMAIL_CONFIRM_DELETE_SIGNATURE)) {
            DOTB.showMessageBox(app_strings.LBL_EMAIL_IE_DELETE_SIGNATURE, app_strings.LBL_EMAIL_ONE_MOMENT);
    ...
    saveOptionsGeneral :  function(displayMessage) {
        ...
        if (!DOTB.collection.prototype.validateTemSet('formEmailSettingsGeneral', 'team_name')) {
    ...
    evalTeamData : function() {
    	if(isAdmin) {
	    	var teamSettingsFolderString = DOTB.email2.composeLayout.teamsSettingsFolder;
	    	DOTB.util.evalScript(teamSettingsFolderString);
    	}
