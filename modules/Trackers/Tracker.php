...
class Tracker extends DotbBean
{
     ...
     * The number of items to return is specified in dotb_config['history_max_viewed']
     * @param uid user_id
    ...
    function get_recently_viewed($user_id, $modules = '')
    {
        if(empty($_SESSION['breadCrumbs'])) {
            ...
            } elseif (!empty($GLOBALS['dotb_config']['history_max_viewed'])) {
                $history_max_viewed = $GLOBALS['dotb_config']['history_max_viewed'];
            } else {
    ...
    function create_tables(){
        ...
        if(defined('TEMPLATE_URL'))$path = DotbTemplateUtilities::getFilePath($path);
        require($path);
