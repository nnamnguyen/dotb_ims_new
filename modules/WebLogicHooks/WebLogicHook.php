...
class WebLogicHook extends DotbBean implements RunnableSchedulerJob
{
    public $id;
    public $name;
    ...
    /**
     * Dispatch request.
     * @param DotbBean $seed a bean that fired event
     * @param string $event event name
     * @param array $arguments event arguments 
     * @param string $id web logic hook id
     */
    public function dispatchRequest(DotbBean $seed, $event, $arguments, $id)
    {
        $this->retrieve($id);
        if (empty($this->id)) {
            return;
    ...
    private function formatRequestData(DotbBean $bean, $event, array $arguments)
    {
        global $current_user;
        ...
        if (!DotbACL::moduleSupportsACL($bean->webhook_target_module) || $bean->ACLAccess('detail')) {
            ...
            foreach ($fieldList as $fieldName => $properties) {
                ...
                $field = $sfh->getDotbField($fieldType);
