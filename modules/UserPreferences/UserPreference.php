...
class UserPreference extends DotbBean
{
    public $db;
    ...
    public function getPreference($name, $category = 'global')
    {
        ...
        if (!isset($cachedPrefs) || (!empty($_SESSION['unique_key']) && $_SESSION['unique_key'] != $dotb_config['unique_key'])) {
        ...
        // check to see if a default preference ( i.e. $dotb_config setting ) exists for this value )
        // if so, return it
    ...
    public function getDefaultPreference($name, $category = 'global')
    {
        ...
        if ( $name == 'datef' ) // Use default date format from Admin panel (Bug57252)
            return $dotb_config['default_date_format'];
        if ( $name == 'timef' )  // Use default time format from Admin panel (Bug57252)
            return $dotb_config['default_time_format'];

        // First check for name matching $dotb_config variable
        if ( isset($dotb_config[$name]) )
            return $dotb_config[$name];

        // Next, check to see if it's one of the common problem ones
        if ( isset($dotb_config['default_'.$name]) )
            return $dotb_config['default_'.$name];

        if ( $name == 'email_link_type' )
            return $dotb_config['email_default_client'];
    ...
    /**
     * Set preference by name and category. Saving will be done in utils.php -> dotb_cleanup
     *
     * @param string $name name of the preference to retreive
    ...
    /**
     * Loads preference by category from database. Saving will be done in utils.php -> dotb_cleanup
     *
     * @param string $category name of the category to retreive, defaults to global scope
    ...
    public function loadPreferences($category = 'global')
    {
        ...
        if (!empty($user->id) && (!isset($cachedPrefs)
                || (!empty($_SESSION['unique_key']) && $_SESSION['unique_key'] != $dotb_config['unique_key']))
        ) {
    ...
    protected function storeToCache($value, $category = null)
    {
        ...
        $ttl = DotbConfig::getInstance()->get('oauth2.max_session_lifetime', ini_get('session.gc_maxlifetime'));
    ...
    {
        $cacheKey = $this->_userFocus->id . '_PREFERENCES';
        //DotbCache has a mem layer that ensures we only actually load from external cache once per request,
    ...
    {
        global $dotb_config, $db, $timedate, $current_user;
        ...
        if(!empty($user) && $this->loadPreferences('global')) {
            ...
            if(empty($timeFormat)) $timeFormat = $dotb_config['default_time_format'];
            if(empty($dateFormat)) $dateFormat = $dotb_config['default_date_format'];
     ...
     * dotb_cleanup if a setPreference has been called during one round trip.
     *
    ...
    public function resetPreferences($category = null)
    {
        ...
        if (!$category) {
            if (!empty($_COOKIE['dotb_user_theme']) && !headers_sent()) {
                setcookie('dotb_user_theme', '', time() - 3600); // expire the dotb_user_theme cookie
            }
        ...
        $unset_value = false )
    {
        ...
        if(!is_admin($current_user)){
            dotb_die('only admins may call this function');
        }
