...
class ForecastsFilterApi extends FilterApi
{
    ...
    {
        if (!DotbACL::checkAccess('Forecasts', $acl)) {
            throw new DotbApiExceptionNotAuthorized('No access to view records for module: Forecasts');
        }
    ...
    protected function createFilter(ServiceBase $api, $user_id, $timeperiod_id, $forecast_type)
    {
        ...
        if ($user_id == false) {
            ...
            if (is_null($user) || is_null($user->id)) {
                throw new DotbApiExceptionInvalidParameter('Provided User is not valid');
            }
        ...
        if (is_null($tp) || is_null($tp->id)) {
            throw new DotbApiExceptionInvalidParameter('Provided TimePeriod is not valid');
        }
