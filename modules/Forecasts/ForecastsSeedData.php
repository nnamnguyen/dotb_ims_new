...
class ForecastsSeedData
{
        ...
        foreach ($timeperiods as $timeperiod_id => $timeperiod) {
            ...
            foreach ($commit_order as $commit_type_array) {
                ...
                if ($commit_type_array[1] === 'Direct') {
                    ...
                    $quota->amount = DotbMath::init()->exp('?*?', array($totals['amount'], $ratio[$key]))->result();
                    ...
                    if ($totals['included_opp_count'] > 0) {
                        $forecast->opp_weigh_value = DotbMath::init()->setScale(0)->exp(
                            '(?/?)/?',
                    ...
                    $forecast->best_case = DotbMath::init()->exp('(?+?)/?', array($totals['best_case'], $totals['won_best'], $ratio[$key]))->result();
                    $forecast->worst_case = DotbMath::init()->exp('(?+?)/?', array($totals['worst_case'], $totals['won_worst'], $ratio[$key]))->result();
                    $forecast->likely_case = DotbMath::init()->exp('(?+?)/?', array($totals['amount'], $totals['won_amount'], $ratio[$key]))->result();
                    ...
                    $forecast->calculatePipelineData(
                        DotbMath::init()->exp('?/?', array($totals['includedClosedAmount'], $ratio[$key]))->result(),
                    ...
                    if ($totals['included_opp_count'] > 0) {
                        $forecast2->opp_weigh_value = DotbMath::init()->setScale(0)->exp(
                            '?/?',
                    ...
                    $forecast2->best_case = DotbMath::init($totals['best_case'])->add($totals['won_best'])->result();
                    $forecast2->worst_case = DotbMath::init($totals['worst_case'])->add($totals['won_worst'])->result();
                    $forecast2->likely_case = DotbMath::init($totals['amount'])->add($totals['won_amount'])->result();
                    ...
                    $quota->currency_id = -99;
                    $quota->amount = DotbMath::init($totals['quota'], 6)->mul($ratio[$key])->result();
                    $quota->amount_base_currency = $quota->amount;
                    $quota->committed = 1;
                    $quota->save();
    ...
    protected static function commitRepItems($user_id, $timeperiod, $forecast_by)
    {
        ...
        $bean->$link_name->buildJoinDotbQuery($sq, array('joinTableAlias' => 'account'));
