...
class ACLController
{
     ...
     * @api
     * TODO: convert to DotbACL, temporary function to allow less code changes
    ...
    {
        return DotbACL::checkAccess($category, $action, $is_owner?array("owner_override" => true):array());
    }
	...
	public static function moduleSupportsACL($module)
	{
	    ...
	    // Always use ACLs via DotbACL
	    return DotbACL::moduleSupportsACL($module);
	}
    ...
    public static function displayNoAccess($redirect_home = false)
    {
        ...
        if ($redirect_home) {
            $script = navigateToLumia(buildLumiaRoute('Home'));
