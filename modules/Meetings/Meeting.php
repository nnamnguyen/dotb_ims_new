<?php
    ...
    class Meeting extends DotbBean {
        // Stored fields
        var $id;
        var $date_entered;
        var $date_modified;
        ...
        public function save($check_notify = false)
        {
            ...
            if (!empty($this->type) && $this->type != 'Dotb' ) {
            ...
            if ( isset($api) && is_a($api,'WebMeeting') && empty($this->in_relationship_update) ) {
                ...
                if ( $response['success'] == TRUE ) {
                    ...
                    // DotbApplication::appendErrorMessage($GLOBALS['app_strings']['ERR_EXTERNAL_API_SAVE_FAIL']);
            ...
            if ($this->update_vcal) {
                ...
                vCal::cache_dotb_vcal($assigned_user);
                if ($this->assigned_user_id != $GLOBALS['current_user']->id) {
                    vCal::cache_dotb_vcal($current_user);
                }
            }
        ...
        function fill_in_additional_detail_fields() {
            ...
            global $timedate;
            $this->time_meridiem = $timedate->AMPMMenu('', $this->time_start, 'onchange="DotbWidgetScheduler.update_time();"');
            $hours_arr = array ();
            $num_of_hours = 13;
            $start_at = 1;
            ...
            if ( !empty($this->id) && !empty($this->type) && $this->type != 'Dotb' && !empty($this->join_url) ) {
                // It's an external meeting
                global $mod_strings;

                $meetingLink = '';
                if ($GLOBALS['current_user']->id == $this->assigned_user_id ) {
                    $meetingLink .= '<a href="index.php?module=Meetings&action=JoinExternalMeeting&meeting_id='.$this->id.'&host_meeting=1" target="_blank">'.DotbThemeRegistry::current()->getImage("start_meeting_inline", 'border="0" ', 18, 19, ".png", translate('LBL_HOST_EXT_MEETING',$this->module_dir)).'</a>';
                }

                $meetingLink .= '<a href="index.php?module=Meetings&action=JoinExternalMeeting&meeting_id='.$this->id.'" target="_blank">'.DotbThemeRegistry::current()->getImage("join_meeting_inline", 'border="0" ', 18, 19, ".png", translate('LBL_JOIN_EXT_MEETING',$this->module_dir)).'</a>';
        ...
        function get_list_view_data() {
            ...
            if($this->status == "Planned") {
                ...
                if(empty($action))
                    $action = "index";
                $setCompleteUrl = "<a id='{$this->id}' onclick='DOTB.util.closeActivityPanel.show(\"{$this->module_dir}\",\"{$this->id}\",\"Held\",\"listview\",\"1\");'>";
                if ($this->ACLAccess('edit')) {
                    $meeting_fields['SET_COMPLETE'] = $setCompleteUrl . DotbThemeRegistry::current()->getImage("close_inline"," border='0'",null,null,'.gif',translate('LBL_CLOSEINLINE'))."</a>";
                } else {
            ...
            $date_db = $meeting_fields['DATE_START']; //$timedate->merge_date_time($meeting_fields['DATE_START'], $meeting_fields['TIME_START']);
            //Custom Field TIME_START_END - By Lap Nguyen
            $DATE_START = $timedate->to_display_date_time($meeting_fields['DATE_START']);
            $DATE_END   = $timedate->to_display_date_time($meeting_fields['DATE_END']);
            if($date_db    < $today    ) {
                $meeting_fields['TIME_START_END']= "<font class='overdueTask'>".substr($DATE_START,0, 10).': '.substr($DATE_START, 11,5).' - '.substr($DATE_END, 11)."</font>";
                $meeting_fields['DATE_START']= "<font class='overdueTask'>".$meeting_fields['DATE_START']."</font>";
            }else if($date_db    < $nextday) {
                $meeting_fields['TIME_START_END'] = "<font class='todaysTask'>".substr($DATE_START,0, 10).': '.substr($DATE_START, 11,5).' - '.substr($DATE_END, 11)."</font>";
                $meeting_fields['DATE_START'] = "<font class='todaysTask'>".$meeting_fields['DATE_START']."</font>";
            } else {
                $meeting_fields['TIME_START_END'] = "<font class='futureTask'>".substr($DATE_START,0, 10).': '.substr($DATE_START, 11,5).' - '.substr($DATE_END, 11)."</font>";
            ...
            if(!empty($this->host_url) && $date_db >= $oneHourAgo) {
                if($this->assigned_user_id == $GLOBALS['current_user']->id){
                    $join_icon = DotbThemeRegistry::current()->getImage('start_meeting_inline', 'border="0"',null,null,'.gif',translate('LBL_HOST_EXT_MEETING',$this->module_dir));
                    ...
                    $join_icon = DotbThemeRegistry::current()->getImage('join_meeting_inline', 'border="0"',null,null,'.gif',translate('LBL_JOIN_EXT_MEETING',$this->module_dir));
        ...
        function set_notification_body($xtpl, &$meeting) {
            ...
            if (strtolower(get_class($meeting->current_notify_user)) == 'contact') {
                $xtpl->assign(
                    "ACCEPT_URL",
                    $dotb_config['site_url'] . '/index.php?entryPoint=acceptDecline&module=Meetings&contact_id=' .
                    $meeting->current_notify_user->id . '&record=' . $meeting->id
                );
            } elseif (strtolower(get_class($meeting->current_notify_user)) == 'lead') {
                $xtpl->assign(
                    "ACCEPT_URL",
                    $dotb_config['site_url'] . '/index.php?entryPoint=acceptDecline&module=Meetings&lead_id=' .
                    $meeting->current_notify_user->id . '&record=' . $meeting->id
                );
            } else {
                $xtpl->assign(
                    "ACCEPT_URL",
                    $dotb_config['site_url'] . '/index.php?entryPoint=acceptDecline&module=Meetings&user_id=' .
                    $meeting->current_notify_user->id . '&record=' . $meeting->id
                );
            }
        ...
        public function create_notification_email($notify_user){
            ...
            $path = DotbConfig::getInstance()->get('upload_dir','upload/') . $this->id;
         ...
         * @param boolean $isUpdate Value captured prior to DotbBean Save
         */
    ...
    function getMeetingsExternalApiDropDown($focus = null, $name = null, $value = null, $view = null)
    {
        ...
        $apiList = dotb_cache_retrieve($cacheKeyName);
        if ($apiList === null) {

            $apiList = ExternalAPIFactory::getModuleDropDown('Meetings');
            $apiList = array_merge(array('Dotb' => $app_list_strings['eapm_list']['Dotb']), $apiList);
            dotb_cache_put($cacheKeyName, $apiList);
        }
