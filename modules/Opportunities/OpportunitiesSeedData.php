...
class OpportunitiesSeedData {
    ...
    public static function populateSeedData($records, $app_list_strings, $accounts, $users)
    {
        ...
        $currency = DotbCurrency::getCurrencyByISO('EUR');
        ...
        while ($records-- > 0) {
            ...
            $opp->amount = $amount;
            $opp->amount_usdollar = DotbMath::init($amount)->div($base_rate)->result();
    ...
    private static function createRevenueLineItems(&$opp, $rlis_to_create, $app_list_strings) {
        ...
        //DotbBean::enterOperation('saving_related');
        while($rlis_created < $rlis_to_create) {
            ...
            if (!$isClosedLost) {
                $opp_amount = DotbMath::init($opp_amount)
                                ->add(DotbCurrency::convertWithRate($rli->likely_case, $base_rate, $opp->base_rate))
                                ->result();
                $opp_best_case = DotbMath::init($opp_best_case)
                                ->add(DotbCurrency::convertWithRate($rli->best_case, $base_rate, $opp->base_rate))
                                ->result();
                $opp_worst_case = DotbMath::init($opp_worst_case)
                                ->add(DotbCurrency::convertWithRate($rli->worst_case, $base_rate, $opp->base_rate))
                                ->result();
            }
            $rlis_created++;
        }
