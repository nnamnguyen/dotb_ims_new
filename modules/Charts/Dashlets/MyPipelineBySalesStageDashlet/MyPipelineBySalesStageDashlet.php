...
class MyPipelineBySalesStageDashlet extends DashletGenericChart
{
    ...
    {
        global $dotb_config, $current_user, $timedate;

		$dotbChart = DotbChartFactory::getInstance();
		$dotbChart->base_url = array( 	'module' => 'Opportunities',
								'action' => 'index',
								'query' => 'true',
								'searchFormTab' => 'advanced_search',
							 );
        $dotbChart->url_params = array( 'assigned_user_id' => $current_user->id );
        $dotbChart->group_by = $this->constructGroupBy();
        ...
        $dotbChart->thousands_symbol = translate('LBL_OPP_THOUSANDS', 'Charts');
		...
		if ($this->mypbss_chart_type == 'hbar'){
			...
			$pipeline_total_string = translate('LBL_TOTAL_PIPELINE', 'Charts') . $dotbChart->currency_symbol . $total . $dotbChart->thousands_symbol;
			$dotbChart->setProperties($pipeline_total_string, $subtitle, 'horizontal bar chart');
		}
		else{
			$dotbChart->setData($this->getChartData($query));
			$total = format_number($dotbChart->getTotal(), 0, 0, array('convert'=>true));
			$pipeline_total_string = translate('LBL_TOTAL_PIPELINE', 'Charts') . $dotbChart->currency_symbol . $total . $dotbChart->thousands_symbol;
			$dotbChart->setProperties($pipeline_total_string, $subtitle, 'funnel chart 3D');
		}
        ...
        {
            $dotbChart->url_params['date_closed_advanced_range_choice'] = 'between';
            $dotbChart->url_params['start_range_date_closed_advanced'] = $timedate->to_display_date($this->mypbss_date_start, false);
            $dotbChart->url_params['end_range_date_closed_advanced'] = $timedate->to_display_date($this->mypbss_date_end, false);
        }
        elseif (!empty($this->mypbss_date_start))
        {
            $dotbChart->url_params['date_closed_advanced_range_choice'] = 'greater_than';
            $dotbChart->url_params['range_date_closed_advanced'] = $timedate->to_display_date($this->mypbss_date_start, false);
        }
        elseif (!empty($this->mypbss_date_end))
        {
            $dotbChart->url_params['date_closed_advanced_range_choice'] = 'less_than';
            $dotbChart->url_params['range_date_closed_advanced'] = $timedate->to_display_date($this->mypbss_date_end, false);
        }
        ...
        return $this->getTitle('') .
            '<div align="center">' .$dotbChart->display($this->id, $xmlFile, '100%', '480', false) . '</div><br />'. $this->processAutoRefresh();
    }
