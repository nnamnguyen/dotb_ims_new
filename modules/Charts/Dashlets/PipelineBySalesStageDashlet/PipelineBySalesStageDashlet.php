...
class PipelineBySalesStageDashlet extends DashletGenericChart
{
    ...
    public function display()
    {
        ...
        $dotbChart->url_params = array('start_range_date_closed' => $this->pbss_date_start,
                                        'end_range_date_closed' => $this->pbss_date_end);
        $dotbChart->group_by = $this->constructGroupBy();
        $dotbChart->setData($this->getChartData($this->constructQuery()));
        ...
        $pipeline_total_string = translate('LBL_TOTAL_PIPELINE', 'Charts') . $dotbChart->currency_symbol . format_number($dotbChart->getTotal(), 0, 0, array('convert'=>true)) . $dotbChart->thousands_symbol;
		if ($this->pbss_chart_type == 'hbar')
            $dotbChart->setProperties($pipeline_total_string, $subtitle, 'horizontal group by chart');
		else
        ...
        return $this->getTitle('') . '<div align="center">' . $dotbChart->display($this->id, $xmlFile, '100%', '480', false) . '</div>'. $this->processAutoRefresh();
    }
