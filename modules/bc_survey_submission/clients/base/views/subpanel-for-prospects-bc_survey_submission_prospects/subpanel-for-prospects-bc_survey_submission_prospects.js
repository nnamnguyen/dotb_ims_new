...
//
            ...
            success: function (result) {
                ...
                if (result == "true" || model.attributes.status == 'Pending') {
                    app.alert.show('send_confirm', {
                        level: 'confirmation',
                        messages: "Are you sure want to submit survey on behalf of client?",
                        onConfirm: _.bind(self.getSurveyURL, self, survey_id, module_type, module_id),
                    });
    ...
    resendSurvey: function (model) {
        ...
        if (model.attributes.survey_send)
        {
            app.alert.show('error', {
                level: 'info',
                messages: 'Survey is already sent for the selected record.',
                autoClose: true
            });
        } else {
            if (app.user.id == model.get('created_by') || app.user.attributes.type == "admin")
            {
                ...
                {
                    var url = App.api.buildURL("bc_survey", "approveRequest", "", {survey_id: survey_id, module_name: module_type, module_id: module_id, resend: 1});
                    App.api.call('GET', url, {}, {
                        success: function (result) {
