...
({
    ...
    load_summary_view: function (element) {
        ...
        if (type == 'pending_res') {
            ...
            var send_type = surveyOptedOutArraDetails['send_type'];
            var opted_out_record = surveyOptedOutArraDetails['opted_out_record'];
//            }
        }

        var arg = 'bc_survey/' + survey_ID + '/openSummaryDetailView';
        app.api.call('create', app.api.buildURL(arg), {type: type, module_name: module_name, pending_res_record: pending_response_record, opted_out_record: opted_out_record, currentRecordCount: currentRecordCount, send_sms_type: send_type},
                {
                    success: function (data) {
                        ...
                        self.send_type_sms = data['sms_type'];
                        self.render();
                    }
                }
        );
