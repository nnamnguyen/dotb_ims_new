...
({
    ...
    contextEvents: {
        ...
        "list:download_qr_code:fire": "download_qr_code"
    },
//    events: {
//        "click [name=create_poll_button]": 'create_poll'
//    },
//Changes for sorting survey list 6 june 2019 by Nikita 
    _bindEvents: function () {
        this._super('_bindEvents');
        this.on('click [name=create_poll_button]', this.create_poll, this);
    },
    ...
    /*download_qr_code from list view*/
    download_qr_code: function (model) {
        var self = this;
        var record_id = model.id;
        $('[name="bc_survey_' + record_id + '"]').find("[name='download_qr_code']").after('<form method="post" id="downloadQRCode" action="index.php?entryPoint=downloadQrCode" style="display:none;"><input type="hidden" value="" id="qr_url" name="qr_url"/><input type="hidden" value="" id="qr_survey_name" name="qr_survey_name"/><div></div></form>');
        var sharableLink = $("#shareable_link_" + record_id).val();
        var url = App.api.buildURL("bc_survey", "generate_qr_code_link_list_view_download", "", {surveyUrlPart: sharableLink});
        App.api.call('GET', url, {}, {
            success: function (data) {
                if (data) {
                    var survey_qr_code = data;
                    $("#downloadQRCode").find("input[name=qr_url]").val(survey_qr_code);
                    $("#downloadQRCode").find("input[name=qr_survey_name]").val(model.get('name'));
        ...
        app.alert.show('survey_link_copied', {
            level: 'success',
            messages: 'Survey QR Code is Downloaded.',
            autoClose: true
        });
    },
    /*
