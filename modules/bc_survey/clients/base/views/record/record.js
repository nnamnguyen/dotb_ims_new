({
    ...
    events: {
        ...
        'click .download_qr_code': 'download_qr_code',
    ...
    delegateButtonEvents: function () {
        ...
        this.context.on('button:manage_sms_template:click', this.manage_sms_templateClicked, this);
    },
    /**
    ...
    handleMouseMove: function (event) {
        this._super('handleMouseMove', [event]);
    },
    handleMouseLeave: function (event) {
        this._super('handleMouseLeave', [event]);
    },
    ...
    view_reportClicked: function () {
        ...
        javascript:parent.DOTB.App.router.navigate("bc_survey/" + record_id + "/layout/report", {trigger: true});
    },
    ...
    translate_surveyClicked: function () {
        var record_id = this.model.id;
        javascript:parent.DOTB.App.router.navigate("bc_survey/" + record_id + "/layout/translate-survey", {trigger: true});
    },
                ...
                // email template does not exists so redirect to create emailm template if confirm is OK
                    ...
                    if (self.model.get('survey_status') == 'Active')
                    {
                        app.alert.show('stop_confirmation', {
                            level: 'confirmation',
                            title: '',
                            /*Changes for script tag in while managing email template 13 JUNE 2019*/
                            messages: 'Email template does not exist. Click Confirm to create email template for ' + self.htmlspecialchars(self.model.get('name')) + '.',
                            onConfirm: function () {
    ...
    //To manage SMS template for the particular Survey 07-12-2019
    manage_sms_templateClicked: function () {
        var self = this;
        var survey_ID = this.model.id;
        self.survey_id_selected = "";
        ...
        App.api.call('GET', url, {}, {
            success: function (data) {
                ...
                if (data['smsTemplateID'] && data['smsTemplateID'].trim() != '') {
                    var newWin = window.open("#bc_survey_sms_template/" + data['smsTemplateID']);
                    if (typeof newWin == "undefined") {
                        app.alert.show('info', {
                            level: 'info',
                            messages: 'Please allow your browser to show pop-ups.',
                ...
                // email template does not exists so redirect to create emailm template if confirm is OK
                    ...
                    if (self.model.get('survey_status') == 'Active')
                    {
                        app.alert.show('stop_confirmation', {
                            ...
                            title: '',
                            /*Changes for script tag in while managing email template 13 JUNE 2019*/
                            messages: 'SMS/WhatsApp template does not exist. Click Confirm to create sms/whatsapp template for ' + self.htmlspecialchars(self.model.get('name')) + '.',
                            onConfirm: function () {
                                self.survey_id_selected = survey_ID;
                        ...
                        app.alert.show('error_inactive_survey', {
                            level: 'error',
                            messages: 'You can not create sms template for inactive survey.',
                            autoClose: true
                        });
                    }
                    // Survey Status :: LoadedTech Customization END
    ...
    _render: function () {
        ...
        $('[data-name="allow_redundant_answers"]').parents('.panel_body').append('<div class="btn btn-primary get_share_link" rel="tooltip" data-original-title="Get Survey Shareable Link" style="display:none;">Get Shareable Link</div>');

        // add "QR Code" image and download Button to record view
//        $('[data-name="allow_redundant_answers"]').next('div').append('<div class="record-label" data-name="survey_qr_code" style="display:none;">QR Code</div>');
        $('[data-name="allow_redundant_answers"]').parents('.panel_body').after('<div class="row-fluid panel_body panel_body" style="margin-top:15px;"><div class="span12 record-cell"><div class="record-label" data-name="survey_qr_code" style="display:none;">Get QR Code</div></div></div>');
...
//            $('[data-name="allow_redundant_answers"]').show();
        }
        //get shrable link   //////////////////////////
        this.get_sharable_link('', 'isRender');
        //QR Code//////////////////////////
        this.get_qr_code('', 'isRender');
    ...
    get_qr_code: function (el, status) {
        //get QR Code //////////////////////////
        var self = this;
        var url = App.api.buildURL("bc_survey", "generate_qrcode_survey_submit_id", "", {survey_id: self.model.get('id'), status: status});
        App.api.call('GET', url, {}, {
            success: function (data) {
                if (data) {
                    var survey_qr_code = data;
                    $('[data-name="survey_qr_code"]').after('<span class="normal index" data-fieldname="survey_qr_code" class="survey_qr_code" data-index=""><span sfuuid="39" class="detail"><img style="width: 19%;" src="' + survey_qr_code + '"><form method="post" id="downloadQRCode" action="index.php?entryPoint=downloadQrCode" style="display:inline-block;"><input type="hidden" value="" id="qr_url" name="qr_url"/><input type="hidden" value="" id="qr_survey_name" name="qr_survey_name"/><div><button type="button" class="btn btn-primary download_qr_code" rel="tooltip" data-original-title="Download QR Code">Download QR Code</button></div></form></span></span>');
                }
            },
        });
    },
    ...
    download_qr_code: function () {
        var self = this;
        var qr_code_link = $("[data-fieldname='survey_qr_code']").find("img").attr("src");
        $("#downloadQRCode").find("input[name=qr_url]").val(qr_code_link);
        $("#downloadQRCode").find("input[name=qr_survey_name]").val(self.model.get('name'));
        $("#downloadQRCode").submit();
    },
    ...
    saveClicked: function () {
        ...
        //changes regarding the predefined data in thank you and welcome page.
        // To save default value of survey thanks page content if value is empty
        if (!this.model.attributes.survey_thanks_page)
        {
            this.model.attributes.survey_thanks_page = 'Thank you for taking the survey.';
        }
        // To save default value of survey welcome page content if value is empty
        if (!this.model.attributes.survey_welcome_page)
        {
            this.model.attributes.survey_welcome_page = 'Thank you for agreeing to take part in our survey. This survey should only take a few minutes to complete. Please click on Next to continue.';
        }
        // ENDF
        var self = this;
                            ...
                            $('#' + question_class_id).find($(que_class)).each(function () {
                                ...
                                $(data_page).find($('#' + que_section)).each(function () {
                                    ...
                                    if (survey_question_detail['que_type'] != 'richtextareabox') {
                                        question = $('[name="question_' + que_section + '"]').val();
                                    }
                                    // Validation for Question title :: 255 max char allowed
                                    if (question && question.length > 255 && question_type != 'richtextareabox') {
                                        ...
                                        $('[name="question_' + que_section + '"]').parent().addClass('error-custom').find('span').attr('style', 'position:relative; z-index:500'); // error icon and red style to input
                                        $('[name="question_' + que_section + '"]').parent().addClass('error-custom').find('.fa-exclamation-circle').attr('title', 'Error. Max 255 character allowed.'); // error message
                                    } else if (question && question.trim() != '' && question_type != 'richtextareabox') {
                                        ...
                                        $('[name="question_' + que_section + '"]').parent().removeClass('error-custom').find('span.que-piping-section').attr('style', 'margin-left:0px;'); // error icon and red style to input
                                        $('[name="question_' + que_section + '"]').parents('.question').find('.general').css('color', '#747474');
                                    } else if ($('[name=uploadImageType_' + que_section + ']:checked').prop('class') != null && $('[name=uploadImageType_' + que_section + ']:checked').prop('class').includes("uploadsImage") && question_type != 'richtextareabox') {
                                        survey_question_detail['question'] = "uploadImage";
                                    } else if ($('[name=uploadImageType_' + que_section + ']:checked').prop('class') != null && $('[name=uploadImageType_' + que_section + ']:checked').prop('class').includes("imageURL") && question_type != 'richtextareabox') {
                                        ...
                                        $('[name="question_' + que_section + '"]').parents('.question').find('.general').css('color', '#747474');
                                    } else if (question_type == 'richtextareabox' && question.trim() == "") {
                                        ...
                                        $('[name="question_' + que_section + '"]').parent().addClass('error-custom').find('span').attr('style', 'position:relative; z-index:500'); // error icon and red style to input
                                        $('[name="question_' + que_section + '"]').parents('.question').find('.general').css('color', 'red');
                                        $('[name="question_' + que_section + '"]').parent().addClass('error-custom').find('.fa-exclamation-circle').attr('title', 'Error. This field is required.'); // error message
                                    }
                                    ...
                                    if (survey_question_detail['que_type'] == "image") {
                                        ...
                                        ;
                                        if ($('[name=helptips_' + que_section + ']').val().trim() == '') {
                                    ...
                                    options_div.each(function () {
                                        ...
                                        $('#' + opid).find($('[name="option_' + survey_question_detail['que_type'] + '"]')).each(function () {
                                            ...
                                            if (survey_question_detail['que_type'] == "radio-button" || survey_question_detail['que_type'] == "check-box" || survey_question_detail['que_type'] == "emojis") {
                                                ...
                                                if (is_image_option && $(this).parent('.options').find('.radioImageUpload').val() == '') {
                                                    $(this).parents('.general_options').parent().find('.general').css('color', 'red');
                                        ...
                                        if (is_other_selected == 1) {
                                            $('#' + opid).parent().parent().find($('[name="label_otherOption_' + survey_question_detail['que_type'] + '_' + count + '"]')).each(function () {
                                                op_count++;
                                                if (survey_question_detail['que_type'] == "radio-button" || survey_question_detail['que_type'] == "check-box" || survey_question_detail['que_type'] == "emojis") {
                                                    if (this.value.trim() != '') {
                                                        $(this).parent().removeClass('error-custom');
                                            ...
                                            switch (sel_logic_act) {
                                                case "redirect_to_page":
                                                    ...
                                                    if (!val || val == 'none') {
                                                        ...
                                                        $(this).find('.logic_action_targtes').find('.redirect_to_page_options').attr('style', 'border:1px solid red;width:290px !important;');
                                                        $(this).find('.logic_action_targtes').parents('.question').find('.skip_logic').css('color', 'red');
                                                    } else {
                                                        $(this).find('.logic_action_targtes').find('.redirect_to_page_options').attr('style', 'width:290px;');
...
//                                                                $(this).find('.clearTd').append(' <span class="error-tooltip add-on" data-container="body" rel="tooltip" title="" data-original-title="It should be in http://www.google.com format."><i class="fa fa fa-exclamation-circle" style="color:red;">&nbsp;</i></span> ');
                                                                $(this).find('.clearTd').append(' <span class="error_msg" style="color:red; font-size:10px;" data-container="body" rel="tooltip" title="" data-original-title="It should be in http://www.google.com format."><i class="fa fa fa-exclamation-circle" style="color:red;"></i></span><span style="font-size: 10px;color: red;" class="error_msg">Error. Invalid URL.</span>');
                                                            }
                                                            $(this).addClass('error-custom');
                                                            $(this).find('[name=redirect_to_url]').attr('style', 'border:1px solid red;margin-bottom:1px;');
                                                            $(this).find('[name=redirect_to_url]').parents('.question').find('.skip_logic').css('color', 'red');
                                                            $(this).find('[name=redirect_to_url]').parents('.question').find('.skip_logic').css('width', '290px');
                                                        } else {
                                                            $(this).find('.clearTd').find('.error_msg').remove();
                                                            $(this).find('[name=redirect_to_url]').attr('style', 'margin-bottom:1px;');
                                                            $(this).removeClass('error-custom');
                                                            $(this).parent().removeClass('error').find('span').attr('style', 'display:none;width:290px');
                                                            $(this).find('[name=redirect_to_url]').parents('.question').find('.skip_logic').css('color', '#747474');
                                                        }
                                                    }
                                                    break;
                                                case "show_hide_question":
                                                    ...
                                                    if (val == '' || val == null) {
                                                        ...
                                                        $(this).find('.logic_action_targtes').find('select.show_hide_question').attr('style', 'width: 290px;border:1px solid red;');
                                                        $(this).find('.logic_action_targtes').parents('.question').find('.skip_logic').css('color', 'red');
                                                    } else {
                                                        $(this).find('.logic_action_targtes').find('.ms-choice').attr('style', 'width: 290px;');
                                                        $(this).removeClass('error-custom');
                                                        $(this).find('.logic_action_targtes').parents('.question').find('.skip_logic').css('color', '#747474');
                                                    }
                                                    break;
            ...
            if (error_on_sync_req_missing)
            {
                ...
                app.alert.show('is_req_msg', {
                    level: 'confirmation',
                    title: '',
                    messages: 'This is a required field. Click "Confirm" to make this a required question.',
                    onConfirm: function () {
    ...
    /** Changes for script tag in survey recoed 13 June 2019*/
