...
<td>
	<table border="0" cellspacing="0" cellpadding="0" width="100%">
        <tr>
            <td align="left" scope="row" colspan="4" style="padding-left: 10px;">{$SAMPLE_URL} &nbsp;{dotb_help text=$MOD.LBL_SAMPLE_URL_HELP}</td>
        </tr>
        <tr>
        ...
        <tr>
            <td align="left" scope="row" colspan="3"><label for="userfile">{$MOD.LBL_SELECT_FILE}</label> <input type="hidden" /><input size="20" id="userfile" name="userfile" type="file"/> &nbsp;{dotb_help text=$MOD.LBL_FILE_UPLOAD_WIDGET_HELP}</td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <tr>
            <td scope="row" colspan="3">
                ...
                &nbsp;<label for="type">{$MOD.LBL_IMPORT_BUTTON}</label> &nbsp;{dotb_help text=$MOD.LBL_CREATE_BUTTON_HELP}
            </td>
          </tr>
          <tr>
            <td scope="row" colspan="3">
                <input id="import_update" class="radio" type="radio" name="type" id="type" value="update" />
                &nbsp;<label for="type">{$MOD.LBL_UPDATE_BUTTON}</label> &nbsp;{dotb_help text=$MOD.LBL_UPDATE_BUTTON_HELP}
            </td>
          </tr>
	</table>
    <br>
    <table border="0" cellspacing="0" cellpadding="0" width="100%" class="preset-settings">
          ...
          <tr>
            <td scope="row" colspan="3">
                <h3>{$MOD.LBL_PUBLISHED_SOURCES}&nbsp;{dotb_help text=$savedMappingHelpText}</h3></td>
          </tr>
          ...
          <tr id="published_import_{$smarty.foreach.published.index}" class="preset-row preset-custom">
            <td scope="row" colspan="2" class="preset-label">
                <input class="radio" type="radio" {if $DEFAULT_MAPPING==$item.IMPORT_ID}checked{/if} name="source" value="custom:{$item.IMPORT_ID}"/>
                &nbsp;{$item.IMPORT_NAME}
            </td>
            <td scope="row" class="preset-actions">
                {if $is_admin}
