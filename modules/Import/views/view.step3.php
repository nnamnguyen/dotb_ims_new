...
class ImportViewStep3 extends ImportView
{
    ...
    {
        global $mod_strings, $app_strings, $current_user, $dotb_config, $app_list_strings, $locale;
        ...
        $dotb_config['import_max_records_per_file'] = ( empty($dotb_config['import_max_records_per_file']) ? 1000 : $dotb_config['import_max_records_per_file'] );
       ...
       //populate import locale  values from import mapping if available, these values will be used througout the rest of the code path
        ...
        $this->ss->assign("DELETE_INLINE_PNG",  DotbThemeRegistry::current()->getImage('basic_search','align="absmiddle" alt="'.$app_strings['LNK_DELETE'].'" border="0"'));
        $this->ss->assign("PUBLISH_INLINE_PNG",  DotbThemeRegistry::current()->getImage('advanced_search','align="absmiddle" alt="'.$mod_strings['LBL_PUBLISH'].'" border="0"'));
    ...
    protected function getMappingClassName($source)
    {
       ...
       if (DotbAutoLoader::requireWithCustom("modules/Import/maps/{$customName}.php") ) {
           return $customName;
       }
       if(DotbAutoLoader::requireWithCustom("modules/Import/maps/{$name}.php") ) {
           return $name;
       }
    ...
    protected function _getCSS()
    {
        return <<<EOCSS
            <style>
                ...
                span.collapse{
                    background: transparent url('index.php?entryPoint=getImage&themeName=Dotb&themeName=Dotb&imageName=dotb-yui-sprites.png') no-repeat 0 -90px;
                    padding-left: 10px;
                    cursor: pointer;
                }

                span.expand{
                    background: transparent url('index.php?entryPoint=getImage&themeName=Dotb&themeName=Dotb&imageName=dotb-yui-sprites.png') no-repeat -0 -110px;
                    padding-left: 10px;
                     cursor: pointer;
                }
                .removeButton{
...
EOCSS;
    ...
    protected function _getJS($required)
    {
        ...
        $sqsWaitImage = DotbThemeRegistry::current()->getImageURL('sqsWait.gif');
        $removeButtonImage = "index.php?entryPoint=getImage&themeName=Dotb&imageName=id-ff-remove.png&v=".getVersionedPath('');
        return <<<EOJAVASCRIPT
