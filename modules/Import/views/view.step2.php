...
class ImportViewStep2 extends ImportView
{
    ...
    public function display()
    {
        ...
        if ($importModule == 'Leads') {
            $this->ss->assign('DEFAULT_MAPPING', $admin->settings['default_mapping_lead']);
        } elseif ($importModule == 'Contacts') {
            $this->ss->assign('DEFAULT_MAPPING', $admin->settings['default_mapping_contact']);
        } elseif ($importModule == 'Prospects') {
            $this->ss->assign('DEFAULT_MAPPING', $admin->settings['default_mapping_prospect']);
        } else {
            $this->ss->assign('DEFAULT_MAPPING', 0);
        }
        ...
        switch ($_REQUEST['import_module']) {
            case 'Leads':
                ...
                break;
            case 'Accounts':
                $download_url = 'custom/uploads/TemplateImport/Template_Import_Account.xlsx';
                break;
            case 'Contacts':
                $download_url = 'custom/uploads/TemplateImport/Template_Import_Contacts.xlsx';
                break;
            case 'Opportunities':
                $download_url = 'custom/uploads/TemplateImport/Template_Import_Opportunity.xlsx';
                break;
            case 'Quotes':
                $download_url = 'custom/uploads/TemplateImport/Template_Import_Quote.xlsx';
                break;
            case 'ProductTemplates':
                $download_url = 'custom/uploads/TemplateImport/Template_Import_ProductTemplate.xlsx';
                break;
            default:
                $download_url = "index.php?entryPoint=export&module=" . urlencode($importModule) . "&action=index&all=true&sample=true";
                break;
        }

        $this->ss->assign("SAMPLE_URL", "<a href=\"javascript: void(0);\" onclick=\"window.location.href='" . $download_url . "'\" >" . $mod_strings['LBL_EXAMPLE_FILE'] . "</a>"); //FIXME
...
document.getElementById('gonext').onclick = function(){
    ...
    {
        ret = confirm(DOTB.language.get("Import", 'LBL_CONFIRM_IMPORT'));
        if (!ret) {
            return false;
        }
    }
...
function publishMapping(elem, publish, mappingId)
{
    ...
    if(publish == 'yes')
    {
        var newTitle = DOTB.language.get('Import','LBL_UNPUBLISH');
        var newPublish = 'no';
    }
    else
    {
        var newTitle = DOTB.language.get('Import','LBL_PUBLISH');
        var newPublish = 'yes';
    }
