...
class Importer
{
    ...
    /**
     * @var dotbToExternalSourceFieldMap
     */
    protected $dotbToExternalSourceFieldMap = array();
    ...
    public function __construct($importSource, $bean)
    {
        ...
        ini_set("max_execution_time", max($dotb_config['import_max_execution_time'],3600));
    ...
    {
        global $dotb_config, $mod_strings, $current_user, $current_language;
        ...
        foreach ($fields_order as $fieldNum) {
            ...
            else if( isset($this->dotbToExternalSourceFieldMap[$field]) && isset($row[$this->dotbToExternalSourceFieldMap[$field]]) )
            {
                $rowValue = strip_tags(trim($row[$this->dotbToExternalSourceFieldMap[$field]]));
            }
            else
            {
                $rowValue = '';
            ...
            if (!empty($_REQUEST["default_value_$field"])) {
                ...
                {
                    $rowValue = implode(', ', DotbFieldTeamset::getTeamsFromRequest($field));
                }
            ...
            if ( $field == 'email' || $field == 'email1' || $field == 'email2' )
            {
                ...
                else
                {
                    ...
                    $defaultOptout  = !empty($GLOBALS['dotb_config']['new_email_addresses_opted_out']);
                    $address = array(
        ...
        if ( !empty($focus->id) )
        {
            ...
            or dotb_die("Error selecting dotbbean: ");
     ...
     *
     * @param DotbBean $focus The parent dotb bean
    ...
    public function handleTagsImport($focus, $row)
    {
        ...
        if ($this->hasTags !== false && $focus->isTaggable()) {
            ...
            // Get the Tag DotbField for handling the saving
            $sfTag = $sfh->getDotbField('tag');
    ...
    protected function cloneExistingBean($focus)
    {
        ...
        if (!($existing_focus->retrieve($focus->id) instanceOf DotbBean) || !$existing_focus->ACLAccess('edit')) {
            return FALSE;
        }
        else
        {
    ...
    protected function removeDeletedBean($focus)
    {
        ...
        $result2 = $focus->db->query($query2) or dotb_die($mod_strings['LBL_ERROR_DELETING_RECORD']." ".$focus->id);
    ...
    protected function saveImportBean($focus, $newRecord)
    {
        ...
        */
        ...
        if ( $focus->object_name == "Contact" && isset($focus->sync_contact) )
        {
            ...
            dotb_die($GLOBALS['mod_strings']['ERR_IMPORT_SYSTEM_ADMININSTRATOR']);
        }
        ...
        // if modified_user_id is set, set the flag to false so DotbBEan will not reset it
        if (isset($focus->modified_user_id) && $focus->modified_user_id && !$hasDataChanges) {
            $focus->update_modified_by = false;
        }
        // if created_by is set, set the flag to false so DotbBEan will not reset it
    ...
    protected function saveMappingFile()
    {
        ...
        for ( $i = 0; $i < $_REQUEST['columncount']; $i++ )
        {
            if (isset($this->importColumns[$i]) && !empty($_REQUEST[$this->importColumns[$i]]))
            {
                ...
                {
                    $teams = DotbFieldTeamset::getTeamsFromRequest($field);
    ...
    /**
     * Sets a translation map from dotb field key to external source key used while importing a row.  This allows external sources
    ...
    {
        $this->dotbToExternalSourceFieldMap = $translator;
    }
    ...
    /**
	 * upon bean save, the relationships are saved by DotbBean->save_relationship_changes() method, but those values depend on
     ...
     *
     * @param DotbBean $bean Target bean
     * @param array $addresses Addresses to be added
     */
    protected function handleEmailUpdate(DotbBean $bean, array $addresses)
    {
     ...
     */
    protected function importAddress(DotbBean $bean, array $address)
    {
        // make sure that operating on email addresses is possible
        if (!isset($bean->emailAddress->addresses) || !is_array($bean->emailAddress->addresses)) {
            return;
        ...
        foreach ($bean->emailAddress->addresses as $beanAddress) {
            if ($beanAddress['email_address'] == $address['email_address']) {
                ...
                // e.g. previous versions of DotBCRM don't export invalid and opt_out
     ...
     * @param $importFields
     */
    protected function correctRealNameFieldDef(DotbBean $focus, $importFields)
    {
        global $current_user;
