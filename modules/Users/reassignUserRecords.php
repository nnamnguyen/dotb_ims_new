...
if(!$GLOBALS['current_user']->isAdminForModule('Users')) {
	dotb_die($app_strings['EXCEPTION_NOT_AUTHORIZED']);
}
...
foreach(DotbAutoLoader::existingCustom('modules/Users/metadata/reassignScriptMetadata.php', 'modules/Users/reassignScriptMetadata_override.php') as $file) {
    include $file;
}
...
///////////////////// BEGIN STEP 2 - Confirm Selections /////////////////////////
	...
	if(empty($modules)){
		dotb_die($mod_strings_users['ERR_REASS_SELECT_MODULE']);
	}

    if($fromuser == $touser){
		dotb_die($mod_strings_users['ERR_REASS_DIFF_USERS']);
	}
    ...
    $dotbFieldTeamSet = new DotbFieldTeamset('Teamset');
    $teams = $dotbFieldTeamSet->getTeamsFromRequest('team_name');
	$team_ids = array_keys($teams);
    $team_id = DotbFieldTeamset::getPrimaryTeamIdFromRequest('team_name', $_REQUEST);
    ...
    if ($tbaConfigurator->isEnabledGlobally()) {
        $selectedIds = $dotbFieldTeamSet->getSelectedTeamIdsFromRequest('team_name', $_REQUEST);
	...
	$dotb_smarty = new Dotb_Smarty();
        $help_img = smarty_function_dotb_help(array("text"=>$mod_strings['LBL_REASS_VERBOSE_HELP']),$dotb_smarty);
