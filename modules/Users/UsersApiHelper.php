...
class UsersApiHelper extends DotbBeanApiHelper
{
    /**
     * @var Config
     */
    ...
    /**
     * Uses the checkUserAccess DotbBean method to see if the user being formatted has access to the given record
     *
     * @param DotbBean $user The user to check access for
     ...
     */
    protected function checkUserAccess(DotbBean $user, $module, $recordId)
    {
    ...
    public function populateFromApi(DotbBean $bean, array $submittedData, array $options = array())
    {
        ...
        if (empty($submittedData) || empty($submittedData['user_name'])) {
            throw new DotbApiExceptionMissingParameter("Missing username");
        }
     ...
     * @return array
     */
    protected function filterIDMModeDisabledFields(DotbBean $bean, array $submittedData)
    {
     ...
     * @return array
     */
    protected function filterEmailField(DotbBean $bean, array $emails)
    {
        /** @var User $bean */
        if (!is_array($emails)) {
            return [];
     ...
     * @return array
     */
    protected function getPrimaryEmailAddressInApiFormat(DotbBean $bean)
    {
    ...
    protected function getIdpConfig()
    {
        if (is_null($this->idpConfig)) {
            $this->idpConfig = new Config(\DotbConfig::getInstance());
        }
