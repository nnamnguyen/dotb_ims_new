...
if (isset($_POST['record']) && !is_admin($current_user) &&
    ...
    dotb_die('Unauthorized access to administration.');
     ...
     !$GLOBALS['current_user']->isAdminForModule('Users')) {
    dotb_die('Unauthorized access to user administration.');
}
...
foreach ($focus->field_defs as $fieldName => $field) {
    if (isset($field['source']) && $field['source'] == 'custom_fields') {
        ...
        if ($sf != null) {
            ...
            $GLOBALS['log']->fatal("Field '$fieldName' does not have a DotbField handler");
        }
    }
}
...
if (!$focus->is_group && !$focus->portal_only) {
    ...
    if (isset($DISPLAY_ARR['display_tabs'])) {
        //Put home back in.  It needs to be first display module in Dotb 7
    ...
    if (isset($_POST['calendar_publish_key'])) {
        $focus->setPreference(
            ...
            DotbCleaner::stripTags($_POST['calendar_publish_key'], false),
            0,
            'global'
        );
    }
...
if (!$focus->verify_data()) {
    ...
    $focus->save();
    $GLOBALS['dotb_config']['disable_team_access_check'] = false;
    ...
    if (($newUser) && !($focus->is_group) && !($focus->portal_only) &&
        isset($dotb_config['passwordsetting']['SystemGeneratedPasswordON']) &&
        $dotb_config['passwordsetting']['SystemGeneratedPasswordON']
    ) {
        $new_pwd = '2';
