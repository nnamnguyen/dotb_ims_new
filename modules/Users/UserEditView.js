...
function user_status_display(field){
		...
		switch (field.value){
		case 'Administrator':
		    document.getElementById('UserTypeDesc').innerHTML=DOTB.language.get('Users',"LBL_ADMIN_DESC");
		    document.getElementById('is_admin').value='1';
		break;
		case 'RegularUser':
			document.getElementById('is_admin').value='0';
			document.getElementById('UserTypeDesc').innerHTML=DOTB.language.get('Users',"LBL_REGULAR_DESC");
		break;
		case 'UserAdministrator':
			document.getElementById('is_admin').value='0';
			document.getElementById('UserTypeDesc').innerHTML=DOTB.language.get('Users',"LBL_USER_ADMIN_DESC");
		break;
        case 'GROUP':
            document.getElementById('is_admin').value='0';
            document.getElementById('is_group').value='1';
			document.getElementById('UserTypeDesc').innerHTML=DOTB.language.get('Users',"LBL_GROUP_DESC");
        break;
        case 'PORTAL_ONLY':
            document.getElementById('is_admin').value='0';
            document.getElementById('is_group').value='0';
            document.getElementById('portal_only').value='1';
            document.getElementById('UserTypeDesc').innerHTML=DOTB.language.get('Users','LBL_PORTAL_ONLY_DESC');
        break;
	}
}
...
function testOutboundSettings()
{
    ...
    {
        isError = true;
        errorMessage += DOTB.language.get('Users',"LBL_MISSING_DEFAULT_OUTBOUND_SMTP_SETTINGS") + "<br/>";
        overlay(DOTB.language.get('app_strings',"ERR_MISSING_REQUIRED_FIELDS"), errorMessage, 'alert');
        return false;
    }
    ...
    {
        isError = true;
        errorMessage += DOTB.language.get('app_strings',"LBL_EMAIL_ACCOUNTS_SMTPUSER") + "<br/>";
    }
...
function sendTestEmail()
{
    ...
    if (trim(fromAddress) == "")
    {
        overlay(DOTB.language.get('app_strings',"ERR_MISSING_REQUIRED_FIELDS"), DOTB.language.get('app_strings',"LBL_EMAIL_SETTINGS_FROM_TO_EMAIL_ADDR"), 'alert');
        return;
    }
    else if (!isValidEmail(fromAddress)) {
        overlay(DOTB.language.get('app_strings',"ERR_INVALID_REQUIRED_FIELDS"), DOTB.language.get('app_strings',"LBL_EMAIL_SETTINGS_FROM_TO_EMAIL_ADDR"), 'alert');
        return;
    }
    ...
    overlay(DOTB.language.get('app_strings',"LBL_EMAIL_PERFORMING_TASK"), DOTB.language.get('app_strings',"LBL_EMAIL_ONE_MOMENT"), 'alert');

    var callbackOutboundTest = {
    	success	: function(o) {
    		hideOverlay();
    		overlay(DOTB.language.get('app_strings',"LBL_EMAIL_TEST_OUTBOUND_SETTINGS"), DOTB.language.get('app_strings',"LBL_EMAIL_TEST_NOTIFICATION_SENT"), 'alert');
    	}
    };
	...
	YAHOO.util.Connect.asyncRequest("POST", "index.php?action=testOutboundEmail&mail_name=system&module=EmailMan&to_pdf=true&dotb_body_only=true", callbackOutboundTest, postDataString);
}
function testOutboundSettingsDialog() {
        ...
        if(!EmailMan.testOutboundDialog) {
            ...
            EmailMan.testOutboundDialog.setHeader(DOTB.language.get('app_strings',"LBL_EMAIL_TEST_OUTBOUND_SETTINGS"));
            ...
            YAHOO.example.container.simpledialog1 =
                new YAHOO.widget.SimpleDialog('simpledialog1',
                                              { width: '300px',
                                                ...
                                                draggable: false,
                                                close: true,
                                                text: DOTB.language.get('Users','LBL_REASS_CONFIRM_REASSIGN'),
                                                constraintoviewport: true,
                                                buttons: [ { text:DOTB.language.get('Users','LBL_REASS_CONFIRM_REASSIGN_YES'), handler:handleYes, isDefault:true },
                                                           { text:DOTB.language.get('Users','LBL_REASS_CONFIRM_REASSIGN_NO'),  handler:handleNo } ]
                                              } );
            YAHOO.example.container.simpledialog1.setHeader(DOTB.language.get('Users','LBL_REASS_CONFIRM_REASSIGN_TITLE'));
...
function verify_data(form)
{
    ...
    if (document.getElementById("required_password").value=='1' 
		...
		add_error_style('EditView',form.new_password.name,
                        DOTB.language.get('app_strings','ERR_MISSING_REQUIRED_FIELDS') + DOTB.language.get('Users','LBL_NEW_PASSWORD') );
        isError = true;
	}
    ...
    if ( !document.EditView.isDuplicate || !document.EditView.isDuplicate.value )
    {
        if (document.EditView.return_id.value != '' && (typeof(form.reports_to_id)!="undefined") && (document.EditView.return_id.value == form.reports_to_id.value)) {
            alert(DOTB.language.get('app_strings','ERR_SELF_REPORTING'));
            return false;
        }
    }

    if (document.EditView.dec_sep.value != '' && (document.EditView.dec_sep.value == "'")) {
        alert(DOTB.language.get('app_strings','ERR_NO_SINGLE_QUOTE') + DOTB.language.get('Users','LBL_DECIMAL_SEP'));
        return false;
    }
    
	if (document.EditView.num_grp_sep.value != '' && (document.EditView.num_grp_sep.value == "'")) {
		alert(DOTB.language.get('app_strings','ERR_NO_SINGLE_QUOTE') + DOTB.language.get('Users','LBL_NUMBER_GROUPING_SEP'));
		return false;
	}
	...
	if( document.getElementById("portal_only") && document.getElementById("portal_only")=='1' &&
		...
		if(document.getElementById("new_password").value != '' || document.getElementById("confirm_pwd").value != '') {
			if(document.getElementById("new_password").value != document.getElementById("confirm_pwd").value) {
				alert(DOTB.language.get('Users','ERR_PASSWORD_MISMATCH'));
				return false;
			}
		}
	}
    ...
    if (window.parent.DOTB &&
        window.parent.DOTB.App &&
        ...
        if (field) {
            ...
            if (!filename || filename && !filename.length) {
                window.parent.DOTB.App.events.trigger("bwc:avatar:removed");
            }
        }
    }
	return true;
...
//    	Set_Cookie('dotb_theme_menu_load','true',30,'/','','');
...
function onUserEditView() {
    ...
    setDotbClientEmailOption();
}

function setDotbClientEmailOption() {
    //disable the dotb client option if the system email setting are not configured properly
    var emailType = document.getElementById('email_link_type'),
        isDotbClientDisabled = emailType.getAttribute('data-dotbclientdisabled');

    if (isDotbClientDisabled && isDotbClientDisabled === 'true') {
        var option = emailType.getElementsByTagName('option');
        for (var i = 0; i < option.length; i++) {
            if (option[i].value.toLowerCase() === 'dotb') {
