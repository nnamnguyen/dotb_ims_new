...
if ($redirect!='0') {
	header('location: ' . $dotb_config['site_url']);
	exit ();
}
...
$dotb_smarty->assign("IS_ADMIN", '1');
$dotb_smarty->assign("ENTRY_POINT", 'Changenewpassword');
$dotb_smarty->assign('return_action', 'login');
$dotb_smarty->assign("APP", $app_strings);
$dotb_smarty->assign("INSTRUCTION", $app_strings['NTC_LOGIN_MESSAGE']);
$dotb_smarty->assign("USERNAME_FIELD", '<td scope="row" width="30%">'.$mod_strings['LBL_USER_NAME'].':</td><td width="70%"><input type="text" size="20" tabindex="1" id="user_name" name="user_name"  value=""</td>');
$dotb_smarty->assign('PWDSETTINGS', $GLOBALS['dotb_config']['passwordsetting']);
$dotb_smarty->assign('SITE_URL', $GLOBALS['dotb_config']['site_url']);


$rules = "'" . $GLOBALS["dotb_config"]["passwordsetting"]["minpwdlength"]
	   . "','" . $GLOBALS['dotb_config']['passwordsetting']['maxpwdlength']
	   . "','" . $GLOBALS['dotb_config']['passwordsetting']['customregex'] . "'";

$dotb_smarty->assign('SUBMIT_BUTTON','<input title="'.$mod_strings['LBL_LOGIN_BUTTON_TITLE']
	.'" class="button" '
