...
class User extends Person {
    ...
    {
        if (!empty($GLOBALS['dotb_config']['disable_password_change']) && !empty($userName) && in_array($userName, self::$demoUsers)) {
            return true;
        } else {
            return false;
        }
	...
	function getUserPrivGuid() {
		...
		if ($userPrivGuid) {
			...
			if (!isset ($_SESSION['setPrivGuid'])) {
				...
				dotb_die("Breaking Infinite Loop Condition: Could not setUserPrivGuid.");
			}
		}
	}
    ...
    */
    ...
    {
        $db = DBManagerFactory::getInstance();
        $users = array('DotbCRMSupport', 'DotbCRMUpgradeUser');
        $where = ' 1=1 ';
	...
	function save($check_notify = false) {
        ...
        if (!empty($dotb_flavor) && !empty($admin->settings['license_enforce_user_limit'])) {
            ...
            if ($this->portal_only != 1 && $this->is_group != 1 && $this->status == 'Active'
                ...
                if ($license_seats_needed >= 0) {
                    ...
                    if (!empty($this->external_auth_only)) {
                        $e = new DotbApiExceptionLicenseSeatsNeeded(
		...
		$GLOBALS['dotb_config']['disable_team_access_check'] = true;
        if($this->status != 'Reserved' && !$this->portal_only) {
		   // If this is not an update, then make sure the new user logic is executed.
            ...
            } else if (empty($GLOBALS['dotb_config']['noPrivateTeamUpdate'])) {
	...
	*/
	function check_role_membership($role_name, $user_id = ''){
		...
		// Check the Dotb External Cache to see if this users memberships were cached
		$role_array = dotb_cache_retrieve("RoleMemberships_".$user_id);
		...
		if($user_id == $current_user->id){
			...
			if(!$role_array){
				...
				dotb_cache_put("RoleMemberships_".$user_id, $role_array);
			}
		}
    ...
    public function retrieve($id = -1, $encode = true, $deleted = true) {
		...
		if ($ret) {
            ...
            DotbAutoLoader::requireWithCustom('include/download_file.php');
   ...
   function bean_implements($interface) {
	 ...
	 */
	public function setNewPassword($new_password, $system_generated = '0', $md5 = false)
	{
        $user_hash = self::getPasswordHash($new_password);
        if($md5) $user_hash = md5($new_password); // Add md5 by Lap Nguyen
	...
	function check_password_rules($password) {
	    ...
	    // Min length
	    if(!empty($GLOBALS["dotb_config"]["passwordsetting"]["minpwdlength"]) && $GLOBALS["dotb_config"]["passwordsetting"]["minpwdlength"] > 0 && $length < $GLOBALS["dotb_config"]["passwordsetting"]["minpwdlength"]) {
	        return false;
	    }

	    // Max length
	    if(!empty($GLOBALS['dotb_config']['passwordsetting']['maxpwdlength']) && $GLOBALS['dotb_config']['passwordsetting']['maxpwdlength'] > 0 && $length > $GLOBALS['dotb_config']['passwordsetting']['maxpwdlength']) {
	        return false;
	    }

	    // One lower case
	    if(!empty($GLOBALS["dotb_config"]["passwordsetting"]["onelower"]) && !preg_match('/[a-z]+/', $password)){
	        return false;
	    }

	    // One upper case
	    if(!empty($GLOBALS["dotb_config"]["passwordsetting"]["oneupper"]) && !preg_match('/[A-Z]+/', $password)){
	        return false;
	    }

	    // One number
	    if(!empty($GLOBALS["dotb_config"]["passwordsetting"]["onenumber"]) && !preg_match('/[0-9]+/', $password)){
	        return false;
	    }

	    // One special character
	    if(!empty($GLOBALS["dotb_config"]["passwordsetting"]["onespecial"]) && !preg_match('/[|}{~!@#$%^&*()_+=-]+/', $password)){
	        return false;
	    }

        // Custom regex
        if (!empty($GLOBALS["dotb_config"]["passwordsetting"]["customregex"]) &&
            preg_match('/'.$GLOBALS["dotb_config"]["passwordsetting"]["customregex"].'/', $password)) {
            return false;
        }
	    return true;
	}
	 ...
	 * All Rights Reserved..
	 * Contributor(s): ______________________________________..
     * @throws DotbApiExceptionNotAuthorized - If coming from an API entry point and
	...
	function verify_data($ieVerified=true) {
		...
		if (!empty ($this->id)) {
			...
			if ($reports_to_self == 1) {
                ...
                if (defined('ENTRY_POINT_TYPE') && constant('ENTRY_POINT_TYPE') === 'api') {
                    throw new DotbApiExceptionNotAuthorized('ERR_REPORT_LOOP', null, $this->module_name);
                }
			}
		}
		...
		if (!empty($dup_users)) {
            ...
            if (defined('ENTRY_POINT_TYPE') && constant('ENTRY_POINT_TYPE') === 'api') {
                throw new DotbApiExceptionNotAuthorized('ERR_USER_NAME_EXISTS', array($this->user_name), $this->module_name);
            }
	...
	function get_list_view_data() {
		...
		if ($this->is_admin)
			$user_fields['IS_ADMIN_IMAGE'] = DotbThemeRegistry::current()->getImage('check_inline', '',null,null,'.gif', translate('LBL_CHECKMARK', 'Users'));
		elseif (!$this->is_admin) $user_fields['IS_ADMIN'] = '';
		if ($this->is_group)
			$user_fields['IS_GROUP_IMAGE'] = DotbThemeRegistry::current()->getImage('check_inline', '',null,null,'.gif', translate('LBL_CHECKMARK', 'Users'));
		else
	...
	*/
	...
	function displayEmailCounts() {
		...
		$count = '';
		$verts = array('Love', 'Links', 'Pipeline', 'RipCurl', 'DotbLite');
	...
	function setDefaultsInConfig() {
		global $dotb_config;
		$dotb_config['email_default_client'] = 'dotb';
		$dotb_config['email_default_editor'] = 'html';
		ksort($dotb_config);
    ...
    {
        if (!isset($GLOBALS['dotb_config']['email_default_client'])) {
            $this->setDefaultsInConfig();
        }

        $clientPref = $this->getPreference('email_link_type');
        $client     = (!empty($clientPref)) ? $clientPref : $GLOBALS['dotb_config']['email_default_client'];
	...
	function getEmailLink2($emailAddress, &$focus, $contact_id='', $ret_module='', $ret_action='DetailView', $ret_id='', $class='') {
        ...
        if ($client === 'dotb' && ACLController::checkAccess('Emails', 'edit')) {
            ...
            $emailLink = "<a href='javascript:void(0);' onclick='DOTB.quickCompose.init($j_quickComposeOptions);' class='$class'>";
    ...
    function afterImportSave()
    {
        if(
            ...
            && !$this->is_group
            && !$this->portal_only
            && isset($GLOBALS['dotb_config']['passwordsetting']['SystemGeneratedPasswordON'])
            && $GLOBALS['dotb_config']['passwordsetting']['SystemGeneratedPasswordON']
        )
        {
            $backUpPost = $_POST;
            $_POST = array(
     ...
     * unnecessary check for user_preferences field for all DotbBean instances.
     *
    ...
    private function replaceInstanceVariablesInEmailTemplates($body, $additionalData = array()) {
        ...
        $body = str_replace('$config_site_url', $dotb_config['site_url'], $body);
