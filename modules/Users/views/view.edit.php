...
class UsersViewEdit extends ViewEdit
{
    ...
    function display()
    {
        ...
        // if (!isset($this->bean->fetched_row['id']) || $_REQUEST['isDuplicate'] == 'true') {
            ...
            //             level: \'confirmation\',
            //             messages: DOTB.language.get(\'Users\', \'LBL_LIMIT_USER\'),
            //             autoClose: false,
            //             onConfirm: function () {
            ...
            //         AND status = 'Active'
            //         AND deleted = 0";
            //     $GLOBALS['log']->fatal($result = $GLOBALS['db']->query("SHOW COLUMNS FROM users LIKE 'teacher_id'"));
        ...
        $idpConfig = new IdmConfig(\DotbConfig::getInstance());
        ...
        $action_button_header[] = <<<EOD
                    <input type="button" id="SAVE_HEADER" title="{$APP['LBL_SAVE_BUTTON_TITLE']}" accessKey="{$APP['LBL_SAVE_BUTTON_KEY']}"
                          class="button primary" onclick="var _form = $('#EditView')[0]; if (!set_password(_form,newrules('{$minpwdlength}','{$maxpwdlength}','{$REGEX}'))) return false; _form.action.value='Save'; {$CHOOSER_SCRIPT} {$REASSIGN_JS} if(verify_data(EditView)) { submit_form(_form); }"
...
EOD;
     ...
     * We override the function from DotbView.php to make sure the create link only appears if the current user
