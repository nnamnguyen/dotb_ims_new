...
class UsersViewDetail extends ViewDetail {
    function preDisplay() {
        global $current_user, $app_strings, $dotb_config;
        ...
        if ((is_admin($current_user) || $_REQUEST['record'] == $current_user->id
            ...
            && !empty($dotb_config['default_user_name'])
            && $dotb_config['default_user_name'] == $this->bean->user_name
            && isset($dotb_config['lock_default_user_name'])
            && $dotb_config['lock_default_user_name']) {
            ...
            if ((is_admin($current_user)|| $GLOBALS['current_user']->isAdminForModule('Users')
                ...
                if (!$current_user->is_group && !$idpConfig->isIDMModeEnabled()) {
                    ...
                    if (!$this->bean->portal_only && !$this->bean->is_group && !$this->bean->external_auth_only
                        && isset($dotb_config['passwordsetting']['SystemGeneratedPasswordON']) && $dotb_config['passwordsetting']['SystemGeneratedPasswordON']){
        ...
        if ( $show_roles ) {
            ...
            $file = DotbAutoLoader::loadExtension("userpage");
            if($file) {
                include $file;
            }
