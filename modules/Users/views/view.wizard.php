...
class ViewWizard extends DotbView
{
	/**
	 * Constructor.
	 */
    ...
    {
        global $mod_strings, $current_user, $locale, $dotb_config, $app_list_strings, $dotb_version;
...
eoq;
		...
		$this->ss->assign("NUM_GRP_SEP",(empty($num_grp_sep) ? $dotb_config['default_number_grouping_seperator'] : $num_grp_sep));
		$this->ss->assign("DEC_SEP",(empty($dec_sep) ? $dotb_config['default_decimal_seperator'] : $dec_sep));
		...
		$this->ss->assign('TIMEOPTIONS', get_select_options_with_id($dotb_config['time_formats'], $current_user->_userPreferenceFocus->getDefaultPreference('default_time_format')));
		$this->ss->assign('DATEOPTIONS', get_select_options_with_id($dotb_config['date_formats'], $current_user->_userPreferenceFocus->getDefaultPreference('default_date_format')));
        ...
        if (array_key_exists($selectedLocaleNameFormat, $dotb_config['name_formats'])) {
            $selectedLocaleNameFormat = $dotb_config['default_locale_name_format'];
        }
        $this->ss->assign('NAMEOPTIONS', get_select_options_with_id($locale->getUsableLocaleNameOptions($dotb_config['name_formats']), $selectedLocaleNameFormat));
