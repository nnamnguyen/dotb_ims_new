...
class UsersController extends DotbController
{
    ...
    protected function action_resetPreferences()
    {
        ...
        if ($user->id !== $GLOBALS['current_user']->id) {
            DotbApplication::redirect("index.php?module=Users&record=" . $_REQUEST['record'] . "&action=DetailView"); //bug 48170]
        }
        echo '<script>parent.DOTB.App.router.navigate("logout/?clear=1", {trigger: true});</script>';
    }
	...
	protected function action_delete()
	{
            ...
            ))
        {
            ...
            if($u->portal_only == '0'){
                ...
                DotbApplication::redirect("index.php?module=Users&action=index");
            }
        }
        else
	...
	protected function action_clearreassignrecords()
	{
        if( $GLOBALS['current_user']->isAdminForModule('Users'))
	       ...
	       dotb_die("You cannot access this page.");
	}
	...
	protected function action_saveuserwizard()
	{
        ...
        $_POST['user_theme'] = (string) DotbThemeRegistry::getDefault();
     ...
     * @see DotbController::action_save()
     */
