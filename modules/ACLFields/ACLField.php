 ...
 * @api
 */
class ACLField extends DotbBean
{
                ...
                if ((
                    ...
                    (
                        ...
                        (
                            ...
                            ) || !empty($def['link']) || in_array($def['type'], DotbBean::$relateFieldTypes)
                        )
                    )
                ) {
...
AND deleted = 0';
    ...
    public static function loadUserFields($module_name, $object, $user_id, $refresh = false)
    {
        ...
        // we need to use $dictionary directly instead of DotbBean as its constructor calls this method,
    ...
    public static function listFilter(&$list, $category, $user_id, $is_owner, $by_key = true, $min_access = 1, $blank_value = false, $addACLParam = false, $suffix = '')
    {
        foreach($list as $key=>$value){
            ...
            if(!isset(self::$field_cache[$is_owner][$field])){
                ...
                $access = DotbACL::getFieldAccess($category, $field, $context);
    ...
    */
     ...
     *
     * @param DotbQuery_Builder_Condition $condition Original condition
     * @param User|null $user ACL user
     *
     * @return DotbQuery_Builder_Where|null ACL specific condition or NULL if not applicable
     */
    public static function generateAclCondition(DotbQuery_Builder_Condition $condition, User $user = null)
    {
     ...
     *
     * @param DotbQuery $query The query to generate the condition for
     * @param string $module The module which the records being selected belong to
     * @param User $user ACL user
     *
     * @return DotbQuery_Builder_Where|null Condition or NULL if not applicable
     */
    protected static function generateIsOwnerCondition(DotbQuery $query, $module, User $user)
    {
        ...
        while (count($fields) > 0) {
            ...
            $condition = new DotbQuery_Builder_Condition($query);
            $condition->setOperator('=')->setField($field)->setValues($user->id)->ignoreAcl();

            $result = new DotbQuery_Builder_Orwhere($query);
            $result->add($condition);

            if ($previous !== null) {
                $isNull = new DotbQuery_Builder_Condition($query);
                $isNull->setField($field)->isNull()->ignoreAcl();

                $and = new DotbQuery_Builder_Andwhere($query);
                $and->add($isNull);
                $and->add($previous);
