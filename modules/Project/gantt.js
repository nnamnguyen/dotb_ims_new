	...
	return {
		...
		getTaskRowMap: function(){
			...
			for(var i=1; i<taskRows.length; i++){
				...
				rowMap[i-1] = { id:DOTB.grid.getActualRow(rowId) };
			}
		...
		createTable: function(view_type, calendar_start, bgcolor){
			...
			if (document.getElementById('gantt_chart_start_date').value.length != 0){
				calendar_start_date = DOTB.grid.getJSDate(document.getElementById('gantt_chart_start_date').value);
				date = DOTB.grid.getJSDate(document.getElementById('gantt_chart_start_date').value);
			}
			else{
				calendar_start_date = DOTB.grid.getJSDate(document.getElementById('calendar_start').value);
				date = DOTB.grid.getJSDate(document.getElementById('calendar_start').value);
			}
	        ...
	        ganttDateEmptyFirstCell.innerHTML = "<a href='#' onclick='DOTB.gantt.moveChart(\"backward\", document.getElementById(\"gantt_chart_start_date\").value, \""+bgcolor+"\");'><img width='8' height='11' border='0' align='absmiddle' alt='Previous' src='index.php?entryPoint=getImage&themeName="+DOTB.themes.theme_name+"&imageName=previous.gif'/></a>";
			...
			if (view_type == 'month'){
				for (var i=0; i<5; i++){
	        		...
	        		if ((nextDate.getTimezoneOffset() - date.getTimezoneOffset()) != 0){
		        		...
		        		dateText = DOTB.grid.getDisplayDate(DSTnextDate);
	        		}
	        		else{
	        			dateText = DOTB.grid.getDisplayDate(nextDate);
	        		}
	        ...
	        ganttDateEmptyLastCell.innerHTML = "<a href='#' onclick='DOTB.gantt.moveChart(\"forward\", document.getElementById(\"gantt_chart_start_date\").value, \""+bgcolor+"\");'><img width='8' height='11' border='0' align='absmiddle' alt='Next' src='index.php?entryPoint=getImage&themeName="+DOTB.themes.theme_name+"&imageName=next.gif'/></a>";
	        ...
	        var offset = calendar_start_date.getDay();
               ...
               tempDate = DOTB.grid.getJSDate(calendar_start);
               today = new Date;
	        ...
	        if (view_type == 'month'){
	            for (var j=offset; j<(35+offset); j++){
	                ...
	                ganttDate.id = DOTB.grid.getDisplayDate(tempDate);

	                if (DOTB.gantt.compareToToday(tempDate)){
	        ...
	        rowMap = DOTB.gantt.getTaskRowMap();

	        if (rowMap.length != 0){
	        	for (var i=0; i<rowMap.length; i++){
	        		DOTB.gantt.addGanttRow(rowMap[i].id, true);
	        	}
	        }
	    },
	    ...
	    moveChart: function(direction, current_gantt_start, bgcolor){
	    	...
	    	var gantt_start_date = DOTB.grid.getJSDate(current_gantt_start);
	    	...
	    	new_gantt_start = DOTB.gantt.fixDSTOffset(gantt_start_date, new_gantt_start);

	    	var return_gantt_date = DOTB.grid.getDisplayDate(new_gantt_start);

	    	document.getElementById('gantt_chart_start_date').value = return_gantt_date;

	    	DOTB.gantt.createTable(document.getElementById('gantt_chart_view').value, return_gantt_date, bgcolor);
	    },
	    ...
	    changeTask: function(task_num){
			...
			task_start_date = DOTB.grid.getJSDate(start_date);
			task_end_date = DOTB.grid.getJSDate(end_date);

	        if (DOTB.gantt.daysBetween(task_start_date, task_end_date) > 0){
	        	duration = DOTB.gantt.daysBetween(task_start_date, task_end_date) + 1;
	        }
	        else{
	        	duration = 1;
	        }
			...
			calendar_end_date.setTime( calendar_start_date.getTime() + (DOTB.gantt.getNumCols()-3)*oneDay );

			taskRow = document.getElementById('gantt_row_'+task_num);
			var emptyRow = false;

			var status = (DOTB.gantt.isParent(task_num)) ? 'parent' : DOTB.gantt.getStatus(progress);
			...
			if (task_start_date > calendar_start_date && task_start_date <= calendar_end_date){
				start = DOTB.gantt.daysBetween(calendar_start_date, task_start_date);
				start++;
			}
			else{
				start = 1
				...
				// the task's duration overlaps, so display the partial bar of the task
					...
					duration = duration - DOTB.gantt.daysBetween(calendar_start_date, task_start_date);
				}
			}
	        ...
	        if (!emptyRow){
		        var maxMiddleCell = DOTB.gantt.getNumCols() - document.getElementById('task_'+task_num+'_id').colSpan - 1;

		        duration = Math.min(maxMiddleCell, duration);
		        document.getElementById('task_'+task_num+'_bar').colSpan = duration;

		        document.getElementById('task_'+task_num).colSpan = DOTB.gantt.getNumCols() - document.getElementById('task_'+task_num+'_id').colSpan - document.getElementById('task_'+task_num+'_bar').colSpan;
		        document.getElementById('task_'+task_num+'_bar').innerHTML = '<div style="width:0%" class="'+status+'" id="bar_'+task_num+'">\u00a0</div>';

		        if (status != 'inprogress'){
		        	DOTB.gantt.animateBar('bar', task_num);
		        }
		        else{
		            DOTB.gantt.setProgress(task_num, progress);
			    }

		        bar.onclick = function() {  DOTB.gantt.taskOverLib(this,DOTB.gantt.popupInfo(task_num),
						  						document.getElementById("description_"+task_num).value); }
	        }
	        else{
	        	/* remove middle and last cells for an empty row */
	        	document.getElementById('task_'+task_num+'_bar').colSpan = DOTB.gantt.getNumCols() - 2;
	    ...
	    setProgress: function(task_num, progress){
	        ...
	        if (uncompleted != 0){
	        	DOTB.gantt.animateBar('inprogress_bar', task_num, progress);
	        }
	        DOTB.gantt.animateBar('bar', task_num, uncompleted);
	    ...
	    addGanttRow: function(task_num, new_table){
            ...
            var mappedRowId = DOTB.grid.getMappedRow(task_num);
	        ...
	        cellLeft.setAttribute('id', 'task_'+DOTB.grid.getMappedRow(task_num)+'_id');

	        // insert Middle Cell
	        var cellMiddle = row.insertCell(1);
	        cellMiddle.setAttribute('id', 'task_'+DOTB.grid.getMappedRow(task_num)+'_bar');

	        // insert Right Cell
	        var cellRight = row.insertCell(2);
	        cellRight.innerHTML = task_num;
	        cellRight.setAttribute('id', 'task_'+DOTB.grid.getMappedRow(task_num));

	        if (document.getElementById('is_milestone_' + DOTB.grid.getMappedRow(task_num)).value == 1){
	        	DOTB.gantt.markAsMilestone(DOTB.grid.getMappedRow(task_num));
	        }

	        DOTB.gantt.changeTask(DOTB.grid.getMappedRow(task_num));
	    },
	    ...
	    unMarkAsMilestone: function(tasknum) {
	    	document.getElementById('task_' + tasknum + '_id').innerHTML = DOTB.grid.getActualRow(tasknum);
			document.getElementById('task_' + tasknum).innerHTML = DOTB.grid.getActualRow(tasknum);
	    },
	    ...
	    selectRow: function(tasknum){
	        ...
	        document.getElementById('start_date').value = DOTB.gantt.calculateDateByDifference(document.getElementById('calendar_start').value, start);
	    ...
	    gridOnly: function(){
			...
			DOTB.grid.showProjectButtons("grid_only");
	    },
	    ...
	    ganttOnly: function(){
			...
			DOTB.grid.showProjectButtons("gantt_only");
	    },
	    ...
	    gridGanttView: function(){
			...
			DOTB.grid.showProjectButtons("grid_gantt");
	    	initUI();
	    },
		...
		sliderInit: function() {
			...
			horizontalSlider.onChange = function(offsetFromStart) {
				if (offsetFromStart == 25)
					DOTB.gantt.createTable('week', document.getElementById('calendar_start').value);
				else if (offsetFromStart == 50)
					DOTB.gantt.createTable('biweek', document.getElementById('calendar_start').value);
				else if (offsetFromStart == 75)
					DOTB.gantt.createTable('month', document.getElementById('calendar_start').value);
		...
		init: function() {
			DOTB.gantt.sliderInit();
			DOTB.gantt.resizerInit();
		},

		popupInfo: function(task_num){
			/* start date */
			var start_date = DOTB.language.get('Project', 'LBL_POPUP_DATE_START') + document.getElementById('date_start_'+task_num).value + "<br />";

			/* end_date */
			var end_date = DOTB.language.get('Project', 'LBL_POPUP_DATE_FINISH') + document.getElementById('date_finish_'+task_num).value + "<br />";

			/* percent_complete */
			var percent_complete = DOTB.language.get('Project', 'LBL_POPUP_PERCENT_COMPLETE') + document.getElementById('percent_complete_'+task_num).value + "<br />";

			/* resources */
			if (document.getElementById('resource_full_name_'+task_num) && document.getElementById('resource_full_name_'+task_num).value != ''){
				var resource_full_name = DOTB.language.get('Project', 'LBL_POPUP_RESOURCE_NAME') + document.getElementById('resource_full_name_'+task_num).value + "<br />";
			}
			else{
		...
		taskOverLib: function(el,info, description){
			DOTB.util.getStaticAdditionalDetails(el,info,description);
		},
