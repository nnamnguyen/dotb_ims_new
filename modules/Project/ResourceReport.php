...
$dotb_smarty->assign("DATE_FORMAT", $timedate->get_date_format());
$dotb_smarty->assign("CURRENT_USER", $current_user->id);
$dotb_smarty->assign("CALENDAR_LANG_FILE", getJSPath(dotb_cached('jsLanguage/'.$GLOBALS['current_language'].'.js')));
...
if (!empty($_REQUEST['resource'])) {
    $dotb_smarty->assign('DATE_START', $request->getValidInputRequest('date_start'));
    $dotb_smarty->assign('DATE_FINISH', $request->getValidInputRequest('date_finish'));
    $dotb_smarty->assign('SELECTED_RESOURCE', $request->getValidInputRequest('resource'));
...
$dotb_smarty->assign("DATE_RANGE_ARRAY", $dateRangeArray);

echo $dotb_smarty->fetch('modules/Project/ResourceReport.tpl');
insert_popup_footer();
