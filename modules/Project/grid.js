	...
	return {
		clickedRow: function(taskId, event) {
			...
			if (event.shiftKey && selectedRows.length > 0) {
				if (DOTB.grid.getActualRow(taskId) > DOTB.grid.getActualRow(selectedRows[selectedRows.length - 1])) {
					for (var i = DOTB.grid.getActualRow(selectedRows[selectedRows.length - 1]) + 1; i <= DOTB.grid.getActualRow(taskId); i++)
						DOTB.grid.toggleSelectRow(DOTB.grid.getMappedRow(i));
				}
				else {
					for (var i = DOTB.grid.getActualRow(selectedRows[selectedRows.length - 1]) - 1; i >=DOTB.grid.getActualRow(taskId) ; i--)
						DOTB.grid.toggleSelectRow(DOTB.grid.getMappedRow(i));
				...
				DOTB.grid.toggleSelectRow(taskId);
			}
			else {
				while(selectedRows.length > 0){
					DOTB.grid.toggleSelectRow(selectedRows[0]);
				}
				DOTB.grid.toggleSelectRow(taskId);
			}
		},
		...
		blurEvent: function(row, div, input, divlink)
		{
			...
			if (document.getElementById(divlink).style.display != 'none') {
					...
					if (divlinkVal.search("Minus.gif") != -1) {
						...
						"<img src='" + imagePathProjectMinus + "' onClick='javascript:DOTB.grid.expandCollapseRow("+ row +")'>&nbsp;" +
						"<b>" + document.getElementById(input).value + "</b>";
					}
					else if (divlinkVal.search("Plus.gif") != -1) {
						document.getElementById(divlink).innerHTML =
						"<img src='" + imagePathProjectPlus + "' onClick='javascript:DOTB.grid.expandCollapseRow("+ row +")'>&nbsp;" +
						"<b>" + document.getElementById(input).value + "</b>";
					}
					else {
						document.getElementById(divlink).innerHTML = document.getElementById(input).value;
		...
		copyRow: function()
		{
			...
			// Clear out cutRows
			cutRows = new Array();
			selectedRows.sort(DOTB.grid.sortNumber);
			while (selectedRows.length != 0) {
				var row = selectedRows[0];
				copyRows.push(row);
				DOTB.grid.unSelectRow(row);
			}
		},
		...
		pasteRow: function()
		{
			...
			if (selectedRows.length == 0)
				...
				insertAtRow = DOTB.grid.getActualRow(selectedRows[0]);
			while (copyRows.length > 0) {
				// Insert blank rows before we can copy the data over.
				DOTB.grid.insertRow(insertAtRow);
				var mappedRow = DOTB.grid.getMappedRow(insertAtRow);
				...
				DOTB.grid.indentRow(mappedRow, true);
				copyRows.splice(copyRows.length - 1, 1);
				DOTB.gantt.changeTask(mappedRow);
			}
		...
		deleteRows: function()
		{
			...
			while (selectedRows.length != 0) {
				...
				DOTB.grid.getChildren(row, allChildren);
				if (allChildren.length > 0) {
					if (confirm(DOTB.language.get('Project', 'NTC_DELETE_TASK_AND_SUBTASKS'))) {
						for (var i = (allChildren.length - 1); i >= 0; i--)
							DOTB.grid.deleteRow(allChildren[i]);
						DOTB.grid.deleteRow(row);
					}
					else
						return;
				}
		...
		{
			var table = document.getElementById(gridName);
			var rowNum = DOTB.grid.getActualRow(row);
			var atRow = row;
			...
			var ancestorObj = DOTB.grid.getAncestors(row, allAncestors, ancestorLevels);
			...
			for (var i = 1; i < predecessors.length; i++) {
				if (predecessors[i] && predecessors[i].length > 0) {
					for (var j = 0; j < predecessors[i].length; j++) {
						if (DOTB.grid.getActualRow(predecessors[i][j]) >= rowNum) {
							DOTB.grid.updateRowWithPredecessorAtIndex(i, j, 'delete', rowNum);
						}
					}
					DOTB.grid.cleanupPredecessorRow(i); //removes extra commas as a result of the predecessor row being deleted.
				}
			}
			...
			for (var i = 1; i <= totalRowsInGrid; i++) {
				if (DOTB.grid.getActualRow(i)!=-1 && DOTB.grid.getActualRow(i) >= rowNum) {
					...
					DOTB.gantt.updateGanttRowMappings(i, newVal);
				}
			}

			for (var i = 1; i < predecessors.length; i++) {
				if (predecessors[i] && predecessors[i].length > 0) {
					DOTB.grid.updatePredecessorsAndDependents(i);
				}
			}
			...
			DOTB.grid.unSelectRow(atRow);
			table.deleteRow(rowNum);
			DOTB.gantt.deleteGanttRow(rowNum);

			DOTB.grid.updateAncestorsTimeData(null, allAncestors);
			DOTB.grid.updateAncestorsPercentComplete(null, allAncestors);
		},
		...
		insertRow: function(atRow)
		{
			...
			if (selectedRows.length == 0 && !atRow) { // If no rows have been selected, add it to the end
				...
				rowNum = atRow;
				atRow = DOTB.grid.getMappedRow(atRow);
				isAtEndOfGrid = false;
			}
			else {
				atRow = selectedRows.sort(DOTB.grid.sortNumber)[0];
				rowNum = DOTB.grid.getActualRow(atRow);
				if (rowNum == 0)
					rowNum = 1;
				isAtEndOfGrid = false;
			}
			...
			cell.onclick = function(event) {DOTB.grid.clickedRow(numRows, event);};
			...
			cell.innerHTML = "<input type=text size=3 style='border:0' name='percent_complete_" + numRows + "' id='percent_complete_" + numRows + "'" +
					" value=0 onBlur='if (DOTB.grid.validatePercentComplete(this)){ " +
						"DOTB.grid.updateAncestorsPercentComplete(" + numRows + ");" +
						" DOTB.gantt.changeTask(" + numRows + ");}'>";

			cell = row.insertCell(2);
			cell.ondblclick = function(){DOTB.grid.toggle("description_" + numRows + "_div", "description_" + numRows, "description_" + numRows + "_divlink");};
			...
			cell.innerHTML =
				...
				" onblur='DOTB.grid.blurEvent(" + numRows + ", \"description_" + numRows + "_div\",\"description_" + numRows + "\", \"description_" + numRows + "_divlink\");'> " +
			...
			cell.innerHTML =
				"<input type=text  style='border:0' size=3 name=duration_" + numRows + " id=duration_" + numRows +
					" value=0 onBlur='if (DOTB.grid.validateDuration(this)){ " +
					"DOTB.grid.calculateEndDate(document.getElementById(\"date_start_" + numRows + "\").value, this.value, " + numRows + ");" +
					" DOTB.grid.updateAllDependentsAfterDateChanges(" + numRows + ");" +
					" DOTB.gantt.changeTask(" + numRows + ");}'>";

			cell = row.insertCell(4);
			cell.innerHTML =
				"<input type='hidden' name='duration_unit_hidden_" + numRows + "' id='duration_unit_hidden_" + numRows + "'>" +
				"<select  style='border:0' name=duration_unit_" + numRows + " id=duration_unit_" + numRows + " onChange='DOTB.grid.changeDurationUnits("+ numRows + ")'>" +
				durationUnitsOptionsString + "</select>";

			cell = row.insertCell(5);
			cell.innerHTML =
				"<input name='date_start_" + numRows + "' id='date_start_" + numRows + "' style='border:0' onchange='parseDate(this, \"" + calendar_dateformat + "\"); DOTB.grid.processStartDate(this, \"" + numRows + "\");' " +
					" type='text' tabindex='2' size='11' maxlength='10' value=''>";

			cell = row.insertCell(6);
			cell.innerHTML =
				"<input name='date_finish_" + numRows + "' id='date_finish_" + numRows + "' style='border:0' onchange='parseDate(this, \"" + calendar_dateformat + "\"); DOTB.grid.processFinishDate(this, \"" + numRows + "\");' " +
					" type='text' tabindex='2' size='11' maxlength='10' value=''>";

			var startDateObj = DOTB.grid.getNextValidDate(new Date());
			var startDate = DOTB.grid.getDisplayDate(startDateObj.date);
			...
			cell.innerHTML =
				"<input type=text  style='border:0' size=10 name=predecessors_" + numRows + " id=predecessors_" + numRows +
					" onBlur='if (DOTB.grid.validatePredecessors(this)){" +
						"DOTB.grid.setDependencyCheckRow(" + numRows + ");" +
						"DOTB.grid.validateDependencyCycles(" + numRows + ");" +
						"DOTB.grid.validatePredecessorIsNotAncestorOrChild(" + numRows + ");"+
						"if (DOTB.grid.dependencyCheckFail != 1){" +
						"DOTB.grid.updatePredecessorsAndDependents(" + numRows + ");" +
						"DOTB.grid.calculateDatesAfterAddingPredecessors(" + numRows + ");" +
						"DOTB.grid.updateAllDependentsAfterDateChanges(" + numRows + ");" +
						"DOTB.gantt.changeTask(" + numRows + ");}}" +
			...
			if (!isProjectTemplate) {
				...
				cell.innerHTML =
					...
					" onChange='DOTB.grid.updateResourceFullNameAndType("+ numRows +");DOTB.grid.calculateEndDate(document.getElementById(\"date_start_"+ numRows + "\").value, document.getElementById(\"duration_" + numRows + "\").value, "+ numRows + "); " +
								"DOTB.grid.calculateDatesAfterAddingPredecessors(" + numRows + ");" +
								"DOTB.grid.updateAllDependentsAfterDateChanges(" + numRows + ");" +
								"DOTB.gantt.changeTask(" + numRows +");'>" +
				...
				cell.innerHTML =
					"<input type=text size=10 style='border:0' onBlur='DOTB.grid.validateDuration(this);' name=actual_duration_"+ numRows + " id=actual_duration_" + numRows + ">";
			}



			DOTB.grid.setupCalendar(numRows, calendar_dateformat, bg_color, gridName, imagePathProjectMinus, imagePathProjectPlus);

			for (var i = 1; i < predecessors.length; i++) {
				if (predecessors[i] && predecessors[i].length > 0) {
					for (var j = 0; j < predecessors[i].length; j++) {
						if (DOTB.grid.getActualRow(predecessors[i][j]) >= rowNum) {
							DOTB.grid.updateRowWithPredecessorAtIndex(i, j, 'insert');
						}
					}
				}
			}
			...
			if (!isAtEndOfGrid) {
				for (var i = 1; i < totalRowsInGrid; i++) {
					if (DOTB.grid.getActualRow(i) >= rowNum) {
						var newVal = parseInt(document.getElementById("mapped_row_" + i).value) + 1;
						document.getElementById("mapped_row_" + i).value = newVal;
						document.getElementById("id_" + i + "_divlink").innerHTML = newVal;
			...
			DOTB.gantt.addGanttRow(rowNum);
			DOTB.grid.indentInsertedRow(numRows);

            DOTB.grid.onAfterInsertRow();
		},
		...
		updatePredecessorsAndDependents: function(row) {
			...
			if (predecessorsVal == "") {
				...
				for (j in predecessors[row]) {
					predecessors[row][j] = DOTB.grid.getMappedRow(parseInt(predecessors[row][j]));
		...
		{
			var newVal = parseInt(DOTB.grid.getActualRow(predecessors[row][index]));
		...
		isHoliday: function(startDate, resource) {
			...
			if (resource != null && resource != "") {
				if (resourceHolidays[resource] && resourceHolidays[resource][DOTB.grid.getDisplayDate(startDate)] != null) {
					return true;
				}
			}
		},
		...
		{
			if (document.getElementById("duration_unit_" + row).value == "Hours")
				return DOTB.grid.calculateEndDateHours(dateStart, duration, row);
			else if (document.getElementById("duration_unit_" + row).value == "Days")
				return DOTB.grid.calculateEndDateHours(dateStart, duration * workDayHours, row);
		},

		calculateEndDateHours: function(dateStart, duration, row)
		{
			var startDate = DOTB.grid.getJSDate(dateStart);
			...
			while (currDuration < parseInt(duration)) {
				while (!DOTB.grid.isDuringWorkingHours(startTime) || DOTB.grid.isHoliday(startDate, resource)) {
			...
			currDate = DOTB.grid.getDisplayDate(currDate);
		...
		{
			if (document.getElementById("duration_unit_" + row).value == "Hours")
				return DOTB.grid.calculateDurationHours(dateStart, dateFinish, row);

			var startDate = DOTB.grid.getJSDate(dateStart);
			var endDate = DOTB.grid.getJSDate(dateFinish);
			var resource = document.getElementById("resource_" + row).value;
			var duration = 0;
			while (startDate.getTime() <= endDate.getTime()) {
				if (!DOTB.grid.isHoliday(startDate, resource))
					duration++;
				startDate.setDate(startDate.getDate() + 1);
			}
		...
		calculateDurationHours: function(dateStart, dateFinish, row) {
			...
			if (!DOTB.grid.isHoliday(endDate, resource)) {
				if (duration != 0)
		...
		{
			var index = DOTB.grid.getRowIndexInSelectedRows(mappedRow);

			// This row hasn't been selected and we need to select it
			if (index == -1) {
				DOTB.grid.selectRow(mappedRow);
			}
			else { //unselect the row
				DOTB.grid.unSelectRow(mappedRow);
			}
		...
		getAncestors: function(row, allAncestors, ancestorLevels)
		{
			...
			return DOTB.grid.getAncestors(allAncestors[allAncestors.length - 1], allAncestors, ancestorLevels);
		},
		...
		getChildren: function(row, allChildren)
		{
			...
			for (var i = 0; i < children.length; i++) {
				DOTB.grid.getChildren(children[i], allChildren);
			}
		},
		...
		addToOutdent: function(row)
		{
			selectedRows.sort(DOTB.grid.sortNumber);
			while (selectedRows.length != 0) {
				DOTB.grid.outdentRow(selectedRows[selectedRows.length-1]);
				DOTB.grid.unSelectRow(selectedRows[selectedRows.length-1]);
			}
		},
		...
		outdentRow: function(row)
		{
			...
			DOTB.grid.getChildren(parentRow, allChildren);

			for (var i = 0; i < allChildren.length; i++) {
				if (DOTB.grid.getActualRow(row) < DOTB.grid.getActualRow(allChildren[i])) {
					if (indentedRows[allChildren[i]].parentRow == parentRow) {
						...
						DOTB.grid.validatePredecessorIsNotAncestorOrChild(allChildren[i], false);
					}
				}
				else {
			...
			if (!parentHasOtherChildren) {
				...
				DOTB.grid.indentRow(parentRow, true);
				DOTB.gantt.removeParentTask(parentRow);
			}
			allChildren = new Array();
			...
			if (allChildren.length > 0) {
				newElementVal += "<img src='" + imagePathProjectMinus + "' onClick='javascript:DOTB.grid.expandCollapseRow("+ row +")'>&nbsp;";
				...
				DOTB.grid.indentRow(row, true);
				DOTB.grid.calculateCumulativePercentComplete(row);
				DOTB.grid.removeDependentChildrenAfterOutdent(row, allChildren);
				DOTB.grid.calculateParentDateDataAfterIndent(row);
				DOTB.gantt.parentTask(row);
			}
			...
			allChildren = new Array();
			DOTB.grid.calculateParentDateDataAfterIndent(parentRow);
			DOTB.grid.calculateCumulativePercentComplete(parentRow);
			DOTB.grid.updateAncestorsTimeData(row);
			DOTB.grid.updateAncestorsPercentComplete(row);
		...
		removeDependentChildrenAfterOutdent: function(row, allChildren) {
			for (var i = 0; i < allChildren.length; i++) {
				...
				for (var j = 0; j < predecessorsArray.length; j++) {
					if (predecessorsArray[j] == DOTB.grid.getActualRow(row)) {
						predecessorsArray[j] = "";
					}
				}
				var predecessorVal = predecessorsArray.join(",");
				document.getElementById("predecessors_" + allChildren[i]).value = predecessorVal;
				DOTB.grid.cleanupPredecessorRow(allChildren[i]);
				DOTB.grid.updatePredecessorsAndDependents(allChildren[i]);
			}
		},
		...
		indentSelectedRows: function ()
		{
			...
			while(tempArray.length != selectedRows.length)
				tempArray.push(DOTB.grid.getActualRow(selectedRows[tempArray.length]));
			tempArray.sort(DOTB.grid.sortNumber);
			while (selectedRows.length != 0) {
				DOTB.grid.addToIndent(DOTB.grid.getMappedRow(tempArray[0]));
				DOTB.grid.unSelectRow(DOTB.grid.getMappedRow(tempArray[0]));
		...
		addToIndent: function(row)
		{
			...
			// This is the parent
			if (indentedRows[row]) {
				for (var i = DOTB.grid.getActualRow(row) - 1; i >= 0 ; i--) {
					if (indentedRows[DOTB.grid.getMappedRow(i)] && indentedRows[DOTB.grid.getMappedRow(i)].parentRow == indentedRows[row].parentRow) {
						indentedRows[row].levelsIndented++;
						indentedRows[row].parentRow = DOTB.grid.getMappedRow(i);
						document.getElementById("parent_" + row).value = DOTB.grid.getMappedRow(i);
						canIndent = true;
						break;
					}
				}
				...
				for (var i = DOTB.grid.getActualRow(row) - 1; i >= 1 ; i--) {
					if (!indentedRows[DOTB.grid.getMappedRow(i)] || (indentedRows[DOTB.grid.getMappedRow(i)].parentRow == -1)) {
						rowObj = new Object();
						rowObj.levelsIndented = 1;
						rowObj.parentRow = DOTB.grid.getMappedRow(i);
						document.getElementById("parent_" + row).value = DOTB.grid.getMappedRow(i);
		...
		indentParentRow: function(row) {
			...
			if (parentElementVal.search(".gif") == -1)
			{
				...
				if (parentDataStart > 0) {
					...
					newParentElementVal = "";
				newParentElementVal += "<img src='" + imagePathProjectMinus + "' onClick='javascript:DOTB.grid.expandCollapseRow("+ indentedRows[row].parentRow +")'>&nbsp;";
			...
			DOTB.gantt.parentTask(indentedRows[row].parentRow);
		},
		...
		indentRow: function(row, formatOnly)
		{
			...
			if (!formatOnly) {
				DOTB.grid.validatePredecessorIsNotAncestorOrChild(row, false);
				for (var i = 0; i < indentedRows[row].childrenRows.length; i++) {
					DOTB.grid.validatePredecessorIsNotAncestorOrChild(indentedRows[row].childrenRows[i], false);
				}

				DOTB.grid.indentParentRow(row);
				DOTB.grid.indentChildrenRows(row);
			}
		},
		...
		indentInsertedRow: function (row) {
			var actualRow = DOTB.grid.getActualRow(row);
			var table = document.getElementById(gridName);
			...
			var nextRow = DOTB.grid.getMappedRow(parseInt(actualRow) + 1);
		...
		getNextValidDate: function(date, startTime, resource) {
			...
			while (!DOTB.grid.isDuringWorkingHours(startTime) || DOTB.grid.isHoliday(date, resource)) {
		...
		calculateDatesAfterAddingPredecessors: function(row) {
			if (DOTB.grid.isLoaded){
				var resource = document.getElementById("resource_" + row).value;

				if (predecessors[row] != null && predecessors[row].length > 0) {
					for (var i = 0; i < predecessors[row].length; i++) {
						if (i == 0) {
							lastEndDate = DOTB.grid.getJSDate(document.getElementById('date_finish_' + predecessors[row][i]).value);
							lastEndTime = document.getElementById('time_finish_' + predecessors[row][i]).value;
							lastEndIndex = predecessors[row][i];
						}
						endDate = DOTB.grid.getJSDate(document.getElementById('date_finish_' + predecessors[row][i]).value);
					...
					var startDate = DOTB.grid.getJSDate(document.getElementById('date_finish_' + lastEndIndex).value);
					var startTime = document.getElementById('time_finish_' + lastEndIndex).value;
					startDateObj = DOTB.grid.getNextValidDate(startDate, startTime, resource);
					startDate = DOTB.grid.getDisplayDate(startDateObj.date);
					document.getElementById('date_start_' + row).value = startDate;
					document.getElementById('time_start_' + row).value = startDateObj.time;
					DOTB.grid.calculateEndDate(document.getElementById('date_start_' + row).value, document.getElementById('duration_' + row).value, row);
					DOTB.gantt.changeTask(row);
				}
			}
		},
		updateAncestorDates: function(ancestorRow, childRow) {
			...
			startDate = DOTB.grid.getJSDate(document.getElementById('date_start_' + ancestorRow).value);
			startTime = document.getElementById('time_start_' + ancestorRow).value;
			endDate = DOTB.grid.getJSDate(document.getElementById('date_finish_' + ancestorRow).value);
			endTime = document.getElementById('time_finish_' + ancestorRow).value;

			childStartDate = DOTB.grid.getJSDate(document.getElementById('date_start_' + childRow).value);
			childStartTime = document.getElementById('time_start_' + childRow).value;
			childEndDate = DOTB.grid.getJSDate(document.getElementById('date_finish_' + childRow).value);
			...
			DOTB.grid.calculateDuration(document.getElementById('date_start_' + ancestorRow).value, document.getElementById('date_finish_' + ancestorRow).value, ancestorRow);
			DOTB.gantt.changeTask(ancestorRow);
			return changed;
		},
		...
		updateAllDependentsAfterDateChanges: function(row) {
			...
			while (extendedDependents && extendedDependents.length > 0) {
				...
				DOTB.grid.calculateDatesAfterAddingPredecessors(firstDependent);
			...
			for (var i in extendedAncestors) {
				DOTB.grid.calculateParentDateDataAfterIndent(extendedAncestors[i]);
			}
		},
		...
		calculateParentDateDataAfterIndent: function(parentRow) {
			if (DOTB.grid.isLoaded){
				...
				for (var i = 0; i < allChildren.length; i++) {
					if (i == 0) {
						firstStartDate = DOTB.grid.getJSDate(document.getElementById('date_start_' + allChildren[i]).value);
						firstStartTime = document.getElementById('time_start_' + allChildren[i]).value;
						lastEndDate = DOTB.grid.getJSDate(document.getElementById('date_finish_' + allChildren[i]).value);
					...
					startDate = DOTB.grid.getJSDate(document.getElementById('date_start_' + allChildren[i]).value);
					startTime = document.getElementById('time_start_' + allChildren[i]).value;
					endDate = DOTB.grid.getJSDate(document.getElementById('date_finish_' + allChildren[i]).value);
				...
				if (allChildren.length > 0) {
					...
					document.getElementById('time_finish_' + parentRow).value =
						document.getElementById('time_finish_' + endDateIndex).value;
					DOTB.grid.calculateDuration(firstStartDate, lastEndDate, parentRow);
					// Calling the functions that the onchange would call directly as the clearStartTime() in the onChange messes up the calcs.
					DOTB.grid.calculateDuration(document.getElementById('date_start_' + parentRow).value, document.getElementById('date_finish_' + parentRow).value, parentRow);
					DOTB.grid.updateAllDependentsAfterDateChanges(parentRow);
					DOTB.gantt.changeTask(parentRow);
				}
			}
		},
		...
		updateAncestorsTimeData: function(row, ancestors) {
			if (ancestors == null) {
				...
				for (var i = 0; i < allAncestors.length; i++)
					DOTB.grid.calculateParentDateDataAfterIndent(allAncestors[i]);
			}
			else {
				for (var i = 0; i < ancestors.length; i++)
					DOTB.grid.calculateParentDateDataAfterIndent(ancestors[i]);
			}
		},
		...
		updateAncestorsPercentComplete: function(row, ancestors) {
			if (ancestors == null) {
				...
				allAncestors = ancestorObj.allAncestors;
				for (var i = 0; i < allAncestors.length; i++) {
					DOTB.grid.calculateCumulativePercentComplete(allAncestors[i]);
				}
			}
			else {
				for (var i = 0; i < ancestors.length; i++) {
					DOTB.grid.calculateCumulativePercentComplete(ancestors[i]);
				}
			}
		},
		...
		reCalculateDates: function() {
			for (var i = 1; i <= totalRowsInGrid; i++) {
				...
				if (document.getElementById('date_start_' + i)) {
				 	DOTB.grid.calculateEndDate(document.getElementById('date_start_' + i).value, document.getElementById('duration_' + i).value, i);
				 	DOTB.grid.calculateDatesAfterAddingPredecessors(i);
					DOTB.grid.updateAllDependentsAfterDateChanges(i);
					DOTB.grid.updateAncestorsTimeData(i);
					DOTB.gantt.changeTask(i);
				}
			}
		},
		...
		expandAll: function() {
			for (var i = 1; i <= totalRowsInGrid; i++) {
				...
				if (document.getElementById(elementId).innerHTML.search("Plus.gif") != -1) {
					...
					var allChildren = new Array();
					DOTB.grid.getChildren(i, allChildren);
					var lastChildRow = -1;
					for (var j = 0; j < allChildren.length; j++) {
						var actualRow = DOTB.grid.getActualRow(allChildren[j]);
						if (actualRow > lastChildRow)
							lastChildRow = actualRow;
					}
					for (var j = parseInt(DOTB.grid.getActualRow(i)) + 1; j <= lastChildRow; j++) {
						document.getElementById("project_task_row_" + DOTB.grid.getMappedRow(j) ).style.display = "";
						DOTB.gantt.showGanttRow(DOTB.grid.getMappedRow(j));
					}
				}
			}
		},
		...
		markAsMilestone: function() {
			...
			while (selectedRows.length != 0) {
				...
				if (document.getElementById('id_' + selectedRows[0] + '_divlink').innerHTML.search(/\*/) == -1){
					...
					DOTB.gantt.markAsMilestone(selectedRows[0]);
				}
				DOTB.grid.unSelectRow(selectedRows[0]);
			}
		},
		...
		unMarkAsMilestone: function() {
			...
			while (selectedRows.length != 0) {
				...
				if (document.getElementById('id_' + selectedRows[0] + '_divlink').innerHTML.search(/\*/) != -1){
					document.getElementById('id_' + selectedRows[0] + '_divlink').innerHTML = DOTB.grid.getActualRow(selectedRows[0]);
					DOTB.gantt.unMarkAsMilestone(selectedRows[0]);
				}
				DOTB.grid.unSelectRow(selectedRows[0]);
			}
		},
		...
		collapseAll: function() {
			for (var i = 1; i <= totalRowsInGrid; i++) {
				...
				if (document.getElementById(elementId).innerHTML.search("Minus.gif") != -1) {
					...
					for (var j = parseInt(DOTB.grid.getActualRow(i)) + 1; j <= lastChildRow; j++) {
						document.getElementById("project_task_row_" + DOTB.grid.getMappedRow(j) ).style.display = "none";
						DOTB.gantt.hideGanttRow(DOTB.grid.getMappedRow(j));
					}
				}
			}
		},
		...
		expandCollapseRow: function(row)
		{
			...
			for (var i = 0; i < allChildren.length; i++) {
				var actualRow = DOTB.grid.getActualRow(allChildren[i]);
				if (actualRow > lastChildRow)
					lastChildRow = actualRow;
			}
			...
			if (document.getElementById(elementId).innerHTML.search("Minus.gif") != -1) {
				...
				for (var i = parseInt(DOTB.grid.getActualRow(row)) + 1; i <= lastChildRow; i++) {
					document.getElementById("project_task_row_" + DOTB.grid.getMappedRow(i) ).style.display = "none";
					DOTB.gantt.hideGanttRow(DOTB.grid.getMappedRow(i));
				}
			}
			else {
				document.getElementById(elementId).innerHTML =
				...
				for (var i = parseInt(DOTB.grid.getActualRow(row)) + 1; i <= lastChildRow; i++) {
					document.getElementById("project_task_row_" + DOTB.grid.getMappedRow(i) ).style.display = "";
					DOTB.gantt.showGanttRow(DOTB.grid.getMappedRow(i));
				}
			}
		...
		processStartDate: function(obj, row){
			if (DOTB.grid.validateDate(obj) && DOTB.grid.validateEndDateAfterStartDate(row)){
		  		DOTB.grid.clearStartTime(row);
				DOTB.grid.calculateDuration(obj.value, document.getElementById("date_finish_" + row).value, row);
				DOTB.grid.updateAllDependentsAfterDateChanges(row);
				DOTB.gantt.changeTask(row);
			}
		},
		processFinishDate: function(obj, row){
			if (DOTB.grid.validateDate(obj) && DOTB.grid.validateEndDateAfterStartDate(row)){
			  	DOTB.grid.clearFinishTime(row);
				DOTB.grid.calculateDuration(document.getElementById("date_start_" + row).value, obj.value, row);
				DOTB.grid.updateAllDependentsAfterDateChanges(row);
				DOTB.gantt.changeTask(row);
			}
		},
		...
		validatePercentComplete: function(widget) {
			...
			if (widget.value < 0 || widget.value > 100 || !isNumeric(widget.value)) {
				alert(DOTB.language.get('Project', 'ERR_PERCENT_COMPLETE'));
				widget.value = "";
		...
		validateDuration: function(widget) {
			...
			if (!isInteger(widget.value)) {
				alert(DOTB.language.get('Project', 'ERR_DURATION'));
				widget.value = "";
				setTimeout("document.getElementById('" + widgetId + "').focus();",1);
				setTimeout("document.getElementById('" + widgetId + "').select();",1);
				return false;
		...
		validateEndDateAfterStartDate: function(row) {
			...
			var startDateObj = DOTB.grid.getJSDate(startDate);
			var endDateObj = DOTB.grid.getJSDate(endDate);
			if (endDateObj.getTime() < startDateObj.getTime()) {
				alert(DOTB.language.get('Project', 'ERR_FINISH_DATE'));
		...
		validateDate: function(widget) {
			var dateObj = DOTB.grid.getJSDate(widget.value);
			var widgetId = widget.id;
			if (dateObj.getDay() == 0 || dateObj.getDay() == 6) {
				alert(DOTB.language.get('Project', 'ERR_DATE'));
				// Bug 25082
				// When the selected date is a non-working day, pick the next valid working day
				dateObj = DOTB.grid.getNextValidDate(dateObj).date;
			}
			...
			var dateStr = DOTB.grid.getDisplayDate(dateObj);
		...
		validatePredecessors: function(widget) {
			...
			for (var i = 0; i < predVals.length; i++) {
				...
				if (isNaN(predVals[i])) {
					alert(DOTB.language.get('Project', 'ERR_PREDECESSORS_INPUT'));
					widget.value = "";
					setTimeout("document.getElementById('" + widgetId + "').focus();",1);
					setTimeout("document.getElementById('" + widgetId + "').select();",1);
					return false;
					...
					alert(DOTB.language.get('Project', 'ERR_PREDECESSORS_OUT_OF_RANGE'));
					widget.value = "";
					setTimeout("document.getElementById('" + widgetId + "').focus();",1);
					setTimeout("document.getElementById('" + widgetId + "').select();",1);
					return false;
		...
		validatePredecessorIsNotAncestorOrChild: function(row, setFocus) {
			...
			var allAncestors = new Array();
			var ancestorLevels = 0;
			DOTB.grid.getAncestors(row, allAncestors, ancestorLevels);
			for (var i = 0; i < predecessorsArray.length; i++) {
				for (var j = 0; j < allAncestors.length; j++) {
					if (predecessorsArray[i] == DOTB.grid.getActualRow(allAncestors[j])) {
						document.getElementById('predecessors_' + row).value = "";
						if (setFocus) {
							alert(DOTB.language.get('Project', 'ERR_PREDECESSOR_IS_PARENT_OR_CHILD_FAIL'));
							...
							DOTB.grid.updatePredecessorsAndDependents(row);
						}
						return false;
					}
				}
			...
			for (var i = 0; i < predecessorsArray.length; i++) {
				for (var j = 0; j < allChildren.length; j++) {
					if (predecessorsArray[i] == DOTB.grid.getActualRow(allChildren[j])) {
						document.getElementById('predecessors_' + row).value = "";
						if (setFocus) {
							alert(DOTB.language.get('Project', 'ERR_PREDECESSOR_IS_PARENT_OR_CHILD_FAIL'));
							setTimeout("document.getElementById('predecessors_" + row + "').focus();",1);
		...
		isAncestor: function(row, allAncestors) {
			for (var i = 0; i < allAncestors.length; i++) {
				if (DOTB.grid.getActualRow(allAncestors[i]) == row)
					return true;
			}
			return false;
		},
		...
		isChildsPredecessor: function (row, allChildren, allAncestors) {
			...
			for (var i = 0; i < allChildren.length; i++) {
				var predecessorsVal = document.getElementById('predecessors_' + DOTB.grid.getMappedRow(allChildren[i])).value;
				var predecessorsArray = predecessorsVal.split(",");
				for (var j = 0; j < predecessorsArray.length; j++) {
					if (predecessorsArray[j] == DOTB.grid.getActualRow(row))
						return true;
					for (var k = 0; k < allAncestors.length; k++) {
						if (predecessorsArray[j] == DOTB.grid.getActualRow(allAncestors[k]))
							return true;
					}
				}
			}
			for (var i = 0; i < allAncestors.length; i++) {
				...
				var predecessorsArray = predecessorsVal.split(",");
				for (var j = 0; j < predecessorsArray.length; j++) {
					if (DOTB.grid.getActualRow(predecessorsArray[j]) == allAncestors[i])
						return true;
				}
			}
			return false;
		...
		validateDependencyCycles: function(row, allAncestors) {
			...
			for (var i = 0; i < predecessorsArray.length; i++) {
				...
				var allChildren = new Array();
				DOTB.grid.getChildren(predecessorsArray[i], allChildren);

				if (predecessorsArray[i] == DOTB.grid.getActualRow(dependencyCheckRow) || DOTB.grid.isAncestor(DOTB.grid.getMappedRow(predecessorsArray[i]), allAncestors) ||
					DOTB.grid.isChildsPredecessor(row, allChildren, allAncestors)) {
					alert(DOTB.language.get('Project', 'ERR_PREDECESSOR_CYCLE_FAIL'));
					document.getElementById('predecessors_' + row).value = "";
					setTimeout("document.getElementById('predecessors_" + row + "').focus();",1);
					setTimeout("document.getElementById('predecessors_" + row + "').select();",1);
					DOTB.grid.dependencyCheckFail = 1;
					return false;
				}
				DOTB.grid.validateDependencyCycles(DOTB.grid.getMappedRow(predecessorsArray[i]), allAncestors);
			}
			return true;
		},
		...
		setUpContextMenu: function() {
			if (document.getElementById('selected_view') && document.getElementById('selected_view').value <= 1) {
		        ...
		        var aMainMenuItems = new Array
		                (DOTB.language.get('Project', 'LBL_INSERT_BUTTON'),
		                 DOTB.language.get('Project', 'LBL_INDENT_BUTTON'),
		                 DOTB.language.get('Project', 'LBL_OUTDENT_BUTTON'),
		                 DOTB.language.get('Project', 'LBL_COPY_BUTTON'),
		                 DOTB.language.get('Project', 'LBL_CUT_BUTTON'),
		                 DOTB.language.get('Project', 'LBL_PASTE_BUTTON'),
		                 DOTB.language.get('Project', 'LBL_DELETE_BUTTON'),
		                 DOTB.language.get('Project', 'LBL_SAVE_BUTTON'),
		                 DOTB.language.get('Project', 'LBL_EXPAND_ALL_BUTTON'),
		                 DOTB.language.get('Project', 'LBL_COLLAPSE_ALL_BUTTON'),
		                 DOTB.language.get('Project', 'LBL_MARK_AS_MILESTONE_BUTTON'),
		                 DOTB.language.get('Project', 'LBL_UNMARK_AS_MILESTONE_BUTTON'),
		                 DOTB.language.get('Project', 'LBL_HIDE_OPTIONAL_COLUMNS_BUTTON'),
		                 DOTB.language.get('Project', 'LBL_SHOW_OPTIONAL_COLUMNS_BUTTON'),
		                 DOTB.language.get('Project', 'LBL_VIEW_TASK_DETAILS_BUTTON'));
		        ...
		        for(var i = 0; i < nMainMenuItems; i++) {
		        	if (aMainMenuItems[i]) {
						...
						oMenuItem.clickEvent.subscribe(
			            	DOTB.grid.onContextMenuItemClick,
		                    oMenuItem,
		                    true
		                );
				...
				oContextMenu.keyDownEvent.subscribe(
					DOTB.grid.onContextMenuKeyDown,
					oContextMenu,
					true
				);
        ...
        onContextMenuItemClick: function(p_sType, p_aArguments, p_oItem) {
            var oLI =
                DOTB.grid.GetListItemFromEventTarget(this.parent.contextEventTarget);
            switch(this.index) {
                case 0:     // Insert Row
                    DOTB.grid.insertRow();
                	break;
                case 1:     // Indent
                    DOTB.grid.indentSelectedRows();
                	break;
                case 2:     // Outdent
                    DOTB.grid.addToOutdent();
                	break;
                case 3:     // Copy
                    DOTB.grid.copyRow();
                	break;
                ...
                case 8:     // Expand All
                	...
                	break;
                case 9:     // Collapse All
                    DOTB.grid.collapseAll();
                	break;
				case 10: 	// Mark as Milestone
					DOTB.grid.markAsMilestone();
					break;
				case 11: 	// Un-Mark as Milestone
					DOTB.grid.unMarkAsMilestone();
					break;
				case 12: 	// Hide Optional Columns
					DOTB.grid.hideOptionalColumns();
					break;
				case 13: 	// Show Optional Columns
					DOTB.grid.showOptionalColumns();
					break;
				case 14: 	// View Task Details
					DOTB.grid.viewTaskDetails();
					break;
            }
        },
		...
		viewTaskDetails: function () {
			...
			if (record.length < 1) {
				...
				alert(DOTB.language.get('Project', 'ERR_TASK_VIEW_DETAILS'));
				return;
			}
		...
		closeTaskDetails: function(){
			document.getElementById("task_detail_area_div").style.display = "none";
			document.getElementById("task_detail_area_div").innerHTML = '<img src="index.php?entryPoint=getImage&themeName='+DOTB.themes.theme_name+'&imageName=img_loading.gif&v='+DOTB.VERSION_MARK+'" border="0" />';
		},
		...
		adjustInitialIndentedRows: function() {
			for (var i = 1; i <= totalRowsInGrid; i++) {
				if (indentedRows[i]) {
					var allAncestors = new Array();
					var ancestorLevels = 0;
					var ancestorObj = DOTB.grid.getAncestors(i, allAncestors, ancestorLevels);
					ancestorLevels = ancestorObj.ancestorLevels;
					allAncestors = ancestorObj.allAncestors;
					indentedRows[i].levelsIndented = ancestorLevels;
					DOTB.grid.indentRow(i, false);
				}
			}
		},
		...
		validateGridForSave: function() {
			for (var i = 1; i <= totalRowsInGrid; i++) {
				if (document.getElementById("description_" + i) &&
					...
					alert(DOTB.language.get('Project', 'ERR_TASK_NAME_FOR_ROW') + DOTB.grid.getActualRow(i) + DOTB.language.get('Project', 'ERR_IS_EMPTY'));
					return false;
				}
			}
			return true;
		...
		save: function() {
			if (DOTB.grid.validateGridForSave()) {
				...
				openConnection = YAHOO.util.Connect.asyncRequest('POST', 'index.php', {success: DOTB.grid.success, failure: DOTB.grid.failure});
			}
		},
		/***********************************************************************************************/
		/* END - SAVE FUNCTIONS		     														   	   */
		...
		exportToPDF: function() {
			...
			if(document.getElementById("pdfclass").value == "EZPDF"){
                ...
                document.getElementById('EditViewGrid').action.value = 'dotbpdf';
			}
		...
		changeView: function() {
			...
			if (parseInt(document.getElementById("gridViewSelect").value) == 5) {
				...
				if (document.getElementById('view_filter_button') == null) {
					...
					button.setAttribute('value', DOTB.language.get('Project', 'LBL_FILTER_VIEW'));
		...
		gridNotLoaded: function(){
			DOTB.grid.isLoaded = false;
		},
		gridLoaded: function() {
			DOTB.grid.isLoaded = true;
		},
...
if(typeof YAHOO != 'undefined') YAHOO.util.Event.addListener(window, 'load', DOTB.grid.setUpContextMenu);
