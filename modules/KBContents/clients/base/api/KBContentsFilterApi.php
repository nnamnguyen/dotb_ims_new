...
use \Dotbcrm\Dotbcrm\Elasticsearch\Query\KBFilterQuery;
...
class KBContentsFilterApi extends FilterApi
{
    ...
    public function filterListSetup(ServiceBase $api, array $args, $acl = 'list')
    {
        ...
        if (!empty($args['mostUseful'])) {
            ...
            $orderBy = new DotbQuery_Builder_Orderby($q, 'DESC');
            $orderBy->addRaw('mu');
    ...
    public function filterList(ServiceBase $api, array $args, $acl = 'list')
    {
        ...
        // 2. Filter without KBBody - just use standard Dotb filtering mechanism.
        if ($bodySearchFilter) {
