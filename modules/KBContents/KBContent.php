...
class KBContent extends DotbBean {
    ...
    public function save($check_notify = false)
    {
        ...
        if(empty($this->id) || !empty($this->new_with_id)) {
            if (empty($this->language)) {
                ...
                if ($lang === null) {
                    throw new DotbApiException('There is no available languages for localization');
                }
                $this->language = $lang;
            }
    ...
    {
        if(empty($this->id) || $this->new_with_id) {
            throw new DotbApiException('Bean must be saved before vote');
        }
    ...
    public function fill_in_relationship_fields()
    {
        ...
        $validUser = $this->usefulness->isValidDotbUser($user);
        $contact_id = null;
        $params = array();
