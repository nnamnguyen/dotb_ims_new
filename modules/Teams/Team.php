<?php
    ...
    class Team extends DotbBean
    {
        ...
        {
            dotb_cache_put("teamname_{$this->id}",$this->name);
        ...
        function has_users_in_teams() {
            require_once('custom/modules/Teams/_helper.php');

            $members = getTeamMembers($this->id, $this->parent_id, '');
            if(count($members) > 0)   return true;
            else  return false;
        ...
        {
            $teamname = dotb_cache_retrieve("teamname_$team_id");
            if ( !empty($teamname) )
                return $teamname;
            ...
            if ( !empty($focus->id) ) {
                $teamname = $focus->name;
                dotb_cache_put("teamname_$team_id",$teamname);
            }
        ...
        {
            $team_array = dotb_cache_retrieve('team_array-'. $add_blank.'ADDBLANK'.$user->id);
            if(!empty($team_array))
                return $team_array;
            ...
            dotb_cache_put('team_array-'.$add_blank.'ADDBLANK'.$current_user->id, $team_array);
