...
class Popup_Picker
{
    ...
    public function __construct()
	{
		...
		if(empty($popupMeta)){
		    $popupMeta = DotbAutoLoader::loadPopupMeta($currentModule, (!empty($_REQUEST['metadata']) && $_REQUEST['metadata'] != 'undefined')?$_REQUEST['metadata']:null);
		}
	...
	function process_page()
	{
		...
		global $app_list_strings, $dotb_version, $dotb_config;

		$output_html = "<script type=\"text/javascript\" src=\"" . getJSPath('include/javascript/dotb_3.js'). "\"></script>";
		$where = '';
...
EOQ;
        ...
        $button = "
        <script>
            DOTB.util.doWhen('document.getElementById(\"popup_query_form\") && window.document.forms.popup_query_form.request_data.value != \"\"', function(){
