 ...
 * This is defined in include/DotbObjects/team_security/vardefs.php as:
...
class TeamSetLink extends Link2 {
	...
	public function add($rel_keys, $additional_values=array(), $save = true) {
        ...
        foreach ($rel_keys as $key) {
            if ($key instanceof DotbBean) { /**@var $key DotbBean*/
                $teamIds[] = $key->id;
            } else { /**@var $key string*/
                $teamIds[] = $key;
            }
    ...
    public function save($checkForUpdate = true, $usedDefaultTeam = false)
    {
        if ($this->_saved == false) {
            ...
            if (empty($GLOBALS['dotb_config']['disable_team_sanity_check'])) {
                if (!empty($this->focus->team_id)) {
                    if (empty($this->_teamList)) {
                        ...
                        //from the UI. In that case the team_id will be set in DotbBean.php by the current user's
                        ...
                        // we set it in DotbBean.php and that this is not an update so we do not have a team_set_id on the bean.
                ...
                if (($this->focus->module_dir != 'Users') && empty($GLOBALS['dotb_config']['disable_team_access_check']) 
					...
					//Remove Private Team - In Team List - BY LAP NGUYEN
            ...
            }//fi empty($GLOBALS['dotb_config']['disable_team_sanity_check']))
