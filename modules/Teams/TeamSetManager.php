...
use Dotbcrm\Dotbcrm\Security\Teams\TeamSet;
...
class TeamSetManager {
			...
			if ($pos !== false) {
				...
				if(count($tokens) >= 3){
                    ...
                    while (($userPrefRow = $prefStmt->fetch())) {
						...
						$team_set_id = DotbArray::staticGet($prefs, implode('.', array_slice($tokens, 2)));
	...
	public static function save(){
        ...
        if (!isset($GLOBALS['dotb_config']['enable_team_module_save'])
            || !empty($GLOBALS['dotb_config']['enable_team_module_save'])) {
     ...
     * @throws DBALException
     */
    public static function removeTeamSetModule(DotbBean $focus, $teamSetId)
    {
        if (empty($teamSetId)) {
            return;
        }
