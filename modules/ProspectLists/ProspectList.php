...
class ProspectList extends DotbBean {
	...
	{
        $moduleName = $this->db->quote('Leads');
        $leads_query = "SELECT l.id AS id, '$moduleName' AS related_type, '' AS \"name\", l.first_name AS first_name, l.last_name AS last_name, l.title AS title, l.salutation AS salutation,
				...
				LEFT JOIN email_addr_bean_rel ear ON  ear.bean_id=l.id AND ear.bean_module = '$moduleName' AND ear.deleted=0
				LEFT JOIN email_addresses ea ON ear.email_address_id=ea.id
				WHERE plp.prospect_list_id = $record_id AND plp.related_type = '$moduleName' AND plp.deleted=0
				AND l.deleted=0
				AND (ear.deleted=0 OR ear.deleted IS NULL)";

        $moduleName = $this->db->quote('Users');
        $users_query = "SELECT u.id AS id, '$moduleName' AS related_type, '' AS \"name\", u.first_name AS first_name, u.last_name AS last_name,u.title AS title, '' AS salutation,
				...
				LEFT JOIN email_addr_bean_rel ear ON  ear.bean_id=u.id AND ear.bean_module = '$moduleName' AND ear.deleted=0
				LEFT JOIN email_addresses ea ON ear.email_address_id=ea.id
				WHERE plp.prospect_list_id = $record_id AND plp.related_type = '$moduleName' AND plp.deleted=0
				AND u.deleted=0
				AND (ear.deleted=0 OR ear.deleted IS NULL)";

        $moduleName = $this->db->quote('Contacts');
        $contacts_query = "SELECT c.id AS id, '$moduleName' AS related_type, '' AS \"name\", c.first_name AS first_name, c.last_name AS last_name,c.title AS title, c.salutation AS salutation,
...
FROM prospect_lists_prospects plp
				...
				LEFT JOIN email_addr_bean_rel ear ON ear.bean_id=c.id AND ear.bean_module = '$moduleName' AND ear.deleted=0
				LEFT JOIN email_addresses ea ON ear.email_address_id=ea.id
				WHERE plp.prospect_list_id = $record_id AND plp.related_type = '$moduleName' AND plp.deleted=0
				AND c.deleted=0
                AND (ear.deleted=0 OR ear.deleted IS NULL)";

        $moduleName = $this->db->quote('Prospects');
        $prospects_query = "SELECT p.id AS id, '$moduleName' AS related_type, '' AS \"name\", p.first_name AS first_name, p.last_name AS last_name,p.title AS title, p.salutation AS salutation,
				...
				LEFT JOIN email_addr_bean_rel ear ON  ear.bean_id=p.id AND ear.bean_module = '$moduleName' AND ear.deleted=0
				LEFT JOIN email_addresses ea ON ear.email_address_id=ea.id
				WHERE plp.prospect_list_id = $record_id AND plp.related_type = '$moduleName' AND plp.deleted=0
				AND p.deleted=0
				AND (ear.deleted=0 OR ear.deleted IS NULL)";

        $moduleName = $this->db->quote('Accounts');
        $accounts_query = "SELECT a.id AS id, '$moduleName' AS related_type, a.name AS \"name\", '' AS first_name, '' AS last_name,'' AS title, '' AS salutation,
				...
				LEFT JOIN email_addr_bean_rel ear ON  ear.bean_id=a.id AND ear.bean_module = '$moduleName' AND ear.deleted=0
				LEFT JOIN email_addresses ea ON ear.email_address_id=ea.id
				WHERE plp.prospect_list_id = $record_id AND plp.related_type = '$moduleName' AND plp.deleted=0
				AND a.deleted=0
				AND (ear.deleted=0 OR ear.deleted IS NULL)";
	...
	function get_list_view_data(){
        ...
        if(count($teams) > 1) {
      	   $temp_array['TEAM_NAME'] .= "<span id='div_{$this->id}_teams'>
					  ...
					  ".DotbThemeRegistry::current()->getImage(MoreDetail, "style='padding: 0px 0px 0px 0px' border='0'", 8, 7, ".gif", $mod_strings['LBL_MORE_DETAIL'])."
						</a>
						</span>";
        }
		return $temp_array;
