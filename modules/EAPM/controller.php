...
class EAPMController extends DotbController
{
    /**
     * API implementation
    ...
    {
        DotbApplication::appendErrorMessage($error);
    ...
    public function pre_save()
    {
        ...
        if(empty($this->bean->id)){
            ...
            if($eapmBean){
                DotbApplication::appendErrorMessage(translate('LBL_APPLICATION_FOUND_NOTICE', $this->bean->module_dir));
    ...
    protected function action_oauth()
    {
        ...
        if(empty($_REQUEST['oauth_error'])) {
            ...
            if ($source && $source->hasTestingEnabled()) {
                try {
                    if (!$source->test()) {
                        dotb_die(translate('LBL_CONNECT_TEST_FAIL', $this->bean->module_dir));
                    }
                } catch (Exception $e) {
                    dotb_die(translate('LBL_CONNECT_TEST_FAIL', $this->bean->module_dir));
                }
    ...
    protected function pre_QuickSave(){
        if(!empty($_REQUEST['application'])){
            ...
            dotb_die("Please pass an application name.");
        }
    }
