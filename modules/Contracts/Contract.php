...
if (!defined('DOTBCRM_SECONDS_PER_DAY')) {
    define('DOTBCRM_SECONDS_PER_DAY', 60 * 60 * 24);
}
...
class Contract extends DotbBean
{
    public $id;
    public $name;
    ...
    protected function _set_contract_term($isFromDb)
    {
        ...
        if (!empty($start_date_timestamp) && !empty($end_date_timestamp)) {
            $this->contract_term = floor(($end_date_timestamp - $start_date_timestamp) / constant('DOTBCRM_SECONDS_PER_DAY'));
        }
    }
    ...
    protected function _set_time_to_expiry($isFromDb)
    {
        ...
        if (!empty($end_date_timestamp)) {
            $this->time_to_expiry = ($end_date_timestamp - $now) / constant('DOTBCRM_SECONDS_PER_DAY');
        }
    }
