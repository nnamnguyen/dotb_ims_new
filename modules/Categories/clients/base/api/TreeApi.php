...
class TreeApi extends FilterApi
{
    ...
    protected function retrieveBean($module, $id = null)
    {
        ...
        if (null === $bean) {
            throw new DotbApiExceptionNotFound('Could not find record in module: ' . $module);
        }
     ...
     * @return array DotbBean An array of DotbBeans that can bound in tree.
     * @throws DotbApiExceptionInvalidParameter
     * @throws DotbApiExceptionNotFound
     * @throws DotbApiException Subject and target beans are not suitable for the operation.
     */
    protected function loadBoundBeans($module, $subject, $target)
    {
        $bean = $this->retrieveBean($module, $subject);
        if (null === $bean) {
            throw new DotbApiExceptionNotFound('Could not find record: ' . $subject . ' in module: ' . $module);
        }

        $target = $this->retrieveBean($module, $target);
        if (null === $target) {
            throw new DotbApiExceptionNotFound('Could not find record: ' . $target . ' in module: ' . $module);
        }

        if ($bean->id === $target->id) {
            throw new DotbApiException('The target node should not be self.');
        }

        if ($target->isDescendantOf($bean)) {
            throw new DotbApiException('The target node should not be descendant.');
        }
     ...
     * @return DotbBean Created bean.
     * @throws DotbApiExceptionInvalidParameter Provided module doesn't implement `NestedBeanInterface`.
     * @throws DotbApiExceptionEditConflict Server and client timestamps are different.
     */
     ...
     * @param ServiceBase $api Api object.
     * @param array $args The arguments array passed in from the API.
     * @return array An array version of the DotbBean.
     */
    ...
    /**
     * This method formats tree data to dotb api collection object.
     * @param ServiceBase $api Api object.
     * @param array $args The arguments array passed in from the API.
    ...
    public function filterSubTree(ServiceBase $api, array $args)
    {
        ...
        if ($linkModuleName != $record->module_name) {
            throw new DotbApiExceptionNotFound('Could not find self referencing in relationship named: ' . $linkName);
        }
    ...
    public function filterTree(ServiceBase $api, array $args)
    {
        ...
        if (!$seed->load_relationship($linkName)) {
            // The relationship did not load.
            throw new DotbApiExceptionNotFound('Could not find a relationship named: ' . $linkName);
        }
