...
class ViewConfig extends DotbView
{
    /**
	...
	public function display()
	{
        ...
        $this->ss->assign(
            ...
            (empty($dotb_config['disable_user_email_config']) ? "checked" : "")
        );
        $this->ss->assign(
            'new_email_addresses_opted_out',
            empty($dotb_config['new_email_addresses_opted_out']) ? '' : 'checked'
        );
        ...
        if(isset($dotb_config['allow_sendmail_outbound']) && $dotb_config['allow_sendmail_outbound'])
        {
            $showSendMail = TRUE;
        ...
        //tracking_entities_location http://www.dotbcrm.com/track/

        //////////////////////////////////////////////////////////////////////////////
        ////	EMAIL SECURITY
        if(!isset($dotb_config['email_xss']) || empty($dotb_config['email_xss'])) {
            $dotb_config['email_xss'] = getDefaultXssTags();
        }

        foreach(unserialize(base64_decode($dotb_config['email_xss'])) as $k => $v) {
