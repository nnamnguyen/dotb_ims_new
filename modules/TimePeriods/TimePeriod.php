...
class TimePeriod extends DotbBean
{
...
AND deleted = 0';
     ...
     *
     * @param $targetStartDate DotbDateTime instance of start date based on current settings
     *
    ...
    public function getGenericStartEndByDuration($duration, $start_date = null)
    {
        ...
        $start = false;
        if (!is_null($start_date)) {
            $start = DotbDateTime::createFromFormat('Y-m-d', $start_date);
            $end = DotbDateTime::createFromFormat('Y-m-d', $start_date);
        }
        if ($start === false) {
            $start = new DotbDateTime();
            $end = new DotbDateTime();
        }
