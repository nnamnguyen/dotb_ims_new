...
class MetaDataConverter
{
    ...
    /**
     * Takes in a 6.6+ version of mobile|portal|lumia list view metadata and
    ...
    /**
     * Takes a Lumia Subpanel view def and returns a BWC compatibile Subpanel view def
     *
    ...
    public function toLegacySubpanelsViewDefs(array $defs, $moduleName)
    {
        ...
        // for BWC, we need to have some top buttons.  Lumia doesn't have buttons in the def
    ...
    /**
     * Convert legacy subpanels view defs to lumia subpanel view defs
     * @param array $defs
     * @param string module
     * @return array
     */
        ...
        foreach ($defs['list_fields'] as $fieldName => $details) {
            ...
            if(!empty($details['widget_class'])) {
                if($details['widget_class'] == 'SubPanelDetailViewLink') {
                    ...
                    // See BR-1436: we will drop unknown widgets for now since Lumia
    ...
    /**
     * This converts the lumia subpanel name to the legacy subpanel name
     * @param array $def - Lumia Subpanel Definition
     ...
     */
    public function toLegacySubpanelLayoutDefs(array $layoutDefs, DotbBean $bean)
    {
        $return = array();
    ...
    /**
     * Takes in a 6.6+ version of mobile|portal|lumia edit|detail view metadata and
    ...
    /**
     * Convert a legacy subpanel name to the new lumia name
     * Examples:
    ...
    /**
     * Convert a legacy subpanel path to the new lumia path
     * @param string $filename the path to a legacy subpanel
     * @param string client the client
     * @return string the new lumia subpanel path
     */
     ...
     *
     * @param DotbBean $bean Parent bean
     * @param string $linkName Subpanel link name
     * @return string|null
     */
    public function getLegacySubpanelFileName(DotbBean $bean, $linkName)
    {
    ...
    protected function buildMenuRoute(array $menuOptions, $link)
    {
        ...
        $url = parse_url($link);
        $currSiteUrl = parse_url($GLOBALS['dotb_config']['site_url']);
    ...
    /**
     * Converts the old profileactions metadata to the new lumia metadata
     *
     * @param array $global_control_links globalcontrollink format
     * @return array new lumia profileactions view metadata
     */
    public function fromLegacyProfileActions(array $global_control_links)
    {
        ...
        $dataItems = array();
        if(!empty($menu)){
            // Convert custom items to lumia format
            foreach ($menu as $option) {
    ...
    /**
     * Convert a single item array into lumia meta format
     *
     * @param array $option
     * @return array lumia profileactions item meta
     */
    public function convertCustomMenu($option){
        ...
        // Lumia list view (eg. #Accounts, #Contacts, Fragment is 'Accounts', 'Contacts')
     ...
     * @return array
     */
    public function convertLegacyViewDefsToLumia($defs, $moduleName, $fieldDefs, $viewType, $viewClient, $defaultLumiaFilter)
    {
        ...
        foreach ($old_fields as $name => $data) {
            ...
            if (!empty($searchFields[$name]['db_field'])) {
                ...
                } else if (!empty($defaultLumiaFilter[$name]['type'])) {
                    $fields[$name]['type'] = $defaultLumiaFilter[$name]['type'];
                }
        ...
        $lumiaViewDefs['fields'] = $fields;
        return $lumiaViewDefs;
    }
    /**
    ...
    {
        $filename = DotbAutoLoader::existingCustomOne("modules/{$moduleName}/metadata/SearchFields.php");
        if (!$filename) {
            // try template now
            $sm = StudioModuleFactory::getStudioModule($moduleName);
            $moduleType = $sm->getType();
            $filename = 'include/DotbObjects/templates/' . $moduleType . '/metadata/SearchFields.php';
            if (!file_exists($filename)) {
                // OK, I give up, no way I can find it, let's use basic ones
                $filename = 'include/DotbObjects/templates/basic/metadata/SearchFields.php';
            }
        }
        $searchFields = array();
        include $filename;
