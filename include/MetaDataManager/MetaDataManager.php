...
use Dotbcrm\Dotbcrm\MetaData\RefreshQueue;
...
DotbAutoLoader::requireWithCustom('include/MetaDataManager/MetaDataHacks.php');
DotbAutoLoader::requireWithCustom('include/MetaDataManager/MetaDataCache.php');

/**
 * This class is for access to metadata for all dotbcrm modules in a read only
...
class MetaDataManager implements LoggerAwareInterface
{
    ...
    protected $sectionMap = array(
        ...
        self::MM_FIELDS => 'getDotbFields',
        self::MM_LABELS => 'getStringUrls',
        self::MM_ORDEREDLABELS => 'getOrderedStringUrls',
        self::MM_VIEWS => 'getDotbViews',
        self::MM_LAYOUTS => 'getDotbLayouts',
        self::MM_DATA => 'getDotbData',
        ...
        self::MM_FILTERS => 'getDotbFilters',
    ...
    public function __construct($platforms = null, $public = false)
    {
        ...
        if ($platforms instanceof DotbBean) {
    ...
    public static function getManager($platform = null, $public = false, $fresh = false)
    {
        ...
        if ($fresh || empty(self::$managers[$key])) {
            ...
            foreach ($platform as $type) {
                ...
                if (DotbAutoLoader::requireWithCustom($file)) {
                    $class = DotbAutoLoader::customClass($mmClass);
                    $found = true;
                    break;
                }
            }
            ...
            if (!$found) {
                DotbAutoLoader::requireWithCustom($path . $class . '.php');
     ...
     *
     * @param $moduleName The name of the dotb module to collect info about.
     ...
     *
     * @param string $moduleName The name of the dotb module to collect info about.
     *
     * @return Array A hash of all of the view data.
     */
    ...
    {
        $filterModules = false;
        if (DotbConfig::getInstance()->get('roleBasedViews') && !($context instanceof MetaDataContextDefault)) {
            $filterModules = true;
        }
    ...
    public function getModuleData($moduleName, MetaDataContextInterface $context = null)
    {
        ...
        $data['ftsEnabled'] = DotbSearchEngineMetadataHelper::isModuleFtsEnabled($moduleName);
    ...
    {
        $relFactory = DotbRelationshipFactory::getInstance();
     ...
     * @param object|bool $bean The DotbBean for getting specific ACL's for a module
    ...
    public function getAclForModule($module, $userObject, $bean = false, $showYes = false)
    {
        ...
        if (!DotbACL::moduleSupportsACL($module)) {
            ...
            $moduleAcls = DotbACL::getUserAccess($module, array(), $context);

            // Bug56391 - Use the DotbACL class to determine access to different actions within the module
            foreach (DotbACL::$all_access as $action => $bool) {
            ...
            if ($outputAcl['access'] == 'yes') {
                ...
                if ($outputAcl['edit'] === 'yes') {
                    $createAcl = DotbACL::getUserAccess($module, ['create' => true], $context);
    ...
    /**
     * Fields accessor, gets dotb fields
     *
     * @return array array of dotbfields with a hash
     */
    public function getDotbFields()
    {
        return $this->getSystemClientData('field');
    }

    /**
     * Filters accessor, gets dotb filter operators
     *
     * @return array array of filters with a hash
     */
    public function getDotbFilters()
    {
     ...
     * @return array
     */
    public function getDotbLayouts()
    {
    ...
    {
        $platforms = array();
        $platformsDefs = DotbAutoLoader::existingCustom('clients/platforms.php');
        if ($platformExtension = DotbAutoLoader::loadExtension('platforms')) {
    ...
    protected static function getPlatformsWithCachesInFilesystem()
    {
        ...
        $caches = glob(dotb_cached('api/metadata/') . '*.*');
    ...
    public static function clearAPICache($deleteModuleClientCache = true, $clearNow = false)
    {
        ...
        if ($clearNow || (defined('DOTB_PHPUNIT_RUNNER') && DOTB_PHPUNIT_RUNNER === true)) {
    ...
    public static function clearAPICacheOnShutdown($deleteModuleClientCache = true, $workingDirectory = "")
    {
        if (!self::getCacheHasBeenCleared()) {
            ...
            $metadataFiles = glob(dotb_cached('api/metadata/') . '*');
            ...
            // clear the platform cache from dotb_cache to avoid out of date data as well as platform component files
            $platforms = self::getPlatformList();
            foreach ($platforms as $platform) {
                $jsFiles = glob(dotb_cached("javascript/{$platform}/") . '*');
                if (is_array($jsFiles)) {
    ...
    public function getServerInfo()
    {
        ...
        $data['flavor'] = $this->getInstanceVersionValue('dotb_flavor');
        $data['version'] = $this->getInstanceVersionValue('dotb_version');
        $data['build'] = $this->getInstanceVersionValue('dotb_build');
        $data['marketing_version'] = $this->getInstanceVersionValue('dotb_mar_version');
        /*
        $data['flavor'] = $GLOBALS['dotb_flavor'];
        $data['version'] = $GLOBALS['dotb_version'];
        $data['build'] = $GLOBALS['dotb_build'];
        $data['marketing_version'] = $GLOBALS['dotb_mar_version'];
        */
        // Product Name for Professional edition.
        $data['product_name'] = "DotB Education";
        // Product Name for Enterprise edition.
        $data['product_name'] = "DotB Moon";
        ...
        $fts_enabled = DotbSearchEngineFactory::getFTSEngineNameFromConfig();
        if (!empty($fts_enabled) && $fts_enabled != 'DotbSearchEngine') {
            $data['fts'] = array(
                'enabled' => true,
                'type' => $fts_enabled,
            );
    ...
    protected function getConfigs()
    {
        ...
        $properties = $this->parseConfigProperties($dotbConfig, $properties);
        ...
        if (isset($dotbConfig['passwordsetting']['forgotpasswordON'])) {
            if ($dotbConfig['passwordsetting']['forgotpasswordON'] === '1' || $dotbConfig['passwordsetting']['forgotpasswordON'] === true) {
        ...
        $auth = AuthenticationController::getInstance($dotbConfig['authenticationClass'] ?? null);

        if ($auth->isExternal()) {
            $configs['externalLogin'] = true;
            $configs['externalLoginSameWindow'] = DotbConfig::getInstance()->get('SAML_SAME_WINDOW');
        }

        if (isset($dotbConfig['analytics'])) {
            $configs['analytics'] = $dotbConfig['analytics'];
        } else {
        ...
        // System name setting for lumia modules
        if (!empty($administration->settings['system_name'])) {
            $configs['systemName'] = $administration->settings['system_name'];
        }

        if (!empty($dotbConfig['unique_key'])) {
            $configs['uniqueKey'] = $dotbConfig['unique_key'];
        }
        ...
        if (isset($dotbConfig['dotb_min_int']) && is_numeric($dotbConfig['dotb_min_int'])) {
            $configs['dotbMinInt'] = $dotbConfig['dotb_min_int'];
        }
        if (isset($dotbConfig['dotb_max_int']) && is_numeric($dotbConfig['dotb_max_int'])) {
            $configs['dotbMaxInt'] = $dotbConfig['dotb_max_int'];
        }
        ...
        //Add Custom Hybrid Module List - By Lap Nguyen
        if (!empty($dotbConfig['hybrid_modules']) && is_array($dotbConfig['hybrid_modules']))
            $configs['hybrid_modules'] = $dotbConfig['hybrid_modules'];
        else $configs['hybrid_modules'] = '';

        /*Add check QuickEdit in ListView - By HP*/
        $configs['quickedit_listview'] = $dotbConfig['quickedit_listview'];

        /*
         * add by TKT
         */
        $configs['site_name'] = $dotbConfig['site_name'];
        $configs['site_url'] = empty($dotbConfig['site_url']) ? 'dev' : $dotbConfig['site_url'];
        $configs['ticket_id'] = empty($dotbConfig['ticket_id']) ? '' : $dotbConfig['ticket_id'];
        if (empty($dotbConfig['modules_use_kanban'])) $configs['modules_use_kanban'] = array();
        else $configs['modules_use_kanban'] = $dotbConfig['modules_use_kanban'];
        $configs['module_icon'] = json_decode(file_get_contents('custom/include/module_icon.json'), true);

        //license
        require_once 'custom/include/KTEncrypt.php';
        $kt = new KTEncrypt();
        $licData = trim(file_get_contents('license.key'), '"');
        $dotbConfig['license'] = unserialize($kt->decode($licData, 'r>((5\tg&z/2y5y#\;'));
        if (empty($dotbConfig['license'])) $configs['dotbheart'] = array();
        else {
            $configs['dotbheart'] = $dotbConfig['license'];
            $configs['dotbheart']['heartbeat'] = $dotbConfig['license']['date_expired'];
            $configs['dotbheart']['date_expired'] = '';
        }
    ...
    {
        return DotbThemeRegistry::current()->getImageURL('company_logo.png', true, true);
    }
    ...
    protected function buildJavascriptComponentFile(&$data, $onlyReturnModuleComponents = false)
    {
        ...
        if (shouldResourcesBeMinified() && DotbMin::isMinifyFast()) {
            $js = DotbMin::minify($js);
        }
        $path = "cache/javascript/dotbcrm0_$hash.js";
        if (!file_exists($path)) {
            mkdir_recursive(dirname($path));
            dotb_file_put_contents_atomic($path, $js);
        }
    ...
    {
        $user = $this->getCurrentUser();
        if (!empty($user->id) && !empty($GLOBALS['dotb_config']['roleBasedViews']) && !$this->public) {
            $list = DotbACL::filterModuleList($list);
        }
    ...
    public function getSystemCurrencies()
    {
        ...
        if (!empty($lcurrency->list)) {
            foreach ($lcurrency->list as $current) {
                ...
                $currency['conversion_rate'] = DotbMath::init($current->conversion_rate)->result();
                $currency['name'] = $current->name;
    ...
    public function getStringUrls($ordered = false)
    {
        ...
        dotb_mkdir(dotb_cached('api/metadata'), null, true);
        ...
        $urlList['default'] = $GLOBALS['dotb_config']['default_language'];
    ...
    {
        return array(
            'version' => $GLOBALS['dotb_config']['js_lang_version'],
        );
    }
    ...
    protected function getLangUrl($language, $ordered = false)
    {
        ...
        $platform = $this->platforms[0];
        return dotb_cached("api/metadata/lang_{$language}_{$platform}{$public_key}{$order_key}.json");
    }
    ...
    protected function buildLanguageFile($language, $modules, $ordered = false)
    {
        ...
        dotb_file_put_contents_atomic($filePath, $data);
    ...
    protected function clearLanguagesCache()
    {
        ...
        $hashes = array();
        $path = dotb_cached("api/metadata/hashes.php");
        @include($path);
        ...
        $files = glob(dotb_cached("api/metadata/*.json"));
     ...
     * @return array
     *
     * @access protected Should be used only by DotbFieldBase and subclasses
     */
    ...
    protected function getMetaDataHacks()
    {
        if (!$this->metaDataHacks) {
            $className = DotbAutoLoader::customClass('MetaDataHacks');
     ...
     *
     * @param DotbBean $seed Seed bean
     * @param array $data Data set
     *
     * @return DotbBean[]
     * @todo Move this to ACLRoleSet when it's merged
     */
    protected static function createCollectionFromDataSet(DotbBean $seed, array $data)
    {
        $result = array();
