<?php
/**
 * Default DotBCRM Logger
 * @api
 */
class DotbLogger implements LoggerTemplate
{
    /**
     * properties for the DotbLogger
     */
	protected $logfile = 'dotbcrm';
	protected $ext = '.log';
    ...
    /**
     * Used by the diagnostic tools to get DotbLogger log file information
     */
    ...
    public function __construct()
    {
        ...
        LoggerManager::setLogger('default','DotbLogger');
	}

	/**
	 * Handles the DotbLogger initialization
	 */
    ...
    /**
	 * Checks to see if the DotbLogger file can be created and written to
	 */
    ...
    /**
	 * Creates the DotbLogger file if it doesn't exist
	 */
        ...
        if ($force || (!empty($rollAt) && filesize($this->full_log_file) >= $rollAt)) {
			...
			for($i = $this->maxLogs - 2; $i > 0; $i --) {
                if (file_exists ( $this->log_dir . $this->logfile . $this->date_suffix . '_'. $i . $this->ext )) {
					...
					dotb_rename($old_name, $new_name);

					//rename ( $this->logfile . $i . $this->ext, $this->logfile . $to . $this->ext );
				}
			}
			//now lets move the current .log file
            dotb_rename ($this->full_log_file, $this->log_dir . $this->logfile . $this->date_suffix . '_1' . $this->ext);
	 ...
	 * Destructor
	 *
	 * Closes the DotbLogger file handle
     */
