...
class DotbForecasting_Chart_Manager extends DotbForecasting_Chart_AbstractChart
{
    /**
     * Constructor
     *
    ...
    public function generateChartJson()
    {
        ...
        foreach ($this->dataArray as $data) {
            $value = array(
                ...
                'likely' => DotbCurrency::convertWithRate($data['likely_case'], $data['base_rate']),
                'likely_adjusted' => DotbCurrency::convertWithRate(
            ...
            if ($config['show_worksheet_best']) {
                $value['best'] = DotbCurrency::convertWithRate($data['best_case'], $data['base_rate']);
                $value['best_adjusted'] = DotbCurrency::convertWithRate(
                    $data['best_case_adjusted'],
                    $data['base_rate']
                );
            }
            if ($config['show_worksheet_worst']) {
                $value['worst'] = DotbCurrency::convertWithRate($data['worst_case'], $data['base_rate']);
                $value['worst_adjusted'] = DotbCurrency::convertWithRate(
    ...
    protected function getQuotaTotalFromData()
    {
        ...
        foreach ($this->dataArray as $data) {
            $quota += DotbCurrency::convertAmountToBase($data['quota'], $data['currency_id']);
        }
    ...
    protected function sortChartColumns($a, $b)
    {
        ...
        foreach ($this->dataset as $dataset) {
            $sumA += DotbCurrency::convertAmountToBase($a[$dataset . '_case_adjusted'], $a['currency_id']);
            $sumB += DotbCurrency::convertAmountToBase($b[$dataset . '_case_adjusted'], $b['currency_id']);
        }
