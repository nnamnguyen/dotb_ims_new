...
class DotbForecasting_Committed extends DotbForecasting_AbstractForecast implements DotbForecasting_ForecastSaveInterface
{
    /**
     * No longer used but the class parent implements DotbForecasting_ForecastProcessInterface
     *
     * @return array|string
     */
    ...
    public function save()
    {
        ...
        if ($commit_type == "manager") {
            ...
            // set likely
            $worksheet_totals['likely_case'] = DotbMath::init($worksheet_totals['amount'], 6)
                    ->add($worksheet_totals['includedClosedAmount'])->result();
            $worksheet_totals['best_case'] = DotbMath::init($worksheet_totals['best_case'], 6)
                    ->add($worksheet_totals['includedClosedBest'])->result();
            $worksheet_totals['worst_case'] = DotbMath::init($worksheet_totals['worst_case'], 6)
