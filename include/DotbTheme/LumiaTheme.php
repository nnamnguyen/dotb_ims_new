...
class LumiaTheme
{
     ...
     */
    public $lessFilesToCompile = array('dotbcrm');
    ...
    public function getCSSURL()
    {
        ...
        if ($this->myTheme !== 'default' && !$this->isDefined()) {
            $clientDefaultTheme = new LumiaTheme($this->myClient, 'default');
    ...
    public function compileTheme($min = true)
    {
        ...
        //Cache the hash in dotb_cache so we don't have to hit the filesystem for etag comparisons
        dotb_cache_put($this->paths['hashKey'], $files);
        return $files;
    }
    ...
    public function previewCss($min = true)
    {
        try {
            ...
            throw new DotbApiExceptionError('lessc fatal error:<br />' . $e->getMessage());
        }
    }
    ...
    public function compileFile($lessFile, $min = true, $writeFile = true)
    {
        try {
            ...
            dotb_mkdir($this->paths['cache'], null, true);
            dotb_file_put_contents($this->getCssFileLocation($lessFile, $hash), $css);
            return $hash;
        } catch (Exception $e) {
            throw new DotbApiExceptionError('lessc fatal error:<br />' . $e->getMessage());
        }
    ...
    public function saveThemeVariables($reset = false)
    {
        ...
        $baseDefaultTheme = new LumiaTheme('base', 'default');
        ...
        if ($reset) {
            ...
            dotb_mkdir($this->paths['custom'], null, true);
            $write = "<?php\n" .
                '// created: ' . date('Y-m-d H:i:s') . "\n" .
                '$lessdefs = ' .
                var_export_helper($lessdefs) . ';';
            dotb_file_put_contents($this->paths['custom'] . 'variables.php', $write);
        }
    }
    ...
    /**
     * Writes a temporary less file dotb importing fixed variables and customer less file.
    ...
    private function makePaths($client, $themeName)
    {
        return array(
            ...
            'cache' => dotb_cached('themes/clients/' . $client . '/' . $themeName . '/'),
    ...
    private function retrieveCssFilesInCache()
    {
        ...
        $hashArray = dotb_cache_retrieve($hashKey);
    ...
    {
        $file = $this->paths['clients'] . $this->myClient . '/dotb.less';
        $baseFile = $this->paths['clients'] . 'base' . '/dotb.less';
    ...
    {
        $urls = array();
        dotb_cache_put($this->paths['hashKey'], $filesArray);
        if (!empty($filesArray)) {
            foreach ($this->lessFilesToCompile as $lessFile) {
