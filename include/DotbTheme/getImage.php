...
if ( isset($_REQUEST['themeName']) && DotbThemeRegistry::current()->name != $_REQUEST['themeName']) {
    DotbThemeRegistry::set($_REQUEST['themeName']);
} elseif ( isset($_SESSION['authenticated_user_theme']) ) {
    DotbThemeRegistry::set($_SESSION['authenticated_user_theme']);
}
...
if (!empty($spriteNamespace)) {
	...
	$filename = DotbThemeRegistry::current()->getImageURL($imageName);
	if ( empty($filename) ) {
		header($_SERVER["SERVER_PROTOCOL"].' 404 Not Found');
		die;
	}
...
$extensions = DotbThemeRegistry::current()->imageExtensions;
