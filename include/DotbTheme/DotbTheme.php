...
class DotbTheme
{
    ...
    /**
     * Maximum dotb version this theme is for; defaults to 5.5.1 as all the themes without this
        ...
        )
    {
        ...
        if ( !inDeveloperMode() ) {
            if ( dotb_is_file($cachedfile = dotb_cached($this->getFilePath().'/pathCache.php'))) {
            ...
            $cachedfile = dotb_cached($this->getFilePath().'/spriteCache.php');
			if(!empty($GLOBALS['dotb_config']['use_sprites']) && dotb_is_file($cachedfile)) {
				$this->_spriteCache = unserialize(dotb_file_get_contents($cachedfile));
			}
        }
    ...
    public function __destruct()
    {
        ...
        $cachedir = dotb_cached($this->getFilePath());
        dotb_mkdir($cachedir, 0775, true);
        ...
        )
    {
		...
		//Add Custom CSS By Lap Nguyen
        $html .= '<link rel="stylesheet" type="text/css" href="themes/RacerX/css/custom.css" />';

		// sprites
		if(!empty($GLOBALS['dotb_config']['use_sprites']) && $GLOBALS['dotb_config']['use_sprites']) {

			// system wide sprites
			if(file_exists(dotb_cached("sprites/default/sprites.css"))) {
				$html .= '<link rel="stylesheet" type="text/css" href="'.getJSPath('cache/sprites/default/sprites.css').'" />';
			}

			// theme specific sprites
			if(file_exists(dotb_cached("sprites/{$this->dirName}/sprites.css"))) {
				$html .= '<link rel="stylesheet" type="text/css" href="'.getJSPath('cache/sprites/'.$this->dirName.'/sprites.css').'" />';
			}

			// parent sprites
			if($this->parentTheme && $parent = DotbThemeRegistry::get($this->parentTheme)) {
				if(file_exists(dotb_cached("sprites/{$parent->dirName}/sprites.css"))) {
					$html .= '<link rel="stylesheet" type="text/css" href="'.getJSPath('cache/sprites/'.$parent->dirName.'/sprites.css').'" />';
				}
			}

			// repeatable sprites
			if(file_exists(dotb_cached("sprites/Repeatable/sprites.css"))) {
...
EOHTML;
        ...
        )
    {
        ...
        $templatePath = DotbAutoLoader::existingCustomOne($this->getTemplatePath().'/'.$templateName);

        if(!$templatePath && isset($this->parentTheme) && DotbThemeRegistry::get($this->parentTheme) instanceOf DotbTheme) {
            $templatePath = DotbThemeRegistry::get($this->parentTheme)->getTemplate($templateName);
        }

        if(!$templatePath) {
            $templatePath = DotbAutoLoader::existingCustomOne($this->getDefaultTemplatePath().'/'.$templateName);
        }
    ...
    public function getImageURL($imageName, $addJSPath = true, $timeModified = false)
    {
        ...
        elseif (isset($this->parentTheme)
                && DotbThemeRegistry::get($this->parentTheme) instanceOf DotbTheme
                && ($filename = DotbThemeRegistry::get($this->parentTheme)->getImageURL($imageName,false)) != '')
            $imagePath = $filename;
    ...
    {
        if ( isset($this->_cssCache[$cssFileName]) && dotb_is_file(dotb_cached($this->_cssCache[$cssFileName])) ) {
            if ( $returnURL )
                return getJSPath("cache/".$this->_cssCache[$cssFileName]);
            else
                return dotb_cached($this->_cssCache[$cssFileName]);
        }
        ...
        if (isset($this->parentTheme)
                && DotbThemeRegistry::get($this->parentTheme) instanceOf DotbTheme
                && ($filename = DotbThemeRegistry::get($this->parentTheme)->getCSSURL($cssFileName,false)) != '')
            $cssFileContents .= file_get_contents($filename);
        else {
            foreach(DotbAutoLoader::existingCustom($defaultFileName) as $cssFile) {
                $cssFileContents .= file_get_contents($cssFile);
            }
        }
        foreach(DotbAutoLoader::existingCustom($fullFileName) as $cssFile) {
            $cssFileContents .= file_get_contents($cssFile);
        }
        ...
        return dotb_cached($fullFileName);
    }
    ...
    {
        if ( isset($this->_jsCache[$jsFileName]) && dotb_is_file(dotb_cached($this->_jsCache[$jsFileName])) ) {
            if ( $returnURL )
                return getJSPath("cache/".$this->_jsCache[$jsFileName]);
            else
                return dotb_cached($this->_jsCache[$jsFileName]);
        }
        ...
        if (isset($this->parentTheme)
                && DotbThemeRegistry::get($this->parentTheme) instanceOf DotbTheme
                && ($filename = DotbThemeRegistry::get($this->parentTheme)->getJSURL($jsFileName,false)) != ''    && !in_array($jsFileName,$this->ignoreParentFiles)) {
           $jsFileContents .= file_get_contents($filename);
       } else {
           foreach(DotbAutoLoader::existingCustom($defaultFileName) as $jsFile) {
               $jsFileContents .= file_get_contents($jsFile);
           }
        }
        foreach(DotbAutoLoader::existingCustom($fullFileName) as $jsFile) {
            $jsFileContents .= file_get_contents($jsFile);
        }
        ...
        // minify the js
        if (shouldResourcesBeMinified() && !dotb_is_file(str_replace('.js', '-min.js', $jsFilePath))) {
            $jsFileContents = DotbMin::minify($jsFileContents);
            $jsFilePath = str_replace('.js','-min.js',$jsFilePath);
            $fullFileName = str_replace('.js','-min.js',$fullFileName);
        }

        // now write the js to cache
        dotb_file_put_contents($jsFilePath,$jsFileContents);
    ...
    public function getAllImages()
    {
        ...
        $theme = $this;
        while (isset($theme->parentTheme) && DotbThemeRegistry::get($theme->parentTheme) instanceOf DotbTheme ) {
            $theme = DotbThemeRegistry::get($theme->parentTheme);
        ...
        foreach($pathsToSearch as $path) {
            ...
            $paths = DotbAutoLoader::existing("custom/$path", $path);
            if($paths) {
                $path = array_pop($paths);
            } else {
                continue;
            }
            foreach(DotbAutoLoader::getDirFiles($path) as $file) {
                $file = basename($file);
...
class DotbThemeRegistry
{
    ...
    /**
     * Name of the current theme; corresponds to an index key in DotbThemeRegistry::$_themes
     *
     * @var string
     */
    ...
    {
        // make sure the we know the dotb version
        global $dotb_version, $dotb_db_version, $dotb_flavor, $dotb_build, $dotb_timestamp;
        ...
        // Check to see if theme is valid for this version of Dotb; return false if not
        $version_ok = false;
        if( isset($themedef['version']['exact_matches']) ){
            $matches_empty = false;
            foreach( $themedef['version']['exact_matches'] as $match ){
                if( $match == $GLOBALS['dotb_version'] ){
                    $version_ok = true;
                }
            }
        }
        if( !$version_ok && isset($themedef['version']['regex_matches']) ){
            ...
            foreach( $themedef['version']['regex_matches'] as $match ){
                if( preg_match( "/$match/", $GLOBALS['dotb_version'] ) ){
                    $version_ok = true;
                }
            }
        }
    ...
    public static function getDefault()
    {
        ...
        if (isset($GLOBALS['dotb_config']['default_theme']) && self::exists($GLOBALS['dotb_config']['default_theme']) && in_array($GLOBALS['dotb_config']['default_theme'], $enabledThemes)) {
            return self::get($GLOBALS['dotb_config']['default_theme']);
        }
    ...
    public static function buildRegistry()
    {
        ...
        if (DotbAutoLoader::existing("custom/themes/default/themedef.php")) {
            $themedef = array();
            require("custom/themes/default/themedef.php");
            $themedefDefault = $themedef;
        }

        foreach(DotbAutoLoader::getFilesCustom("themes", true) as $file) {
            if(DotbAutoLoader::existing("$file/themedef.php")) {
                $themedef = array();
                ...
                if ( self::exists($themedef['dirName']) ) {
                	...
                	foreach ( DotbTheme::getThemeDefFields() as $field )
        ...
        if ( !isset($GLOBALS['dotb_config']['default_theme']) || !self::set($GLOBALS['dotb_config']['default_theme']) ) {
            if ( count(self::availableThemes()) == 0 )
            {
                dotb_die('No valid themes are found on this instance');
            } else {
     ...
     * from upgrades.  It attempts to look for the Dotb theme and if not found, defaults to return the name of the last theme
    ...
    public static function availableThemes()
    {
        ...
        if ( isset($GLOBALS['dotb_config']['disabled_themes']) )
            $disabledThemes = explode(',',$GLOBALS['dotb_config']['disabled_themes']);
