...
/**
 * TemplateHandler builds templates using DotbFields and a generic view.
...
class TemplateHandler {
    ...
    function buildTemplate($module, $view, $tpl, $ajaxSave, $metaDataDefs) {
        ...
        if($view == 'EditView' || strpos($view,'QuickCreate') || $ajaxSave || $view == "ConvertLead") {
            ...
            $dotbbean = new stdClass;
            $dotbbean->field_defs = $defs;
            $dotbbean->module_dir = $module;

            $javascript = new javascript();
            $view = $view == 'QuickCreate' ? "QuickCreate_{$module}" : $view;
            $javascript->setFormName($view);

            $javascript->setDotbBean($dotbbean);
            if ($view != "ConvertLead")
            ...
            foreach ($dotbbean->field_defs as $name => $def) {
               if($def['type']=='relate' &&
                  isset($defs2[$name]) &&
                  ...
                  if(isset($mod_strings[$def['vname']])
                        ...
                        || translate($def['vname'],$dotbbean->module_dir) != $def['vname']) {
                     $vname = $def['vname'];
                  }
                  else{
                     $vname = "undefined";
    ...
    function deleteTemplate($module, $view) {
        if(is_file($this->cacheDir . $this->templateDir . $module . '/' .$view . '.tpl')) {
            ...
            DotbCache::cleanFile($cache_file_name);
                ...
                if($field['type'] == 'relate' && isset($field['module']) && (preg_match('/_name$|_c$/si',$name) || !empty($field['quicksearch']))) {
                    if (!preg_match('/_c$/si',$name)
                        ...
                        if($matches[0] == 'Campaigns') {
                            ...
                            //primary_XXX fields with the Account's billing address values).
                            //addtionalFields are key/value pair of fields to fill from Accounts(key) to Contacts(value)
                            $billingKey = DotbArray::staticGet($f, 'displayParams.billingKey');
                            $shippingKey = DotbArray::staticGet($f, 'displayParams.shippingKey');
                            $additionalFields = DotbArray::staticGet($f, 'displayParams.additionalFields');
    ...
    function createDependencyJavascript($fieldDefs, $viewDefs, $view, $module = null) {
        //Use a doWhen to wait for the page to be fulled loaded (!DOTB.util.ajaxCallInProgress())
        // TODO check if this isn't broken...
        $js = "<script type=text/javascript>\n"
            . "DOTB.util.doWhen('!DOTB.util.ajaxCallInProgress()', function(){\n"
            . "DOTB.forms.AssignmentHandler.registerView('$view');\n";
        ...
        $js .= "\nYAHOO.util.Event.onContentReady('$viewId', DOTB.forms.AssignmentHandler.loadComplete);";
        $js .= "});</script>";
