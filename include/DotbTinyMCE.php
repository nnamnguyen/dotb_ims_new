...
class DotbTinyMCE
{
    ...
    function getInstance($targets = "")
    {
        ...
        if (DotbAutoLoader::existing('include/javascript/tiny_mce/langs/' . $lang . '.js')) {
            $config['language'] = $lang;
        }
        $config['directionality']          = DotbThemeRegistry::current()->directionality;
...
eoq;
    ...
    {
        if (DotbAutoLoader::existing($this->customConfigFile)) {
            require_once($this->customConfigFile);
        }

        $defs = DotbAutoLoader::loadExtension('tinymce');
        if ($defs) {
            require($defs);
        }
    ...
    {
        if (DotbAutoLoader::existing($this->customPluginConfigFile)) {
    ...
    {
        if (DotbAutoLoader::existing($this->customDefaultConfigFile)) {
