...
class TeamBasedACLConfigurator
{
    ...
    {
        return DotbConfig::getInstance()->get(self::CONFIG_KEY, self::getDefaultConfig());
    }
    ...
    public static function implementsTBA($module)
    {
        if (!isset(self::$implementationCache[$module])) {
            $bean = BeanFactory::newBean($module);
            // need to check if $bean is instance of DotbBean
            // because of DynamicFields module and DynamicField class which is not a DotbBean
            self::$implementationCache[$module] =
                $bean
                && $bean instanceof DotbBean
     ...
     * @param DotbBean $bean
     */
    public function removeAllTBAValuesFromBean(DotbBean $bean)
    {
