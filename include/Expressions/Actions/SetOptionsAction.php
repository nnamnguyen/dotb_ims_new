...
class SetOptionsAction extends AbstractAction{
	...
	static function getJavascriptClass() {
		return  "
		DOTB.forms.SetOptionsAction = function(target, keyExpr, labelExpr) {
		...
		DOTB.util.extend(DOTB.forms.SetOptionsAction, DOTB.forms.AbstractAction, {
			exec: function(context) {
				...
				if (context.view)
				{
					...
					var visAction = new DOTB.forms.SetVisibilityAction(this.target, (empty ? 'false' : 'true'), '');
					...
					if (keys instanceof Array && field.options != null)
					{
						...
						if (typeof(labels) == 'string') //get translated values from Dotb Language
						{
							var fullSet = DOTB.language.get('app_list_strings', labels);
							labels = [];
							for (var i in keys)
							{
						...
						if(!field.multiple && field.value != selected) {
							DOTB.forms.AssignmentHandler.assign(this.target, field.value);
						}
						...
						if ( DOTB.forms.AssignmentHandler.ANIMATE && !empty)
					...
					//Check if we are on a detailview and just need to hide the field
						...
						var va = new DOTB.forms.SetVisibilityAction(this.target, 'false', '');
						va.exec(context);
					}
				}
			}
	...
	function getJavascriptFire() {
		return  "new DOTB.forms.SetOptionsAction('{$this->targetField}','{$this->keysExpression}', '{$this->labelsExpression}')";
	}
