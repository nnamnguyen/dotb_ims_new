...
class SetRequiredAction extends AbstractAction
{
    ...
    {
        return "
DOTB.forms.SetRequiredAction = function(variable, expr, label) {
 ...
 */
DOTB.util.extend(DOTB.forms.SetRequiredAction, DOTB.forms.AbstractAction, {
    ...
    exec: function(context) {
     ...
     bwcExec : function(context, required) {
        var el = DOTB.forms.AssignmentHandler.getElement(this.variable);
        if ( typeof(DOTB.forms.FormValidator) != 'undefined' )
            DOTB.forms.FormValidator.setRequired(el.form.name, el.name, this.required);
    ...
    {
        return "new DOTB.forms.SetRequiredAction('{$this->targetField}','{$this->expression}', '{$this->targetLabel}')";
    }
