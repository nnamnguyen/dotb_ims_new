...
class Parser {
	...
	static function evaluate($expr, $context = false)
	{
        ...
        {
            $var = substr($expr, 1);
            $ret = new DotbFieldExpression($var);
        ...
        if (preg_match('/^\$[a-zA-Z0-9_\-]+\.[a-zA-Z0-9_\-]+$/', $expr))
        {
            ...
            $linkField = new DotbFieldExpression($link);
        ...
        if (empty(static::$function_cache)) {
            $cacheFile = dotb_cached('Expressions/functionmap.php');
    ...
    public static function getFormulaRelateFields($expr, $linkName = '')
    {
        ...
        if (static::isRelatedExpression($expr)) {
            ...
            if (is_array($params)) {
                ...
                // but not it's value
                if ($params[0] instanceof DotbFieldExpression) {
