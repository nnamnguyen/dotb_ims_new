...
class AverageRelatedExpression extends NumericExpression
{
    ...
    public function evaluate()
    {
        ...
        //This should be of relate type, which means an array of DotbBean objects
        ...
        foreach ($linkField as $bean) {
            ...
            if (!empty($bean->$relfield) && is_numeric($bean->$relfield)) {
                ...
                $ret = DotbMath::init($ret)->add($value)->result();
            }
        }

        return DotbMath::init($ret)->div(count($linkField))->result();
    }
    ...
    {
        return <<<JS
        // this is only supported in Lumia
        if (App === undefined) {
