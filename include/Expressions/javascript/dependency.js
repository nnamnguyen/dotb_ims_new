 ...
 * DOTB Calculated Fields/Dependent Dropdowns implementation.
 * This is integrated heavily with the DOTB Expressions engine
...
if ( typeof(DOTB.forms.animation) == 'undefined') DOTB.forms.animation = {};
 ...
 */
var AH = DOTB.forms.AssignmentHandler = function() {
	// pass ...
}
...
AH.registerView = function(view, startEl) {
    ...
    var nodes = YAHOO.util.Selector.query("span.dotb_field", startEl);
	for (var i in nodes) {
		if (nodes[i].id != "")
...
AH.getValue = function(variable, view, ignoreLinks) {
	...
	if(field.tagName.toLowerCase() == "input" && field.type.toLowerCase() == "checkbox") {
		return field.checked ? DOTB.expressions.Expression.TRUE : DOTB.expressions.Expression.FALSE;
	}
	...
	if (field.className && (field.className == "DateTimeCombo" || field.className == "Date")){
		return DOTB.util.DateUtils.parse(field.value, "user");
	}
	...
	{
		var asNum = DOTB.expressions.unFormatNumber(field.value);
...
AH.assign = function(variable, value, flash)
{
    ...
    else if (Dom.hasClass(field, "imageUploader"))
	{
		...
		field.checked = value == DOTB.expressions.Expression.TRUE || value === true;
	}
    ...
    else if(value instanceof Date)
    {
        if (Dom.hasClass(field, "date_input"))
			field.value = DOTB.util.DateUtils.formatDate(value);
		else {
            if (Dom.hasClass(field, "DateTimeCombo"))
                AH.setDateTimeField(field, value);

            field.value = DOTB.util.DateUtils.formatDate(value, true);
        }
    }
	else {
	...
	// fire onchange
	DOTB.util.callOnChangeListers(field);
...
AH.setDateTimeField = function(field, value)
{
	...
	if (mer){
		...
		var a = DOTB.expressions.userPrefs.timef.indexOf("A") != -1 ?
...
AH.loadComplete = function()
{
    ...
    for (var i = 0; i < AH.QUEUEDDEPS.length; i++)
    {
        DOTB.forms.Trigger.fire.call(AH.QUEUEDDEPS[i].trigger);
    }
...
{
    if (fields.length > 0){
        module = module || DOTB.forms.AssignmentHandler.getValue("module");
        record = record || DOTB.forms.AssignmentHandler.getValue("record");
        ...
        for (var i = fields.length - 1; i >= 0; i--)
        {
            ...
            {
                var linkDef = DOTB.forms.AssignmentHandler.getLink(fields[i].link);
                if (linkDef && linkDef.id_name && linkDef.module) {
                    ...
                    if (idField && (idField.tagName == "INPUT" || idField.hasAttribute("data-id-value")))
                    {
                        ...
                        fields[i].relId = DOTB.forms.AssignmentHandler.getValue(linkDef.id_name, false, true);
        ...
        if (fields.length > 0)
        {
            var r = http_fetch_sync("index.php", DOTB.util.paramsToUrl({
...
AH.getRelatedField = function(link, ftype, field, view){
    ...
    var linkDef = DOTB.forms.AssignmentHandler.getLink(link);
    var currId;
    if (linkDef.id_name)
    {
        currId = DOTB.forms.AssignmentHandler.getValue(linkDef.id_name, false, true);
    }
    ...
    if (typeof(linkDef[ftype]) == "undefined"
        ...
        linkDef = DOTB.forms.AssignmentHandler.getLink(link);
    }
...
/**
 * Resets all of the DotbLogic Form Handler variables and registers to clean any cruft from previously loaded views.
 */
AH.reset = function() {
    AH.VARIABLE_MAP = {};
    AH.LISTENERS = {};
...
DOTB.forms.FormExpressionContext = function(formName)
{
...
DOTB.util.extend(DOTB.forms.FormExpressionContext, DOTB.expressions.ExpressionContext, {
	getValue: function(varname)
	{
		var SE = DOTB.expressions, toConst = SE.ExpressionParser.prototype.toConstant;
    ...
    getAppListStrings : function(list) {
        return DOTB.language.get('app_list_strings', list);
    },
    parseDate: function(date, type) {
        return DOTB.util.DateUtils.parse(date, type);
    },
 ...
 */
DOTB.forms.DefaultExpressionParser = new DOTB.expressions.ExpressionParser();

/**
 * @STATIC
 * Parses expressions given a variable map.<br>
 */
DOTB.forms.evalVariableExpression = function(expression, varmap, view)
{
	return DOTB.forms.DefaultExpressionParser.evaluate(expression, new DOTB.forms.FormExpressionContext(view));
}
 ...
 */
DOTB.forms._performRangeReplace = function(expression)
{
...
DOTB.forms.getFieldsFromExpression = function(expression)
{
	var re = /[^$]*?\$(\w+)[^$]*?/g,
		matches = [],
		result;
 ...
 */
DOTB.forms.Dependency = function(trigger, actions, falseActions, testOnLoad, form)
{
	...
	this.context = new DOTB.forms.FormExpressionContext(form);
...
DOTB.forms.Dependency.prototype.getRelatedFields = function () {
    ...
    DOTB.forms.AbstractAction.prototype.evalExpression = function (exp, context) {
        return DOTB.forms.DefaultExpressionParser.evaluate(exp, context).evaluate();
    }
     ...
     */
    DOTB.forms.Trigger = function (variables, condition) {
    ...
    DOTB.forms.Trigger.prototype._attachListeners = function () {
        ...
        for (var i = 0; i < this.variables.length; i++) {
            ...
            if (!el) continue;
            if (el.type && el.type.toUpperCase() == "CHECKBOX") {
                YAHOO.util.Event.addListener(el, "click", DOTB.forms.Trigger.fire, this, true);
            } else {
                YAHOO.util.Event.addListener(el, "change", DOTB.forms.Trigger.fire, this, true);
            }
        }
    }
     ...
     */
    DOTB.forms.Trigger.fire = function () {
        // eval the condition
        var eval;
        var val;
        try {
            eval = DOTB.forms.DefaultExpressionParser.evaluate(this.condition, this.context);
        } catch (e) {
            if (!DOTB.isIE && console && console.log) {
    ...
    DOTB.forms.FlashField = function (field, to_color) {
        ...
        if (DOTB.forms.flashInProgress[field.id])
            return;
        DOTB.forms.flashInProgress[field.id] = true;
        ...
        oButtonAnim.onComplete.subscribe(function () {
            if (this.attributes.backgroundColor.to == to_color) {
                ...
                DOTB.forms.flashInProgress[field.id] = false;
            }
        });
            ...
            if (tabView.get) {
                ...
                for (var i in tabs) {
                    if (i != tabView.get("activeIndex") && (tabs[i].get("contentEl") == parentDiv
                        ...
                        if (DOTB.forms.flashInProgress[label.parentNode.id])
                            return;
                        ...
                        tabAnim.onComplete.subscribe(function () {
                            if (this.attributes.color.to == '#F00') {
                                ...
                                DOTB.forms.flashInProgress[label.parentNode.id] = false;
                            }
                        });
                        DOTB.forms.flashInProgress[label.parentNode.id] = true;
                        tabAnim.animate();
                    }
                }
            }
