 ...
 * 3) Change the namespace for Expression and subclasses (eg. DOTB.expressions.math.Expression)
...
if ( typeof(DOTB.expressions) == 'undefined' ) DOTB.expressions = {};
if ( typeof(DOTB.util) == 'undefined' ) DOTB.util = {};
 ...
 */
DOTB.util.extend = function(subc, superc, overrides) {
 ...
 */
DOTB.expressions.Expression = function() {
};
 ...
 */
DOTB.expressions.Expression.INFINITY = -1;

/**
 * The various types supported by Expression.
 */
DOTB.expressions.Expression.STRING_TYPE   	= "string";
DOTB.expressions.Expression.NUMERIC_TYPE  	= "number";
DOTB.expressions.Expression.DATE_TYPE 	 	= "date";
DOTB.expressions.Expression.TIME_TYPE 	 	= "time";
DOTB.expressions.Expression.BOOLEAN_TYPE 	= "boolean";
DOTB.expressions.Expression.ENUM_TYPE 	 	= "enum";
DOTB.expressions.Expression.GENERIC_TYPE  	= "generic";

/**
 * The two boolean values.
 */
DOTB.expressions.Expression.TRUE  = "true";
DOTB.expressions.Expression.FALSE = "false";


/**
 * Initializes the Expression object.
 */
DOTB.expressions.Expression.prototype.init = function(params) {
 ...
 */
DOTB.expressions.Expression.prototype.getParameters = function() {
	return this.params;
};
 ...
 */
DOTB.expressions.Expression.prototype.validateParameters = function() {
	...
	if ( types instanceof Array && count != DOTB.expressions.Expression.INFINITY && count != types.length ) {
		throw ("Parameter types must be valid and match the parameter count");
	}

	// make sure types is valid
	if ( typeof(types) == 'string' ) {
		if ( DOTB.expressions.Expression.TYPE_MAP[types] == null ) {
			throw ("Invalid type requirement '" + types + "'");
		}
	} else {
		for ( var i = 0; i < types.length; i++ ) {
			if ( typeof( DOTB.expressions.Expression.TYPE_MAP[types[i]]) == 'undefined' ) {
	...
	if ( count != DOTB.expressions.Expression.INFINITY && params instanceof Array && params.length != count ) {
		throw ("Requires exactly " + count + " parameter(s)");
	}
 ...
 */
DOTB.expressions.Expression.prototype.getParamCount = function() {
	return DOTB.expressions.expressions.Expression.INFINITY;
};

/**
 * Enforces the parameter types.
 */
DOTB.expressions.Expression.prototype.isProperType = function(variable, type) {
	if ( type instanceof Array ) {
		return false;
	}

	// retrieve the class
	var c = DOTB.expressions.Expression.TYPE_MAP[type];
	...
	switch(type) {
		case DOTB.expressions.Expression.STRING_TYPE:
			return ( isInstance || typeof(variable) == 'string' );
			break;
		case DOTB.expressions.Expression.NUMERIC_TYPE:
			return ( isInstance || typeof(variable) == 'number' );
			break;
		case DOTB.expressions.Expression.BOOLEAN_TYPE:
			if ( variable instanceof DOTB.expressions.expressions.Expression ) {
				variable = variable.evaluate();
			}
			return ( isInstance || variable == DOTB.expressions.Expression.TRUE || variable == DOTB.expressions.Expression.FALSE );
			break;
	}
 ...
 */
DOTB.expressions.Expression.prototype.evaluate = function() {
 ...
 */
DOTB.expressions.Expression.prototype.getParameterTypes = function() {
	// does nothing .. needs to be overridden
 ...
 */
DOTB.expressions.NumericExpression = function(params) {
};
DOTB.util.extend(DOTB.expressions.NumericExpression, DOTB.expressions.Expression, {
	/**
	 * All parameters have to be a number by default.
	 */
	getParameterTypes: function() {
		return DOTB.expressions.Expression.NUMERIC_TYPE;
	}
});
 ...
 */
DOTB.expressions.StringExpression = function(params) {
};
DOTB.util.extend(DOTB.expressions.StringExpression, DOTB.expressions.Expression, {
	/**
	 * All parameters have to be a string by default.
	 */
	getParameterTypes: function() {
		return DOTB.expressions.Expression.STRING_TYPE;
	}
});
 ...
 */
DOTB.expressions.BooleanExpression = function(params) {
};
DOTB.util.extend(DOTB.expressions.BooleanExpression, DOTB.expressions.Expression, {
	/**
	 * All parameters have to be a dotb-boolean by default.
	 */
	getParameterTypes: function() {
		return DOTB.expressions.Expression.BOOLEAN_TYPE;
	}
});
 ...
 */
DOTB.expressions.EnumExpression = function(params) {
};
DOTB.util.extend(DOTB.expressions.EnumExpression, DOTB.expressions.Expression, {
	/**
	 * All parameters have to be an enumeration by default.
	 */
	getParameterTypes: function() {
		return DOTB.expressions.Expression.ENUM_TYPE;
	}
});
 ...
 */
DOTB.expressions.DateExpression = function(params) {
};
DOTB.util.extend(DOTB.expressions.DateExpression, DOTB.expressions.Expression, {
	/**
	 * All parameters have to be date by default.
	 */
	getParameterTypes: function() {
		return DOTB.expressions.Expression.DATE_TYPE;
	}
});
 ...
 */
DOTB.expressions.TimeExpression = function(params) {
};
DOTB.util.extend(DOTB.expressions.TimeExpression, DOTB.expressions.Expression, {
	/**
	 * All parameters have to be time by default.
	 */
	getParameterTypes: function() {
		return DOTB.expressions.Expression.TIME_TYPE;
	}
});
 ...
 */
DOTB.expressions.Expression.TYPE_MAP	= {
		"number" 	: DOTB.expressions.NumericExpression,
		"string" 	: DOTB.expressions.StringExpression,
		"date" 		: DOTB.expressions.DateExpression,
		"time" 		: DOTB.expressions.TimeExpression,
		"boolean" 	: DOTB.expressions.BooleanExpression,
		"enum" 		: DOTB.expressions.EnumExpression,
		"generic" 	: DOTB.expressions.Expression
};
 ...
 */
DOTB.expressions.ConstantExpression = function(params) {
	this.init(params);
}

DOTB.util.extend(DOTB.expressions.ConstantExpression, DOTB.expressions.NumericExpression, {
	evaluate: function() {
 ...
 */
DOTB.expressions.StringLiteralExpression = function(params) {
	this.init(params);
}
DOTB.util.extend(DOTB.expressions.StringLiteralExpression, DOTB.expressions.StringExpression, {
	evaluate: function() {
		return this.getParameters();
	},
	getParamCount: function() {
 ...
 */
DOTB.expressions.FalseExpression = function(params) {
	this.init(params);
}
DOTB.util.extend(DOTB.expressions.FalseExpression, DOTB.expressions.BooleanExpression, {
	evaluate: function() {
 ...
 */
DOTB.expressions.TrueExpression = function(params) {
	this.init(params);
}
DOTB.util.extend(DOTB.expressions.TrueExpression, DOTB.expressions.BooleanExpression, {
	evaluate: function() {
 ...
 */
DOTB.expressions.ExpressionParser = function() {
	// nothing
};
 ...
 * object.
 */
DOTB.expressions.ExpressionParser.prototype.evaluate = function(expr)
{
	...
	if ( DOTB.expressions.FunctionMap[func] == null )
	...
	return new DOTB.expressions.FunctionMap[func](args);
}
 ...
 */
DOTB.expressions.ExpressionParser.prototype.toConstant = function(expr) {
	// a raw numeric constant
	if (isFinite(expr) && !isNaN(parseFloat(expr))) {
		return new DOTB.expressions.ConstantExpression( parseFloat(expr) );
	}

	// a pre defined numeric constant
	var fixed = DOTB.expressions.NumericConstants[expr];
	if ( fixed != null && typeof(fixed) != 'undefined' )
		return new DOTB.expressions.ConstantExpression( parseFloat(fixed) );

	// a constant string literal
	if ( (/^".*"$/).exec(expr) != null ) {
		expr = expr.substring(1, expr.length-1);		// remove start/end quotes
		return new DOTB.expressions.StringLiteralExpression( expr );
	}

	// a boolean
	if ( expr == "true" ) {
		return new DOTB.expressions.TrueExpression();
	} else if ( expr == "false" ) {
		return new DOTB.expressions.FalseExpression();
	}
	...
	if ( (/^(0[0-9]|1[0-2])\/([0-2][0-9]|3[0-1])\/[0-3][0-9]{3,3}$/).exec(expr) != null ) {
		...
		return new DOTB.expressions.DateExpression([day, month, year]);
	}
	...
	if ( (/^([0-1][0-9]|2[0-4]):[0-5][0-9]:[0-5][0-9]$/).exec(expr) != null ) {
		...
		return new DOTB.expressions.TimeExpression([hour, minute, second]);
	}
