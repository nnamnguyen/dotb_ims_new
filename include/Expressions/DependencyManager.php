...
class DependencyManager
{
    ...
    /**
     * Returns a full set of the DotbLogic Dependencies to drive the business logic for a given view.
     * @static
    ...
    public static function getDependenciesForView($viewdef, $view = "", $module = "")
    {
        ...
        //Lumia metadata panel dependencies
    ...
    /**
     * Returns the set of the custom DotbLogic Dependencies defined in the dependency metadata
    ...
    private static function getModuleDependencyMetadata($module)
    {
        ...
        foreach (DotbAutoLoader::existingCustom("modules/$module/metadata/dependencydefs.php") as $loc)
        {
            require $loc;
        }
        $defs = DotbAutoLoader::loadExtension("dependencies", $module);
        if($defs) {
            require $defs;
        }
    ...
    /**
     * @static
     * @param  $user User, user to return DotbLogic variables for
     * @return string
     */
    public static function getJSUserVariables($user)
    {
        $ts = TimeDate::getInstance();
        return "DOTB.expressions.userPrefs = " . json_encode(array(
    ...
    public static function getLinkFields($fields, $view)
    {
        ...
        return "DOTB.forms.AssignmentHandler.LINKS['$view'] = " . json_encode($links) . "\n";
    }
    ...
    protected static function validLinkField($def)
    {
        ...
        $rel = DotbRelationshipFactory::getInstance()->getRelationship($def['relationship']);
        if ($rel === false) {
    ...
    /**
     * Used to mark a view as editable for the purpose of DotbLogic Dependencies
