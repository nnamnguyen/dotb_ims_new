...
abstract class source{
	...
	public function loadMapping() {
 		...
 		$map = DotbAutoLoader::existingCustomOne("modules/Connectors/connectors/sources/{$dir}/mapping.php");
 		if(!empty($map)) {
 		    require $map;
 		}
     ...
     */
 	public function loadVardefs() {
		...
		$defs = DotbAutoLoader::existingCustomOne("modules/Connectors/connectors/sources/{$dir}/vardefs.php");
		if(!empty($defs)) {
			require $defs;
		}
	...
	public function loadConfig()
	{
		$config = array();
		$dir = str_replace('_','/',get_class($this));
		foreach(DotbAutoLoader::existingCustom("modules/Connectors/connectors/sources/{$dir}/config.php") as $file) {
		    require $file;
		}
		$this->_config = $config;
	...
	public function getOriginalMapping() {
 		$mapping = array();
 		$dir = str_replace('_','/',get_class($this));
        foreach(DotbAutoLoader::existing("modules/Connectors/connectors/sources/{$dir}/mapping.php") as $file) {
            require $file;
        }
		return $mapping;
 	}
