...
class component
{
    ...
    /**
     * fillBean
     * This function wraps the call to getItem, but takes an additional DotbBean argument
     * and loads the DotbBean's fields with the results as defined in the connector
     ...
     * @param $bean DotbBean instance to load values into
     ...
     * @param $args Array of arguments to pass into getItem
     * @param $module String value of the module to map bean to
     * @param $bean Array to load DotbBean intances into
        ...
        if (!empty($list)) {
            ...
            if ($hasOptions) {
                ...
                foreach ($results as $key => $bean) {
                    foreach ($optionFields as $sourceField => $dotbField) {
                        $options_map = $options[$field_defs[$sourceField]['options']];
                        $results[$key]->$dotbField =  !empty($options_map[$results[$key]->$dotbField]) ? $options_map[$results[$key]->$dotbField] : $results[$key]->$dotbField;
                    }
                } //foreach
            }
        }
    ...
    public function mapOutput($bean, $result)
    {
        if (is_object($bean)) {
            ...
            $mapped = array();
            if (!empty($mapping)) {
                foreach ($mapping as $source_field => $dotb_field) {
                    $bean->$dotb_field = $this->getFieldValue($bean, $result, $source_field);
                    $mapped[$source_field] = $bean->$dotb_field;
                }
            } else {
    ...
    private function getFieldValue($bean, $result, $source_field)
    {
        ...
        $value = DotbArray::staticGet($result, $out_field);
