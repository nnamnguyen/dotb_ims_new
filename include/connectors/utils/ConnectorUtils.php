...
class ConnectorUtils
{
    ...
    public static function getViewDefs($filter_sources = array())
    {
        ...
        foreach($connectors as $id=>$ds) {
           ...
           $map = DotbAutoLoader::existingCustomOne($ds['directory'] . '/mapping.php');
           if(!empty($map)) {
               require $map;
           }
    ...
    public static function getConnectors($refresh = false, $save = true)
    {
        ...
        if ($refresh || !DotbAutoLoader::existing(self::$sourcePaths['conn'])) {
            $sources = array_merge(
                self::getSources(self::$sourcePaths['base']),
                self::getSources(self::$sourcePaths['cust'], true)
            );

            DotbAutoLoader::ensureDir(dirname(self::$sourcePaths['conn']));
    ...
    public static function cleanMetaDataFile($module)
    {
       ...
       self::removeHoverField($viewdefs, $module);
        ...
        if(file_exists($cachedfile = dotb_cached("modules/{$module}/DetailView.tpl")) && !unlink($cachedfile)) {
     ...
     *
     * @param mixed $displayParams Array value of display parameters passed from the DotbField code
    ...
    public static function getConnectorStrings($source_id, $language = '')
    {
        ...
        $defaultLanguage = $GLOBALS['dotb_config']['default_language'] . '.lang.php';
        ...
        foreach (DotbAutoLoader::existingCustom($files) as $file) {
            require $file;
            if (isset($connector_strings) && is_array($connector_strings)) {
                $strings = dotbLangArrayMerge($strings, $connector_strings);
            }
        }
    ...
    */
    public static function setConnectorStrings($source_id, $connector_strings, $language = '')
    {
        ...
        if (!dotb_mkdir($langPath, 755, true) || !write_array_to_file("connector_strings", $connector_strings, $path)) {
           //Log error and return empty array
