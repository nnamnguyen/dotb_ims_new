...
abstract class DBManager implements LoggerAwareInterface
{
	 ...
	 * create_user		Can create users for Dotb
    ...
    /**
     * Sets where properties for empty conditions on the DotbQuery object
     * @param DotbQuery_Builder_Where $where DotbQuery where object
     * @param string $field The field to compare
     * @param DotbBean $bean DotbBean
     * @return DotbQuery_Builder_Where
     */
    public function setEmptyWhere(DotbQuery_Builder_Where $where, $field, $bean = false)
    {
        $where->queryOr()
              ->equals($field, '', $bean)
              ->isNull($field, $bean);
    ...
    /**
     * Sets where properties for not empty conditions on the DotbQuery object
     * @param DotbQuery_Builder_Where $where DotbQuery where object
     * @param string $field The field to compare
     * @param DotbBean $bean DotbBean
     * @return DotbQuery_Builder_Where
     */
    public function setNotEmptyWhere(DotbQuery_Builder_Where $where, $field, $bean = false)
    {
        $where->queryAnd()
		...
		if(!empty($message)) {
			...
			if ($dieOnError || $this->dieOnError) {
				if(isset($GLOBALS['app_strings']['ERR_DB_FAIL'])) {
					dotb_die($GLOBALS['app_strings']['ERR_DB_FAIL']);
				} else {
					dotb_die("Database error. Please check dotbcrm.log for details.");
				}
			} else {
    ...
    public function dump_slow_queries($query)
    {
        ...
        if (!empty($dotb_config['xhprof_config'])) {
            DotbXHprof::getInstance()->trackSQL($query, $this->query_time);
        }

        $do_the_dump = isset($dotb_config['dump_slow_queries'])
            ? $dotb_config['dump_slow_queries'] : false;
        $slow_query_time_msec = isset($dotb_config['slow_query_time_msec'])
            ? $dotb_config['slow_query_time_msec'] : 5000;
	...
	protected function addDistinctClause(&$sql)
	{
		...
		if(!empty($GLOBALS['dotb_config']['disable_count_query']) && (stripos($sql, 'count(*)') === false && stripos($sql, ' JOIN (select tst.team_set_id from') !==false)){
	...
	*/
     ...
     *
     * @param DotbBean $bean DotbBean instance
     * @return bool
     */
	public function insert(DotbBean $bean)
	{
     ...
     * @return bool
     */
    public function update(DotbBean $bean)
    {
        $dataFields = array();
        $dataValues = array();
	 ...
	 *
	 * @param  array $beans Dotbbean instance(s)
	 ...
	 * @param DotbBean $bean  Dotbbean instance
	 */
	public function createTable(DotbBean $bean)
	{
	 ...
	 */
	protected function createConstraintSql(DotbBean $bean)
	{
	 ...
	 */
	public function repairTable(DotbBean $bean, $execute = true)
	{
     ...
     */
	public function createIndex(DotbBean $bean, $fieldDefs, $name, $unique = true)
	{
     ...
     * @return bool query result
	 */
	public function dropTable(DotbBean $bean)
	{
     ...
     *
     * @param DotbBean $bean   DotbBean containing the field
     * @param array  $fieldDefs Vardef definition of the field
     * @return bool query result
     */
	public function deleteColumn(DotbBean $bean, $fieldDefs)
	{
		$tablename = $bean->getTableName();
     ...
     * @deprecated
     *
     * @param  DotbBean $bean         the bean from which table we will generate insert stmts
     ...
     */
	public function generateInsertSQL(DotbBean $bean, $select_query, $start, $count = -1, $table, $is_related_query = false)
	{
		...
		if($count == -1){
			$count 	= $dotb_config['list_max_entries_per_page'];
		}
    ...
    public function sqlLikeString($str, $wildcard = '%', $appendWildcard = true)
    {
        ...
        // The config wildcard exists because there may be a case where a Dotb user would want
        // to use a non-standard character (e.g. '@') as a wildcard character for search.
        if (!empty($GLOBALS['dotb_config']['search_wildcard_char']) &&
            is_string($GLOBALS['dotb_config']['search_wildcard_char']) &&
            strlen($GLOBALS['dotb_config']['search_wildcard_char']) === 1
        ) {
            $likeChar = $GLOBALS['dotb_config']['search_wildcard_char'];
        } else {
            $likeChar = $wildcard;
        }

        // Add wildcard at the beginning of the search string.
        if (!empty($GLOBALS['dotb_config']['search_wildcard_infront']) &&
...
/********************** SQL FUNCTIONS ****************************/
     ...
     */
	public function createTableSQL(DotbBean $bean)
	{
		$tablename = $bean->getTableName();
		$fieldDefs = $bean->getFieldDefinitions();
		$indices = $bean->getIndices();
	 ...
	 * @return array
	 */
	protected function updateWhereArray(DotbBean $bean, array $where = array())
	{
		if (count($where) == 0) {
	 ...
	 *
	 * @param  DotbBean $bean DotbBean that describes the table
	 * @param  array  $whereArray Optional, where conditions in an array
	 * @return string
	 */
    protected function getWhereClause(DotbBean $bean, array $whereArray = array())
    {
	 ...
	 * @return string SQL Select Statement
	 */
	public function createIndexSQL(DotbBean $bean, array $fields, $name, $unique = true)
	{
	 ...
	 * @return string SQL statement
	 */
	public function dropTableSQL(DotbBean $bean)
	{
	 ...
	 * @return string SQL statement
	 */
	public function deleteColumnSQL(DotbBean $bean, $fieldDefs)
	{
     ...
     * @deprecated Use DotbBean::auditSQL()
     */
    protected function auditSQL(DotbBean $bean, $changes, $event_id)
	{
     ...
     * @param array $changes List of changes, contains 'before' and 'after'
     * @return bool query result
     * @deprecated Use DotbBean::saveAuditRecords()
     */
    public function save_audit_records(DotbBean $bean, $changes)
	{
     ...
     * @return array
     */
    public function getDataChanges(DotbBean &$bean, array $options = [])
    {
    ...
    public function getStateChanges(DotbBean $bean, array $prevState, array $options = [])
    {
            ...
            foreach ($fields as $field => $vardefs) {
                ...
                if (!empty($field_type) && $field_type === 'currency') {
                    if (empty($before_value)) {
                        ...
                        $before_value = DotbCurrency::convertAmountToBase($before_value, $bean->currency_id);
                    }
                    if (empty($after_value)) {
                        //further processing expects a string, so change empty array into blank string
                        $after_value = '';
                    } else {
                        $after_value = DotbCurrency::convertAmountToBase($after_value, $bean->currency_id);
                    }
                }
     ...
     * @param DotbBean $bean Dotbbean instance that was changed
     * @return array
     * @deprecated Use DotbBean::getAuditDataChanges()
     */
    ...
    */
     ...
     * I.e. generate a unique Dotb id in a sub select of an insert statement.
     * @abstract
     * @return string
     */
