...
class OracleManager extends DBManager
{
    ...
    /**
     * Sets where properties for empty conditions on the DotbQuery object. Since
    ...
    /**
     * Sets where properties for not empty conditions on the DotbQuery object. Since
     * Oracle does things a little differently with empty strings we need to
     * define this here to keep Oracle happy.
     * @param DotbQuery_Builder_Where $where DotbQuery where object
     * @param string $field The field to compare
    ...
    protected function checkQuery($sql, $object_name = false)
    {
        ...
        $badQuery = array();
        $minCost = (!empty($GLOBALS['dotb_config']['check_query_cost']))?$GLOBALS['dotb_config']['check_query_cost']:10;
        ...
        if (!empty($badQuery)) {
            foreach ($badQuery as $table=>$data ) {
                if(!empty($data)){
                    ...
                    if(!empty($GLOBALS['dotb_config']['check_query_log'])){
    ...
    public function limitQuery($sql, $start, $count, $dieOnError = false, $msg = '', $execute = true)
    {
        ...
        if ($start ==0 && !empty($matches[3])) {
            ...
            if(!empty($GLOBALS['dotb_config']['check_query'])){
            	$this->checkQuery($sql);
         	}
         	if($execute) {
    ...
    public function connect(array $configOptions = null, $dieOnError = false)
    {
        ...
        if(!$this->database){
                ...
                if (!$this->database) {
                	...
                	if($dieOnError) {
                        if(isset($GLOBALS['app_strings']['ERR_NO_DB'])) {
                            dotb_die($GLOBALS['app_strings']['ERR_NO_DB']);
                        } else {
                            dotb_die("Could not connect to the database. Please refer to dotbcrm.log for details.");
                        }
                    } else {
                	    return false;
                	}
                ...
                if($this->database && $this->getOption('persistent')){
                    $_SESSION['administrator_error'] = "<B>Severe Performance Degradation: Persistent Database Connections not working.  Please set \$dotb_config['dbconfigoption']['persistent'] to false in your config.php file</B>";
                }
        }
    ...
    public function installConfig()
    {
        return array(
        	'LBL_DBCONFIG_ORACLE' =>  array(
                ...
                'setup_db_create_dotbsales_user' => false,
            ),
