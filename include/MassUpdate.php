...
use Dotbcrm\Dotbcrm\IdentityProvider\Authentication\Config as IdmConfig;
...
class MassUpdate
{
    ...
    public function __construct(Request $request = null)
    {
        ...
        $this->idmConfig =  new IdmConfig(\DotbConfig::getInstance());
    }

	/**
	  * set the dotb bean to its internal member
	  * @param dotb bean reference
	  */
	...
	/**
	  ...
	  */
		...
		foreach($_POST as $post=>$value){
			if(is_array($value)){
				if(empty($value)){
					unset($_POST[$post]);
                            ...
                            if (!$update_blank) {
				if( isset($this->dotbbean->field_defs[$post]) && $this->dotbbean->field_defs[$post]['type'] == 'radioenum' && isset($_POST[$post]) ){
				  $_POST[$post] = '';
				}else{
				  unset($_POST[$post]);
                                }
			...
			if(is_string($value) && isset($this->dotbbean->field_defs[$post])) {
		        if(($this->dotbbean->field_defs[$post]['type'] == 'bool'
				 	|| (!empty($this->dotbbean->field_defs[$post]['custom_type']) && $this->dotbbean->field_defs[$post]['custom_type'] == 'bool'
				 	))){
				 		if(strcmp($value, '2') == 0)$_POST[$post] = 0;
				 		if(!empty($this->dotbbean->field_defs[$post]['dbType']) && strcmp($this->dotbbean->field_defs[$post]['dbType'], 'varchar') == 0 ){
                            $_POST[$post] = strcmp($value, '1') == 0 ? 'on' : 'off';
				 		}
    			}

			    if( ($this->dotbbean->field_defs[$post]['type'] == 'radioenum' && isset($_POST[$post]) && strlen($value) == 0)
			    || ($this->dotbbean->field_defs[$post]['type'] == 'enum' && $value == '__DotbMassUpdateClearField__') // Set to '' if it's an explicit clear
			    ){
				    $_POST[$post] = '';
			    }
                if ($this->dotbbean->field_defs[$post]['type'] == 'bool') {
			    ...
			    if($this->dotbbean->field_defs[$post]['type'] == 'date' && !empty($_POST[$post])){
			        $_POST[$post] = $timedate->to_db_date($_POST[$post], false);
			    }
                if($this->dotbbean->field_defs[$post]['type'] == 'datetime' && !empty($_POST[$post])){
			        $_POST[$post] = $timedate->to_db($this->date_to_dateTime($post, $value));
			    }
			    if($this->dotbbean->field_defs[$post]['type'] == 'datetimecombo' && !empty($_POST[$post])){
        ...
        if (!empty($_REQUEST['uid']) && !isset($_REQUEST['entire'])) {
            ...
            $query = $this->dotbbean->create_new_list_query($order_by, $this->where_clauses, array(), array(), 0, '', false, $this, true, true);
		...
		if(isset($_POST['mass']) && is_array($_POST['mass'])  && $_REQUEST['massupdate'] == 'true'){
            ...
            if ($this->dotbbean->object_name == 'User' && isset($_POST['is_admin'])) {
			...
			foreach($_POST['mass'] as $id){
				...
				if(isset($_POST['Delete'])){
					...
					$this->dotbbean->disable_row_level_security = true;
					$this->dotbbean->retrieve($id);
					if($this->dotbbean->ACLAccess('Delete')){
                        // Fix for RS-1267 - We should not processing mass delete for global team
                        if ($this->dotbbean->object_name == 'Team' &&
                            $this->dotbbean->id == $this->dotbbean->global_team
                        ) {
                            $retval[] = $id;
                            DotbApplication::appendErrorMessage($app_strings['LBL_MASSUPDATE_DELETE_GLOBAL_TEAM']);
                            continue;
                        }

					    if ($this->dotbbean->object_name == 'Team' && $this->dotbbean->has_records_in_modules()) {
                            if(!isset($_SESSION['REASSIGN_TEAMS'])) {
                                $_SESSION['REASSIGN_TEAMS'] = array();
                            }
                            $_SESSION['REASSIGN_TEAMS'][] = $this->dotbbean->id;
						    continue;
						}
                        $this->dotbbean->mark_deleted($id);
                        // ideally we should use after_delete logic hook
                        $searchEngine = DotbSearchEngineFactory::getInstance();
                        $searchEngine->delete($this->dotbbean);
                    } else {
                        $retval[] = $id;
                    }
				}
				else {
					if($this->dotbbean->object_name == 'Contact' && isset($_POST['Sync'])){ // special for contacts module
						if($_POST['Sync'] == 'true') {
							$this->dotbbean->retrieve($id);
							if($this->dotbbean->ACLAccess('Save')){
								if($this->dotbbean->object_name == 'Contact'){

									$this->dotbbean->contacts_users_id = $current_user->id;
									$this->dotbbean->save(false);
								}
							}
						}
						elseif($_POST['Sync'] == 'false') {
							...
							if($this->dotbbean->ACLAccess('Save')){
								if($this->dotbbean->object_name == 'Contact'){
									if (!isset($this->dotbbean->users))
									{
										$this->dotbbean->load_relationship('user_sync');
									}
									$this->dotbbean->contacts_users_id = null;
									$this->dotbbean->user_sync->delete($this->dotbbean->id, $current_user->id);
								}
							}
						}
					...
					if($count++ != 0) {
                       ...
                       $this->dotbbean = $this->dotbbean->getCleanCopy();
					}

					$this->dotbbean->retrieve($id);

					if($this->dotbbean->ACLAccess('Save', array("massupdate" => true))) {
						$_POST['record'] = $id;
						$_GET['record'] = $id;
						$_REQUEST['record'] = $id;
						$newbean=$this->dotbbean;
						...
						if (isset( $this->dotbbean->assigned_user_id)) {
							$old_assigned_user_id = $this->dotbbean->assigned_user_id;
	                    ...
	                    if (!empty($_POST['optout_primary'])) {
	                    	...
	                    	if (isset($this->dotbbean->emailAddress)) {
	                    		if (!empty($this->dotbbean->emailAddress->addresses)) {
	                    			foreach($this->dotbbean->emailAddress->addresses as $key =>$emailAddressRow) {
	    ...
	    )
	{
		...
		if(!$this->dotbbean->ACLAccess('edit', true) && !$this->dotbbean->ACLAccess('massupdate', true)) {
		    return '';
		}
		...
		if($this->dotbbean->object_name == 'Contact')
		{
			...
			$even = false;
		} else if($this->dotbbean->object_name == 'Employee') {
			$this->dotbbean->field_defs['employee_status']['type'] = 'enum';
			$this->dotbbean->field_defs['employee_status']['massupdate'] = true;
			$this->dotbbean->field_defs['employee_status']['options'] = 'employee_status_dom';
		} else if($this->dotbbean->object_name == 'InboundEmail'){
			$this->dotbbean->field_defs['status']['type'] = 'enum';
			$this->dotbbean->field_defs['status']['options'] = 'user_status_dom';
		}
        ...
        if (in_array($this->dotbbean->module_name, $this->idmConfig->getIDMModeDisabledModules())
            && $this->idmConfig->isIDMModeEnabled()
        ) {
            $banned += $this->idmConfig->getIDMModeDisabledFields();
        }

		foreach($this->dotbbean->field_defs as $field)
		{
			   if(!$this->dotbbean->ACLFieldAccess($field['name'], 'edit')) {
			   	  continue;
			   }

            if ($this->dotbbean->object_name == 'User' && $field['name'] == 'is_admin') {
					...
					switch($field["type"])
					{
						...
						case "multienum":
							if(!empty($field['isMultiSelect']))
							{
								...
								$functionValue = getFunctionValue(isset($field['function_bean']) ? $field['function_bean'] : null, $field['function'], array($this->dotbbean, $field['name'], '', 'MassUpdate'));
		...
		if($this->dotbbean->object_name == 'Email'
...
eoq;
        ...
        if ($this->dotbbean->object_name == 'User' && ($current_user->is_admin || $current_user->isAdminForModule('Users'))) {
            $html .=<<<eoq
...
EOJS;
		...
		if($field_count > 0)
		{
			...
			$html = "<div id='massupdate_form' style='display:none;'><table width='100%' cellpadding='0' cellspacing='0' border='0' class='formHeader h3Row'><tr><td nowrap><h3><span>" . $app_strings['LBL_MASS_UPDATE']."</h3></td></tr></table>";
			if($this->dotbbean->ACLAccess('Delete', true) && !$hideDeleteIfNoFieldsAvailable){
	...
	function addParent($displayname, $field){
		...
		'field_to_name_array' => array(
			...
			".DotbThemeRegistry::current()->getImage("id-ff-select", '', null, null, ".png", $app_strings['LBL_ID_FF_SELECT'])."
			</button></span>";
			$parent_type = $field['parent_type'];
			$parent_types = $app_list_strings[$parent_type];
            $disabled_parent_types = DotbACL::disabledModuleList($parent_types);
		...
		$popup_request_data = array(
			...
			'field_to_name_array' => array(
                ...
                $qsName = array(
						...
						'limit' => '30','no_match_text' => $app_strings['ERR_SQS_NO_MATCH']);
                $qsName = $json->encode($qsName);
				$img = DotbThemeRegistry::current()->getImageURL("id-ff-select.png");
				//
				///////////////////////////////////////
...
EOQ;
        $dtscript = getVersionedScript('include/DotbFields/Fields/Datetimecombo/Datetimecombo.js');
		$html = <<<EOQ
		<td scope="row" width="20%">$displayname</td>
...
EOQ;
	...
	function date_to_dateTime($field, $value) {
        ...
        if (isset($this->dotbbean->field_defs[$field]['group'])) {
            $group =  $this->dotbbean->field_defs[$field]['group'];
            if (isset($this->dotbbean->field_defs[$group."_flag"]) && isset($_POST[$group."_flag"])
                && $_POST[$group."_flag"] == 1) {
                return "";
            }
        }

        $oldDateTime = $this->dotbbean->$field;
	...
	function checkClearField($field, $value) {
	    if ($value == 1 && strpos($field, '_flag')) {
            ...
            if (isset($this->dotbbean->field_defs[$field]['group'])) {
                $group =  $this->dotbbean->field_defs[$field]['group'];
                if (isset($this->dotbbean->field_defs[$group])) {
                    $_POST[$group] = "";
                }
            }
	    }
    ...
    function generateSearchWhere($module, $query) {//this function is similar with function prepareSearchForm() in view.list.php
        ...
        if(DotbAutoLoader::existing('modules/'.$module.'/SearchForm.html')){
            if(DotbAutoLoader::existing('modules/' . $module . '/metadata/SearchFields.php')) {
    ...
    {
        return DotbAutoLoader::loadSearchFields($module);
    }
    ...
    public function doMassUpdateFieldsExistForFocus()
    {
        static $banned = array('date_modified'=>1, 'date_entered'=>1, 'created_by'=>1, 'modified_user_id'=>1, 'deleted'=>1,'modified_by_name'=>1,);
        foreach($this->dotbbean->field_defs as $field) {
            if(!$this->dotbbean->ACLFieldAccess($field['name'], 'edit')) continue;
            if(!isset($banned[$field['name']]) && (!isset($field['massupdate']) || !empty($field['massupdate']))){
                if(isset($field['type']) && $field['type'] == 'relate' && isset($field['id_name']) && $field['id_name'] == 'assigned_user_id')
                    $field['type'] = 'assigned_user_name';
