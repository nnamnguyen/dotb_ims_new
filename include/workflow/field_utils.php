...
function get_display_text($temp_module, $field, $field_value, $adv_type=null, $ext1=null, $context=null){
    ...
    if($target_type == 'currency_id' && !empty($field_value)) {
        $currency = DotbCurrency::getCurrencyByID($field_value);
...
//Used primarily for alert templates
    ...
    $dotbField = DotbFieldHandler::getDotbField($target_type);
    $field_value = $dotbField->getEmailTemplateValue($field_value,$temp_module->field_defs[$field], $context);
 ...
 * @param DotbBean $source_object
 */
function execute_special_logic($field_name, DotbBean $source_object)
{
	...
	if(DotbAutoLoader::requireWithCustom('modules/'.$source_object->module_dir.'/SaveOverload.php'))
	{
