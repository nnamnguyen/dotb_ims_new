...
class DotbWorkflowPluginList
{
    public static $list;
    static function getList() {
        if(is_null(self::$list)) {
            if(DotbAutoLoader::existing('custom/workflow/plugins/plugin_list.php')) {
	...
	function trigger_createstep1(& $opt_array)
	{
		...
		$plugin_list = DotbWorkflowPluginList::getList();
		...
		if(!empty($plugin_list['trigger']['createstep1'])){
			foreach($plugin_list['trigger']['createstep1'] as $plugin_array){
				//check to see if plugin list exists
				foreach(DotbAutoLoader::existing('custom/workflow/plugins/'.$plugin_array["directory"].'/'.$plugin_array['meta_file'].'.php') as $file){
					require_once $file;
				}
	...
	function grab_plugin_function($plugin_array, & $opt)
	{
		...
		foreach(DotbAutoLoader::existing('custom/workflow/plugins/'.$plugin_array["directory"].'/'.$plugin_array["file"].'.php') as $file) {
			require_once $file;
...
//ACTION HOOOKS
	...
	function extract_plugin_list()
	{
		...
		foreach(DotbAutoLoader::getDirFiles("custom/workflow/plugins", true) as $file) {
		    foreach(DotbAutoLoader::existing("$file/component_list.php") as $comp_file) {
		        include $comp_file;
                	//bug 62487  - Corrects the array key that is used in custom/workflow/plugins/plugin_list.php, which makes is compatible with DotBCRM 6.5.x
                	$file = basename($file);
