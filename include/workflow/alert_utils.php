...
function get_user_alert_details(& $focus, $user_meta_array, & $address_array){
	...
	if($user_meta_array['user_type'] == "assigned_team_target")
	{
	   ...
	   $maxAlloweedUsers = !empty($GLOBALS['dotb_config']['max_users_team_notification'] ) ? $GLOBALS['dotb_config']['max_users_team_notification'] : 100;
...
function create_alert_email($notify_user) {
		global $dotb_version, $dotb_config, $app_list_strings, $current_user;
		...
		$xtpl->assign("URL", "{$dotb_config['site_url']}/index.php?module={$focus->module_dir}&action=DetailView&record={$focus->id}");
 ...
 *
 * @param DotbBean $focus The record.
...
function get_invite_link(& $focus, $notify_user_id=""){
	...
	if($notify_user_id!=""){
		...
		$accept_url = $dotb_config['site_url'].'/index.php?entryPoint=acceptDecline&module=Meetings&user_id='.$notify_user_id.'&record='.$focus->id;
...
function get_system_default_body(&$mail_object, $focus, &$notify_user) {
    ...
    if (!isset($_SESSION['authenticated_user_language']) || empty($_SESSION['authenticated_user_language'])) {
        $currentLanguage = $dotb_config['default_language'];
    } else {
