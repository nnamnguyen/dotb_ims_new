...
class ViewQuickcreate extends ViewAjax
{
    ...
    public function preDisplay()
    {
    	if(!empty($_REQUEST['source_module']) && $_REQUEST['source_module'] != 'undefined' && !empty($_REQUEST['record'])) {
			...
			if ( $this->bean instanceOf DotbBean
    ...
    public function display()
    {
		...
		$source = DotbAutoLoader::existingCustomOne($base . strtolower($view) . 'defs.php');
		if(!$source) {
		    //if our view does not exist default to EditView
		    $view = 'EditView';
		    $source = DotbAutoLoader::existingCustomOne($base . 'editviewdefs.php');
		}
