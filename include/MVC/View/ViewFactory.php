...
class ViewFactory{
    ...
    public static function loadView(
		...
		if(!empty($target_module)) {
		    $view_file = DotbAutoLoader::existingCustomOne('modules/'.$target_module.'/views/view.'.$type.'.php');
		...
		if(empty($view_file)) {
		    $view_file = DotbAutoLoader::existingCustomOne('modules/'.$module.'/views/view.'.$type.'.php');
		}
		if(empty($view_file)) {
			$view_file = DotbAutoLoader::existingCustomOne('include/MVC/View/views/view.'.$type.'.php');
		}
		if(!empty($view_file)) {
		...
		if(empty($view)) {
    		// Default to DotbView if still nothing found/built
		    $view = new DotbView();
		}
    ...
    static function _loadConfig(&$view, $type)
    {
		...
		$view_config = dotb_cache_retrieve("VIEW_CONFIG_FILE_".$view->module."_TYPE_".$type);
		if(!$view_config){
		    $view_config_all = array('actions' => array(), 'req_params' => array(),);
		    foreach(DotbAutoLoader::existingCustom(
			...
			dotb_cache_put("VIEW_CONFIG_FILE_".$view->module."_TYPE_".$type, $view_config);
		}
	 ...
	 * proper view object
	 *
	 * @return a valid DotbView
	 */
    private static function buildFromFile($file, &$bean, $view_object_map, $type, $module)
    {
		require_once($file);
		//try ModuleViewType first then try ViewType if that fails then use DotbView
		$class = DotbAutoLoader::customClass(ucfirst($module).'View'.ucfirst($type));
		...
		$class = DotbAutoLoader::customClass('View'.ucfirst($type));
		if(class_exists($class)){
            return ViewFactory::buildClass($class, $bean, $view_object_map);
		}
		//Now check if there is a custom DotbView for generic handling
		// autoloader will check filesystem
        $class = DotbAutoLoader::customClass('DotbView');
		//if all else fails return DotbView
    ...
    private static function buildClass($class, $bean, $view_object_map)
    {
		...
		if($view instanceof DotbView){
			return $view;
		}else
			return new DotbView($bean, $view_object_map);
	}
}
