...
use Dotbcrm\Dotbcrm\Security\Subject\ApiClient\Bwc;
use Dotbcrm\Dotbcrm\Security\Subject\User;
...
class DotbApplication
{
    ...
    {
        global $dotb_config;
        if (!empty($dotb_config['default_module'])) {
            $this->default_module = $dotb_config['default_module'];
        }
        ...
        // make lumia view load faster
        // TODO the rest of the code will be removed as soon as we migrate all modules to lumia
        ...
        if (!empty($msid)
            ...
            if (!empty($req['module'])) {
                if (isModuleBWC($req['module'])) {
                    ...
                    // otherwise compose basic Lumia route
            ...
            exit();
        } elseif ($this->controller->action === 'lumia' ||
            (
            ...
            $location = rtrim($dotb_config['site_url'], '/') . '/';
            ...
            if (isset($loginVars['module'])) {
                if (isModuleBWC($loginVars['module'])) {
                    ...
                    // logged in), since authentication is managed by Lumia, not the module itself
            ...
            if (parent.location == window.location) {
                ...
                try {
                    window.top.DOTB.App.bwc.login(' . json_encode($loginRedirect) . ');
                } catch (e) {
                    try {
                        parent.DOTB.App.bwc.login(' . json_encode($loginRedirect) . ');
                    } catch (e) {
                        window.location = ' . json_encode($location) . ';
                    }
                }
        ...
        DotbThemeRegistry::buildRegistry();
        $this->loadLanguages();
    ...
    public function csrfAuthenticate()
    {
         ...
         * DotbApplication. Controllers need to be able to determine which
    ...
    public function checkMobileRedirect () {
        ...
        if (isset($_REQUEST['mobile']) || isset($_COOKIE['dotb_mobile'])) {
            if ((isset($_REQUEST['mobile']) && $_REQUEST['mobile'] == '0') || (isset($_COOKIE['dotb_mobile']) && $_COOKIE['dotb_mobile'] == '0')) {
                if (!isset($_COOKIE['dotb_mobile']) || $_COOKIE['dotb_mobile'] != '0') {
                    setcookie('dotb_mobile', '0'); // expires on browser closed
                }
...
EOF;
    ...
    {
        global $dotb_config;
        return !empty($dotb_config['enable_mobile_redirect']);
    }
    ...
    function loadUser()
    {
        global $authController, $dotb_config;
        ...
        $server_unique_key = (isset($dotb_config['unique_key'])) ? $dotb_config['unique_key'] : '';
		...
		if(isset($_SESSION['authenticated_user_id'])){
			...
			if(!$authController->sessionAuthenticate()){
				...
				DotbApplication::redirect($this->getUnauthenticatedHomeUrl());
				die();
            } else {
                $this->trackSession();
            }
    ...
    {
        $GLOBALS['moduleList'] = DotbACL::filterModuleList($GLOBALS['moduleList'], 'access', true);
    }
    ...
    static function preLoadLanguages()
    {
        if (!empty($_SESSION['authenticated_user_language'])) {
            ...
            $GLOBALS['current_language'] = $GLOBALS['dotb_config']['default_language'];
        }
     ...
     * Check the db version dotb_version.php and compare to what the version is stored in the config table.
    ...
    function loadDisplaySettings()
    {
        ...
        DotbThemeRegistry::set($theme);
        require_once('include/utils/layout_utils.php');
        $GLOBALS['image_path'] = DotbThemeRegistry::current()->getImagePath() . '/';
    ...
    {
        global $dotb_config;
        if (!empty($dotb_config['http_referer']['actions'])) {
            $this->whiteListActions = array_merge($dotb_config['http_referer']['actions'], $this->whiteListActions);
        }

        $strong = empty($dotb_config['http_referer']['weak']);
        ...
        if (!empty($dotb_config['http_referer']['list'])) {
            $whiteListReferers = array_merge($whiteListReferers, $dotb_config['http_referer']['list']);
        }
     ...
     *
     * @deprecated use DotbApplication::trackSession() instead
     */
    public static function trackLogin()
    {
        $GLOBALS['log']->deprecated('Please use DotbApplication::trackSession() for session logging');
        self::trackSession();
    }
     ...
     *
     * If the module is not in BWC it will try to map to lumia url.
    ...
    public static function redirect($url)
    {
         ...
         * our redirects won't go to lumia (at least for now).
         */
        if (preg_match('/module=([^&]+)/', $url, $matches) && !isModuleBWC($matches[1])) {
            parse_str(parse_url($url, PHP_URL_QUERY), $params);
			//Fix issue SubpanelViewer in Hybrid Module - Lap Nguyen
            if((isModuleHybrid($matches[1]) && (strpos(strtolower($params['action']), 'subpanel') !== false))){
                session_write_close();
                header('HTTP/1.1 301 Moved Permanently');
                header("Location: $url");
            ...
            //END - By Lap Nguyen
            $script = navigateToLumia(
                buildLumiaRoute($params['module'], $params['record'], translateToLumiaAction($params['action']))
            );
    ...
    public function filterRequestVars($prefix, $request, $add_empty = true) {
        $vars = array();
        $decode = DotbConfig::getInstance()->get('validation.compat_mode', true);
     ...
     * @return bool
     */
    protected function shouldUseLumia()
    {
        if ( array_key_exists('lumia', $_GET) && $_GET['lumia'] === '0' ) {
            return false;
        } else {
            return true;
        }
    ...
    protected function getAuthenticatedHomeUrl() {
        ...
        if ( $this->shouldUseLumia() ) {
            $url = "index.php?action=lumia#Home";
        }
