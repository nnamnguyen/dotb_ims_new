...
/**
 * Main DotBCRM controller
 * @api
 */
class DotbController
{
	/**
	...
	/**
	 * @var array additional paramters that will be passed through to the view if a DotbView is displayed
	 */
     ...
     *
     * This logic is being called from DotbApplication for all non-GET reqs.
     *
	...
	/**
	 * Called from DotbApplication and is meant to perform the setup operations
	 * on the controller.
	 *
	 */
    ...
    */
	...
	private function loadMapping($var, $merge = false){
		$$var = dotb_cache_retrieve("CONTROLLER_". $var . "_".$this->module);
		if(!$$var){
			...
			foreach(DotbAutoLoader::existingCustom("include/MVC/Controller/{$var}.php", "modules/{$this->module}/{$var}.php") as $file) {
			    require $file;
			}

			$varname = str_replace(" ","",ucwords(str_replace("_"," ", $var)));
			foreach(DotbAutoLoader::existing("custom/application/Ext/$varname/$var.ext.php", "custom/modules/{$this->module}/Ext/$varname/$var.ext.php") as $file) {
			    require $file;
			}

			dotb_cache_put("CONTROLLER_". $var . "_".$this->module, $$var);
		}
		$this->$var = $$var;
	}

	/**
	 * This method is called from DotbApplication->execute and it will bootstrap the entire controller process
	 */
    ...
    protected function handleException(Exception $e)
    {
        ...
        DotbMetric_Manager::getInstance()->handleException($e);
	...
	protected function no_action(){
		dotb_die($GLOBALS['app_strings']['LBL_NO_ACTION']);
	}
	...
	protected function redirect(){
		...
		if(!empty($this->redirect_url))
			DotbApplication::redirect($this->redirect_url);
	}
	...
	public function pre_save(){
		// This code is replicated for the API's in DotbApi::updateBean()
		if(!empty($_POST['assigned_user_id']) && $_POST['assigned_user_id'] != $this->bean->assigned_user_id && $_POST['assigned_user_id'] != $GLOBALS['current_user']->id && empty($GLOBALS['dotb_config']['exclude_notifications'][$this->bean->module_dir])){
			$this->bean->notify_on_save = true;
		}
		$GLOBALS['log']->debug("DotbController:: performing pre_save.");
        $sfh = new DotbFieldHandler();
		foreach($this->bean->field_defs as $field => $properties) {
			$type = !empty($properties['custom_type']) ? $properties['custom_type'] : $properties['type'];
		    $sf = $sfh->getDotbField(ucfirst($type), true);
    ...
    public function action_spot()
    {
        ...
        if($searchEngine instanceOf DotbSearchEngine)
        {
            $this->view = 'spot';
        }
        else
	...
	protected function post_delete(){
		$return_module = isset($_REQUEST['return_module']) ?
			...
			$GLOBALS['dotb_config']['default_module'];
		$return_action = isset($_REQUEST['return_action']) ?
			$_REQUEST['return_action'] :
			$GLOBALS['dotb_config']['default_action'];
	...
	protected function action_massupdate(){
		...
		if(!empty($_REQUEST['massupdate']) && $_REQUEST['massupdate'] == 'true' && (!empty($_REQUEST['uid']) || !empty($_REQUEST['entire']))){
            ...
            DotbAutoLoader::requireWithCustom('include/MassUpdate.php');
            $massUpdateClass = DotbAutoLoader::customClass('MassUpdate');
			...
			dotb_die("You must massupdate at least one record");
		}
	}
	/**
	...
	/**
	 * Action to handle when using a file as was done in previous versions of Dotb.
	 */
	...
	private function blockFileAccess(){
		...
		if(!empty($GLOBALS['dotb_config']['admin_access_control']) && $GLOBALS['dotb_config']['admin_access_control']){
	...
	protected function callLegacyCode()
	{
	    ...
	    if((DotbAutoLoader::existing("modules/{$this->module}/{$file}.php") &&
    	    !DotbAutoLoader::existing("modules/{$this->module}/views/view.{$action}.php")) ||
    	    (DotbAutoLoader::existing("custom/modules/{$this->module}/{$file}.php") &&
    	    !DotbAutoLoader::existing("custom/modules/{$this->module}/views/view.{$action}.php"))) {
