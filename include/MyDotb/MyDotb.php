...
class MyDotb{
    ...
    function checkDashletDisplay () {
		...
		if((!in_array($this->type, $GLOBALS['moduleList'])
		    ...
		    $displayDashlet = DotbACL::checkAccess($this->type, 'list', array("owner_override" => true));
		}
	...
	function addDashlet(){
		...
		if(!is_file(dotb_cached('dashlets/dashlets.php'))) {
	...
	function dashletsDialog(){
		...
		$dotb_smarty->assign('LBL_CLOSE_DASHLETS', $mod_strings['LBL_CLOSE_DASHLETS']);
		$dotb_smarty->assign('LBL_ADD_DASHLETS', $mod_strings['LBL_ADD_DASHLETS']);
		$dotb_smarty->assign('APP', $app_strings);
		$dotb_smarty->assign('moduleName', $this->type);

		if ($this->type == 'Home'){
			$dotb_smarty->assign('modules', $dashletsList);
			$dotb_smarty->assign('tools', $toolsList);
		}

		$dotb_smarty->assign('charts', $chartsList);

		$html = $dotb_smarty->fetch('include/MyDotb/tpls/addDashletsDialog.tpl');
	...
	function getReportCharts(){
		...
		$dotb_smarty->assign('reportCharts', $DashletsDialog->dashlets[$category]);

		$html = $dotb_smarty->fetch('include/MyDotb/tpls/retrieveReportCharts.tpl');
		$json = getJSONobj();
	...
	function searchModuleToolsDashlets($searchStr, $category){
		...
		$dotb_smarty->assign('lblSearchResults', $app_strings['LBL_SEARCH_RESULTS']);
		$dotb_smarty->assign('lblSearchCategory', $searchCategoryString);

		$dotb_smarty->assign('moduleName', $this->type);
		$dotb_smarty->assign('searchString', $searchStr);

		$dotb_smarty->assign('dashlets', $searchResult[$dashletIndex]);

		return $dotb_smarty->fetch('include/MyDotb/tpls/dashletsSearchResults.tpl');
	}
	...
	function searchChartsDashlets($searchStr){
		...
		$dotb_smarty->assign('lblSearchResults', $app_strings['LBL_SEARCH_RESULTS']);
		$dotb_smarty->assign('searchString', $searchStr);
		$dotb_smarty->assign('charts', $searchResult['Charts']);
		$dotb_smarty->assign('globalReports', $searchResult['global']);
		$dotb_smarty->assign('myTeamReports', $searchResult['myTeams']);
		$dotb_smarty->assign('mySavedReports', $searchResult['mySaved']);
		$dotb_smarty->assign('myFavoriteReports', $searchResult['myFavorites']);

		return $dotb_smarty->fetch('include/MyDotb/tpls/chartDashletsSearchResults.tpl');
	}
	...
	function addPage(){
		global $current_user, $dotb_version, $dotb_config, $dotb_flavor;
		global $app_strings;
		global $current_language;
        ...
        $newPageName = DotbCleaner::stripTags(from_html($newPageName), false);
		...
		$dotb_smarty->assign('selectedPage', sizeof($pages) - 1);
        $dotb_smarty->assign('mod',$home_mod_strings);
        $dotb_smarty->assign('app',$GLOBALS['app_strings']);
		$dotb_smarty->assign('lblAddDashlets', $home_mod_strings['LBL_ADD_DASHLETS']);
		$dotb_smarty->assign('numCols', $newPage['numColumns']);
		...
		return $dotb_smarty->fetch('include/MyDotb/tpls/retrievePage.tpl');
	}
	...
	function retrievePage(){
		global $current_user, $dotb_version, $dotb_config, $dotb_flavor, $current_language;
	    ...
	    $chartStyleCSS = DotbThemeRegistry::current()->getCSSURL('chart.css');
	    $chartColorsXML = DotbThemeRegistry::current()->getImageURL('dotbColors.xml');

	    $chartStringsXML = $dotb_config['tmp_dir'].'chart_strings.' . $current_language .'.lang.xml';

		$dotbChart = DotbChartFactory::getInstance();


        if (!file_exists($chartStringsXML)) {
            $dotbChart->generateChartStrings($chartStringsXML);
		}
		        ...
		        if(!empty($id) && isset($dashlets[$id]) && is_file($dashlets[$id]['fileLocation'])) {
					...
					if($myDashlet->checkDashletDisplay()) {
						...
						if ($dashlets[$id]['className'] == 'ChartsDashlet'){
							...
							$chartsArray[$id]['xmlFile'] = dotb_cached("xml/") . $dashlets[$id]['reportId'] . '_saved_chart.xml';
							...
							if (in_array($dashlets[$id]['className'], $predefinedChartsList)){
								$chartsArray[$id] = array();
								$chartsArray[$id]['id'] = $id;
								$chartsArray[$id]['xmlFile'] = $dotbChart->getXMLFileName($id);
								$chartsArray[$id]['width'] = '100%';
								$chartsArray[$id]['height'] = '480';
								$chartsArray[$id]['styleSheet'] = $chartStyleCSS;
								$chartsArray[$id]['colorScheme'] = $chartColorsXML;
						...
						try {
							...
							$toggleHeaderToolsetScript .= "DOTB.myDotb.attachToggleToolsetEvent('$id');";
		...
		$dotb_smarty->assign('imagePath', $GLOBALS['image_path']);
		$dotb_smarty->assign('lblLnkHelp', $GLOBALS['app_strings']['LNK_HELP']);
		$dotb_smarty->assign('mod', return_module_language($current_language, 'Home'));
        $dotb_smarty->assign('app', $GLOBALS['app_strings']);

		$dotb_smarty->assign('maxCount', empty($dotb_config['max_dashboards']) ? 15 : $dotb_config['max_dashboards']);
		$dotb_smarty->assign('dashletCount', $count);
		$dotb_smarty->assign('columns', $display);
		$dotb_smarty->assign('selectedPage', $selectedPage);
		$dotb_smarty->assign('numCols', $numCols);
		if(!empty($dotb_config['lock_homepage']) && $dotb_config['lock_homepage'] == true) $dotb_smarty->assign('lock_homepage', true);

		$htmlOutput = $dotb_smarty->fetch('include/MyDotb/tpls/retrievePage.tpl');
	...
	function savePageTitle(){
		...
		$pages[$_REQUEST['pageId']]['pageTitle'] = DotbCleaner::stripTags(from_html($newPageName), false);
		$current_user->setPreference('pages', $pages, 0, $this->type);
