...
class DashletsDialog {
    ...
    function getDashlets($category='') {
        ...
        require_once($GLOBALS['dotb_config']['cache_dir'].'dashlets/dashlets.php');
        ...
        foreach($dashletsFiles as $className => $files) {
            if (!empty($files['meta']) && file_exists($files['meta'])) {
                ...
                if (!empty($dashletMeta[$files['class']]['icon'])) {
                    ...
                    $imageName = DotbThemeRegistry::current()->getImageURL(basename($dashletMeta[$files['class']]['icon']), false);
                    if ( !empty($imageName) ) {
                        if (dotb_is_file($imageName))
				...
				elseif(!empty($dashletMeta[$files['class']]['module']) && (!in_array($dashletMeta[$files['class']]['module'], $GLOBALS['moduleList']) && !in_array($dashletMeta[$files['class']]['module'], $GLOBALS['modInvisList'])) && (!in_array('Activities', $GLOBALS['moduleList']))){
                	...
                	//check ACL ACCESS
                	if(!empty($dashletMeta[$files['class']]['module'])) {
                	    if(!DotbACL::checkAccess($dashletMeta[$files['class']]['module'], 'view', array('owner_override' => true))) {
                	        $displayDashlet = false;
                	    }
                	    if(!DotbACL::checkAccess($dashletMeta[$files['class']]['module'], 'list', array('owner_override' => true))) {
                	        $displayDashlet = false;
                	    }
                	}
                }
                ...
                if ($displayDashlet){
					$cell = array( 'title' => $title,
								   ...
								   'onclick' => 'return DOTB.myDotb.addDashlet(\'' . $className . '\', \'' . $type . '\', \''.(!empty($dashletMeta[$files['class']]['module']) ? $dashletMeta[$files['class']]['module'] : '' ) .'\');',
                                   'icon' => $icon,
