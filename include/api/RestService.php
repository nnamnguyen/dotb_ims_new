...
use Dotbcrm\Dotbcrm\Security\Subject\ApiClient\Rest as RestApiClient;
...
class RestService extends ServiceBase
{
    ...
    public function execute()
    {
        ...
        try {
            ...
            if (!$this->checkVersionSupport($this->getVersion(), $this->min_version, $this->max_version)) {
                throw new DotbApiExceptionIncorrectVersion(
            ...
            if ($route == false) {
                throw new DotbApiExceptionNoMethod('Could not find any route that accepted a path like: '.$this->request->rawPath);
            }
            ...
            if ( !isset($route['noLoginRequired']) || $route['noLoginRequired'] == false ) {
                if (!$isLoggedIn) {
                    if (!$loginException) {
                    //    $this->needLogin(); // Fix some errors - don't understand - By Lap Nguyen
                    } else {
            ...
            DotbMetric_Manager::getInstance()->setTransactionName('rest_' . $module . '_' . $route['method']);
    ...
    {
        return DotbConfig::getInstance()->get('site_url');
    }
    ...
    protected function handleException(Exception $exception)
    {
        ...
        if ( is_a($exception,"DotbApiException") ) {
        ...
        if (isset($_REQUEST['format']) && $_REQUEST['format'] == 'dotb-html-json') {
     ...
     * @throws DotbApiExceptionRequestTooLarge gets thrown on file uploads if the request failed
     */
    protected function authenticateUser()
    {
        ...
        if ( !empty($token) ) {
            try {
                $oauthServer = \DotbOAuth2Server::getOAuth2Server($platform);
                ...
                if (isset($_SESSION['authenticated_user_id'])) {
                    ...
                    if (!$valid) {
                        ...
                        $e = new DotbApiExceptionInvalidGrant();
                    }
                }
    ...
    protected function grabToken()
    {
        ...
        $allowGet = (bool) DotbConfig::getInstance()->get('allow_oauth_via_get', false);
    ...
    {
        if (isset($args['format']) && $args['format'] == 'dotb-html-json') {
        } else {
    ...
    protected function setResourceURIBase($options = array())
    {
        ...
        if (empty($this->resourceURIBase)) {
            ...
            if (isset($_REQUEST['__dotb_url']) && strpos($_SERVER['REQUEST_URI'], $_SERVER['SCRIPT_NAME']) === false) {
