...
class ServiceDictionarySoap extends ServiceDictionary {
    ...
    protected function generateWsdl( $rawFunctions, $rawTypes ) {
        ...
        $definitions->setAttributeNS('http://www.w3.org/2000/xmlns/' ,'xmlns:tns', 'http://www.dotbcrm.com/dotbcrm');
        $definitions->setAttribute('targetNamespace','http://www.dotbcrm.com/dotbcrm');
        
        // Types
        $types = $doc->createElement('wsdl:types');
        $definitions->appendChild($types);
        $schema = $doc->createElement('xsd:schema');
        $schema->setAttribute('targetNamespace','http://www.dotbcrm.com/dotbcrm');
        ...
        $portType->setAttribute('name','DotbSoapPort');
        $definitions->appendChild($portType);
        
        // Binding
        $binding = $doc->createElement('wsdl:binding');
        $binding->setAttribute('name','DotbSoapBinding');
        $binding->setAttribute('type','tns:DotbSoapPort');
        ...
        $service->setAttribute('name','DotbSoapService');
        $servicePort = $doc->createElement('wsdl:port');
        $servicePort->setAttribute('name','DotbSoapPort');
        $servicePort->setAttribute('binding','tns:DotbSoapBinding');
        $servicePortAddress = $doc->createElement('wsdlsoap:address');
        // FIXME: Change to a real address
        $servicePortAddress->setAttribute('location','http://localhost/lumia/ent/api/soap.php');
