...
abstract class DotbApi
{
    ...
    public function requireArgs(array $args, $requiredFields = array())
    {
        foreach ( $requiredFields as $fieldName ) {
            if ( !array_key_exists($fieldName, $args) ) {
                throw new DotbApiExceptionMissingParameter('Missing parameter: '.$fieldName);
            }
        }
    }
     ...
     * @param $bean DotbBean The fully loaded bean to format
     * @param $options array Formatting options
     * @return array An array version of the DotbBean with only the requested fields (also filtered by ACL)
     */
    protected function formatBean(ServiceBase $api, array $args, DotbBean $bean, array $options = array())
    {
    ...
    protected function formatBeans(ServiceBase $api, array $args, $beans, array $options = array())
    {
        ...
        foreach ($beans as $bean) {
            if (!is_subclass_of($bean, 'DotbBean')) {
                continue;
            }
     ...
     * @return DotbBean The loaded bean
     */
    protected function loadBean(ServiceBase $api, array $args, $aclToCheck = 'view', array $options = array())
    {
        ...
        if ($api->action == 'save' && ($bean == false || $bean->deleted == 1)) {
            throw new DotbApiExceptionNotAuthorized('DOTB_API_EXCEPTION_RECORD_NOT_AUTHORIZED', array('save'));
        }
        ...
        if (DotbACLStatic::fixUpActionName($aclToCheck) != 'view' && !$bean->ACLAccess(DotbACLStatic::fixUpActionName($aclToCheck), $options)) {
            throw new DotbApiExceptionNotAuthorized('DOTB_API_EXCEPTION_RECORD_NOT_AUTHORIZED',array($aclToCheck));
        }
    ...
    /**
     * Fetches data from the $args array and updates the bean with that data
     * @param $bean DotbBean The bean to be updated
     ...
     * @return id Bean id
     */
    protected function updateBean(DotbBean $bean, ServiceBase $api, array $args)
    {
     ...
     *
     * @param DotbBean $bean The bean to be populated
     ...
     */
    protected function populateBean(DotbBean $bean, ServiceBase $api, array $args)
    {
        ...
        } catch(DotbApiExceptionEditConflict $conflict) {
            $api->action = 'view';
        ...
        if ( $errors !== true ) {
            ...
            throw new DotbApiExceptionInvalidParameter('There were validation errors on the submitted data. Record was not saved.');
        }
    }
     ...
     *
     * @param DotbBean $bean The bean to be saved
     * @param ServiceBase $api
     * @param array $args API arguments
     */
    ...
    /**
     * Toggle Favorites
     * @param DotbBean $module
     * @param type $favorite
     * @return bool
     */

    protected function toggleFavorites(DotbBean $bean, $favorite)
    {
        ...
        $fav_id = DotbFavorites::generateGUID($module,$record);

        // get it even if its deleted
        $fav = BeanFactory::getBean('DotbFavorites', $fav_id, array("deleted" => false));
        ...
        elseif($favorite && empty($fav->id)) {
            $fav = BeanFactory::newBean('DotbFavorites');
            $fav->id = $fav_id;
        ...
        if($reindexBean === true) {
            $searchEngine = DotbSearchEngineFactory::getInstance(DotbSearchEngineFactory::getFTSEngineNameFromConfig());

            if($searchEngine instanceof DotbSearchEngineAbstractBase) {
     ...
     *
     * @param DotbBean $bean The bean to check on
     ...
     * @throws DotbApiExceptionNotAuthorized
     */
    protected function verifyFieldAccess(DotbBean $bean, $field, $action = 'access', $context = array()) {
        if (!$bean->ACLFieldAccess($field, $action, $context)) {
            // @TODO Localize this exception message
            throw new DotbApiExceptionNotAuthorized('Not allowed to ' . $action . ' ' . $field . ' field in ' . $bean->object_name . ' module.');
        }
    }
     ...
     *
     * @param DotbBean $bean The bean to record in the tracker table
     */
    public function trackAction(DotbBean $bean)
    {
        ...
        if ( empty($bean->id) || (isset($bean->new_with_id) && $bean->new_with_id) ) {
            ...
            // Tracking bean saves/creates happens in the DotbBean so it is always recorded
            return;
        }
     ...
     *
     * @param array $args API arguments
     * @param DotbBean $seed The bean to validate the value against.
     ...
     *
     * @throws DotbApiExceptionRequestTooLarge
     * @throws DotbApiExceptionMissingParameter
     */
    protected function checkPostRequestBody()
    {
        if (empty($_FILES)) {
            ...
            if ($contentLength && $postMaxSize && $contentLength > $postMaxSize) {
                // @TODO Localize this exception message
                throw new DotbApiExceptionRequestTooLarge('Attachment is too large');
            }

            // @TODO Localize this exception message
            throw new DotbApiExceptionMissingParameter('Attachment is missing');
        }
    }
    ...
    protected function checkPutRequestBody($length)
    {
        ...
        if ($contentLength && $length < $contentLength) {
            // @TODO Localize this exception message
            throw new DotbApiExceptionRequestTooLarge('File is too large');
        } elseif (!$length) {
            throw new DotbApiExceptionMissingParameter('File is missing or no file data was received.');
        }
    }
    ...
    {
        $maxListLimit = DotbConfig::getInstance()->get('max_list_limit');
