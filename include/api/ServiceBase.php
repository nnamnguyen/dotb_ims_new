...
use Dotbcrm\Dotbcrm\Security\Validator\Constraints\Platform as PlatformConstraint;
...
abstract class ServiceBase implements LoggerAwareInterface
{
    ...
    protected function loadApiClass($route) {
        if (!DotbAutoLoader::requireWithCustom($route['file']) ) {
            throw new DotbApiException('Missing API file.');
        }

        if ( ! class_exists($route['className']) ) {
            throw new DotbApiException('Missing API class.');
        }
    ...
    protected function loadUserEnvironment()
    {
        ...
        $current_language = $GLOBALS['dotb_config']['default_language'];
   ...
   /**
     ...
     * @throws DotbApiExceptionNeedLogin
    ...
    public function needLogin(Exception $e = null)
    {
       if($e) {
        ...
        $isIDMModeEnabled = (new Config(\DotbConfig::getInstance()))->isIDMModeEnabled();
        if ($isIDMModeEnabled && !extension_loaded('gmp')) {
            throw new DotbApiExceptionError('ERR_FOR_IDM_MODE_GMP_REQUIRED', null, 'Users');
        }
        $loginExc = new DotbApiExceptionNeedLogin($message);
     ...
     *
     *      `$dotb_config['disable_unknown_platforms'] = true`
     *
    ...
    public function validatePlatform($platform)
    {
        ...
        $strict = (bool) DotbConfig::getInstance()->get('disable_unknown_platforms', true);
        ...
        if ($raiseException) {
            throw new DotbApiExceptionInvalidParameter("EXCEPTION_INVALID_PLATFORM");
        }
    }
}
