...
class ServiceDictionaryRest extends ServiceDictionary
{
    ...
    public function lookupRoute($path, $version, $requestType, $platform)
    {
        ...
        if (!isset($this->dict[$pathLength])) {
            ...
            throw new DotbApiExceptionNoMethod('Could not find a route with '.$pathLength.' elements');
        }
        ...
        if (empty($route) && $platform != 'mobile' && isset($this->dict[$pathLength]['mobile'])) {
            $route = $this->lookupChildRoute($this->dict[$pathLength]['mobile'], $path, $version);
        }
