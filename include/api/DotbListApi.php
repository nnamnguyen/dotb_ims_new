...
abstract class DotbListApi extends DotbApi {
     ...
     * @param DotbBean|null $seed This is the seed bean that feeds the list, if you pass in a null seed then the fields are not validated
     * @return array An array with the options limit, offset, fields and order_by set
     */
    public function parseArguments(ServiceBase $api, array $args, DotbBean $seed = null)
    {
            ...
            foreach ( $orderBys as $order ) {
                ...
                if ( $seed != null ) {
                    if ( $this->checkAcls && !$seed->ACLFieldAccess($column,'list') ) {
                        throw new DotbApiExceptionNotAuthorized('No access to view field: '.$column.' in module: '.$seed->module_dir);
                    }
