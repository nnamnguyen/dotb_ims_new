...
function confirmDialog(arrayContents, formName) {
	...
	if (data['account_id'] && (newData.split("\n").length - 1) > 2)
	{
		...
		{
			if(confirm(DOTB.language.get('app_strings', 'NTC_OVERWRITE_ADDRESS_PHONE_CONFIRM') + '\n\n' + newData))
			{
				popupConfirm = 1;
			} else {
				popupConfirm = -1;	
...
function toggleMore(spanId, img_id, module, action, params){
	...
	DOTB.util.additionalDetailsRpcCall = window.setTimeout('toggle_more_go()', 250);
}

// The following line of code was copy / pasted in a whole bunch of modules.
DOTB.util.doWhen("window.document.forms['popup_query_form'] != null "
