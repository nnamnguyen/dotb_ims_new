...
function reportTypeChanged() {
	...
	if ( report_type == 'summary') {
		...
		document.getElementById('tab_link_columns_tab').innerHTML = DOTB.language.get('Reports', 'LBL_4_CHOOSE') ;
		...
		document.getElementById('tab_link_columns_tab').innerHTML = DOTB.language.get('Reports', 'LBL_3_CHOOSE') ;
...
function addFilterInput(cell,filter) {
	...
	if (qualifier_name == 'between') {
        ...
        var no = new Object();
        no['value'] = 'yes';
        no['text'] = DOTB.language.languages.app_list_strings.checkbox_dom[1];
        var yes = new Object();
        yes['value'] = 'no';
        yes['text'] = DOTB.language.languages.app_list_strings.checkbox_dom[2];
        var options = [ yes, no ];
...
function buildOuterJoinHTML(info) {
	...
	text += '<img border="0" class="inlineHelpTip" src="' + image_path +'help.gif" onclick="DOTB.util.showHelpTips(this,\''+ lbl_optional_help +'\')"/>';
