    ...
    YAHOO.util.Event.onDOMReady(function(){
        ...
        Event.on(showButtonElement, "click", function() {
            ...
            if (!dialog) {
                ...
                dialog.setHeader(DOTB.language.get('app_strings', 'LBL_MASSUPDATE_DATE'));
                var dialogBody = '<p class="callnav_today"><a href="javascript:void(0)"  id="callnav_today">' + DOTB.language.get('app_strings', 'LBL_EMAIL_DATE_TODAY') + '</a></p><div id="' + showButtonElement.id + '_div"></div>';
                ...
                Event.addListener("callnav_today", "click", function(){ 
                    ...
                    DOTB.util.callOnChangeListers(input);
            ...
            if (!calendar) {
                var navConfig = {
                    strings : {
                        month: DOTB.language.get('app_strings', 'LBL_CHOOSE_MONTH'),
                        year: DOTB.language.get('app_strings', 'LBL_ENTER_YEAR'),
                        submit: DOTB.language.get('app_strings', 'LBL_EMAIL_OK'),
                        cancel: DOTB.language.get('app_strings', 'LBL_CANCEL_BUTTON_LABEL'),
                        invalidYear: DOTB.language.get('app_strings', 'LBL_ENTER_VALID_YEAR')
                    },
                ...
                if(typeof DOTB.language.languages['app_list_strings'] != 'undefined' && DOTB.language.languages['app_list_strings']['dom_cal_month_long'] != 'undefined')
                {
                	if(DOTB.language.languages['app_list_strings']['dom_cal_month_long'].length == 13)
                	{
                	   DOTB.language.languages['app_list_strings']['dom_cal_month_long'].shift();
                	}
                	calendar.cfg.setProperty('MONTHS_LONG', DOTB.language.languages['app_list_strings']['dom_cal_month_long']);
                }

                // Configure the month and days label with localization support where defined
                if (typeof DOTB.language.languages['app_list_strings'] != 'undefined' && DOTB.language.languages['app_list_strings']['dom_cal_month_short'] != 'undefined') {
                    if (DOTB.language.languages['app_list_strings']['dom_cal_month_short'].length == 13) {
                        DOTB.language.languages['app_list_strings']['dom_cal_month_short'].shift();
                    }
                    calendar.cfg.setProperty('MONTHS_SHORT', DOTB.language.languages['app_list_strings']['dom_cal_month_short']);
                }

                if(typeof DOTB.language.languages['app_list_strings'] != 'undefined'  && typeof DOTB.language.languages['app_list_strings']['dom_cal_day_short'] != 'undefined')
                {
                	if(DOTB.language.languages['app_list_strings']['dom_cal_day_short'].length == 8)
                	{
                	   DOTB.language.languages['app_list_strings']['dom_cal_day_short'].shift();
                	}                	
                	calendar.cfg.setProperty('WEEKDAYS_SHORT', DOTB.language.languages['app_list_strings']['dom_cal_day_short']);
                }
