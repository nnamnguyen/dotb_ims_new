...
function getModuleNameFromParts(parts) {
    if (parts.length == 0) {
        return parts[0];
    }
    var moduleName = '';
...
function addModuleSelectFilter(cell,filter, row) {
    ...
    option_info['text'] = '<b>'+module_defs[current_module].label+'</b>'; //Add Bold label - HP
    option_info['selected'] = selected;
    options[options.length] = option_info;

    var parts = filter.column_name.split(':');
    var selected_link_name = getModuleNameFromParts(parts);
    ...
    for(var i in link_defs) {
        ...
        option_info['text'] = '<b>'+label+'</b>'; //Add Bold label - HP
        option_info['selected'] = selected;
        options[options.length] = option_info;
        j = j + 1;
        if (selected) {
...
function addFilterInput(cell,filter) {
    ...
    //END: By Lap Nguyen
        ...
        if ( qualifier_name == 'is' || qualifier_name =='is_not') {
            ...
            ajax_get_option_array(table_key, field.name,row ,filter);  //Add filter By Lap Nguyen
        } else {
            addFilterInputText(row,filter);
        }
...
function addFilterInputDate(row,filter) {
    ...
    var imgSrcAttr = 'index.php?entryPoint=getImage&themeName=' + DOTB.themes.theme_name + '&imageName=jscalendar.gif';
...
function newSelectSpanElement(name, inputTime){
    ...
    // Fix core: remove time picker in select is between - By HP
    var text = '';
    if(dname != 'namestart' && dname != 'nameend') {
        text +=  '<select tabindex="0" size="1" id="'+dname+'_hours" onchange="timeValueUpdate(\''+dname+'\');">';
        for(i=1; i <= 12; i++) {
            val = i < 10 ? "0" + i : i;
...
function addFilterInputDatetimecombo(row, filter) {
    ...
    var imgSrcTheme = DOTB.themes.theme_name;
...
function addFilterInputSelectMultiple(row,options,filter) {
    ...
    filter_row.input_field1 = null;
    //Add By Lap Nguyen
    $(cell).find('select').select2({ width: '350px' });
    //END
}
...
function addFilterInputDatetimesBetween(row,filter) {
    ...
    // Add by HP to hide time picker
    div1.appendChild(this.newSelectSpanElement('namestart', '12:00am'));
    ...
    // Add by HP to hide time picker
    div3.appendChild(this.newSelectSpanElement('nameend', '11:45pm'));
...
function addFilterInputRelate(row,field,filter,isCustom) {
    ...
    var postData = '&module=Reports&action=get_quicksearch_defaults&to_pdf=true&dotb_body_only=true&parent_form=EditView&parent_module='+ module_name+'&parent_field='+sqs_field_name;
...
function buildSelectHTML(info, showHidden, id,onBlur) {
    ...
    for(i=0; i < info['options'].length; i++) {
        ...
        // Fix label Error 1 - Lap Nguyen
        if(option['text'] != null && option['text'] != ''){
            //Dictionary
            option['text'] = option['text'].replace("LBL_CONTACTS_J_PAYMENT_1_FROM_J_PAYMENT_TITLE", "Student");
            option['text'] = option['text'].replace("LBL_J_CLASS_J_FEEDBACK_1_FROM_J_FEEDBACK_TITLE", "Class");
            //Xu ly tay
            var n_err1 = option['text'].indexOf("LBL_");
            if(n_err1 !== -1){
                var module_err =  option['value'].substring(0,option['value'].indexOf(":"));

                var new_label = DOTB.language.get(module_err, option['text'].substring(n_err1));
                option['text'] = option['text'].substring(0,n_err1);
                if(new_label != 'undefined')
                    option['text'] += new_label;
                // Fix Label 2
                var n_err2 = option['text'].indexOf(module_err);
                if(n_err2 >= 0 && typeof module_err !== "undefined"){
                    option['text'] = option['text'].replace(module_err, DOTB.language.get(module_err, 'LBL_MODULE_NAME'));
                }
            }
            var n_err3 = option['text'].indexOf("C_");
            var n_err4 = option['text'].indexOf("J_");
            if( n_err3 !== -1 || n_err4 !== -1){
                var n_space      = option['text'].indexOf(" ");
                var n_err        = (n_err3 >= 0) ? n_err3 : n_err4;
                var module_err   = option['text'].substring(n_err, n_space);
                var module_label = module_defs[module_err].label;
                if(module_label != 'undefined')
                    option['text'] = option['text'].replace(module_err, module_label);
            }
        }
...
function showDuplicateOverlib(el,reportType, canCovertToMatrix) {
    if (reportType == 'tabular') {
        ...
        'document.EditView.submit();\' href=\'#\'>' + DOTB.language.get('Reports', 'LBL_DUPLICATE_AS_ORIGINAL') + '</a>' +
        '<a style=\'width: 150px\' class=\'menuItem\' onmouseover=\'hiliteItem(this,"yes");\' ' +
        'onmouseout=\'unhiliteItem(this);\' ' +
        'onclick=\'document.EditView.to_pdf.value=\"\";document.EditView.to_csv.value=\"\";document.EditView.action.value=\"ReportsWizard\";document.EditView.save_as.value=\"true\";' +
        'document.EditView.save_as_report_type.value=\"summation\";document.EditView.submit();\' href=\'#\'>' + DOTB.language.get('Reports', 'LBL_DUPLICATE_AS_SUMMATON') + '</a>' +
        '<a style=\'width: 150px\' class=\'menuItem\' onmouseover=\'hiliteItem(this,"yes");\' ' +
        'onmouseout=\'unhiliteItem(this);\' ' +
        'onclick=\'document.EditView.to_pdf.value=\"\";document.EditView.to_csv.value=\"\";document.EditView.action.value=\"ReportsWizard\";document.EditView.save_as.value=\"true\";' +
        'document.EditView.save_as_report_type.value=\"summation_with_details\";document.EditView.submit();\' href=\'#\'>' + DOTB.language.get('Reports', 'LBL_DUPLICATE_AS_SUMMATION_DETAILS') + '</a>' +
        '<a style=\'width: 150px\' class=\'menuItem\' onmouseover=\'hiliteItem(this,"yes");\' ' +
        'onmouseout=\'unhiliteItem(this);\' ' +
        'onclick=\'document.EditView.to_pdf.value=\"\";document.EditView.to_csv.value=\"\";document.EditView.action.value=\"ReportsWizard\";document.EditView.save_as.value=\"true\";' +
        'document.EditView.save_as_report_type.value=\"matrix\";document.EditView.submit();\' href=\'#\'>' + DOTB.language.get('Reports', 'LBL_DUPLICATE_AS_MATRIX') + '</a>';
    }
        ...
        'onmouseout=\'unhiliteItem(this);\' ' +
        'onclick=\'document.EditView.to_pdf.value=\"\";document.EditView.to_csv.value=\"\";document.EditView.action.value=\"ReportsWizard\";document.EditView.save_as.value=\"true\";' +
        'document.EditView.save_as_report_type.value=\"tabular\";document.EditView.submit();\' href=\'#\'>' + DOTB.language.get('Reports', 'LBL_DUPLICATE_AS_ROWS_AND_COLS') + '</a>';
