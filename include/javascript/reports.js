...
DOTB.reports = function () {
    ...
    var lbl_and = DOTB.language.get("Reports", 'LBL_AND');
    var lbl_select = DOTB.language.get("Reports", 'LBL_SELECT');
    var lbl_remove = DOTB.language.get("Reports", 'LBL_REMOVE');
    var lbl_missing_fields = DOTB.language.get("Reports", 'LBL_MISSING_FIELDS');
    var lbl_at_least_one_display_column = DOTB.language.get("Reports", 'LBL_AT_LEAST_ONE_DISPLAY_COLUMN');
    var lbl_at_least_one_summary_column = DOTB.language.get("Reports", 'LBL_AT_LEAST_ONE_SUMMARY_COLUMN');
    var lbl_missing_input_value = DOTB.language.get("Reports", 'LBL_MISSING_INPUT_VALUE');
    var lbl_missing_second_input_value = DOTB.language.get("Reports", 'LBL_MISSING_SECOND_INPUT_VALUE');
    var lbl_nothing_was_selected = DOTB.language.get("Reports", 'LBL_NOTHING_WAS_SELECTED');
    var lbl_none = DOTB.language.get("Reports", 'LBL_NONE');
    var lbl_outer_join_checkbox = DOTB.language.get("Reports", 'LBL_OUTER_JOIN_CHECKBOX');
    var lbl_add_related = DOTB.language.get("Reports", 'LBL_ADD_RELATE');
    var lbl_del_this = DOTB.language.get("Reports", 'LBL_DEL_THIS');
    var lbl_alert_cant_add = DOTB.language.get("Reports", 'LBL_ALERT_CANT_ADD');
    var lbl_related_table_blank = DOTB.language.get("Reports", 'LBL_RELATED_TABLE_BLANK');
    var lbl_optional_help = DOTB.language.get("Reports", 'LBL_OPTIONAL_HELP');
    ...
    return {
        ...
        init: function (img, report_def_str, user_array, origImgPath) {
            ...
            if (report_def_str == "") {
                ...
                DOTB.reports.showWizardNamedStep('report_type');
            } else {
                current_module = report_def_str.module;
                full_table_list = report_def_str.full_table_list;
                DOTB.reports.buildTree(current_module);
                DOTB.reports.populateFieldGrid(current_module, "", "");
                report_type = report_def_str.report_type;
                DOTB.FiltersWidget.init(imgPath);
                ...
                if (report_def_str.filters_def.Filter_1) {
                    DOTB.reports.loadFilters(report_def_str.filters_def.Filter_1);
                    initInProgress = 1;
                    DOTB.reports.saveFilters();
                    initInProgress = 0;
                } else
                    DOTB.FiltersWidget.addGroupToPanel('filter_designer_div', DOTB.language.get('Reports', 'LBL_FILTER'));
                DOTB.reports.showWizardNamedStep('report_type');
                DOTB.reports.showWizardNamedStep('module_select');
                DOTB.reports.showWizardNamedStep('filters');
                numerical_chart_column = document.ReportsWizardForm.numerical_chart_column;
                ...
                if (report_type == 'summation' || report_type == 'summation_with_details') {
                    DOTB.reports.showWizardNamedStep('group_by');
                    DOTB.reports.showWizardNamedStep('display_summaries');
                }
                if (report_def_str.group_defs) {
                    ...
                    for (i = 0; i < report_def_str.group_defs.length; i++) {
                        ...
                        DOTB.reports.addFieldToGroupByOnLoad(report_def_str.group_defs[i], report_def_str.summary_order_by);
                    }
                }
                ...
                if (report_def_str.summary_columns) {
                    ...
                    for (i = 0; i < report_def_str.summary_columns.length; i++) {
                        ...
                        if (!alreadyAdded)
                            DOTB.reports.addFieldToDisplaySummariesOnLoad(report_def_str.summary_columns[i], null, report_def_str.summary_order_by);
                ...
                if (report_def_str.display_columns) {
                    ...
                    DOTB.reports.showWizardNamedStep('display');
                    for (i = 0; i < report_def_str.display_columns.length; i++)
                        DOTB.reports.addFieldToDisplayColumnsOnLoad(report_def_str.display_columns[i], report_def_str.order_by);
                ...
                if (report_def_str.chart_type) {
                    ...
                    DOTB.reports.showWizardNamedStep('chart_options');
                    chart_type = report_def_str.chart_type;
                    chart_description = report_def_str.chart_description;

                }
                DOTB.reports.showWizardNamedStep('report_details');
                if (!document.getElementById('resultsDiv')) {
                    if (document.ReportsWizardForm.current_step.value == "")
                        DOTB.reports.showWizardNamedStep('filters');
                    else
                        DOTB.reports.showWizardNamedStep(document.ReportsWizardForm.current_step.value);
                }
            }
        },
        ...
        loadFilters: function (filters, parentId) {
            ...
            panels = DOTB.FiltersWidget.getPanels();
            if (panels.length > 0) {
                DOTB.FiltersWidget.addGroupToPanel(parentId + "_body_div", parentId, operator);
            } else {
                DOTB.FiltersWidget.addGroupToPanel('filter_designer_div', DOTB.language.get('Reports', 'LBL_FILTER'), operator);
            }
            ...
            for (var i in filters) {
                ...
                if (current_filter.operator) {
                    DOTB.reports.loadFilters(current_filter, id);
                } else {
                    DOTB.reports.addFilterOnLoad(current_filter, id + "_table");
                }
            }
        },
        ...
        getListFieldDef: function (field, link, module) {
            if (field == 'team_set_id') {
                link = DOTB.reports.getTeamSetIdTableValue(link, current_module, false);
        ...
        saveDisplayColumns: function () {
            ...
            for (var i = 1; i < table.rows.length; i++) {
                ...
                var field_def = DOTB.reports.getListFieldDef(field, link, module);
                ...
                if (field == 'team_set_id') {
                    field_def.table_key = DOTB.reports.getTeamSetIdTableValue(field_def.table_key, current_module, true);
                }
        ...
        saveGroupBy: function () {
            ...
            if (table.rows.length == 2 && isMatrix) {
                alert(DOTB.language.get('Reports', 'LBL_MINIMUM_2_GROUP_BY'));
                return false;
            }
            ...
            for (var i = 1; i < table.rows.length; i++) {
                ...
                var group_by_def = DOTB.reports.getListFieldDef(field, link, module);

                group_by_def.table_key = cells[0].getElementsByTagName('input')[1].value;
                if (field == 'team_set_id') {
                    group_by_def.table_key = DOTB.reports.getTeamSetIdTableValue(group_by_def.table_key, current_module, true);
                }
        ...
        saveFilter: function (filterRow) {
            ...
            if (filter_def.name == 'team_set_id') {
                ...
                filter_def.table_key = DOTB.reports.getTeamSetIdTableValue(filter_def.table_key, current_module, false);
                ...
                field['vname'] = DOTB.language.get('app_strings', 'LBL_TEAMS');
        ...
        saveFilters: function () {
            ...
            var panels = DOTB.FiltersWidget.getPanels();
            if (document.getElementById('inlineFiltersHelpTable'))
                document.getElementById(DOTB.language.get('Reports', 'LBL_FILTER') + ".1_body_div").innerHTML = "";
                    ...
                    if (field.name == 'team_set_id') {
                        ...
                        for (l = 0; l < input_arr.length; l++) {
                            if (input_arr[l].type == 'hidden' && /^id_/.test(input_arr[l].id) && trim(input_arr[l].value) != '') {
                                ...
                                //See DotbWidgetFieldteam_set_id.php file to see how this creates the primary team query portion.
                        ...
                        }                                                                                             //This part edited by Lap Nguyen - Handle select2
                    } else if (typeof (input_arr[0]) != 'undefined' && input_arr[0].id.search(/runtime_filter/) == -1 && input_arr[0].className.search(/select2/) == -1 ) {
                        filter_def.input_name0 = input_arr[0].value;
        ...
        showWizardStepModuleSelect: function () {
            ...
            if (currWizardStep < 2) {
                ...
                if (report_type == 'tabular') {
                    ...
                    DOTB.language.get('Reports', 'LBL_ROWS_AND_COLUMNS_REPORT') + ' : ' +
                    '<a href="javascript:DOTB.reports.showWizardStep(0,\'module_select\');" id="report_type_outline" class="btn-arrow-right-success">' + DOTB.language.get('Reports', 'LBL_SELECT_MODULE') + '</a> ' +
                    '<span class="btn-arrow-right">' + DOTB.language.get('Reports', 'LBL_DEFINE_FILTERS') + '</span>' +
                    '<span class="btn-arrow-right">' + DOTB.language.get('Reports', 'LBL_CHOOSE_DISPLAY_COLS') + '</span>' +
                    '<span class="btn-arrow-right">' + DOTB.language.get('Reports', 'LBL_REPORT_DETAILS') + '</span>' + '<br/><br/>';
                } else if (report_type == 'summation') {
                    var reportTypeLabel = DOTB.language.get('Reports', 'LBL_SUMMATION_REPORT');
                    if (isMatrix)
                        reportTypeLabel = DOTB.language.get('Reports', 'LBL_MATRIX_REPORT');
                    document.getElementById('wizard_outline_div').innerHTML =
                    reportTypeLabel + ' : ' +
                    '<a href="javascript:DOTB.reports.showWizardStep(0,\'module_select\');" id="report_type_outline" class="btn-arrow-right-success">' + DOTB.language.get('Reports', 'LBL_SELECT_MODULE') + '</a> ' +
                    '<span class="btn-arrow-right">' + DOTB.language.get('Reports', 'LBL_DEFINE_FILTERS') + '</span>' +
                    '<span class="btn-arrow-right">' + DOTB.language.get('Reports', 'LBL_SELECT_GROUP_BY') + '</span>' +
                    '<span class="btn-arrow-right">' + DOTB.language.get('Reports', 'LBL_DISPLAY_SUMMARIES') + '</span>' +
                    '<span class="btn-arrow-right">' + DOTB.language.get('Reports', 'LBL_CHART_OPTIONS') + '</span>' +
                    '<span class="btn-arrow-right">' + DOTB.language.get('Reports', 'LBL_REPORT_DETAILS') + '</span>' + '<br/><br/>';
                } else if (report_type == 'summation_with_details') {
                    document.getElementById('wizard_outline_div').innerHTML =
                    DOTB.language.get('Reports', 'LBL_SUMMATION_REPORT_WITH_DETAILS') + ' : ' +
                    '<a href="javascript:DOTB.reports.showWizardStep(0,\'module_select\');" id="report_type_outline" class="btn-arrow-right-success">' + DOTB.language.get('Reports', 'LBL_SELECT_MODULE') + '</a> ' +
                    '<span class="btn-arrow-right">' + DOTB.language.get('Reports', 'LBL_DEFINE_FILTERS') + '</span>' +
                    '<span class="btn-arrow-right">' + DOTB.language.get('Reports', 'LBL_SELECT_GROUP_BY') + '</span>' +
                    '<span class="btn-arrow-right">' + DOTB.language.get('Reports', 'LBL_DISPLAY_SUMMARIES') + '</span>' +
            ...
            var oldCurrLink = '<a href="javascript:DOTB.reports.showWizardStep(0,\'module_select\');" id="report_type_outline" class="btn-arrow-right-success">' + DOTB.language.get('Reports', 'LBL_SELECT_MODULE') + '</a>';
        ...
        showWizardStepFilters: function () {
            ...
            if (currWizardStep < 3) {
                ...
                if (report_type == 'tabular') {
                    ...
                    DOTB.language.get('Reports', 'LBL_ROWS_AND_COLUMNS_REPORT') + ' : ' +
                    '<a href="javascript:DOTB.reports.showWizardStep(0,\'module_select\');" id="report_type_outline" class="btn-arrow-right-success">' + DOTB.language.get('Reports', 'LBL_SELECT_MODULE') + '</a> ' +
                    '<a href="javascript:DOTB.reports.showWizardStep(0,\'filters\');" id="report_type_outline" class="btn-arrow-right-success">' + DOTB.language.get('Reports', 'LBL_DEFINE_FILTERS') + '</a> ' +
                    '<span class="btn-arrow-right">' + DOTB.language.get('Reports', 'LBL_CHOOSE_DISPLAY_COLS') + '</span>' +
                    '<span class="btn-arrow-right">' + DOTB.language.get('Reports', 'LBL_REPORT_DETAILS') + '</span>' + '<br/><br/>';
                } else if (report_type == 'summation') {
                    var reportTypeLabel = DOTB.language.get('Reports', 'LBL_SUMMATION_REPORT');
            ...
            document.getElementById('wizard_outline_div').innerHTML = document.getElementById('wizard_outline_div').innerHTML.replace(/<\/b>/gi, '');
            var currentOutline = document.getElementById('wizard_outline_div').innerHTML;
            var oldCurrLink = '<a href="javascript:DOTB.reports.showWizardStep(0,\'filters\');" id="report_type_outline" class="btn-arrow-right-success">' + DOTB.language.get('Reports', 'LBL_DEFINE_FILTERS') + '</a>';
            var currLink = oldCurrLink.replace(/<a href/i, '<b><a href');
            currLink = currLink.replace(/<\/a>/i, '</b></a>');
            document.getElementById('wizard_outline_div').innerHTML = currentOutline.replace(oldCurrLink, currLink);
            DOTB.reports.populateFieldGrid(current_module, "", "");
        ...
        showWizardStepDisplayColumns: function () {
            ...
            if (report_type == 'tabular' && currWizardStep < 4) {
                ...
                '<a href="javascript:DOTB.reports.showWizardStep(0,\'module_select\');" id="report_type_outline" class="btn-arrow-right-success">' + DOTB.language.get('Reports', 'LBL_SELECT_MODULE') + '</a> ' +
                '<a href="javascript:DOTB.reports.showWizardStep(0,\'filters\');" id="report_type_outline" class="btn-arrow-right-success">' + DOTB.language.get('Reports', 'LBL_DEFINE_FILTERS') + '</a> ' +
                '<a href="javascript:DOTB.reports.showWizardStep(0,\'display\');" id="report_type_outline" class="btn-arrow-right-success">' + DOTB.language.get('Reports', 'LBL_CHOOSE_DISPLAY_COLS') + '</a> ' +
                '<span class="btn-arrow-right">' + DOTB.language.get('Reports', 'LBL_REPORT_DETAILS') + '</span><br/><br/>';
                currWizardStep++;
                DOTB.reports.showDisplayColumnsPanel();
            } else if (report_type == 'summation_with_details' && currWizardStep < 6) {
                document.getElementById('wizard_outline_div').innerHTML =
                DOTB.language.get('Reports', 'LBL_SUMMATION_REPORT_WITH_DETAILS') + ' : ' +
                '<a href="javascript:DOTB.reports.showWizardStep(0,\'module_select\');" id="report_type_outline" class="btn-arrow-right-success">' + DOTB.language.get('Reports', 'LBL_SELECT_MODULE') + '</a> ' +
                '<a href="javascript:DOTB.reports.showWizardStep(0,\'filters\');" id="report_type_outline" class="btn-arrow-right-success">' + DOTB.language.get('Reports', 'LBL_DEFINE_FILTERS') + '</a> > ' +
                '<a href="javascript:DOTB.reports.showWizardStep(0,\'group_by\');" id="report_type_outline" class="btn-arrow-right-success">' + DOTB.language.get('Reports', 'LBL_SELECT_GROUP_BY') + '</a> > ' +
                '<a href="javascript:DOTB.reports.showWizardStep(0,\'display_summaries\');" id="report_type_outline" class="btn-arrow-right-success">' + DOTB.language.get('Reports', 'LBL_DISPLAY_SUMMARIES') + '</a> ' +
                '<a href="javascript:DOTB.reports.showWizardStep(0,\'display\');" id="report_type_outline" class="btn-arrow-right-success">' + DOTB.language.get('Reports', 'LBL_CHOOSE_DISPLAY_COLS') + '</a> ' +
                '<span class="btn-arrow-right">' +DOTB.language.get('Reports', 'LBL_CHART_OPTIONS')  + '</span>'+
                '<span class="btn-arrow-right">' +DOTB.language.get('Reports', 'LBL_REPORT_DETAILS') + '</span><br/><br/>';
                currWizardStep++;
            ...
            document.getElementById('wizard_outline_div').innerHTML = document.getElementById('wizard_outline_div').innerHTML.replace(/<\/b>/gi, '');
            var currentOutline = document.getElementById('wizard_outline_div').innerHTML;
            var oldCurrLink = '<a href="javascript:DOTB.reports.showWizardStep(0,\'display\');" id="report_type_outline" class="btn-arrow-right-success">' + DOTB.language.get('Reports', 'LBL_CHOOSE_DISPLAY_COLS') + '</a>';
            var currLink = oldCurrLink.replace(/<a href/i, '<b><a href');
            currLink = currLink.replace(/<\/a>/i, '</b></a>');
            document.getElementById('wizard_outline_div').innerHTML = currentOutline.replace(oldCurrLink, currLink);
            DOTB.reports.populateFieldGrid(current_module, "", "");
        ...
        showWizardStepReportDetails: function () {
            ...
            if (report_type == 'tabular') {
                ...
                '<a href="javascript:DOTB.reports.showWizardStep(0,\'filters\');" id="report_type_outline" class="btn-arrow-right-success">' + DOTB.language.get('Reports', 'LBL_DEFINE_FILTERS') + '</a> ' +
                '<a href="javascript:DOTB.reports.showWizardStep(0,\'display\');" id="report_type_outline" class="btn-arrow-right-success">' + DOTB.language.get('Reports', 'LBL_CHOOSE_DISPLAY_COLS') + '</a> ' +
                '<b><a href="javascript:DOTB.reports.showWizardStep(0,\'report_details\');" id="report_type_outline" class="btn-arrow-right-success">' + DOTB.language.get('Reports', 'LBL_REPORT_DETAILS') + '</a></b><br/><br/>';
            } else if (report_type == 'summation') {
                var reportTypeLabel = DOTB.language.get('Reports', 'LBL_SUMMATION_REPORT');
                if (isMatrix)
                    reportTypeLabel = DOTB.language.get('Reports', 'LBL_MATRIX_REPORT');
                ...
                '<a href="javascript:DOTB.reports.showWizardStep(0,\'group_by\');" id="report_type_outline" class="btn-arrow-right-success">' + DOTB.language.get('Reports', 'LBL_SELECT_GROUP_BY') + '</a> ' +
                '<a href="javascript:DOTB.reports.showWizardStep(0,\'display_summaries\');" id="report_type_outline" class="btn-arrow-right-success">' + DOTB.language.get('Reports', 'LBL_DISPLAY_SUMMARIES') + '</a> ' +
                '<a href="javascript:DOTB.reports.showWizardStep(0,\'chart_options\');" id="report_type_outline" class="btn-arrow-right-success">' + DOTB.language.get('Reports', 'LBL_CHART_OPTIONS') + '</a> ' +
                '<b><a href="javascript:DOTB.reports.showWizardStep(0,\'report_details\');" id="report_type_outline" class="btn-arrow-right-success">' + DOTB.language.get('Reports', 'LBL_REPORT_DETAILS') + '</a></b><br/><br/>';
            ...
            DOTB.reports.addOptionalSelect();
        },
        showWizardStepGroupBy: function () {
            ...
            if (report_type == 'summation' && currWizardStep < 4) {
                ...
                '<span class="btn-arrow-right">'+DOTB.language.get('Reports', 'LBL_REPORT_DETAILS') + '</span><br/><br/>';
                currWizardStep++;
                DOTB.reports.showGroupByColumns();
                DOTB.reports.showDisplaySummaries();
                document.getElementById("display_summaries_div").style.display = "none";
            ...
            document.getElementById('wizard_outline_div').innerHTML = document.getElementById('wizard_outline_div').innerHTML.replace(/<\/b>/gi, '');
            var currentOutline = document.getElementById('wizard_outline_div').innerHTML;
            var oldCurrLink = '<a href="javascript:DOTB.reports.showWizardStep(0,\'group_by\');" id="report_type_outline" class="btn-arrow-right-success">' + DOTB.language.get('Reports', 'LBL_SELECT_GROUP_BY') + '</a>';
            var currLink = oldCurrLink.replace(/<a href/i, '<b><a href');
            currLink = currLink.replace(/<\/a>/i, '</b></a>');
            document.getElementById('wizard_outline_div').innerHTML = currentOutline.replace(oldCurrLink, currLink);
            DOTB.reports.populateFieldGrid(current_module, "", "");
        ...
        showWizardStepDisplaySummaries: function () {
            ...
            document.getElementById('wizard_outline_div').innerHTML = document.getElementById('wizard_outline_div').innerHTML.replace(/<\/b>/gi, '');
            var currentOutline = document.getElementById('wizard_outline_div').innerHTML;
            var oldCurrLink = '<a href="javascript:DOTB.reports.showWizardStep(0,\'display_summaries\');" id="report_type_outline" class="btn-arrow-right-success">' + DOTB.language.get('Reports', 'LBL_DISPLAY_SUMMARIES') + '</a>';
            var currLink = oldCurrLink.replace(/<a href/i, '<b><a href');
            currLink = currLink.replace(/<\/a>/i, '</b></a>');
            document.getElementById('wizard_outline_div').innerHTML = currentOutline.replace(oldCurrLink, currLink);
            DOTB.reports.populateFieldGrid(current_module, "", "");
        ...
        showWizardStepChartOptions: function () {
            ...
            document.getElementById('wizard_outline_div').innerHTML = document.getElementById('wizard_outline_div').innerHTML.replace(/<\/b>/gi, '');
            var currentOutline = document.getElementById('wizard_outline_div').innerHTML;
            var oldCurrLink = '<a href="javascript:DOTB.reports.showWizardStep(0,\'chart_options\');" id="report_type_outline" class="btn-arrow-right-success">' + DOTB.language.get('Reports', 'LBL_CHART_OPTIONS') + '</a>';
            var currLink = oldCurrLink.replace(/<a href/i, '<b><a href');
            currLink = currLink.replace(/<\/a>/i, '</b></a>');
            document.getElementById('wizard_outline_div').innerHTML = currentOutline.replace(oldCurrLink, currLink);
        ...
        saveChartOptions: function () {
            ...
            DOTB.reports.setNumericalChartColumnType();
        },
        saveCurrentStep: function () {
            switch (currEditorDiv) {
                case "filters":
                    if (!DOTB.reports.saveFilters())
                        return false;
                    break;
                case "group_by":
                    if (!DOTB.reports.saveGroupBy())
                        return false;
                    // For the case when you come back to a report and delete all the group-bys.
                    if (group_defs.length == 0 && summary_columns.length > 0 && !DOTB.reports.saveDisplaySummaries())
                        return false;
                    if (group_defs.length > 0 && !DOTB.reports.saveDisplaySummaries())
                        return false;
                    break;
                case "display_summaries":
                    if (!DOTB.reports.saveDisplaySummaries())
                        return false;
                    break;
                case "display":
                    if (!DOTB.reports.saveDisplayColumns())
                        return false;
                    break;
                case "chart_options":
                    DOTB.reports.saveChartOptions();
                    break;
                case "report_details":
                    break;
            }
        ...
        showWizardNamedStep: function (stepName) {
            switch (stepName) {
                case "report_type":
                    DOTB.reports.showWizardStepReportType();
                    break;
                case "module_select":
                    DOTB.reports.showWizardStepModuleSelect();
                    break;
                case "filters":
                    DOTB.reports.showWizardStepFilters();
                    break;
                case "display":
                    DOTB.reports.showWizardStepDisplayColumns();
                    break;
                case "report_details":
                    DOTB.reports.showWizardStepReportDetails()
                    break;
                case "group_by":
                    DOTB.reports.showWizardStepGroupBy();
                    break;
                case "display_summaries":
                    DOTB.reports.showWizardStepDisplaySummaries();
                    break;
                case "chart_options":
                    DOTB.reports.showWizardStepChartOptions();
            }
        },
        showWizardStepTabular: function (isPrev, stepName) {
            if (stepName) {
                return DOTB.reports.showWizardNamedStep(stepName);
            }
            if (isPrev) {
                switch (currEditorDiv) {
                    case "":
        ...
        showWizardStep: function (isPrev, stepName) {
            var isValid = DOTB.reports.saveCurrentStep();
            if (!isValid && !stepName)
                return false;
            if (isValid && report_type == 'tabular')
                DOTB.reports.showWizardStepTabular(isPrev, stepName);
            else if (isValid && report_type == 'summation')
                DOTB.reports.showWizardStepSummation(isPrev, stepName);
                else if (isValid && report_type == 'summation_with_details')
                    DOTB.reports.showWizardStepSummationWithDetails(isPrev, stepName);
        },
        prepareReportForProcessing: function () {
            ...
            if (display_cols.length == 0 && report_type != 'summation') {
                alert(DOTB.language.get('Reports', 'LBL_AT_LEAST_ONE_DISPLAY_COLUMN'));
                return false;
            }
            if (summary_columns.length == 0 && report_type != 'tabular') {
                alert(DOTB.language.get('Reports', 'LBL_AT_LEAST_ONE_SUMMARY_COLUMN'));
                return false;
            }
            ...
            if (group_defs.length > 3 && isMatrix) {
                alert(DOTB.language.get('Reports', 'LBL_MAXIMUM_3_GROUP_BY'));
                return false;
            }
            ...
            if (got_sort == 0) {
                ...
                var sort_by_elem = DOTB.reports.getFieldDef(document.ReportsWizardForm.sort_by.value);
                sort_by_elem.table_key = DOTB.reports.getLinkedFieldName(document.ReportsWizardForm.sort_by.value);
            ...
            if (got_summary_sort == 0) {
                ...
                var summaryFieldObj = DOTB.reports.getLinkedFieldSummaryName(document.ReportsWizardForm.summary_sort_by.value);
                ...
                if (typeof (DOTB.reports.getListFieldDef(summaryFieldName, summaryFieldLink, summaryFieldModule).group_function) != 'undefined' && summaryFieldName != 'count') {
                    summary_sort_by_elem.name = key_arr[key_arr.length - 2];
                    summary_sort_by_elem.group_function = DOTB.reports.getListFieldDef(summaryFieldName, summaryFieldLink, summaryFieldModule).group_function;
                    summary_sort_by_elem.column_function = key_arr[2];
                } else if (typeof (DOTB.reports.getListFieldDef(summaryFieldName, summaryFieldLink, summaryFieldModule).column_function) != 'undefined') {
                    summary_sort_by_elem.name = key_arr[key_arr.length - 2];
                    summary_sort_by_elem.group_function = DOTB.reports.getListFieldDef(summaryFieldName, summaryFieldLink, summaryFieldModule).column_function;
                    summary_sort_by_elem.column_function = key_arr[2];
                } // else if
                else {
                    summary_sort_by_elem = DOTB.reports.getFieldDef(document.ReportsWizardForm.summary_sort_by.value);
                }
        ...
        checkReportDetails: function () {
            if (trim(document.ReportsWizardForm.save_report_as.value) == "") {
                alert(DOTB.language.get('Reports', 'LBL_REPORT_NAME') + DOTB.language.get('Reports', 'LBL_CANNOT_BE_EMPTY'));
                return false;
            }
            if (trim(document.ReportsWizardForm.assigned_user_name.value) == "") {
                alert(DOTB.language.get('Reports', 'LBL_OWNER') + DOTB.language.get('Reports', 'LBL_CANNOT_BE_EMPTY'));
                return false;
            }
            if (!is_primary_team_selected('ReportsWizardForm', 'team_name')) {
                alert(DOTB.language.get('app_strings', 'ERR_NO_PRIMARY_TEAM_SPECIFIED'));
                return false;
            }
        ...
        saveReport: function () {
            if (!DOTB.reports.saveCurrentStep())
                return false;
            if (DOTB.reports.checkReportDetails()) {
                document.ReportsWizardForm.save_report.value = 'on';
                document.ReportsWizardForm.current_step.value = currEditorDiv;
                if (DOTB.reports.prepareReportForProcessing()) {
        ...
        deleteReport: function () {
            if (confirm(DOTB.language.get('app_strings', 'NTC_DELETE_CONFIRMATION'))) {
        ...
        */
        ...
        orderBySelected: function (rowNum, sortDir) {
            ...
            directionSpan.innerHTML = '<select id="orderByDirection"><option value="a">' + DOTB.language.get('Reports', 'LBL_ASCENDING') + '</option>' +
            '<option value="d">' + DOTB.language.get('Reports', 'LBL_DESCENDING') + '</option></select>';
        ...
        summaryOrderBySelected: function (rowNum, sortDir) {
            ...
            + DOTB.language.get('Reports', 'LBL_ASCENDING')
            + '</option>'
            + '<option value="d">'
            + DOTB.language.get('Reports', 'LBL_DESCENDING')
            + '</option></select>';
            if (typeof (sortDir) != 'undefined') {
                if (sortDir == 'a')
        ...
        addFieldToGroupBy: function (e) {
            ...
            cell.setAttribute('scope', 'row');
            cell.innerHTML = "&nbsp;&nbsp;<img onclick='DOTB.reports.deleteGroupBy(\"group_by_row_" + totalGroupByRows + "\")' src='index.php?entryPoint=getImage&themeName=" + DOTB.themes.theme_name + "&imageName=delete_inline.gif' alt='" + DOTB.language.get("Reports", "LBL_REMOVE") + "'>";
            DOTB.reports.addToFullTableList('group_by_row_' + totalGroupByRows, fieldGridCell.getRecord(fieldGridCell.getSelectedRows()[0]).getData('parents'));
            var dd11 = YAHOO.util.Dom.get(id);
            dd11.dd = new DOTB.reports.reportDDProxy(id, 'groupBy');

            DOTB.reports.addFieldToDisplaySummaries(e, 'group_by_row_' + totalGroupByRows);
        },
        ...
        addFieldToDisplaySummaries: function (e, linkedGroupById) {
            ...
            if (report_type == 'summation') {
                cell.innerHTML = "<input type='radio' name='summary_order_by_radio' id='summary_order_by_radio_" + id + "' onClick='DOTB.reports.summaryOrderBySelected(\"" + id + "\")'></input>";
                cell.innerHTML += "&nbsp;&nbsp;<span id='summaryOrderByDirectionDiv_" + id + "'></span>";
                cell = row.insertCell(4);
                cell.setAttribute('scope', 'row');
            }
            if (typeof (linkedGroupById) == 'undefined')
                cell.innerHTML = "&nbsp;&nbsp;<img onclick='DOTB.reports.deleteDisplaySummary(this)' src='index.php?entryPoint=getImage&themeName=" + DOTB.themes.theme_name + "&imageName=delete_inline.gif' alt='" + DOTB.language.get("Reports", "LBL_REMOVE") + "'>";
            DOTB.reports.addToFullTableList(id, fieldGridCell.getRecord(fieldGridCell.getSelectedRows()[0]).getData('parents'));
            if (typeof (linkedGroupById) == 'undefined') {
                var dd11 = YAHOO.util.Dom.get(id);
                dd11.dd = new DOTB.reports.reportDDProxy(id, 'group_summaries');
            }
        ...
        addFieldToDisplayColumns: function (e) {
            ...
            cell.setAttribute("onmouseover", "this.style.cursor = 'move'");
            cell = row.insertCell(2);
            cell.innerHTML = "<input type='text' size='50' id= 'display_cols_label_'" + totalDisplayColRows + "' value='" + colLabel + "' title='" + colLabel + DOTB.language.get('Reports', 'LBL_LABEL') + "' onclick='this.focus();'>";
            cell = row.insertCell(3);
            cell.innerHTML = "<input type='radio' title='" + DOTB.language.get('Reports', 'LBL_ORDER_BY') + "' name='order_by_radio' id='order_by_radio_" + totalDisplayColRows + "' onClick='DOTB.reports.orderBySelected(" + totalDisplayColRows + ")'></input>";
            cell.innerHTML += "&nbsp;&nbsp;<span id='orderByDirectionDiv_" + totalDisplayColRows + "'></span>";
            cell = row.insertCell(4);
            cell.innerHTML = "&nbsp;&nbsp;<img onclick='DOTB.reports.deleteDisplayCol(\"display_cols_row_" + totalDisplayColRows + "\")' src='index.php?entryPoint=getImage&themeName=" + DOTB.themes.theme_name + "&imageName=delete_inline.gif' alt='" + DOTB.language.get("Reports", "LBL_REMOVE") + "'>";
            DOTB.reports.addToFullTableList('display_cols_row_' + totalDisplayColRows, fieldGridCell.getRecord(fieldGridCell.getSelectedRows()[0]).getData('parents'));

            var dd11 = YAHOO.util.Dom.get('display_cols_row_' + totalDisplayColRows);
            dd11.dd = new DOTB.reports.reportDDProxy('display_cols_row_' + totalDisplayColRows, 'group');
        ...
        addOptionalSelect: function () {
            var htmlStr = '';
            DOTB.reports.cleanFullTableList();

            for (i in full_table_list) {
                if (i != "self") {
                    htmlStr += "<br/><input onclick='DOTB.reports.checkOptional(this);' type='checkbox' id='outer_join_checkbox_" + i + "'";
        ...
        showGroupByColumns: function () {
            ...
            var panelHtml = "<table id='groupByTable' width='100%'><tr><td width='4%' class='dataLabel'>&nbsp;&nbsp;&nbsp;&nbsp;</td><td width='30%' class='dataLabel'><b>" + DOTB.language.get('Reports', 'LBL_COLUMN_NAME') +
            "</td><td class='dataLabel'>&nbsp;</td></tr>" +
            "<tr id='group_by_help_row'><td>&nbsp;&nbsp;&nbsp;</td><td colspan=2><table width='70%' valign='center'><tr><td>" + DOTB.language.get('Reports', 'LBL_GROUP_BY_HELP_DESC') + "</td></tr></table></td></tr></table>";

            var title = "<h3 class='spantitle'>" + DOTB.language.get('Reports', 'LBL_SELECT_GROUP_BY') + "<span id='group_by_help'><i class='far fa-info-circle' style='vertical-align: middle;font-size:20px;margin:0px 5px;cursor:pointer' onclick='DOTB.util.showHelpTips(this,\"" + DOTB.language.get('Reports', 'LBL_GROUP_BY_HELP_DESC') + "\");'></i></span></h3>";
        ...
        showDisplaySummaries: function () {
            ...
            module = current_module;
            var panelHtml = "<table id='displaySummariesTable' width='100%'><tr><td width='4%' class='dataLabel'>&nbsp;&nbsp;&nbsp;&nbsp;</td><td width='30%' class='dataLabel'><b>" + DOTB.language.get('Reports', 'LBL_COLUMN_NAME') +
            "</td><td class='dataLabel'>&nbsp;</td><td width='30%'><b>";
            if (report_type == 'summation')
                panelHtml += DOTB.language.get('Reports', 'LBL_ORDER_BY') + "</b></td></tr>";
            panelHtml +=
            "<tr id='display_summary_help_row'><td>&nbsp;&nbsp;&nbsp;</td><td colspan=2><table width='70%' valign='center'><tr><td>" + DOTB.language.get('Reports', 'LBL_DISPLAY_SUMMARY_HELP_DESC') + "</td></tr></table></td></tr></table>";

            var title = "<h3 class='spantitle'>" + DOTB.language.get('Reports', 'LBL_DISPLAY_SUMMARIES') + "<span id='display_summary_help'><i class='far fa-info-circle' style='vertical-align: middle;cursor:pointer;font-size:20px;margin:0px 5px' onclick='DOTB.util.showHelpTips(this,\"" + DOTB.language.get('Reports', 'LBL_DISPLAY_SUMMARY_HELP_DESC') + "\");'></i></span></h3>";
        ...
        showDisplayColumnsPanel: function () {
            ...
            module = current_module;
            var panelHtml = "<table id='displayColsTable' width='100%'><tr><th width='4%'>&nbsp;&nbsp;&nbsp;&nbsp;</th><th width='30%' scope='col'><b>" + DOTB.language.get('Reports', 'LBL_COLUMN_NAME') +
            "</th><th width='30%' scope='col'><b>" + DOTB.language.get('Reports', 'LBL_LABEL') + "</th>" +
            "<th width='30%' scope='col'><b>" + DOTB.language.get('Reports', 'LBL_ORDER_BY') + "</th><th></th></tr>" +
            "<tr id='display_cols_help_row'><td>&nbsp;&nbsp;&nbsp;</td><td colspan=3><table width='70%' valign='center'><tr><td>" + DOTB.language.get('Reports', 'LBL_DISPLAY_COLS_HELP_DESC') + "</td></tr></table></td></tr></table>";

            var title = "<h3 class='spantitle'>" + DOTB.language.get('Reports', 'LBL_CHOOSE_DISPLAY_COLS') + "</h3>";
            ...
            var toolTip = new YAHOO.widget.Tooltip("tt3", {
                context: "display_cols_help",
                text: DOTB.language.get('Reports', 'LBL_DISPLAY_COLS_HELP_DESC'),
        ...
        deleteDisplayCol: function (row) {
            ...
            DOTB.reports.deleteFromFullTableList(row);
        ...
        deleteGroupBy: function (row) {
            ...
            if (document.getElementById('display_summaries_row_' + row)) {
                ...
                DOTB.reports.deleteFromFullTableList('display_summaries_row_' + row);
        ...
        deleteDisplaySummary: function (oTrigger) {
            ...
            DOTB.reports.deleteFromFullTableList(oRow.id);
            var displaySummaryTbl = document.getElementById('displaySummariesTable');
            var tBody = displaySummaryTbl.childNodes[0];
            tBody.removeChild(oRow);
            DOTB.reports.rebuildSummaryIndexes(); // Bug #27623 We need to rebuild indexes
        },
        ...
        rebuildSummaryIndexes: function () {
            ...
            for (var i = 0; i < oNode.childNodes.length; i++) {
                ...
                DOTB.reports.rebuildSummaryNode(oItem, oldIndex, iIndex);
            }
        ...
        rebuildSummaryNode: function (oNode, oldIndex, iIndex) {
            ...
            for (var i = 0; i < oNode.childNodes.length; i++) {
                DOTB.reports.rebuildSummaryNode(oNode.childNodes[i], oldIndex, iIndex);
            }
        },
        ...
        addFilterQualify: function (htmlTableCell, filter) {
            ...
            select_info['onchange'] = "DOTB.reports.filterTypeChanged(" + current_filter_id + ");";
            ...
            htmlTableCell.innerHTML = DOTB.reports.buildSelectHTML(select_html_info);
        ...
        filterTypeChanged: function (index) {
            ...
            DOTB.reports.refreshFilterInput(filter, index);
        },
        refreshFilterInput: function (filter, index) {
            ...
            DOTB.reports.addFilterInput(filter_row.input_cell, filter, filter_row.id);
        },
        ...
        addFilterInputSelectSingle: function (row, options, filter, rowId) {
            ...
            cell.innerHTML = DOTB.reports.buildSelectHTML(select_html_info);
            var filter_row = filters_arr[filters_count_map[current_filter_id]];
        ...
        addFilterInputSelectMultiple: function (row, options, filter, rowId) {
            ...
            //Add By Lap Nguyen
            var field = filter_row.field;
            if(field.type == 'fullname' || field.type == 'name' || field.type == 'id' || field.type == 'relate'
                || field.type == 'username' || field.type == 'assigned_user_name')
                $('select[id="'+select_html_info.select.id+'"]').select2({ width: '250px' });
            //END
        },
        ...
        addFilterInputRelate: function (row, field, filter, isCustom) {
            ...
            new_input.onclick = function () {
                ...
                return open_popup(module_name, 600, 400, "", true, false, {"call_back_function": "DOTB.reports.set_form_return_reports", "form_name": "ReportsWizardForm", "field_to_name_array": {"id": "id", "name": "name"}});
            }
            ...
            var postData = '&module=Reports&action=get_quicksearch_defaults&to_pdf=true&dotb_body_only=true&parent_form=ReportsWizardForm&parent_module=' + module_name + '&parent_field=' + sqs_field_name;
        ...
        addFilterInputDate: function (row, filter) {
            ...
            if (typeof (filter.input_name0) != 'undefined' && filter.input_name0.length > 0) {
                filter.input_name0 = DOTB.reports.to_display_date(filter.input_name0);
            }
            ...
            img_element.setAttribute('src', "index.php?entryPoint=getImage&themeName=" + DOTB.themes.theme_name + "&imageName=jscalendar.gif");
        ...
        addFilterInputDateBetween: function (row, filter) {
            ...
            img_element1.setAttribute('src', "index.php?entryPoint=getImage&themeName=" + DOTB.themes.theme_name + "&imageName=jscalendar.gif");
            ...
            filter.input_name1 = DOTB.reports.to_display_date(filter.input_name1);
            new_input.value = filter.input_name1;
            new_input.name = "text_input";
            new_input.size = "12";
            new_input.maxsize = "255";
            ...
            img_element2.setAttribute('src', "index.php?entryPoint=getImage&themeName=" + DOTB.themes.theme_name + "&imageName=jscalendar.gif");
        ...
        newSelectSpanElement: function (name, inputTime) {
            ...
            var text = '<select tabindex="0" size="1" id="' + dname + '_hours" onchange="DOTB.reports.timeValueUpdate(\'' + dname + '\');">';
            for (i = 1; i <= 12; i++) {
                val = i < 10 ? "0" + i : i;
            ...
            text += '</select>';
            text += ' : ';
            text += '<select tabindex="0" size="1" id="' + dname + '_minutes" onchange="DOTB.reports.timeValueUpdate(\'' + dname + '\');">';
            ...
            text += '\n</select>';
            text += ' <select tabindex="0" size="1" id="' + dname + '_meridiem" onchange="DOTB.reports.timeValueUpdate(\'' + dname + '\');"> ';
        ...
        addFilterInputDatetimesBetween: function (row, filter) {
            ...
            filter.input_name2 = DOTB.reports.to_display_date(filter.input_name2);
        ...
        addFilterInput: function (htmlTableCell, filter, rowId) {
            ...
            if (field.name == 'team_set_id') {
                DOTB.reports.addFilterTeamSet(row, field, filter);
            } else if (qualifier_name == 'between') {
                DOTB.reports.addFilterInputTextBetween(row, filter);
                DOTB.reports.addRunTimeCheckBox(row, filter, rowId);
            } else if (qualifier_name == 'between_dates') {
                DOTB.reports.addFilterInputDateBetween(row, filter);
                DOTB.reports.addRunTimeCheckBox(row, filter, rowId);
            } else if (qualifier_name == 'between_datetimes') {
                DOTB.reports.addFilterInputDatetimesBetween(row, filter);
                DOTB.reports.addRunTimeCheckBox(row, filter, rowId);
            } else if (qualifier_name == 'empty' || qualifier_name == 'not_empty') {
                DOTB.reports.addFilterNoInput(row, filter);
                //This part edit By Lap Nguyen
                ...
                if ((field_type == 'username' || field_type == 'assigned_user_name') && qualifier_name == 'empty' && typeof (filter.name) == 'undefined') {
                    alert(DOTB.language.get("Reports", 'LBL_USER_EMPTY_HELP'));
                }
            } else if (qualifier_name.indexOf("_n_days") != -1) {
                DOTB.reports.addFilterInputText(row, filter);
                DOTB.reports.addRunTimeCheckBox(row, filter, rowId);
            }
            //Add filter datetimecombo look like datetime - BY Lap Nguyen
            else if (field_type == 'date' || field_type == 'datetime' || field.type == 'datetimecombo') {
                if (qualifier_name.indexOf('tp_') == 0) {
                    DOTB.reports.addFilterInputEmpty(row, filter);
                    DOTB.reports.addRunTimeCheckBox(row, filter, rowId);
                } else {
                    DOTB.reports.addFilterInputDate(row, filter);
                    DOTB.reports.addRunTimeCheckBox(row, filter, rowId);
                }
            }
            //            else if ( field.type == 'datetimecombo') {
            ...
            //
            //                else {
            //                    DOTB.reports.addFilterInputDatetimecombo(row,filter);
            //                    DOTB.reports.addRunTimeCheckBox(row,filter,rowId);
            //                }
            //            }
            //END - BY Lap Nguyen
                ...
                //This part edit By Lap Nguyen
                if (typeof filter.qualifier_name != "undefined"){ //Add By Lap Nguyen
                    qualifier_name = filter.qualifier_name;
                }
                add_filter_option(filter_row.qualify_select,qualifier_name);

                if (qualifier_name == 'is' || qualifier_name == 'is_not'){
                    DOTB.reports.addFilterInputRelate(row, field, filter, false);
                    DOTB.reports.addRunTimeCheckBox(row, filter, rowId);
                } else if(qualifier_name == 'one_of' || qualifier_name == 'not_one_of'){
                    ajax_get_option_array(module, field.name,row ,filter, filter_row.id);  //Add filter By Lap Nguyen
                } else{
                    DOTB.reports.addFilterInputText(row, filter);
                    DOTB.reports.addRunTimeCheckBox(row, filter, rowId);
                }
                ...
                if (qualifier_name == 'is' || qualifier_name == 'is_not'){
                    DOTB.reports.addFilterInputRelate(row, field, filter, true);
                    DOTB.reports.addRunTimeCheckBox(row, filter, rowId);
                } else if(qualifier_name == 'one_of' || qualifier_name == 'not_one_of'){
                    ajax_get_option_array(module, field.name,row ,filter, filter_row.id);  //Add filter By Lap Nguyen
                } else{
                ...
                if (users_array == "") {
                    DOTB.reports.loadXML();
                }
                if (qualifier_name == 'one_of' || qualifier_name == 'not_one_of') {
                    DOTB.reports.addFilterInputSelectMultiple(row, users_array, filter, rowId);
                    DOTB.reports.addRunTimeCheckBox(row, filter, rowId);
                } else {
                    DOTB.reports.addFilterInputSelectSingle(row, users_array, filter, rowId);
                    DOTB.reports.addRunTimeCheckBox(row, filter, rowId);
                }
                ...
                if (qualifier_name == 'one_of' || qualifier_name == 'not_one_of') {
                    DOTB.reports.addFilterInputSelectMultiple(row, field.options, filter, rowId);
                    DOTB.reports.addRunTimeCheckBox(row, filter, rowId);
                } else {
                    DOTB.reports.addFilterInputSelectSingle(row, field.options, filter, rowId);
                    DOTB.reports.addRunTimeCheckBox(row, filter, rowId);
                }
            } else if (field_type == 'bool') {
                var options = ['yes', 'no'];
                DOTB.reports.addFilterInputSelectSingle(row, options, filter, rowId);
                DOTB.reports.addRunTimeCheckBox(row, filter, rowId);
            } else if (field_type == 'teamset') {
                DOTB.reports.addTeamset(row, field.options, filter);
            } else {
                DOTB.reports.addFilterInputText(row, filter);
                DOTB.reports.addRunTimeCheckBox(row, filter, rowId);
            }
        ...
        addRunTimeCheckBox: function (row, filter, rowId) {
            ...
            cell.appendChild(new_input);
            cell.innerHTML += " <b><label for='runtime_filter_" + rowId + "' />" + DOTB.language.get('Reports', 'LBL_RUN_TIME_LABEL') + "</input></b>";
            cell.innerHTML += '&nbsp;<img src="index.php?entryPoint=getImage&themeName=' + DOTB.themes.theme_name + '&imageName=helpInline.png"  alt="' + DOTB.language.get("Reports", 'LBL_ALT_INFORMATION') + '" onclick="DOTB.util.showHelpTips(this,\'' + DOTB.language.get('Reports', 'LBL_RUNTIME_HELP') + '\');" class="inlineHelpTip">';
        ...
        addFilterTeamSet: function (row, field, filter) {
            ...
            if (DOTB.isIE && current_filter_id == 0) {
        ...
        */
        ...
        getParentsUIString: function (filter) {
            ...
            if (typeof DOTB.language.languages['app_list_strings']['moduleList'][parentsUIStrSoFar] != 'undefined') {
                parentsUIStrSoFar = DOTB.language.languages['app_list_strings']['moduleList'][parentsUIStrSoFar];
            }
            var previousModule = parentsArr[0];
            for (var i = 1; i < parentsArr.length; i++) {
                prevParentsStr = parentsStrSoFar;
        ...
        addFilterOnLoad: function (filter, current_filters_table) {
            ...
            if (fieldName == 'team_set_id') {
                ...
                filter.table_key += '> ' + DOTB.language.get('app_strings', 'LBL_TEAMS');
            ...
            if (link == "self") {
                ...
                if (typeof DOTB.language.languages['app_list_strings']['moduleList'][module] != 'undefined') {
                    module_str = DOTB.language.languages['app_list_strings']['moduleList'][module];
                }
                parentsUIStrSoFar = module_str;
            } else {
                var parentsUIStrSoFar = DOTB.reports.getParentsUIString(filter);
            }
            ...
            DOTB.reports.addToFullTableList(current_filters_table + '_filter_row_' + totalFilterRows,
            ...
            DOTB.reports.addFilterQualify(qualify_cell, filter);
            DOTB.reports.addFilterInput(input_cell, filter, filterRow.id);

            var delete_cell = document.createElement('td');
            filterRow.appendChild(delete_cell);
            delete_cell.innerHTML = "&nbsp;&nbsp;<img onclick='DOTB.reports.deleteFilter(" + current_filter_id + ",\"" + current_filters_table + "_filter_row_" + totalFilterRows + "\", \"" + current_filters_table + "\")' src='index.php?entryPoint=getImage&themeName=" + DOTB.themes.theme_name + "&imageName=delete_inline.gif' alt='" + DOTB.language.get("Reports", "LBL_REMOVE") + "'>";
        ...
        addFieldToDisplayColumnsOnLoad: function (displayCol, orderBy) {
            ...
            if (link == "self") {
                ...
                var parentsUIStrSoFar = DOTB.reports.getParentsUIString(displayCol);
            }
            ...
            cell = row.insertCell(3);
            cell.setAttribute('scope', 'row');
            cell.innerHTML = "<input type='radio' name='order_by_radio' id='order_by_radio_" + totalDisplayColRows + "' onClick='DOTB.reports.orderBySelected(" + totalDisplayColRows + ")' >";
            cell.innerHTML += "&nbsp;&nbsp;<span id='orderByDirectionDiv_" + totalDisplayColRows + "'></span>";
            if (typeof (orderBy) != "undefined" && orderBy.length > 0) {
                if (orderBy[0].name == displayCol.name && orderBy[0].table_key == displayCol.table_key) {
                    ...
                    DOTB.reports.orderBySelected(totalDisplayColRows, orderBy[0].sort_dir);
                }
            }
            ...
            DOTB.reports.addToFullTableList('display_cols_row_' + totalDisplayColRows,
                parentsUIStrSoFar, link);
        ...
        addFieldToGroupByOnLoad: function (groupByColumn, summaryOrderBy) {
            ...
            if (link == "self") {
                ...
                var parentsUIStrSoFar = DOTB.reports.getParentsUIString(groupByColumn);
            }
            ...
            DOTB.reports.addToFullTableList('group_by_row_' + totalGroupByRows,
                parentsUIStrSoFar, link);
            ...
            var dd11 = YAHOO.util.Dom.get(id);
            dd11.dd = new DOTB.reports.reportDDProxy(id, 'groupBy');
            DOTB.reports.addFieldToDisplaySummariesOnLoad(groupByColumn, 'group_by_row_' + totalGroupByRows, summaryOrderBy);
        },
        addFieldToDisplaySummariesOnLoad: function (summaryColumn, linkedGroupById, summaryOrderBy) {
            ...
            if (link == "self") {
                ...
                var parentsUIStrSoFar = DOTB.reports.getParentsUIString(summaryColumn);
            }
            ...
            cell.setAttribute('scope', 'row');
            if (report_type == 'summation') {
                cell.innerHTML = "<input type='radio' name='summary_order_by_radio' id='summary_order_by_radio_" + id + "' onClick='DOTB.reports.summaryOrderBySelected(\"" + id + "\")' >";
                cell.innerHTML += "&nbsp;&nbsp;<span id='summaryOrderByDirectionDiv_" + id + "'></span>";
                if (typeof (summaryOrderBy) != "undefined" && summaryOrderBy.length > 0) {
                    if (report_type == 'summation' && summaryOrderBy[0].name == summaryColumn.name && summaryOrderBy[0].table_key == summaryColumn.table_key) {
                        ...
                        DOTB.reports.summaryOrderBySelected(id, summaryOrderBy[0].sort_dir);
                    }
                }
                cell = row.insertCell(4);
                cell.setAttribute('scope', 'row');
            ...
            if (linkedGroupById == null)
                cell.innerHTML = "&nbsp;&nbsp;<img onclick='DOTB.reports.deleteDisplaySummary(this)' src='index.php?entryPoint=getImage&themeName=" + DOTB.themes.theme_name + "&imageName=delete_inline.gif' '" + DOTB.language.get("Reports", "LBL_REMOVE") + "'>";
            DOTB.reports.addToFullTableList(id,
                parentsUIStrSoFar, link);
        ...
        copyFilter: function (origFilterRowId, origFilterRow, current_filters_table, filterDef) {
            ...
            DOTB.reports.addFilterQualify(qualify_cell, filterDef);
            DOTB.reports.addFilterInput(input_cell, filterDef, filterRow.id);
        ...
        loadXML: function () {
            ...
            if (window.XMLHttpRequest) { // code for Mozilla, Safari, etc
                ...
                xmlhttp.onreadystatechange = DOTB.reports.loadUsers;
                xmlhttp.send(null);
                DOTB.reports.loadUsers();
        ...
        getParentsStringOfCurrentNode: function () {
            ...
            if (selectedNode) {
                ...
                if (typeof DOTB.language.languages['app_list_strings']['moduleList'][current_module] != 'undefined') {
                    current_module_str = DOTB.language.languages['app_list_strings']['moduleList'][current_module];
                }
        ...
        populateFieldGrid: function (module, link_name, parent_module, comboLabel) {
            ...
            if (DOTB.reports.myDataTable != null) {
                DOTB.reports.myDataTable.destroy();
            } // if
            ...
            var parentsStr = DOTB.reports.getParentsStringOfCurrentNode();
            var parentsLink = DOTB.reports.getParentsLinksOfCurrentNode();
            ...
            var myColumnDefs = [
                {key: "field_label", minWidth: 150, sortable: true, resizeable: false, label: DOTB.language.get('Reports', 'LBL_FIELD_NAME'), abbr: '" scope="col'} //injecting 'scope' into YUI parameter for 508 compliance
            ];
            ...
            var title = "<h3 class='spantitle'>" + DOTB.language.get('Reports', 'LBL_AVAILABLE_FIELDS') + " : " + comboLabel + "<span id='fields_panel_help'><i class='far fa-info-circle' style='vertical-align: middle;font-size:20px;margin:0px 5px;cursor:pointer' onclick='DOTB.util.showHelpTips(this,\"" + DOTB.language.get('Reports', 'LBL_FIELDS_PANEL_HELP_DESC') + "\");'></i></span></h3>";

            var oldOffsetHeight = document.getElementById("autocomplete").offsetHeight;
            DOTB.reports.autocompletemodule.header.innerHTML = title;
            ...
            myDataTable.subscribe("rowClickEvent", DOTB.reports.gridRowClickHandler);
            ...
            oAutoComp.doBeforeLoadData = function (sQuery, oResponse, oPayload) {
                DOTB.reports.myDataTable.initializeTable();
                DOTB.reports.myDataTable.addRows(oResponse.results);
                DOTB.reports.myDataTable.render();
            }
            myDataTable.render();
            dtBodyOffsetHeight = 0;
            if (DOTB.reports.dtBodyOffsetHeight != null) {
                dtBodyOffsetHeight = DOTB.reports.dtBodyOffsetHeight;
            } else {
                DOTB.reports.dtBodyOffsetHeight = YAHOO.util.Dom.getElementsByClassName("yui-dt-bd", "", document.getElementById("module_fields"))[0].offsetHeight;
                dtBodyOffsetHeight = DOTB.reports.dtBodyOffsetHeight
            }
            ...
            dtInput.value = DOTB.language.get('Reports', 'LBL_COMBO_TYPE_AHEAD');
            ...
            dtInput.onfocus = function () {
                if (this.value == DOTB.language.get('Reports', 'LBL_COMBO_TYPE_AHEAD')) {
                    this.value = "";
                } // if
            } // fn
            dtInput.onblur = function () {
                if (this.value == '') {
                    this.value = DOTB.language.get('Reports', 'LBL_COMBO_TYPE_AHEAD');
                }
            } // fn
            var clearButton = document.getElementById("clearButton");
            clearButton.onclick = function () {
                var dtInput = document.getElementById("dt_input");
                if (dtInput.value != DOTB.language.get('Reports', 'LBL_COMBO_TYPE_AHEAD')) {
                    dtInput.value = DOTB.language.get('Reports', 'LBL_COMBO_TYPE_AHEAD');
                    myDataTable.initializeTable();
                    myDataTable.addRows(fieldGridJSON);
                    myDataTable.render();
            ...
            DOTB.reports.myDataTable = myDataTable;
        ...
        gridRowClickHandler: function (e, object) {
            ...
            if (currEditorDiv == 'filters') {
                var panels = DOTB.FiltersWidget.getPanels();
                if (panels.length > 1) {
                    DOTB.reports.addFieldContextMenu(e, fieldGridCell);
                } else {
                    var item = new Object();
                    item.text = panels[0].id;
                    DOTB.reports.addFieldToFilter(item, fieldGridCell);
                }
            } else if (currEditorDiv == 'group_by') {
                DOTB.reports.addFieldToGroupBy(fieldGridCell);
            } else if (currEditorDiv == 'display') {
                DOTB.reports.addFieldToDisplayColumns(fieldGridCell);
            } else if (currEditorDiv == 'display_summaries') {
                DOTB.reports.addFieldToDisplaySummaries(fieldGridCell);
            } // else if

        },
        addFieldToFilterEvent: function (e, eventobj, obj) {
            DOTB.reports.addFieldToFilter(obj.item, obj.e);
        },
        addFieldContextMenu: function (e, object) {
            var panels = DOTB.FiltersWidget.getPanels();
            var myContextMenu = null;
            if (DOTB.reports.filterFieldsMenu != null) {
                myContextMenu = DOTB.reports.filterFieldsMenu;
                myContextMenu.destroy();
            }
            myContextMenu = new YAHOO.widget.Menu("mycontextmenu", {'xy': YAHOO.util.Event.getXY(e.event), visible: true});
            myContextMenu.addItem(DOTB.language.get('Reports', 'LBL_ADD_FILTER_TO'));
            for (var i = 0; i < panels.length; i++) {
                if (document.getElementById(panels[i].id + "_body_div") && panels[i].id != DOTB.language.get('Reports', 'LBL_FILTER') + ".1") {
                    var item = new Object();
                    item.text = panels[i].id;
                    var menuItem = new YAHOO.widget.MenuItem(panels[i].id, {text: panels[i].id, onclick: {fn: DOTB.reports.addFieldToFilterEvent, obj: {'item': item, 'e': object}}});
                    myContextMenu.addItem(menuItem);
                } // if
            } // for
            DOTB.reports.filterFieldsMenu = myContextMenu;
        ...
        buildTree: function (module) {
            ...
            var moduleTree = DOTB.reports.module_tree;
            if (!moduleTree) {
                var title = "<h3 class='spantitle'>" + DOTB.language.get('Reports', 'LBL_RELATED_MODULES') + "<span id='related_modules_panel_help'><i class='far fa-info-circle' style='vertical-align: middle;font-size:20px;margin:0px 5px;cursor:pointer' onclick='DOTB.util.showHelpTips(this,\"" + DOTB.language.get('Reports', 'LBL_RELATED_MODULES_PANEL_HELP_DESC') + "\");'></i></span></h3>";
                ...
                DOTB.reports.autocompletemodule = autocompletemodule;

                // generate YUI tree
                var module_tree = new YAHOO.widget.TreeView("module_tree");
                module_tree.singleNodeHighlight = true;
                //turn dynamic loading on for entire tree:
                module_tree.setDynamicLoad(DOTB.reports.loadModuleTreeData, 1);
                var root = module_tree.getRoot();
                var tempNode = new YAHOO.widget.TextNode({
                    href: "javascript:DOTB.reports.populateFieldGrid('" + module + "','','');",
                    label: module_str,
                    category: module
                    },
                    root, false);
                ...
                DOTB.reports.module_tree = module_tree;
                module_tree.render();
                tempNode.expand();
            } // if
        },
        ...
        loadModuleTreeData: function (node, fnLoadComplete) {
            ...
            var dtInput = document.getElementById("dt_input");
            dtInput.value = DOTB.language.get('Reports', 'LBL_COMBO_TYPE_AHEAD');
            var uri = "index.php?action=BuildReportModuleTree&module=Reports&page=report&dotb_body_only=true&to_pdf=true" + "&report_module=" + nodeCategory;
            YAHOO.util.Connect.asyncRequest('POST', uri, {
                success: DOTB.reports.renderModuleTreeData,
                argument: {
                    node: node,
        ...
        moduleButtonClick: function (currModule, buttonObj) {
            if (currWizardStep > 2 && !confirm(DOTB.language.get('Reports', 'LBL_MODULE_CHANGE_PROMPT')))
                return false;
            ...
            if (DOTB.reports.module_tree != null) {
                DOTB.reports.module_tree.destroy();
                delete DOTB.reports.module_tree;
            } // if
            DOTB.reports.changeSelectedModule(currModule);
            DOTB.reports.showWizardStep(0);
        },
        ...
        changeSelectedModule: function (current_module) {
            var panels = DOTB.FiltersWidget.getPanels();
            for (var i = 0; i < panels.length; i++) {
                DOTB.FiltersWidget.removeGroup(panels[i].id, panels[i].parentId);
            }
            DOTB.reports.buildTree(current_module);
            DOTB.FiltersWidget.init(imgPath);
            DOTB.FiltersWidget.addGroupToPanel('filter_designer_div', DOTB.language.get('Reports', 'LBL_FILTER'));
...
DOTB.reports.reportDDProxy = function (id, sGroup, config) {
    if (document.getElementById(id)) {
        var Dom = YAHOO.util.Dom;
        DOTB.reports.reportDDProxy.superclass.constructor.call(this, id, sGroup, config);
...
YAHOO.extend(DOTB.reports.reportDDProxy, YAHOO.util.DDProxy, {
...
function ajax_get_option_array(table_key, field_name, row ,filter, rowId){
    var report_id = $('input[name=id]').val();
    var module_lap = table_key;
    $.ajax({
        url: "index.php?module=Reports&action=custom_ajax&dotb_body_only=true",
        type: "POST",
        async: false, //waiting for ajax finish THEN continue
        data:
        {
        ...
        success: function(data){
            DOTB.reports.addFilterInputSelectMultiple(row, data.opt_arrar, filter, rowId);
            DOTB.reports.addRunTimeCheckBox(row, filter, rowId);
        },
    });
