        ...
        for(var qsField in qsFields){
            ...
            if (!document.forms[qs_obj.form].elements[qsFields[qsField].id].readOnly && qs_obj['disable'] != true) {
                ...
                if (!loaded) {
	                    	...
	                    	for(var i in this.fields) {		
	                    		for (var key in this.qs_obj.field_list) {
	                	           ...
	                	           if (this.fields[i] == this.qs_obj.field_list[key] && 
                                       ...
                                       DOTB.util.callOnChangeListers(document.forms[this.qs_obj.form].elements[this.qs_obj.populate_list[key]]);
	                	           }
	                	       }
	                    	}
                            DOTB.util.callOnChangeListers(this._elTextbox);
	                    },
                    ...
                    //of dotb_3.js when building the alert message contents.
                    if(/^(billing_|shipping_)?account_name$/.exec(qsFields[qsField].name))
                    {
                       ...
                       search.setFields = function(data, filter) 
                       {
	        			    ...
	        			    if(label_str != current_label_data_str && current_label_data_str != label_data_str) {    	
	        			    	...
	        			    	warning_label = DOTB.language.translate(module_key, 'NTC_OVERWRITE_ADDRESS_PHONE_CONFIRM') + '\n\n' + label_data_str;       			    	
                    ...
                    if(DOTB.isIE && navigator.appVersion.match(/MSIE (.\..)/)[1] < 8 && /team_name/.test(search.getInputEl().id)) {
