...
DOTB.reportsInlineEdit = function() {
	return {
		...
		inlineSelect: function(options, div, current_value, module, record, field_name, field_type) {
			...
			onBlur="DOTB.reportsInlineEdit.inlineSave(\""+div+"\",\""+module+"\",\""+record+"\",\""+field_name+"\",\""+field_type+"\",\""+escape(current_value)+"\")";
			id = 'input_' + div;
		...
		inlineEdit: function(div, current_value, module, record, field_name, field_type, currency_id, currency_symbol) {
			if (field_name == 'name') {
				document.getElementById(div).innerHTML = "<input type ='text' id='input_"+ div +"' value='"+ current_value +"' onBlur='DOTB.reportsInlineEdit.inlineSave(\""+div+"\",\""+module+"\",\""+record+"\",\""+field_name+"\",\""+field_type+"\",\""+current_value+"\")'>";
				...
				document.getElementById(div).innerHTML = DOTB.reportsInlineEdit.inlineSelect(options, div, current_value,module, record, field_name, field_type);
			}
			else if (field_type == 'currency') {
				document.getElementById(div).innerHTML = "<input type ='text' id='input_"+ div +"' value='"+ current_value +"' onBlur='DOTB.reportsInlineEdit.inlineSave(\""+div+"\",\""+module+"\",\""+record+"\",\""+field_name+"\",\""+field_type+"\",\""+current_value+"\",\""+currency_id+"\",\""+currency_symbol+"\")'>";
				document.getElementById('input_'+ div).focus();
		...
		inlineRestore: function(div,module,record,field_name,field_type,currency_id,currency_symbol,current_value,changed,currency_value) {
			if (field_name == 'name') {
				...
				document.getElementById(div).innerHTML += "<img src='index.php?entryPoint=getImage&themeName=" + DOTB.themes.theme_name + "&imageName=edit_inline.png' onClick='DOTB.reportsInlineEdit.inlineEdit(\""+div+"\",\""+current_value+"\",\""+module+"\",\""+record+"\",\""+field_name+"\",\""+field_type+"\")'></img>";
			}
				...
				document.getElementById(div).innerHTML += "<img src='index.php?entryPoint=getImage&themeName=" + DOTB.themes.theme_name + "&imageName=edit_inline.png' onClick='DOTB.reportsInlineEdit.inlineEdit(\""+div+"\",\""+currency_value+"\",\""+module+"\",\""+record+"\",\""+field_name+"\",\""+field_type+"\",\""+currency_id+"\",\""+currency_symbol+"\")'></img>";
			}						
		...
		inlineSave: function(div,module,record,field_name,field_type,old_value,currency_id,currency_symbol) {
			...
			var callback = {
					success:function(o){
						...
						DOTB.reportsInlineEdit.inlineRestore(div,module,record,field_name,field_type,currency_id,currency_symbol,saved_value,true,current_value);
				},
					failure: function(o){}
				}
			if (current_value != old_value) {
				...
				var postData = '&module=Reports&action=saveInline&to_pdf=true&dotb_body_only=true&save_module='+module+'&save_record='+record+
				...
				DOTB.reportsInlineEdit.inlineRestore(div,module,record,field_name,field_type,currency_id,currency_symbol,current_value,false,current_value);
			}
		}
	};
}();
