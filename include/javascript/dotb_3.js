...
/**
 * Namespace for Dotb Objects
 */
if (typeof(DOTB) == "undefined") {
    DOTB = {
        ...
        namespace: function(ns) {
            ...
            return ((typeof DOTB[ns] === "object") && (DOTB[ns] !== null)) ? DOTB[ns] : false;
        },
...
// Namespaces
DOTB.namespace("themes");
DOTB.namespace("tour");

/**
 * Namespace for Homepage
 */
 DOTB.namespace("dotbHome");

/**
 * Namespace for Subpanel Utils
 */
DOTB.namespace("subpanelUtils");

/**
 * AJAX status class
 */
DOTB.namespace("ajaxStatusClass");

/**
 * Tab selector utils
 */
DOTB.namespace("tabChooser");

/**
 * General namespace for Dotb utils
 */
DOTB.namespace("utils");
DOTB.namespace("savedViews");

/**
 * Dashlet utils
 */
DOTB.namespace("dashlets");
DOTB.namespace("unifiedSearchAdvanced");

DOTB.namespace("searchForm");
DOTB.namespace("language");
DOTB.namespace("Studio");
DOTB.namespace("contextMenu");
DOTB.namespace("config");
 ...
 * @deprecated
 *
 * As of Dotb version 6.2.3 (MSIE Version 9) this function is deprecated.  The preferred method is to use the
...
DOTB.isSupportedBrowser = function(){
...
DOTB.isIECompatibilityMode = function(){
...
function validate_form(formname, startsWith){
    ...
    invalidTxt = DOTB.language.get('app_strings', 'ERR_INVALID_VALUE');
					...
					if(!bail){
						switch(validate[formname][i][typeIndex]){
                            ...
                            case 'teamset_mass':
                                ...
                                if(replace_selected && !validation_passed) {
                                   add_error_style(formname, primary_field_id, DOTB.language.get('app_strings', 'ERR_NO_PRIMARY_TEAM_SPECIFIED'));
                                   isError = true;
                                }
                                break;
                            case 'teamset':
                                   ...
                                   if(document.getElementById(table_element_id)) {
                                       ...
                                       if(!has_primary) {
                                          ...
                                          add_error_style(formname, field_id, DOTB.language.get('app_strings', 'ERR_NO_PRIMARY_TEAM_SPECIFIED'));
                                       }
                                   }
                                   break;
                            case 'checkboxset':
						...
						if(typeof validate[formname][i][jstypeIndex]  != 'undefined'/* && !isError*/){
							...
							switch(validate[formname][i][jstypeIndex]){
                                ...
                                break;
                                case 'url':
                                    if (!isUrl(trim(form[validate[formname][i][nameIndex]].value))) {
                                        isError = true;
                                        add_error_style(formname, validate[formname][i][nameIndex], validate[formname][i][msgIndex] + " " + DOTB.language.get('app_strings', 'LBL_VALIDATE_URL'));
                                    }
                                break;
                                case 'range':
                                    if(!inRange(trim(form[validate[formname][i][nameIndex]].value), validate[formname][i][minIndex], validate[formname][i][maxIndex])){
                                        isError = true;
                                        var lbl_validate_range = DOTB.language.get('app_strings', 'LBL_VALIDATE_RANGE');
                                        ...
                                        {
                                            add_error_style(formname, validate[formname][i][nameIndex], validate[formname][i][msgIndex] + " " + DOTB.language.get('app_strings', 'MSG_SHOULD_BE') + ' ' + validate[formname][i][minIndex] + ' ' + DOTB.language.get('app_strings', 'MSG_OR_GREATER'));
                                        }
                                        else if (typeof validate[formname][i][maxIndex] == 'number')
                                        {
                                            add_error_style(formname, validate[formname][i][nameIndex], validate[formname][i][msgIndex] + " " + DOTB.language.get('app_strings', 'MSG_IS_MORE_THAN') + ' ' + validate[formname][i][maxIndex]);
                                        }
                                    }
                                break;
                                case 'isbefore':
                                    ...
                                    if(	typeof compareTo != 'undefined'){
                                        if(trim(compareTo.value) != '' || (validate[formname][i][allowblank] != 'true') ) {
                                            ...
                                            if(trim(date1).length != 0 && !isBefore(date1,date2)){
                                                ...
                                                add_error_style(formname, validate[formname][i][nameIndex], validate[formname][i][msgIndex] + "(" + date1 + ") " + DOTB.language.get('app_strings', 'MSG_IS_NOT_BEFORE') + ' ' +date2);
                                            }
                                        }
                                    }
                                break;
                                case 'maxlen':
                                    ...
                                    if (typeof maximum != 'undefined') {
                                        if (value.length > maximum) {
                                            isError = true;
                                            add_error_style(formname, validate[formname][i][nameIndex], validate[formname][i][msgIndex] + " " + DOTB.language.get('app_strings', 'MSG_EXCEEDS_MAXLEN'));
                                        }
                                    }
                                    break;
                                case 'less':
                                    ...
                                    if(	typeof maximum != 'undefined'){
                                        if(value>maximum) {
                                            isError = true;
                                            add_error_style(formname, validate[formname][i][nameIndex], validate[formname][i][msgIndex] +" " +DOTB.language.get('app_strings', 'MSG_IS_MORE_THAN')+ ' ' + validate[formname][i][altMsgIndex]);
                                        }
                                    }
                                break;
                                case 'more':
                                    ...
                                    if(	typeof minimum != 'undefined'){
                                        if(value<minimum) {
                                            isError = true;
                                            add_error_style(formname, validate[formname][i][nameIndex], validate[formname][i][msgIndex] +" " +DOTB.language.get('app_strings', 'MSG_SHOULD_BE')+ ' ' + minimum + ' ' + DOTB.language.get('app_strings', 'MSG_OR_GREATER'));
                                        }
                                    }
                                break;
                                case 'binarydep':
...
/*	nsingh: BUG#15102
	...
	if(formWithPrecision){
		if (!isValidPrecision(formWithPrecision.float.value, formWithPrecision.precision.value)){
			isError = true;
			add_error_style(formname, 'default', DOTB.language.get('app_strings', 'ERR_COMPATIBLE_PRECISION_VALUE'));
		}else if(!isError){
			isError = false;
		}
	}
...
function http_fetch_sync(url, post_data, headers) {
	...
	if (DOTB.util.isLoginPage(global_xmlhttp.responseText))
		return false;
...
function sendAndRetrieve(theForm, theDiv, loadingStr) {
	...
	if(typeof loadingStr == 'undefined') DOTB.language.get('app_strings', 'LBL_LOADING');
...
// -- start dotbListView class
// js functions used for ListView
function dotbListView() {
}


dotbListView.prototype.confirm_action = function(del) {
	if (del == 1) {
		return confirm( DOTB.language.get('app_strings', 'NTC_DELETE_CONFIRMATION_NUM') + dotbListView.get_num_selected(true)  + DOTB.language.get('app_strings', 'NTC_DELETE_SELECTED_RECORDS'));
	}
	else {
		return confirm( DOTB.language.get('app_strings', 'NTC_UPDATE_CONFIRMATION_NUM') + dotbListView.get_num_selected(true)  + DOTB.language.get('app_strings', 'NTC_DELETE_SELECTED_RECORDS'));
	}

}
dotbListView.get_num_selected = function (asString) {
    var result = 0;
...
dotbListView.update_count = function(count, add) {
...
dotbListView.prototype.use_external_mail_client = function(no_record_txt, module) {
	selected_records = dotbListView.get_checks_count();
...
dotbListView.prototype.use_external_mail_client_callback = function(o)
{
    if (o.responseText)
        location.href = 'mailto:' + o.responseText;
}

dotbListView.prototype.send_form_for_emails = function(select, currentModule, action, no_record_txt,action_module,totalCount, totalCountError) {
	if ( typeof(DOTB.config.email_dotbclient_listviewmaxselect) != 'undefined' ) {
	    maxCount = 10;
	}
	else {
	    maxCount = DOTB.config.email_dotbclient_listviewmaxselect;
	}
    ...
    var callback =
	{
	  success: function(o) {
	      ...
	      DOTB.quickCompose.init(quickComposePackage);
	  }
	}
...
dotbListView.prototype.send_form = function(select, currentModule, action, no_record_txt,action_module,return_info) {
	if (document.MassUpdate.select_entire_list.value == 1) {

		if(dotbListView.get_checks_count() < 1) {
		   alert(no_record_txt);
		   return false;
		}
...
dotbListView.get_checks_count = function() {
	ar = new Array();
...
dotbListView.get_checks = function() {
	ar = new Array();
	if(typeof document.MassUpdate != 'undefined' ){
	if(document.MassUpdate.uid.value != '') {
		oldUids = document.MassUpdate.uid.value.split(',');
...
dotbListView.prototype.order_checks = function(order,orderBy,moduleString){
	checks = dotbListView.get_checks();
...
dotbListView.prototype.save_checks = function(offset, moduleString) {
	checks = dotbListView.get_checks();
	if(typeof document.MassUpdate != 'undefined'){
...
dotbListView.prototype.check_item = function(cb, form) {
	if(cb.checked) {
		dotbListView.update_count(1, true);
        if(dotbListView.get_checks_count() == $(".checkbox").length -2)
        {
		...
		if(typeof form != 'undefined' && form != null) {
			dotbListView.prototype.updateUid(cb, form);
		}
	}
	dotbListView.prototype.toggleSelected();
}

dotbListView.prototype.toggleSelected = function() {

	var numSelected = dotbListView.get_num_selected();
...
dotbListView.prototype.updateUid = function(cb  , form){
...
dotbListView.prototype.check_entire_list = function(form, field, value, list_count) {
	...
	dotbListView.update_count(list_count, false);
    dotbListView.prototype.toggleSelected();
}

dotbListView.prototype.check_all = function(form, field, value, pageTotal) {
	// count number of items
	count = 0;
	...
	for (i = 0; i < form.elements.length; i++) {
		if(form.elements[i].name == field && !(form.elements[i].disabled == true && form.elements[i].checked == false)) {
			...
			form.elements[i].checked = value;
			if(!value){
				dotbListView.prototype.updateUid(form.elements[i], form);
			}
		}
	}
	if (pageTotal >= 0)
		dotbListView.update_count(pageTotal);
 	else if(value)
		dotbListView.update_count(count, true);
	else
		dotbListView.update_count(-1 * count, true);

	dotbListView.prototype.toggleSelected();
}
dotbListView.check_all = dotbListView.prototype.check_all;
dotbListView.confirm_action = dotbListView.prototype.confirm_action;

dotbListView.prototype.check_boxes = function() {
	var inputsCount = 0;
	var checkedCount = 0;
...
function check_used_email_templates() {
	...
	var call_back = {
		success:function(r) {
			if(r.responseText != '') {
				if(!confirm(DOTB.language.get('app_strings','NTC_TEMPLATES_IS_USED') + r.responseText)) {
					return false;
				}
			}
			document.MassUpdate.submit();
...
dotbListView.prototype.send_mass_update = function(mode, no_record_txt, del) {
		...
		case 'selected':
			...
			if(typeof(document.MassUpdate.current_admin_id)!='undefined' && document.MassUpdate.module!= 'undefined' && document.MassUpdate.module.value == 'Users' && (document.MassUpdate.UserType.value!='' || document.MassUpdate.status.value!='')) {
				...
				if(reg_for_current_admin_id.test(document.MassUpdate.uid.value)) {
					...
					alert(DOTB.language.get('Users','LBL_LAST_ADMIN_NOTICE'));
					return false;
				}
			}
			break;
	...
	if(!dotbListView.confirm_action(del))
		return false;
...
dotbListView.prototype.clear_all = function() {
	document.MassUpdate.uid.value = '';
	document.MassUpdate.select_entire_list.value = 0;
	dotbListView.check_all(document.MassUpdate, 'mass[]', false);
    $(document.MassUpdate.massall).each(function(){
...
sListView = new dotbListView();
// -- end dotbListView class
...
DOTB.ajaxStatusClass = function() {};
DOTB.ajaxStatusClass.prototype.statusDiv = null;
DOTB.ajaxStatusClass.prototype.oldOnScroll = null;
DOTB.ajaxStatusClass.prototype.shown = false; // state of the status window

// reposition the status div, top and centered
DOTB.ajaxStatusClass.prototype.positionStatus = function() {
...
DOTB.ajaxStatusClass.prototype.createStatus = function(text) {
...
DOTB.ajaxStatusClass.prototype.showStatus = function(text) {
	if(!this.statusDiv) {
...
// public - hide it
DOTB.ajaxStatusClass.prototype.hideStatus = function(text) {
...
DOTB.ajaxStatusClass.prototype.flashStatus = function(text, time){
	this.showStatus(text);
	window.setTimeout('ajaxStatus.hideStatus();', time);
}


var ajaxStatus = new DOTB.ajaxStatusClass();
// --- end ajax status class

/**
 * Unified Search Advanced - for global search
 */
DOTB.unifiedSearchAdvanced = function() {
	var usa_div;
	var usa_img;
	var usa_open;
	var usa_content;
	...
	return {
		init: function() {
			DOTB.unifiedSearchAdvanced.usa_div = document.getElementById('unified_search_advanced_div');
			DOTB.unifiedSearchAdvanced.usa_img = document.getElementById('unified_search_advanced_img');

			if(!DOTB.unifiedSearchAdvanced.usa_div || !DOTB.unifiedSearchAdvanced.usa_img) return;
			var attributes = { height: { to: 300 } };
            DOTB.unifiedSearchAdvanced.anim_open = new YAHOO.util.Anim('unified_search_advanced_div', attributes );
			DOTB.unifiedSearchAdvanced.anim_open.duration = 0.75;
			DOTB.unifiedSearchAdvanced.anim_close = new YAHOO.util.Anim('unified_search_advanced_div', { height: {to: 0} } );
			DOTB.unifiedSearchAdvanced.anim_close.duration = 0.75;

			DOTB.unifiedSearchAdvanced.usa_img._x = YAHOO.util.Dom.getX(DOTB.unifiedSearchAdvanced.usa_img);
			DOTB.unifiedSearchAdvanced.usa_img._y = YAHOO.util.Dom.getY(DOTB.unifiedSearchAdvanced.usa_img);


			DOTB.unifiedSearchAdvanced.usa_open = false;
			DOTB.unifiedSearchAdvanced.usa_content = null;

		   YAHOO.util.Event.addListener('unified_search_advanced_img', 'click', DOTB.unifiedSearchAdvanced.get_content);
		},
		...
		animate: function(data) {
			...
			if(data) {
				DOTB.unifiedSearchAdvanced.usa_content = data.responseText;
				DOTB.unifiedSearchAdvanced.usa_div.innerHTML = DOTB.unifiedSearchAdvanced.usa_content;
			}
			if(DOTB.unifiedSearchAdvanced.usa_open) {
				document.UnifiedSearch.advanced.value = 'false';
				DOTB.unifiedSearchAdvanced.anim_close.animate();
			}
			else {
				document.UnifiedSearch.advanced.value = 'true';
				DOTB.unifiedSearchAdvanced.usa_div.style.display = '';
				YAHOO.util.Dom.setX(DOTB.unifiedSearchAdvanced.usa_div, DOTB.unifiedSearchAdvanced.usa_img._x - 90);
				YAHOO.util.Dom.setY(DOTB.unifiedSearchAdvanced.usa_div, DOTB.unifiedSearchAdvanced.usa_img._y + 15);
				DOTB.unifiedSearchAdvanced.anim_open.animate();
			}
	      	DOTB.unifiedSearchAdvanced.usa_open = !DOTB.unifiedSearchAdvanced.usa_open;
...
if(typeof YAHOO != 'undefined') YAHOO.util.Event.addListener(window, 'load', DOTB.unifiedSearchAdvanced.init);
...
DOTB.ui = {
	...
	toggleHeader : function() {
		...
		if(h != null) {
			...
			alert(DOTB.language.get("app_strings", "ERR_NO_HEADER_ID"));
		}
	}
};
                                ...
                                Y.io(el.src, {
                                    ...
                                    on: {
                                        success: function(transactionid, response, arguments) {
                                            DOTB.util.globalEval(response.responseText);
                                        }
                                    }
                                });
                            }
                        } else {
                            DOTB.util.globalEval(el.innerHTML || el.innerText);
                        }
                    } catch (e) {
		...
		setMaxLength: function() { // modified from http://www.quirksmode.org/dom/maxlength.html
			...
			for (var i=0;i<x.length;i++) {
				if (x[i].getAttribute('maxlength')) {
					x[i].onkeyup = x[i].onchange = DOTB.util.checkMaxLength;
					x[i].onkeyup();
				}
			}
		},
		...
		getAdditionalDetails: function(bean, id, spanId, show_buttons) {
			...
			go = function() {
				...
				success = function(data) {
					...
					DOTB.util.additionalDetailsCache[id] = new Array();
					DOTB.util.additionalDetailsCache[id]['body'] = result['body'];
					DOTB.util.additionalDetailsCache[id]['caption'] = result['caption'];
					DOTB.util.additionalDetailsCache[id]['width'] = result['width'];
					DOTB.util.additionalDetailsCache[id]['theme'] = result['theme'];
					ajaxStatus.hideStatus();
					return oReturn(DOTB.util.additionalDetailsCache[id]['body'], DOTB.util.additionalDetailsCache[id]['caption'], DOTB.util.additionalDetailsCache[id]['width'], DOTB.util.additionalDetailsCache[id]['theme']);
				}

				if(typeof DOTB.util.additionalDetailsCache[id] != 'undefined')
					return oReturn(DOTB.util.additionalDetailsCache[id]['body'], DOTB.util.additionalDetailsCache[id]['caption'], DOTB.util.additionalDetailsCache[id]['width'], DOTB.util.additionalDetailsCache[id]['theme']);

				if(typeof DOTB.util.additionalDetailsCalls[id] != 'undefined') // call already in progress
					return;
				...
				DOTB.util.additionalDetailsCalls[id] = YAHOO.util.Connect.asyncRequest('GET', url, {success: success, failure: success});

				return false;
			}
			DOTB.util.additionalDetailsRpcCall = window.setTimeout('go()', 250);
		},
		clearAdditionalDetailsCall: function() {
			if(typeof DOTB.util.additionalDetailsRpcCall == 'number') window.clearTimeout(DOTB.util.additionalDetailsRpcCall);
		},
		/**
...
DOTB.util.additionalDetailsCache = new Array();
DOTB.util.additionalDetailsCalls = new Array();
if(typeof YAHOO != 'undefined') YAHOO.util.Event.addListener(window, 'load', DOTB.util.setMaxLength); // allow textareas to obey maxlength attrib

DOTB.savedViews = function() {
	var selectedOrderBy;
	...
	return {
		setChooser: function() {
			...
			if (!DOTB.savedViews.clearColumns)
		...
		saved_search_action: function(action, delete_lang) {
			...
			if(action == 'save') {
				if(document.search_form.saved_search_name.value.replace(/^\s*|\s*$/g, '') == '') {
					alert(DOTB.language.get('app_strings', 'LBL_SAVED_SEARCH_ERROR'));
					return;
				}
			}
			...
			DOTB.ajaxUI.submitForm(document.search_form);
		},
		...
		handleForm: function() {
			DOTB.tabChooser.movementCallback = function(left_side, right_side) {
         ...
         // which was breaking Calls and Meetings
				...
				for(i in left_side.childNodes) { // fill in order by options
					if(typeof left_side.childNodes[i].nodeName != 'undefined' &&
						...
						typeof DOTB.savedViews.columnsMeta[left_side.childNodes[i].value] != 'undefined' && // check if column is sortable
						typeof DOTB.savedViews.columnsMeta[left_side.childNodes[i].value]['sortable'] == 'undefined' &&
						DOTB.savedViews.columnsMeta[left_side.childNodes[i].value]['sortable'] != false) {
							nodeCount++;
							...
							if(optionNode.value == DOTB.savedViews.selectedOrderBy)
			...
			DOTB.tabChooser.movementCallback(document.getElementById('display_tabs_td').getElementsByTagName('select')[0]);

			// This check is needed for the Activities module (Calls/Meetings/Tasks).
			if (document.search_form.orderBy)
				document.search_form.orderBy.options.value = DOTB.savedViews.selectedOrderBy;

			// handle direction
			if(DOTB.savedViews.selectedSortOrder == 'DESC') document.getElementById('sort_order_desc_radio').checked = true;
			else document.getElementById('sort_order_asc_radio').checked = true;
		}
	};
}();

DOTB.searchForm = function() {
	var url;
	return {
		...
		searchFormSelect: function(view, previousView) {
			...
			var handleDisplay = function() { // hide other divs
                ...
                a_key = DOTB.language.get("app_strings", "LBL_ADV_SEARCH_LNK_KEY");
                     ...
                     if(document.search_form.elements[num]) {
                         ...
                         if(typeof el.type != 'undefined' && typeof el.name != 'undefined' && el.name.match(pattern)) {
                             ...
                             if(typeof document.search_form[advanced_input_name] != 'undefined')  // if advanced input of same name exists
                                 DOTB.searchForm.copyElement(advanced_input_name, el);
                         }
                     }
                 }
			}
			...
			if(document.getElementById(module + theView + 'SearchForm').innerHTML == '') {
				...
				var success = function(data) {
					...
					if(theView == 'saved_views') {
						if(typeof columnsMeta != 'undefined') DOTB.savedViews.columnsMeta = columnsMeta;
						if(typeof selectedOrderBy != 'undefined') DOTB.savedViews.selectedOrderBy = selectedOrderBy;
						if(typeof selectedSortOrder != 'undefined') DOTB.savedViews.selectedSortOrder = selectedSortOrder;
					}
				...
				if(theView == 'saved_views') // handle the tab chooser
					url += '&displayColumns=' + DOTB.savedViews.displayColumns + '&hideTabs=' + DOTB.savedViews.hideTabs + '&orderBy=' + DOTB.savedViews.selectedOrderBy + '&sortOrder=' + DOTB.savedViews.selectedSortOrder;
		...
		clear_form: function(form, skipElementNames) {
            ...
            for( var i = 0; i < elemList.length ; i++ ) {
                ...
                if ( typeof(elem.type) != 'undefined' && typeof(skipElementNames) != 'undefined'
                        && DOTB.util.arrayIndexOf(skipElementNames, elem.name) != -1 )
                {
                    continue;
                }
...
DOTB.tabChooser = function () {
	...
	return {
			...
			freezeOptions: function(left_name, right_name, target) {
				if(!DOTB.tabChooser.frozenOptions) { DOTB.tabChooser.frozenOptions = []; }
				if(!DOTB.tabChooser.frozenOptions[left_name]) { DOTB.tabChooser.frozenOptions[left_name] = []; }
				if(!DOTB.tabChooser.frozenOptions[left_name][right_name]) { DOTB.tabChooser.frozenOptions[left_name][right_name] = []; }
				if(typeof target == 'array') {
					for(var i in target) {
						DOTB.tabChooser.frozenOptions[left_name][right_name][target[i]] = true;
					}
				} else {
					DOTB.tabChooser.frozenOptions[left_name][right_name][target] = true;
				}
			},
			...
			left_to_right: function(left_name, right_name, left_size, right_size) {
				DOTB.savedViews.clearColumns = false;
			    ...
			    var frozen_options = DOTB.tabChooser.frozenOptions;
			...
			right_to_left: function(left_name, right_name, left_size, right_size, max_left) {
			    ...
			    var frozen_options = DOTB.tabChooser.frozenOptions;
			    frozen_options = DOTB.tabChooser.frozenOptions && DOTB.tabChooser.frozenOptions[right_name] && DOTB.tabChooser.frozenOptions[right_name][left_name]?DOTB.tabChooser.frozenOptions[right_name][left_name]:[];
			    ...
			    if(max_left != '' && (display_columns_ref.length + selected_right.length) > max_left) {
                    alert(DOTB.language.get('app_strings','LBL_MAXIMUM_OF') + max_left + DOTB.language.get('app_strings','LBL_COLUMNS_CAN_BE_DISPLAYED'));
					return;
			    }
...
DOTB.language = function() {
    return {
        languages : new Array(),

        setLanguage: function(module, data) {
           if (!DOTB.language.languages) {

           }
            DOTB.language.languages[module] = data;
        },

        get: function(module, str) {
            if(typeof DOTB.language.languages[module] == 'undefined' || typeof DOTB.language.languages[module][str] == 'undefined')
            {
                return 'undefined';
            }
            return DOTB.language.languages[module][str];
        },
...
DOTB.contextMenu = function() {
	return {
		...
		registerObject: function(objectType, id, metaData) {
			DOTB.contextMenu.objects[id] = new Object();
            DOTB.contextMenu.objects[id] = {'objectType' : objectType, 'metaData' : metaData};
		},
		/**
		...
		registerObjectType: function(name, menuItems) {
			DOTB.contextMenu.objectTypes[name] = new Object();
			DOTB.contextMenu.objectTypes[name] = {'menuItems' : menuItems, 'objects' : new Array()};
		},
		/**
		...
		onContextMenuItemClick: function(p_sType, p_aArguments, p_oItem) {
            var oLI = DOTB.contextMenu.getListItemFromEventTarget(this.parent.contextEventTarget);
            id = this.parent.contextEventTarget.parentNode.id; // id of the target
            funct = eval(DOTB.contextMenu.objectTypes[DOTB.contextMenu.objects[id]['objectType']]['menuItems'][this.index]['action']);
            funct(this.parent.contextEventTarget, DOTB.contextMenu.objects[id]['metaData']);
		},
		/**
		 * Initializes all context menus registered
		 **/
		init: function() {
			for(var i in DOTB.contextMenu.objects) { // make a variable called objects in objectTypes containg references to all triggers
                if(typeof DOTB.contextMenu.objectTypes[DOTB.contextMenu.objects[i]['objectType']]['objects'] == 'undefined')
                    DOTB.contextMenu.objectTypes[DOTB.contextMenu.objects[i]['objectType']]['objects'] = new Array();
				DOTB.contextMenu.objectTypes[DOTB.contextMenu.objects[i]['objectType']]['objects'].push(document.getElementById(i));
			}
            // register the menus
			for(var i in DOTB.contextMenu.objectTypes) {
	            var oContextMenu = new YAHOO.widget.ContextMenu(i, {'trigger': DOTB.contextMenu.objectTypes[i]['objects']});
				var aMainMenuItems = DOTB.contextMenu.objectTypes[i]['menuItems'];
	            ...
	            for(var j = 0; j < nMainMenuItems; j++) {
	                ...
	                oMenuItem.clickEvent.subscribe(DOTB.contextMenu.onContextMenuItemClick, oMenuItem, true);
	                oContextMenu.addItem(oMenuItem);
	            }
	            //  Add a "move" event handler to the context menu
	            oContextMenu.moveEvent.subscribe(DOTB.contextMenu.onContextMenuMove, oContextMenu, true);
	            // Add a "keydown" event handler to the context menu
	            oContextMenu.keyDownEvent.subscribe(DOTB.contextMenu.onContextMenuItemClick, oContextMenu, true);
...
DOTB.contextMenu.actions = function() {
	return {
		/**
...
function open_popup(module_name, width, height, initial_filter, close_popup, hide_clear_button, popup_request_data, popup_mode, create, metadata)
{
	...
	win = DOTB.util.openWindow(URL, windowName, windowFeatures);
				...
				if(window.document.forms[form_name].elements[the_key].tagName == 'SELECT') {
					...
					for(var i = 0; i < selectField.options.length; i++) {
						if(selectField.options[i].text == displayValue) {
                            ...
                            DOTB.util.callOnChangeListers(selectField);
							break;
						}
					}
				} else {
					window.document.forms[form_name].elements[the_key].value = displayValue;
                    DOTB.util.callOnChangeListers(window.document.forms[form_name].elements[the_key]);
				}
			}
...
function set_return(popup_reply_data)
{
	...
	if(typeof name_to_value_array != 'undefined' && name_to_value_array['account_id'])
	{
        ...
        if(label_data_str != label_str && current_label_data_str != label_str){
        	...
        	// Bug 48726 End
        	else if(confirm(DOTB.language.get('app_strings', 'NTC_OVERWRITE_ADDRESS_PHONE_CONFIRM') + '\n\n' + label_data_str))
			{
        		set_return_basic(popup_reply_data,/\S/);
			}
        	else
	...
	var call_back = {
		success:function(r) {
			if(r.responseText == 'true') {
				if(!confirm(DOTB.language.get('app_strings','NTC_TEMPLATE_IS_USED'))) {
					return false;
				}
			} else {
...
DOTB.image = {
    ...
    confirm_imagefile : function(field_name) {
            ...
            if (fileExtension == "jpg" || fileExtension == "jpeg"
                ...
                field.value=null;
                alert(DOTB.language.get('app_strings', 'LBL_UPLOAD_IMAGE_FILE_INVALID'));
            }
    },

    lightbox : function(image)
	{
        if (typeof(DOTB.image.lighboxWindow) == "undefined")
			DOTB.image.lighboxWindow = new YAHOO.widget.SimpleDialog('dotbImageViewer', {
             type:'message',
	            resizable:false,
	            modal:true,
	            id:'dotbMsgWindow',
		...
		DOTB.image.lighboxWindow.setBody("<img src='" + image + "'> </img>");
		DOTB.image.lighboxWindow.render(document.body);
        DOTB.image.lighboxWindow.show();
		DOTB.image.lighboxWindow.center()
    }
}

DOTB.append(DOTB.util, {
...
ajaxCallInProgress: function(){
    ...
    if (typeof (send_back) != "function"){
        ...
        if (!c) return true;
        t = YAHOO.lang.trim(DOTB.util.innerText(c));
    }
    return DOTB_callsInProgress != 0 || t == "";
},
...
closeActivityPanel: {
    show:function(module,id,new_status,viewType,parentContainerId){
        if (DOTB.util.closeActivityPanel.panel)
			DOTB.util.closeActivityPanel.panel.destroy();
	    var singleModule = DOTB.language.get("app_list_strings", "moduleListSingular")[module];
	    singleModule = typeof(singleModule != 'undefined') ? singleModule.toLowerCase() : '';
	    var closeText =  DOTB.language.get("app_strings", "LBL_CLOSE_ACTIVITY_CONFIRM").replace("#module#",singleModule);
        DOTB.util.closeActivityPanel.panel =
	    new YAHOO.widget.SimpleDialog("closeActivityDialog",
	             { width: "300px",
	               ...
	               buttons: [ { text:DOTB.language.get("app_strings", "LBL_EMAIL_OK"), handler:function(){
	                   if (DOTB.util.closeActivityPanel.panel)
                            DOTB.util.closeActivityPanel.panel.hide();

                        ajaxStatus.showStatus(DOTB.language.get('app_strings', 'LBL_SAVING'));
                        var args = "action=save&id=" + id + "&record=" + id + "&status=" + new_status + "&module=" + module + "&csrf_token=" + DOTB.csrf.form_token;
                        ...
                        YAHOO.util.Connect.asyncRequest('POST', 'index.php', callback, args);
	                          ...
	                          { text:DOTB.language.get("app_strings", "LBL_EMAIL_CANCEL"),  handler:function(){DOTB.util.closeActivityPanel.panel.hide(); }} ]
	             } );

	    DOTB.util.closeActivityPanel.panel.setHeader(DOTB.language.get("app_strings", "LBL_CLOSE_ACTIVITY_HEADER"));
        DOTB.util.closeActivityPanel.panel.render(document.body);
        DOTB.util.closeActivityPanel.panel.show();
    }
},
...
DOTB.clearRelateField = function(form, name, id)
{
    if (typeof form[name] == "object"){
        form[name].value = '';
        DOTB.util.callOnChangeListers(form[name]);
    }

    if (typeof form[id] == "object"){
        form[id].value = '';
        DOTB.util.callOnChangeListers(form[id]);
    }
};
if(typeof(DOTB.AutoComplete) == 'undefined') DOTB.AutoComplete = {};

DOTB.AutoComplete.getOptionsArray = function(options_index){
    var return_arr = [];

    var opts = DOTB.language.get('app_list_strings', options_index);
...
if(typeof(DOTB.MultiEnumAutoComplete) == 'undefined') DOTB.MultiEnumAutoComplete = {};

DOTB.MultiEnumAutoComplete.getMultiSelectKeysFromValues = function(options_index, val_string){
    var opts = DOTB.language.get('app_list_strings', options_index);
...
DOTB.MultiEnumAutoComplete.getMultiSelectValuesFromKeys = function(options_index, val_string){
    var opts = DOTB.language.get('app_list_strings', options_index);
...
function clickToEditPassword(elId, passwordPlaceholder)
{
    ...
    if ($el && $el.val() == passwordPlaceholder) {
        var instructions = $('<span class="button">'+DOTB.language.get("app_strings", "LBL_CLICK_TO_EDIT")+'</span>');
