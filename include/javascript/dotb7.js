...
(function(app) {
    ...
    app.events.on("app:view:change", function() {
        ...
        //Add Title to Hybrid Module - By Lap Nguyen
        var hmods = app.config.hybrid_modules;
        
        if (_.isEmpty(metadata) || metadata.isBwcEnabled || _.contains(hmods, module)) {//End: By Lap Nguyen
    ...
    app.Router = app.Router.extend({
        ...
        bwcRedirect: function(options) {
            ...
            if (_.isArray(options.args) && options.args[0]) {
                ...
                if (meta && meta.isBwcEnabled) {
                    var lumiaAction = options.args[2] || options.route,
                        bwcAction = app.bwc.getAction(lumiaAction);

                    if (bwcAction !== lumiaAction) {
                        action = bwcAction;
                    }
        ...
        hasAccessToModule: function(options) {
            ...
            if (showWizard) {
                ...
                // https://github.com/dotbcrm/Mango/pull/18722#discussion_r11782561
                // Will be addressed in SC-2761.
