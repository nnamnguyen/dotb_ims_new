 ...
 */
sw.MessageBox = {
	progressTemplate : "{body}<br><div class='dotb-progress-wrap'><div class='dotb-progress-bar'/></div>",
	promptTemplate : "{body}:<input id='dotb-message-prompt' class='dotb-message-prompt' name='dotb-message-prompt'></input>",
	show: function(config) {
		var myConf = sw.MessageBox.config = {
			...
			id:'dotbMsgWindow',
			close:true,
			title:DOTB.language.get("app_strings", "LBL_MESSAGE_BOX_TITLE"),
			msg: " ",
			buttons: [ ]
		};
		...
		if (config['type'] && config['type'] == "prompt") {
			myConf['buttons'] = [{
				text: DOTB.language.get("app_strings", "LBL_EMAIL_CANCEL"), handler:YAHOO.DOTB.MessageBox.hide
			},{
				text: DOTB.language.get("app_strings", "LBL_EMAIL_OK"), handler:config['fn'] ?
								function(){
									var returnValue = config['fn'](YAHOO.util.Dom.get("dotb-message-prompt").value);
									if (typeof(returnValue) == "undefined" || returnValue) {
										YAHOO.DOTB.MessageBox.hide();
									} // if
								}
								: YAHOO.DOTB.MessageBox.hide, isDefault:true
			}];
		} else if ((config['type'] && config['type'] == "alert")) {
			myConf['buttons'] = [{
				text: 		DOTB.language.get("app_strings", "LBL_EMAIL_OK"),
				handler:	config['fn'] ?
								function(){YAHOO.DOTB.MessageBox.hide(); config['fn']();}
							:
								YAHOO.DOTB.MessageBox.hide,
				isDefault:true
			}]
			...
			myConf['buttons'] = [{
				text: DOTB.language.get("app_strings", "LBL_EMAIL_YES"), handler:config['fn'] ?
								function(){config['fn']('yes');YAHOO.DOTB.MessageBox.hide();}
								: YAHOO.DOTB.MessageBox.hide, isDefault:true
			},{
				text: DOTB.language.get("app_strings", "LBL_EMAIL_NO"), handler:config['fn'] ?
								function(){config['fn']('no');YAHOO.DOTB.MessageBox.hide();}
								: YAHOO.DOTB.MessageBox.hide
			}];
		}
	...
	updateProgress: function(percent, message) {
		...
		var barEl = Dom.getElementsByClassName("dotb-progress-bar", null, YAHOO.DOTB.MessageBox.panel.element)[0];
		if (percent > 100)
			percent = 100;
		else if (percent < 0)
			percent = 0;
...
sw.SelectionGrid = function(containerEl, columns, dataSource, config){
	...
	if (config.forceMulti){
        this.subscribe("rowClickEvent", function(o){
            ...
            o.event = DOTB.util.clone(o.event);
	...
	initAttributes: function(attr) {
        ...
        this.setAttributeConfig("label", {
            ...
            method: function(value) {
            	...
            	Dom.addClass(closeButton, "dotb-tab-close");
...
/**
 * The dotb Tree is a YUI tree with node construction based on AJAX data built in.
 */
...
(function() {
    ...
    YAHOO.widget.Record = function(oLiteral) {
        ...
        if (DOTB.reports && DOTB.reports.overrideRecord) {
