...
/**
 * This is the callback function called from DotbRPCClient.prototype.call_method
 * found below.
...
//////////////////////////////////////////////////
// class: DotbVCalClient
// async retrieval/parsing of vCal freebusy info
//
//////////////////////////////////////////////////

DotbClass.inherit("DotbVCalClient","DotbClass");

function DotbVCalClient() {
	this.init();
}

DotbVCalClient.prototype.init = function() {}

DotbVCalClient.prototype.load = function(user_id, request_id) {
    ...
    YAHOO.util.Connect.asyncRequest('GET', './vcal_server.php?type=vfb&source=outlook&user_id=' + user_id, {
        success: function (result) {
            ...
            GLOBAL_REGISTRY.freebusy[user_id] = DotbVCalClient.prototype.parseResults(result.responseText, false);
            // parse for current user adjusted vCal
            GLOBAL_REGISTRY.freebusy_adjusted[user_id] = DotbVCalClient.prototype.parseResults(result.responseText, true);
...
DotbVCalClient.prototype.parseResults = function(textResult, adjusted) {
    ...
    while (((result = match.exec(textResult))) != null) {
        ...
        if (adjusted) {// send back adjusted for current_user
            startdate = DotbDateTime.parseAdjustedDate(result[1], dst_start, dst_end, gmt_offset_secs);
            enddate = DotbDateTime.parseAdjustedDate(result[2], dst_start, dst_end, gmt_offset_secs);
        }
        else { // GMT
            startdate = DotbDateTime.parseUTCDate(result[1]);
            enddate = DotbDateTime.parseUTCDate(result[2]);
        }
		...
		while (startdate.valueOf() < enddate.valueOf()) {
			var hash = DotbDateTime.getUTCHash(startdate);
...
DotbVCalClient.parseResults = DotbVCalClient.prototype.parseResults;
//////////////////////////////////////////////////
// class: DotbRPCClient
// wrapper around async JSON-RPC client class
//
//////////////////////////////////////////////////
DotbRPCClient.allowed_methods = ['retrieve','query','save','set_accept_status','get_objects_from_module', 'email', 'get_user_array', 'get_full_list'];

DotbClass.inherit("DotbRPCClient","DotbClass");

function DotbRPCClient() {
	this.init();
}
 ...
 */
DotbRPCClient.prototype.allowed_methods = ['retrieve','query','get_objects_from_module'];

DotbRPCClient.prototype.init = function() {
 ...
 * and the synchronous call in dotb_3.js.
 * @param method
 * @param args
 * @param synchronous Pass in true if synchronous call is desired
 */
DotbRPCClient.prototype.call_method = function(method, args, synchronous) {
    var result,
        transaction,
...
var global_rpcClient =  new DotbRPCClient();
