	...
	var Datepicker = function(element, options){
		...
		if (this.isInput) {
			...
			if (this.element.parent().find('.add-on').length) {
			    // Only proxy on the first one (e.g. calendar icon; if a DotBCRM datetimecombo, we'll have a
			    // "clock" for time and we don't want to open the calendar when that's clicked (UIUX-1110 / SP-1362)
			    // Clicking on clock time icon is handled by the datetimecombo.js controller in DotBCRM.
