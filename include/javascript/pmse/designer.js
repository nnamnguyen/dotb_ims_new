...
/*global translate, $, document, window, DOTB_FLAVOR, AdamProject, AdamCanvas, AdamEvent,
 AdamGateway, AdamActivity, AdamArtifact, AdamFlow, getAutoIncrementName, jCore,
 location, DOTB_URL, adamUID, RestClient, DOTB_REST, parseInt, DOTB_AJAX_URL,
...
var project,
    ...
    PMSE_DESIGNER_FORM_TRANSLATIONS = {
        ...
        TITLE_DOTB_DATE: translate('LBL_PMSE_ADAM_UI_TITLE_DOTB_DATE'),
...
function renderProject (prjCode) {
    var pmseCurrencies, currencies, dotbCurrencies, currentCurrency, i;
    ...
    pmseCurrencies = [];
    currencies = DOTB.App.metadata.getCurrencies();
        ...
        if (currencies.hasOwnProperty(currID)) {
            if (currencies[currID].status === 'Active') {
                pmseCurrencies.push({
                    ...
                    preferred: currID === DOTB.App.user.getCurrency().currency_id,
    ...
    project = new AdamProject({
        metadata: [
            ...
            },
            {
                ...
                data: DOTB.App.date.toDatepickerFormat(DOTB.App.user.attributes.preferences.datepref)
            },
            {
...
//    project.restClient.setRestfulBehavior(DOTB_REST);
//    if (!DOTB_REST) {
//        project.restClient.setBackupAjaxUrl(DOTB_AJAX_URL);
//    }
