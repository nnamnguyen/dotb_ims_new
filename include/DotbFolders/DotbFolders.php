...
class DotbFolder {
	 ...
	 * @param string beanId GUID of DotbBean being moved
	 */
	...
	/**
	 * Sets a user's preferences for subscribe folders (Dotb only)
...
ENDW;
	...
	function generateDotbsDynamicFolderQuery()
	{
		global $current_user;
	...
	function getListItemsForEmailXML($folderId, $page = 1, $pageSize = 10, $sort = '', $direction='') {
		...
		if($this->is_dynamic) {
			$r = $this->db->limitQuery(from_html($this->generateDotbsDynamicFolderQuery() . $order), $start, $pageSize);
		} else {
		...
		while($a = $this->db->fetchByAssoc($r)) {
			...
			$temp['uid'] = $a['id'];
			$temp['mbox'] = 'dotb::'.$a['polymorphic_module'];
			$temp['ieId'] = $folderId;
		...
		$metadata = array();
		$metadata['mbox'] = $app_strings['LBL_EMAIL_DOTB_FOLDER'].': '.$this->name;
	...
	function getCountItems ( $folderId ) {
		...
		if ($this->is_dynamic) {
	    	...
	    	$modified_select_query = preg_replace($pattern, $replacement, $this->generateDotbsDynamicFolderQuery(), 1);
	...
	/**
	 * Convenience method, pass a DotbBean and User to this to add anything to a given folder
	 */
	...
	function getCountNewItems($id, $criteria, $folder) {
		...
		return $dotbFolder->getCountUnread($id);
	}
	...
	/**
	 * Builds up a metanode for folder refresh (Dotb folders only)
	 */
	...
	function buildTreeNodeFolders($a, $nodePath, $folderStates, $subscriptions) {
		...
		$folderNode->dynamic_load = true;
        $folderNode->set_property('click', "DOTB.email2.listView.populateListFrameDotbFolder(YAHOO.namespace('frameFolders').selectednode, '{$a['id']}', 'false');");
        $folderNode->set_property('ieId', 'folder');
