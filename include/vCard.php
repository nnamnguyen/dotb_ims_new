...
class vCard
{
    ...
    /**
     * Method parses vCard and creates a Bean in Dotb from it
     *
     * @param $filename - Uploaded vCard file
     * @param string $module - Module we're importing
     * @return string - id of the created bean
     * @throws DotbException if all required fields are not present
     */
    public function importVCard($filename, $module = 'Contacts')
    {
        ...
        for ($index = 0; $index < sizeof($lines); $index++) {
            ...
            if ($encoding !== $GLOBALS['dotb_config']['default_charset']) {
                    ...
                    if (!empty($value)) {
                        if ($bean instanceof Contact || $bean instanceof Lead) {
                            ...
                            if (!isset($result->id)) {
                                ...
                                if (DotbAutoLoader::existing('custom/include/vCardTrimStrings.php')) {
        ...
        foreach ($bean->get_import_required_fields() as $key => $value) {
            if (empty($bean->$key)) {
                throw new DotbException('LBL_EMPTY_REQUIRED_VCARD');
            }
        }
