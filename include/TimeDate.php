<?php
    ...
    class TimeDate
    {
        ...
        /**
         * Return DB Date cum auto-check DB Date Format
         * @by Lap Nguyen
         *
         ...
         */
        public function convertToDBDate($date, $convert_tz = false){
            if(!empty($date)){
                if(!$this->check_matching_format($date, self::DB_DATE_FORMAT))
                    return $this->to_db_date($date, $convert_tz);
                else return $date;
            }else
        ...
        public function get_date_format(User $user = null)
        {
            ...
            if (empty($datef)) {
                $datef = $GLOBALS['dotb_config']['default_date_format'];
            }
            if (empty($datef)) {
                $datef = '';
            }
        ...
        public function get_time_format(/*User*/ $user = null)
        {
            ...
            if (empty($timef)) {
                $timef = $GLOBALS['dotb_config']['default_time_format'];
            }
            if (empty($timef)) {
                $timef = '';
            }
        ...
        {
            try {
                $dt = DotbDateTime::createFromFormat($format, $date);
                if (!is_object($dt)) {
                    return false;
                }
            } catch (Exception $e) {
        ...
        {
            try {
                return DotbDateTime::createFromFormat(self::DB_DATETIME_FORMAT, $date, self::$gmtTimezone);
            } catch (Exception $e) {
        ...
        {
            try {
                return DotbDateTime::createFromFormat(self::DB_DATE_FORMAT, $date, self::$gmtTimezone);
            } catch (Exception $e) {
        ...
        {
            try {
                return DotbDateTime::createFromFormat($format, $date, self::$gmtTimezone);
            } catch (Exception $e) {
        ...
        public function fromUser($date, User $user = null)
        {
            $res = null;
            try {
                $res = DotbDateTime::createFromFormat($this->get_date_time_format($user), $date, $this->_getUserTZ($user));
            } catch (Exception $e) {
        ...
        {
            try {
                return DotbDateTime::createFromFormat($this->get_time_format($user), $date, $this->_getUserTZ($user));
            } catch (Exception $e) {
        ...
        {
            try {
                return DotbDateTime::createFromFormat($this->get_date_format($user), $date, $convert_tz?$this->_getUserTZ($user):self::$gmtTimezone);
            } catch (Exception $e) {
        ...
        {
            try {
                return new DotbDateTime($date, $this->_getUserTZ($user));
            } catch (Exception $e) {
        ...
        /**
         * Build a new DotbDateTime object from the input date and time
         ...
         * @return DotbDateTime DateTime object representing the input date
         */
        public function fromIso($inputDate, User $user = null)
        {
            try {
                ...
                if ( !empty($inputSplit[7]) ) {
                    ...
                    $date = DotbDateTime::createFromFormat(self::DB_DATETIME_FORMAT,$formattedDate,self::$gmtTimezone);
                    ...
                    return DotbDateTime::createFromFormat(self::DB_DATETIME_FORMAT,$formattedDate,$this->_getUserTZ($user));
                }
            } catch (Exception $e) {
        ...
        /**
         * Build a new DotbDateTime object from the input date
        ...
        /**
         * Build a new DotbDateTime object from the input time
        ...
        public function fromIsoTime($inputDate, User $user = null)
        {
            try {
                if ( strlen($inputDate) > 8 ) {
                    ...
                    $date = DotbDateTime::createFromFormat(self::DB_TIME_FORMAT,substr($inputDate,0,8),self::$gmtTimezone);
                    $date->adjustByIsoOffset(substr($inputDate,8));

                    return $date;
                } else {
                    // This time doesn't have an attached offset, convert it according to the user's date and time
                    return DotbDateTime::createFromFormat(self::DB_TIME_FORMAT,$inputDate,$this->_getUserTZ($user));
                }
            } catch (Exception $e) {
                $GLOBALS['log']->error("fromIsoTime: Conversion of $inputDate from ISO Time failed. {$e->getMessage()}");
            }
        ...
        protected function _convert($date, $fromFormat, $fromTZ, $toFormat, $toTZ, $expand = false)
        {
            ...
            try {
                ...
                $phpdate = DotbDateTime::createFromFormat($fromFormat, $date, $fromTZ);
                if ($phpdate == false) {
        ...
        public function splitTime($date, $format)
        {
            if (! ($date instanceof DateTime)) {
                $date = DotbDateTime::createFromFormat($format, $date);
            }
        ...
        {
            try {
                $phpdate = DotbDateTime::createFromFormat($this->get_date_time_format(),
        ...
        {
            if(!$this->allow_cache) {
                return new DotbDateTime("now", $userTz?$this->_getUserTz():self::$gmtTimezone);
            }
        ...
        protected $date_expressions = array(
            ...
            //Add new range - By Lap Nguyen
            'before_today' => array("-99 years", "-1 day"),
            'after_today' => array("+99 years", "+1 day"),
            'before_last_7_days' => array("-99 years", "-6 days"),
            'after_next_7_days' => array("+99 years", "+6 day"),
            'before_last_30_days' => array("-99 years", "-29 days"),
            'after_next_30_days' => array("+99 years", "+29 day"),
        );
        ...
        function handle_offset($date, $format, $to = true, $user = null, $usetimezone = null)
        {
            ...
            $dateobj = new DotbDateTime($date, $to? self::$gmtTimezone : $tz);
        ...
        public function getDSTRange($year, $zone = null)
        {
            ...
            $year_date = DotbDateTime::createFromFormat("Y", $year, self::$gmtTimezone);
         ...
         * @deprecated moved to DotbView
         * @return string JS code
         */
        function get_javascript_validation()
        {
            return DotbView::getJavascriptValidation();
        }
        ...
        function get_user_time_format()
        {
            ...
            if (! empty($time_pref) && ! empty($dotb_config['time_formats'][$time_pref])) {
                return $dotb_config['time_formats'][$time_pref];
            }
        ...
        //Add By lap nguyen
        function time_Ago($datetime, $convert_tz = false) {
            if($convert_tz)
                $datetime = $this->to_db_date_time($datetime, $convert_tz);

            // Calculate difference between current
            // time and given timestamp in seconds
            $diff     = strtotime($this->nowDb()) - strtotime($datetime);
