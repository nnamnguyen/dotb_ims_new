...
class ViewDotbFieldCollection{
    var $ss; // Dotb Smarty Object
    var $bean;
    var $bean_id;
    var $name;
    var $value_name;
    ...
    function process_editview(){
        ...
        if(isset($this->displayParams['collection_field_list'])){
            ...
            foreach($this->displayParams['collection_field_list'] as $k=>$v){
                ...
                $this->displayParams['collection_field_list'][$name]['label'] = "{dotb_translate label='{$collection_field_vardef['vname']}' module='{$this->related_module}'}";//translate($collection_field_vardef['vname'], $this->related_module);
...
FRA;
     ...
     */
    function display(){
        $cacheRowFile = dotb_cached('modules/') . $this->module_dir .  '/collections/'. $this->name . '.tpl';
    ...
    function findTemplate($view, $classList = null)
    {
        ...
        if(!is_array($classList)) {
            ...
            $classList = array($this->type,str_replace('ViewDotbField','',$lastClass));
            while ( $lastClass = get_parent_class($lastClass) ) {
                $classList[] = str_replace('ViewDotbField','',$lastClass);
            }
        }
