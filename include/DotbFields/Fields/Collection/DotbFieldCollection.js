...
if(typeof(DOTB.collection) == "undefined") {
    DOTB.collection = function(form_name, field_name, module, popupData){
    ...
    DOTB.collection.prototype = {
        ...
        add_secondaries: function(){
            ...
            YAHOO.util.Event.onContentReady(clone_id, function(c){
                ...
                if (!_.isUndefined(window.parent.DOTB) && !_.isUndefined(window.parent.DOTB.App) && !_.isUndefined(window.parent.DOTB.App.view)) {
        ...
        init_clone: function(values){
            ...
            var Field0 = DOTB.isIE ?
            	DOTB.collection.safe_clone(this.cloneField[0], true) :
        ...
        js_more: function(val){
            if(this.show_more_image){
	            ...
	            if(this.more_status == false){
	                more_.src = "index.php?entryPoint=getImage&themeName="+DOTB.themes.theme_name+"&imageName=advanced_search.gif";
	                ...
	                more_.src = "index.php?entryPoint=getImage&themeName="+DOTB.themes.theme_name+"&imageName=basic_search.gif";
	                this.more_status=false;
                ...
                if (!_.isUndefined(window.parent.DOTB) && !_.isUndefined(window.parent.DOTB.App) && !_.isUndefined(window.parent.DOTB.App.view) ) {
                    var model = window.parent.DOTB.App.controller.layout.getComponent('bwc').bwcModel;
                    model.set($(arrow).attr('name'), arrow.value);
                }
	            var more_div = document.getElementById('more_div_'+this.field_element_name);
	            if(more_div) {
	              more_div.innerHTML = arrow.value == 'show' ? DOTB.language.get('app_strings','LBL_HIDE') : DOTB.language.get('app_strings','LBL_SHOW');
                    if (radios.length == 1) {
        ...
        create_clone: function() {
            ...
            this.cloneField[0] = DOTB.isIE ?
                DOTB.collection.safe_clone(oneField, true) :
        ...
        js_more_detail: function(id){
            ...
            if(more_img.style.display == 'inline'){
                more_img.src = "index.php?entryPoint=getImage&themeName="+DOTB.themes.theme_name+"&imageName=advanced_search.gif";
            }else{
                more_img.src = "index.php?entryPoint=getImage&themeName="+DOTB.themes.theme_name+"&imageName=basic_search.gif";
            }
        },
        /*
	...
	DOTB.collection.safe_clone = function(e, recursive)
	{
		if (e.nodeName == "#text")
		{
        ...
        if(DOTB.isIE7 && e.tagName.toLowerCase() == 'input')
        {
		...
		for (var i in properties)
		{
			if (e[properties[i]])
			{
                ...
                if ((properties[i] != 'style' || !DOTB.isIE) &&
		...
		if(recursive)
		{
			for (var i in e.childNodes)
			{
				...
				{
					var child = DOTB.collection.safe_clone(e.childNodes[i], true);
