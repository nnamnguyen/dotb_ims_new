...
<input type="hidden" class="DateTimeCombo" id="{{$idname}}" name="{{$idname}}" value="{$fields[{{dotbvar key='name' stringFormat=true}}].value}">
<script type="text/javascript" src="{dotb_getjspath file="include/DotbFields/Fields/Datetimecombo/Datetimecombo.js"}"></script>
<script type="text/javascript">
var combo_{{$idname}} = new Datetimecombo("{$fields[{{dotbvar key='name' stringFormat=true}}].value}", "{{$idname}}", "{$TIME_FORMAT}", "{{$tabindex}}", '{{$displayParams.showNoneCheckbox}}', false, true);
...
//    removing it here to mirror the Datetime DotbField, where the validation is not added at this level
