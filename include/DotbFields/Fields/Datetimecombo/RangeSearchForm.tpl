...
{assign var="id" value={{dotbvar key='name' string=true}} }
{{else}}
...
{html_options options={{dotbvar key='options' string=true}} selected=$starting_choice}
</select>
</div>
...
{dotb_getimage name="jscalendar" ext=".gif" alt=$APP.LBL_ENTER_DATE|escape:'html' other_attributes=$other_attributes}
{{/if}}
{{if $displayParams.showFormats}}
&nbsp;(<span class="dateFormat">{$USER_DATEFORMAT}</span>)
{{/if}}
