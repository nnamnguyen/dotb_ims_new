...
/**
 * DotbFieldBase translates and displays fields from a vardef definition into different formats
 * including DetailView, ListView, EditView. It also provides Search Inputs and database queries
 * to handle searching
 *
 * @property Dotb_Smarty $ss
 */
class DotbFieldBase {
    /**
     * A simple error property to be accessed by calling code for child object
     * methods that process data rather than just manipulate it (like DotbFieldFile
     * and DotbFieldImage when uploading)
     *
     * @var string
     */
    public $error;
    ...
    protected function getBase($view)
    {
        if(!isset(self::$base[$view])) {
            if(!empty($GLOBALS['current_language'])) {
            	self::$base[$view] = DotbAutoLoader::existingCustomOne("include/DotbFields/Fields/Base/{$GLOBALS['current_language']}.$view.tpl");
            }
            if(empty(self::$base[$view])) {
            	self::$base[$view] = DotbAutoLoader::existingCustomOne("include/DotbFields/Fields/Base/$view.tpl");
            }
        }
    ...
    protected function findTemplate($view, $formName = '')
    {
        ...
        if (isset($this->formTemplateMap[$formName])) {
            ...
            $classList = array($this->type, str_replace('DotbField', '', $lastClass));
            while ($lastClass = get_parent_class($lastClass)) {
                $classList[] = str_replace('DotbField', '', $lastClass);
            }
        ...
        foreach ( $classList as $className ) {
            if(isset($current_language)) {
                $tplName = DotbAutoLoader::existingCustomOne('include/DotbFields/Fields/'. $className .'/'. $current_language . '.' . $view .'.tpl');
                if ($tplName) {
                    break;
                }
            }
            $tplName = DotbAutoLoader::existingCustomOne('include/DotbFields/Fields/'. $className .'/'. $view .'.tpl');
            if ($tplName) {
                break;
            }
        }
    ...
    /**
     * Formats a field for the Dotb API
     *
     * @param array     $data
     * @param DotbBean $bean
     * @param array     $args
    ...
    function getListViewSmarty($parentFieldArray, $vardef, $displayParams, $col) {
        ...
        //because they are already formated in DotbBean.php in the function get_list_view_array() as fix of bug #21672
    ...
    public function displayFromFunc($displayType, $parentFieldArray, $vardef, $displayParams, $tabindex = 0)
    {
        ...
        if ( $displayType == 'ListView'
            ...
            if ( $returnsHtml ) {
                ...
                return '{dotb_run_helper include="'.$includeFile.'" func="'.$funcName.'" bean=$bean field="'.$fieldName.'" value=$fields.'.$fieldName.'.value displayType="'.$displayType.'"}';
            }
        }
    }
     ...
     * getSearchInput
     *
     * This function allows the DotbFields to handle returning the search input value given arguments (typically from $_REQUEST/$_POST)
     * and a search string.
     *
     * @param $key String value of key to search for
     * @param $args Mixed value containing haystack to search for value in
     * @return $value Mixed value that the DotbField should return
     */
     ...
     *
     * @param DotbBean $bean - the bean performing the mass update
     * @param array $params - an array of paramester relevant to the save, which will be an array passed up to the API
     * @param string $field - The name of the field to save (the vardef name, not the form element name)
     * @param array $properties - Any properties for this field
     */
    public function apiMassUpdate(DotbBean $bean, array $params, $field, $properties) {
        ...
        )
    {
        if( isset($vardef['len']) ) {
            ...
            $value = dotb_substr($value, $vardef['len']);
        }
    ...
    /**
     * isRangeSearchView
     * This method helps determine whether or not to display the range search view code for the dotb field
     * @param array $vardef entry representing the dotb field's definition
     ...
     *
     * @see DotbFieldEnum.php, DotbFieldInt.php, DotbFieldFloat.php, DotbFieldRelate.php
     * @param	mixed	$parentFieldArray Array or Object of data where the field's value comes from
     * @param	array	$vardef The vardef entry linked to the DotbField instance
     ...
     * @param DotbBean $bean - the Bean
     * @param DotbQuery $q - the Query
     * @param DotbQuery_Builder_Where $where - the Where statement
     ...
     */
    public function fieldNeedsSecondaryQuery($fieldName, DotbBean $bean)
    {
     ...
     * @param string $fieldName - the field we are fixing
     * @param DotbBean $seed - The seed bean to run the query off of
     * @param Array $beans - An array of DotbBeans keyed by the ID.
     */
    public function runSecondaryQuery($fieldName, DotbBean $seed, array $beans)
    {
        return;
    }
     ...
     * @return bool
     */
    public function isErased(DotbBean $bean, $fieldName)
    {
