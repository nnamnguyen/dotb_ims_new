...
<script type="text/javascript">
    ...
    collection["{$displayParams.formName}_{$vardef.name}"] = new DOTB.collection('{$displayParams.formName}', '{$vardef.name}', '{$module}', '{$displayParams.popupData}');
...
<!-- BEGIN Add and collapse -->
        <td rowspan='1' scope='row' style='white-space:nowrap; word-wrap:normal;'>
            &nbsp;
            {if !$hideShowHideButton}
            <span onclick="javascript:collection['{$displayParams.formName}_{$vardef.name}'].js_more();" id='more_{$displayParams.formName}_{$vardef.name}' style="text-decoration:none;" title="{dotb_translate label="LBL_HIDE_SHOW"}">
            ...
            {dotb_getimage name="advanced_search.gif" attr=$attr}
            <span id="more_div_{$displayParams.formName}_{$vardef.name}" >{dotb_translate label='LBL_SHOW'}</span>
        	</span>
        	{/if}
        </td>
...
<!-- END Labels Line -->
    <tr id="lineFields_{$displayParams.formName}_{$vardef.name}_0">
        <td scop='row' valign='top'>
            ...
            <input type="text" name="{$vardef.name}_collection_0" id="{$displayParams.formName}_{$vardef.name}_collection_0" class="sqsEnabled" tabindex="{$tabindex}" size="{$displayParams.size}" value=""  title="{dotb_translate label='LBL_TEAM_SELECTED_TITLE'}"  autocomplete="off" {$displayParams.readOnly} {$displayParams.field}>
...
<!-- BEGIN Remove and Radio -->
        <td valign='top' align='left' nowrap class="teamset-row">
			...
			{capture assign="alt"}{dotb_translate label="LBL_ID_FF_REMOVE"}{/capture}
        ...
        <td valign='top' align='center' class="teamset-row">
            ...
            &nbsp;
            <input id="primary_{$vardef.name}_collection_0" name="primary_{$vardef.name}_collection" type="radio" class="radio" {if $displayParams.primaryChecked}checked="checked" {/if} title="{dotb_translate label='LBL_TEAM_SELECT_AS_PRIM_TITLE'}"value="0" onclick="collection['{$displayParams.formName}_{$vardef.name}'].changePrimary(true);"/>
            </span>
        </td>
        {if $isTBAEnabled}
            <td valign='top' align='center' class="teamset-row">
            ...
            <input id="selected_{$vardef.name}_collection_0" name="selected_{$vardef.name}_collection_0"
                   ...
                   title="{dotb_translate label='LBL_TEAM_TBSELECTED_TITLE'}"
                   {else}title="{dotb_translate label='LBL_TEAM_SELECT_AS_TBSELECTED_TITLE'}"{/if}/>
            </span>
            </td>
        {/if}
        <td>
    ...
    <table style="border-spacing: 0pt;">
        <tr>
    		<td nowrap>
            	...
            	<input type="radio" class="radio" name="{$vardef.name}_type" value="replace" checked> {dotb_translate label='LBL_REPLACE_BUTTON'}
    			<input type="radio" class="radio" name="{$vardef.name}_type" value="add"> {dotb_translate label='LBL_ADD_BUTTON'}
                </div>	
    		</td>
    	</tr>    
    </table>
