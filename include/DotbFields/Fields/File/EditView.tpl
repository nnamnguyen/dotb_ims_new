...
{{/if}}
<script type="text/javascript" src='{{dotb_getjspath file="include/DotbFields/Fields/File/DotbFieldFile.js"}}'></script>
...
{if !empty({{dotbvar key='value' stringFormat=true}}) }
...
<input type="hidden" name="{{$idName}}" id="{{$idName}}" value="{{dotbvar key='value'}}">
{{if isset($vardef.allowEapm) && $vardef.allowEapm}}
...
{{if isset($vardef.allowEapm) && $vardef.allowEapm}}
{if isset($fields.{{$vardef.docType}}) && !empty($fields.{{$vardef.docType}}.value) && $fields.{{$vardef.docType}}.value != 'Dotb' && !empty($fields.{{$vardef.docUrl}}.value) }
...
{{/if}}
{if !$noChange}
<input type='button' class='button' id='remove_button' value='{$APP.LBL_REMOVE}' onclick='DOTB.field.file.deleteAttachment("{{$idName}}","{{$vardef.docType}}",this);'>
{/if}
</span>
{if !$noChange}
...
<span id="{{$idName}}_more">{dotb_image name="advanced_search" width="8px" height="8px"}</span>
<span id="{{$idName}}_less" style="display: none;">{dotb_image name="basic_search" width="8px" height="8px"}</span>
...
{{else}}
    maxlength="255"
{{/if}} autocomplete="off" value="{if !empty($fields[{{$vardef.docId}}].value)}{{dotbvar key='name'}}{/if}">

{{if empty($displayParams.hideButtons) }}
<span class="id-ff multiple">
<button type="button" name="{{$idName}}_remoteSelectBtn" id="{{$idName}}_remoteSelectBtn" tabindex="{{$tabindex}}" title="{dotb_translate label="{{$displayParams.accessKeySelectTitle}}"}" class="button firstChild" value="{dotb_translate label="{{$displayParams.accessKeySelectLabel}}"}"
onclick="DOTB.field.file.openPopup('{{$idName}}'); return false;">
{dotb_getimage alt=$app_strings.LBL_ID_FF_SELECT name="id-ff-select" ext=".png" other_attributes=''}</button>
<button type="button" name="{{$idName}}_remoteClearBtn" id="{{$idName}}_remoteClearBtn" tabindex="{{$tabindex}}" title="{$APP.LBL_CLEAR_BUTTON_TITLE}" class="button lastChild" value="{$APP.LBL_CLEAR_BUTTON_LABEL}" onclick="DOTB.field.file.clearRemote('{{$idName}}'); return false;">
{dotb_getimage name="id-ff-clear" alt=$app_strings.LBL_ID_FF_CLEAR ext=".png" other_attributes=''}
</button>
</span>
{{/if}}
</span>
...
DOTB.field.file.setupEapiShowHide("{{$idName}}","{{$vardef.docType}}","{$form_name}");
{rdelim});
...
YAHOO.util.Event.addListener(['{{$idName}}_file', '{{$vardef.docType}}'], 'change', DOTB.field.file.checkFileExtension,{ldelim} fileEl: '{{$idName}}_file', targEl: '{{$displayParams.onchangeSetFileNameTo}}'{rdelim});
{rdelim}
{rdelim});
</script>
{{/if}}
