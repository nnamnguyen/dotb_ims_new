...
class DotbFieldCurrency extends DotbFieldFloat 
{
    function getListViewSmarty($parentFieldArray, $vardef, $displayParams, $col) {
        ...
        $baseCurrency = DotbCurrency::getBaseCurrency();
        ...
        $currencySymbol = !empty($parentFieldArray['CURRENCY_SYMBOL']) ?
            ...
            : DotbCurrency::getCurrencyByID($currencyId)->symbol;
        ...
        if (empty($currencyId) || !empty($vardef['is_base_currency'])) {
            ...
            if ($current_user->getPreference('currency_show_preferred')) {
                ...
                if (!empty($parentFieldArray['BASE_RATE']) && $parentFieldArray['BASE_RATE'] <> 1) {
                    $amount = DotbCurrency::convertWithRate($amount, 1.0, $parentFieldArray['BASE_RATE']);
                } else {
                    $amount = DotbCurrency::convertWithRate($amount, 1.0, $userCurrency->conversion_rate);
                }
            } else {
    ...
    public function importSanitize($value, $vardef, $focus, ImportFieldSanitize $settings)
    {
        ...
        $base_currency = DotbCurrency::getBaseCurrency();
        ...
        if (isset($vardef['convertToBase']) && $vardef['convertToBase']) {
            ...
            try {
                $value = DotbCurrency::convertAmountFromBase($value, $settings->currency_id);
            } catch (DotbMath_Exception $sme) {
        ...
        return DotbCurrency::formatAmount($value, $currency_id, 6, '.', '', false);
    }
    ...
    public function exportSanitize($value, $vardef, $focus, $row = array())
    {
        ...
        if (isset($vardef['convertToBase']) && $vardef['convertToBase']) {
            ...
            $value = DotbCurrency::convertWithRate($value, $baseRate);
            $currency_id = '-99';
        } elseif (isset($vardef['is_base_currency']) && $vardef['is_base_currency']) {
            $currency_id = '-99';
        } else {
        ...
        return DotbCurrency::formatAmountUserLocale($value, $currency_id);
    }
    ...
    /**
     * Since DotbFieldFloat override this method, we need to do the same here and always return true as we want
