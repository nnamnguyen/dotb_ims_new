...
class DashletGeneric extends Dashlet {
   /**
    ...
    public function __construct($id, $options = null)
    {
        ...
        // this should be taken out when dotbwidgets change
    ...
    function processDisplayOptions() {
        ...
        $this->configureSS = new Dotb_Smarty();
        // column chooser
        ...
        // display rows
        $displayRowOptions = $GLOBALS['dotb_config']['dashlet_display_row_options'];
            ...
            if(!empty($params)) {
                ...
                switch($widgetDef['type']) {// handle different types
                    ...
                    case 'assigned_user_name':
                        // This type runs through the DotbWidgetFieldname class, and needs a little extra help to make it through
	...
	{
    	foreach(DotbAutoLoader::existing('custom/modules/'.$this->seedBean->module_dir.'/metadata/dashletviewdefs.php') as $customMetadate) {
        ...
        if(!empty($this->displayTpl))
        {
            ...
            foreach($this->lvs->data['pageData']['urls'] as $type => $url) {
                ...
                if($type == 'orderBy')
                    ...
                    $this->lvs->data['pageData']['urls'][$type] = preg_replace('/(action=.*&)/Ui', 'action=DynamicAction&DynamicAction=displayDashlet&', $url) . '&dotb_body_only=1&id=' . $this->id;
            }
