...
class DotbCleaner
{
    ...
    function __construct()
    {
        ...
        if(!is_dir(dotb_cached("htmlclean"))) {
        ...
        $config->set('Cache.SerializerPath', dotb_cached("htmlclean"));
        $config->set('URI.Base', $dotb_config['site_url']);
        ...
        if(!empty($GLOBALS['dotb_config']['html_allow_objects'])) {
            // for object
        ...
        // for link
        $config->set('HTML.DefinitionID', 'Dotb HTML Def');
        $config->set('HTML.DefinitionRev', 3);
        $config->set('Cache.SerializerPath', dotb_cached('htmlclean/'));
        // IDs are namespaced
        $config->set('Attr.EnableID', true);
        $config->set('Attr.IDPrefix', 'dotb_text_');
        ...
        $uri->addFilter(new DotbURIFilter(), $config);
 ...
 */
class DotbURIFilter extends HTMLPurifier_URIFilter
{
    public $name = 'DotbURIFilter';
    protected $allowed = array();

    public function prepare($config)
    {
        global $dotb_config;
        if(!empty($dotb_config['security_trusted_domains']) && is_array($dotb_config['security_trusted_domains']))
        {
            $this->allowed = $dotb_config['security_trusted_domains'];
        }
    }
    ...
    public function filter(&$uri, $config, $context)
    {
    	...
    	// suspiciously like DotBCRM query, reject
