...
class DotbOAuth2StoragePortal extends DotbOAuth2StoragePlatform {
    /**
    ...
    public function setupVisibility() {
        ...
        $default_acls = DotbBean::getDefaultACL();
        // This one overrides the Static ACL's, so disable that
        unset($default_acls['DotbACLStatic']);
        $default_acls['DotbACLStatic'] = false;
        $default_acls['DotbACLSupportPortal'] = true;
        DotbBean::setDefaultACL($default_acls);
        DotbACL::resetACLs();

        $default_visibility = DotbBean::getDefaultVisibility();
        $default_visibility['SupportPortalVisibility'] = true;
        DotbBean::setDefaultVisibility($default_visibility);
   	...
   	public function checkUserCredentials(IOAuth2GrantUser $storage, $client_id, $username, $password)
   	{
        ...
        if ( $portalApiUser == null ) {
           // Can't login as a portal user if there is no API user
            throw new DotbApiExceptionPortalNotConfigured();
        }
        ...
        if ( !empty($contact) ) {
            ...
            if(!$sessionManager->canAddSession()) {
                ...
                throw new DotbApiExceptionNeedLogin('too_many_concurrent_connections',array('Too many concurrent sessions'));
            }
            ...
            throw new DotbApiExceptionNeedLogin(translate('ERR_INVALID_PASSWORD', 'Users'));
        }
    }
