...
/**
 * Dotb OAuth2.0 Base Storage system, allows the OAuth2 library we are using to
 ...
 */
class DotbOAuth2StorageBase extends DotbOAuth2StoragePlatform {
    /**
    ...
    public function getUserBean($user_id) {
        if($user_id=="1" || $user_id=='serviceapi') $this->numSessions=10000;
	...
	public function checkUserCredentials(IOAuth2GrantUser $storage, $client_id, $username, $password)
    {
        ...
        if ( $loginSuccess ) {
            ...
            throw new DotbApiExceptionNeedLogin($message);
        }
    }
    ...
    public function loadUserFromName($username)
    {
        ...
        if (empty($userBean)) {
            throw new DotbApiExceptionNeedLogin();
        }
        return $userBean;
    }
