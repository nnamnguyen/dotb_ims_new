...
/**
 * Dotb OAuth2.0 Storage system, allows the OAuth2 library we are using to
 * store and retrieve data.
 * This class should only be used by the OAuth2 library and cannot be relied
 * on as a stable API for any other sources.
 */
class DotbOAuth2Storage implements IOAuth2GrantUser, IOAuth2RefreshTokens, DotbOAuth2StorageInterface, IOAuth2GrantExtension {
    /**
     * The client platform
     *
     * @var string
     ...
     *
     * @var DotbOAuth2StoragePlatform
     */
    protected $platformStore;

    /**
     * The DotBCRM User record for this user
     * @var User
     */
    protected $userBean;
    ...
    // BEGIN METHOD FROM DotbOAuth2StorageInterface
    /**
     ...
     * @return DotbBean The user from the name
     */
    ...
    // END METHOD FROM DotbOAuth2StorageInterface
    ...
    public function getClientDetails($client_id)
    {
        ...
        // Auto-create beans for "dotb" client only as it doesn't exist yet
        if ( $clientBean == null ) {
            if ($client_id != 'dotb') {
                return false;
            }
    ...
    public function isClientAllowed($clientBean)
    {
        ...
        // client key is either dotb OR the client type matches the named type
    ...
    public function getAccessToken($oauth_token)
    {
        ...
        if ( isset($_SESSION['oauth2']) ) {
            ...
            // It's not an oauth2 session, but a normal dotb session we will let them pass
            return array(
                'client_id'=>'dotb',
    ...
    public function setAccessToken($oauth_token, $client_id, $user_id, $expires, $scope = NULL)
    {
        ...
        throw new DotbApiException('Could not start session because client type was not found');
    }
    ...
    protected function setPlatformStore($platform = null) {
        if (empty($this->platformStore) || ($platform && ($store = $this->platformStore->getPlatformName()) != $platform)) {
            ...
            $platform_class = 'DotbOAuth2Storage' . ucfirst(strtolower($platform));

            if($platform != 'base' && DotbAutoLoader::requireWithCustom("include/DotbOAuth2/{$platform_class}.php")) {
                $oauthStorageName = DotbAutoLoader::customClass($platform_class);
            } else {
                DotbAutoLoader::requireWithCustom('include/DotbOAuth2/DotbOAuth2StorageBase.php');
                $oauthStorageName = DotbAutoLoader::customClass('DotbOAuth2StorageBase');
            }
     ...
     *
     * @return DotbOAuth2StoragePlatform
     */
    ...
    public function checkGrantExtension($uri, array $inputData, array $authHeaders)
	{
        if($uri == self::SAML_GRANT_TYPE) {
            ...
            return $this->checkUserCredentials('dotb', '', '');
