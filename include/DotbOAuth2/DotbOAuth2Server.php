...
/**
 * Dotb OAuth2.0 server, is a wrapper around the php-oauth2 library
 * @api
 */
class DotbOAuth2Server extends OAuth2
{
    ...
    public static function getOAuth2Server($platform = null)
    {
        if (!isset(static::$currentOAuth2Server)) {
            ...
            $isIDMModeEnabled = $idpConfig->isIDMModeEnabled() && $platform != DotbOAuth2ServerOIDC::PORTAL_PLATFORM;
            $oidcPostfix = $isIDMModeEnabled ? 'OIDC' : '';
            DotbAutoLoader::requireWithCustom('include/DotbOAuth2/DotbOAuth2Storage'.$oidcPostfix.'.php');
            $oauthStorageName = DotbAutoLoader::customClass('DotbOAuth2Storage'.$oidcPostfix);
            $oauthStorage = new $oauthStorageName();

            DotbAutoLoader::requireWithCustom('include/DotbOAuth2/DotbOAuth2Server'.$oidcPostfix.'.php');
            $oauthServerName = DotbAutoLoader::customClass('DotbOAuth2Server'.$oidcPostfix);
    ...
    public function getSudoToken($userName, $clientId, $platform)
    {
        ...
        } catch (\DotbApiExceptionNeedLogin $e) {
        }
