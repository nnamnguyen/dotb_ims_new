...
class DownloadFile {
     ...
     *
     * @param DotbBean $bean The DotbBean to get the file for
     * @param string $field The field name to get the file for
     * @param boolean $forceDownload force to download the file if true.
     */
    public function getFile(DotbBean $bean, $field, $forceDownload = false) {
     ...
     *
     * @param DotbBean $bean The bean to get the info for
    ...
    {
        if (!$bean instanceof DotbBean || empty($bean->id) || empty($bean->{$field})) {
            // @TODO Localize this exception message
            throw new Exception('Invalid DotbBean');
            return false;
        }
    ...
    public function getFileInfo($bean, $field) {
        if ($this->validateBeanAndField($bean, $field, 'file')) {
                    ...
                    // Quick existence check to make sure we are actually working
                    // on a real file
            if (!file_exists($filepath) && ($bean->doc_type == "Dotb" || empty($bean->doc_type))) {
                return false;
            }
     ...
     *
     * @param array $beans The list of DotbBean(s) to get the archive for
     * @param string $field The field name to get the file for
...
class DownloadFileApi extends DownloadFile
{
    ...
    {
        if(empty($info['path'])) {
            throw new DotbApiException('No file name supplied');
        }

        if (!empty($info['doc_type']) && $info['doc_type'] != "Dotb") {
