...
class DotbDependentDropdown {
	...
	function init($metadata) {
		if(is_string($metadata)) {
            ...
            if (file_exists($metadata)) {
				$dotbDependentDropdown = array();
				/*
				 * The metadata file should be prepped in an associative array.
				 * The array should be named "$dotbDependentDropdown".
				 *
				 * Examine:
				 * "include/DotbDependentDropdown/metadata/dependentDropdown.
				 * php" for a commented example.
				 */
				include($metadata); // provides $dotbDependentDropdown

				foreach($dotbDependentDropdown as $key => $type) {
					if(is_array($type)) {
						foreach($type as $k => $v) {
							if($k == 'elements') {
						...
						if(!$this->verifyMetadata($type)) {
							if($this->debugMode) {
								$this->debugOutput("DotbRouting: metadata initialization failed.  Please check your metadata source.");
							}
						}
					}
					$dotbDependentDropdown[$key] = $type;
				} // end foreach of metadata

				/* we made it through all checks so assign this to the output attribute */
				$this->metadata = $dotbDependentDropdown;
			}
			else {
				if($this->debugMode) $this->debugOutput("DotbRouting could not find file [ {$metadata} ]");
			}
		} // end is_string();
	} // end init()
	...
	function initElement($element, $alwaysMerge) {
		if($this->isValidElement($element)) {
			$mergedElement = dotbArrayMerge($this->defaults, $element);
			...
			if(isset($mergedElement['handlers'])) {
				if(is_array($mergedElement['handlers']) && !empty($mergedElement['handlers'])) {
					...
					if($this->debugMode) {
						$this->debugOutput("DotbRouting: element contains 'handlers' but is not an array:");
			...
			if($this->debugMode) {
				$this->debugOutput("DotbRouting is trying to initialize a non-element:");
				$this->debugOutput($element);
			}
		}
	}
		...
		if(isset($metadata['elements']) && !empty($metadata['elements']) && is_array($metadata['elements'])) {
			...
			foreach($elements as $indexName => $element) {
				...
				if(!isset($element['type']) && in_array($element['type'], $this->validTypes)) {
					if($this->debugMode) {
						$this->debugOutput("DotbRouting: valid 'type' not found:"); $this->debugOutput($element);
					}
					return false;
				}
				...
				switch($element['type']) {
					case "select":
						if(isset($element['values'])) {
							...
							if(is_array($element['values']) && !empty($element['values'])) {
								...
								foreach($element['values'] as $key => $value) {
									if(!array_key_exists($key, $nextElement['handlers'])) {
										if($this->debugMode) {
											$this->debugOutput("DotbRouting: next-order element is missing a handler for value: [ {$key} ]");
							...
							if($this->debugMode) {
								$this->debugOutput("DotbRouting: 'select' element found, no 'values' defined."); $this->debugOutput($element);
							}
							return false;
						}
				...
				if(isset($element['handlers']) && !empty($element['handlers'])) {
					...
					if(!$result) {
						if($this->debugMode) {
							$this->debugOutput("DotbRouting: metadata verification for 'handlers' failed: "); $this->debugOutput($fakeMetadata);
						}
						return false;
					}
				}
			...
			if($this->debugMode) {
				$this->debugOutput((count($metadata) > 1) ? "DotbRouting: all checks passed, valid metadata confirmed" : "DotbRouting: 'handlers' checks passed, valid metadata confirmed.");
			}
			return true;
		} else {
			if($this->debugMode) {
				$this->debugOutput("DotbRouting: Your metadata does not contain a valid 'elements' array:");$this->debugOutput($metadata);
			}
		}
		return false;
	}
