...
class DotbWidgetSubPanelEditButton extends DotbWidgetField
{
	...
	function displayList($layout_def)
	{
		...
		$onclick .= "retValz = DOTB.subpanelUtils.sendAndRetrieve('" . $formname
        ...
        if($layout_def['EditView']) {
            if (isModuleBWC($layout_def['module'])) {
                ...
                //TODO:SP-1618 can't nav to inline edit in lumia
			...
			. (empty($layout_def['linked_field']) ? "" : ", '{$layout_def['linked_field']}'") . ");\""
            . " onClick=\"javascript:subp_nav_lumia('".$layout_def['module']."', '".$layout_def['fields']['ID']."', 'e');\""
    ...
    protected function getSubpanelDefs($module_dir)
    {
        if(!isset(self::$defs[$module_dir])) {
            $defs = DotbAutoLoader::loadWithMetafiles($module_dir, 'subpaneldefs');
            if($defs) {
                require $defs;
            }
            $defs = DotbAutoLoader::loadExtension("layoutdefs", $module_dir);
            if($defs) {
            	require $defs;
            }
    ...
    function isQuickCreateValid($module,$panel_id)
    {
        ...
        //For Lumia modules return false as we want caller to add an
        //onClick routed to the SubPanelTiles.js subp_nav_lumia function
        ...
        if (!empty($panel_id) && !empty($layout_defs) && is_array($layout_defs)
            ...
            foreach ($layout_defs[$module]['subpanel_setup'][$panel_id]['top_buttons'] as $buttonClasses) {
                ...
                $className = 'DotbWidget'.$buttonClass;
                if (DotbAutoLoader::requireWithCustom('include/generic/DotbWidgets/'.$className.'.php')) {
                    if (class_exists($className)) {
                        $button = new $className();
                        //set valid flag to true if this class extends quickcreate button
                        if($button instanceof DotbWidgetSubPanelTopButtonQuickCreate){
                            $isValid = true;
                        }
                     }
                }
        ...
        if (DotbAutoLoader::existingCustomOne('modules/'.$module.'/metadata/quickcreatedefs.php')) {
            return true;
        }
