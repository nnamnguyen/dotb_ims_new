...
class DotbWidgetFieldCurrency extends DotbWidgetFieldInt
{
    ...
    public function displayList($layout_def)
    {
            ...
            if (empty($layout_def['group_function'])) {
                ...
                $currency = $current_user->getPreference('currency_show_preferred')
                    ? DotbCurrency::getUserLocaleCurrency()
                    : DotbCurrency::getBaseCurrency();
        ...
        if (!empty($record)) {
            ...
            global $dotb_config;
            if (isset ($dotb_config['enable_inline_reports_edit']) && $dotb_config['enable_inline_reports_edit']) {
                $str .= "&nbsp;" .DotbThemeRegistry::current()->getImage("edit_inline","border='0' alt='Edit Layout' align='bottom' onClick='DOTB.reportsInlineEdit.inlineEdit(\"$div_id\",\"$value\",\"$module\",\"$record\",\"$field_name\",\"$field_type\",\"$currency_id\",\"$symbol\");'");
            }
	        $str .= "</div>";
	        return $str;
        }
    ...
    public function displayListPlain($layout_def)
    {
        ...
        if (empty($layout_def['group_function']) && $current_user->getPreference('currency_show_preferred')) {
            $user_currency = DotbCurrency::getUserLocaleCurrency();

            if ($user_currency->id != $row_currency['currency_id']) {
                $value = DotbCurrency::convertAmount($value, $row_currency['currency_id'], $user_currency->id);
                $format_id = $user_currency->id;
            }
        }

        return DotbCurrency::formatAmountUserLocale($value, $format_id);
    }
