...
class DotbWidgetSubPanelTopButtonQuickCreate extends DotbWidgetSubPanelTopButton
{
    public function &_get_form($defines, $additionalFormFields = null)
    {
        ...
        //Revert bwc Create View if is is Hybrid Module - By Lap Nguyen
        if(!isModuleHybrid($defines['module'])){
        ...
        $button = '<form onsubmit="return DOTB.subpanelUtils.sendAndRetrieve(this.id, \'subpanel_' . $defines['subpanel_definition']->name . '\', \'' . addslashes($app_strings['LBL_LOADING']) . '\');" action="index.php" method="post" name="form" id="form' . $form . "\">\n";
