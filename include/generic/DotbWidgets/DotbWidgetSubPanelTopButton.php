...
class DotbWidgetSubPanelTopButton extends DotbWidget
{
    public $module;
    public $title;
    public $access_key;
    ...
    /**
     * Flag that determines if the requested subpanel should open a lumia quick
     * create form or a BWC one. This is set in {@see _get_form_lumia()}.
     * @var boolean
     */
    public $lumia;
...
// make it a member variable instead of passing it multiple layers with extra copying.
	...
	*/
     ...
     * lumia (not bwc) module. See SP-1630: Clicking Create from BWC subpanels
     * for lumia should open lumia create view.
     * @param $defines
     * @return a lumia compatable button or falsy
     */
    function _get_form_lumia($defines) {
        global $app_strings;
        global $subpanel_item_count;
        global $current_language;

        $lumiaReadySubPanelCreates = array(
        ...
        //purposes of determining whether to redirect to lumia create or not,
        //treat History (a bwc) as a Note (a lumia).
        ...
        if (!isModuleBWC($module) && in_array($defines['widget_class'], $lumiaReadySubPanelCreates)) {
            ...
            if ($this->enableActionMenu) {
                $button = '<form data-legacy-subpanel-create="1" action="index.php" method="post" name="form" id="' . $form . "\">\n".
                    "<a href='#' onClick=\"javascript:subp_nav_lumia(
                        '" . $module . "','" . $parentId . "','c', '" . $link . "'
                    );\"".
                    " class='create_from_bwc_to_lumia' id=\"$id\">". $label .'</a>';
            } else {
                $button = '<form data-legacy-subpanel-create="1" action="index.php" method="post" name="form" id="' . $form . "\">\n".
                    "<input type='button' onClick=\"javascript:subp_nav_lumia(
                        '" . $module . "','" . $parentId . "','c', '" . $link . "'
                    );\"".
                    " class='create_from_bwc_to_lumia' id=\"$id\" value=\"$label\">";
            }

            // Set the lumia flag so that calling codes knows what to do
            $this->lumia = true;
            return $button;
        }
        return false;
    ...
    public function display(array $defines, $additionalFormFields = array())
	{
         ...
         */
        if ( DotbWidget::isModuleHidden( !empty($this->module) ? $this->module : $defines['module'] ) )
        {
            return '';
        }
        ...
        if ( isset($_REQUEST['layout_def_key']) && $_REQUEST['layout_def_key'] == 'UserEAPM' ) {
            ...
            // $this->lumia is set in _get_form_lumia()
            if ($this->lumia !== true) {
