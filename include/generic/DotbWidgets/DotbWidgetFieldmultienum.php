...
class DotbWidgetFieldMultiEnum extends DotbWidgetFieldEnum {
    ...
    public function queryOrderBy($layout_def) {
        return DotbWidgetReportField::queryOrderBy($layout_def);
    }
    ...
    {
        $customModules = array();
        $customFiles = glob('modules/*/*_dotb.php', GLOB_NOSORT);
        foreach ($customFiles as $customFile) {
            $moduleName = str_replace('_dotb', '', pathinfo($customFile, PATHINFO_FILENAME));
