...
if (isset($_REQUEST['return_type'])  && $_REQUEST['return_type'] == 'report') {
    ...
    $focus = BeanFactory::getBean($_REQUEST['module'], $_REQUEST['record']);
 	...
 	if(!empty($_REQUEST['select_entire_list']) &&  $_REQUEST['select_entire_list'] != 'undefined' && isset($_REQUEST['current_query_by_page'])){
        ...
        if(empty($seed)) dotb_die($GLOBALS['app_strings']['ERROR_NO_BEAN']);
 		$where_clauses = '';
...
if ($refreshsubpanel) {
	...
	$GLOBALS['log']->debug("Location: index.php?dotb_body_only=1&module=".$_REQUEST['module']."&subpanel=".$_REQUEST['subpanel_module_name']."&action=SubPanelViewer&inline=1&record=".$_REQUEST['record']);
	if( empty($_REQUEST['refresh_page']) || $_REQUEST['refresh_page'] != 1){
		$inline = isset($_REQUEST['inline'])?$_REQUEST['inline']: $inline;
		header("Location: index.php?dotb_body_only=1&module=".$_REQUEST['module']."&subpanel=".$_REQUEST['subpanel_module_name']."&action=SubPanelViewer&inline=$inline&record=".$_REQUEST['record']);
	}
	exit;
}
