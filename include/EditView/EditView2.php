...
class EditView
{
    ...
    */
    ...
    {
        $this->idpConfig =  new IdmConfig(\DotbConfig::getInstance());
    }
    ...
    function setup($module, $focus = null, $metadataFile = null, $tpl = 'include/EditView/EditView.tpl', $createFocus = true)
    {
        ...
        if (empty($GLOBALS['dotb_config']['showDetailData']))
        {
        ...
        if (!empty($this->metadataFile) && DotbAutoLoader::existing($this->metadataFile)) {
    ...
    function render()
    {
        ...
        if ($this->view == 'EditView' && !empty($GLOBALS['dotb_config']['forms']['requireFirst'])){
            $this->requiredFirst();
        }
    ...
    {
        global $mod_strings, $dotb_config, $app_strings, $app_list_strings;
        ...
        {
            $this->th->ss->assign('PAGINATION', DotbVCR::menu($this->module, $this->offset, $this->focus->is_AuditEnabled(), ($this->view == 'EditView')));
        }
        ...
        if ($this->focus)
        {
            ...
            foreach ($this->focus->toArray() as $name => $value)
            {
                ...
                {
                    if(isset($GLOBALS['dotb_config']['enable_autocomplete']) && $GLOBALS['dotb_config']['enable_autocomplete'] == true)
                    {
    ...
    protected function getLockedFieldIcon()
    {
        ...
        return DotbThemeRegistry::current()->getImage(
            'lock.png',
