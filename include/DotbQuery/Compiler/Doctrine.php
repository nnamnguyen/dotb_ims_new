...
/**
 * Compiler of DotbQuery to Doctrine query builder
 */
class DotbQuery_Compiler_Doctrine
{
    /**
     * @var DBManager
     */
     ...
     *
     * @param DotbQuery $query The query being compiled
     *
     * @return QueryBuilder
     * @throws DotbQueryException
     */
    public function compile(DotbQuery $query)
    {
        $builder = $this->db->getConnection()
            ->createQueryBuilder();

        if ($query->union instanceof DotbQuery_Builder_Union) {
     ...
     * @throws DotbQueryException
     */
    protected function compileUnionQuery(QueryBuilder $builder, DotbQuery $query)
    {
        ...
        $sql = str_replace('SELECT DISTINCT ', $sql, $builder->getSQL());  // Fix bug UNION - DISTINCT - By Lap Nguyen
     ...
     * @throws DotbQueryException
     */
    protected function compileSelectQuery(QueryBuilder $builder, DotbQuery $query)
    {
     ...
     * @param DotbQuery $query The query being compiled
     */
    protected function compileSelect(QueryBuilder $builder, DotbQuery $query)
    {
     ...
     * @throws DotbQueryException
     */
    protected function compileFrom(QueryBuilder $builder, DotbQuery $query)
    {
        $bean = $query->getFromBean();
        if (!$bean) {
            throw new DotbQueryException('The primary bean is not specified');
        }
        ...
        // DotbQuery will determine if we actually need to add the table or not.
     ...
     * @throws DotbQueryException
     */
    protected function compileJoins(QueryBuilder $builder, DotbQuery $query)
    {
     ...
     *
     * @param QueryBuilder $builder Query builder
     * @param DotbQuery_Builder_Join $join Join specification
     *
     * @throws DotbQueryException
     */
    protected function compileJoin(QueryBuilder $builder, DotbQuery_Builder_Join $join)
    {
        if ($join->table instanceof DotbQuery
     ...
     * @param DotbBean $bean The bean whose erased fields need to be retrieved
     ...
     * @return bool
     */
    protected function isPiiFieldsSelected(DotbBean $bean, DotbQuery $query, string $tableAlias) : bool
    {
     ...
     * @return array
     */
    protected function getSelectFieldsByTable(DotbBean $bean, DotbQuery $query, string $tableAlias) : array
    {
        return array_merge(
     ...
     * @throws DotbQueryException
     */
    protected function compileWhere(QueryBuilder $builder, DotbQuery $query)
    {
        if ($query->shouldSkipDeletedRecords()) {
            $where = new DotbQuery_Builder_Andwhere($query);
            if ($query->where) {
     ...
     * @param DotbQuery $query The query being compiled
     */
    protected function compileGroupBy(QueryBuilder $builder, DotbQuery $query)
    {
     ...
     * @param DotbQuery $query The query being compiled
     */
    protected function compileHaving(QueryBuilder $builder, DotbQuery $query)
    {
        if ($query->having) {
            $builder->having(
     ...
     */
    protected function compileOrderBy(QueryBuilder $builder, DotbQuery $query, $applyOrderStability)
    {
     ...
     *
     * @param DotbQuery $query The query being compiled
     * @param DotbQuery_Builder_Orderby[] $orderBy List of already existing `ORDER BY` defs
     * @return DotbQuery_Builder_Orderby[]
     */
    protected function applyOrderByStability(DotbQuery $query, array $orderBy)
    {
        if (count($orderBy) == 0) {
            return $orderBy;
        }
        ...
        $uniqueCol = new DotbQuery_Builder_Orderby($query, end($orderBy)->direction);

        if ($firstColumn instanceof DotbQuery_Builder_Field_Raw) {
            return $orderBy;
        }
     ...
     * @param DotbQuery $query The query being compiled
     */
    protected function compileLimit(QueryBuilder $builder, DotbQuery $query)
    {
     ...
     *
     * @param DotbQuery_Builder_Field $field Field specification
     * @return string
     */
    protected function compileField(DotbQuery_Builder_Field $field)
    {
        if ($field instanceof DotbQuery_Builder_Field_Raw) {
            $sql = $field->field;
        } else {
            if ($field->isNonDb()) {
                return '';
     ...
     *
     * @param QueryBuilder $builder Query builder
     * @param DotbQuery_Builder_Where $expression
     * @return string|\Doctrine\DBAL\Query\Expression\CompositeExpression
     */
    protected function compileExpression(QueryBuilder $builder, DotbQuery_Builder_Where $expression)
    {
        $expressions = array();
        ...
        foreach ($expression->conditions as $condition) {
            if ($condition instanceof DotbQuery_Builder_Where) {
                $compiledField = $this->compileExpression($builder, $condition);
                if (count($compiledField) > 0) {
                    $expressions[] = $compiledField;
                }
            } elseif ($condition instanceof DotbQuery_Builder_Condition) {
     ...
     *
     * @param QueryBuilder $builder Query builder
     * @param DotbQuery_Builder_Condition $condition Condition
     * @return string|null
     *
     * @throws DotbQueryException
     */
    protected function compileCondition(QueryBuilder $builder, DotbQuery_Builder_Condition $condition)
    {
        global $current_user;
        ...
        if ($condition->isNull) {
            ...
            switch ($condition->operator) {
                ...
                default:
                    ...
                    if ($condition->values instanceof DotbQuery
                        || $condition->values instanceof QueryBuilder) {
                        $sql .= '(' . $this->compileSubQuery($builder, $condition->values) . ')';
                    } elseif ($condition->values instanceof DotbQuery_Builder_Field) {
     ...
     * @param string $operator Internal DotbQuery operator
     * @param DotbQuery|QueryBuilder|array|string $set
    ...
    {
        if ($set instanceof DotbQuery || $set instanceof QueryBuilder) {
     ...
     * @param DotbQuery|QueryBuilder $subQuery Subquery
     * @return string
     * @throws DotbQueryException
     */
    protected function compileSubQuery(QueryBuilder $builder, $subQuery)
    {
        if ($subQuery instanceof DotbQuery) {
