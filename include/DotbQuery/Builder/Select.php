...
class DotbQuery_Builder_Select
{
    /**
     * @var DotbQuery_Builder_Field_Select[]
     */
    public $select = array();
    ...
    public function field($columns)
    {
        ...
        foreach ($columns as $column) {
            $field = new DotbQuery_Builder_Field_Select($column, $this->query);
    ...
    {
        $field = new DotbQuery_Builder_Field_Raw($columns, $this->query);
        if (!empty($alias)) {
            $field->alias = $alias;
        } else {
            $alias = md5($columns);
