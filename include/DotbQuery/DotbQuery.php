...
class DotbQuery
{
     ...
     * @var null|DotbQuery_Builder_Select
     */
    public $select = null;

    /**
     * This is the Union Object
     * @var null|DotbQuery_Builder_Union
     */
    public $union = null;

    /**
     * @var DotbQuery_Builder_Groupby[]
     */
    ...
    {
        $this->select = new DotbQuery_Builder_Select($this, array());
     ...
     *
     * @return null|DotbQuery_Builder_Select
     */
     ...
     *
     * @param DotbQuery $select Query object.
     * @param bool $all (optional) Indicates if 'UNION ALL' should be used or not. Default is `true`.
     * @return DotbQuery_Builder_Union instance of union object.
     */
    public function union(DotbQuery $select, $all = true)
    {
        if (!is_object($this->union)) {
            $this->union = new DotbQuery_Builder_Union($this);
        }
     ...
     * @return DotbQuery
     */
    public function from(DotbBean $bean, $options = array())
    {
        ...
        if (!empty($alias)) {
            ...
            if (strtolower($alias) != $newAlias) {
                throw new DotbQueryException("From alias is more than the max allowed length for an alias");
            }
        }
    ...
    {
        if (isset($this->where)) {
            if (!$this->where instanceof DotbQuery_Builder_Andwhere) {
            ...
            $this->where = new DotbQuery_Builder_Andwhere($this);
        }
    ...
    {
        $where = new DotbQuery_Builder_Andwhere($this);
        $where->addRaw($sql);
        if (!isset($this->where)) {
            $this->where = new DotbQuery_Builder_Andwhere($this);
        }
    ...
    {
        if (isset($this->where)) {
            if (!$this->where instanceof DotbQuery_Builder_Orwhere) {
                throw new DotbQueryException(sprintf(
                    'Cannot change the top level WHERE operator from %s to %s',
                    $this->where->operator(),
                    'OR'
            ...
            $this->where = new DotbQuery_Builder_Orwhere($this);
        }
    ...
    public function joinTable($table, $options = array())
    {
        ...
        $join = new DotbQuery_Builder_Join($table, $options);
        $join->query = $this;
    ...
    public function join($link_name, $options = array())
    {
        ...
        if (!empty($options['favorites']) || $link_name == 'favorites') {
            $sfOptions = $options;
            $sf = BeanFactory::getDefinition('DotbFavorites');
            $options['alias'] = $sf->addToDotbQuery($this, $sfOptions);
        } else {
    ...
    public function joinSubpanel($bean, $link_name, $options = array())
    {
        ...
        if (!$bean->load_relationship($link_name)) {
            throw new DotbApiExceptionInvalidParameter("Unable to load link $link_name");
        }
        ...
        $bean->$link_name->buildJoinDotbQuery($this, $joinParams);
    ...
    public function setJoinOn($options = array())
    {
        if ($this->rname_link !== false && !empty($options['baseBeanId'])) {
            ...
            if ($field) {
                ...
                throw new DotbQueryException('Relationship Field Not Found');
            }
        }
    }
        ...
        if (!empty($this->group_by)) {
            ...
            foreach ($this->select->select as $selectFieldKey => $selectField) {
                ...
                if (empty($groupByCols[$selectField->table . '.' . $selectField->field])) {
                    ...
                    if (get_class($selectField) == 'DotbQuery_Builder_Field_Raw') {
    ...
    /**
     * Converts DotbQuery into Doctrine DBAL query
     *
     * @return \Doctrine\DBAL\Query\QueryBuilder
     */
    public function compile()
    {
        $compiler = new DotbQuery_Compiler_Doctrine($this->db);
    ...
    {
        $groupBy = new DotbQuery_Builder_Groupby($this);
    ...
    public function having($conditions)
    {
        if (isset($this->having)) {
            if (!$this->having instanceof DotbQuery_Builder_Andwhere) {
                throw new DotbQueryException(sprintf(
            ...
            $this->having = new DotbQuery_Builder_Andwhere($this);
        }
    ...
    {
        if (isset($this->having)) {
            if (!$this->having instanceof DotbQuery_Builder_Orwhere) {
                throw new DotbQueryException(sprintf(
                    'Cannot change the top level HAVING operator from %s to %s',
                    $this->where->operator(),
                    'OR'
            ...
            $this->having = new DotbQuery_Builder_Orwhere($this);
        }
    ...
    {
        $having = new DotbQuery_Builder_Andwhere($this);
    ...
    {
        $orderBy = new DotbQuery_Builder_Orderby($this, $direction);
    ...
    protected function rebuildFields()
    {
        if (!empty($this->select)) {
            foreach ($this->select->select as $field) {
                if ($field instanceof DotbQuery_Builder_Field) {
                ...
                if (!empty($joinObj->on)) {
                    foreach ($joinObj->on as $whereObj) {
                        ...
                        foreach ($whereObj->conditions as $conditionObj) {
                            if ($conditionObj->field instanceof DotbQuery_Builder_Field) {
        ...
        if (!empty($this->order_by)) {
            foreach ($this->order_by as $orderObj) {
                if ($orderObj->column instanceof DotbQuery_Builder_Field) {
        ...
        if (!empty($this->group_by)) {
            foreach ($this->group_by as $groupByObj) {
                if ($groupByObj->column instanceof DotbQuery_Builder_Field) {
    ...
    /**
     * Load Beans uses Link2 to take a DotbQuery object and add the joins needed to take a link and make the connection
     *
     * @param Linkname $join
     * @param $alias
     */
    protected function loadBeans($join, $options)
    {
        ...
        if (empty($bean->$join)) {
            throw new DotbApiExceptionInvalidParameter("Invalid link $join");
        }

        $bean->$join->buildJoinDotbQuery(
            $this,
            array(
                'joinTableAlias' => $alias,
                'joinType' => $joinType,
    ...
    public function getTableBean($table_name)
    {
        ...
        if (!isset($this->table_beans[$table_name])) {
            ...
            if ($link_name == 'favorites') {
                ...
                $bean = BeanFactory::getDefinition('DotbFavorites');
    ...
    /**
     * Returns the DotbBean Object that is the subject of this query.
     ...
     *
     * @return null|DotbQuery_Builder_Join
     */
     ...
     * @return string
     */
    public function getCustomTableAlias(DotbBean $bean, ?string $alias) : string
    {
        if (!empty($alias)) {
            return $alias . '_cstm';
        }
