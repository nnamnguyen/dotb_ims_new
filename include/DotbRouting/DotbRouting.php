...
class DotbRouting {
	...
	function save($rules) {
		...
		// need $sdd->metadata
		$sdd = new DotbDependentDropdown("include/DotbDependentDropdown/metadata/dependentDropdown.php");
		...
		Array(
			...
			(
				...
				(
				    [action0] => move_mail
				    [action1] => dotb::9e559ad1-a900-3c38-d846-464c931908be
				)
			    [100] => Array
				(
				    [action0] => move_mail
	...
	function setRuleStatus($id, $status) {
		...
		$GLOBALS['log']->fatal("DOTBROUTING: Could not save rule status [ {$status} ] for user [ {$this->user->user_name} ] rule [ {$id} ]");
	}
	...
	function saveRulesToFile() {
		...
		$GLOBALS['log']->info("DOTBROUTING: Saving rules file [ {$file} ]");
	...
	function preflightCache() {
		...
		$this->rulesCache = dotb_cached("routing/{$moduleDir}");
	 ...
	 * rule format:
		$routingRules['1'] = array(
			0 => array(
				'id' => 'xxxxxxxxxxxxxxxx',
				'name' => 'Move Email to Dotb Folder [test]',
				'active' => true,
				'all'	=> true,
				'criteria' => array(
					array(
	...
	function processRule($bean, $focusRule, $args) {
		if($this->_checkCriteria($bean, $focusRule)) {
			$GLOBALS['log']->debug("********** DOTBROUTING: rule matched [ {$focusRule['name']} ] -  processing action");
			return $this->_executeAction($bean, $focusRule, $args);
		} else {
			$GLOBALS['log']->debug("********** DOTBROUTING: rule not matched, not processing action");
			return false;
		}
	}
	 ...
	 * @param object $bean DotbBean to be manipulated
	 * @param mixed $args Extra arguments if needed
	 */
	function processRules($bean, $args=null) {
		if(empty($bean)) {
			$GLOBALS['log']->fatal("**** DOTBROUTING: processRules - invalid input object, returning false");
			return false;
		}

		$GLOBALS['log']->debug("**** DOTBROUTING: processing for [ {$bean->name} ]");

		// basic sandboxing
		if(!isset($this->rules) || empty($this->rules)) {
			$GLOBALS['log']->debug("**** DOTBROUTING: processRules - no rule defined, returning false");
			return false;
		}
	...
	function _executeAction($bean, $focusRule, $extraArgs) {
		...
		include_once("include/DotbRouting/baseActions.php");
		...
		foreach($focusRule['actions'] as $actionBox) {
			...
			if(function_exists($function)) {
				...
				$GLOBALS['log']->debug("********** DOTBROUTING: action eval'd [ {$customCall} ]");

				$ruleProcessed = eval($customCall);
			} else {
				$GLOBALS['log']->fatal("********** DOTBROUTING: action not matched [ {$function} ] - not processing action");
			}
		}
		if($ruleProcessed)
			$GLOBALS['log']->debug("DOTBROUTING: returning TRUE from _executeAction()");
		else
			$GLOBALS['log']->debug("DOTBROUTING: returning FALSE from _executeAction()");
	 ...
	 * @param object $bean DotbBean to process
	...
	function _checkCriteria(&$bean, &$focusRule) {
		if(!isset($focusRule['criteria'])) {
			$GLOBALS['log']->debug("********** DOTBROUTING: focusCriteria['criteria'] empty");
			return false;
		}
		...
		foreach($focusRule['criteria'] as $criteria) {
			if(is_array($criteria)) {
				...
				switch($crit['crit0']) {
					...
					case "priority_low":
						$GLOBALS['log']->debug("********* DOTBROUTING: got priority criteria");
						$flagged = ($bean->flagged == 1) ? 'priority_high' : 'priority_normal';

						// no match
						if($flagged != $crit['crit0']) {
							if($focusRule['all'] == true) {
								$GLOBALS['log']->debug("********** DOTBROUTING: 'ALL' flag found and crit field not matched: [ {$crit['crit0']} for bean of type {$bean->module_dir} ]");
								return false;
							}
						} else {
					...
					case "cc_addr":
					case "description":
						$GLOBALS['log']->debug("********* DOTBROUTING: got match-type criteria [ {$crit['crit1']} ]");
                        $GLOBALS['log']->debug("********* DOTBROUTING: matching [ {$crit['crit2']} to {$bean->{$crit['crit0']}} ]");
						switch($crit['crit1']) {
							/**
							...
							case "match":
                                ...
                                if (isset($bean->{$crit['crit0']})) {
									...
									if(!preg_match($regex, $field)) {
										if($focusRule['all'] == true) {
											$GLOBALS['log']->debug("********** DOTBROUTING: 'ALL' flag found and crit field not matched: [ {$crit['crit0']} -> {$crit['crit2']} for bean of type {$bean->module_dir} ]");
											$GLOBALS['log']->debug("********** DOTBROUTING: 'ALL' [ value: {$field} ] [ regex: {$regex} ]");
											return false;
										}
									} else {
									...
									$GLOBALS['log']->debug("********** DOTBROUTING: crit field not found: [ {$crit['crit0']} for bean of type {$bean->module_dir} ]");
								}
							break;
							...
							case "notmatch":
                                if (isset($bean->{$crit['crit0']})) {
									...
									if(preg_match($regex, $field)) {
										...
										if($focusRule['all'] == true) {
											$GLOBALS['log']->debug("********** DOTBROUTING: 'ALL' flag found and crit field not matched: [ {$crit['crit0']} -> {$crit['crit2']} for bean of type {$bean->module_dir} ]");
                                            $GLOBALS['log']->debug("********** DOTBROUTING: 'ALL' [ value: {$bean->{$crit['crit0']}} ] [ regex: {$regex} ]");
											return false;
										}
									} else {
										// got a match, return true if match 'any'
					 ...
					 */
					default:
						$GLOBALS['log']->debug("********** DOTBROUTING: criteria for rule does not match any rule definitions: [ {$crit['crit0']} ]");
					break;
				}
			}
		}
	...
	function getActionsDOM() {
		...
		$this->customActions = dotb_cached("routing/customActions.php");
	...
	function getRulesList() {
		...
		$ret = $smarty->fetch("include/DotbRouting/templates/rulesList.tpl");
	...
	function renderRuleRow($rule) {
		...
		$ret .= "<input class='input' id='{$rule['id']}' onclick='DOTB.routing.ui.setRuleStatus(this);' type='checkbox' name='activeRules[]' value='{$rule['id']}'{$active}>";
		$ret .= "</td>";
		$ret .= "<td style='padding:2px;' id='name{$rule['id']}'>";
		$ret .= "<a class='listViewThLinkS1' href='javascript:DOTB.routing.editRule(\"{$rule['id']}\")'>{$rule['name']}</a>";
		$ret .= "</td>";
		$ret .= "<td style='padding:2px;' id='remove{$rule['id']}'>";
		$ret .= "<a class='listViewThLinkS1' href='javascript:DOTB.routing.ui.deleteRule(\"{$rule['id']}\")'>";
		$ret .= DotbThemeRegistry::current()->getImage("minus", "class='img' border='0'", null, null, ".gif", $mod_strings['LBL_DELETE']);
		$ret .= "</td>";
		$ret .= "</tr>";
