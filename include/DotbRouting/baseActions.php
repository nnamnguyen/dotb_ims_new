 ...
 * The file should look like the following:
	...
	?>
 *****************************************************************************/
$file = dotb_cached("routing/customActions.php");
if(file_exists($file)) {
	include($file);
}
...
function delete_file($action, $file) {
	...
	if(file_exists($file)) {
		...
		if(strpos($file, $dotb_config['cache_dir']) !== false) {
			if(unlink($file)) {
				return true;
			}
		}
	...
	$GLOBALS['log']->error("*** DotbRouting: Rule delete_file did not complete successfully [ file: {$file} ].");
	return false;
}
...
function copy_mail($action, $bean, $ie, $copy=true) {
	...
	if($args[0] == 'dotb') {
		// we're dealing with a target Dotb Folder
		$folder_id = $args[1];

		$GLOBALS['log']->fatal("*** DOTBROUTING: dest folder is Dotb Folder");
		// destination is a Dotb folder
		$sf = new DotbFolder();
		...
		if($sf->retrieve($folder_id)) {
			...
			$GLOBALS['log']->fatal("*** DOTBROUTING: baseActions:copy_mail: could not retrieve DotbFolder with id [ {$folder_id} ]");
		}
	} else {
		$ieId = $args[1];
		$folder = "";
		...
		$GLOBALS['log']->fatal("*** DOTBROUTING: baseActions:copy_email [ {$folder} ] [ {$ieId} ] [ {$bean->uid} ]");
		$ie = BeanFactory::getBean('InboundEmail', $ieId, array('disable_row_level_security' => true));
		$GLOBALS['log']->fatal("*** DOTBROUTING: dest folder is IMAP Folder");
