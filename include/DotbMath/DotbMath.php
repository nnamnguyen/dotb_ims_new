 ...
 * echo floor(DotbMath::init(0.1)->add(0.7)->mul(10)->result());
 *
 * DotbMath can also process math expressions. Expressions have an advantage over
 ...
 * Examples:
 *
 * DotbMath::init()->exp("1+2/3*(4+5)^2")->result();
 * // reusable expression with params
 * DotbMath::init()->exp("1+?-4*(?+?)",array($p1,$p2,$p3))->result();
 *
 */
class DotbMath
{
     ...
     * example: $foo = DotbMath::init(500,2)->add(45)->mul(3);
     *
     * @param  mixed $value
     * @param  int   $scale
     * @return DotbMath
    ...
    protected function _applyOperation($operator, $params = null)
    {
        switch ($operator) {
            ...
            case 'comp':
                ...
                break;
            default:
                throw new DotbMath_Exception("unknown operator '{$operator}'");
                break;
        }
    }
        ...
        switch ($type) {
            ...
            default:
                if (!is_numeric($value)) {
                    ...
                    throw new DotbMath_Exception("{$message}");
                }
                break;
            case 'int':
            case 'intpos':
    ...
    public function exp($exp, $args = array())
    {
        ...
        if (!is_string($exp)) {
            throw new DotbMath_Exception('expression must be a string');
        }
        if (!is_array($args)) {
            throw new DotbMath_Exception('expression args must be an array');
        }
        if (count($args) > 0) {
            foreach ($args as $arg) {
        ...
        if (substr_count($exp, '?') !== count($args)) {
            throw new DotbMath_Exception('number of args mismatch number of ? in exp');
        }
        // expression parenthesis must be balanced
        if (substr_count($exp, '(') !== substr_count($exp, ')')) {
            throw new DotbMath_Exception('parenthesis mismatch');
        }
        ...
        for ($x = 0, $y = strlen($exp); $x < $y; $x++) {
            ...
            if ($char == '?') {
                ...
                if (!$isAfterOperand && !in_array($char, array('+', '-'))) {
                    throw new DotbMath_Exception("grouped operators error");
                }
                ...
                while (($last_op = array_pop($stack))) {
                    if ($last_op === null) {
                        ...
                        throw new DotbMath_Exception('exp() unbalanced parenthesis');
                    } elseif ($last_op == '(') {
                ...
                if (!in_array($char, array(' '))) {
                    throw new DotbMath_Exception('invalid expression syntax');
                }
            }
        }
