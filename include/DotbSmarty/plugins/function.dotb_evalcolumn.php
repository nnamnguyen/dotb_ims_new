...
/**
 * Smarty {dotb_evalcolumn} function plugin
 *
 * Type:     function<br>
 * Name:     dotb_evalcolumn<br>
 ...
 * @param Smarty
 */
function smarty_function_dotb_evalcolumn($params, &$smarty)
{
    ...
    if (!empty($params['var']['assign'])) {
        ...
        // we can't do this through customCode directly because the dotb_field smarty function returns smarty code to run on the second pass
        if (!empty($code) && strpos($code,'@@FIELD@@') !== FALSE ) {
            ...
            $fieldText = smarty_function_dotb_field($sfp, $smarty);
