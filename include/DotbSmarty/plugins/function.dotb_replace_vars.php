 ...
 * @return unknown
 */
function smarty_function_dotb_replace_vars($params, &$smarty)
{
	if(empty($params['subject']))  {
	    $smarty->trigger_error("dotbvar: missing 'subject' parameter");
	    return;
	} 
	$fields = empty($params['fields']) ? $smarty->get_template_vars('fields') : $params['fields'];
    $subject = replace_dotb_vars($params['subject'], $fields, !empty($params['use_curly']));
