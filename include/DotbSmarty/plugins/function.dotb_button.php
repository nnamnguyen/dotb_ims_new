...
r51275 - 2011-03-15 - jpark - Author: Justin Park<jpark@dotbcrm.com>
    #51275: Add appendTo on dotbbutton to keep the list of buttons to generate rendering list

r58076 - 2010-09-03 10:29:58 -0700 (Fri, 03 Sep 2010) - kjing - Author: Rob Aagaard <rob@dotbcrm.com>
    #39462: Fix the Shortcut bar quick create full form button to cancel the confirm navigation alert for that form.

r57805 - 2010-08-18 16:11:22 -0700 (Wed, 18 Aug 2010) - kjing - Author: Rob Aagaard <rob@dotbcrm.com>
...
r57466 - 2010-07-14 17:38:01 -0700 (Wed, 14 Jul 2010) - kjing - Author: dwheeler@dotbcrm.com <dwheeler@dwheeler-laptop>
...
Author: dwheeler@dotbcrm.com <dwheeler@dwheeler-laptop.(none)>
    Merge branch 'master' of git+ssh://github.com/dotbcrm/Mango

r57317 - 2010-07-08 07:01:39 -0700 (Thu, 08 Jul 2010) - kjing - Author: Rob Aagaard <rob@dotbcrm.com>
...
The new mechanism works out from the save or cancel button, respectively, until it encounters a subpanel (marked by a new CSS class 'quickcreate'). It then uses that subpanel name in later operations. This mechanism is now reliable for relationship-based subpanel names. Furthermore, it does not require modifications to the method-chain. To fix this issue by passing the subpanel name along the method-chain would require changing the method signature of the smarty method, dotb_button, which would be very broad-ranging.
...
Spoke with Ran more about this issue. It turns out the "Select" button was not the only button that needed to have a unique id. All subpanel and subpanel quick create buttons where no unique id exists for the element are candidates for change. Modified dotb widget subpanel classes to use a unique id for "Create" and "Select" buttons. Modified function.dotb_button.php to uniquely identify subpanel buttons as well.
...
Need to add code to set the return_id value in include/DotbSmarty/plugins/function.dotb_button.php when clicking on the Duplicate button so that this value is passed into the EditView and a cancel operation can return the user to the previous screen correctly.
Modified:
include/DotbSmarty/plugins/function.dotb_button.php
...
Modified function.dotb_button.php to make extra checks to see if the return action is set to DetailView, that a valid id value is set.  Otherwise, there's no way to pull up the DetailView contents.
Modified:
include/DotbSmarty/plugins/function.dotb_button.php
...
Fixed include/DotbSmarty/plugins/function.dotb_button.php file.   The button for Audit log was being created with the $_REQUEST id.  However, this value was built for the first request so subsequent requests to view the Audit log would always go back to the id built in the cached .tpl file.  Modified to use {$fields.id.value} instead.
Modified:
Fixed include/DotbSmarty/plugins/function.dotb_button.php
...
r25408 - 2007-08-10 18:12:47 -0700 (Fri, 10 Aug 2007) - clee - Fix for 14268.  Removed the audit link from the VCR Control row and moved to the DetailView/EdtiView header.tpl files.  The link is shown for all modules IF the DotbBean's isAuditEnabled value is set to true.
Updated files:
include/DetailView/header.tpl
include/EditView/EditView2.php
include/EditView/header.tpl
include/EditView/DotbVCR.php
include/DotbSmarty/plugins/function.dotb_button.php
include/DotbFields/Parsers/MetaParser.php
...
r24531 - 2007-07-23 01:25:31 -0700 (Mon, 23 Jul 2007) - clee - Fix for 13778.  Changed include/DotbSmarty/plugins/function.dotb_button.php so that FIND_DUPLICATE button will set return_action to "DetailView" (used only in Contacts and Accounts DetailView Meta-data files).
 ...
 * {{dotb_button module='{{$module}}' id='{{$form.buttons[$id]}}' view='EditView'}}
 *
 *
 * @author Collin Lee {clee@dotbcrm.com}
 */

/**
 * smarty_function_dotb_button
 ...
 */
function smarty_function_dotb_button($params, $smarty)
{
   if(empty($params['module'])) {
   	  $smarty->trigger_error("dotb_button: missing required param (module)");
   } else if(empty($params['id'])) {
   	  $smarty->trigger_error("dotb_button: missing required param (id)");
   } else if(empty($params['view'])) {
   	  $smarty->trigger_error("dotb_button: missing required param (view)");
   }
   ...
   $location = (empty($params['location'])) ? "" : "_".$params['location'];
    ...
    if (isset($GLOBALS['dotb_config']['enable_action_menu']) && $GLOBALS['dotb_config']['enable_action_menu']===false) {
   ...
   if(!is_array($type)) {
   	  ...
   	  switch(strtoupper($type)) {
			case "SEARCH":
                $output = '<input tabindex="2" title="{$APP.LBL_SEARCH_BUTTON_TITLE}" onclick="DOTB.savedViews.setChooser();" class="button" type="submit" name="button" value="{$APP.LBL_SEARCH_BUTTON_LABEL}" id="search_form_submit"/>&nbsp;';
			break;
			...
			case "CANCEL":
                ...
                $cancelButton .= 'parent.DOTB.App.router.buildRoute(\'{$smarty.request.return_module|escape:"url"}\', \'{$fields.id.value|escape:"url"}\', \'{$smarty.request.return_action|escape:"url"}\')';
                $cancelButton .= '{elseif !empty($smarty.request.return_module) || !empty($smarty.request.return_action) || !empty($smarty.request.return_id)}';
                $cancelButton .= 'parent.DOTB.App.router.buildRoute(\'{$smarty.request.return_module|escape:"url"}\', \'{$smarty.request.return_id|escape:"url"}\', \'{$smarty.request.return_action|escape:"url"}\')';
                $cancelButton .= '{else}';
                $cancelButton .= "parent.DOTB.App.router.buildRoute('$module')";
                $cancelButton .= '{/if}';
                $cancelButton .= '{/capture}';
                $cancelButton .= '<input title="{$APP.LBL_CANCEL_BUTTON_TITLE}" accessKey="{$APP.LBL_CANCEL_BUTTON_KEY}" class="button" onclick="parent.DOTB.App.bwc.revertAttributes();parent.DOTB.App.router.navigate({$cancelReturnUrl}, {literal}{trigger: true}{/literal}); return false;" type="button" name="button" value="{$APP.LBL_CANCEL_BUTTON_LABEL}" id="' . $type . $location . '"> ';
                $output = $cancelButton;
			break;

			case "DELETE":
                $output = '{if $bean->aclAccess("delete")}<input title="{$APP.LBL_DELETE_BUTTON_TITLE}" accessKey="{$APP.LBL_DELETE_BUTTON_KEY}" class="button" onclick="'.$js_form.' _form.return_module.value=\'' . $module . '\'; _form.return_action.value=\'ListView\'; _form.action.value=\'Delete\'; if(confirm(\'{$APP.NTC_DELETE_CONFIRMATION}\')) DOTB.ajaxUI.submitForm(_form);" type="submit" name="Delete" value="{$APP.LBL_DELETE_BUTTON_LABEL}" id="delete_button">{/if} ';
            break;
			...
			case "SUBPANELSAVE":
                ...
                $output = '{if $bean->aclAccess("save")}<input title="{$APP.LBL_SAVE_BUTTON_TITLE}"  class="button" onclick="'.$js_form.' _form.action.value=\'Save\';if(check_form(\''.$view.'\'))return DOTB.subpanelUtils.inlineSave(_form.id, \'' . $params['module'] . '_subpanel_save_button\');return false;" type="submit" name="' . $params['module'] . '_subpanel_save_button" id="' . $params['module'] . '_subpanel_save_button" value="{$APP.LBL_SAVE_BUTTON_LABEL}">{/if} ';

            break;
			case "SUBPANELCANCEL":
                $output = '<input title="{$APP.LBL_CANCEL_BUTTON_TITLE}" class="button" onclick="return DOTB.subpanelUtils.cancelCreate($(this).attr(\'id\'));return false;" type="submit" name="' . $params['module'] . '_subpanel_cancel_button" id="' . $params['module'] . '_subpanel_cancel_button" value="{$APP.LBL_CANCEL_BUTTON_LABEL}"> ';
			...
			case "CONNECTOR":
				...
				return '';
            case "PDFVIEW":
                $output='{dotb_button module="$module" id="REALPDFVIEW" view="$view" form_id="formDetailView" record=$fields.id.value}';
                break;
            case "REALPDFVIEW":
                if(empty($params['record'])) {
                    $smarty->trigger_error("dotb_button: missing required param (record)");
                }
                ...
                if (!empty($pdfManagerList) || !empty($tplLayouts)) {
                    if(DotbThemeRegistry::current()->name != "Classic") {
                        ...
                        $output = '
                            ...
                            //quote legacy templates
                            if($module == "Quotes") {
                                foreach($tplLayouts as $dotbpdf=>$path) {
                                    if (!$enable_action_menu) {
                                        $urlParams[] = array(
                                            'module' => $module,
                                            'record' => $record,
                        ...
                        if (!$enable_action_menu) {
                            if (!empty($urlParams)) {
                                ...
                                $output .= '<li class="dotb_action_button">';
                                ...
                                foreach ($urlParams as $tplButton) {
                                    if (isset($tplButton['pdf_template_id'])) {
                                        $parentLocation = 'index.php?module='.$module.'&record='.$tplButton['record'].'&action='.$tplButton['action'].'&dotbpdf='.$tplButton['dotbpdf'].'&pdf_template_id='.$tplButton['pdf_template_id'];
                                        $output .= '<li><a id="'.$tplButton['name'].'_pdfview" '. 'href="'.$parentLocation.'">'.$tplButton['name'].'</a></li>';
                                    } else {
                                        // legacy templates
                                        $parentLocation = 'index.php?module='.$module.'&record='.$tplButton['record'].'&action='.$tplButton['action'].'&dotbpdf='.$tplButton['dotbpdf'].'&email_action='.$tplButton['email_action'];
                            ...
                            $output .= smarty_function_dotb_menu(array(    'id'                    => "pdfview_action_menu",
                                                                            'items'                 => $pdfItems,
                        ...
                        $output = '
                            <script type="text/javascript">
                                ...
                                $output .= '\';
                                DOTB.util.showHelpTips(el,menu);
                                }
                            </script>
                ...
                break;
            case "PDFEMAIL":
                $output='{dotb_button module="$module" id="REALPDFEMAIL" view="$view" form_id="formDetailView" record=$fields.id.value}';
                break;
            case "REALPDFEMAIL":
                $output = '';
                    ...
                    if(DotbThemeRegistry::current()->name != "Classic") {
                        ...
                        if (!$enable_action_menu) {
                            if (!empty($urlParams)) {
                                ...
                                foreach ($urlParams as $tplButton) {
                                    if (isset($tplButton['pdf_template_id'])) {
                                        $parentLocation = 'index.php?module='.$module.'&record='.$tplButton['record'].'&action='.$tplButton['action'].'&dotbpdf='.$tplButton['dotbpdf'].'&pdf_template_id='.$tplButton['pdf_template_id'].'&to_email='.$tplButton['to_email'];
                                ...
                                foreach($pdfManagerList as $pdfTemplate){
                                    ...
                                    if($module == "Quotes") {
                                        ...
                                        foreach($tplLayouts as $dotbpdf=>$path) {
                                            ...
                                            $output .= '<a style="width: 150px" class="menuItem" onmouseover="hiliteItem(this,\\\'yes\\\');" onmouseout="unhiliteItem(this);" onclick="" href="index.php?' . http_build_query($urlParams, '', '&') . '">' . $dotbpdf . '</a>' ;
                                        }
                                    }
...
<input title="{\$APP.LBL_SHARE_BUTTON_TITLE}" accessKey="{\$APP.LBL_SHARE_BUTTON_KEY}"
  class="button" onclick="parent.DOTB.App.bwc.shareRecord('{$params['module']}', '{\$fields.id.value}', this.form.share_button_name.value)" type="button" name="button" value="{\$APP.LBL_SHARE_BUTTON_LABEL}">
</form>
ENDB;
              $output = $shareButton;
              break;
      ...
      return $output;
   } else if(is_array($type) && isset($type['dotb_html'])) {

       $dom_tree = DotbHtml::parseDotbHtml($type['dotb_html']);
       replaceFormClick($dom_tree, $js_form);
       $output = DotbHtml::createHtml($dom_tree);
       ...
       $dom_tree = DotbHtml::parseHtmlTag($type['customCode']);
       $hidden_exists = false;
       ...
       if($hidden_exists) {
           ...
           $form['hidden'][] = DotbHtml::createHtml($hidden_fields);
 ...
 *
 * @param array $dom_tree - Cascade array form generated by DotbHtml::parseHtmlTag
...
function replaceFormClick(&$dom_tree = array(), $js_form = '', &$hidden_field_exists = false) {
    ...
    //Replace the JS syntax where the dotb_button contains the event handler for this.form
