...
Modified DotbFieldBase file, displayFromFunc function to propagate the tabindex down the call stack and the email address widget to utilize the value
...
r54369 - 2010-02-08 16:33:57 -0800 (Mon, 08 Feb 2010) - rob - Bug 35453: Add support for vardef functions to return various html, bypassing the dotb fields
...
/**
 * Smarty {dotb_field} function plugin
 *
 * Type:     function
 * Name:     dotb_run_helper
 * Purpose:  Runs helper functions as defined in the vardef for specific fields
 * 
 * @author Rob Aagaard {rob at dotbcrm.com}
 * @param array
 * @param Smarty
 */

function smarty_function_dotb_run_helper($params, &$smarty)
{
    $error = false;
    
    if(!isset($params['func'])) {
        $error = true;
        $smarty->trigger_error("dotb_field: missing 'func' parameter");
    }
    if(!isset($params['displayType'])) {
        $error = true;
        $smarty->trigger_error("dotb_field: missing 'displayType' parameter");
    }
