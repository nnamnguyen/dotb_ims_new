 ...
 */
function smarty_function_dotb_button_slider($params, &$smarty)
{
   if(empty($params['module'])) {
   	  $smarty->trigger_error("dotb_button_slider: missing required param (module)");
   } else if(empty($params['buttons'])) {
   	  $smarty->trigger_error("dotb_button_slider: missing required param (buttons)");
   } else if(empty($params['view'])) {
   	  $smarty->trigger_error("dotb_button_slider: missing required param (view)");
   }
   ...
   if(is_array($buttons)) {
   	  if(count($buttons) <= 2){
   	  	foreach($buttons as $val => $button){
   	  		$str .= smarty_function_dotb_button(array('module' => $module, 'id' => $button, 'view' => $view), $smarty);
   	  	}
   	  }else{
