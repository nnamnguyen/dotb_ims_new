...
- Added DotbTheme and DotbThemeRegistry objects, updating everywhere in the app to use them.
- Converted the Dotb Theme to the new style, which involved:
 - moved all PHP and HTML out of the themes, into DotbView or the include/utils/layout_utils.php directory.
...
/**
 * Smarty {dotb_include} function plugin
 *
 * Type:     function<br>
 * Name:     dotb_include<br>
 ...
 * @param Smarty
 */
function smarty_function_dotb_getimagepath($params, &$smarty)
{
	if(!isset($params['file'])) {
		   $smarty->trigger_error($GLOBALS['app_strings']['ERR_MISSING_REQUIRED_FIELDS'] . 'file');
	}
 	return DotbThemeRegistry::current()->getImageURL($params['file']);
}
?>