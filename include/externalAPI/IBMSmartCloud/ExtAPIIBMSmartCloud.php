...
class ExtAPIIBMSmartCloud extends OAuthPluginBase implements WebMeeting,WebDocument {
    ...
    function __construct() {
        if ( isset($GLOBALS['dotb_config']['ll_base_url']) ) {
            $this->url = $GLOBALS['dotb_config']['ll_base_url'];
        }
    ...
    function scheduleMeeting($bean) {
        $bean->join_url = $this->api_data['joinURL'].'&TagCode=DotbCRM&TagID='.$bean->id;
        $bean->host_url = $this->api_data['hostURL'].'?TagCode=DotbCRM&TagID='.$bean->id;
