...
class OAuthPluginBase extends ExternalAPIBase implements ExternalOAuthAPIPlugin {
    ...
    {
        $this->setupOauthKeys();
        $oauth = new DotbOAuth($this->oauthParams['consumerKey'], $this->oauthParams['consumerSecret'], $this->getOauthParams());
   ...
   public function oauthLogin()
   {
        ...
        if($stage == 0) {
            ...
            $callback_url = $dotb_config['site_url'].'/index.php?module=EAPM&action=oauth&record='.$this->eapmBean->id;
            ...
            if(empty($request_token_info['oauth_token_secret']) || empty($request_token_info['oauth_token'])){
                ...
                DotbApplication::redirect("{$authReq}?oauth_token={$request_token_info['oauth_token']}");
            }
        } else {
