...
class ExternalAPIFactory
{
    ...
    public static function loadFullAPIList($forceRebuild=false, $ignoreDisabled = false) {
        ...
        $cached=dotb_cached('include/externalAPI.cache.php');
        ...
        foreach ( $apiFullList as $apiName => $apiOpts ) {
            ...
            // Special handling for the show/hide of the Meeting Password field, we need to create a dropdown for the Dotb Logic code.
        ...
        $cached_tmp = dotb_cached('include/externalAPI.cache-tmp.php');
        ...
        $fd = fopen(dotb_cached('include/externalAPI.cache-tmp.js'),'w');
        fwrite($fd,"//This file is auto generated by ".basename(__FILE__)."\nDOTB.eapm = ".json_encode($apiFullList).";\n\n");
        fclose($fd);
        rename(dotb_cached('include/externalAPI.cache-tmp.js'),dotb_cached('include/externalAPI.cache.js'));
    ...
    public static function clearCache() {
        ...
        $cached=dotb_cached('include/externalAPI.cache.js');
        if ( file_exists($cached) ) {
            unlink($cached);
        }
    }
