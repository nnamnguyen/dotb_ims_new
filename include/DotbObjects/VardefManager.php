...
class VardefManager{
    ...
    /**
     * this method is called within a vardefs.php file which extends from a DotbObject.
     * It is meant to load the vardefs from the DotbObject.
     */
    static function createVardef($module, $object, $templates = array('default'), $object_name = false)
    {
        ...
        {
            $vardef_paths = array(
                DotbAutoLoader::loadExtension("vardefs", $module),
    ...
    {
        return DotbAutoLoader::existingCustom(
            'include/DotbObjects/templates/' . $template . '/vardefs.php',
            'include/DotbObjects/implements/' . $template . '/vardefs.php'
        );
    }
    ...
    {
        // We will search in base and custom DotbObjects templates
        $paths = array(
            'include/DotbObjects/templates/',
            'custom/include/DotbObjects/templates/',
        );
    ...
    public static function addTemplate($module, $object, $template, $object_name = false, $skipUses = false)
    {
        ...
        if (!empty($templates[$template])) {
            ...
            // maintain a record of this objects inheritance from the DotbObject templates...
    ...
    public static function saveCache($module, $object)
    {
        ...
        $sc = self::$dotbConfig ?: self::$dotbConfig = DotbConfig::getInstance();
        if ($sc->get('noFilesystemMetadataCache', false)) {
            ...
            dotb_file_put_contents_atomic($file, $out);
        }
    }
     ...
     * @param string object_name the name of the object we are clearing this is for dotb_cache
     */
    ...
    protected static function _clearCache($module_dir = '', $object_name = '')
    {
        if(!empty($module_dir) && !empty($object_name)){
            ...
            if ($sc->get('noFilesystemMetadataCache', false)) {
                ...
                $file = dotb_cached(self::getCacheFileName($module_dir, $object_name));
                if (file_exists($file)) {
                    unlink($file);
                }
            }
    ...
    public static function refreshVardefs(
        ...
        // some tests do new DotbBean(), we can't do much with it here.
        ...
        if (empty($dictionary[$object]) || empty($params['related_calc_fields_only'])) {
            if ($includeExtension === true) {
                $vardef_paths = array(
                    ...
                    DotbAutoLoader::loadExtension("vardefs", $moduleDir),
            ...
            $found = false;
            //search a predefined set of locations for the vardef files
            foreach(DotbAutoLoader::existing($vardef_paths) as $path){
                require($path);
                $found = true;
            }
            ...
            if ($found){
                ...
                self::addDotbACLStatic($object, $module);
            }
        ...
        if (!empty($dictionary[$object])) {
            ...
            DotbBean::clearLoadedDef($object);
        }
        unset(self::$inReload[$guard_name]);
    }

    /**
     * Add default DotbACLStatic
     *
     * @param string $object Object name
     * @param string $module Module name
     */
    protected static function addDotbACLStatic($object, $module)
    {
        global $dictionary;
        // Put ACLStatic into vardefs for beans supporting ACLs
        if(!empty($dictionary[$object]) && !isset($dictionary[$object]['acls']['DotbACLStatic'])){
            ...
            if ($class) {
                ...
                if (($instance instanceof DotbBean) && $instance->bean_implements('ACL')) {
                    $dictionary[$object]['acls']['DotbACLStatic'] = true;
                    DotbACL::resetACLs($module);
                }
            }
        }
    }
    ...
    public static function getLinkFieldForRelationship($module, $object, $relName, $byLinkName = false)
    {
        ...
        $cacheValue = dotb_cache_retrieve($cacheKey);
        if(!empty($cacheValue)) {
            return $cacheValue;
        }
        ...
        dotb_cache_put($cacheKey, $results);
        return $results;
    }
        ...
        foreach($linkFields as $name => $def)
        {
            ...
            if (!empty($def['module']))
                ...
                if (!empty($dictionary[$relName]['relationships'][$relName]))
                    ...
                    $relDef = DotbRelationshipFactory::getInstance()->getRelationshipDef($relName);
                    if (!$relDef)
                        continue;
                }
     ...
     * developers.dotbcrm.com website to find articles relating to the use of this field.
     *
    ...
    {
        if (isset($GLOBALS['dotb_config']['require_accounts']))
        {
            ...
            {
                $vardef['fields']['account_name']['required'] = $GLOBALS['dotb_config']['require_accounts'];
            }
    ...
    protected static function loadFromCache($module, $object) {
        ...
        if ($sc->get('noFilesystemMetadataCache', false)) {
            ...
            $cachedfile = dotb_cached(self::getCacheFileName($module, $object));
