...
class Person extends Basic
{
     ...
     * @inheritDoc
     */
    public function populateRelatedBean(DotbBean $newbean)
    {
    ...
    public function getFreeBusySchedule(array $options = array())
    {
        ...
        foreach ($lines as $line) {
            if (preg_match('/^FREEBUSY.*?:([^\/]+)\/([^\/]+)/i', $line, $matches)) {
                $datesArray = array(
                    DotbDateTime::createFromFormat(vCal::UTC_FORMAT, $matches[1], $utc),
    ...
    protected function getVCalData(array $options)
    {
        ...
        $useFreeBusyCache = !empty($dotb_config['freebusy_use_vcal_cache']);

        $vcalBean = BeanFactory::newBean('vCals');
        if (!$useFreeBusyCache && !empty($options['start']) && !empty($options['end'])) {
            $dotbDateTimeStart = $timedate->fromIso($options['start']);
            $dotbDateTimeEnd = $timedate->fromIso($options['end']);
            $vcalData = $vcalBean->get_vcal_freebusy($this, false, $dotbDateTimeStart, $dotbDateTimeEnd);
        } else {
