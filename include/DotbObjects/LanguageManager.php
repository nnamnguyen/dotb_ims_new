...
class LanguageManager
{
    ...
    public static function createLanguageFile($module, $templates = array('default'), $refresh = false)
    {
        ...
        if(empty($lang))
            $lang = $GLOBALS['dotb_config']['default_language'];
		
		if(empty(self::$createdModules[$module]) && ($refresh || !file_exists(dotb_cached('modules/').$module.'/language/'.$lang.'.lang.php'))){
    ...
    public static function loadTemplateLanguage($module, $templates, $lang, $loaded_mod_strings)
    {
		...
		foreach($templates as $template){
			...
			$loaded_mod_strings = dotbLangArrayMerge($loaded_mod_strings, $temp);
		}
    ...
    public static function addTemplate($module, $lang, $template)
    {
		...
		if(empty($templates[$template])){
		    foreach(DotbAutoLoader::existing(
		        'include/DotbObjects/templates/' . $template . '/language/'.$lang.'.lang.php',
		        'include/DotbObjects/implements/' . $template . '/language/'.$lang.'.lang.php'
		    ) as $path) {
		        require($path);
    ...
    public static function saveCache($module, $lang, $loaded_mod_strings, $additonal_objects = array())
    {
		...
		// put the item in the dotb cache.
		$key = self::getLanguageCacheKey($module,$lang);
		dotb_cache_put($key,$loaded_mod_strings);
	 ...
	 * @param string lang the name of the object we are clearing this is for dotb_cache
	 */
    public static function clearLanguageCache($module_dir = '', $lang = '')
    {
		if(empty($lang)) {
			$languages = array_keys($GLOBALS['dotb_config']['languages']);
		} else {
	 ...
	 * @param string lang the name of the language file we are clearing this is for dotb_cache
	 */
    private static function _clearCache($module_dir = '', $lang)
    {
		if(!empty($module_dir) && !empty($lang)){
			$file = dotb_cached('modules/').$module_dir.'/language/'.$lang.'.lang.php';
			if(file_exists($file)){
				unlink($file);
    ...
    public static function refreshLanguage(
		...
		foreach(DotbAutoLoader::existing($lang_paths) as $path){
		    require($path);
            if(!empty($mod_strings)){
			    if (function_exists('dotbArrayMergeRecursive')){
				    $loaded_mod_strings = dotbArrayMergeRecursive($loaded_mod_strings, $mod_strings);
				} else{
					$loaded_mod_strings = dotbLangArrayMerge($loaded_mod_strings, $mod_strings);
				}
			}
		}
    ...
    public static function loadModuleLanguage($module, $lang, $refresh = false)
    {
		...
		if(!$refresh)
		{
			$return_result = dotb_cache_retrieve($key);
		...
		// Some of the vardefs do not correctly define dictionary as global.  Declare it first.
		$cachedfile = dotb_cached('modules/').$module.'/language/'.$lang.'.lang.php';
		...
		if(file_exists($cachedfile)){
			...
			if(!empty($mod_strings))
				dotb_cache_put($key,$mod_strings);
    ...
    {
        $jsFiles = array();
        getFiles($jsFiles, dotb_cached('jsLanguage'));
    ...
    public static function invalidateJsLanguageCache()
    {
        ...
        if (empty($dotb_config['js_lang_version'])) {
            $dotb_config['js_lang_version'] = 1;
        } else {
            $dotb_config['js_lang_version']++;
        }

        rebuildConfigFile($dotb_config, $dotb_version);
    }
	...
	public static function getEnabledAndDisabledLanguages()
	{
	    ...
	    $disabled_list = array();
	    if (isset($dotb_config['disabled_languages'])) {
	        if(!is_array($dotb_config['disabled_languages'])){
	            $disabled_list = array_flip(explode(',', $dotb_config['disabled_languages']));
	        } else {
	            $disabled_list = array_flip($dotb_config['disabled_languages']);
	        }
	    }
