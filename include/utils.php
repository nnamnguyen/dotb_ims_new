<?php
    ...
    function make_dotb_config(&$dotb_config)
    {
        ...
        $dotb_config = array (
            ...
            'dotb_version' => empty($dotb_version) ? 'unknown' : $dotb_version,
    ...
    function get_dotb_config_defaults()
    {
        global $locale;
        /**
        ...
        $dotb_config_defaults = array (
            ...
            'default_theme' => return_session_value_or_default('site_default_theme', 'Dotb'),
        ...
        $dotb_config_defaults['default_currencies'] = $locale->getDefaultCurrencies();

        $dotb_config_defaults = dotbArrayMerge($locale->getLocaleConfigDefaults(), $dotb_config_defaults);

        $dotb_config_defaults[TeamBasedACLConfigurator::CONFIG_KEY] = TeamBasedACLConfigurator::getDefaultConfig();
        return( $dotb_config_defaults );
    }

    /**
     * @deprecated use DotbView::getMenu() instead
     */
    ...
    function get_notify_template_file($language)
    {
        ...
        */
        ...
        $file = DotbAutoLoader::existingCustomOne("include/language/en_us.notify_template.html",
            "include/language/{$language}.notify_template.html");

        return $file;
    }

    function dotb_config_union( $default, $override )
    {
        ...
        foreach ($default as $key => $value) {
            if ( !array_key_exists($key, $override) ) {
                ...
                $override[$key] = dotb_config_union( $value, $override[$key] );
            }
        }
    ...
    {
        global $dotb_config;
        $lang = isset($dotb_config['languages']) ? $dotb_config['languages'] : array();
        if (!empty($dotb_config['disabled_languages'])) {
            $disabledLanguages = explode(',', $dotb_config['disabled_languages']);

            // Make sure we don't disable the default language
            if (($key = array_search($dotb_config['default_language'], $disabledLanguages)) !== false) {
    ...
    function get_language_display($key)
    {
        ...
        return $dotb_config['languages'][$key];
    }
    ...
    function get_assigned_user_name($assigned_user_id, $is_group = '')
    {
        ...
        if (!empty($GLOBALS['dotb_config']['disable_user_cache'])) {
    ...
    function get_user_name($id)
    {
     ...
     function get_full_user_name($id){
        global $db;

        if(empty($db))
            $db = DBManagerFactory::getInstance();

        $q = "SELECT full_user_name FROM users WHERE id='{$id}'";
        $a = $db->fetchOne($q);

        return (empty($a)) ? '' : $a['full_user_name'];
    }
    ...
    function showFullName()
    {
        ...
        if (is_null($showFullName)) {
            $sysPref = !empty($dotb_config['use_real_names']);
    ...
    function return_app_list_strings_language($language, $useCache = true)
    {
        ...
        if ($useCache) {
            ...
            $cache_entry = dotb_cache_retrieve($cache_key);
            if (!empty($cache_entry)) {
                return $cache_entry;
            }
        }

        $default_language = !empty($dotb_config['default_language']) ? $dotb_config['default_language'] : $language;
        ...
        foreach ($langs as $key => $lang) {
            ...
            if ($key > 0) {
                ...
                $app_list_strings = dotbArrayIntersectMerge($app_list_strings, $app_list_strings_array[$lang]);
            }

            $files = DotbAutoLoader::existing(
        ...
        if ($useCache) {
            dotb_cache_put($cache_key, $return_value);
        }
    ...
    function _mergeCustomAppListStrings($file , $appListStrings)
    {
        ...
        $app_list_strings = dotbArrayMergeRecursive($app_list_strings_original , $app_list_strings);
    ...
    {
        global $app_strings, $app_list_strings, $dotb_config;
        ...
        foreach ($app_strings_array as $app_strings_item) {
            $app_strings = dotbLangArrayMerge($app_strings, $app_strings_item);
        }
    ...
    function return_module_language($language, $module, $refresh=false)
    {
        ...
        if(!file_exists(dotb_cached('modules/'). $module . '/language/'.$language.'.lang.php')
        ...
        if ($bean && !empty($bean->module_dir) && $bean->module_dir !== $module) {
            $loaded_mod_strings = dotbLangArrayMerge(
        ...
        // If we are in debug mode for translating, turn on the prefix now!
        if (!empty($dotb_config['translation_string_prefix']) && isset($loaded_mod_strings)) {
    ...
    function return_mod_list_strings_language($language,$module)
    {
        ...
        // cn: bug 6048 - merge en_us with requested language
        $mod_list_strings = dotbLangArrayMerge($en_mod_list_strings, $mod_list_strings);
    ...
    {
        global $mod_strings, $dotb_config, $current_language;

        $language_used = $language;
        $default_language = !empty($dotb_config['default_language']) ? $dotb_config['default_language'] : $language;
        $path = DotbThemeRegistry::get($theme)->getFilePath();
        ...
        if (!isset($theme_strings)) {
            ...
            require(DotbThemeRegistry::get($theme)->getFilePath()."/language/$default_language.lang.php");
        ...
        // If we are in debug mode for translating, turn on the prefix now!
        if ($dotb_config['translation_string_prefix']) {
    ...
    /**
     * determines if a passed string matches the criteria for a Dotb GUID
     * @param string $guid
    ...
    /**
     * Create a unique key and keep it in globals so it can be used by make_dotb_config()
     ...
     * @deprecated Use Dotbcrm\Dotbcrm\Util\Uuid
     */
    function create_guid()
    {
        return Dotbcrm\Dotbcrm\Util\Uuid::uuid1();
    }
    ...
    function displayStudioForCurrentUser()
    {
        ...
        foreach ($access as $key=>$mod) {
            if (DotbAutoLoader::existingCustomOne("modules/{$mod}/metadata/studio.php")) {
     ...
     * Contributor(s): ______________________________________..
     *
     * @deprecated use DotbThemeRegistry::get($theme)->name instead
     */
    function get_theme_display($theme)
    {
        return DotbThemeRegistry::get($theme)->name;
    }
     ...
     * Contributor(s): ______________________________________..
     *
     * @deprecated use DotbThemeRegistry::availableThemes() instead.
     */
    function get_themes()
    {
        return DotbThemeRegistry::availableThemes();
    }
    ...
    function get_select_options_with_id_separate_key ($label_list, $key_list, $selected_key, $massupdate=false)
    {
        ...
        if ($massupdate) {
            $replacement .= "/OPTION>\n<OPTION value='__DotbMassUpdateClearField__'><"; // Giving the user the option to unset a drop down list. I.e. none means that it won't get updated
        }
     ...
     * exit code.
     */
    function dotb_die($error_message = '', $exit_code = 1)
    {
...
EOQ;
    ...
    function unTranslateNum($num)
    {
        ...
        if ($dec_sep == null) {
            ...
            $dec_sep = (empty($user_dec_sep) ? $dotb_config['default_decimal_seperator'] : $user_dec_sep);
        }
        if ($num_grp_sep == null) {
            $user_num_grp_sep = $current_user->getPreference('num_grp_sep');
            $num_grp_sep = (empty($user_num_grp_sep) ? $dotb_config['default_number_grouping_seperator'] : $user_num_grp_sep);
        }
    ...
    {
        return DotbCleaner::cleanHtml($str, false);
    }
    ...
    function clean_xss($str, $cleanImg=true)
    {
        ...
        if(empty($dotb_config['email_xss']))
            $dotb_config['email_xss'] = getDefaultXssTags();

        $xsstags = unserialize(base64_decode($dotb_config['email_xss']));
        ...
        if (isset($dotb_config['security_trusted_domains']) && !empty($dotb_config['security_trusted_domains']) && is_array($dotb_config['security_trusted_domains'])) {
            if (is_array($cssUrlMatches) && count($cssUrlMatches) > 0) {
                // normalize whitelist
                foreach ($dotb_config['security_trusted_domains'] as $k => $v) {
                    $dotb_config['security_trusted_domains'][$k] = strtolower($v);
                }
                ...
                foreach ($cssUrlMatches[0] as $match) {
                    ...
                    if (!in_array($baseUrl, $dotb_config['security_trusted_domains'])) {
                        $matches[] = $match;
                    }
                }
            }
     ...
     * in \Dotbcrm\Dotbcrm\Security\Validator\Constraints\LegacyCleanStringValidator.
     */
    function clean_string($value, $filter = "STANDARD", $dieOnBadData = true)
    {
        ...
        if (preg_match($filters[$filter], $value)) {
            ...
            if ($dieOnBadData) {
                die("Bad data passed in; <a href=\"{$dotb_config['site_url']}\">Return to Home</a>");
            }
    ...
    function clean_incoming_data()
    {
        ...
        if (isset($dotb_config['default_theme'])) clean_string($dotb_config['default_theme']);
    ...
    {
        return dotb_cache_put("{$category}-{$name}", $value);
    }

    function get_register_value($category,$name)
    {
        return dotb_cache_retrieve("{$category}-{$name}");
    }

    function clear_register_value($category,$name)
    {
        return dotb_cache_clear("{$category}-{$name}");
    }
    ...
    /**
     * @deprecated use DotbTheme::getImage()
     */
    function get_image($image,$other_attributes,$width="",$height="",$ext='.gif',$alt="")
    {
        return DotbThemeRegistry::current()->getImage(basename($image), $other_attributes, empty($width) ? null : $width, empty($height) ? null : $height, $ext, $alt );
    }
    /**
     * @deprecated use DotbTheme::getImageURL()
     */
    function getImagePath($image_name)
    {
        return DotbThemeRegistry::current()->getImageURL($image_name);
    }
    ...
    function getWebPath($relative_path)
    {
        ...
        if(defined('TEMPLATE_URL'))$relative_path = DotbTemplateUtilities::getWebPath($relative_path);
    ...
    {
        if(empty($GLOBALS['dotb_config']['js_custom_version'])) $GLOBALS['dotb_config']['js_custom_version'] = 1;
        ...
        $str = substr(base64_encode(md5("{$GLOBALS['js_version_key']}|{$GLOBALS['dotb_config']['js_custom_version']}|$additional_attrs", true)), 0, -2);
    ...
    function get_emails_by_assign_or_link($params)
    {
        ...
        if (!empty($GLOBALS['dotb_config']['hide_history_contacts_emails']) && isset($GLOBALS['dotb_config']['hide_history_contacts_emails'][$bean->module_name])) {
            $showEmailsOfRelatedContacts = empty($GLOBALS['dotb_config']['hide_history_contacts_emails'][$bean->module_name]);
        }
    ...
    function parse_list_modules(&$listArray)
    {
        ...
        $returnArray = DotbACL::filterModuleList($listArray, 'access', true);
        asort($returnArray);
    ...
    */
     ...
     * This function is only intended to be used for DotBCRM internal development.
     ...
     * the Logger. This function is only intended to be used for DotBCRM internal
    ...
    /**
     * Get Dotb root directory
     * @return string
     */
    function dotb_root_dir()
    {
        return realpath(dirname(__FILE__) . '/..');
    }

    /**
     * Clean up Dotb environment
     * @param bool $exit Should we exit() after we're done?
     */
    function dotb_cleanup($exit = false)
    {
        static $called = false;
        if($called)return;
        $called = true;
        $root_path = dotb_root_dir();
        ...
        && (empty($_REQUEST['dotb_body_only']) || !$_REQUEST['dotb_body_only'] )
    ...
    */
    ...
    if (isset($dotb_config['stack_trace_errors']) && $dotb_config['stack_trace_errors']) {
    ...
    */
     ...
     * @author	customized for Dotb by Chris N.
     */
     ...
     */
    function dotbArrayIntersectMerge($gimp, $dom)
    {
    ...
    /**
     * Identical to dotbArrayMerge but with some speed improvements and used specifically to merge
     ...
     * @return array
     */
    function dotbLangArrayMerge($gimp, $dom)
    {
     ...
     * @return array beaten gimp
     */
    function dotbArrayMerge($gimp, $dom)
    {
        if (is_array($gimp) && is_array($dom)) {
            foreach ($dom as $domKey => $domVal) {
    ...
    /**
     * Similiar to dotbArrayMerge except arrays of N depth are merged.
     *
     * @param array gimp the array whose values will be overloaded
     * @param array dom the array whose values will pwn the gimp's
     * @return array beaten gimp
     */
    function dotbArrayMergeRecursive($gimp, $dom)
    {
        if (is_array($gimp) && is_array($dom)) {
            foreach ($dom as $domKey => $domVal) {
                if (array_key_exists($domKey, $gimp)) {
                    if (is_array($domVal) && is_array($gimp[$domKey])) {
                        $gimp[$domKey] = dotbArrayMergeRecursive($gimp[$domKey], $domVal);
                    } else {
                        $gimp[$domKey] = $domVal;
                    }
                } else {
    ...
    function getTrackerSubstring($name)
    {
        ...
        if (!isset($max_tracker_item_length)) {
            if (isset($dotb_config['tracker_max_display_length'])) {
                $max_tracker_item_length = (is_int($dotb_config['tracker_max_display_length']) && $dotb_config['tracker_max_display_length'] > 0 && $dotb_config['tracker_max_display_length'] < 50) ? $dotb_config['tracker_max_display_length'] : 30;
            } else {
     ...
     */
    function rebuildConfigFile($dotb_config, $dotb_version)
    {
        // add defaults to missing values of in-memory dotb_config
        $dotb_config = dotbArrayMerge(get_dotb_config_defaults(), $dotb_config );
        if (isset($dotb_config['dotbbeet'])) {
            //$dotb_config['dotbbeet'] is only set in COMM
            unset($dotb_config['dotbbeet']);
        }
        if (isset($dotb_config['disable_team_access_check'])) {
            //$dotb_config['disable_team_access_check'] is a runtime configration,
            //no need to write to config.php
            unset($dotb_config['disable_team_access_check']);
        }
        if (isset($dotb_config['team_based_acl']['enabled_modules'])) {
            ...
            $dotb_config['team_based_acl']['enabled_modules'] = array();
        }
        // need to override version with default no matter what
        $dotb_config['dotb_version'] = $dotb_version;

        ksort( $dotb_config );

        if ( write_array_to_file( "dotb_config", $dotb_config, "config.php" ) ) {
            return true;
        } else {
            return false;
        }
     ...
     * not always match that of $dotb_config['site_url'].  Basically, the
    ...
    function getJavascriptSiteURL()
    {
        ...
        if (!empty($_SERVER['HTTP_REFERER'])) {
            ...
            $site_url = preg_replace('/^http[s]?\:\/\/[^\/]+/',$replacement_url,$dotb_config['site_url']);
        } else {
            $site_url = preg_replace('/^http(s)?\:\/\/[^\/]+/',"http$1://".$_SERVER['HTTP_HOST'],$dotb_config['site_url']);
    ...
    */
    ...
    function _getIcon($iconFileName)
    {
        ...
        $iconFound = DotbThemeRegistry::current()->getImageURL($iconName,false);
    ...
    function getStudioIcon($iconFileName='', $altFileName='', $width='48', $height='48', $align='baseline', $alt='' )
    {
        ...
        return DotbThemeRegistry::current()->getImage($iconName, "align=\"$align\" border=\"0\"", $width, $height);
    }
    ...
    function chartStyle()
    {
        return DotbThemeRegistry::current()->getCSSURL('chart.css');
    }
     ...
     *
     * @return dotbColors.xml to use
     */
    function chartColors()
    {
        if (DotbThemeRegistry::current()->getCSSURL('dotbColors.xml')=='')
            return DotbThemeRegistry::current()->getImageURL('dotbColors.xml');
        return DotbThemeRegistry::current()->getCSSURL('dotbColors.xml');
    }
    ...
    {
        return !empty($GLOBALS['dotb_config']['developerMode']);
    }
    ...
    function shouldResourcesBeMinified()
    {
        ...
        if (!isset($dotb_config['minify_resources'])) {
            return !inDeveloperMode();
        }

        return (bool) $dotb_config['minify_resources'];
    }
    ...
    {
        if ( !isset($GLOBALS['dotb_config']['allow_pop_inbound']) || ! $GLOBALS['dotb_config']['allow_pop_inbound'] ) {
     ...
     */
    function dotb_substr($string, $length, $charset='UTF-8')
    {
     ...
     */
    function dotb_ucfirst($string, $charset='UTF-8')
    {
    ...
    {
        global $dotb_web_service_order_by;
        //If the order_by field is not valid, return 0;
        if (empty($dotb_web_service_order_by) || !isset($a->$dotb_web_service_order_by) || !isset($b->$dotb_web_service_order_by)) {
            return 0;
        }
        if (is_object($a->$dotb_web_service_order_by) || is_object($b->$dotb_web_service_order_by)
        || is_array($a->$dotb_web_service_order_by) || is_array($b->$dotb_web_service_order_by))
        {
            return 0;
        }
        if ($a->$dotb_web_service_order_by < $b->$dotb_web_service_order_by) {
            return -1;
        } else {
            return 1;
        }
    ...
    function order_beans($beans, $field_name)
    {
        ...
        global $dotb_web_service_order_by;
        $dotb_web_service_order_by = $field_name;
    ...
    function getFTSEngineType()
    {
        ...
        if (isset($GLOBALS['dotb_config']['full_text_engine']) && is_array($GLOBALS['dotb_config']['full_text_engine'])) {
            foreach ($GLOBALS['dotb_config']['full_text_engine'] as $name => $defs) {
                return $name;
            }
        }
    ...
    {
        $newfile = DotbAutoLoader::existingCustomOne($file);
    ...
    function get_help_url($send_edition = '', $send_version = '', $send_lang = '', $send_module = '', $send_action = '', $dev_status = '', $send_key = '', $send_anchor = '')
    {
        ...
        if (!empty($dotb_config['custom_help_url'])) {
            $sendUrl = $dotb_config['custom_help_url'];
        } else {
            $baseUrl = "http://dotb.vn/ho-tro";
            $sendUrl = $baseUrl;
        }
     ...
     * @param string $value
     */
    function assignConcatenatedValue(DotbBean $bean, $fieldDef, $value)
    {
    ...
    /**
     * Ensures that the dotbcrm/cache has been built. If not, this builds.
     * @param DotbMinifyUtils $minifyUtils
     * @param string $rootDir Path to start from.
     * @return  string The path to dotb lumia min file.
     */
    function ensureCache($minifyUtils, $rootDir)
    {
        ...
        $dotbLumiaPath = $rootDir . '/cache/javascript/dotbcrm.min.js';
        if (!file_exists($dotbLumiaPath) || !is_file($dotbLumiaPath)) {
            require_once 'jssource/JSGroupings.php';
            $minifyUtils->ConcatenateFiles($rootDir);
        }

        return $dotbLumiaPath;
    }
    ...
    {
        // Create a reasonable default. Using dotb_lumia since that is what was
        // used in ensureCache as a default
        if (empty($files)) {
            $files = array('dotbcrm.min.js');
        }
        ...
        foreach ($files as $f) {
            ...
            if ($addPath) {
                $path = "$root/" . dotb_cached("javascript/$f");
            } else {
                $path = "$root/$f";
            }
     ...
     * @param  string  $module The name of the module to check
     * @return boolean
     * Create By Lap Nguyen
     */
    function isModuleHybrid($module)
    {
        return in_array($module, $GLOBALS['dotb_config']['hybrid_modules']);
    }

    /**
     * Translate a bwc action to a lumia one.
     *
     * @param string $action The action to find a lumia mapped one.
     * @return string The mapped action or the action given if no action map was
     *   found.
     */
    function translateToLumiaAction($action)
    {
    ...
    /**
     * Builds a lumia route from a BWC page.
     *
     * This will NOT check if the module is in BWC or not, it will trigger the
     * navigation to lumia url.
     * @see isModuleBWC() to check if a module is in BWC or not.
     * This expects the lumia actions.
     * @see translateToLumiaAction() if you need to know which action to map to.
     *
     * @param string $module The lumia module to return to.
     * @param string $id (optional) The record id to return to.
     * @param string $action (optional) The action to return to.
     * @return string The lumia route based on the arguments given.
     */
    function buildLumiaRoute($module, $id = null, $action = null)
    {
        $route = $module;
        if (!empty($id)) {
            $route .= "/$id";
    ...
    /**
     * Navigate to a lumia url from a BWC page.
     *
     * @see buildLumiaRoute() to build the url param to pass to this function.
     *
     * @param string $url The lumia url to navigate to.
     * @return string The script to use for a lumia navigation
     */
    function navigateToLumia($url)
    {
        return "parent.DOTB.App.router.navigate('$url', {trigger: true});";
    }
    ...
    {
        $required = array();
        $files = DotbAutoLoader::existingCustom('include/required_list_items.php');
        foreach ($files as $file) {
            require $file;
    ...
    {
        $string = '';
        if (isset($GLOBALS['dotb_config']['unique_key'])) {
            $string .= $GLOBALS['dotb_config']['unique_key'];
        }
        if (isset($GLOBALS['dotb_version'])) {
            $string .= $GLOBALS['dotb_version'];
        }
        if (isset($GLOBALS['dotb_flavor'])) {
            $string .= $GLOBALS['dotb_flavor'];
        }
     ...
     */
    function dotb_upgrade_print($param, $backtrace)
    {
        if ($backtrace) {
     ...
     */
    function dotb_upgrade_print_r()
    {
        dotb_upgrade_print('print_r', getBacktraceData(__FUNCTION__));
    }

    /**
     * Replace base var_dump function call
     */
    function dotb_upgrade_var_dump()
    {
        dotb_upgrade_print('var_dump', getBacktraceData(__FUNCTION__));
    }

    /**
     * Replace base echo function call
     */
    function dotb_upgrade_echo()
    {
        dotb_upgrade_print('echo', getBacktraceData(__FUNCTION__));
    }

    /**
     * Replace base exit function call
     */
    function dotb_upgrade_exit()
    {
        dotb_upgrade_print('exit', getBacktraceData(__FUNCTION__));
    }

    /**
     * Replace base die function call
     */
    function dotb_upgrade_die()
    {
        dotb_upgrade_print('die', getBacktraceData(__FUNCTION__));
    }
     ...
     * @return String
     */
    function replace_dotb_vars($subject, $fields, $use_curly = false)
    {
        $lDelim = "[";
        $rDelim = "]";
        if ($use_curly) {
    ...
    function validate_ip($clientIp, $sessionIp)
    {
        ...
        if (isset($dotb_config['verify_client_ip']) && !$dotb_config['verify_client_ip']) {
            return true;
        }
