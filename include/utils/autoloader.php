...
class DotbAutoLoader
{
     ...
     * DotbAclFoo.php => data/acl/DotbACLFoo.php
     * @var array prefix => directory
     */
    public static $prefixMap = array(
        'DotbACL' => "data/acl/",
        'DotbWidget' => "include/generic/DotbWidgets/",
        'Zend_' => 'vendor/',
        'DotbJob' => 'include/DotbQueue/jobs/',
    ...
    public static function init($useConfig = true)
    {
        if ($useConfig === true) {
            ...
            if (self::$devMode) {
                @unlink(dotb_cached(self::CLASS_CACHE_FILE));
            }
    ...
    public static function getComposerClassMap()
    {
        ...
        return array_map(array('DotbAutoLoader', 'normalizeFilePath'), $classMap);
    }
    ...
    public static function autoload($class)
	{
		...
		if($file = self::getFilenameForDotbWidget($class)) {
            self::$classMap[$class] = $file;
			self::$classMapDirty = true;
			return true;
		}
    ...
    /**
     * getFilenameForDotbWidget
     *
     * This method attempts to autoload classes starting with name "DotbWidget".  It first checks for the file
     * in custom/include/generic/DotbWidgets directory and if not found defaults to include/generic/DotbWidgets.
     * This method is used so that we can easily customize and extend these DotbWidget classes.
     *
     * Can not be served by prefixMap because of the lowercasing in class names.
     *
     * @static
     * @param $class String name of the class to load
     * @return String file of the DotbWidget class; false if none found
     */
    protected static function getFilenameForDotbWidget($class)
    {
        //Only bother to check if the class name starts with DotbWidget
        if(strpos($class, 'DotbWidgetField') !== false) {
            //We need to lowercase the portion after DotbWidgetField
            $name = substr($class, 16);
            if(empty($name)) {
                return false;
            }
            $class = 'DotbWidgetField' . strtolower($name);
            return self::requireWithCustom("include/generic/DotbWidgets/{$class}.php");
        }
        return false;
    }
    ...
    public static function existing()
    {
        ...
        foreach($files as $file) {
            ...
            if(is_array($file)) {
                $out += call_user_func_array(array("DotbAutoLoader", "existing"), $file);
                continue;
            }
            if (file_exists($file)) {
                $out[] = $file;
    ...
    public static function existingCustom()
    {
        ...
        foreach($files as $file) {
            if(empty($file)) continue;
            if(is_array($file)) {
                $out += call_user_func_array(array("DotbAutoLoader", "existingCustom"), $file);
                continue;
            }
            if (file_exists($file)) {
                $out[] = $file;
    ...
    {
        $files = func_get_args();
        $out = call_user_func_array(array("DotbAutoLoader", "existingCustom"), $files);
        if(empty($out)) {
            return null;
        } else {
     ...
     * This method is allows for easy loading of arbitrary expression classes by the DotbLogic Expression parser.
     *
     * @static
     * @param $class String name of the class to load
	...
	public static function loadPopupMeta($module, $metadata = null)
	{
	    ...
	    if(!empty($metadata)) {
	    	$defs = DotbAutoLoader::loadWithMetafiles($module, $metadata);
	    }

	    if(!$defs) {
	    	$defs = DotbAutoLoader::loadWithMetafiles($module, 'popupdefs');
	    }
        if($defs) {
            require $defs;
            return $popupMeta;
    ...
    {
        $GLOBALS['log']->deprecated('\DotbAutoLoader::fileExists is deprecated, use file_exists()');
    ...
    {
        $GLOBALS['log']->deprecated('\DotbAutoLoader::loadFileMap is deprecated');
    }
    ...
    public static function buildClassCache()
    {
        ...
        if (!self::$devMode) {
            write_array_to_file("class_map", $class_map, dotb_cached(self::CLASS_CACHE_FILE));
        }
    ...
    public static function loadClassMap()
    {
        ...
        if (!self::$devMode) {
            @include dotb_cached(self::CLASS_CACHE_FILE);
        }
	...
	{
        $GLOBALS['log']->deprecated('\DotbAutoLoader::addToMap is deprecated');
	}
	...
	{
        $GLOBALS['log']->deprecated('\DotbAutoLoader::delFromMap is deprecated');
	}
	...
	{
        $GLOBALS['log']->deprecated('\DotbAutoLoader::unlink is deprecated, use unlink()');
	    unlink($filename);
	}
	 ...
	 * @param bool $save Save map to file?
	 * @return bool Success?
     * @deprecated use dotb_touch() instead
	 */
	public static function touch($filename, $save = false)
	{
        $GLOBALS['log']->deprecated('\DotbAutoLoader::touch is deprecated, use dotb_touch()');
        return dotb_touch($filename);
	}
	...
	{
        $GLOBALS['log']->deprecated('\DotbAutoLoader::put is deprecated, use file_put_contents()');
	...
	public static function ensureDir($dir)
	{
        ...
        return dotb_mkdir($dir, null, true);
	}
	...
	{
        $GLOBALS['log']->deprecated('\DotbAutoLoader::saveMap is deprecated');
	}
    ...
    public static function saveClassMap()
    {
        if (!self::$devMode && self::$classMapDirty && !empty(self::$classMap)) {
            write_array_to_file("class_map", self::$classMap, dotb_cached(self::CLASS_CACHE_FILE));
            self::$classMapDirty = false;
        }
    }
    ...
    public static function normalizeFilePath($filename)
    {
        if (!isset(self::$baseDirs)) {
            self::$baseDirs = array(DOTB_BASE_DIR);
