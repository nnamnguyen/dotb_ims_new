...
{
    global $dotb_version, $dotb_flavor, $server_unique_key, $current_language, $current_module, $current_action;
    global $app_strings;

    $blankImageURL = DotbThemeRegistry::current()->getImageURL('blank.gif');
    $printImageURL = DotbThemeRegistry::current()->getImageURL("print.gif");
    $helpImageURL  = DotbThemeRegistry::current()->getImageURL("help.gif");
...
EOHTML;
        if ($show_help) {
            ...
            $the_form .= <<<EOHTML
&nbsp;
    <a href='index.php?module=Administration&action=SupportPortal&view=documentation&version={$dotb_version}&edition={$dotb_flavor}&lang={$current_language}&help_module={$current_module}&help_action={$current_action}&key={$server_unique_key}'
 ...
 *
 * @deprecated use DotbView::getModuleTitle() for MVC modules, or getClassicModuleTitle() for non-MVC modules
 *
    ...
    )
{
    ...
    if (!empty($iconPath)) {
        $the_title .= '<h2>';
        $breadCrumbSymbol = DotbView::staticGetBreadCrumbSymbol();
    ...
    if ($show_create) {
        ...
        $createRecordURL = DotbThemeRegistry::current()->getImageURL('create-record.gif');
        $the_title .= <<<EOHTML
&nbsp;
...
function getClassicModuleTitle($module, $params, $show_create=false, $index_url_override='', $create_url_override='')
{
    ...
    if ($show_create) {
        $the_title .= "<span class='utils'>";
        $createRecordImage = DotbThemeRegistry::current()->getImageURL('create-record.gif');
 ...
 *
 * @param  $theme string the name of the current theme, ignorred to use DotbThemeRegistry::current() instead.
 * @return string HTML
 */
    ...
    )
{
    ...
    $themeCSS = DotbThemeRegistry::current()->getCSS();

    $langHeader = get_language_header();

    //The DotbView will insert the header now, this function should no longer do the actual head element.
    if ($includeJS)
    {
        echo <<<EOHTML
...
EOHTML;
    ...
    if (isset($dotb_config['meta_tags']) && isset($dotb_config['meta_tags']['IE_COMPAT_MODE']))
    {
        echo $dotb_config['meta_tags']['IE_COMPAT_MODE'];
    }

    echo "<title>{$app_strings['LBL_BROWSER_TITLE']}</title>" . $themeCSS;
    if ($includeJS)
    {
        $charset = isset($app_strings['LBL_CHARSET']) ? $app_strings['LBL_CHARSET'] : $dotb_config['default_charset'];
        echo '<meta http-equiv="Content-Type" content="text/html; charset="{$charset}">';
        echo '<script type="text/javascript" src="' . getJSPath('include/javascript/jquery/jquery-min.js') . '"></script>';
        echo '<script type="text/javascript" src="'
            . getJSPath('include/javascript/jquery/jquery-migrate-1.4.1.min.js') . '"></script>';
        echo '<script type="text/javascript" src="' . getJSPath('cache/javascript/dotbcrm3.min.js') . '"></script>';
        echo '<script type="text/javascript" src="' . getJSPath('cache/javascript/dotbcrm1.min.js') . '"></script>';
        echo '</head>';
    }
