 ...
 */
function dotb_mkdir($pathname, $mode=null, $recursive=false, $context='') {
    $mode = get_mode('dir_mode', $mode);

    if ( dotb_is_dir($pathname,$mode) )
        return true;
    ...
    if($result){
        if(!dotb_chmod($pathname, $mode)){
            return false;
        }
        if(!empty($GLOBALS['dotb_config']['default_permissions']['user'])){
            if(!dotb_chown($pathname)){
                return false;
            }
        }
        if(!empty($GLOBALS['dotb_config']['default_permissions']['group'])){
            if(!dotb_chgrp($pathname)) {
                return false;
            }
        }
    }
 ...
 */
function dotb_fopen($filename, $mode, $use_include_path=false, $context=null){
 ...
 */
function dotb_file_put_contents($filename, $data, $flags=null, $context=null){
    //check to see if the file exists, if not then use touch to create it.
    if(!file_exists($filename)){
        dotb_touch($filename);
    }
...
/**
 * dotb_file_put_contents_atomic
 * This is an atomic version of dotb_file_put_contents.  It attempts to circumvent the shortcomings of file_put_contents
 ...
 */
function dotb_file_put_contents_atomic($filename, $data, $mode='wb', $use_include_path=false, $context=null){
    ...
    if ($temp === false || !($f = @fopen($temp, $mode))) {
        ...
        if (!($f = @fopen($temp, $mode))) {
            trigger_error("dotb_file_put_contents_atomic() : error writing temporary file '$temp'", E_USER_WARNING);
            return false;
        }
    }
    ...
    {
        error_log("dotb_file_put_contents_atomic() : external rename failure '$temp' -> '$filename' - retrying");
        @scandir($dir);
        @unlink($filename);
        if (!@rename($temp, $filename))
        {
            ...
            @unlink($temp);
            trigger_error("dotb_file_put_contents_atomic() : fatal rename failure '$temp' -> '$filename'", E_USER_ERROR);
        }
    }

    if(file_exists($filename))
    {
        return dotb_chmod($filename);
    }
 ...
 */
function dotb_file_get_contents($filename, $use_include_path=false, $context=null){
    if (!file_exists($filename)) {
 ...
 *
 */
function dotb_touch($filename, $time=null, $atime=null) {

    if (!empty($GLOBALS['dotb_config']['default_permissions']['dir_mode'])) {
        $dirmode = $GLOBALS['dotb_config']['default_permissions']['dir_mode'];
    } else {
        $dirmode = null;
    }

    $result = dotb_mkdir(dotbDirname($filename), $dirmode, true);
    ...
    if(!empty($GLOBALS['dotb_config']['default_permissions']['file_mode'])){
        dotb_chmod($filename, $GLOBALS['dotb_config']['default_permissions']['file_mode']);
    }
    if(!empty($GLOBALS['dotb_config']['default_permissions']['user'])){
        dotb_chown($filename);
    }
 ...
 * @return boolean   Returns TRUE on success or FALSE on failure.
 */
function dotb_chmod($filename, $mode=null) {
    if(!is_windows()){
        if ($mode === null) {
 ...
 * @return boolean - Returns TRUE on success or FALSE on failure.
 */
function dotb_chown($filename, $user='') {
    if(!is_windows()){
        if(strlen($user)){
            return chown($filename, $user);
        }else{
            if(strlen($GLOBALS['dotb_config']['default_permissions']['user'])){
                $user = $GLOBALS['dotb_config']['default_permissions']['user'];
                return chown($filename, $user);
            }else{
                return false;
            }
 ...
 * @return boolean - Returns TRUE on success or FALSE on failure.
 */
function dotb_chgrp($filename, $group='') {
    if(!is_windows()){
        if(!empty($group)){
            return chgrp($filename, $group);
        }else{
            if(!empty($GLOBALS['dotb_config']['default_permissions']['group'])){
                $group = $GLOBALS['dotb_config']['default_permissions']['group'];
                return chgrp($filename, $group);
            }else{
                return false;
            }
...
function get_mode($key = 'dir_mode', $mode=null) {
    if ( !is_int($mode) )
        $mode = (int) $mode;
    if(!is_windows()){
        $conf_inst=DotbConfig::getInstance();
        $mode = $conf_inst->get('default_permissions.'.$key, $mode);
    }
    return $mode;
}

function dotb_is_dir($path, $mode='r'){
    if(defined('TEMPLATE_URL'))return is_dir($path, $mode);
    return is_dir($path);
}

function dotb_is_file($path, $mode='r'){
 ...
 * @param string $file
 */
function dotb_cached($file)
{
    static $cdir = null;
    if(empty($cdir) && !empty($GLOBALS['dotb_config']['cache_dir'])) {
        $cdir = rtrim($GLOBALS['dotb_config']['cache_dir'], '/\\');
    }
    if(empty($cdir)) {
        $cdir = "cache";
    }
