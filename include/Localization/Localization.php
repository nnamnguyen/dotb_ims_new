...
class Localization {
    ...
    {
		global $dotb_config;
		$this->localeNameFormatDefault = empty($dotb_config['locale_name_format_default']) ? 's f l' : $dotb_config['default_name_format'];
		$this->loadCurrencies();
	}
    ...
    {
        $class = __CLASS__;
        if (DotbAutoLoader::load('custom/include/Localization/Localization.php')) {
	...
	/**
	 * returns an array of Dotb Config defaults that are determined by locale settings
	 * @return array
	 */
	function getLocaleConfigDefaults() {
		$coreDefaults = array(
            ...
            'name_formats'                          => array('s f l' => 's f l','s l f' => 's l f','f l' => 'f l','l f' => 'l f'),
	 ...
	 */
	function getPrecedentPreference($prefName, $user=null, $dotbConfigPrefName = '') {
		global $current_user;
		global $dotb_config;
        ...
        //rrs: 33086 - give the ability to pass in the preference name as stored in $dotb_config.
        if (!empty($dotbConfigPrefName)) {
            $prefName = $dotbConfigPrefName;
        }
        ...
        if($prefName == 'num_grp_sep' && empty($dotbConfigPrefName) && is_null($userPref)) {
            return null;
        }
        
        // cn: 9549 empty() call on a value of 0 (0 significant digits) resulted in a false-positive.  changing to "isset()"
        $pref = (!isset($dotb_config[$prefName]) || (empty($dotb_config[$prefName]) && $dotb_config[$prefName] !== '0')) ? $pref : $dotb_config[$prefName];
	...
	function loadCurrencies() {
        ...
        $load = dotb_cache_retrieve('currency_list');
        if ( !is_array($load) ) {
			// load default from config.php
			$this->currencies['-99'] = array(
				'name'		=> $dotb_config['default_currency_name'],
				'symbol'	=> $dotb_config['default_currency_symbol'],
				'conversion_rate' => 1
		    );
            ...
            dotb_cache_put('currency_list',$this->currencies);
        } else {
	...
	/**
	 * retrieves default OOTB currencies for dotb_config and installer.
    ...
    public function translateCharset($string, $fromCharset, $toCharset = 'UTF-8', $forceIconv = false, $addBOM = false)
    {
        ...
        {
            global $dotb_config;
            if (!empty($dotb_config['export_excel_compatible']) && $addBOM === true) {
	...
	/**
	 * returns the charset preferred in descending order: User, Dotb Config, DEFAULT
	...
	function getCurrencySymbol($user=null) {
		...
		$currency = DotbCurrency::getCurrencyByID($currencyId);
...
eoq;
     ...
     * Returns formatted name according to $current_user's locale settings
     *
     * @param DotbBean|string $beanOrModuleName DotbBean object or module name
    ...
    /**
     * Returns names of DotbBean fields that are used in current formatting macro.
     *
     * @param DotbBean|string $beanOrModuleName DotbBean object or module name
     *
     * @return array
    ...
    protected function getBean($beanOrModuleName)
    {
        ...
        } elseif ($beanOrModuleName instanceof DotbBean) {
    ...
    public function getAuthenticatedUserLanguage()
    {
        ...
        return $GLOBALS['dotb_config']['default_language'];
    }
} // end class def
