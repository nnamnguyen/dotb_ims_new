...
class CalendarEvents
{
     ...
     */
    public function isEventRecurring(DotbBean $bean)
    {
        if (!in_array($bean->module_name, $this->calendarEventModules)) {
            ...
            throw new DotbException('LBL_CALENDAR_EVENT_RECURRENCE_MODULE_NOT_SUPPORTED', array($bean->module_name));
        }
    ...
    {
        return DotbConfig::getInstance()->get('calendar.max_repeat_count', 1000);
    }
     ...
     *
     * @param DotbBean $parentBean
     * @return array
     */
    public function buildParamsForRecurring(DotbBean $parentBean)
    {
        $params = array();
     ...
     * @throws DotbException
     */
    public function saveRecurringEvents(DotbBean $parentBean)
    {
        if (!$this->isEventRecurring($parentBean)) {
            $logmsg = 'SaveRecurringEvents() : Event is not a Recurring Event';
            $GLOBALS['log']->error($logmsg);
            throw new DotbException('LBL_CALENDAR_EVENT_NOT_A_RECURRING_EVENT', array($parentBean->object_name));
        }

        if (!empty($parentBean->repeat_parent_id)) {
            $logmsg = 'SaveRecurringEvents() : Event received is not the Parent Occcurrence';
            $GLOBALS['log']->error($logmsg);
            throw new DotbException('LBL_CALENDAR_EVENT_IS_NOT_A_PARENT_OCCURRENCE', array($parentBean->object_name));
        }
     ...
     * @param DotbBean Child Calendar Event Bean
     * @param array Tag Beans currently existing on Child (optional - defaults to empty array)
     */
    public function reconcileTags(array $parentTagBeans, DotbBean $childBean, $childTagBeans = array())
    {
        $sf = DotbFieldHandler::getDotbField('tag');
    ...
    public function buildRecurringSequence($date_start, $params)
    {
        ...
        $start = DotbDateTime::createFromFormat($GLOBALS['timedate']->get_date_time_format(), $date_start);
        $options['start'] = $start;

        if (!empty($options['until'])) {
            $end = DotbDateTime::createFromFormat($GLOBALS['timedate']->get_date_format(), $options['until']);
        ...
        $limit = DotbConfig::getInstance()->get('calendar.max_repeat_count', 1000);
     ...
     * @param DotbDateTime $current : the next Date to be considered as a Result Candidate
            ...
            case 'On': {
                if (!empty($options['ordinal']) && !empty($options['unit'])) {
                    ...
                    if (!empty($app_list_strings['repeat_ordinal_dom'][$ordinal]) &&
                        ...
                        switch ($unit) {
                            case 'Sun': {
                                $targetDay = DotbDateTime::DOW_SUN;
                                break;
                            }
                            case 'Mon': {
                                $targetDay = DotbDateTime::DOW_MON;
                                break;
                            }
                            case 'Tue': {
                                $targetDay = DotbDateTime::DOW_TUE;
                                break;
                            }
                            case 'Wed': {
                                $targetDay = DotbDateTime::DOW_WED;
                                break;
                            }
                            case 'Thu': {
                                $targetDay = DotbDateTime::DOW_THU;
                                break;
                            }
                            case 'Fri': {
                                $targetDay = DotbDateTime::DOW_FRI;
                                break;
                            }
                            case 'Sat': {
                                $targetDay = DotbDateTime::DOW_SAT;
                                break;
                            }
     ...
     * @param DotbBean parent Bean
     */
    protected function markRepeatDeleted(DotbBean $parentBean)
    {
     ...
     * @return array events saved
     */
    protected function saveRecurring(DotbBean $parentBean, array $repeatDateTimeArray)
    {
    ...
    public function formatDateTime($type, $dtm, $toFormat, $user=null)
    {
        ...
        $dotbDateTime = $this->getDotbDateTime($type, $dtm, $user);
        if (!empty($dotbDateTime)) {
            $result = $dotbDateTime->formatDateTime($type, $toFormat, $user);
        }
        return $result;
    }

    /**
     * Return a DotbDateTime Object given any Date to Time Format
     * @param string type of the second argument : one of 'date', 'time', 'datetime', 'datetimecombo'
     * @param string  formatted date, time or datetime field in DB, ISO, or User Format
     * @param User whose timezone preferences are to be used (optional - defaults to current user)
     * @return DotbDateTime
     */
    public function getDotbDateTime($type, $dtm, $user=null)
    {
        global $timedate;
        $dotbDateTime = null;
        if (!empty($dtm)) {
            $dotbDateTime = $timedate->fromUserType($dtm, $type, $user);
            if (empty($dotbDateTime)) {
                $dotbDateTime = $timedate->fromDBType($dtm, $type);
            }
            if (empty($dotbDateTime)) {
                switch($type) {
                    case 'time':
                        $dotbDateTime = $timedate->fromIsoTime($dtm);
                        break;
                    case 'date':
                    case 'datetime':
                    case 'datetimecombo':
                    default:
                        $dotbDateTime = $timedate->fromIso($dtm);
                        break;
                }
            }
        }
     ...
     *
     * @param DotbBean $bean - Schedulable Event - i.e Meeting, Call
     * @param DotbDateTime $userDateTime in Database Format (UTC)
     */
    public function setStartAndEndDateTime(DotbBean $bean, DotbDateTime $dateStart)
    {
        global $current_user;
        ...
        if(empty($bean->date_end)) $bean->date_end = $dtm->asDb();
     ...
     *
     * @param DotbBean $event
     * @param DotbBean $invitee
     * @param string $status
    ...
    /**
     * Returns a DotbQuery object that can be used to fetch all of the child events in a recurring series.
     *
     * @param DotbBean $parent
     * @return DotbQuery Modify the object to restrict the result set based on additional conditions.
     * @throws DotbQueryException
     */
    protected function getChildrenQuery(DotbBean $parent)
    {
        $GLOBALS['log']->debug(sprintf(
     ...
     *
     * @param DotbQuery $query The DotbQuery object to use to retrieve the records.
     * @param Closure $callback The function to call for each child record. The database row -- as an array -- is
     * passed to the callback.
     */
    protected function repeatAction(DotbQuery $query, Closure $callback)
    {
        $limit = 200;
        $offset = 0;
