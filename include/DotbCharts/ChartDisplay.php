...
class ChartDisplay
{
    ...
    /**
     * Return the DotbChart's object with all the values set and ready for display/consumption.
     *
     * @return JitReports|string
     */
    public function getDotbChart()
    {
        if ($this->canDrawChart()) {

            /* @var $dotbChart JitReports */
            $dotbChart = DotbChartFactory::getInstance('', 'Reports');

            $dotbChart->setData($this->chartRows);
            global $do_thousands;
            ...
            if (isset($reportDef['group_defs'])) {
                ...
                foreach ($reportDef['group_defs'] as $group_def) {
                    ...
                    // check for any unsupported drillthru fields for lumia modules
                    if ($allowDrillthru) {
                ...
                $dotbChart->setDisplayProperty('groupName', $groupByLabels[0]);
                $dotbChart->setDisplayProperty('groupType', $groupByTypes[0]);
                $xDataType = $groupByTypes[0] === 'string' ? 'ordinal' : $groupByTypes[0];

                if (isset($groupByLabels[1])) {
                    $dotbChart->setDisplayProperty('seriesName', $groupByLabels[1]);
                    $dotbChart->setDisplayProperty('seriesType', $groupByTypes[1]);
                }

                $dotbChart->group_by = $groupByNames;
            }

            $dotbChart->setDisplayProperty('xDataType', $xDataType);
            $dotbChart->setDisplayProperty('yDataType', $yDataType);
            $dotbChart->setDisplayProperty('allow_drillthru', $allowDrillthru);
    ...
    public function legacyDisplay($id, $is_dashlet = false)
    {
        ...
        $dotbChart = $this->getDotbChart();
        if (is_object($dotbChart)) {
            $dotbChart->reporter = $this->reporter;
            $xmlFile = $this->get_cache_file_name();
            $dotbChart->saveXMLFile($xmlFile, $dotbChart->generateXML());
            return $dotbChart->display($guid, $xmlFile, $width, $height);
        }
    ...
    /**
     * Generate the xml string from the DotbChart Object
     *
     * @return string
     */
    public function generateXML()
    {
        return $this->getDotbChart()->generateXML();
    }
    ...
    {
        return $this->getDotbChart()->buildJson($this->generateXML(), true);
    }
}
