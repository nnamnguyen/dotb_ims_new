...
function loadDotbChart(chartId, jsonFilename, css, chartConfig, chartParams, callback) {
    // get chartId from params or use the default for dotb
    ...
    var params = _.extendOwn({
        ...
        label: DOTB.charts.translateString('LBL_DASHLET_SAVED_REPORTS_CHART'),
    ...
    var myLocale = DOTB.charts.getLocale();
    var tooltipTemplate = DOTB.charts._getTooltipTemplate(chartConfig.chartType);

    // chart display strings
    var chartStrings = DOTB.charts.getChartStrings(chartConfig.chartType);
    var displayErrorMsg = DOTB.charts.translateString('LBL_CANNOT_DISPLAY_CHART_MESSAGE', 'Reports');
    var noDataAvailable = DOTB.charts.translateString('LBL_NO_DATA_AVAILABLE');

    // get and save the fiscal start date
    DOTB.charts.defineFiscalYearStart();

    // instantiate Sucrose chart
    switch (chartConfig.chartType) {

        case 'barChart':
            DOTB.charts.get(jsonFilename, params, function(data) {
                var json;
                var barChart;
                // it will be set to chart.locality() after instantiation
                var locality = {};

                if (DOTB.charts.dataIsEmpty(data)) {
                    DOTB.charts.renderError(chartId, noDataAvailable);
                    return;
                }

                json = DOTB.charts.transformDataToD3(data, params, chartConfig);

                if (json.properties && json.properties.labels && json.properties.labels.length > 50) {
                    DOTB.charts.renderError(chartId, displayErrorMsg);
                    return;
                }
                ...
                barChart = sucrose.charts.multibarChart()
                    ...
                    .tooltipContent(function(eo, properties) {
                        return DOTB.charts.formatTooltipMultiple(eo, properties, barChart, tooltipTemplate, params);
                    })
                ...
                if (isReportView) {
                    ...
                    DOTB.charts.trackWindowResize(barChart);

                    if (imageExportType) {
                        DOTB.charts.saveImageFile(chartId, barChart, json, jsonFilename, imageExportType);
                    } else {
                        DOTB.charts.renderChart(chartId, barChart, json);
                    }
                } else {
                    DOTB.charts.renderChart(chartId, barChart, json);
                ...
                DOTB.charts.callback(callback, barChart, chartId, params, data);
            });
            break;
        ...
        case 'lineChart':
            DOTB.charts.get(jsonFilename, params, function(data) {
                ...
                lineChart = sucrose.charts.lineChart()
                    ...
                    .tooltips(params.show_tooltips)
                    .tooltipContent(function(eo, properties) {
                        return DOTB.charts.formatTooltipMultiple(eo, properties, lineChart, tooltipTemplate, params);
                    })
                    .direction(params.direction)
                    .showTitle(params.show_title)
                    .showLegend(params.show_legend)
                ...
                if (json.data.length) {
                    ...
                    if (xTickLabels.length > 0) {
                        ...
                        tickFormat = function(d) { return d3dotb.timeFormat('%x')(new Date(d)); };
                    }
                }
                ...
                if (isReportView) {
                    ...
                    DOTB.charts.trackWindowResize(lineChart);

                    if (imageExportType) {
                        DOTB.charts.saveImageFile(chartId, lineChart, json, jsonFilename, imageExportType);
                    } else {
                        DOTB.charts.renderChart(chartId, lineChart, json);
                    }
                } else {
                    DOTB.charts.renderChart(chartId, lineChart, json);
                ...
                DOTB.charts.callback(callback, lineChart, chartId, params, data);
            });
            break;
        ...
        case 'pieChart':
            DOTB.charts.get(jsonFilename, params, function(data) {
                ...
                pieChart = sucrose.charts.pieChart()
                    ...
                    .tooltips(params.show_tooltips)
                    .tooltipContent(function(eo, properties) {
                        return DOTB.charts.formatTooltipSingle(eo, properties, pieChart, tooltipTemplate, params);
                    })
                    .showTitle(params.show_title)
                    .showLegend(params.show_legend)
                    .colorData(params.colorData)
                    ...
                    //     var n = d3dotb.select('#d3_' + chartId).node(),
                ...
                if (isReportView) {
                    ...
                    DOTB.charts.trackWindowResize(pieChart);

                    if (imageExportType) {
                        DOTB.charts.saveImageFile(chartId, pieChart, json, jsonFilename, imageExportType);
                    } else {
                        DOTB.charts.renderChart(chartId, pieChart, json);
                    }
                } else {
                    DOTB.charts.renderChart(chartId, pieChart, json);
                ...
                DOTB.charts.callback(callback, pieChart, chartId, params, data);
            });
            break;
        ...
        case 'funnelChart':
            DOTB.charts.get(jsonFilename, params, function(data) {
                ...
                funnelChart = sucrose.charts.funnelChart()
                    ...
                    .tooltips(params.show_tooltips)
                    .tooltipContent(function(eo, properties) {
                        return DOTB.charts.formatTooltipSingle(eo, properties, funnelChart, tooltipTemplate, params);
                    })
                    .direction(params.direction)
                    .colorData(params.colorData)
                    .fmtValue(function(d) {
                ...
                if (isReportView) {
                    ...
                    DOTB.charts.trackWindowResize(funnelChart, chartId, data);

                    if (imageExportType) {
                        DOTB.charts.saveImageFile(chartId, funnelChart, json, jsonFilename, imageExportType);
                    } else {
                        DOTB.charts.renderChart(chartId, funnelChart, json);
                    }
                } else {
                    DOTB.charts.renderChart(chartId, funnelChart, json);
                ...
                DOTB.charts.callback(callback, funnelChart, chartId, params, data);
            });
            break;
        ...
        case 'gaugeChart':
            DOTB.charts.get(jsonFilename, params, function(data) {
                ...
                json = DOTB.charts.transformDataToD3(data, params, chartConfig);
                maxValue = d3dotb.max(json.data.map(function(d) { return d.y; }));
                ...
                if (isReportView) {
                    ...
                    DOTB.charts.trackWindowResize(gaugeChart);

                    if (imageExportType) {
                        DOTB.charts.saveImageFile(chartId, gaugeChart, json, jsonFilename, imageExportType);
                    } else {
                        DOTB.charts.renderChart(chartId, gaugeChart, json);
                    }
                } else {
                    DOTB.charts.renderChart(chartId, gaugeChart, json);
                }

                DOTB.charts.callback(callback, gaugeChart, chartId, params, data);
            });
            break;
    }
}
    ...
    DOTB.charts = {
        ...
        callback: function(callback, chart, chartId, params, chartData) {
            ...
            chart.seriesClick(_.bind(function(data, eo, chart, labels) {
                ...
                this.dotbApp.alert.show('listfromreport_loading', {
                    level: 'process',
        ...
        defineFiscalYearStart: function() {
            ...
            this.dotbApp.api.call('GET', this.dotbApp.api.buildURL('TimePeriods/' + fiscalYear + '-01-01'), null, {
        ...
        setFiscalStartDate: function(firstQuarter) {
            ...
            this.dotbApp.cache.set('fiscaltimeperiods', {'annualDate': fiscalYearStart});
        },
        ...
        getFiscalStartDate: function() {
            var timeperiods = this.dotbApp.cache.get('fiscaltimeperiods');
        ...
        getDateValues: function(label, type) {
            var dateParser = this.dotbApp.date;
            var userLangPref = this.dotbApp.user.getLanguage() || 'en_us';
            var datePatterns = {
                year: 'YYYY', // 2017
        ...
        getFieldDef: function(groupDef, reportDef) {
            ...
            if (groupDef.table_key === 'self') {
                return this.dotbApp.metadata.getField({name: groupDef.name, module: module});
            }

            // Need to parse something like 'Accounts:contacts:assigned_user_link:user_name'
            var relationships = groupDef.table_key.split(':');
            var fieldsMeta = this.dotbApp.metadata.getModule(module, 'fields');
            var fieldDef;
            for (var i = 1; i < relationships.length; i++) {
                ...
                fieldsMeta = this.dotbApp.metadata.getModule(module, 'fields');
            }
        ...
        _getModuleFromRelationship: function(relationshipName, module) {
            var relationship = this.dotbApp.metadata.getRelationship(relationshipName);
        ...
        openDrawer: function(drawerContext) {
            var currentModule = this.dotbApp.drawer.context.get('module');

            // This needs to set to target module for Merge to show the target module fields
            this.dotbApp.drawer.context.set('module', drawerContext.chartModule);

            this.dotbApp.drawer.open({
                layout: 'drillthrough-drawer',
                context: drawerContext
            }, _.bind(function() {
                // reset the drawer module
                if (currentModule) {
                    this.dotbApp.drawer.context.set('module', currentModule);
                }
            }, this, currentModule));
        },
        ...
        renderError: function(id, message) {
            $('#d3_' + id).empty();
            d3dotb.select('.reportChartContainer')
                .style('height', 'auto');
            d3dotb.select('.reportChartContainer .chartContainer')
                .style('float', 'none')
        ...
        translateString: function(appString, module) {
            if (DOTB.App) {
                // Lumia
                if (module) {
                    return DOTB.App.lang.get(appString, module);
                } else {
                    return DOTB.App.lang.get(appString);
                }
            ...
            } else if (DOTB.language) {
                // BWC not works?
                if (module) {
                    return DOTB.language.get(module, appString);
                } else {
                    return DOTB.language.get('app_strings', appString);
                }
            } else {
                return appString;
            }
        ...
        translateListStrings: function(appList) {
            if (DOTB.App) {
                // Lumia
                return DOTB.App.lang.getAppListStrings(appList);
            } else if (app) {
                // BWC works
                return app.lang.getAppListStrings(appList);
            } else if (DOTB.language) {
                // BWC not works?
                return DOTB.language.get('app_list_strings', appList);
            } else {
                return appList;
            }
        },
        ...
        translateDataToD3: function(json, params, config) {
            var msg = 'The DOTB.charts.translateDataToD3(json, params, config) method is deprecated. ' +
                    'Please use DOTB.charts.transformDataToD3(json, params, config) method instead.';
            this.dotbApp.logger.warn(msg);
                ...
                switch (config.chartType) {
                    ...
                    case 'funnelChart':
                        data = json.values.map(function(d, i) {
                            ...
                            // some data provided to dotbCharts do not include
        ...
        saveImageFile: function(id, chart, json, jsonfilename, imageExt, saveTo, complete) {
            ...
            d3dotb.select(d3ChartId + ' svg').remove();

            d3dotb.select(d3ChartId)
                .append('svg')
            ...
            d3dotb.select(d3ChartId).selectAll('.sc-axis line')
              .style('stroke', '#DDD')
         ...
         * @param template Lumia handlebars template view
         * @param params chart display control parameters
         */
        formatTooltipSingle: function(eo, properties, chart, template, params) {
            ...
            point.value = eo.data && eo.data.label ?
                eo.data.label :
                yIsCurrency ?
                    this.dotbApp.currency.formatAmountLocale(value, locale.currency_id) :
            ...
            if (!_.isUndefined(key) && key !== label) {
                ...
                point.seriesLabel = seriesType === 'string' ?
                    ...
                    seriesType === 'numeric' ?
                        ...
                        seriesType === 'currency' ?
                            this.dotbApp.currency.formatAmountLocale(key, locale.currency_id) :
                            key;
            }
        ...
        getUserPreferences: function() {
            return this.dotbApp.user.get('preferences') || {};
        },
        ...
        userPreference: function(pref) {
            return this.dotbApp.user.getPreference(pref) || pref;
        },
        ...
        _getSystemPreferences: function() {
            var config = this.dotbApp.config;
            var currency = this.dotbApp.currency;
        ...
        /**
         * Given a Dotb user time format preference
        ...
        /**
         * Given the name of a Dotb language pack set of date strings
        ...
        _getTooltipTemplate: function(type) {
            ...
            return this.dotbApp.template.getField('chart', template);
        }
    };
})(jQuery);
