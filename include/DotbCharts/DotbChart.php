...
class DotbChart
{
    ...
    public function __construct()
    {
        ...
        if ($GLOBALS['current_user']->getPreference('currency')) {
            ...
            $this->currency_symbol = $GLOBALS['dotb_config']['default_currency_symbol'];
            $this->div = 1;
    ...
    /**
     * returns the header for the constructed xml file for dotbcharts
     *
     * @param     nothing
    ...
    function xmlHeader()
    {
        ...
        $header .= "<dotbcharts version=\"1.0\">\n";

        return $header;
    }

    /**
     * returns the footer for the constructed xml file for dotbcharts
     *
     * @param     nothing
    ...
    /**
     * returns the properties tag for the constructed xml file for dotbcharts
     *
     * @param     nothing
     ...
     *
     * @param $dotbSetValue Mixed value
    ...
    function saveXMLFile($xmlFilename, $xmlContents)
    {
        ...
        if (!dotb_is_dir($dir)) {
            dotb_mkdir($dir, null, true);
        }

        // open file
        if (!$fh = dotb_fopen($xmlFilename, 'w')) {
    ...
    function generateChartStrings($xmlFile)
    {
        ...
        $chartStringsXML .= "<dotblanguage version=\"1.0\">\n";
    ...
    function display($name, $xmlFile, $width = '320', $height = '480', $resize = false)
    {
        ...
        global $current_language, $theme, $dotb_config,$app_strings;

        $this->app_strings = $app_strings;
        $this->chartStringsXML = dotb_cached("xml/").'chart_strings.' . $current_language .'.lang.xml';
    ...
    function getDashletScript($id, $xmlFile = "")
    {
        ...
        $xmlFile = (!$xmlFile) ? $dotb_config['tmp_dir']. $current_user->id . '_' . $this->id . '.xml' : $xmlFile;
        $chartStringsXML = $GLOBALS['dotb_config']['tmp_dir'].'chart_strings.' . $current_language .'.lang.xml';

        $this->ss->assign('chartName', $id);
        $this->ss->assign('chartXMLFile', $xmlFile);
        $this->ss->assign('chartStyleCSS', DotbThemeRegistry::current()->getCSSURL('chart.css'));
        $this->ss->assign('chartColorsXML', DotbThemeRegistry::current()->getImageURL('dotbColors.xml'));
        $this->ss->assign('chartLangFile', $GLOBALS['dotb_config']['tmp_dir'].'chart_strings.' . $GLOBALS['current_language'] .'.lang.xml');
