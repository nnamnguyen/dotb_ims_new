...
class JsChart extends DotbChart {
	protected $ss;
	var $xmlFile;
	var $jsonFilename;
	var $chartId;
	...
	function getDashletScript($id,$xmlFile="") {
		...
		global $dotb_config, $current_user, $current_language;
		$this->id = $id;
		$this->chartId = $id;
		$this->xmlFile = (!$xmlFile) ? dotb_cached("xml/".$current_user->getUserPrivGuid()."_{$this->id}.xml") : $xmlFile;
	...
	function getConfigProperties() {
		$path = DotbThemeRegistry::current()->getImageURL('dotbColors.xml',false);
	...
	function saveJsonFile($jsonContents) {
		...
		// open file
		if (!$fh = dotb_fopen($this->jsonFilename, 'w')) {
