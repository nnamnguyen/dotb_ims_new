 ...
 */
function loadDotbChart (chartId, jsonFilename, css, chartConfig, params, callback) {
                        ...
                        if(DOTB.charts.isDataEmpty(data)){
                            ...
                            var paretoChart = nv.models.paretoChart()
                                ...
                                .tooltipBar(function(key, x, y, e, graph) {
                                    ...
                                    return '<p>' + DOTB.App.lang.get('LBL_SALES_STAGE', 'Forecasts') + ': <b>' + key + '</b></p>' +
                                        '<p>' + DOTB.App.lang.get('LBL_AMOUNT', 'Forecasts') + ': <b>' + val + '</b></p>' +
                                        '<p>' + DOTB.App.lang.get('LBL_PERCENT', 'Forecasts') + ': <b>' + x + '%</b></p>'
                                })
                                .showControls(false)
                                .colorData( 'default' )
                                .colorFill( 'default' )
                                ...
                                .strings({
                                    legend: {
                                        close: DOTB.App.lang.get('LBL_CHART_LEGEND_CLOSE'),
                                        open: DOTB.App.lang.get('LBL_CHART_LEGEND_OPEN')
                                    },
                                    noData: DOTB.App.lang.get('LBL_CHART_NO_DATA')
                                });
                            ...
                            d3.select('#' + chartId)
                                .append('svg')
                                .datum( DOTB.charts.translateDataToD3(json,params) )
                            ...
                            DOTB.charts.setChartObject(paretoChart);
                        }
                        DOTB.charts.callback(callback);
                    });
                    break;
			case "barChart":
                DOTB.charts.get(jsonFilename, params, function(data) {
                    if(DOTB.charts.isDataEmpty(data)){
                                ...
                                onShow: function(tip, elem) {
                                    ...
                                    if(elem.type == 'marker') {
                                        ...
                                        if(elem.link != 'undefined' && elem.link != undefined && elem.link != '') {
                                            drillDown = ($jit.util.isTouchScreen()) ? "<br><a href='"+ elem.link +"'>"+DOTB.language.get('app_strings','LBL_CLICK_TO_DRILLDOWN')+"</a>" : "<br>"+DOTB.language.get('app_strings','LBL_CLICK_TO_DRILLDOWN');
                                        } else {
                                            drillDown = "";
                                        }
                        ...
                        var list = DOTB.charts.generateLegend(barChart, chartId);
                        ...
                        DOTB.charts.trackWindowResize(barChart, chartId, data);
                        barChart.json = json;
                        that.chartObject = barChart;

                        DOTB.charts.setChartObject(barChart);
			...
			case "lineChart":
                DOTB.charts.get(jsonFilename, params, function(data) {
                    if(DOTB.charts.isDataEmpty(data)){
                        ...
                        var list = DOTB.charts.generateLegend(lineChart, chartId);

                        //save canvas to image for pdf consumption
                        $jit.util.saveImageTest(chartId,jsonFilename,chartConfig["imageExportType"]);

                        DOTB.charts.trackWindowResize(lineChart, chartId, data);
			...
			case "pieChart":
                DOTB.charts.get(jsonFilename, params, function(data) {
                    if(DOTB.charts.isDataEmpty(data)){
                        ...
                        //end
                        //dynamically add legend to list
                        var list = DOTB.charts.generateLegend(pieChart, chartId);


                        //save canvas to image for pdf consumption
                        $jit.util.saveImageTest(chartId,jsonFilename,chartConfig["imageExportType"]);

                        DOTB.charts.trackWindowResize(pieChart, chartId, data);
			...
			case "funnelChart":
                DOTB.charts.get(jsonFilename, params, function(data) {
                    if(DOTB.charts.isDataEmpty(data)){
                        ...
                        //dynamically add legend to list
                        var list = DOTB.charts.generateLegend(funnelChart, chartId);
			...
			case "gaugeChart":
                DOTB.charts.get(jsonFilename, params, function(data) {
                    if(DOTB.charts.isDataEmpty(data)){
                        ...
                        var list = DOTB.charts.generateLegend(gaugeChart, chartId);

                        //save canvas to image for pdf consumption
                        $jit.util.saveImageTest(chartId,jsonFilename,chartConfig["imageExportType"]);

                        DOTB.charts.trackWindowResize(gaugeChart, chartId, data);
