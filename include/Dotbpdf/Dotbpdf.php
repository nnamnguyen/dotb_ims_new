...
DotbAutoLoader::requireWithCustom('include/Dotbpdf/dotbpdf_config.php');
require_once('vendor/tcpdf/tcpdf.php');
require_once('include/Dotbpdf/DotbpdfHelper.php');

use Dotbcrm\Dotbcrm\Security\InputValidation\InputValidation;
use Dotbcrm\Dotbcrm\Security\Crypto\Blowfish;

class Dotbpdf extends TCPDF
{
    /**
    ...
    function __construct($bean = null, $dotbpdf_object_map = array(),$orientation=PDF_PAGE_ORIENTATION, $unit=PDF_UNIT, $format=PDF_PAGE_FORMAT, $unicode=true, $encoding='UTF-8', $diskcache=false){
        global $locale;
      //  $encoding = $locale->getExportCharset();
        ...
        $this->bean = $bean;
        $this->dotbpdf_object_map = $dotbpdf_object_map;
        if(!empty($_REQUEST["dotbpdf"])){
            $request = InputValidation::getService();
            $this->action = $request->getValidInputRequest('dotbpdf', array('Assert\Regex' => array('pattern' => '/^[a-z0-9_-]+$/i')));
        }
    }
    ...
    function preDisplay(){
        ...
        $this->SetProtection(
            $protection,
            Blowfish::decode(Blowfish::getKey('dotbpdf_pdf_user_password'), PDF_USER_PASSWORD),
            Blowfish::decode(Blowfish::getKey('dotbpdf_pdf_owner_password'), PDF_OWNER_PASSWORD)
        );
