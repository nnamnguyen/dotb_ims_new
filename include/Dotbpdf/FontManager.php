...
class FontManager{
    ...
    /**
     * This method parse the font path defined in the dotbpdf config file
    ...
    public function listFontFiles(){
        $this->fontList=array();
        if(file_exists($cachedfile = dotb_cached("Dotbpdf/cachedFontList.php"))) {
            require $cachedfile;
            $this->fontList=$cachedFontList;
            return true;
        }else{
            if($this->parseFolder()){
                $cacheDir = create_cache_directory('Dotbpdf/');
    ...
    /**
     * This method add a font to DotBCRM from a font file and a metric file using MakeFont()
        ...
        if(empty($filename)){
            ...
            if(!$this->loadFontFile()){
                if(!mkdir_recursive(K_PATH_CUSTOM_FONTS)){
                    ...
                    dotb_rename($filename.".php", K_PATH_CUSTOM_FONTS.basename($filename.$styleLetter.".php"));
                    ...
                    if(file_exists($filename.".z")){
                        dotb_rename($filename.".z", K_PATH_CUSTOM_FONTS.basename($filename.$styleLetter.".z"));
                    ...
                    if(file_exists($filename.".ctg.z")){
                        dotb_rename($filename.".ctg.z", K_PATH_CUSTOM_FONTS.basename($filename.$styleLetter.".ctg.z"));
