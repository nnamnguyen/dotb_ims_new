...
class DotbpdfPdfmanager extends DotbpdfSmarty
{
    ...
    public function preDisplay()
    {
        ...
        if ($this->module == 'Quotes' && $previewMode === FALSE) {
            ...
            $fields['taxrate_value'] = format_number_dotbpdf($this->bean->taxrate_value, $locale->getPrecision(), $locale->getPrecision(), array('percentage' => true));;
            ...
            foreach ($product_bundle_list as $ordered_bundle) {
                ...
                foreach ($product_bundle_line_items as $product_bundle_line_item) {
                    ...
                    if ($product_bundle_line_item->object_name == "ProductBundleNote") {
                        ...
                        $bundleFields['products'][$count]['ext_price'] = format_number_dotbpdf($product_bundle_line_item->discount_price * $product_bundle_line_item->quantity, $locale->getPrecision(), $locale->getPrecision(), $format_number_array);                                        
                    }
    ...
    private function buildTemplateFile($pdfTemplate, $previewMode = FALSE)
    {
        if (!empty($pdfTemplate)) {
            ...
            $tpl_filename = dotb_cached('modules/PdfManager/tpls/' . $pdfTemplate->id . '.tpl');

            if ($previewMode !== FALSE) {
                $tpl_filename = dotb_cached('modules/PdfManager/tpls/' . $pdfTemplate->id . '_preview.tpl');
            ...
            dotb_file_put_contents($tpl_filename, $pdfTemplate->body_html);
    ...
    public function Output($name="doc.pdf", $dest='I')
    {
        ...
        if (isset($_REQUEST['to_email']) && $_REQUEST['to_email']=="1") {
            ...
            if($email_id=="") {
                ...
                DotbApplication::redirect("index.php?module=Emails&action=Compose&record=".$email_id."&replyForward=true&reply=");
            }
        }
