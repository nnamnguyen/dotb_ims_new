 ...
 */
abstract class DotbCacheAbstract
{
    /**
    ...
    {
        if ( isset($GLOBALS['dotb_config']['cache_expire_timeout']) )
            $this->_expireTimeout = $GLOBALS['dotb_config']['cache_expire_timeout'];
        if ( isset($GLOBALS['dotb_config']['unique_key']) )
            $this->_keyPrefix = $GLOBALS['dotb_config']['unique_key'];
    }
    ...
    {
        $this->_cacheRequests++;
        if ( !DotbCache::$isCacheReset && (!$this->useLocalStore || !isset($this->_localStore[$key]))) {
    ...
    public function set($key, $value, $ttl = null)
    {
        if ( is_null($value) )
        {
            $value = DotbCache::EXTERNAL_CACHE_NULL_VALUE;
        }
    ...
    {
        return strtolower(str_replace('DotbCache','',get_class($this)));
    }
    ...
    {
        if ( !empty($GLOBALS['dotb_config']['external_cache_disabled'])
                && $GLOBALS['dotb_config']['external_cache_disabled'] == true ) {
            return false;
        }

        if (defined('DOTBCRM_IS_INSTALLING')) {
            return false;
        }

        if ( isset($GLOBALS['dotb_config']['external_cache_force_backend'])
                && ( $GLOBALS['dotb_config']['external_cache_force_backend'] != (string) $this ) ) {
            return false;
        }
