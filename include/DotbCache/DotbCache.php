...
class DotbCache
{
    const EXTERNAL_CACHE_NULL_VALUE = "DOTB_CACHE_NULL_ZZ";
    ...
    {
        self::$_cacheInstance = new DotbCachePsr(CacheInterface::class, 1000, null);
    }

    /**
     * Elects a backend based on their priority and availability
     *
     * @return DotbCacheAbstract|null
     */
    public static function electBackend()
    {
        /** @var DotbCacheAbstract $backend */
        $backend = null;
        $lastPriority = 1000;

        $locations = DotbAutoLoader::getFilesCustom('include/DotbCache');

        foreach ($locations as $location) {
            $class = basename($location, '.php');

            if ($class === 'DotbCache' || $class === 'DotbCachePsr') {
                continue;
            }

            require_once $location;

            if (class_exists($class) && is_subclass_of($class, 'DotbCacheAbstract')) {
                $GLOBALS['log']->debug("Found cache backend $class");

                /** @var DotbCacheAbstract $instance */
                $instance = new $class();
    ...
    /**
     * Returns the instance of the DotbCacheAbstract object, cooresponding to the external
     * cache being used.
     *
     * @return DotbCacheAbstract
     *
     * @deprecated use Psr\SimpleCache\CacheInterface instead
     */
    public static function instance()
    {
        if ( !is_subclass_of(self::$_cacheInstance,'DotbCacheAbstract') )
            self::_init();
 ...
 * @return mixed
 */
function dotb_cache_retrieve($key)
{
    return DotbCache::instance()->$key;
}
 ...
 * @param int|null $ttl
 */
function dotb_cache_put($key, $value, $ttl = null)
{
    DotbCache::instance()->set($key,$value, $ttl);
}
 ...
 */
function dotb_cache_clear($key)
{
    unset(DotbCache::instance()->$key);
}
 ...
 */
function dotb_cache_reset()
{
    DotbCache::instance()->reset();
    DotbCache::cleanOpcodes();
}

/**
 * Flush the cache in its entirety including the local and external store along with the opcodes.
 */
function dotb_cache_reset_full()
{
    DotbCache::instance()->resetFull();
    DotbCache::cleanOpcodes();
}
