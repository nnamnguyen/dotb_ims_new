...
/**
 * SQL DotbCache backend - currently only MySQL backends are supported
 * awaiting full optimized upsert capability on the other DB backends.
 *
 * @deprecated
 */
class DotbCacheDb extends DotbCacheAbstract
{
    /**
     * @var DBManager
     */
    ...
    public function useBackend()
    {
        ...
        if ($this->dotbConfig->get('external_cache_disabled_db', false)) {
            return false;
        }
    ...
    public function __construct()
    {
        ...
        $this->dotbConfig = DotbConfig::getInstance();

        // garbage collection settings
        $this->gcProbability = $this->dotbConfig->get('external_cache_db_gc_probability', $this->gcProbability);
        $this->gcThreshold = $this->dotbConfig->get('external_cache_db_gc_threshold', $this->gcThreshold);
    }
    ...
    protected function _setExternal($key, $value)
    {
        ...
        $this->logger->debug("DotbCacheDb writing key '$key' using hash '$hashedKey'");
    ...
    protected function gc()
    {
        ...
        $msg = "DotbCacheDb running garbage collection - took $runtime msecs";
    ...
    protected function decode($data, $key)
    {
        ...
        if ($decodeData === false) {
            $this->logger->error("DotbCacheDb base64 decode failure for key '$key'");
            return null;
        }

        $value = @unserialize($decodeData);
        if ($value === false) {
            $this->logger->error("DotbCacheDb unserialize failure for key '$key'");
            return null;
        }
