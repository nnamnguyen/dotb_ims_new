...
(function() {
	...
	if (DOTB.EmailAddressWidget) return;
	
	var Dom = YAHOO.util.Dom;
	
	DOTB.EmailAddressWidget = function(module) {
        this.idmMode = {
            disabledForModule: false,
            cloudConsoleUrl: ''
        };
		if (!DOTB.EmailAddressWidget.count[module]) DOTB.EmailAddressWidget.count[module] = 0;
		this.count = DOTB.EmailAddressWidget.count[module];
		DOTB.EmailAddressWidget.count[module]++;
		this.module = module;
		...
		DOTB.EmailAddressWidget.instances[this.id] = this;
	}
	
	DOTB.EmailAddressWidget.instances = {};
	DOTB.EmailAddressWidget.count = {};
	
	DOTB.EmailAddressWidget.prototype = {
		...
		'<td><span>&nbsp;</span><img id="removeButton0" name="0" src="index.php?entryPoint=getImage&amp;themeName=Dotb&amp;imageName=delete_inline.gif"/></td>' +
		...
		retrieveEmailAddress: function (event) {
            var callbackFunction = function success(data) {
		        ...
		        if(savePressed || this.enterPressed) {
		           setTimeout("DOTB.EmailAddressWidget.instances." + this.id + ".forceSubmit()", 2100);
		    ...
		    if(/emailAddress\d+$/.test(targetEl.id) && isValidEmail(targetEl.value) && !this.verifying && verifyElementFlag.value == "false") {
		        ...
		        verifyElementFlag.parentNode.appendChild(verifiedTextNode);
		        verifiedTextNode.innerHTML = DOTB.language.get('app_strings', 'LBL_VERIFY_EMAIL_ADDRESS');
		        this.verifying = true;
		...
		addEmailAddress : function (tableId, address, primaryFlag, replyToFlag, optOutFlag, invalidFlag, emailId) {
            ...
            //else default to 0 
            var tabIndexCount = 0;
            if(typeof(DOTB.TabFields) !='undefined' && typeof(DOTB.TabFields['email1']) != 'undefined'){
                tabIndexCount = DOTB.TabFields['email1'];
            }
            ...
            newContent.setAttribute("title", DOTB.language.get('app_strings', 'LBL_EMAIL_TITLE'));
            ...
            removeButtonImg.setAttribute('src', "index.php?entryPoint=getImage&themeName="+DOTB.themes.theme_name+"&imageName=id-ff-remove-nobg.png");
		    ...
		    addToValidateVerified(this.emailView, this.id + "emailAddressVerifiedFlag" + this.numberEmailAddresses, 'bool', false, DOTB.language.get('app_strings', 'LBL_VERIFY_EMAIL_ADDRESS'));
		    ...
		    if(primaryFlag == '1' || (this.numberEmailAddresses == 0)) {
                ...
                newContent.setAttribute("title", DOTB.language.get('app_strings', 'LBL_EMAIL_PRIM_TITLE'));
		    }
		    ...
		    if(optOutFlag == '1') {
                ...
                newContent.setAttribute("title", DOTB.language.get('app_strings', 'LBL_EMAIL_OPT_TITLE'));
		    }
		    
		    if(invalidFlag == '1') {
		        newContentInvalidFlag.setAttribute("checked", "true");
                newContent.setAttribute("title", DOTB.language.get('app_strings', 'LBL_EMAIL_INV_TITLE'));
		    }
		    newContent.eaw = this;
            ...
            if (YAHOO.env.ua.ie > 0) {
                ...
                YAHOO.util.Event.addListener(newContent, "change",
                        function(ev, el){DOTB.util.callOnChangeListers(el);}, emailcontainer
                );
            }
		    
		    // Add validation to field
            this.EmailAddressValidation(this.emailView, this.id+ 'emailAddress' + this.numberEmailAddresses,this.emailIsRequired, DOTB.language.get('app_strings', 'LBL_EMAIL_ADDRESS_BOOK_EMAIL_ADDR'));
    ...
    function () { addToValidate(ev, fn, 'email', r, stR);});
        ...
        showErrDismissUpdatePrimaryEmail: function() {
            parent.DOTB.App.alert.show('dismiss_delete_primary_email', {
                level: 'error',
                messages: DOTB.language
		...
		forceSubmit : function () {
            ...
            if(theForm) {
               ...
               if(this.emailView == 'EditView') {
                  ...
                  DOTB.subpanelUtils.inlineSave(theForm.id, theForm.module.value+'_subpanel_save_button');
               }
            }
        } //forceSubmit
    };
