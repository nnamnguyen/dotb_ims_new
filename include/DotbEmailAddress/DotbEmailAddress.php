...
class DotbEmailAddress extends DotbBean
{
    ...
    public function __construct() {
        ...
        $this->opt_out = !empty($GLOBALS['dotb_config']['new_email_addresses_opted_out']);
        self::$count++;
    }
    ...
    function handleLegacySave($bean, $prefix = "")
    {
        ...
        if (!isset($_REQUEST) || !isset($_REQUEST['useEmailWidget'])) {
            if (empty($this->addresses) || !isset($_REQUEST['massupdate'])) {
                ...
                // determine if what is being saved is a dotb7 style $bean->email
     ...
     * 
     * @param DotbBean $bean The bean to get emails for
     ...
     * @param DotbBean $bean
     */
    public function populateLegacyFields(DotbBean $bean)
    {
        $primary = null;
        $alternate = array();
    ...
    public function addAddress($addr, $primary=false, $replyTo=false, $invalid=false, $optOut=false, $email_id = null, $validate = true) {
        ...
        if ($validate && !DotbEmailAddress::isValidEmail($addr)) {
            $GLOBALS['log']->fatal("DOTBEMAILADDRESS: email address is not valid [ {$addr} ]");
            return false; 
        }
        ...
        foreach ($this->addresses as $k => $address) {
            if ($address['email_address'] == $addr) {
                ...
                if ($this->boolVal($address['primary_address']) && !empty($diffCount)) {
                    $GLOBALS['log']->fatal("DOTBEMAILADDRESS: Existing primary address could not be overriden [ {$addr} ]");
                    return false;
                }
            }
        }
     ...
     * @return bool
     */
    public function removeLegacyAddressForBean(DotbBean $bean, ?string $addr)
    {
        $found = false;
        for ($i = 1; $i <= self::MAX_LEGACY_EMAILS; $i++) {
            $email = 'email' . $i;
    ...
    public function splitEmailAddress($addr)
    {
        ...
        if (!DotbEmailAddress::isValidEmail($email)) {
     ...
     * @param String $id - the GUID of the original DotbEmailAddress bean,
     *        in case a "email has changed" WorkFlow has triggered - hack to allow workflow-induced changes
     *        to propagate to the new DotbEmailAddress - see bug 39188
     ...
     * @param DotbEmailAddress $addrBean
     * @param string $address
     ...
     * @return bool
     */
    private function didEmailAddressChange(DotbEmailAddress $addrBean, string $address, $invalid, $opt_out)
    {
     ...
     *
     * @param DotbEmailAddress $seed
     * @param string $addressCaps
     * @param string|null $id
     *
     * @return null|DotbEmailAddress
     */
    private function getEmailAddrBean(DotbEmailAddress $seed, $addressCaps, $id)
    {
        if (!empty($id)) {
    ...
    {
        if (!($this->smarty instanceOf Dotb_Smarty))
            $this->smarty = new Dotb_Smarty();

        global $app_strings, $dictionary, $beanList;

        $idmConfig = new IdmConfig(\DotbConfig::getInstance());
        ...
        $template = empty($tpl) ? "include/DotbEmailAddress/templates/forEditView.tpl" : $tpl;
    ...
    function getEmailAddressWidgetWirelessEdit($id, $module, $asMetadata = false)
    {
        ...
        $newEmail = $this->smarty->fetch("include/DotbEmailAddress/templates/forWirelessEdit.tpl");
    ...
    function getEmailAddressWidgetDetailView($focus, $tpl = '')
    {
        ...
        $templateFile = empty($tpl) ? "include/DotbEmailAddress/templates/forDetailView.tpl" : $tpl;
    ...
    function getEmailAddressWidgetDuplicatesView($focus)
    {
        ...
        return $this->smarty->fetch("include/DotbEmailAddress/templates/forDuplicatesView.tpl");
    }
    ...
    /**
     * Returns the boolean representation of possible dotb email address DB values.
