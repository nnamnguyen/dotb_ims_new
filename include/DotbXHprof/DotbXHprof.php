 ...
 *
 * To run profiler you should call DotbXHprof::getInstance()->start();
 * To stop profiler you should call DotbXHprof::getInstance()->end();
 ...
 *
 * To see reports you can use https://github.com/dotbcrm/xhprof-viewer or https://github.com/lox/xhprof
 *
 * If you want to customize DotbXHprof you should create file in custom/include/DotbXHprof/ folder and name file as name of your custom class
 * Change $dotb_config['xhprof_config']['manager'] to be name of your custom class
 * Custom class has to extend from DotbXHprof
 * If custom class doesn't exist or doesn't extend from DotbXHprof then DotbXHprof be used
 *
 * To enable profiling you should add next properties to config_override.php
 *
 * @see DotbXHprof::$enable            for $dotb_config['xhprof_config']['enable']
 * @see DotbXHprof::$manager           for $dotb_config['xhprof_config']['manager']
 * @see DotbXHprof::$log_to            for $dotb_config['xhprof_config']['log_to']
 * @see DotbXHprof::$sample_rate       for $dotb_config['xhprof_config']['sample_rate']
 * @see DotbXHprof::$ignored_functions for $dotb_config['xhprof_config']['ignored_functions']
 * @see DotbXHprof::$flags             for $dotb_config['xhprof_config']['flags']
 * @see DotbXHprof::$track_sql             for $dotb_config['xhprof_config']['track_sql']
 * @see DotbXHprof::$track_sql_backtrace   for $dotb_config['xhprof_config']['track_sql_backtrace']
 * @see DotbXHprof::$track_elastic         for $dotb_config['xhprof_config']['track_elastic']
 * @see DotbXHprof::$track_elastic_backtrace   for $dotb_config['xhprof_config']['track_elastic_backtrace']
 * @see DotbXHprof::$save_to               for $dotb_config['xhprof_config']['save_to']
 * @see DotbXHprof::$filter_wt             for $dotb_config['xhprof_config']['filter_wt']
 * @see DotbXHprof::$memory_limit             for $dotb_config['xhprof_config']['memory_limit']
 * @see DotbXHprof::$mongodb_uri           for $dotb_config['xhprof_config']['mongodb_uri']
 * @see DotbXHprof::$mongodb_db            for $dotb_config['xhprof_config']['mongodb_db']
 * @see DotbXHprof::$mongodb_collection    for $dotb_config['xhprof_config']['mongodb_collection']
 * @see DotbXHprof::$mongodb_options       for $dotb_config['xhprof_config']['mongodb_options']
 */
use Dotbcrm\Dotbcrm\Security\InputValidation\InputValidation;
use Dotbcrm\Dotbcrm\Performance\Dbal\XhprofLogger;

class DotbXHprof
{
    /**
     * @var DotbXHprof instance of profiler
     */
    protected static $instance = null;

    /**
     * @var bool enable profiler or not, it will be disabled by some reasons
     * @see DotbXHprof::loadConfig()
     */
    protected static $enable = false;

    /**
     * @var string class of manager for customization, has to extend from DotbXHprof class
     */
    ...
    /**
     * Populates configuration from $dotb_config to self properties
     */
    protected static function loadConfig()
    {
        if (!empty($GLOBALS['dotb_config']['xhprof_config'])) {
            foreach ($GLOBALS['dotb_config']['xhprof_config'] as $k => $v) {
            ...
            if (empty(static::$log_to)) {
                ...
                if (empty(static::$log_to)) {
                    ...
                    error_log("Warning: Must specify directory location for XHProf runs. " .
                        ...
                        . "\$dotb_config['xhprof_config']['log_to'] dotb config " .
        ...
        // enable DotbXhprofLogger class for Doctrine
    ...
    public static function getInstance()
    {
        ...
        static::$manager = !empty($GLOBALS['dotb_config']['xhprof_config']['manager']) ?
            $GLOBALS['dotb_config']['xhprof_config']['manager'] :
            static::$manager;

        if (is_file('custom/include/DotbXHprof/' . static::$manager . '.php')) {
            require_once 'custom/include/DotbXHprof/' . static::$manager . '.php';
        } elseif (is_file('include/DotbXHprof/' . static::$manager . '.php')) {
            require_once 'include/DotbXHprof/' . static::$manager . '.php';
        }
    ...
    static public function detectAction()
    {
        ...
        // index.php
        if (!empty($GLOBALS['app']) && $GLOBALS['app'] instanceof DotbApplication && $GLOBALS['app']->controller instanceof DotbController) {
        ...
        elseif (!empty($GLOBALS['service_object']) && $GLOBALS['service_object'] instanceof DotbSoapService) {
        ...
        elseif (!empty($GLOBALS['service_object']) && $GLOBALS['service_object'] instanceof DotbRestService) {
