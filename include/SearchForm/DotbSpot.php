...
class DotbSpot
{
    ...
    public function searchAndDisplay($query, $modules, $offset = -1)
    {
        ...
        return $ss->fetch(DotbAutoLoader::existingCustomOne('include/SearchForm/tpls/DotbSpot.tpl'));
	}
	...
	{
	    return DotbAutoLoader::loadSearchFields($moduleName);
	}
    ...
    protected function _performSearch($query, $modules, $offset = -1, $limit = 20, $options = array())
    {
        ...
        foreach ($modules as $moduleName) {
            ...
            $searchFields = DotbSpot::getSearchFields($moduleName);
            ...
            if ($limit < -1) {
                ...
                if ($limit == -1) {
                    $limit = $GLOBALS['dotb_config']['list_max_entries_per_page'];
                }
