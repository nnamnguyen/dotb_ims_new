...
class SearchForm
{
    ...
    public function display($header = true)
    {
		...
		DotbACL::listFilter($this->module, $this->fieldDefs, array("owner_override" => true),
    ...
    public function getOptions()
    {
     ...
     protected function locateFile($file)
     {
        ...
        return DotbAutoLoader::lookupFile($this->options['locator_class_params'][0], $file);
     }
  ...
  function displaySavedSearchSelect(){
    ...
    function _displayTabs($currentKey)
    {
        ...
        foreach ($values as $param => $value) {
            $script[] = 'DOTB.savedViews.' . $param . ' = ' . json_encode($value) . ';';
        }
     ...
     public function generateSearchWhere($add_custom_fields = false, $module='') {
         ...
         foreach($this->searchFields as $field=>$parms) {
             ...
             if ($type == 'int' && isset($parms['value']) && !empty($parms['value'])) {
                 $intField = DotbFieldHandler::getDotbField('int');
    ...
    private function isEmptyDropdownField($name='', $value=array())
    {
     ...
     public static function retrieveSearchDefs($module)
     {
         ...
         $searchdefs = array();
         $file = DotbAutoLoader::loadWithMetafiles($module, 'searchdefs');
         if($file) {
         	require $file;
         }
