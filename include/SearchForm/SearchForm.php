...
class SearchForm {
   /**
    ...
    public function __construct($module, $seedBean, $tpl = null)
    {
        ...
        if(empty($tpl)) {
            if(!empty($GLOBALS['layout_edit_mode'])){
            	 $this->tpl = dotb_cached('studio/custom/working/modules/' . $module . '/SearchForm.html');
            }
            else {
    ...
    function displayTabs($currentKey) {
        ...
        $tabPanel = new DotbWidgetTabs($this->tabs, $currentKey, 'DOTB.searchForm.searchFormSelect');
        ...
        $str .= '<script>$.extend(DOTB.savedViews, '.json_encode($params).');</script>';
    ...
    function displayHeader($view) {
        ...
        if(is_admin($current_user) && $module != 'DynamicLayout' && !empty($_SESSION['editinplace'])){
            $header_text = "<a href='index.php?action=index&module=DynamicLayout&from_action=SearchForm&from_module=".htmlspecialchars($module, ENT_QUOTES, 'UTF-8')."'>".DotbThemeRegistry::current()->getImage("EditLayout","border='0' align='bottom'",null,null,'.gif','Edit Layout')."</a>";
        }
    ...
    function displayAdvanced($header = true, $return = false, $listViewDefs='', $lv='') {
        ...
        if(!empty($listViewDefs) && !empty($lv)){
            ...
            $this->xtpl->assign('ADVANCED_SEARCH_IMG', DotbThemeRegistry::current()->getImageURL('advanced_search.gif'));
    ...
    function displaySavedViews($listViewDefs, $lv, $header = true, $return = false) {
        ...
        if($header) {
            ...
            echo '<script>DOTB.savedViews.handleForm();</script>';
        }
        else {
    ...
    function getButtons() {
        ...
        $str = "<input tabindex='2' title='{$app_strings['LBL_SEARCH_BUTTON_TITLE']}' onclick='DOTB.savedViews.setChooser()' class='button' type='submit' name='button' value='{$app_strings['LBL_SEARCH_BUTTON_LABEL']}' id='search_form_submit'/>&nbsp;";
        $str .= "<input tabindex='2' title='{$app_strings['LBL_CLEAR_BUTTON_TITLE']}' onclick='DOTB.searchForm.clear_form(this.form); return false;' class='button' type='button' name='clear' value=' {$app_strings['LBL_CLEAR_BUTTON_LABEL']} ' id='search_form_clear'/>";
        ...
        if(!empty($SAVED_SEARCHES_OPTIONS) && $this->showSavedSearchOptions){
            $str .= "   <span class='white-space'>
                        ...
                        {$SAVED_SEARCHES_OPTIONS}
                        <span id='go_btn_span' style='display:none'><input tabindex='2' title='go_select' id='go_select'  onclick='DOTB.searchForm.clear_form(this.form);' class='button' type='button' name='go_select' value=' {$app_strings['LBL_GO_BUTTON_LABEL']} '/></span>
                    </span>
                    </form>";
        }
        $str .= "
