...
class SubPanel
{
	...
	function ProcessSubPanelListView($xTemplatePath, &$mod_strings)
	{
		...
		$ListView->xTemplateAssign("DELETE_INLINE_PNG", DotbThemeRegistry::current()->getImage('delete_inline','align="absmiddle" border="0"',null,null,'.gif',$app_strings['LBL_DELETE_INLINE']));
		$ListView->xTemplateAssign("REMOVE_INLINE_PNG", DotbThemeRegistry::current()->getImage('delete_inline','align="absmiddle" border="0"',null,null,'.gif',$app_strings['LBL_ID_FF_REMOVE']));
		$header_text= '';
		...
		if(is_admin($current_user) && $_REQUEST['module'] != 'DynamicLayout' && !empty($_SESSION['editinplace']))
		{
			...
			$header_text= "&nbsp;<a href='index.php?action=index&module=DynamicLayout&from_action=$file_name&from_module=$mod_name&mod_lang="
				.$_REQUEST['module']."'>".DotbThemeRegistry::current()->getImage("EditLayout","border='0' align='bottom'",null,null,'.gif','Edit Layout')."</a>";
		}
		...
		$ListView->records_per_page = $dotb_config['list_max_entries_per_subpanel'] + 0;
	...
	{
		$modules = array();
		foreach(DotbAutoLoader::getDirFiles("modules", true) as $dir) {
  ...
  public static function getModuleSubpanels($module)
  {
    ...
    function saveSubPanelDefOverride($panel, $subsection, $override)
    {
        ...
        $fp = dotb_fopen("{$layoutPath}/{$extname}.php", 'w');
        ...
        foreach (DotbAutoLoader::existing(
                     ...
                     DotbAutoLoader::loadExtension($layoutDefsExtName, $panel->parent_bean->module_dir)
                 ) as $file
        ) {
            include $file;
        }
