<?php
    ...
    class aSubPanel
    {
        ...
        public function __construct($name, $instance_properties, $parent_bean, $reload = false, $original_only = false, $forApi = false)
        {
            ...
            {
                if (!DotbAutoLoader::existing('modules/' . $this->_instance_properties [ 'module' ])){
                ...
                $loaded = false;
                foreach(DotbAutoLoader::existing($def_path) as $file) {
                    require $file;
                    $loaded = true;
                }
        ...
        protected function set_panel_definition(array $definition)
        {
            ...
            && $this->template_instance instanceof DotbBean;
         ...
         *
         * @param  DotbBean $bean   Instance of DotbBean which is displayed
         *                           in the subpanel
         * @param  array     $fields Definition if list fields
         *
         * @return array             Expanded definition
         */
        public function expand_list_fields(DotbBean $bean, array $fields)
        {
            $expanded = array();
    ...
    class SubPanelDefinitions
    {
        ...
        function open_layout_defs($reload = false, $layout_def_key = '', $original_only = false)
        {
            ...
            if (empty ($this->layout_defs) || $reload || (!empty ($layout_def_key) && !isset ($layout_defs [$layout_def_key]))) {
                if (!$original_only) {
                    // Added by Hieu Pham to check if hybrid module then use bwc subpannel
                    $isHybridModule = in_array($this->_focus->module_dir, $GLOBALS['dotb_config']['hybrid_modules']);
                    if (isModuleBWC($this->_focus->module_dir) || $isHybridModule) {
                        $def_path = array('modules/' . $this->_focus->module_dir . '/metadata/'.($this->platform == 'mobile' ? 'wireless.' : '').'subpaneldefs.php');
                        $def_path[] = DotbAutoLoader::loadExtension($this->platform == 'mobile' ? 'wireless_subpanels' : 'layoutdefs', $this->_focus->module_dir);
                        foreach (DotbAutoLoader::existing($def_path) as $file) {
                            require $file;
                        }
                    } else {
                ...
                // convert lumia subpanels to the array the SubpanelDefinitions are looking for
                if ($this->_focus instanceof DotbBean && !isModuleBWC($this->_focus->module_dir) && isset($viewdefs)) {
