...
function subp_nav(m,i,a,t,r){
    ...
    var url, app = window.parent.DOTB.App;

    //BWC: Preseve legacy way if the "go to" module is a bwc; ignores lumia
    //modules which get handled later by subp_nav_lumia function  below
...
* */
function subp_nav_lumia(m, i, a, link) {
    ...
    if (!app.metadata.getModule(m).isBwcEnabled) {
        ...
        url = view.convertToLumiaUrl('index.php?module=' + m + '&action=' + '&record=' + i);
...
function sub_p_rem(sp,lf,li,rp){
	...
	return_url = "index.php?module="+get_module_name()+"&action=SubPanelViewer&subpanel="+sp+"&record="+get_record_id()+"&dotb_body_only=1&inline=1";
...
function sp_rem_conf(){
	return confirm(DOTB.language.get('app_strings', 'NTC_REMOVE_CONFIRMATION'))
}
...
function sp_del_conf(){
	return confirm(DOTB.language.get('app_strings', 'NTC_DELETE_CONFIRMATION'))
}
...
function set_return_and_save_background(popup_reply_data)
{
    ...
    query_array.push('csrf_token=' + DOTB.csrf.form_token);
...
function got_data(args, inline)
{
	...
	if (list_subpanel != null) {
        ...
        var bwcComponent = window.parent.DOTB.App.controller.layout.getComponent('bwc');

        if(inline){
            if (bwcComponent) {
                bwcComponent.confirmMemLeak(list_subpanel);
                $('a', list_subpanel).off('.bwc.dotbcrm');
            }
        ...
        DOTB.util.evalScript(args.responseText);
...
function showSubPanel(child_field,url,force_load,layout_def_key)
{
	...
	if (force_load || typeof( child_field_loaded[child_field] ) == 'undefined')
	{
		...
		if ( typeof (url) == 'undefined' || url == null)
		{
			...
			url = 'index.php?dotb_body_only=1&module='+module+'&subpanel='+child_field+'&action=SubPanelViewer&inline=' + inline + '&record='+id + '&layout_def_key='+ layout_def_key;
		}
...
DOTB.subpanelUtils = function() {
	...
	return {
		...
		onDrag: function(e, id) {
			originalLayout = DOTB.subpanelUtils.getLayout(true, true);
		},

		// called when subpanel is dropped
		onDrop: function(e, id) {
			newLayout = DOTB.subpanelUtils.getLayout(true, true);
		  	if(originalLayout != newLayout) { // only save if the layout has changed
				DOTB.subpanelUtils.saveLayout(newLayout);
		  	}
		},
		...
		saveLayout: function(order) {
			...
			if(typeof DOTB.subpanelUtils.currentSubpanelGroup != 'undefined') {
				var orderList = DOTB.subpanelUtils.getLayout(false, true);
				var currentGroup = DOTB.subpanelUtils.currentSubpanelGroup;
			}
			var success = function(data) {
				ajaxStatus.showStatus(DOTB.language.get('app_strings', 'LBL_SAVED_LAYOUT'));
				window.setTimeout('ajaxStatus.hideStatus()', 2000);
				if(typeof DOTB.subpanelUtils.currentSubpanelGroup != 'undefined') {
					DOTB.subpanelUtils.reorderSubpanelSubtabs(currentGroup, orderList);
				}
			}

			url = 'index.php?module=Home&action=SaveSubpanelLayout&layout=' + order + '&layoutModule=' + currentModule;
			if(typeof DOTB.subpanelUtils.currentSubpanelGroup != 'undefined') {
				url = url + '&layoutGroup=' + encodeURI(DOTB.subpanelUtils.currentSubpanelGroup);
			}
		...
		inlineSave: function(theForm, buttonName) {
			...
			var success = function(data) {
                ...
                if (typeof(result) != 'undefined' && result != null && result['status'] == 'dupe') {
                    ...
                    DOTB.subpanelUtils.cancelCreate(buttonName);
                    ...
                    ajaxStatus.showStatus(DOTB.language.get('app_strings', 'LBL_SAVED'));
		...
		sendAndRetrieve: function(theForm, theDiv, loadingStr) {
            ...
            if (quickCreateDiv)
            {
                ...
                if (form)
                {
                    ...
                    DOTB.subpanelUtils.cancelCreate(buttonName, function()
                    {
                        cancelled = true;
                    });
			...
			function success(data) {
				var theDivObj = document.getElementById(theDiv),
                    ...
                    form_el;
                DOTB.subpanelUtils.dataToDOMAvail = false;
                ...
                DOTB.subpanelUtils.removeSubPanel();

				subpanelContents[theDiv] = {};
				subpanelContents[theDiv]['list'] = theDivObj;
				subpanelContents[theDiv]['newDiv'] = document.createElement('div');
				subpanelContents[theDiv]['newDiv'].innerHTML = '<script type="text/javascript">DOTB.subpanelUtils.dataToDOMAvail=true;</script>' + data.responseText;
				...
				button_elements = YAHOO.util.Selector.query('ul.DotbActionMenu', theDiv, false);
				YAHOO.util.Dom.setStyle(button_elements, 'display', 'none');
                ...
                if (!DOTB.subpanelUtils.dataToDOMAvail) {
			...
			loadingStr = loadingStr || DOTB.language.get('app_strings', 'LBL_LOADING');
		...
		loadSubpanelGroupFromMore: function(group){
			DOTB.subpanelUtils.updateSubpanelMoreTab(group);
			DOTB.subpanelUtils.loadSubpanelGroup(group);
		},

		updateSubpanelMoreTab: function(group){
			// Update Tab
			var moreTab = document.getElementById(DOTB.subpanelUtils.subpanelMoreTab + '_sp_tab');
			moreTab.id = group + '_sp_tab';
			moreTab.getElementsByTagName('a')[0].innerHTML = group;
			moreTab.getElementsByTagName('a')[0].href = "javascript:DOTB.subpanelUtils.loadSubpanelGroup('"+group+"');";

			// Update Menu
			var menuLink = document.getElementById(group+'_sp_mm');
			menuLink.id = DOTB.subpanelUtils.subpanelMoreTab+'_sp_mm';
			menuLink.href = "javascript:DOTB.subpanelUtils.loadSubpanelGroupFromMore('"+DOTB.subpanelUtils.subpanelMoreTab+"');";
			menuLink.innerHTML = DOTB.subpanelUtils.subpanelMoreTab;

			DOTB.subpanelUtils.subpanelMoreTab = group;
		},
        ...
        removeSubPanel: function() {
            ...
            if (currentPanelEl != null) {
                ...
                // TODO review these DOTB.ajaxUI.* methods
                DOTB.ajaxUI.cleanGlobals();
		...
		loadSubpanelGroup: function(group){
			if(group == DOTB.subpanelUtils.currentSubpanelGroup) return;

			if(DOTB.subpanelUtils.loadedGroups[group]){
				DOTB.subpanelUtils.updateSubpanel(group);
			}else{

				DOTB.subpanelUtils.loadedGroups.push(group);
				var needed = [];
				for(group_sp in DOTB.subpanelUtils.subpanelGroups[group]){
					if(typeof(DOTB.subpanelUtils.subpanelGroups[group][group_sp]) == 'string' && !document.getElementById('whole_subpanel_'+DOTB.subpanelUtils.subpanelGroups[group][group_sp])){
						needed.push(DOTB.subpanelUtils.subpanelGroups[group][group_sp]);
					}
				}
				var success = function(){
					DOTB.subpanelUtils.updateSubpanelEventHandlers(needed);
					DOTB.subpanelUtils.updateSubpanels(group);
				};
				...
				if(needed.length){
					...
					DOTB.util.retrieveAndFill(DOTB.subpanelUtils.requestUrl + needed.join(','),'subpanel_list', null, success, null, true);
				}else{
					DOTB.subpanelUtils.updateSubpanels(group);
				}
			}
			DOTB.subpanelUtils.setGroupCookie(group);
		},
		...
		updateSubpanels: function(group){
			...
			for(group_sp in DOTB.subpanelUtils.subpanelGroups[group]){
                if ( typeof(DOTB.subpanelUtils.subpanelGroups[group][group_sp]) != 'string' )
                {
                    continue;
                }
				var cur = document.getElementById('whole_subpanel_'+DOTB.subpanelUtils.subpanelGroups[group][group_sp]);
			...
			DOTB.subpanelUtils.updateSubpanelTabs(group);
		},

		updateSubpanelTabs: function(group){
			if(DOTB.subpanelUtils.showLinks){
				DOTB.subpanelUtils.updateSubpanelSubtabs(group);
				document.getElementById('subpanelSubTabs').innerHTML = DOTB.subpanelUtils.subpanelSubTabs[group];
			}

			oldTab = document.getElementById(DOTB.subpanelUtils.currentSubpanelGroup+'_sp_tab');
			if(oldTab){
				oldTab.className = '';
			...
			DOTB.subpanelUtils.currentSubpanelGroup = group;
			ajaxStatus.hideStatus();
		},

		updateSubpanelEventHandlers: function(){
			if(SubpanelInitTabNames){
				SubpanelInitTabNames(DOTB.subpanelUtils.getLayout(false));
			}
		},

		reorderSubpanelSubtabs: function(group, order){
			DOTB.subpanelUtils.subpanelGroups[group] = order;
			if(DOTB.subpanelUtils.showLinks==1){
				DOTB.subpanelUtils.updateSubpanelSubtabs(group);
				if(DOTB.subpanelUtils.currentSubpanelGroup == group){
					document.getElementById('subpanelSubTabs').innerHTML = DOTB.subpanelUtils.subpanelSubTabs[group];
				}
			}
		},
		...
		updateSubpanelSubtabs: function(group){
			var notFirst = 0;
			var preMore = DOTB.subpanelUtils.subpanelGroups[group].slice(0, DOTB.subpanelUtils.subpanelMaxSubtabs);

			DOTB.subpanelUtils.subpanelSubTabs[group] = '<table border="0" cellpadding="0" cellspacing="0" height="20" width="100%" class="subTabs"><tr>';

			for(var sp_key = 0; sp_key < preMore.length; sp_key++){
				if(notFirst != 0){
					DOTB.subpanelUtils.subpanelSubTabs[group] += '<td width="1"> | </td>';
				}else{
					notFirst = 1;
				}
				DOTB.subpanelUtils.subpanelSubTabs[group] += '<td nowrap="nowrap"><a href="#'+preMore[sp_key]+'" class="subTabLink">'+DOTB.subpanelUtils.subpanelTitles[preMore[sp_key]]+'</a></td>';
			}
			if(document.getElementById('MoreSub'+group+'PanelMenu')){
				DOTB.subpanelUtils.subpanelSubTabs[group] += '<td nowrap="nowrap"> | &nbsp;<span class="subTabMore" id="MoreSub'+group+'PanelHandle" style="margin-left:2px; cursor: pointer; cursor: hand;" align="absmiddle" onmouseover="DOTB.subpanelUtils.menu.tbspButtonMouseOver(this.id,\'\',\'\',0);">&gt;&gt;</span></td>';
			}
			DOTB.subpanelUtils.subpanelSubTabs[group] += '<td width="100%">&nbsp;</td></tr></table>';

			// Update the more menu for the current group
			var postMore = DOTB.subpanelUtils.subpanelGroups[group].slice(DOTB.subpanelUtils.subpanelMaxSubtabs);
			...
			if(postMore && subpanelMenu){
				...
				for(var sp_key = 0; sp_key < postMore.length; sp_key++){
					subpanelMenu.innerHTML += '<a href="#'+postMore[sp_key]+'" class="menuItem" parentid="MoreSub'+group+'PanelMenu" onmouseover="hiliteItem(this,\'yes\'); closeSubMenus(this);" onmouseout="unhiliteItem(this);">'+DOTB.subpanelUtils.subpanelTitles[postMore[sp_key]]+'</a>';
				}
				subpanelMenu += '</div>';
			}
		},
		...
		setGroupCookie: function(group){
			Set_Cookie(DOTB.subpanelUtils.tabCookieName, group, 3000, false, false,false);
		}
	};
}();

DOTB.subpanelUtils.menu = function(){
	return {
		tbspButtonMouseOver : function(id,top,left,leftOffset){ //*//
			closeMenusDelay = eraseTimeout(closeMenusDelay);
			if (openMenusDelay == null){
				openMenusDelay = window.setTimeout("DOTB.subpanelUtils.menu.spShowMenu('"+id+"','"+top+"','"+left+"','"+leftOffset+"')", delayTime);
			}
		},
		spShowMenu : function(id,top,left,leftOffset){ //*//
			...
			DOTB.subpanelUtils.menu.spPopupMenu(id, menu, top,left,leftOffset);
		},
