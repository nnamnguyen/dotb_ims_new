...
/**
 * Dotb DateTime container
 * Extends regular PHP DateTime with useful services
 * @api
 */
class DotbDateTime extends DateTime
{
    const DOW_SUN = 0;
    const DOW_MON = 1;
    const DOW_TUE = 2;
	...
	public function __call($name, $args)
	{
		...
		$GLOBALS['log']->fatal("DotbDateTime: unknowm method $name called");
		dotb_die("DotbDateTime: unknowm method $name called");
		return false;
	}
	...
	public function get_datetime_by_index_today($hour_index)
	{
		...
		{
			dotb_die("hour is outside of range");
		}
	 ...
	 * @param DotbDateTime $start_time Start time
	 * @param DotbDateTime $end_time End time
     * @return array
     */
    ...
    protected function _strptime($time, $format)
    {
       ...
       if(empty(self::$strptime_long_mon)) {
        ...
        if ( isset($regexp['positions']['F']) && !empty($dateparts[$regexp['positions']['F']])) {
            ...
            if(isset(self::$dotb_strptime_long_mon[$mon])) {
                $data["tm_mon"] = self::$dotb_strptime_long_mon[$mon];
            } else {
                return false;
            }
        }
        if ( isset($regexp['positions']['M']) && !empty($dateparts[$regexp['positions']['M']])) {
            ...
            if(isset(self::$dotb_strptime_short_mon[$mon])) {
                $data["tm_mon"] = self::$dotb_strptime_short_mon[$mon];
            } else {
                return false;
            }
        }
    ...
    /**
     * Format DotbDateTime as date, dime or datetime string in any of "db", "iso" or "user" formats
    ...
    /**
     * Return a new DotbDateTime object holding the Date for the Next
    ...
    /**
     * Return a new DotbDateTime object holding the Date for the Previous
     * matching Day of Week.
     * Current Date is Not Included.
     *
     * @param int $dow
    ...
    /**
     * Return an array of DotbDateTime objects for all of the current month's dates
     * that match the Days of Week supplied.
     * @param  array  Days Of Week (0-6)
     * @return array(DotbDateTime)
     */
