...
{if count($data) == 0}
    ...
    {if $displayEmptyDataMesssages}
        {if strlen($query) == 0}
                ...
                {capture assign="helpLink"}<a target="_blank" href='?module=Administration&action=SupportPortal&view=documentation&version={$dotb_info.dotb_version}&edition={$dotb_info.dotb_flavor}&lang=&help_module={$currentModule}&help_action=&key='>{$APP.LBL_CLICK_HERE}</a>{/capture}
                <p class="msg">
...
{if $hideTable == false}
	                ...
	                {if $params.sortable|default:true}
						...
						{if $params.orderBy|default:$colHeader|lower == $pageData.ordering.orderBy}
							{if $pageData.ordering.sortOrder == 'ASC'}
								...
								{dotb_getimage name=$imageName attr='align="absmiddle" border="0" ' alt="$alt_sort"}
							{else}
		...
		{foreach name=rowIteration from=$data key=id item=rowData}
			...
			<tr height='20' class='{$_rowColor}S1'>
				...
				<td width='1%' class='nowrap'>
				 ...
				 {else}
	                    <input title="{dotb_translate label='LBL_SELECT_THIS_ROW_TITLE'}" onclick='sListView.check_item(this, document.MassUpdate)' type='checkbox' class='checkbox' name='mass[]' value='{$rowData.ID}'>
				 {/if}
				</td>
				{/if}
				{if $favorites}
	...
	data-record='{$rowData.ID}' data-module='{$pageData.bean.moduleDir}'
	 data-list = 'true' class="quickEdit"
	                >
	                    ...
	                    {dotb_getimage name="edit_inline.gif" attr='border="0" ' alt="$alt_edit"}</a>
	                {/if}
	            </td>
	...
	{capture assign=url}index.php?module={$linkModule}&offset={$offset}&stamp={$pageData.stamp}&return_module={$linkModule}&action={$action}&record={$record}{/capture}
	                        <{$pageData.tag.$id[$params.ACLTag]|default:$pageData.tag.$id.MAIN} href="{dotb_ajax_url url=$url}">
						{/if}
...
{literal}
    function lvg_dtails(id){{/literal}
        return DOTB.util.getAdditionalDetails( '{$pageData.bean.moduleDir|default:$params.module}',id, 'adspan_'+id);{literal}}{/literal}
{literal}
