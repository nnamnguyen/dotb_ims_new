...
class ListViewData {
     ...
     * otherwise leave it empty and it will use DotbBean::create_list_count_query
     */
    var $count_query = '';

	/**
	 * Constructor sets the limitName to look up the limit in $dotb_config
	 *
	 ...
	 */
	function getLimit() {
		return $GLOBALS['dotb_config'][$this->limitName];
	}
	...
	function getListViewData($seed, $where, $offset=-1, $limit = -1, $filter_fields=array(),$params=array(),$id_field = 'id',$singleSelect=true) {
        ...
        $this->seed = $seed;
        $totalCounted = empty($GLOBALS['dotb_config']['disable_count_query']);
        ...
        DotbVCR::store($this->seed->module_dir,  $main_query);
		if($count != 0) {
			...
			reset($rows);
            $additionalDetailsFile = DotbAutoLoader::existingCustomOne('modules/' . $seed->module_dir . '/metadata/additionalDetails.php');
			while($row = current($rows)){
                ...
                if($temp->isFavoritesEnabled()){
					$data[$dataIndex]['star'] = DotbFavorites::generateStar(!empty($row['my_favorite']), $temp->module_dir, $temp->id);
				}
        ...
        DotbVCR::recordIDs($this->seed->module_dir, array_keys($idIndex), $offset, $totalCount);
        $module_names = array(
            'Prospects' => 'Targets'
        );
	...
	function getAdditionalDetailsAjax($id)
    {
        ...
        $jscalendarImage = DotbThemeRegistry::current()->getImageURL('info_inline.gif');
    ...
    function getAdditionalDetails($fields, $adFunction, $editAccess)
    {
        ...
        if(trim($results['string']) == '')
        {
            $results['string'] = $app_strings['LBL_NONE'];
         	...
         	$close = false;
            $extra = "<img alt='{$app_strings['LBL_INFOINLINE']}' style='padding: 0px 5px 0px 2px' border='0' onclick=\"DOTB.util.getStaticAdditionalDetails(this,'";
	        ...
	        {
	            $extra .=  "<a title=\'{$app_strings['LBL_EDIT_BUTTON']}\' href={$results['editLink']}><img style=\'margin-left: 2px;\' border=\'0\' src=\'".DotbThemeRegistry::current()->getImageURL('edit_inline.png')."\'></a>";
	            $close = true;
	        }
	        $close = (!empty($results['viewLink'])) ? true : $close;
	        $extra .= (!empty($results['viewLink']) ? "<a title=\'{$app_strings['LBL_VIEW_BUTTON']}\' href={$results['viewLink']}><img style=\'margin-left: 2px;\' border=\'0\' src=".DotbThemeRegistry::current()->getImageURL('view_inline.png')."></a>" : '');

            if($close == true) {
            	$closeVal = "true";
            	$extra .=  "<a title=\'{$app_strings['LBL_ADDITIONAL_DETAILS_CLOSE_TITLE']}\' href=\'javascript: DOTB.util.closeStaticAdditionalDetails();\'><img style=\'margin-left: 2px;\' border=\'0\' src=\'".DotbThemeRegistry::current()->getImageURL('close.png')."\'></a>";
            } else {
            	$closeVal = "false";
            }
            $extra .= "',".$closeVal.")\" src='".DotbThemeRegistry::current()->getImageURL('info_inline.png')."' class='info'>";
