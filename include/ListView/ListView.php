...
function processListView($seed, $xTemplateSection, $html_varName)
{
    ...
    $populateOnly = $this->ignorePopulateOnly ? FALSE : (!empty($dotb_config['save_query']) && $dotb_config['save_query'] == 'populate_only');
    ...
    $this->shouldProcess = is_subclass_of($seed, "DotbBean")
    ...
    if(is_subclass_of($seed, "DotbBean")) {
        if(!$seed->ACLAccess('list', true)) {
         ...
         if(!$seed->ACLAccess('export', true)) {
                $dotb_config['disable_export']= true;
         }
    }
...
function process_dynamic_listview($source_module, $dotbbean,$subpanel_def)
{
        ...
        $list_data = $this->processUnionBeans($dotbbean,$subpanel_def, $html_var);
        ...
        //echo $list_data['query']; //- By Lap Nguyen
     ...
     * @param $aItem dotb bean to check against
    ...
    public function process_dynamic_listview_rows($data, $parent_data, $xTemplateSection, $html_varName, $subpanel_def)
    {
        ...
        $this->xTemplate->assign('CHECKALL', DotbThemeRegistry::current()->getImage('blank', '', 1, 1, ".gif", ''));
            ...
            foreach ($thepanel->get_list_fields() as $field_name => $list_field) {
                ...
                if ($usage != 'query_only') {
                    ...
                    if (isset($aItem->field_defs[strtolower($list_field['name'])])) {
                        // We need to see if a dotb field exists for this field type first,
                        // if it doesn't, toss it at the old dotbWidgets. This is for
                        ...
                        if (isset($vardef['type'])) {
                            ...
                            $tmpField = DotbFieldHandler::getDotbField($fieldType, true);
                        } else {
                            $tmpField = null;
                        }

                        if ($tmpField !== null) {
                            $widget_contents = DotbFieldHandler::displaySmarty($list_field['fields'], $vardef, 'ListView', $list_field);
                        } else {
                            // No DotbField for this particular type
                            // Use the old, icky, DotbWidget for now
                        ...
                        if (isset($list_field['widget_class']) && $list_field['widget_class'] == 'SubPanelDetailViewLink') {
                            // We need to call into the old DotbWidgets for the time being, so it can generate a proper link with all the various corner-cases handled
...
*/
    ...
    if(!$this->initialized) {
        global $dotb_config;
        $this->records_per_page = $dotb_config['list_max_entries_per_page'] + 0;
        $this->initialized = true;
        global $app_strings, $currentModule;
        $this->local_theme = DotbThemeRegistry::current()->__toString();
        $this->local_app_strings =$app_strings;
        $this->local_image_path = DotbThemeRegistry::current()->getImagePath();
...
*/
 function setImagePath($image_path) {
    ...
    if(empty($this->local_image_path)) {
        $this->local_image_path = DotbThemeRegistry::get($this->local_theme)->getImagePath();
    }
...
function getUserVariable($localVarName, $varName) {
    ...
    * @desc INTERNAL FUNCTION Handles List Views using seeds that extend DotbBean
        ...
        $seed is a seed that extends DotbBean
        * Portions created by DotBCRM are Copyright (C) DotBCRM, Inc..
        * All Rights Reserved..
        * Contributor(s): ______________________________________..
    */
    function processDotbBean($xtemplateSection, $html_varName, $seed) {
        ...
        //echo $main_query; // By Lap Nguyen

        DotbVCR::store($seed->module_dir,  $main_query);
        //ADDING VCR CONTROL
    ...
    function processUnionBeans($dotbbean, $subpanel_def, $html_var = 'CELL') {

		$last_detailview_record = $this->getSessionVariable("detailview", "record");
		if(!empty($last_detailview_record) && $last_detailview_record != $dotbbean->id){
			$GLOBALS['record_has_changed'] = true;
		}
		$this->setSessionVariable("detailview", "record", $dotbbean->id);
        ...
        if(!empty($this->response)){
            ...
            $response = DotbBean::get_union_related_list($dotbbean,$this->sortby, $this->sort_order, $this->query_where, $current_offset, -1,-1,$this->query_limit,$subpanel_def);
        ...
        global $list_view_row_count;
        $list_view_row_count = $row_count;
        $this->processListNavigation('dyn_list_view', $html_var, $current_offset, $next_offset, $previous_offset, $row_count, $dotbbean,$subpanel_def);
    ...
    * Contributor(s): ______________________________________..
    */
    function processListNavigation($xtemplateSection, $html_varName, $current_offset, $next_offset, $previous_offset, $row_count, $dotbbean=null, $subpanel_def=null, $col_count = 20) {
        ...
        if(empty($this->query_limit)  || $this->query_limit > $this->records_per_page) {
            ...
            if(0 == $current_offset) {
                $start_link = "<button type='button' name='listViewStartButton' title='{$this->local_app_strings['LNK_LIST_START']}' class='button' disabled>".DotbThemeRegistry::current()->getImage("start_off","aborder='0' align='absmiddle'",null,null,'.gif',$this->local_app_strings['LNK_LIST_START'])."</button>";
                $previous_link = "<button type='button' name='listViewPrevButton' title='{$this->local_app_strings['LNK_LIST_PREVIOUS']}' class='button' disabled>".DotbThemeRegistry::current()->getImage("previous_off","border='0' align='absmiddle'",null,null,'.gif',$this->local_app_strings['LNK_LIST_PREVIOUS'])."</button>";
            } else {
                if($this->multi_select_popup) {// nav links for multiselect popup, submit form to save checks.
                    $start_link = "<button type='button' class='button' name='listViewStartButton' title='{$this->local_app_strings['LNK_LIST_START']}' onClick='javascript:save_checks(0, \"{$moduleString}\");'>".DotbThemeRegistry::current()->getImage("start","border='0' align='absmiddle'",null,null,'.gif',$this->local_app_strings['LNK_LIST_START'])."</button>";
                    $previous_link = "<button type='button' class='button' name='listViewPrevButton' title='{$this->local_app_strings['LNK_LIST_PREVIOUS']}' onClick='javascript:save_checks($previous_offset, \"{$moduleString}\");'>".DotbThemeRegistry::current()->getImage("previous","border='0' align='absmiddle'",null,null,'.gif',$this->local_app_strings['LNK_LIST_PREVIOUS'])."</button>";
                } elseif($this->shouldProcess) {
                    $start_link = "<button type='button' class='button' name='listViewStartButton' title='{$this->local_app_strings['LNK_LIST_START']}' onClick='location.href=\"$start_URL\"; sListView.save_checks(0, \"{$moduleString}\");'>".DotbThemeRegistry::current()->getImage("start","border='0' align='absmiddle'",null,null,'.gif',$this->local_app_strings['LNK_LIST_START'])."</button>";
                    $previous_link = "<button type='button' class='button' name='listViewPrevButton' title='{$this->local_app_strings['LNK_LIST_PREVIOUS']}' onClick='location.href=\"$previous_URL\"; sListView.save_checks($previous_offset, \"{$moduleString}\");'>".DotbThemeRegistry::current()->getImage("previous","border='0' align='absmiddle'",null,null,'.gif',$this->local_app_strings['LNK_LIST_PREVIOUS'])."</button>";
                } else {
                    $onClick = '';
                    ...
                    $start_link = "<button type='button' class='button' name='listViewStartButton' title='{$this->local_app_strings['LNK_LIST_START']}' onClick=".$onClick.">".DotbThemeRegistry::current()->getImage("start","border='0' align='absmiddle'",null,null,'.gif',$this->local_app_strings['LNK_LIST_START'])."</button>";
                    ...
                    $previous_link = "<button type='button' class='button' name='listViewPrevButton' title='{$this->local_app_strings['LNK_LIST_PREVIOUS']}' onClick=".$onClick.">".DotbThemeRegistry::current()->getImage("previous","border='0' align='absmiddle'",null,null,'.gif',$this->local_app_strings['LNK_LIST_PREVIOUS'])."</button>";
                }
            }

            if($last_offset <= $current_offset) {
                $end_link = "<button type='button' name='listViewEndButton' title='{$this->local_app_strings['LNK_LIST_END']}' class='button' disabled>".DotbThemeRegistry::current()->getImage("end_off","border='0' align='absmiddle'",null,null,'.gif',$this->local_app_strings['LNK_LIST_END'])."</button>";
                $next_link = "<button type='button' name='listViewNextButton' title='{$this->local_app_strings['LNK_LIST_NEXT']}' class='button' disabled>".DotbThemeRegistry::current()->getImage("next_off","aborder='0' align='absmiddle'",null,null,'.gif',$this->local_app_strings['LNK_LIST_NEXT'])."</button>";
            } else {
                if($this->multi_select_popup) { // nav links for multiselect popup, submit form to save checks.
                    $end_link = "<button type='button' name='listViewEndButton' class='button' title='{$this->local_app_strings['LNK_LIST_END']}' onClick='javascript:save_checks($last_offset, \"{$moduleString}\");'>".DotbThemeRegistry::current()->getImage("end","border='0' align='absmiddle'",null,null,'.gif',$this->local_app_strings['LNK_LIST_END'])."</button>";
                    if(!empty($dotb_config['disable_count_query'])) {
                        $end_link = '';
                    }
                    $next_link = "<button type='button' name='listViewNextButton' title='{$this->local_app_strings['LNK_LIST_NEXT']}' class='button' onClick='javascript:save_checks($next_offset, \"{$moduleString}\");'>".DotbThemeRegistry::current()->getImage("next","border='0' align='absmiddle'",null,null,'.gif',$this->local_app_strings['LNK_LIST_NEXT'])."</button>";
                } elseif($this->shouldProcess) {
                    $end_link = "<button type='button' name='listViewEndButton' class='button' title='{$this->local_app_strings['LNK_LIST_END']}' onClick='location.href=\"$end_URL\"; sListView.save_checks(\"end\", \"{$moduleString}\");'>".DotbThemeRegistry::current()->getImage("end","border='0' align='absmiddle'",null,null,'.gif',$this->local_app_strings['LNK_LIST_END'])."</button>";
                    $next_link = "<button type='button' name='listViewNextButton' class='button' title='{$this->local_app_strings['LNK_LIST_NEXT']}' onClick='location.href=\"$next_URL\"; sListView.save_checks($next_offset, \"{$moduleString}\");'>".DotbThemeRegistry::current()->getImage("next","border='0' align='absmiddle'",null,null,'.gif',$this->local_app_strings['LNK_LIST_NEXT'])."</button>";
                } else {
                    $onClick = '';
                    ...
                    $next_link = "<button type='button' name='listViewNextButton' class='button' title='{$this->local_app_strings['LNK_LIST_NEXT']}' onClick=".$onClick.">".DotbThemeRegistry::current()->getImage("next","border='0' align='absmiddle'",null,null,'.gif',$this->local_app_strings['LNK_LIST_NEXT'])."</button>";
                    ...
                    $end_link = "<button type='button' name='listViewEndButton' class='button' title='{$this->local_app_strings['LNK_LIST_END']}' onClick=".$onClick.">".DotbThemeRegistry::current()->getImage("end","border='0' align='absmiddle'",null,null,'.gif',$this->local_app_strings['LNK_LIST_END'])."</button>";
            ...
            if($this->show_select_menu)
            {
                ...
                if (!empty($GLOBALS['dotb_config']['disable_count_query']) && $GLOBALS['dotb_config']['disable_count_query'] === true && $total > $pageTotal) {
                    $this->show_plus = true;
                    $total =  $pageTotal;
                    $total_label = $total.'+';
                } else {
                ...
                //$select_link = "<a id='select_link' onclick='return select_dialog();' href=\"javascript:void(0)\">".$this->local_app_strings['LBL_LINK_SELECT']."&nbsp;".DotbThemeRegistry::current()->getImage('MoreDetail', 'border=0', 11, 7, '.png', $app_strings['LBL_MOREDETAIL'])."</a>";
                $menuItems = array(
                ...
                require_once('include/DotbSmarty/plugins/function.dotb_action_menu.php');
                $select_link = smarty_function_dotb_action_menu(array(
            ...
            if($_REQUEST['module'] == 'Home' || $this->local_current_module == 'Import'
                ...
                || !empty($dotb_config['disable_export'])
                || (!empty($dotb_config['admin_export_only']) && !$current_user->isAdminForModule($_REQUEST['module']))
            )
            {
            ...
            if($this->show_paging == true) {
                ...
                if ($subpanel_def != null) {
                    ...
                    $subpanelTiles = new SubPanelTiles($dotbbean);
                    $html_text .= "<table border=\"0\" cellpadding=\"0\" cellspacing=\"0\" width=\"100%\"><tr><td align=\"left\"  >";
    ...
    */
    function processListRows($data, $xtemplateSection, $html_varName)
    {
        ...
        global $even_bg;
        global $hilite_bg;
        global $app_strings, $dotb_version, $dotb_config;
        global $currentModule;
    ...
    // Bug #2786
                $additionalDetailsFile = DotbAutoLoader::existingCustomOne('modules/' . $aItem->module_dir . '/metadata/additionalDetails.php');
                ...
                if($aItem->ACLAccess('Delete')) {
                    ...
                    $delete .= smarty_function_dotb_csrf_form_token(array(), $this);
                    ...
                    $delete .= "</form>\n";
                    require_once('include/DotbSmarty/plugins/function.dotb_action_menu.php');
                    $fields['DELETE_BUTTON'] = smarty_function_dotb_action_menu(array(
    ...
    * @param unknown $xTemplateSection
    * @param unknown $html_varName
    * @desc PUBLIC FUNCTION Handles List Views using seeds that extend DotbBean
        $XTemplateSection is the section in the XTemplate file that should be parsed usually main
        $html_VarName is the variable name used in the XTemplateFile e.g. TASK
        $seed is a seed there are two types of seeds one is a subclass of DotbBean, the other is a list usually created from a dotb bean using get_list
    ...
    */
    ...
    function processListViewTwo($seed, $xTemplateSection, $html_varName) {
        ...
        $isDotbBean = is_subclass_of($seed, "DotbBean");
        $list = null;

        if($isDotbBean) {
            $list = $this->processDotbBean($xTemplateSection, $html_varName, $seed);
        } else {
            $list = $seed;
        }
        ...
        if($this->is_dynamic) {
            ...
            $this->xTemplateAssign('CHECKALL', DotbThemeRegistry::current()->getImage('blank', '', 1, 1, ".gif", ''));
    ...
    function getArrowStart() {
        $imgFileParts = pathinfo(DotbThemeRegistry::current()->getImageURL("arrow.gif"));
    ...
    {
        $ext = ( DotbThemeRegistry::current()->pngSupport ? "png" : "gif" );

        if (!isset($upDown) || empty($upDown)) {
            $upDown = "";
        }
        return "&nbsp;<img border='0' src='".DotbThemeRegistry::current()->getImageURL("arrow{$upDown}.{$ext}")."' ";
    }
    ...
    public static function getArrowUpDownEnd($upDown)
    {
        ...
        $imgFileParts = pathinfo(DotbThemeRegistry::current()->getImageURL("arrow{$upDown}.gif"));
	...
	function getArrowImageSize() {
		...
		$image = DotbThemeRegistry::current()->getImageURL("arrow.gif",false);
        ...
        dotb_cache_put($cache_key, $result);
        return $result;
    }
    ...
    public static function getArrowUpDownImageSize($upDown)
    {
        ...
        $image = DotbThemeRegistry::current()->getImageURL("arrow{$upDown}.gif",false);
 ...
 function setAdditionalDetails($value = true, $function = '') {
    ...
    private function formatRowDates(DotbBean $bean, array $row) : array
    {
