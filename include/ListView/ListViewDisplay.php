...
class ListViewDisplay {
	...
	function shouldProcess($moduleDir){
		...
		if(!empty($GLOBALS['dotb_config']['save_query']) && $GLOBALS['dotb_config']['save_query'] == 'populate_only'){
	...
	/**
	 * Setup the class
	 * @param seed DotbBean Seed DotbBean to use
	...
	function buildSelectLink($id = 'select_link', $total=0, $pageTotal=0, $location="top") {
		...
		$close_inline_img = DotbThemeRegistry::current()->getImage('close_inline', 'border=0', null, null, ".gif", $app_strings['LBL_CLOSEINLINE']);
		$menuItems = array(
	...
	{
	    global $app_strings;
		$closeText = DotbThemeRegistry::current()->getImage('close_inline', 'border=0', null, null, ".gif", $app_strings['LBL_CLOSEINLINE']);
		$moreDetailImage = DotbThemeRegistry::current()->getImageURL('MoreDetail.png');
		$menuItems = array();
	...
	protected function buildExportLink($loc = 'top')
	{
        ...
        return "<a href='javascript:void(0)' id=\"export_listview_". $loc ."\" onclick=\"return sListView.send_form("
            ...
            . "DOTB.language.get('app_strings', 'LBL_LISTVIEW_NO_SELECTED')"
	...
	protected function buildMergeDuplicatesLink($loc = 'top')
	{
        ...
        if (isset($dictionary[$this->seed->object_name]['duplicate_merge']) && $dictionary[$this->seed->object_name]['duplicate_merge']==true ) {
            ...
            $onclick = 'if (dotbListView.get_checks_count() > 1) {'
...
EOF;
     ...
     * We need vardefs to be in displayColumns for a further processing (e.g. in DotbField)
