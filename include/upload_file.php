...
class UploadFile
{
    ...
    public function confirm_upload()
    {
        ...
        if ($_FILES[$this->field_name]['error'] != UPLOAD_ERR_OK) {
            if ($_FILES[$this->field_name]['error'] != UPLOAD_ERR_NO_FILE) {
                if ($_FILES[$this->field_name]['error'] == UPLOAD_ERR_INI_SIZE) {
                    ...
                    $errMess = string_format($GLOBALS['app_strings']['UPLOAD_ERROR_TEXT_SIZEINFO'], array($_FILES[$this->field_name]['error'], self::$filesError[$_FILES[$this->field_name]['error']], $dotb_config['upload_maxsize']));
        ...
        // Added Dotb API Override flag to FILES to allow PUT API hits to work
        if (is_uploaded_file($_FILES[$this->field_name]['tmp_name']) || (isset($_FILES[$this->field_name]['_DOTB_API_UPLOAD']) && $_FILES[$this->field_name]['_DOTB_API_UPLOAD'] === true)) {
            if ($_FILES[$this->field_name]['size'] > $dotb_config['upload_maxsize']) {
                $this->setError('fatal', "ERROR: uploaded file was too big: max filesize: {$dotb_config['upload_maxsize']}");
                return false;
            }
        } else {
            return false;
    ...
    function final_move($bean_id, $temporary = false)
    {
        ...
        if ($temporary === true) {
            ...
            if (!is_dir($tempFolder)) {
                dotb_mkdir($tempFolder, 0755, true);
            }
        ...
        if ($this->use_soap) {
            ...
            if (strstr($destination, 'IMPORT_') && ($this->file_ext == 'xlsx' || $this->file_ext == 'xls')) {
                // add by TKT to import from excel file
                require_once("custom/include/PHPExcel/vendor/autoload.php");
                $inputFileType = PHPExcel_IOFactory::identify($_FILES[$this->field_name]['tmp_name']);
                $objReader = PHPExcel_IOFactory::createReader($inputFileType);
                $objPHPExcel = $objReader->load($_FILES[$this->field_name]['tmp_name']);
                $objPHPExcel->setActiveSheetIndex(0);
                $col = PHPExcel_Cell::columnIndexFromString($objPHPExcel->getActiveSheet()->getHighestColumn());
                if($col==2) $objPHPExcel->getActiveSheet()->setCellValue('B1','...');
                ...
                if (!UploadStream::move_uploaded_file($_FILES[$this->field_name]['tmp_name'], $destination)) {
                    if (isset($_FILES[$this->field_name]['_DOTB_API_UPLOAD']) && $_FILES[$this->field_name]['_DOTB_API_UPLOAD'] === true) {
     ...
     * @param DotbBean $bean Related bean
     * @param string $bean_id
    ...
    {
        if (!empty($doc_type) && $doc_type != 'Dotb') {
            global $dotb_config;
            $destination = $this->get_upload_path($bean_id);
            dotb_rename($destination, str_replace($bean_id, $bean_id . '_' . $file_name, $destination));
            ...
            if (!$result['success']) {
                dotb_rename($new_destination, str_replace($bean_id . '_' . $file_name, $bean_id, $new_destination));
                $bean->doc_type = 'Dotb';
                // FIXME: Translate
...
class UploadStream
{
    ...
    public static function getDir()
    {
        if (empty(self::$upload_dir)) {
            self::$upload_dir = rtrim($GLOBALS['dotb_config']['upload_dir'], '/\\');
            if (empty(self::$upload_dir)) {
                self::$upload_dir = "upload";
            }
            if (!file_exists(self::$upload_dir)) {
                dotb_mkdir(self::$upload_dir, 0755, true);
            }
        }
    ...
    {
        if (isset($GLOBALS['dotb_config']['upload_wrapper_class'])) {
            DotbAutoLoader::requireWithCustom("include/{$GLOBALS['dotb_config']['upload_wrapper_class']}.php");
            if (class_exists($GLOBALS['dotb_config']['upload_wrapper_class'])) {
                self::$wrapper_class = $GLOBALS['dotb_config']['upload_wrapper_class'];
            } else {
    ...
    {
        return dotb_mkdir($this->getFSPath($path), 0755, true);
    }
