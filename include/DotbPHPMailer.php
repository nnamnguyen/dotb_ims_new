 ...
 */
class DotbPHPMailer extends PHPMailerProxy
{
    ...
    public function __construct()
    {
        ...
        $this->Hostname = $dotb_config['host_name'];
        $this->WordWrap		= 996;
        // cn: gmail fix
...
eoq;
    ...
    public function replaceImageByRegex($regex, $local_prefix, $object = false)
    {
        ...
        foreach($matches[2] as $match) {
            ...
            if($object) {
                ...
                if(isset($beanname)) {
                    $bean = DotbModule::get($beanname)->loadBean();
    ...
    public function handleAttachments($notes) {
		...
		// cn: bug 4864 - reusing same DotbPHPMailer class, need to clear attachments
        ...
        $this->Body = preg_replace(';=\s*"'.preg_quote(dotb_cached('images/'), ';').';','="cid:',$this->Body);

        $this->replaceImageByRegex("(?:{$dotb_config['site_url']})?/?cache/images/", dotb_cached("images/"));

        //Replace any embeded images using the secure entryPoint for src url.
        $this->replaceImageByRegex("(?:{$dotb_config['site_url']})?index.php[?]entryPoint=download&(?:amp;)?[^\"]+?id=", "upload://", true);
