...
>
    ...
    {{foreach name=section from=$sectionPanels key=label item=panel}}
        ...
        {{if (isset($tabDefs[$label_upper].newTab) && $tabDefs[$label_upper].newTab == true)}}
            ...
            <li><a id="tab{{$tabCount}}" href="javascript:void(0)"><em>{dotb_translate label='{{$label}}' module='{{$module}}'}</em></a></li>
        {{/if}}
    {{/foreach}}
    </ul>
    {{/if}}
		...
		{{/if}}
			...
			<td width='{{$def.templateMeta.widths[$smarty.foreach.colIteration.index].label}}%' scope="col">
                ...
                {{if isset($colData.field.popupHelp) || isset($fields[$colData.field.name]) && isset($fields[$colData.field.name].popupHelp) }}
                   ...
                   {{/if}}
                   {dotb_help text=$popupText WIDTH=400}
                {{/if}}
                {{if !empty($colData.field.name)}}
                {else}
			...
			<td width='{{$def.templateMeta.widths[$smarty.foreach.colIteration.index].field}}%' {{if $colData.colspan}}colspan='{{$colData.colspan}}'{{/if}} {{if isset($fields[$colData.field.name].type) && $fields[$colData.field.name].type == 'phone'}}class="phone"{{/if}}>
				...
				{{if ($colData.field.customCode && !$colData.field.customCodeRenderField) || $colData.field.assign}}
					...
					<span id="{{$colData.field.name}}" class="dotb_field">{{dotb_evalcolumn var=$colData.field colData=$colData}}</span>
				{{elseif $fields[$colData.field.name] && !empty($colData.field.fields) }}
				    {{foreach from=$colData.field.fields item=subField}}
				        {{if $fields[$subField]}}
				        	{counter name="panelFieldCount"}
				            {{dotb_field parentFieldArray='fields' tabindex=$tabIndex vardef=$fields[$subField] displayType='DetailView'}}&nbsp;
				        {{else}}
				        	{counter name="panelFieldCount"}
				            {{$subField}}
				        {{/if}}
				...
				{{elseif $fields[$colData.field.name]}}
					{counter name="panelFieldCount"}
					{{dotb_field parentFieldArray='fields' vardef=$fields[$colData.field.name] displayType='DetailView' displayParams=$colData.field.displayParams typeOverride=$colData.field.type}}
				{{/if}}
...
{{/if}}
<script type="text/javascript">
DOTB.util.doWhen("typeof collapsePanel == 'function'",
        function(){ldelim}
            var dotb_panel_collase = Get_Cookie("dotb_panel_collase");
            if(dotb_panel_collase != null) {ldelim}
                dotb_panel_collase = YAHOO.lang.JSON.parse(dotb_panel_collase);
                for(panel in dotb_panel_collase['{{$module}}_d'])
                    if(dotb_panel_collase['{{$module}}_d'][panel])
                        collapsePanel(panel);
                    else
                        expandPanel(panel);
            {rdelim}
