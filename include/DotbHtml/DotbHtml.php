...
/**
 * DotbHtml is a static class that provides a collection of helper methods for creating HTML DOM elements.
 *
 * @auther Justin Park(jpark@dotbcrm.com)
 *
 */
class DotbHtml {
    const SINGLE_QUOTE = "'";
    ...
    public static function parseDotbHtml($dotb_html = array()) {
        $output = array();
        $input_types = array(
            'submit', 'button', 'hidden', 'checkbox', 'input'
        );

        if(in_array($dotb_html['type'], $input_types)) {
            $dotb_html['htmlOptions']['type'] = (empty($dotb_html['htmlOptions']['type'])) ? $dotb_html['type'] : $dotb_html['htmlOptions']['type'];
            $dotb_html['htmlOptions']['value'] = $dotb_html['value'];

            $output = array_merge(array(
                'tag' => 'input',
                'self_closing' => true,
            ), $dotb_html['htmlOptions']);
        }
        if(isset($dotb_html['template'])) {
            $output = array(
                'smarty' => array(
                    array(
                        'template' => $dotb_html['template'],
                        '[CONTENT]' => $output
                    )
                )
            );
