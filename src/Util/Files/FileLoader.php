...
namespace Dotbcrm\Dotbcrm\Util\Files;

use Dotbcrm\Dotbcrm\Security\Validator\Validator;
use Dotbcrm\Dotbcrm\Security\Validator\Constraints\File;
 ...
 * with auto loading concerns which belong the \DotbAutoLoader.
 *
 */
class FileLoader
{
    protected static $dotbBaseDirs = array();
     ...
     *
     * @see \Dotbcrm\Dotbcrm\Security\Validator\Constraints\FileValidator
     *
    ...
    /**
     * Get base directories. This returns the dotb base directory and
     * optionally the shadow instance directory when shadow is in use.
     * @return array
     */
    public static function getBaseDirs()
    {
        if (empty(self::$dotbBaseDirs)) {
            self::$dotbBaseDirs = array(DOTB_BASE_DIR);
            if (defined('SHADOW_INSTANCE_DIR')) {
                self::$dotbBaseDirs[] = SHADOW_INSTANCE_DIR;
            }

            foreach (self::$dotbBaseDirs as $key => $dir) {
                self::$dotbBaseDirs[$key] = self::getRealPath($dir);
            }
        }
        return self::$dotbBaseDirs;
    }
