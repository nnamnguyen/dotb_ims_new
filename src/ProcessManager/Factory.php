...
class Factory
{
    ...
    public static function getPMSEClass($name, $prefix = '')
    {
        ...
        foreach ($paths as $path) {
            $custom = 'Custom' . $name;
            if (\DotbAutoLoader::requireWithCustom("custom/$path{$custom}.php") !== false) {
        ...
        if (empty($class)) {
            foreach ($paths as $path) {
                if (\DotbAutoLoader::requireWithCustom("$path{$name}.php") !== false) {
    ...
    public static function getFieldEvaluator(array $def, $new = false)
    {
        ...
        if (!isset(static::$cache['evaluators'][$type]) || $new === true) {
            ...
            $nsRoot = 'Dotbcrm\\Dotbcrm\\ProcessManager\\Field\\Evaluator\\';

            // Get the class name for this field type, getting the custom class if
            // found
            $class = \DotbAutoLoader::customClass($nsRoot . $type);

            // Set the base class name, getting the custom class if found
            $base = \DotbAutoLoader::customClass($nsRoot . 'Base');
    ...
    public static function getElement($name = '')
    {
        ...
        if (\DotbAutoLoader::requireWithCustom("custom/$path{$custom}.php") !== false) {
            $return = new $custom;
        } elseif (\DotbAutoLoader::requireWithCustom("$path{$name}.php") !== false) {
    ...
    public static function getException($type = '', $message = '', $code = 0, $previousException = null)
    {
        ...
        $nsRoot = 'Dotbcrm\\Dotbcrm\\ProcessManager\\Exception\\';

        // Get the full class name for this exception, getting the custom class if found
        $exceptionClass = \DotbAutoLoader::customClass($nsRoot . $exceptionClassName);

        // Get the full default class name, getting the custom class if found
        $defaultClass = \DotbAutoLoader::customClass($nsRoot . $defaultClassName);
