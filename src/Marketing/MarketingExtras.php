...
namespace Dotbcrm\Dotbcrm\Marketing;
...
class MarketingExtras
{
    ...
    {
        $dotbDetails = $this->getDotbDetails();
        $queryParams = array(
            'language' => $this->chooseLanguage($language),
            'version' => $dotbDetails['version'],
            'flavor' => strtolower($dotbDetails['flavor']),
            'build' => $dotbDetails['build'],
        );

        $marketingExtrasSandboxTest = $this->getDotbConfig('marketing_extras_sandbox_test');
    ...
    {
        return $this->getDotbConfig('marketing_extras_enabled', false);
    }
    ...
    {
        $marketingExtrasUrl = $this->getDotbConfig('marketing_extras_url');
     ...
     *   language, then falls back to the default language of this Dotb
    ...
    public function chooseLanguage(?string $language): string
    {
        ...
        $defaultLanguage = $this->getDotbConfig('default_language');
    ...
    /**
     * Get the dotb_config global variable.
     * @param string $key Key to get.
     * @param * $default Default value.
     * @return * The value of the config flag, or the default.
     */
    private function getDotbConfig(string $key, $default = null)
    {
    ...
    /**
     * Retrieve the build number, flavor, and version of this Dotb instance.
     * @return array An array consisting of build number, flavor, and version
     *   details.
     */
    private function getDotbDetails(): array
    {
        global $dotb_build, $dotb_flavor, $dotb_version;
