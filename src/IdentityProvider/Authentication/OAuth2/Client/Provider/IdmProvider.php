...
namespace Dotbcrm\Dotbcrm\IdentityProvider\Authentication\OAuth2\Client\Provider;
...
use Dotbcrm\Dotbcrm\League\OAuth2\Client\Grant\JwtBearer;
...
class IdmProvider extends BasicGenericProvider
{
    ...
    /**
     * Get DotbCache instance.
     *
     * @return \DotbCacheAbstract
     */
    protected function getDotbCache()
    {
        return \DotbCache::instance();
    }
    ...
    protected function setCache($key, $value, $ttlName)
    {
        ...
        if ($ttl > 0) {
            $this->getDotbCache()->set($key, $value, $ttl);
        }
    }
    ...
    {
        return $this->getDotbCache()->get($key);
    }
