...
use Dotbcrm\Dotbcrm\Elasticsearch\Exception\IndexManagerException;
...
class IndexManager
{
     ...
     * `$dotb_config['full_text_engine']['Elastic']['reindex_refresh_interval']`
     *
     * @var int|string
     */
     ...
     *
     * Use `$dotb_config['full_text_engine']['Elastic']['reindex_zero_replica'] = true`
     *
     ...
     * 1. New field and new dotbbean field
     *      - Can be added dynamically
     *      - No further action as need as no data exists yet for this field
     * 2. New field for existing dotbbean field
     *      - Can be added dynamically
     ...
     * might come in hand but is not absolutely required as dotb should
    ...
    /**
     * Get index settings from $dotb_config
     * @param Index $index
     * @return array
     */
    protected function getIndexSettingsFromConfig(Index $index)
    {
        ...
        // We do NOT accept analysis settings anymore in `$dotb_config`
     ...
     *
     * This implies when the index.refresh_interval is set using $dotb_config,
     ...
     * that dotb controls the replica settings for its indices and no other
