...
use Dotbcrm\Dotbcrm\SearchEngine\MetaDataHelper;
use Dotbcrm\Dotbcrm\Elasticsearch\Adapter\Client;
use Dotbcrm\Dotbcrm\Elasticsearch\Index\IndexPool;
use Dotbcrm\Dotbcrm\Elasticsearch\Index\IndexManager;
use Dotbcrm\Dotbcrm\Elasticsearch\Mapping\MappingManager;
use Dotbcrm\Dotbcrm\Elasticsearch\Indexer\Indexer;
use Dotbcrm\Dotbcrm\Elasticsearch\Queue\QueueManager;
use Dotbcrm\Dotbcrm\Elasticsearch\Exception\ContainerException;
use DotbAutoLoader;
use DotbConfig;
use DotbArray;
...
class Container
{
    ...
    public static function create()
    {
         ...
         */
        $class = DotbAutoLoader::customClass('Dotbcrm\\Dotbcrm\\Elasticsearch\\Container');
        return new $class();
    }
     ...
     *
     *      \Dotbcrm\Dotbcrm\custom\src\Elasticsearch\Container
     *
    ...
    protected function initLogger()
    {
        ...
        $xhprof = DotbConfig::getInstance()->get('xhprof_config');
        if (!empty($xhprof)) {
            $this->logger->setXhProf(\DotbXHprof::getInstance());
        }
    }
    ...
    {
        $prefix = DotbConfig::getInstance()->get('unique_key', 'dotbcrm');
        $config = DotbArray::staticGet($this->getConfig('engine'), 'index_strategy', array());
    ...
    {
        $config = DotbArray::staticGet($this->getConfig('engine'), 'index_settings', array());
        $this->indexManager = new IndexManager($config, $this);

        // reindex refresh interval from config
        $interval = DotbArray::staticGet($this->getConfig('engine'), 'reindex_refresh_interval');
        if (null !== $interval) {
            $this->indexManager->setReindexRefreshInterval($interval);
        }

        // reindex zero replica usage from config
        $zeroReplica = DotbArray::staticGet($this->getConfig('engine'), 'reindex_zero_replica');
        if ($zeroReplica) {
    ...
    public function registerProviders()
    {
        $this->registerProvider(
            'GlobalSearch',
            'Dotbcrm\Dotbcrm\Elasticsearch\Provider\GlobalSearch\GlobalSearch'
        );

        $this->registerProvider(
            'Visibility',
            'Dotbcrm\Dotbcrm\Elasticsearch\Provider\Visibility\Visibility'
        );
    }
