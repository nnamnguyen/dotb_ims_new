...
$server->register(
        ...
        array('return'=>'tns:set_entry_result'),
        $NAMESPACE);
function portal_get_dotb_id($session){
    $error = new SoapError();
    if(portal_validate_authenticated($session)){
...
$server->register(
        'portal_get_dotb_contact_id',
        array('session'=>'xsd:string'),
        array('return'=>'tns:set_entry_result'),
        $NAMESPACE);
function portal_get_dotb_contact_id($session){
    $error = new SoapError();
    if(portal_validate_authenticated($session)){
        ...
        if(isset($filter) && is_array($filter)){
            ...
            foreach($filter as $nvOp){
                ...
                if(isset($name) && (isset($value) || isset($value_array)) && isset($operator)){
                    ...
                    if(isset($dotb->field_defs[$name])){
                        // MFH - Added Support For Custom Fields in Searches
                        $cstm = isset($dotb->field_defs[$name]['source']) && $dotb->field_defs[$name]['source'] == 'custom_fields' ? '_cstm' : '';

                        $where .=  "$dotb->table_name$cstm.$name $operator ";
                        if($dotb->field_defs['name']['type'] == 'datetime'){
