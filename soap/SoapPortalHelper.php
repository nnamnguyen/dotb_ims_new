...
use Dotbcrm\Dotbcrm\Security\Validator\Constraints\Sql\OrderBy;
use Dotbcrm\Dotbcrm\Security\Validator\Constraints\Sql\OrderDirection;
...
function get_bugs_in_contacts($in, $orderBy = '')
    ...
    validateOrderBy($orderBy);
        ...
        $dotb = BeanFactory::newBean('Contacts');
        $dotb->disable_row_level_security = true;
        set_module_in($dotb->build_related_in($query), 'Bugs');
    }
...
function get_bugs_in_accounts($in, $orderBy = '')
    ...
    validateOrderBy($orderBy);
        ...
        $dotb = BeanFactory::newBean('Accounts');
        $dotb->disable_row_level_security = true;
...
function get_cases_in_contacts($in, $orderBy = '')
    ...
    validateOrderBy($orderBy);
        ...
        $dotb = BeanFactory::newBean('Contacts');
        $dotb->disable_row_level_security = true;
        set_module_in($dotb->build_related_in($query), 'Cases');
    }
...
function get_notes_in_module($in, $module, $orderBy = '')
    ...
    validateOrderBy($orderBy);
        ...
        $dotb = BeanFactory::newBean($module);
        if(empty($dotb)) {
            return array();
        }
        ...
        $note = BeanFactory::newBean('Notes');
        $note->disable_row_level_security = true;
        return $dotb->build_related_list($query, $note);
    }
    ...
    validateOrderBy($orderBy);
        ...
        $count_query = $dotb->create_list_count_query($query);
        if(!empty($count_query))
        {
            // We have a count query.  Run it and get the results.
            $result = $dotb->db->query($count_query, true, "Error running count query for $dotb->object_name List: ");
            $assoc = $dotb->db->fetchByAssoc($result);
            if(!empty($assoc['c']))
            {
                $rows_found = $assoc['c'];
            }
        }
        $list = $dotb->build_related_list($query, $rel, $row_offset, $limit);
...
function get_accounts_from_contact($contact_id, $orderBy = '')
    ...
    validateOrderBy($orderBy);
        ...
        $dotb = BeanFactory::newBean('Contacts');
        $dotb->disable_row_level_security = true;
        set_module_in($dotb->build_related_in($query), 'Accounts');
    }
...
function get_contacts_from_account($account_id, $orderBy = '')
    ...
    validateOrderBy($orderBy);
        ...
        $dotb = BeanFactory::newBean('Accounts');
        $dotb->disable_row_level_security = true;
        set_module_in($dotb->build_related_in($query), 'Contacts');
    }
...
function build_relationship_tree($contact){
    ...
    foreach($accounts as $id){
        if(!isset($dotb_config['portal_view']) || $dotb_config['portal_view'] != 'single_user'){
...
function portal_get_entry_list_limited($session, $module_name,$where, $order_by, $select_fields, $row_offset, $limit){
    ...
    if($module_name == 'Cases'){
        ...
        $dotb = BeanFactory::newBean('Cases');
