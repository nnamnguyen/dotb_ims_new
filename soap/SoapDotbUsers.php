...
function login($user_auth, $application){
	global $dotb_config, $system_config;
...
/**
 * Use the same logic as in DotbAuthenticate to validate the ip address
 *
...
function is_valid_ip_address($session_var){
	...
	if (!isset ($dotb_config['verify_client_ip']) || $dotb_config['verify_client_ip'] == true) {
 ...
 * @param String $max_results -- The maximum number of records to return.  The default is the dotb configuration value for 'list_max_entries_per_page'
	...
	foreach($list as $value)
	{
        ...
        if (property_exists($value, 'currency_id'))
        {
            if (!isset($currencies[$value->currency_id])) {
                $currencies[$value->currency_id] = DotbCurrency::getCurrencyByID($value->currency_id);
            }
...
/**
 * Retrieve a list of DotbBean's based on provided IDs.
 *
 * @param String $session -- Session ID returned by a previous call to login.
 * @param String $module_name -- The name of the module to return records from.  This name should be the name the module was developed under (changing a tab name is studio does not affect the name that should be passed into this method)..
...
function set_entry($session,$module_name, $name_value_list){
    ...
    try{
        ...
        $GLOBALS['log']->info('End: SoapDotbUsers->set_entry');
...
/**
 * Retrieve the specific flavor of dotb.
 *
 * @return String   'PRO' -- For Professional
 *                  'ENT' -- For Enterprise
 */
function get_dotb_flavor(){
 global $dotb_flavor;
...
/**
 * Retrieve the version number of Dotb that the server is running.
 *
 * @return String -- The current dotb version number.
 *                   '1.0' on error.
 */
function get_server_version(){
	...
	if(isset($admin->settings['info_dotb_version'])){
		return $admin->settings['info_dotb_version'];
	}else{
		return '1.0';
	}
	...
	if($module1 == "Contacts" && $module2 == "Users"){
    	...
    	if(!$key) {
            ...
            if ($module1 == "Quotes" && $module2 == "ProductBundles") {
                ...
                // do whatever Dotb does when the same
 ...
 *
 * @param string $user_name 		- username of the Dotb User
 * @param string $password			- password of the Dotb User
...
function get_mailmerge_document($session, $file_name, $fields)
{
    ...
    if(!preg_match('/^dotbdata[\.\d\s]+\.php$/', $file_name)) {
        $error->set_error('no_records');
        return array('result'=>'', 'error'=>$error->get_soap_array());
    }
    $html = '';

    $file_name = dotb_cached('MergedDocuments/').pathinfo($file_name, PATHINFO_BASENAME);
...
function get_document_revision($session,$id)
{
    ...
    if(!empty($dr->filename)){
        ...
        $contents = base64_encode(dotb_file_get_contents($filename));
...
/**
 * Update or create a list of DotbBeans, returning details about the records created/updated
 *
 * @param String $session -- Session ID returned by a previous call to login.
 * @param String $module_name -- The name of the module to return records from.  This name should be the name the module was developed under (changing a tab name is studio does not affect the name that should be passed into this method)..
 * @param Array $name_value_lists -- Array of Bean specific Arrays where the keys of the array are the DotbBean attributes, the values of the array are the values the attributes should have.
 * @param Array  $select_fields -- A list of the fields to be included in the results. This optional parameter allows for only needed fields to be retrieved.
 * @return Array    'name_value_lists' --  Array of Bean specific Arrays where the keys of the array are the DotbBean attributes, the values of the array are the values the attributes should have.
 *                  'error' -- The SOAP error if any.
 */
