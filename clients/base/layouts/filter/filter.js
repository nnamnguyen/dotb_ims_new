 ...
 * @alias DOTB.App.view.layouts.BaseFilterLayout
 * @extends View.Layout
 */
({
    ...
    handleFilterChange: function(id) {
        ...
        _.each(ctxList, function(ctx) {
            ...
            //Set new order by - By Lap Nguyen
            var owr_orderBy = filter.get('orderBy');
            var ori_orderBy = ctx.get('collection').orderBy;
            if (!_.isUndefined(owr_orderBy) && !_.isEqual(ori_orderBy, owr_orderBy) )
            ctx.get('collection').orderBy = owr_orderBy;
        });
