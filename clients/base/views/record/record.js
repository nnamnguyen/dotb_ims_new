({
    ...
    initialize: function (options) {
        ...
        var self = this;
        app.events.on("app:view:custom-preview", function (activity) {
            var quickView = self.layout.getComponent('preview');
            if (!quickView) {
                ...
                self.layout._components.push(quickView);
                self.layout.$el.append(quickView.$el);
            }
            /**triggers an event to show the pop up quick create view*/
            self.layout.trigger("app:view:custom-preview", activity);
        });
    },
    ...
    handleSave: function () {
        ...
        //add by TKT handle dup check when edit
        if (App.metadata.getModule(App.controller.context.attributes.module).dupCheckEnabled && this.currentState == 'edit') {
            async.waterfall([
                _.bind(this.findDuplicateAndSave, this)
            ]);
        } else {
            this._saveModel();
    ...
    findDuplicateAndSave: function () {
        var url = App.api.buildURL(this.module, 'duplicateCheck');
        var data = App.data.getEditableFields(this.model);
        var success = _.bind(function (collection) {
            if (typeof collection.records.length != "undefined" && collection.records.length > 0) this.findDuplicates(this.model, this.module);
            else {
                this._saveModel();
                this.$('.record-save-prompt').hide();
        ...
        return App.api.call('create', url, data, {
            success: success
        });
    },
