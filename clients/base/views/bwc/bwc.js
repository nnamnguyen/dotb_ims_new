 ...
 * @alias DOTB.App.view.views.BaseBwcView
 * @extends View.View
 */
({
    className: 'bwc-frame',
    ...
    initialize: function(options) {
        // If (for some reason) we're trying to directly access old Home/Dashboards, for redirect to lumia #Home
     ...
     * - clear any '.bwc.dotbcrm' event (namespace for any bind in this view);
     ...
     * - rewrite links for lumia modules;
    ...
    _renderHtml: function() {
        ...
        this.$('iframe').on('load', function() {
            ...
            self._rewriteLinksForLumia(this.contentWindow);
            self._rewriteNewWindowLinks(this.contentWindow);
            self._cloneBodyClasses(this.contentWindow);

            $('html', this.contentWindow.document).on('click.bwc.dotbcrm', function() {
     ...
     * @inheritdoc
     *
     * Opens the appropriate lumia create layout in a drawer.
     *
     ...
     *
     * Confirms that the lumia hash is always matching the url in the iFrame
    ...
    /**
     * Gets the lumia url based on a given bwc hyperlink.
     * @param {String} href the bwc hyperlink.
     * @return {String} the new lumia hyperlink (empty string if unable to convert).
     */
    convertToLumiaUrl: function(href) {
    ...
    /**
     * Rewrites old link element to the new lumia router.
     *
     ...
     *
     * @param {HTMLElement} The link `<a>` to rewrite into a lumia url.
     */
    convertToLumiaLink: function(elem) {
        elem = $(elem);
        ...
        var module = this.moduleRegex.exec(href);
        var dataLumiaRewrite = elem.attr('data-lumia-rewrite');
        ...
        if (
            ...
            dataLumiaRewrite === 'false' ||
            (_.isArray(action) && action[1] === 'dotbpdf') //Leave PDF downloads for bwc modules
        ) {
            return;
        }
        var lumiaUrl = this.convertToLumiaUrl(href);
        elem.attr('href', baseUrl + '#' + lumiaUrl);
        elem.data('lumiaProcessed', true);
        ...
        elem.on('click.bwc.dotbcrm', function(e) {
            if (e.button !== 0 || e.ctrlKey || e.metaKey) {
                return;
            }
            e.stopPropagation();
            parent.DOTB.App.router.navigate(lumiaUrl, {trigger: true});
            return false;
        });
     ...
     */
    convertToLumiaErrors: function($errors) {
        if ($errors.length === 0) {
            return;
        }
    ...
    rewriteLinks: function() {
        ...
        this._rewriteLinksForLumia(frame);
        this._rewriteNewWindowLinks(frame);
    },

    /**
     * Rewrite old links on the frame given to the new lumia router.
     *
     * This will match all hrefs that contain "module=" on it and if the module
     * isn't blacked listed, then rewrite into lumia url.
     * Since iFrame needs full URL to lumia urls (to provide copy paste urls,
     ...
     * @private
     */
    _rewriteLinksForLumia: function(frame) {
        var self = this;

        frame.$('a[href*="module="]').each(function(i, elem) {
            self.convertToLumiaLink(elem);
        });
    },
     ...
     * to lumia already and make them lumia bwc compatible. This will
     * assume that all links to lumia modules are already rewritten.
     *
     * @param {Window} frame The `contentWindow` of the frame to rewrite links.
     * @private
     */
    ...
    /**
     * Unbinds all events that were hooked in this view with the `bwc.dotbcrm`
     ...
     *     parent.DOTB.App.view.views.BaseBwcView.prototype.methodWithBind();
     *     // memory leak will happen if `methodWithBind` doesn't use the
     *     // `.bwc.dotbcrm` namespace.
     *
    ...
    unbindDom: function() {
        ...
        $('a', bwcWindow.document).off('.bwc.dotbcrm');
        $('html', bwcWindow.document).off('.bwc.dotbcrm');
    },
    ...
    confirmMemLeak: function(target) {
        app.logger.debug(function() {
            var registered = _.reduce($('a', target), function(memo, el) {
                ...
                return memo + _.where(_.flatten(events), {namespace: 'bwc.dotbcrm'}).length;
            }, 0);

            return 'Clear ' + registered + ' event(s) in `bwc.dotbcrm`.';
        });
    },
