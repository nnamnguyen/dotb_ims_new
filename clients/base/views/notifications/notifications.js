 ...
 * @alias DOTB.App.view.views.BaseNotificationsView
 * @extends View.View
 */
({
    ...
    _initReminders: function () {
        ...
        _.each(['Calls', 'Meetings', 'Tasks'], function (module) {
            ...
            if (module === 'Tasks') {
                this._alertsCollections[module].options = {
                    ...
                    fields: ['date_start', 'id', 'name', 'remind_popup', 'parent_name']
                };
            } else if (module === 'Calls') {
                this._alertsCollections[module].options = {
                    limit: this.meta && parseInt(this.meta.remindersLimit, 10) || 100,
                    fields: ['date_start', 'id', 'name', 'reminder_time', 'parent_name']
                };
            } else {
                this._alertsCollections[module].options = {
                    limit: this.meta && parseInt(this.meta.remindersLimit, 10) || 100,
                    fields: ['date_start', 'id', 'name', 'reminder_time', 'parent_name', 'location']
                };
            }
        }, this);
                ...
                this._alertsCollections[module].filterDef = _.extend({},
                    {
                        ...
                        'status': {'$not_in': ['Cancel', 'Completed']}
                    }
                );
            } else {
                this._alertsCollections[module].filterDef = _.extend({},
            ...
            _.chain(collection.models)
                .filter(function (model) {
                    if (model.module === "Tasks") {
                        var needDate = (new Date(model.get('date_start'))).getTime() - parseInt(model.get('remind_popup'), 10) * 1000;
                    } else {
    ...
    _showReminderAlert: function (model) {
        ...
        _.defer(function () {
            $.confirm({
                icon: App.config.module_icon[model.module]['src'],
                title: model.module,
                content: message,
                buttons: {
                    "OK": {
