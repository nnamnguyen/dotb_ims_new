 ...
 * @alias DOTB.App.view.views.BaseLoginView
 * @extends View.View
 */
({
    /**
...
//        // Set the page title to 'DotBCRM' while on the login screen
//        $(document).attr('title', 'DotBCRM');
    },
    ...
    login: function () {
        var username = this.$('input[name=username]').val().trim(),
            password = this.$('input[name=password]').val().trim();
        if (username == '' || password == '') {
            app.alert.show('required_input_full', {
                level: 'error',
                messages: app.lang.get('LBL_PLEASE_INPUT_USER_PASS', 'Users'),
                autoClose: true
            });
            return;
        }
     ...
     */
    showDotbLoginForm: function (error) {
     ...
     * login setup we need to do prior to rendering the rest of the Dotb app.
     */
    postLogin: function () {
