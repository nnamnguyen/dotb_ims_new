 ...
 * @alias DOTB.App.view.views.BaseFilterRowsView
 * @extends View.View
 */
({
    events: {
    ...
    populateFilter: function() {
        ...
        iis_template = this.context.editingFilter.get('is_template');

        if(iis_template){ //Revert original values. - By Lap Nguyen
            var filterDef = this.context.editingFilter.getSynced().filter_definition;
        }else
            var filterDef = this.context.editingFilter.get('filter_template') || this.context.editingFilter.get('filter_definition');
    ...
    initValueField: function($row) {
        ...
        if (operation === '$between' || operation === '$dateBetween') {
            ...
            if (fieldType === 'currency' && $row.data('value')) {
                ...
                if(fieldType == 'date'){
                    var date_value = $row.data('value');
                    var today = new Date();
                    switch (date_value) {
                        case 'today':
                            date_value = app.date.format(today,'Y-m-d');
                            break;
                        case 'last_7_days':
                            var days = 7;
                            var res = today.setTime(today.getTime() - (days * 24 * 60 * 60 * 1000));
                            date_value = app.date.format(new Date(res),'Y-m-d');
                            break;
                        case 'next_7_days':
                            var days = 7;
                            var res = today.setTime(today.getTime() + (days * 24 * 60 * 60 * 1000));
                    ...
                    model.set(fieldDef.id_name || fieldName, date_value);
                }else
