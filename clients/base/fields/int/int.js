 ...
 * @alias DOTB.App.view.fields.BaseIntField
...
({
    ...
    /**
     * This validates int doesn't exceed min/max value defined in dotb config.
     *
    ...
    _doValidateMinMaxInt: function(fields, errors, callback) {
        ...
        if (!_.isUndefined(app.config.dotbMinInt)) {
            minValue = Math.max(minValue, app.config.dotbMinInt);
        }
        if (!_.isUndefined(app.config.dotbMaxInt)) {
            maxValue = Math.min(maxValue, app.config.dotbMaxInt);
        }
        if (value < minValue) {
