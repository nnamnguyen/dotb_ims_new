 ...
 * @alias DOTB.App.view.fields.BaseButtonField
...
({
    ...
    _render:function() {
        //Tuan Anh check edit mode in list view
        if(this.tplName == 'list' && this.action == 'edit'){
            if(this.def.type != "editablelistbutton"){
                return 0;
            }
        }
        //End
        if(this.def.customRoute){
            if(this.def.name_button == 'import_button'){
                this.fullRoute = app.bwc.buildRoute('Import',null,'Step1',{'import_module': this.module});
            }else if (this.def.name_button == 'report_button'){
                this.fullRoute ='Reports?filterModule=' + this.module;
            }else{
