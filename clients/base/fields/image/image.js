...
({
    ...
    _render: function () {
        ...
        if (this.view.action === 'list' && this.action === 'edit') {
            ...
            if (this.view && this.view.meta && this.view.meta.type === 'list') {
                //Modified by Tuan Anh resize picture
    ...
    format: function (value) {
        if (value) {
            value = this.buildUrl() + "&_hash=/resize/" + value;
        }
        return value;
    },
    ...
    resizeWidget: function () {
        ...
        this.$('.delete').remove();
        var icon = this.preview === true ? 'times' : 'trash-alt';
        image.closest('label, a').after('<span class="image_btn delete far fa-' + icon + ' " />');
    },
    ...
    //Create by Tuan Anh to popup image
    popupImage: function () {
        var sourceImage = this.value.replace("resize", "origin");
        var img    = new Image();
        img.onload = function() {
            ...
            '    position: relative;"><img style="height:'+this.height+'px; width: '+this.width+'px;" src="'+sourceImage+'" class="imagepreview"></div>';
