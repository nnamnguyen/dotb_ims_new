 ...
 * @alias DOTB.App.view.fields.BaseActiondropdownField
...
({
    ...
    initialize: function (options) {
         ...
         * @property {string}
         */
        this.caretIcon = this.def.icon || 'fa-angle-down';
    ...
    _orderButtons: function (fields) {
        ...
        if (_.isUndefined(app.config.edit_list_view)) {
            if (this.dropdownFields.length > 2) this.isRenderDropdown = true;
        } else {
            if (_.isUndefined(app.config.edit_list_view[this.module]) || !app.config.edit_list_view[this.module]) {
                if (this.dropdownFields.length > 2) this.isRenderDropdown = true;
            }else{
                this.isRenderDropdown = true;
            }
        }


        _.each(this.dropdownFields, function (field) {
            field.def.customButton = false;
            field.def.icon = '';
            field.def.tooltip = '';
        });
    },
