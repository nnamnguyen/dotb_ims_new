...
class RelateRecordApi extends DotbApi
{
     ...
     * @param $primaryBean DotbBean The near side of the link
     ...
     * @return array Two elements: The link name, and the DotbBean of the far end
    ...
    protected function checkRelatedSecurity(
        ...
        if ( ! $primaryBean->ACLAccess($securityTypeLocal) ) {
            throw new DotbApiExceptionNotAuthorized('No access to '.$securityTypeLocal.' records for module: '.$args['module']);
        }
        ...
        if ( $securityTypeRemote == 'edit' && ! $linkSeed->ACLAccess($securityTypeRemote) ) {
            throw new DotbApiExceptionNotAuthorized('No access to '.$securityTypeRemote.' records for module: '.$linkModuleName);
        }
    ...
    public function getRelatedRecord(ServiceBase $api, array $args)
    {
        ...
        if (!empty($related[0]->id)) {
            ...
            if (!$relatedBean->retrieve($args['remote_id'])) {
                ...
                throw new DotbApiExceptionNotFound('Could not find the related bean');
            }
     ...
     * @param array $args The arguments array passed in from the API.
     * @return array Two elements, 'record' which is the formatted version of $primaryBean, and 'related_record' which is the formatted version of $relatedBean
     * @throws DotbApiExceptionError In case if the module API has improper interface
     */
     ...
     * @throws DotbApiExceptionInvalidParameter If wrong arguments are passed
     * @throws DotbApiExceptionNotFound If bean can't be retrieved.
     */
    ...
    protected function normalizeLinkIds($ids)
    {
        ...
        foreach ($ids as $record) {
            if (is_array($record)) {
                if (!isset($record['id'])) {
                    throw new DotbApiExceptionInvalidParameter('Related record ID is not specified in array notation');
                }
                $id = $record['id'];
    ...
    public function updateRelatedLink(ServiceBase $api, array $args)
    {
        ...
        if (isset($primaryBean->field_defs[$linkName])) {
            ...
            if (isset($def['type']) && $def['type'] == 'link' && !empty($def['readonly'])) {
                throw new DotbApiExceptionNotAuthorized("Cannot update related records on readonly relationships");
            }
        }
