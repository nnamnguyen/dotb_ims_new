<?php
    ...
    class ModuleApi extends DotbApi
    {
        ...
        public function getEnumValues(ServiceBase $api, array $args)
        {
            ...
            if (!isset($bean->field_defs[$args['field']])) {
                throw new DotbApiExceptionNotFound('field not found');
            }
            ...
            if ($value === false) {
                throw new DotbApiExceptionNotFound('options not found');
            }
        ...
        public function createBean(ServiceBase $api, array $args, array $additionalProperties = array())
        {
            ...
            if (!$bean->ACLAccess('save', $this->aclCheckOptions)) {
                ...
                throw new DotbApiExceptionNotAuthorized('EXCEPTION_CREATE_MODULE_NOT_AUTHORIZED', $args);
            }
         ...
         * @throws DotbApiExceptionInvalidParameter
         */
        public function updateRelatedRecords(ServiceBase $api, DotbBean $bean, array $args)
        {
        ...
        public function retrieveRecord(ServiceBase $api, array $args)
        {
            ...
            if (!$bean->ACLAccess('view', $this->aclCheckOptions)) {
                throw new DotbApiExceptionNotAuthorized('DOTB_API_EXCEPTION_RECORD_NOT_AUTHORIZED', array('view'));
            }
        ...
        public function setFavorite(ServiceBase $api, array $args)
        {
            ...
            if (!$bean->ACLAccess('view', $this->aclCheckOptions)) {
                ...
                throw new DotbApiExceptionNotAuthorized('EXCEPTION_FAVORITE_MODULE_NOT_AUTHORIZED', $args);
            }
         ...
         * @param DotbBean $bean The bean associated.
         * @return array Array of additional related fields
         */
        protected function getRelatedFields(array $args, DotbBean $bean)
        {
         ...
         *
         * @param array $args The request arguments.
         * @param DotbBean $bean The bean associated with the file.
         * @throws DotbApiExceptionInvalidParameter If the file mime types differ
         *   from $imageFileMimeTypes.
         */
        protected function moveTemporaryFiles(array $args, DotbBean $bean)
        {
            ...
            $configDir = DotbConfig::getInstance()->get('upload_dir', 'upload');
            ...
            foreach ($fileFields as $fieldName => $def) {
                ...
                if (!empty($bean->picture)) {
                    $picName = explode("/", $bean->picture)[2];
                    if ($picName == $args[$fieldName . '_guid']) {
                        continue;
                    }
                }
                ...
                if ($def['type'] === 'image') {
                    ...
                    // should not be duplicated from DotbFieldFile.
                    ...
                    !verify_image_file($filepath)
                        ...
                        throw new DotbApiExceptionInvalidParameter(string_format(
                ...
                //Modified by Tuan Anh to resize avatar
                if ($def['type'] === 'image') {
                    $desOri = UploadStream::STREAM_NAME . '://resize/' . $filename;
                    $imageOri = new ImageHelper($filepath);
                    $imageOri->resize(220,220);
                    $imageOri->save($desOri);

                    $desOri = UploadStream::STREAM_NAME . '://origin/' . $filename;
                    $imageOri = new ImageHelper($filepath);
                    $imageOri->resize(500);
                    $imageOri->save($desOri);

                    //remove tmp files - Lap Nguyen
         ...
         * @param ServiceBase $api
         * @param array $args API arguments
         * @param DotbBean $bean The saved bean
         *
         * @return array Formatted representation of the bean
         */
        protected function formatBeanAfterSave(ServiceBase $api, array $args, DotbBean $bean)
        {
            $api->action = 'view';
         ...
         * @param DotbBean $bean
         */
        protected function processAfterCreateOperations(array $args, DotbBean $bean)
        {
            $this->requireArgs($args, array('module'));
         ...
         * @throws DotbApiExceptionInvalidParameter
         */
        protected function getRelatedRecordArguments(DotbBean $bean, array $args, $action)
        {
            $arguments = array();
         ...
         * @throws DotbApiExceptionNotFound
         */
        protected function unlinkRelatedRecords(ServiceBase $service, DotbBean $bean, array $ids)
        {
            $api = $this->getRelateRecordApi();
            foreach ($ids as $linkName => $items) {
                foreach ($items as $id) {
         ...
         */
        protected function createRelatedRecords(ServiceBase $service, DotbBean $bean, array $data)
        {
            $api = $this->getRelateRecordApi();
