...
class CalendarEventsApi extends ModuleApi
{
     ...
     * @param DotbBean $bean
     */
    public function generateRecurringCalendarEvents(DotbBean $bean)
    {
     ...
     * @param array $args
     * @return array
     * @throws DotbApiExceptionInvalidParameter - when updating using the 'all_recurrences' option, the id of the
     *         Parent (root) bean must be provided.
     */
    public function updateRecurringCalendarEvent(DotbBean $bean, ServiceBase $api, array &$args)
    {
        if (!empty($bean->repeat_parent_id) && ($bean->repeat_parent_id !== $bean->id)) {
            throw new DotbApiExceptionInvalidParameter('ERR_CALENDAR_CANNOT_UPDATE_FROM_CHILD');
        }
     ...
     * @param DotbBean $bean
     */
    public function deleteRecurrences(DotbBean $bean)
    {
     ...
     * @return array
     */
    protected function initializeArgs(array $args, DotbBean $bean = null)
    {
        $repeatType = '';
        if (!empty($bean)) {
    ...
    protected function adjustStartDate(array &$args)
    {
        if (!empty($args['repeat_type']) && !empty($args['date_start'])) {
            ...
            if (empty($sequence)) {
                throw new DotbApiExceptionMissingParameter('ERR_CALENDAR_NO_EVENTS_GENERATED');
            }
     ...
     * @return bool
     */
    protected function shouldAutoInviteParent(DotbBean $bean, array $args)
    {
