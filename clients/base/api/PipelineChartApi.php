...
class PipelineChartApi extends DotbApi
{
    /**
    ...
    public function pipeline(ServiceBase $api, array $args)
    {
        ...
        foreach ($rows as $row) {
            ...
            $base_amount = DotbCurrency::convertWithRate($row[$amount_field], $row['base_rate']);

            // add the new value into what was already there
            $data[$row['sales_stage']]['total'] = DotbMath::init($data[$row['sales_stage']]['total'])->add(
                $base_amount
            )->result();
        ...
        $return_data = array();
        $series = 0;
        $previous_value = DotbMath::init('0');
        foreach ($data as $key => $item) {
            $value = DotbCurrency::convertAmountFromBase($item['total'], $currency->id);
            ...
            $return_data[] = array(
                ...
                'values' => array(      // the values used in the grid
                    array(
                        'series' => $series++,
                        'label' => DotbCurrency::formatAmount($value, $currency->id, 0),
        ...
        //return the total from the DotbMath Object.
        $total = DotbCurrency::convertAmountFromBase($total->result(), $currency->id);
        return array(
            'properties' => array(
                'title' => $mod_strings['LBL_PIPELINE_TOTAL_IS'] . DotbCurrency::formatAmount($total, $currency->id),
                'total' => $total,
                'scale' => 1000,
     ...
     */
    protected function buildQuery(ServiceBase $api, DotbBean $seed, $tp, $amount_field, $type = 'user')
    {
        // build out the query
        $sq = new DotbQuery();
    ...
    protected function getTimeperiod($tp_id = '')
    {
        ...
        if (DotbACL::checkAccess('Forecasts', 'access') && $forecast_settings['is_setup'] == 1) {
        ...
        if (is_null($tp) || empty($tp->id)) {
            throw new DotbApiExceptionInvalidParameter('Provided TimePeriod is invalid');
        }
