...
class FileApi extends DotbApi
{
    ...
    public function saveFilePut(ServiceBase $api, array $args, $stream = 'php://input')
    {
        ...
        // Mock a $_FILES array member, adding in _DOTB_API_UPLOAD to allow file uploads
        $_FILES[$args['field']] = array(
            ...
            'error' => 0,
            'size' => $filesize,
            '_DOTB_API_UPLOAD' => true, // This is in place to allow bypassing is_uploaded_file() checks
        );
    ...
    {
        //Needed by DotbFieldImage.php to know if we are saving a temporary image
        $args['temp'] = $temporary;

        // Get the field
        $field = $args['field'];

        // To support field prefixes like Dotb proper
        ...
        if (empty($_FILES[$filesIndex])) {
            ...
            throw new DotbApiExceptionMissingParameter("Incorrect field name for attachement: $filesIndex");
        }
        ...
        if (isset($def['type']) && ($def['type'] == 'image' || $def['type'] == 'file')) {
            ...
            $sf = $sfh->getDotbField($def['type']);
            if ($sf) {
                // DotbFieldFile expects something different than DotbFieldImage
                if ($def['type'] == 'file') {
                    ...
                    // Handle setting the files array to what DotbFieldFile is expecting
                ...
                if (!empty($sf->error)) {
                    ...
                    // if file > php.ini upload_maxsize, we still may have a file > dotbcrm maxsize
                    $this->deleteIfFails($bean, $args);
                    throw new DotbApiExceptionRequestTooLarge($sf->error);
                }
        ...
        // @TODO Localize this exception message
        throw new DotbApiExceptionError("Unexpected field type: " . $def['type']);
    }
     ...
     * @param ServiceBase $api The service base
     * @param array $args Arguments array built by the service base
     * @throws DotbApiExceptionMissingParameter When field name is missing.
     * @throws DotbApiExceptionNotFound When file cannot be found.
     * @throws DotbApiExceptionNotAuthorized When there is no access to record in module.
     */
    public function getFile(ServiceBase $api, array $args)
    {
        ...
        if (empty($bean->{$field})) {
            // @TODO Localize this exception message
            throw new DotbApiExceptionNotFound("The requested file $args[module] :: $field could not be found.");
        }

        //Tuan Anh modified folder get picture
        if(isset($bean->field_defs[$field]['type']) && $bean->field_defs[$field]['type'] == 'image' && !empty($args['_hash'])){
            $bean->{$field} = $args['_hash'];
        }
        //End
        ...
        try {
            ...
            throw new DotbApiExceptionNotFound($e->getMessage(), null, null, 0, $e);
        }
    }
     ...
     * @throws DotbApiExceptionError|DotbApiExceptionNoMethod|DotbApiExceptionRequestMethodFailure
     */
                ...
                if ($def['type'] == 'file') {
                    if (method_exists($bean, 'deleteAttachment')) {
                        if (!$bean->deleteAttachment()) {
                            // @TODO Localize this exception message
                            throw new DotbApiExceptionRequestMethodFailure('Removal of attachment failed.');
                        }
                    } else {
                        // @TODO Localize this exception message
                        throw new DotbApiExceptionNoMethod('No method found to remove attachment.');
                    }
                } else {
                    $upload = new UploadFile($field);
                    $upload->unlink_file($bean->$field);

                    //Uplink image - By Lap Nguyen
                    $des_resize = UploadStream::STREAM_NAME . '://resize/' . $bean->$field;
                    $des_origin = UploadStream::STREAM_NAME . '://origin/' . $bean->$field;
                    if (file_exists($des_resize)) unlink($des_resize);
                    if (file_exists($des_origin)) unlink($des_origin);
                    //End - By Lap Nguyen
     ...
     * @param array $args Arguments array built by the service base
     *
     * @throws DotbApiExceptionNotFound When record, name of relationship or file cannot be found.
     * @throws DotbApiExceptionNotAuthorized When there is no access to record.
     * @throws DotbApiExceptionMissingParameter When field name is missing.
     * @throws DotbApiExceptionInvalidParameter When relationship name is incorrect.
     */
    public function getArchive(ServiceBase $api, array $args)
    {
        ...
        if (!$record->load_relationship($linkName)) {
            ...
            throw new DotbApiExceptionNotFound('Could not find a relationship named: ' . $args['link_name']);
        }
        ...
        if (!$linkSeed->ACLAccess('list')) {
            throw new DotbApiExceptionNotAuthorized('No access to list records for module: ' . $linkModuleName);
        }
        $field = $args['field'];
    ...
    /**
     * Provides ability to mark a DotbBean deleted if related file upload failed (and user passed
     ...
     *
     * @param DotbBean $bean The bean to get the info from
     * @param string $field The field name to get the file data from
     * @param ServiceBase $api The calling API service object
     * @return array|bool
     */
    protected function getFileInfo(DotbBean $bean, $field, ServiceBase $api)
    {
        $info = array();
