 ...
 *
 */
class UnifiedSearchApi extends DotbListApi {
    ...
    public function globalSearch(ServiceBase $api, array $args) {
        ...
        $searchEngine = DotbSearchEngineFactory::getInstance($this->determineDotbSearchEngine($api, $args, $options), array(), false);
        ...
        */
        ...
        if(!empty($options['moduleList']))
        {
            foreach($options['moduleList'] AS $module)
            {
                ...
                //use the local database search, i.e., return 'DotbSearchEngine' here.
        ...
        $fts = DotbSearchEngineFactory::getFTSEngineNameFromConfig();
     ...
     * @param ServiceBase $api The API class of the request
     * @param array $args The arguments array passed in from the API
     * @param $searchEngine DotbSearchEngine The DotbSpot search engine created using the Factory in the caller
     * @param $options array An array of options to pass through to the search engine, they get translated to the $searchOptions array so you can see exactly what gets passed through
     ...
     */
    protected function globalSearchFullText(ServiceBase $api, array $args, DotbSearchEngineAbstractBase $searchEngine, array $options)
    {
        $api->action = 'list';
        $returnedRecords = array();

        // DotbSearchEngine uses moduleFilter instead of moduleList, pass it along
    ...
    /**
     * This function is used to hand off the global search to the built-in DotbSearchEngine (aka DotbSpot)
     * @param ServiceBase $api The API class of the request
     * @param array $args The arguments array passed in from the API
     * @param $searchEngine DotbSearchEngine The DotbSpot search engine created using the Factory in the caller
     * @param $options array An array of options to pass through to the search engine, they get translated to the $searchOptions array so you can see exactly what gets passed through
