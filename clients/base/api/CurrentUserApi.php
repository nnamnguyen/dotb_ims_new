...
class CurrentUserApi extends DotbApi
{
    /**
    ...
    public function setExpiredPassword($user_data)
    {
        ...
        if (hasPasswordExpired($user_data['user_name'])) {
            ...
            $passwordSettings = $GLOBALS['dotb_config']['passwordsetting'];
            $user_data['password_requirements'] = $this->getPasswordRequirements($passwordSettings);
        }
        return $user_data;
    }

    //Essentially 7.X version of legacy smarty_function_dotb_password_requirements_box
     ...
     * @param array $args
     * @return array
     * @throws DotbApiExceptionMissingParameter|DotbApiExceptionNotFound
     */
    public function updatePassword(ServiceBase $api, array $args)
    {
        ...
        if (empty($args['old_password']) || empty($args['new_password'])) {
            ...
            throw new DotbApiExceptionMissingParameter('Error: Missing argument.');
        } else {
    ...
    protected function getUserPrefLanguage(User $user)
    {
        ...
        if (!empty($_SESSION['authenticated_user_language'])) {
            ...
            // if nothing exists, get the dotb_config default language
            $language = $GLOBALS['dotb_config']['default_language'];
        }
    ...
    protected function getUserData(ServiceBase $api, array $options)
    {
        ...
        //Add By Lap nguyen
        $user_data['phone_mobile'] = $current_user->phone_mobile;
        $user_data['phone_work'] = $current_user->phone_work;
        $user_data['phone_other'] = $current_user->phone_other;
        ...
        $sf = DotbFieldHandler::getDotbField('email');
     ...
     * @return array
     */
    protected function changePassword(DotbBean $bean, $old, $new)
    {
    ...
    public function userPreferences(ServiceBase $api, array $args)
    {
        ...
        if(empty($data['dashboard_id'])){
            ...
            $dashboard = BeanFactory::retrieveBean('Dashboards', $data['dashboard_id']);
            if(empty($dashboard)) $data['dashboard_id'] = 0;
        }
