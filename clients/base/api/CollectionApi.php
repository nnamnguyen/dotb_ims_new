 ...
 * Collection API
 */
abstract class CollectionApi extends DotbApi
{
    /**
     ...
     *
     * Any DotbApiException's that are thrown when retrieving related records are caught and added to the return value.
     ...
     * @throws DotbApiExceptionInvalidParameter when arguments are invalid and thus cannot be normalized
     */
    protected function normalizeArguments(array $args, CollectionDefinitionInterface $definition)
    {
        ...
        // convert fields to a array for consistent behavior with DotbApi::formatBeans
     ...
     *
     * @return array Normalized value
     * @throws DotbApiExceptionInvalidParameter when the stored filter given isn't found.
     */
    protected function normalizeStoredFilter(array $args, CollectionDefinitionInterface $definition)
    {
        ...
        foreach ($filter as $filterName) {
            if (!$definition->hasStoredFilter($filterName)) {
                throw new DotbApiExceptionInvalidParameter('Stored filter not found');
            }
        }
        ...
        foreach ($orderBy as $alias => $direction) {
            ...
            foreach ($sourceData as $source => $data) {
                ...
                if (!$fieldDef) {
                    ...
                    throw new DotbApiExceptionError('Unable to load field definition');
                }
