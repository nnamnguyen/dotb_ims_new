...
class OAuth2Api extends DotbApi
{
    ...
    protected function getOAuth2Server(array $args)
    {
        ...
        $oauth2Server = DotbOAuth2Server::getOAuth2Server($platform);
    ...
    public function token(ServiceBase $api, array $args)
    {
        ...
        if ( !$validVersion ) {
            throw new DotbApiExceptionClientOutdated();
        }
        ...
        try {
            ...
            $cleanupChance = isset($GLOBALS['dotb_config']['token_cleanup_probability'])?(int)$GLOBALS['dotb_config']['token_cleanup_probability']:10;
    ...
    public function sudo(ServiceBase $api, array $args)
    {
        ...
        if (!$token) {
            throw new DotbApiExceptionRequestMethodFailure("Could not setup a token for the requested user");
        }
    ...
    protected function sendSessionCookie()
    {
        ...
        // user logs back in.
        $expire = time() + DotbConfig::getInstance()->get(
     ...
     * BWC logins so that BWC sessions stay in sync with lumia sessions.
     */
