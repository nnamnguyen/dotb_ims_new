 ...
 */
class MassUpdateApi extends DotbApi {
    ...
    public function massDelete(ServiceBase $api, array $args)
    {
        ...
        if (empty($args['massupdate_params']['uid'])) {
            throw new DotbApiExceptionMissingParameter('Missing required parameter uid');
        }
    ...
    public function massUpdate(ServiceBase $api, array $args)
    {
        ...
        if (!$bean instanceof DotbBean) {
            throw new DotbApiExceptionInvalidParameter("Invalid bean, is module valid?");
        }
        $action = $this->delete? 'delete': 'save';
        if (!$bean->ACLAccess($action))
        {
            throw new DotbApiExceptionNotAuthorized('No access to mass update records for module: '.$mu_params['module']);
        }
        $mu_params['action'] = $action;

        $massUpdateJob = new DotbJobMassUpdate();
