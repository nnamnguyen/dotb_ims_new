({
    ...
    events: {
        ...
        'click .remove_que': 'remove_icon_clicked', // remove icon of question or option clicked
        ...
        'click #dateicon': 'show_datepicker', //show datepicker for date question-type
        ...
        'click .cloneQue': 'cloneQueClicked', // Clone Question button clicked
        'click .page_sel_clone': 'clonePageClicked', // Clone to Page selected
        'click .disable_que,.disable_section_que': 'disabled_icon_clicked', // disable icon of question or option clicked
        'click .enable_que,.enable_section_que': 'enable_icon_clicked', // enable icon of question or option clicked
        'click .que-piping,.ans-piping': 'quePipingClicked', // enable question piping for question button clicked
        'change .que_sel_qpipe': 'qpipeSelected', // Pipe question from selection que
    },
     ...
     */
    _render: function () {
        var editAction = this.view.currentState;
        if (((editAction != "edit" || this.action != "edit") && this.$el.parents('.main-pane').find('#detail_view').length == 0) || (editAction == 'edit' && this.$el.parents('.main-pane').find('#page_1').find('[name=txt_page_title]').length == 0)) {
            this._super('_render');
        }
            ...
            request.xhr.done(function () {
                ...
                if (self.model.get('name') && typeof localStorage['copiedFromModelId'] != "undefined" && localStorage['copyImageId'] != "null" && localStorage['copyBGImageId'] != "null") {
                    ...
                    if ((typeof copyModuleID !== 'undefined' && copyModuleID !== '') && (typeof copyImageId !== 'undefined' && copyImageId !== '')) {
                        ...
                        {
                            self.$el.parents('.main-pane').find('[data-name=survey_logo]').find('.image_preview').append('<img src="' + App.api.serverUrl + '/' + copyFromModule + '/' + copyModuleID + '/file/' + copyImagField + '?format=dotb-html-json&platform=base&_hash=' + copyImageId + '" style="width: 96px; height: 60px;" />')
                            self.$el.parents('.main-pane').find('[data-name=survey_logo]').find('.image_preview').find('.fa-plus').remove();
                        } else {
                            self.$el.parents('.main-pane').find('[data-name=survey_logo]').find('.image_preview').find('img').attr('src', App.api.serverUrl + '/' + copyFromModule + '/' + copyModuleID + '/file/' + copyImagField + '?format=dotb-html-json&platform=base&_hash=' + copyImageId)
                        }
                    ...
                    if ((typeof copyModuleID != 'undefined' && copyModuleID != '') && (typeof copyImageId != 'undefined' && copyImageId != '')) {
                        self.$el.parents('.main-pane').find('[data-name=survey_background_image]').find('.image_preview').find('img').attr('src', App.api.serverUrl + '/' + copyFromModule + '/' + copyModuleID + '/file/' + copyBGImagField + '?format=dotb-html-json&platform=base&_hash=' + copyImageId);
        ...
        if (self.model && self.model.get('survey_type') == 'poll')
        {
            ...
            self.$el.parents('.main-pane').next('.sidebar-content').find('.survey_theme').css('cursor', 'default');
        ...
        if (this.$el.find('#txt_page_title').length == 0 && editAction != "view") { // by default one page should display
            if (this.$el.parents('.main-pane').find('.Survey_Pages').find('.SurveyPage').length == 0 && editAction == "create") {
                this.$el.parents('.main-pane').find('.Survey_Pages').html('<div class="SurveyPage" tabindex="-1"><p align="center">Add a Survey Page</p><div class="add-survey-page"><p align="center"><a><i style="opacity:0.8" class="fa fa-plus fa-4x" id="AddNewSurveyPage_icon"></i></a></p></div></div>');
            }
        ...
        if (!self.sync_module_fields && !self.sync_module_fields_types) {
        ...
        if (self.record_id != null) {
            ...
            if (self.survey_status == null) {
                ...
                App.api.call('GET', url, {}, {
                    success: function (data) {
                        ...
                        if ((editAction != "edit" || self.action != "edit")) {
                            ...
                            self.survey_status = data['survey_status'];
                        ...
                        if (data && data['restrict_edit'] == "1") { // survey is send so disable all kind of edit of page
                            ...
                            //Changes regarding edit survey when submission > 0 5th august 2019 
                            if (data && data['submissionCount_edit'] == "1") {
                                self.submissionCount_edit = "true";
                                self.$el.parents('.main-pane').find('[data-fieldname=enable_data_piping]').find('[type=checkbox]').attr('disabled', true);
                                if (self.$el.parents('.main-pane').find('.enable_data_piping_field:checked').length != 0) {
                                    self.$el.parents('.main-pane').find('[data-name=sync_module]').find('div:first').show();
                                    self.$el.parents('.main-pane').find('[data-fieldname="sync_module"]').show();
                                    self.$el.parents('.main-pane').find('[data-fieldname="sync_module"]').css('pointer-events', 'none');

                                    self.$el.parents('.main-pane').find('[data-name=sync_type]').find('div:first').show();
                                    self.$el.parents('.main-pane').find('[data-fieldname="sync_type"]').show();
                                    self.$el.parents('.main-pane').find('[data-fieldname="sync_type"]').css('pointer-events', 'none');
                                }
                            } else {
                                self.submissionCount_edit = "false";
                                if (self.$el.parents('.main-pane').find('.enable_data_piping_field:checked').length != 0) {
                                    self.$el.parents('.main-pane').find('[data-name=sync_module]').find('div:first').show();
                                    self.$el.parents('.main-pane').find('[data-fieldname="sync_module"]').show();
                        ...
                        // check dotb 7.7.* or not
                        if (data && data['dotb_latest'] == "1")
                        {
                            self.dotb_latest = true;
                        } else {
                            self.dotb_latest = false;
                        }
                ...
                var editAction = self.view.currentState;
                if ((editAction == "edit" || self.action == "edit") && self.record_id != null) {
                    self.$el.parents('.main-pane').find('.get_share_link').hide();
                    //Changes for QR Code
                    self.$el.parents('.main-pane').find('[data-name="survey_qr_code"]').hide();
                    self.$el.parents('.main-pane').find('[data-fieldname="survey_qr_code"]').hide();
                    ...
                    self.surveypagesdetailView(1, self.record_id, "");
                    self.$el.parents('.main-pane').find('.data-page').attr('style', ''); // open all page collpase
                } else {
                    if (self.model && self.model.get('survey_type') != 'poll') {
                        //changes for not showing Allow Redundant Answer in Detail view of Survey by kairvi
//                            self.$el.parents('.main-pane').find('[data-name="allow_redundant_answers"]').show();
                    }
                    if (self.$el.parents('.main-pane').find('.get_share_link').next('input').length == 0) {
                        self.$el.parents('.main-pane').find('.get_share_link').show();
                    } else {
                        self.$el.parents('.main-pane').find('.get_share_link').hide();
                    }
                    self.$el.parents('.main-pane').find('.open_link').show();
                    self.$el.parents('.main-pane').find('.copy_survey_link').show();
                    self.$el.parents('.main-pane').find('[data-name="survey_qr_code"]').show();
                    self.$el.parents('.main-pane').find('[data-fieldname="survey_qr_code"]').show();
                    self.surveypagesdetailView('', self.record_id, "");
                }
...
//                            self.$el.parents('.main-pane').find('[data-name="allow_redundant_answers"]').show();
        ...
        $(document).on('click', '.enable_data_piping_field', function (el) {
            ...
            if ($(el.currentTarget).parents('.main-pane').find('.enable_data_piping_field:checked').length != 0)
            {
                ...
                app.alert.show('change_sync_module_confirmation', {
                    level: 'confirmation',
                    title: '',
                    messages: 'Disabling data piping will reset existing question\'s sync field. Are you sure that you want to proceed?',
                    onConfirm: function () {
                        ...
                        $.each(current_el.parents('.main-pane').find('.question'), function () {
                            ...
                            if ($(this).find('.sync_field_selection').val() != "Select Field") {
                                var question_type_id = $(this).attr('id');
                                $(this).find('.general_options').find('[name="enable_scoring_' + question_type_id + '"]').prop('checked', '');
                                $(this).find('.general_options').find('[name="enable_scoring_' + question_type_id + '"]').prop('checked', '');
                                $(this).find('.general_options').find('[name="is_image_option_' + question_type_id + '"]').prop('checked', '')
                                ...
                                if (muti_choice_que_type != 'boolean')
                                {
                                    if (muti_choice_que_type == 'radio-button' || muti_choice_que_type == 'check-box') {
                                        ...
                                        data += "                 <input type='file' name='radioImage_" + muti_choice_que_type + "' class='inherit-width radioImageUpload' style='display:none;width: 175px;margin-top:5px;max-width:50%; margin-left: 1px;'><span style='display:none;margin-left: -13px; margin-top: 6px; position: relative; z-index: 500; width: auto;' class='spanRadioUploadError'><a><i class='fa fa-exclamation-circle' title='Error. This field is required.' style='color:red;font-size:12px;'></i></a></span><input type='text' name='option_" + muti_choice_que_type + "' placeholder='Option' class='inherit-width' style='max-width:50%;'>";
                                        data += "                 <input type='number' name='score_" + muti_choice_que_type + "'  value='1' class='inherit-width score_weight' style='max-width:7%; display:none;margin-left: 4px;margin-top: 5px;' min='0' onpaste='return false;'>";
                                        ...
                                        data += "                 <input type='file' name='radioImage_" + muti_choice_que_type + "' class='inherit-width radioImageUpload' style='display:none;width: 175px;margin-top:5px;max-width:50%; margin-left: 1px;'><span style='display:none;margin-left: -13px; margin-top: 6px; position: relative; z-index: 500; width: auto;' class='spanRadioUploadError'><a><i class='fa fa-exclamation-circle' title='Error. This field is required.' style='color:red;font-size:12px;'></i></a></span><input type='text' name='option_" + muti_choice_que_type + "' placeholder='Option' class='inherit-width' style='max-width:50%;margin-top:5px;'>";
                                        data += "                 <input type='number' name='score_" + muti_choice_que_type + "' value='2' class='inherit-width score_weight' style='width:7%;display:none;margin-left: 4px;margin-top: 5px;' min='0' onpaste='return false;'>";
                                        ...
                                        data += "                 <input type='number' name='score_" + muti_choice_que_type + "' value='2' class='inherit-width score_weight' style='display:none;margin-left: 4px;margin-top: 5px;' min='0' onpaste='return false;'>";
                                        data += "                 <a href='javascript:void(0);' class='btn  " + muti_choice_que_type + "_remove_option que_" + current_que_seq + "' id='1' style='width:7%;display:none;margin-left:2px; margin-top:5px;'><i class='fa fa-times'></i></a>";
                                        data += "                 <a href='javascript:void(0);' class='btn  " + muti_choice_que_type + "_add_option que_" + current_que_seq + "' style='margin-left:2px; margin-top:5px;' id='1'><i class='fa fa-plus' ></i></a>";
                                        data += "               </div>";
                                        $(this).parents('.main-pane').find('#' + muti_choice_que_type + '_options_div_' + current_que_seq).html(data);
        ...
        if ((self.model && self.model.get('survey_type') == 'poll') || (self.context && self.context.attributes.IsPOll))
        {
            ...
            if (self.context.attributes.IsCreatePoll) {
                ...
                create_poll_html += '         <input type="number" name="score_radio-button" value="1" class="inherit-width score_weight" style="margin-left: 4px;margin-top: 5px;max-width:7%;display:none;" min="0" onpaste="return false;">     ';
                ...
                create_poll_html += '             <input type="text" name="option_radio-button"  placeholder="Option" class="inherit-width" style="margin-top:5px;max-width:50%;margin-left:1px;">   ';
                create_poll_html += '             <input type="number" name="score_radio-button" value="2" class="inherit-width score_weight" style="margin-left: 4px;margin-top: 5px;max-width:7%; display:none;" min="0" onpaste="return false;">       ';
    ...
    themeDetailView: function (survey_theme) {
        ...
        if (self.action == "edit") {
            ...
            if ($('[data-fieldname=survey_theme]').find('span').find('img').length == 0) {
                ...
                $('[data-fieldname=survey_theme]').find('span').append('<img style="width: 20%;display: inherit;" src="custom/include/survey-img/' + theme_file + '" class="SurveyTheme" />');
            } else if ($('[data-fieldname=survey_theme]').find('span').find('img').attr('src') != 'custom/include/survey-img/' + theme_file) {
                $('[data-name=survey_theme]').find('[data-fieldname=survey_theme]').find('span').text(themeLbl);
     ...
     */
    /* Changes done for Script type tags in deatil view 13th JUNE 2019*/
    surveypagesdetailView: function (editFlag, record_id, is_loading) {

        app.alert.show('loading_view', {level: 'process', title: 'Loading', autoclose: false});
        if (this.model)
        {
            var record_id = this.model.id;
        }
        ...
        if (editFlag == 1) {
            if ($('#data-page_1').find('.question').length == 0) {
                var url = App.api.buildURL("bc_survey", "get_survey", "", {record_id: record_id, type: type});
                App.api.call('GET', url, {}, {
                    success: function (data) {
                        var editAction = self.view.currentState;
                        if ((editAction == "edit") && self.record_id != null) {
                            if (data) {
                                ...
                                if (editFlag == 1) {
                                    ...
                                    self.themeDetailView();
                                    $('.Survey_Pages').attr('id', 'edit_view');
                                }
                            }
                        }
                    }
        ...
        } else if ($('#detail_view').find('#page_1').length == 0 && self.save_pending == false) {
            // call api to save record via php
            var url = App.api.buildURL("bc_survey", "get_survey", "", {record_id: record_id, type: type, editFlag: 1});
            App.api.call('GET', url, {}, {
                success: function (data) {
                    var editAction = self.view.currentState;
                    if ((editAction != "edit") && self.record_id != null) {
                        if (data) {
                            ...
                            $('.Survey_Pages').attr('id', 'detail_view');
                        }
                    }
                }
            });
        }
        app.alert.dismiss('loading_view');
    },
    /**
    ...
    edit_page_clicked: function (el) {
        ...
        if (this.individualEdit)
        {
            app.alert.show('error', {
                level: 'error',
                messages: 'You need to submit the current opened page before editing other page.',
                autoClose: true
            });
        } else {
            document.getElementById('new-page').ondragstart = function () {
            ...
            if (self.model && self.model.get('survey_type') == 'poll')
            {
                var sent_msg = 'This poll is active now and cannot be edited. Please create a new poll.';
            } else {
                var sent_msg = 'This survey is active now and cannot be edited. Please create a new survey.';
            }
            var url = App.api.buildURL("bc_survey", "isSurveySend", "", {record: self.record_id});
            App.api.call('GET', url, {}, {
                success: function (data) {
                    if (data && data['restrict_edit'] == "1") { // survey is send so disable all kind of edit of page
                        ...
                        self.restrictEdit = "false";
                        //Changes regarding edit survey when submission > 0 5th aygyst 2019 
                        if (data && data['submissionCount_edit'] == "1") {
                            self.submissionCount_edit = "true";
                        } else {
                            self.submissionCount_edit = "false";
    ...
    remove_page_clicked: function (el) {
        ...
        if (current_page_count > 1) {
            app.alert.show('remove_page', {
                level: 'confirmation',
                messages: "Are you sure to remove this page?",
                onConfirm: function () {
                    ...
                    if ($('.remove_closed').length == 1) {
                        $('.remove_closed').css('display', 'none');
                    }
                },
                onCancel: function () {
                },
    ...
    remove_icon_clicked: function (el) {
        var self = this;
        //fetch id of remove icon clicked 
        var val = el.currentTarget.childNodes['0'].id;
        if (!val) {
        ...
        if (val.search("remove_que_") >= 0) {
            ...
            if (self.$el.parents('.main-pane').find(que_id).prev().hasClass('question-section') && (!self.$el.parents('.main-pane').find(que_id).next().hasClass('question') || self.$el.parents('.main-pane').find(que_id).next().hasClass('question-section')))
            {
                app.alert.show('remove_question', {
                    level: 'confirmation',
                    messages: "Removing this question will also remove its section header. Are you sure to remove this question ?",
                    onConfirm: function () {
                ...
                app.alert.show('remove_question', {
                    level: 'confirmation',
                    messages: "Are you sure to remove this question?",
                    onConfirm: function () {
                        var id = 'que_' + val.split('_')[2];
                        var remove_que_id = self.$el.parents('.main-pane').find('.' + id).find('.que_id').val();
                        if (remove_que_id) {
                        ...
                        self.$el.parents('.main-pane').find("div" + que_id).remove(); // remove question
                    },
                    onCancel: function () {
                    },
                    autoClose: false
    ...
    addNewPageButton: function () {
        ...
        if (self.individualEdit)
        {
            app.alert.show('error', {
                level: 'error',
                messages: 'You must submit the current open page before adding new page.',
                autoClose: true
            });
        } else {
            self.page_counter++;
            ...
            $.each(self.$el.parents('.main-pane').find('.thumbnail_page'), function () {
                var id = this.id;
                if ($('#' + id).find('.disable_que').length != 0) {
                    $('#' + id).find('.remove_closed').css('display', 'none')
                }
            });
    ...
    makeDraggable: function () {
        ...
        $(".Survey_Pages").droppable({
            ...
            drop: function (e, ui) {
                ...
                var new_page_html = self.new_page_html();
                self.$el.parents('.main-pane').find('.SurveyPage').before(new_page_html);
                //hide Closed question option if disabled question is not equal to zero
                $.each(self.$el.parents('.main-pane').find('.thumbnail_page'), function () {
                    var id = this.id;
                    if ($('#' + id).find('.disable_que').length != 0) {
                        $('#' + id).find('.remove_closed').css('display', 'none')
    ...
    makePageDroppable: function () {
        ...
        $(".data-page").droppable
                ({
                    ...
                    drop: function (e, ui) {
                        ...
                        if (current_drop.includes("textbox") == true) {
                            ...
                            data += self.editviewTextBox(self.que_counter, questions, true);
                            data += "</div>";
                            data += "</div>";
                        }
                        ...
                        if (current_drop.includes("commentbox") == true) {
                            ...
                            data += self.editviewCommentBox(self.que_counter, questions, true);
                            data += "</div>";
                            data += "</div>";
                        }
                        if (current_drop.includes("richtextareabox") == true) {
                            ...
                            data += self.editviewRichTextBox(self.que_counter, questions, true);
                            data += "</div>";
                            data += "</div>";
                        }
                        // govind
                        if (current_drop.includes("boolean") == true) {
                            ...
                            data += self.editviewBoolean(self.que_counter, questions, true);
                            data += "</div>";
                            data += "</div>";
                        }
                        ...
                        if (multi_choice_que == true) {
                            ...
                            data += self.editviewMultiChoice(self.que_counter, questions, true);
                            data += "</div>";
                            data += "</div>";
                            multi_choice_que = '';
                        }
                        ...
                        if (current_drop.includes("contact-information") == true) {
                            ...
                            data += self.editviewContactInformation(self.que_counter, questions, true);
                            data += "</div>";
                            data += "</div>";
                        }
                        ...
                        if (current_drop.includes("rating-survey") == true) {
                            ...
                            data += self.editviewRating(self.que_counter, questions, true);
                            data += "</div>";
                            data += "</div>";
                        }
                        ...
                        if (current_drop.includes("date-time") == true) {
                            ...
                            data += self.editviewDatetime(self.que_counter, questions, true);
                            data += "</div>";
                            data += "</div>";
                        }
                        ...
                        if (current_drop.includes("survey-video") == true) {
                            ...
                            data += self.editviewVideo(self.que_counter, questions, true);
                            data += "</div>";
                            data += "</div>";
                        }
                        ...
                        if (current_drop.includes("survey-image") == true) {
                            ...
                            data += self.editviewImage(self.que_counter, questions, true);
                            data += "</div>";
                            data += "</div>";
                        }
                        ...
                        if (current_drop.includes("scale") == true) {
                            ...
                            data += self.editviewScale(self.que_counter, questions, true);
                            data += "</div>";
                            data += "</div>";
                        }
                        ...
                        if (current_drop.includes("netpromoterscore") == true) {
                            ...
                            data += self.editviewNetPromoterScore(self.que_counter, questions, true);
                            data += "</div>";
                            data += "</div>";
                        }
                        ...
                        if (current_drop.includes("emojis") == true) {
                            ...
                            data += self.editviewEmojis(self.que_counter, questions, true);
                            data += "</div>";
                            data += "</div>";
                        }
                        ...
                        if (current_drop.includes("matrix") == true) {
                            ...
                            data += self.editviewMatrix(self.que_counter, questions, true);
                            data += "</div>";
                            data += "</div>";
                            multi_choice_que = '';
                        }
                        ...
                        if (current_drop.includes("doc-attachment") == true) {
                            ...
                            data += self.editviewDocAttachment(self.que_counter, questions, true);
                            data += "</div>";
                            data += "</div>";
                        }
                ...
                if (typeof ui.item.prop('id') != 'undefined') {
                    var typeQueArr = ui.item.prop('id').split("_");
    ...
    show_piping_option: function (el) {
        ...
        if (self.$el.parents('.main-pane').find('[data-fieldname=enable_data_piping]').find('[type=checkbox]:checked').length != 0)
        {
            ...
            if (self.action == "detail") {
                ...
                if (self.submissionCount_edit == "true") {
                    var que_name = el.currentTarget.parentElement.parentElement.parentElement.parentElement.parentElement.classList[0];
                    self.$el.parents('.main-pane').find('.' + que_name).find('.piping_options').hide();
                    self.$el.parents('.main-pane').find('.' + que_name).find('.piping').removeClass('active');
                } else {
    ...
    new_page_html: function () {
        ...
        if ($('.remove_closed').length >= 1) {
            $('.remove_closed').css('display', 'inline-block');
        }
        if ($('.remove_closed').length >= 1) {
            new_page_html += "<div class = 'btn-group remove_closed' style='margin-top:5px;'><a class='btn remove_page' id='remove_page_" + self.page_counter + "'><i data-action = \"loading\" class = \"fa fa-times\"  track = \"click:dashletToolbarCog\" ></i>&nbsp; Remove </a>\n\
            ...
            new_page_html += "<div class = 'btn-group remove_closed' style='margin-top:5px;display:none;'><a class='btn remove_page' id='remove_page_" + self.page_counter + "'><i data-action = \"loading\" class = \"fa fa-times\"  track = \"click:dashletToolbarCog\" ></i>&nbsp; Remove </a>\n\
                          <a class='btn dropdown-toggle' data-toggle='dropdown'> <i class='fa fa-caret-down'></i></a>";
            new_page_html += '<ul class="dropdown-menu left setting-action" id="setting-action_' + self.page_counter + '">';
            new_page_html += '        <div>';
            //add page above the current page option
    ...
    showQuestionSectionClicked: function (el) {
        ...
        {
            if (self.submissionCount_edit == "true") {
                if (!$(el.currentTarget).parents('.question.survey-body').hasClass('que_disabled')) {
                    var survey_data = '';
                    ...
                    survey_data += '                    </div>';
                    survey_data += '                    <div class="span1"><div class="btn remove-section" style="margin-bottom:2px;"><i class="fa fa-times" title="Remove Section Header"></i></div></div>';
                    survey_data += '</div>';
                    $(el.currentTarget).parents('.question').before(survey_data);
                } else {
                    app.alert.show('question_after_section_not_exists', {
                        level: 'error',
                        messages: "After Section Header Block atleast one question should exists.",
                        autoClose: true
                    });
                }
            } else {
    ...
    removeSectionHeader: function (el) {
        app.alert.show('remove_options', {
            level: 'confirmation',
            messages: "Are you sure to remove this section ?",
            onConfirm: function () {
    ...
    add_option_clicked: function (el) {
        ...
        if (que_type == 'matrix') {
            ...
            self.addOption(que_id, que_type, op_count, self.submissionCount_edit, 1);
        } else {
            self.addOption(que_id, que_type, op_count, self.submissionCount_edit);
        }
    ...
    remove_option_clicked: function (el) {
        ...
        app.alert.show('remove_options', {
            level: 'confirmation',
            messages: "Are you sure to remove this " + remove_type + " ?",
            onConfirm: function () {
                ...
                if (que_type == 'matrix') {
                    ...
                    self.removeOption(que_id, que_type, op_count, self.submissionCount_edit, 1);
                } else {
                    self.removeOption(que_id, que_type, op_count, self.submissionCount_edit);
                }
                if (que_type == 'multiselectlist' || que_type == 'check-box')
                {
                    // Update Minimim answer limit selection
    ...
    showImageUploadInRadioQue: function (el) {
        ...
        // if IsImageOption is not checked then hide File Uploads
            ...
            $(advance_option_parent).find('.ver_ele').prop('checked', true);
     ...
     * @editmode if current dropping area is editview then append to that view
     */
    addOption: function (que_id, que_type, op_count, submissionCount_edit, editmode) {
        var self = this;
        //add more option to multi select list
        var option_html = '';
        //append remove button if only one option exists
        ...
        if (remove_button_flag == 'none') {
            if (self.$el.parents('.main-pane').find($('#' + que_type + '_options_div_' + que_id)).parent().parent().parent().find('.remove_que').length == 1) {
        ...
        {
            option_html += "<input type='number' name='score_" + que_type + "' value='" + new_weight + "' class='inherit-width score_weight' style='max-width:7%; margin-left:4px; margin-top:5px;' min='0' onpaste='return false;'>";
        } else {
            option_html += "<input type='number' name='score_" + que_type + "' value='" + new_weight + "' class='inherit-width score_weight' style='max-width:7%; margin-left:4px; margin-top:5px;display:none;' min='0' onpaste='return false;'>";
        }
        if (que_type != "dropdownlist") {
            option_html += '          <span class="ans-piping-section"><a class="ans-piping"><i class="fa fa-link" title="Answer Piping"></i></a></span>';
        }
     ...
     * @editmode if current dropping area is editview then remove to that view
     */
    removeOption: function (que_id, que_type, op_count, submissionCount_edit, editmode) {
        var self = this;
    ...
    edit_que_typeClicked: function (el) {
        ...
        if (sync_field && sync_field != 'Select Field')
        {
            var answer_detail = new Object();
            var answer_detail_for_skipLogic = new Object();
            var conf_msg = 'Editing question type will override your current data and sync field will reset. Are you sure that you want to proceed?';
        } else {
    ...
    removeQueDropdown: function (question_type, current_que_id) {
        ...
        if (question_type == "textbox") {
            ...
            survey_data += '<i class="fa fa-th" style="font-size:13px;">&nbsp; Matrix / Grid </i>&nbsp;<i class="fa fa-pencil queTypeChange" title="Change Question Type"></i>';
     ...
     */
    confirmEditQueType: function (current_que_type, current_que_id, changed_type, question, is_required, helptips, display_boolean_label, question_id, answer_detail, options_Arr, sync_module, sync_field, answer_key) {
        var self = this;
        var general_options = {};
        var new_que_html = '';
        ...
        if (changed_type == 'check-box' || changed_type == 'radio-button' || changed_type == 'multiselectlist' || changed_type == 'dropdownlist') {
            ...
            general_options = self.general_option_multichoice(current_que_id, changed_type, question_id, answer_detail, answer_key);
    ...
    /*
     * cloneQueClicked : Clone question cliked so if 1 page then clone to last index otherwise ask for in which page to copy this question
     */
    cloneQueClicked: function (el, selected_page) {
        var self = this;
        var page_count = this.$el.parents('.main-pane').find('.thumbnail_page').length;
        var current_page = $(el.currentTarget).parents('.thumbnail_page').attr('id').split("_")[1];
        var que_count = this.que_counter;
        // if Page are more than 1, then ask for which page you want to copy this question
        if (page_count > 1 && !selected_page) {
            if ($(el.currentTarget).parents('.que-close').find('.page_selector').length == 0) {
                var page_selector = '<select class="page_sel_clone" style="max-width: 100%;"><option value="NA">Select Page</option>';
                $.each(this.$el.parents('.main-pane').find('.thumbnail_page'), function () {
                    var page_seq = $(this).attr('id').split("_")[1];
                    var page_title = $(this).find('#txt_page_title').val();
                    page_selector += '<option value="' + page_seq + '">' + page_title + '</option>';
                });
                page_selector += "</select>";

                var html = '<div class="page_selector" style="max-width:200px;padding: 8px;border-radius: 4px;background-color: #E6E0E0;box-shadow: 1px 1px 10px #c5c5c5;border: 1px solid #d5d5d5;margin-top:-10px;float:right;"><label style="font-size: 11px;color: inherit;">Select page to clone</label>' + page_selector + '</div>';
                $(el.currentTarget).parents('.que-close').append(html);
            } else { // Already added so, toggle : remove page selector
                $(el.currentTarget).parents('.que-close').find('.page_selector').remove();
            }
        }
        // If total pages are 1, then add question to same page
        if (page_count <= 1 || selected_page) {
            if (!selected_page) {
                selected_page = 1;
            }
            var questions = {};
            // General Tab :-------------------------------------------------------------------
            questions['que_type'] = $(el.currentTarget).parents('.question').attr('id').split('_')[0];
            var que_seq = $(el.currentTarget).parents('.question').attr('id').split('_')[1];
            questions['que_title'] = $(el.currentTarget).parents('.question').find('[name=question_' + questions['que_type'] + '_' + que_seq + ']').val();
            if ($(el.currentTarget).parents('.question').find('[name=enable_scoring_' + questions['que_type'] + '_' + que_seq + ']:checked').length == 1) {
                questions['enable_scoring'] = 'Yes';
            } else {
                questions['enable_scoring'] = 'No';
            }
            if ($(el.currentTarget).parents('.question').find('[name=enable_otherOption_' + questions['que_type'] + '_' + que_seq + ']:checked').length == 1) {
                questions['enable_otherOption'] = 'Yes';
            } else {
                questions['enable_otherOption'] = 'No';
            }
            // Options----------------------------------------------------------------------
            var que_section = questions['que_type'] + '_' + que_seq;
            var options_div = this.$el.parents('.main-pane').find('#' + questions['que_type'] + '_options_div_' + que_seq);
            var answer_detail_main = new Object();
            var new_option_id = 0;
            options_div.each(function () {
                var opid = this.id;
                self.$el.parents('.main-pane').find('#' + opid).find(self.$el.parents('.main-pane').find('[name="option_' + questions['que_type'] + '"]')).each(function () {
                    var answer_detail = new Object();
                    if (this.value.trim() != '') {
                        answer_detail['option_' + new_option_id] = new Object();
                        answer_detail['option_' + new_option_id]['option'] = this.value.trim();
                        answer_detail_main[new_option_id] = new Object();
                        ...
                        answer_detail_main[new_option_id]['option_' + new_option_id] = answer_detail['option_' + new_option_id];
                        new_option_id++;
                    }
                    questions['answers'] = answer_detail_main;
                });
            ...
            if (otherOptionLabel) {
                answer_detail['option_' + new_option_id] = new Object();
                answer_detail['option_' + new_option_id]['option'] = otherOptionLabel;
                answer_detail['option_' + new_option_id]['answer_type'] = 'other';
                answer_detail['option_' + new_option_id]['weight'] = otherOptionScore;
                answer_detail_main = answer_detail;
                questions['other_option'] = answer_detail_main;
            }
            // Advance Tab :------------------------------------------------------------------------
            questions['question_help_comment'] = $(el.currentTarget).parents('.question').find('[name=helptips_' + questions['que_type'] + '_' + que_seq + ']').val();
            if ($(el.currentTarget).parents('.question').find('[name=is_required_' + questions['que_type'] + '_' + que_seq + ']:checked').length == 1) {
                questions['is_required'] = 'Yes';
            } else {
                questions['is_required'] = 'No';
            }
            if ($(el.currentTarget).parents('.question').find('[name=is_question_seperator_' + questions['que_type'] + '_' + que_seq + ']:checked').length == 1) {
                questions['is_question_seperator'] = 'Yes';
            } else {
                questions['is_question_seperator'] = 'No';
            }
            // MultiChoice Questions
            if (questions['que_type'] == 'check-box' || questions['que_type'] == 'radio-button' || questions['que_type'] == 'dropdownlist' || questions['que_type'] == 'multiselectlist') {
                if ($(el.currentTarget).parents('.question').find('[name=is_sort_' + questions['que_type'] + '_' + que_seq + ']:checked').length == 1) {
                    questions['is_sort'] = 'Yes';
                } else {
                    questions['is_sort'] = 'No';
                }
                questions['limit_min'] = $(el.currentTarget).parents('.question').find('[name=limit_min_' + questions['que_type'] + '_' + que_seq + ']').val();
                questions['advance_type'] = $(el.currentTarget).parents('.question').find('[name=display_' + questions['que_type'] + '_' + que_seq + ']:checked').val();
            }
            // Boolean
            else if (questions['que_type'] == 'boolean') {
                questions['display_boolean_label'] = $(el.currentTarget).parents('.question').find('[name=display_label_' + questions['que_type'] + '_' + que_seq + ']').val();
            }
            // Matrix
            else if (questions['que_type'] == 'matrix') {
                // Matrix Rows---------------------------------------------------------------------
                var rows_div = this.$el.parents('.main-pane').find('#' + questions['que_type'] + '_row_div_' + que_seq);
                var rows_main = new Object();
                var new_option_id = 0;
                questions['rows'] = new Object();
                rows_div.each(function () {
                    var opid = this.id;
                    self.$el.parents('.main-pane').find('#' + opid).find(self.$el.parents('.main-pane').find('[name="row_' + questions['que_type'] + '"]')).each(function () {
                        var answer_detail = new Object();
                        if (this.value.trim() != '') {
                            new_option_id++;

                            rows_main[new_option_id] = this.value.trim();
                        }
                    });

                });
                questions['matrix_row'] = JSON.stringify(rows_main);
                // Matrix Columns------------------------------------------------------------------------
                var cols_div = this.$el.parents('.main-pane').find('#' + questions['que_type'] + '_column_div_' + que_seq);
                var cols_main = new Object();
                var new_option_id = 0;
                questions['matrix_col'] = new Object();
                cols_div.each(function () {
                    var opid = this.id;
                    self.$el.parents('.main-pane').find('#' + opid).find(self.$el.parents('.main-pane').find('[name="column_' + questions['que_type'] + '"]')).each(function () {
                        var answer_detail = new Object();
                        if (this.value.trim() != '') {
                            new_option_id++;

                            cols_main[new_option_id] = this.value.trim();
                        }
                    });

                });
                questions['matrix_col'] = JSON.stringify(cols_main);
                questions['advance_type'] = $(el.currentTarget).parents('.question').find('[name=display_type_' + questions['que_type'] + '_' + que_seq + ']:checked').val();
            }
            // NPS
            else if (questions['que_type'] == 'netpromoterscore') {
                var left = $(el.currentTarget).parents('.question').find('[name=left_' + questions['que_type'] + '_' + que_seq + ']').val();
                var right = $(el.currentTarget).parents('.question').find('[name=right_' + questions['que_type'] + '_' + que_seq + ']').val();
                questions['advance_type'] = left + '-' + right;
            }
            // TextBox
            else if (questions['que_type'] == 'textbox') {
                questions['advance_type'] = $(el.currentTarget).parents('.question').find('[name=datatype_' + questions['que_type'] + '_' + que_seq + ']').val();
                questions['maxsize'] = $(el.currentTarget).parents('.question').find('[name=size_' + questions['que_type'] + '_' + que_seq + ']').val();
                questions['min'] = $(el.currentTarget).parents('.question').find('[name=min_' + questions['que_type'] + '_' + que_seq + ']').val();
                questions['max'] = $(el.currentTarget).parents('.question').find('[name=max_' + questions['que_type'] + '_' + que_seq + ']').val();
                questions['precision'] = $(el.currentTarget).parents('.question').find('[name=precision_' + questions['que_type'] + '_' + que_seq + ']').val();

            }
            // Commentbox
            else if (questions['que_type'] == 'commentbox') {
                questions['maxsize'] = $(el.currentTarget).parents('.question').find('[name=size_' + questions['que_type'] + '_' + que_seq + ']').val();
                questions['min'] = $(el.currentTarget).parents('.question').find('[name=rows_' + questions['que_type'] + '_' + que_seq + ']').val();
                questions['max'] = $(el.currentTarget).parents('.question').find('[name=cols_' + questions['que_type'] + '_' + que_seq + ']').val();
            }
            // Scale
            else if (questions['que_type'] == 'scale') {
                var left = $(el.currentTarget).parents('.question').find('[name=left_' + questions['que_type'] + '_' + que_seq + ']').val();
                var middle = $(el.currentTarget).parents('.question').find('[name=middle_' + questions['que_type'] + '_' + que_seq + ']').val();
                var right = $(el.currentTarget).parents('.question').find('[name=right_' + questions['que_type'] + '_' + que_seq + ']').val();
                questions['advance_type'] = left + '-' + middle + '-' + right;
                questions['scale_slot'] = $(el.currentTarget).parents('.question').find('[name=step_' + questions['que_type'] + '_' + que_seq + ']').val();
                questions['min'] = $(el.currentTarget).parents('.question').find('[name=start_' + questions['que_type'] + '_' + que_seq + ']').val();
                questions['max'] = $(el.currentTarget).parents('.question').find('[name=end_' + questions['que_type'] + '_' + que_seq + ']').val();
            }
            // Rating
            else if (questions['que_type'] == 'rating') {
                questions['maxsize'] = $(el.currentTarget).parents('.question').find('[name=starNo_' + questions['que_type'] + '_' + que_seq + ']').val();
            }
            // Attachment
            else if (questions['que_type'] == 'doc-attachment') {
                questions['file_size'] = $(el.currentTarget).parents('.question').find('[name=file_size_' + questions['que_type'] + '_' + que_seq + ']').val();
                questions['file_extension'] = ($(el.currentTarget).parents('.question').find('[name=file_extension_' + questions['que_type'] + '_' + que_seq + ']').val()).toString();
            }
            // Date time
            else if (questions['que_type'] == 'date-time') {
                if ($(el.currentTarget).parents('.question').find('[name=is_datetime_' + questions['que_type'] + '_' + que_seq + ']:checked').length == 1) {
                    questions['is_datetime'] = 'Yes';
                } else {
                    questions['is_datetime'] = 'No';
                }
                if ($(el.currentTarget).parents('.question').find('[name=allow_future_dates_' + questions['que_type'] + '_' + que_seq + ']:checked').length == 1) {
                    questions['allow_future_dates'] = 'Yes';
                } else {
                    questions['allow_future_dates'] = 'No';
                }
                questions['min'] = $(el.currentTarget).parents('.question').find('[name=startDate_' + questions['que_type'] + '_' + que_seq + ']').val();
                questions['max'] = $(el.currentTarget).parents('.question').find('[name=endDate_' + questions['que_type'] + '_' + que_seq + ']').val();
            }
            // Contact Information
            else if (questions['que_type'] == 'contact-information') {

                var selectedCheckbox = $(el.currentTarget).parents('.question').find('.requiredFields').find('input:checked').length;
                var i = 0;
                var selectedValues = new Array();
                for (i = 0; i < selectedCheckbox; i++) {
                    var input = $(el.currentTarget).parents('.question').find('.requiredFields').find('input:checked')[i];

                    selectedValues[i] = $(input).attr('name').split("_")[0];
                }
                var selectedValuesString = selectedValues.toString();
                questions['advance_type'] = selectedValuesString.split(',').join(" ");
            }


            this.editviewQuestion_data(questions, selected_page, que_count, true, true);
            this.que_counter++;
            app.alert.show('success_que_cloned', {
                level: 'success',
                messages: 'Question copied successfully.',
                autoClose: true
            });
            if ($(el.currentTarget).parents('.que-close').find('.page_selector').length == 1) {
                $(el.currentTarget).parents('.que-close').find('.page_selector').remove();
            }
        }

    },
    clonePageClicked: function (el) {
        var page_seq = $(el.currentTarget).val();
        if (page_seq != 'NA') {
            this.cloneQueClicked(el, page_seq);
        }
    },
    /*
     * open popup to select question for Question Piping FROM
     * 
     * @param {type} el
     * @returns {String}
     */
    quePipingClicked: function (el) {
        if ($(el.currentTarget).hasClass('que-piping') == true) {
            var is_que_piping = true;
            var selector = 'que_pipe_selector';
        } else {
            $(el.currentTarget).parents('.general_options').find('.ans_pipe_selector').hide();
            var is_que_piping = false;
            var selector = 'ans_pipe_selector';
        }
        var que_seq = $(el.currentTarget).parents('.question').attr('id').split('_')[1];
        var current_page_seq = $(el.currentTarget).parents('.thumbnail_page').attr('id').split("_")[1];
        // If not added que selector then so que selector for que piping
        if ($(el.currentTarget).parent().find('.' + selector).length == 0) {
            var que_arrived = false;
            var is_above_que = false;
            var que_selector = '<select class="que_sel_qpipe" style="max-width: 100%;"><option value="NA">Select Question</option>';
            $.each(this.$el.parents('.main-pane').find('.thumbnail_page'), function () {
                var page_seq = $(this).attr('id').split("_")[1];
                $.each($(this).find('.question'), function () {
                    var que_type_inloop = $(this).attr('id').split('_')[0];
                    var que_seq_inloop = $(this).attr('id').split('_')[1];
                    var que_section = que_type_inloop + '_' + que_seq_inloop;
                    if (que_type_inloop == 'textbox' || que_type_inloop == 'commentbox' || que_type_inloop == 'radio-button' || que_type_inloop == 'dropdownlist' || que_type_inloop == 'scale' || que_type_inloop == 'rating' || que_type_inloop == 'netpromoterscore' || que_type_inloop == 'emojis') {
                        if ($('#' + que_section).find('.piping_sequence').length != 0) {
                            var piping_sequence = $('#' + que_section).find('.piping_sequence').val();
                        } else {
                            var piping_sequence = que_seq_inloop;
                        }
                        // Add Questions in dropdown
                        if (que_arrived == false && que_seq != que_seq_inloop) {
                            is_above_que = true;

                            var que_title = $(this).find("[name=question_" + que_type_inloop + "_" + que_seq_inloop + "]").val();
                            que_selector += '<option value="' + piping_sequence + '">' + que_title + '</option>';
                        }
                        // If Question arrived in loop then stop adding below questions
                        else {
                            que_arrived = true;
                        }
                    }
                    // If Question arrived in loop then stop adding below questions
                    if (que_seq == que_seq_inloop) {
                        que_arrived = true;
                    }
                });
            });
            que_selector += "</select>";
            var que_type = $(el.currentTarget).parents('.question').attr('id').split('_')[0];
            // If above question exists then show popup
            if (is_above_que) {
                if (is_que_piping) {
                    var html = '<div class="que_pipe_selector" style="position:absolute;max-width:200px;padding: 8px;border-radius: 4px;background-color: #E6E0E0;box-shadow: 1px 1px 10px #c5c5c5;border: 1px solid #d5d5d5;margin-top:9px;margin-left:-190px;z-index: 9;"><label style="font-size: 11px;color: inherit;">Select question to pipe :</label>' + que_selector + '</div>';
                } else if (!is_que_piping && que_type == 'emojis') {
                    var html = '<div class="ans_pipe_selector" style="position:absolute;max-width:200px;padding: 8px;border-radius: 4px;background-color: #E6E0E0;box-shadow: 1px 1px 10px #c5c5c5;border: 1px solid #d5d5d5;margin-top: -20px;margin-left: 252px;z-index: 9;"><label style="font-size: 11px;color: inherit;">Select question to pipe :</label>' + que_selector + '</div>';
                } else {
                    var html = '<div class="ans_pipe_selector" style="position:absolute;max-width:200px;padding: 8px;border-radius: 4px;background-color: #E6E0E0;box-shadow: 1px 1px 10px #c5c5c5;border: 1px solid #d5d5d5;margin-top: -20px;margin-left: 415px;z-index: 9;"><label style="font-size: 11px;color: inherit;">Select question to pipe :</label>' + que_selector + '</div>';
                }
                $(el.currentTarget).parent().append(html);
            }
            // If no above question then show alert
            else {
                app.alert.show('no_above_que_pipe', {
                    level: 'info',
                    messages: 'No Question exists to pipe.',
                    autoClose: true
                });
            }
        } else { // Already added so, toggle : remove que pipe selector
            if ($(el.currentTarget).hasClass('que-piping') == true) {
                $(el.currentTarget).parent().find('.que_pipe_selector').remove();
            } else {
                $(el.currentTarget).parent().find('.ans_pipe_selector').remove();
            }
        }

    },
    /*
     * Question selected for question piping
     * 
     * @param {type} el
     * @returns {undefined}
     */
    qpipeSelected: function (el) {

        if ($(el.currentTarget).parent().hasClass('que_pipe_selector') == true) {
            var is_que_piping = true;
        } else {
            var is_que_piping = false;
        }
        var que_piping_seq = $(el.currentTarget).val();
        var que_seq = $(el.currentTarget).parents('.question').attr('id').split('_')[1];
        var que_type = $(el.currentTarget).parents('.question').attr('id').split('_')[0];
        if (que_piping_seq && que_piping_seq != 'NA') {
            if (is_que_piping == true) {
                // Add Question Piping Tag
                var current_title = $(el.currentTarget).parents('.question').find("[name=question_" + que_type + "_" + que_seq + "]").val();
                var new_title = current_title + ' {{Q' + que_piping_seq + '}}';
                $(el.currentTarget).parents('.question').find("[name=question_" + que_type + "_" + que_seq + "]").val(new_title);
                // Remove selector
                $(el.currentTarget).parent().remove();


                app.alert.show('suc_qpipe_added', {
                    level: 'success',
                    messages: 'Question Piping added successfully.',
                    autoClose: true
                });
            } else {
                // Add Answer Piping Tag
                var que_type = $(el.currentTarget).parents('.question').attr('id').split('_')[0];
                if (que_type == 'radio-button' || que_type == 'dropdownlist' || que_type == 'check-box' || que_type == 'multiselectlist' || que_type == 'emojis')
                    var current_answer = $(el.currentTarget).parents('.options').find("input[type=text]").val();
                var new_title = current_answer + ' {{Q' + que_piping_seq + '}}';
                $(el.currentTarget).parents('.options').find("input[type=text]").val(new_title);
                // Remove selector
                $(el.currentTarget).parent().remove();
                ...
                app.alert.show('suc_qpipe_added', {
                    level: 'success',
                    messages: 'Answer Piping added successfully.',
                    autoClose: true
                });
            }
        }
...
//            if (this.submissionCount_edit == "true") {
//                data += '          <div class="dashlet-tab piping" style="pointer-events: none;">Piping</div>';
//            } else {
...
//            }
        ...
        data += ' <a class="btn cloneQue" href="javascript:void(0);" title="Clone Question" id="clone_que_' + que_counter + '">';
        data += '    <i class="fa  fa-copy" style="font-size:12px;font-weight:bold">&nbsp;</i></a>';
        data += ' <a class="btn showQuestionSection" href="javascript:void(0);" title="Add Section Header" id="add_section_header_' + que_counter + '">';
        data += '    <i class="fa fa-bars" style="font-size:12px;">&nbsp;</i></a>';
        ...
        data += "        <div class='question_title_div'>";
        ...
        data += '          <span class="que-piping-section"><a class="que-piping"><i class="fa fa-link" title="Question Piping"></i></a></span>';
        data += "        </div>";
        data += "      </div>";
        data += "</div>";
        data += "</div>";
        ...
        if (typeof answer_detail == 'object' && Object.keys(answer_detail).length != 0) {
            $.each(answer_detail, function (k, value) {
                ...
                if (typeof value != "undefined")
                {
                    ...
                    data += "                 <input type='number' name='score_" + muti_choice_que_type + "' value='" + op_count + "' id='score_" + k + "' class='inherit-width score_weight' style='max-width:7%; margin-left: 4px;margin-top: 5px;display:none;' min='0' onpaste='return false;'>";
            ...
            data += "<input type='number' name='score_boolean'  value='1' class='inherit-width score_weight' style='margin-left: 4px;margin-top: 5px;max-width:7%; display:none;' min='0' onpaste='return false;'>";
            data += "</div>";
            data += "<div class='options' id='option_1'>";
            ...
            data += "<input type='number' name='score_boolean' value='2' class='inherit-width score_weight' style='margin-left: 4px;margin-top: 5px;max-width:7%; display:none;' min='0' onpaste='return false;'>";
            data += "</div>";
        }
        data += "</div>";
        data += "</div>";
...
//            }
    ...
    general_option_emojis: function (que_counter, question_id) {
        ...
        data += "                 <input type='number' name='score_emojis'  value='1' class='inherit-width score_weight' style='max-width:7%; display:none;' min='0' onpaste='return false;'>";
        data += '                 <span class="ans-piping-section"><a class="ans-piping"><i class="fa fa-link" title="Answer Piping"></i></a></span>';
        data += "               </div></div>";
        ...
        data += '                 <span class="ans-piping-section"><a class="ans-piping"><i class="fa fa-link" title="Answer Piping"></i></a></span>';
        data += "                 <input type='number' name='score_emojis' value='2' class='inherit-width score_weight' style='max-width:7%; margin-top:5px;display:none;' min='0' onpaste='return false;'>";
        data += "               </div></div>";
        ...
        data += "                 <input type='number' name='score_emojis' value='3' class='inherit-width score_weight' style='max-width:7%; margin-top:5px;display:none;' min='0' onpaste='return false;'>";
        data += '                 <span class="ans-piping-section"><a class="ans-piping"><i class="fa fa-link" title="Answer Piping"></i></a></span>';
        data += "               </div></div>";
        ...
        data += "                 <input type='number' name='score_emojis' value='4' class='inherit-width score_weight' style='max-width:7%; margin-top:5px;display:none;' min='0' onpaste='return false;'>";
        data += '                 <span class="ans-piping-section"><a class="ans-piping"><i class="fa fa-link" title="Answer Piping"></i></a></span>';
        data += "               </div></div>";
        ...
        data += "                 <input type='number' name='score_emojis' value='5' class='inherit-width score_weight' style='max-width:7%; margin-top:5px;display:none;' min='0' onpaste='return false;'>";
        data += '                 <span class="ans-piping-section"><a class="ans-piping"><i class="fa fa-link" title="Answer Piping"></i></a></span>';
        data += "               </div></div>";
...
//            }
        ...
        data += "<div class='general_options'>";
        data += "<div class='row'>";
        data += "       <div class='span1'>Type</div><div class='span5 '><i class='fa fa-th' style='font-size:13px;'>&nbsp; Matrix / Grid </i>&nbsp;<i class='fa fa-pencil queTypeChange'></i></div>";
        data += "</div>";
        //question  display
        data += "<div class='row'>";
        data += "       <div class='span1'>Question</div>";
     ...
     */
    general_option_multichoice: function (que_counter, muti_choice_que_type, question_id, answer_detail, answer_key) {
        var que_icon = '';
...
//            }
        ...
        if (typeof answer_detail == 'object' && Object.keys(answer_detail).length != 0) {
            ...
            $.each(answer_detail, function (k, value) {
                if (typeof value != "undefined")
                {
                    ...
                    data += "                 <input type='number' name='score_" + muti_choice_que_type + "' value='" + op_count + "' id='score_" + k + "' class='inherit-width score_weight' style='max-width:7%; margin-left: 4px;margin-top: 5px;display:none;' min='0' onpaste='return false;'>";
                    data += "                 <input type='hidden' name='piping_values_" + muti_choice_que_type + "' value='" + answer_key[k] + "' id='piping_values_" + k + "' class='inherit-width piping_values'>";
                    if (muti_choice_que_type != "dropdownlist") {
                        data += '                 <span class="ans-piping-section"><a class="ans-piping"><i class="fa fa-link" title="Answer Piping"></i></a></span>';
                    }
                    data += "                 <a href='javascript:void(0);' class='btn  " + muti_choice_que_type + "_remove_option que_" + que_counter + "' id='" + op_count + "' style='margin-left:2px; margin-top:5px;'><i class='fa fa-times'></i></a>";
            ...
            if (op_count == 1) {
                ...
                data += "                 <input type='text' name='option_" + muti_choice_que_type + "' placeholder='Option' class='inherit-width' style='max-width:50%;margin-top:5px;'>";
                data += "                 <input type='number' name='score_" + muti_choice_que_type + "'  value='2' class='inherit-width score_weight' style='max-width:7%; margin-left: 4px;margin-top: 5px;display:none;' min='0' onpaste='return false;'>";
                if (muti_choice_que_type != "dropdownlist") {
                    data += '                 <span class="ans-piping-section"><a class="ans-piping"><i class="fa fa-link" title="Answer Piping"></i></a></span>';
                }
        ...
        //If no option then show default two option input
            ...
            data += "                 <input type='text' name='option_" + muti_choice_que_type + "' placeholder='Option' class='inherit-width' style='max-width:50%;'>";
            data += "                 <input type='number' name='score_" + muti_choice_que_type + "'  value='1' class='inherit-width score_weight' style='margin-left: 4px;margin-top: 5px;max-width:7%; display:none;' min='0' onpaste='return false;'>";
            if (muti_choice_que_type != "dropdownlist") {
                data += '                 <span class="ans-piping-section"><a class="ans-piping"><i class="fa fa-link" title="Answer Piping"></i></a></span>';
            }
            ...
            data += "                 <input type='text' name='option_" + muti_choice_que_type + "' placeholder='Option' class='inherit-width' style='max-width:50%;margin-top:5px;'>";
            data += "                 <input type='number' name='score_" + muti_choice_que_type + "' value='2' class='inherit-width score_weight' style='margin-left: 4px;max-width:7%; margin-top:5px;display:none;' min='0' onpaste='return false;'>";
            if (muti_choice_que_type != "dropdownlist") {
                data += '                 <span class="ans-piping-section"><a class="ans-piping"><i class="fa fa-link" title="Answer Piping"></i></a></span>';
            }
        ...
        data += "      <input type='number' value='0' style='width:7%;margin-left: 1px;display:none;' Placeholder='score' name='option_score_" + muti_choice_que_type + "_" + que_counter + "' class='inherit-width other_weight' min='0' onpaste='return false;'/>";
        data += "      </div>";
        data += "</div>";
        data += "</div>"; //general option complete
    ...
    detailviewQuestion_data: function (questions, page_sequence, question_sequence, enable_data_piping) {
        var self = this;
        var survey_data = '';
        if ($('[value="' + questions['que_id'] + '"]').length == 0) {
            if (questions['que_type'] == 'section-header')
            {
                ...
                survey_data += '  <div class="row">';
                survey_data += '       <div class="span11" style="display:inline-block"><b style="font-size: 14px;">Section Title : </b><span style="font-size:14px;">';
                survey_data += questions['que_title'];
                survey_data += '       </span></div>';
                if (questions['disabled_question'] == "Yes") {
                    survey_data += '<a rel="tooltip" title="" data-original-title="Disabled Section Header" rel="tooltip" style="display:inline-block;" disabled><i class="fa fa-lock" style="color:red;"></i></a>';
                }
                survey_data += '  </div>';
                survey_data += '</div>';
            } else {
                ...
                //Changes regarding survey detail view same as survey edit view
                if (question_type == "check-box" || question_type == "radio-button" || question_type == "multiselectlist" || question_type == "dropdownlist" || question_type == "boolean" || question_type == "emojis") {
                    survey_data += '<div class="row">';
                    ...
                    survey_data += '          <div class="dashlet-tab active general"><a style="margin-top: 30px;background:transparent !important;">General</a></div>';
                    survey_data += '          <div class="dashlet-tab advance"><a style="margin-top: 30px;background:transparent !important;">Advanced Option</a></div>';
                    survey_data += '          <div class="dashlet-tab change_type_skip_logic"><i class="fa">&nbsp;</i><a style="margin-top: 15px;background:transparent !important;">Logic</a></div>';
                    if (question_type != "emojis") {
                        if (this.submissionCount_edit == "true") {
                            survey_data += '          <div class="dashlet-tab piping"><div style="margin-top: 30px;background:transparent !important;">Piping</div></div>'
                        } else {
                            survey_data += '          <div class="dashlet-tab piping"><a style="margin-top: 30px;background:transparent !important;">Piping</a></div>'
                        }
                    }
                    survey_data += '      </div>';
                    survey_data += '     </div>';
                    survey_data += '    </div>';
                    survey_data += '  </div>';
                    //Advance Option in Detail view
                    survey_data += "<div class='advance_options' style='display:none;'>";
                    //help tip html
                    survey_data += "<div class='row' >";
                    survey_data += "      <div class='span1' style='margin-top:10px;'>Help Tip </div>";
                    if (questions['question_help_comment'] != "") {
                        survey_data += "<div class='span11' style='margin-top:10px;'><input type='text' name='helptips_commentbox_" + question_type + "' value='" + questions['question_help_comment'] + "' placeholder='Enter help tip for question' class='inherit-width' style='max-width:80%;margin-left: 1px;' disabled></div>";
                    } else {
                        survey_data += "<div class='span11' style='margin-top:10px;'><input type='text' name='helptips_commentbox_" + question_type + "' placeholder='Enter help tip for question' class='inherit-width' style='max-width:80%;margin-left: 1px;' disabled></div>";
                    }
                    survey_data += "</div>";
                    ...
                    if (question_type == "boolean") {
                        ...
                        if (display_boolean_label != "") {
                            survey_data += "      <div class='span11'><input type='text' name='display_label_" + questions['que_type'] + "_" + question_sequence + "'  placeholder='' value='" + display_boolean_label + "' style='max-width:80%; margin-left:1px;' class='inherit-width' disabled/></div>";
                        } else {
                            survey_data += "      <div class='span11'><input type='text' name='display_label_" + questions['que_type'] + "_" + question_sequence + "'  placeholder='' style='max-width:80%; margin-left:1px;' class='inherit-width' disabled/></div>";
                        }
                    ...
                    survey_data += '<div class="row">';
                    //is_required
                    survey_data += "      <div class='span1'>Is Required </div>";
                    if (questions['is_required'] == 'Yes') {
                        survey_data += "      <div class='span2'><input type='checkbox' style='margin:2px 7px 7px 7px' name='is_required_" + questions['que_type'] + "_" + question_sequence + "' checked='checked' class='inherit-width' disabled/></div>";
                    } else {
                        survey_data += "      <div class='span2'><input type='checkbox' style='margin:2px 7px 7px 7px' name='is_required_" + questions['que_type'] + "_" + question_sequence + "' class='inherit-width' disabled/></div>";
                    }
                    if (question_type != "boolean" && question_type != "emojis") {
                        //sorting
                        survey_data += "      <div class='span1'>Sortable </div>";
                        if (questions['is_sort'] == 'Yes') {
                            survey_data += "      <div class='span2'><input type='checkbox' style='margin:2px 7px 7px 7px' name='is_sort_" + questions['que_type'] + "_" + question_sequence + "' checked='checked' class='inherit-width' disabled/></div>";
                        } else {
                            survey_data += "      <div class='span2'><input type='checkbox' style='margin:2px 7px 7px 7px' name='is_sort_" + questions['que_type'] + "_" + question_sequence + "' class='inherit-width' disabled/></div>";
                        }
                    }
                    //Add Question Seperator
                    survey_data += "      <div class='span2'>Add Question Separator?</div>";
                    if (questions['is_question_seperator'] == 'Yes') {
                        survey_data += "      <div class='span2'><input type='checkbox' style='margin:2px 7px 7px 7px' name='is_question_seperator_" + questions['que_type'] + "_" + question_sequence + "' checked='checked' class='inherit-width' disabled/></div>";
                    } else {
                        survey_data += "      <div class='span2'><input type='checkbox' style='margin:2px 7px 7px 7px' name='is_question_seperator_" + questions['que_type'] + "_" + question_sequence + "' class='inherit-width' disabled/></div>";
                    }
                    survey_data += "</div>";

                    if (question_type == "check-box" || question_type == "multiselectlist") {
                        survey_data += '<div class="row">';
                    ...
                    if (question_type == "check-box" || question_type == "radio-button" || question_type == "multiselectlist" || question_type == "dropdownlist") {
                        //display
                        survey_data += "<div class='row'>";
                        ...
                        if (questions['advance_type'] == 'Vertical' || questions['advance_type'] == '' || questions['advance_type'] == null) {
                            survey_data += "          <input type='radio' style='margin-right: 0px; " + showVertical + "' value='Vertical' name='display_" + questions['que_type'] + "_" + question_sequence + "'  class='inherit-width ver_ele' checked disabled><a href='javascript:void(0);' class='btn ver_ele' style='margin-bottom-6px;margin-left-6px;" + showVertical + "'><i class='fa fa-ellipsis-v ver_ele' style='" + showVertical + "'></i>&nbsp;Vertical&nbsp;&nbsp;</a>";
                        } else {
                            survey_data += "          <input type='radio' style='margin-right: 0px; " + showVertical + "' value='Vertical' name='display_" + questions['que_type'] + "_" + question_sequence + "'  class='inherit-width ver_ele' disabled><a href='javascript:void(0);' class='btn ver_ele' style='margin-bottom-6px;margin-left-6px;" + showVertical + "'><i class='fa fa-ellipsis-v ver_ele' style='" + showVertical + "'></i>&nbsp;&nbsp;Vertical&nbsp;&nbsp;</a>";
                        }
                        if (questions['advance_type'] == 'Horizontal') {
                            survey_data += "          <input type='radio' style='margin-right: 0px;' value='Horizontal' name='display_" + questions['que_type'] + "_" + question_sequence + "'  class='inherit-width hoz_ele' checked disabled><a href='javascript:void(0);' class='btn hoz_ele' style='margin-bottom-6px;margin-left-6px;'><i class='fa fa-ellipsis-h hoz_ele'></i>&nbsp;Horizontal</a>";
                        } else {
                            survey_data += "          <input type='radio' style='margin-right: 0px;' value='Horizontal' name='display_" + questions['que_type'] + "_" + question_sequence + "'  class='inherit-width hoz_ele' disabled><a href='javascript:void(0);' class='btn hoz_ele' style='margin-bottom-6px;margin-left-6px;'><i class='fa fa-ellipsis-h hoz_ele'></i>&nbsp;Horizontal</a>";
                        }
                        survey_data += "       </div>";
                    }
                    if (questions['que_type'] == 'radio-button' || questions['que_type'] == 'check-box') {
                        ...
                        if (questions['show_option_text'] == 'Yes') {
                            survey_data += "      <input type='checkbox' style='margin:2px 7px 7px 7px' name='show_option_text_" + questions['que_type'] + "_" + question_sequence + "' checked='checked' class='inherit-width' disabled/>";
                        } else {
                            survey_data += "      <input type='checkbox' style='margin:2px 7px 7px 7px' name='show_option_text_" + questions['que_type'] + "_" + question_sequence + "' class='inherit-width' disabled/>";
                        }
                        survey_data += "</div>";
                    }
                    if (question_type == "check-box" || question_type == "radio-button" || question_type == "multiselectlist" || question_type == "dropdownlist") {
                    ...
                    //Logic Option in Detail view
                    var answerDetail = new Array();
                    survey_data += "<div class='skip_logic_options' style='display:none;'>";
                    survey_data += "<input type='hidden' id='page_sequence_count' value='" + page_sequence + "' >";
                    survey_data += "<div class='row'>";
                    survey_data += "<table id='logicRows_table' class='logic-data-table' cellspacing='5' cellpadding='5'>";
                    survey_data += "<thead>";
                    survey_data += "<tr><th>If answer is ...&nbsp;<i class='fa fa-info-circle' title='If selected answer is from below answer' style='font-size:14px;'></i></th><th>Then skip to ...&nbsp;<i class='fa fa-info-circle'  title='If answer is selected then perform logic as below' style='font-size:14px;'></i></th><th><span class='clear_skip_logic_All'>Logic Applied</span></th></tr>";
                    survey_data += "</thead>";
                    survey_data += "<tbody>";
                    $.each(questions['answers'], function (aid, answer) {
                        $.each(answer, function (ans_id, ans_data) {
                            if (questions['que_type'] == 'boolean') {
                                if (ans_data['option'] == "Yes") {
                                    answerDetail[ans_id] = ans_data;
                                }
                            } else {
                                answerDetail[ans_id] = ans_data;
                            }
                    ...
                    $.each(questions['skip_logic'], function (answer_id, ans) {
                        if (typeof answerDetail[answer_id] != "undefined") {
                            survey_data += " <tr class='logicRow ' id='logicRow_" + answer_id + "'>";
                            survey_data += "       <td class='option_value logic-table-first-column' id='ans_option_" + answer_id + "'>" + answerDetail[answer_id]['option'] + "</td>";
                            survey_data += "       <td class='logic_actions_td'><select id='logic_actions_" + answer_id + "' class='logic_actions' name='logic_actions' >";
                            if (ans['logic_action'] == "redirect_to_page") {
                                survey_data += "              <option value='no_logic' >No Logic</option>";
                                survey_data += "              <option value='redirect_to_page' selected>Redirect To Page</option>";
                                survey_data += "              <option value='eop' >End of Survey</option>";
                                survey_data += "              <option value='redirect_to_url' >Redirect To URL</option>";
                                survey_data += "             <option value='show_hide_question' >Show/Hide Questions</option>";
                                survey_data += "         </select></td>";
                                survey_data += "  <td class='clearTd'><span style='cursor: pointer;' class='clear_skip_logic'>" + ans['logic_target_name'] + "</span></td>";
                                survey_data += "  </tr>";
                            } else if (ans['logic_action'] == "eop") {
                                survey_data += "              <option value='no_logic' >No Logic</option>";
                                survey_data += "              <option value='redirect_to_page'>Redirect To Page</option>";
                                survey_data += "              <option value='eop' selected>End of Survey</option>";
                                survey_data += "              <option value='redirect_to_url' >Redirect To URL</option>";
                                survey_data += "             <option value='show_hide_question' >Show/Hide Questions</option>";
                                survey_data += "         </select></td>";
                                survey_data += "  <td class='clearTd'><span style='cursor: pointer;' class='clear_skip_logic'>End of Survey</span></td>";
                                survey_data += "  </tr>";
                            } else if (ans['logic_action'] == "redirect_to_url") {
                                survey_data += "              <option value='no_logic' >No Logic</option>";
                                survey_data += "              <option value='redirect_to_page'>Redirect To Page</option>";
                                survey_data += "              <option value='eop' >End of Survey</option>";
                                survey_data += "              <option value='redirect_to_url' selected>Redirect To URL</option>";
                                survey_data += "             <option value='show_hide_question' >Show/Hide Questions</option>";
                                survey_data += "         </select></td>";
                                survey_data += "  <td class='clearTd'><span style='cursor: pointer;' class='clear_skip_logic'>" + ans['logic_target'] + "</span></td>";
                                survey_data += "  </tr>";
                            } else if (ans['logic_action'] == "show_hide_question") {
                                survey_data += "              <option value='no_logic' >No Logic</option>";
                                survey_data += "              <option value='redirect_to_page'>Redirect To Page</option>";
                                survey_data += "              <option value='eop' >End of Survey</option>";
                                survey_data += "              <option value='redirect_to_url' >Redirect To URL</option>";
                                survey_data += "             <option value='show_hide_question' selected>Show/Hide Questions</option>";
                                survey_data += "         </select></td>";
                                survey_data += "  <td class='clearTd'><span style='cursor: pointer;' class='clear_skip_logic'>" + ans['logic_target_name'] + "</span></td>";
                                survey_data += "  </tr>";
                            } else {
                                ...
                                survey_data += "             <option value='show_hide_question' >Show/Hide Questions</option>";
                                survey_data += "         </select></td>";
                                survey_data += "  <td class='clearTd'><span style='cursor: pointer;' class='clear_skip_logic'>No Logic</span></td>";
                                survey_data += "  </tr>";
                            }

                        } else if (questions['enable_otherOption'] == "Yes") {
                            survey_data += " <tr class='logicRow ' id='logicRow_" + answer_id + "'>";
                            survey_data += "       <td class='option_value logic-table-first-column' id='ans_option_" + answer_id + "'>" + questions['other_option'][answer_id]['option'] + "</td>";
                            survey_data += "       <td class='logic_actions_td'><select id='logic_actions_" + answer_id + "' class='logic_actions' name='logic_actions' >";
                            if (ans['logic_action'] == "redirect_to_page") {
                                survey_data += "              <option value='no_logic' >No Logic</option>";
                                survey_data += "              <option value='redirect_to_page' selected>Redirect To Page</option>";
                    ...
                    survey_data += "<div class='piping_options' style='display:none;'>";
                    survey_data += '<div class="row">';
                    survey_data += "      <div class='span3 '>Disable Piping &nbsp;";
                    survey_data += "                         <input type='checkbox' style='margin-top:3px;' class='disable_piping_question' name='disable_piping_textbox_" + question_sequence + "' " + is_checked + " disabled/> </div>";
                    ...
                    $.each(this.sync_module_fields[self.model.get('sync_module')], function (key, field) {
                        // Piping tab contents
                        var is_checked = '';
                    ...
                    if (question_type != "doc-attachment" && question_type != "video" && question_type != "image") {
                        if (self.submissionCount_edit == "true") {
                            survey_data += '          <div class="dashlet-tab piping no_selected_piping"><div style="cursor:not-allowed;background:transparent !important;">Piping</div></div>'
                        } else {
                            survey_data += '          <div class="dashlet-tab piping"><a style="margin-top: 30px;background:transparent !important;">Piping</a></div>'
                        }
                    }
                    ...
                    if (question_type == "video") {
                        ...
                        survey_data += "      <div class='span11'><textarea name='description_" + questions['que_type'] + "_" + question_sequence + "' style='max-width:80%;margin-left: 1px;'  class='inherit-width' disabled>" + desc + "</textarea></div>";
                        survey_data += "</div>"; //is_required row closed
                    }
                    if (question_type == "commentbox") {
                        survey_data += "<div class='row'>";
                        survey_data += "       <div class='span1'>Rows</div><div class='span5'>";
                        survey_data += "        <input type='text' style='width:55%; margin-left:1px;' placeholder='Rows' value='" + questions['min'] + "' name='rows_commentbox_" + question_sequence + "'  class='inherit-width numericField' ></div>";
                        survey_data += "      <div class='span1'>Columns</div>";
                        survey_data += "      <div class='span5'><input type='text' style='width:55%; margin-left:1px;' placeholder='Columns' value='" + questions['max'] + "' name='cols_commentbox_" + question_sequence + "'  class='inherit-width numericField' ></div>";
                        survey_data += "</div>";
                    }
                    if (question_type == "scale") {
                        ...
                        survey_data += "          <input maxlength='20' type='text' placeholder='Left' name='left_" + questions['que_type'] + "_" + question_sequence + "'  class='inherit-width alphaField' value='" + left + "' style='width:25%; margin-left:1px;' disabled>";
                        survey_data += "          <input maxlength='20' type='text' placeholder='Middle' name='middle_" + questions['que_type'] + "_" + question_sequence + "'  class='inherit-width alphaField' value='" + middle + "' style='width:25%; margin-left:1px;' disabled>";
                        survey_data += "          <input maxlength='20' type='text' placeholder='Right' name='right_" + questions['que_type'] + "_" + question_sequence + "'  class='inherit-width alphaField' value='" + right + "' style='width:25%; margin-left:1px; ' disabled></div>";
                        survey_data += "</div>";
                    }
                    ...
                    if (question_type == "netpromoterscore") {
                        ...
                        survey_data += '<div class="row">';
                        survey_data += "       <div class='span1'>Display Label</div><div class='span5'>";
                        survey_data += "          <input maxlength='20' type='text' placeholder='Left' name='left_" + questions['que_type'] + "_" + question_sequence + "'  class='inherit-width' value='" + left + "' style='width:25%; margin-left:1px;' disabled>";
                        survey_data += "          <input maxlength='20' type='text' placeholder='Right' name='right_" + questions['que_type'] + "_" + question_sequence + "'  class='inherit-width' value='" + right + "' style='width:25%; margin-left:1px; ' disabled></div>";
                        survey_data += "</div>";
                    }
                    var option_count = 0;
                    if (questions['answers'])
                    ...
                    survey_data += '<div class="row">';
                    if (question_type != "video" && question_type != "image") {
                        //is_required
                        survey_data += "      <div class='span1'>Is Required </div>";
                        if (questions['is_required'] == 'Yes') {
                            survey_data += "      <div class='span1'><input type='checkbox' style='margin:2px 7px 7px 7px' name='is_required_" + questions['que_type'] + "_" + question_sequence + "' checked='checked' class='inherit-width' disabled/></div>";
                        } else {
                            survey_data += "      <div class='span1'><input type='checkbox' style='margin:2px 7px 7px 7px' name='is_required_" + questions['que_type'] + "_" + question_sequence + "' class='inherit-width' disabled/></div>";
                        }
                    }

                    if (question_type == "date-time") {
                        survey_data += "      <div class='span1'>Show Time</div>";
                        ...
                        if (questions['is_datetime'] == 'Yes') {
                            survey_data += "      <div class='span1'><input type='checkbox' " + disabled + " name='is_datetime_" + questions['que_type'] + "_" + question_sequence + "' checked='checked' class='inherit-width' disabled/></div>";
                        } else {
                            survey_data += "      <div class='span1'><input type='checkbox' " + disabled + "  name='is_datetime_" + questions['que_type'] + "_" + question_sequence + "' class='inherit-width' disabled/></div>";
                        }
                    }
                    //Add Question Seperator
                    survey_data += "      <div class='span2'>Add Question Separator?</div>";
                    ...
                    if (question_type == "date-time") {
                        // Allow Future Dates
                        survey_data += "      <div class='span2' >Allow Future Dates ?</div>";
                        if (questions['allow_future_dates'] == 'No') {
                            survey_data += "      <div class='span1'><input type='checkbox' name='allow_future_dates_" + questions['que_type'] + "_" + question_sequence + "' class='inherit-width'/></div>";
                        } else {
                            survey_data += "      <div class='span1'><input type='checkbox' name='allow_future_dates_" + questions['que_type'] + "_" + question_sequence + "' checked='checked' class='inherit-width'/></div>";
                        }
                    }
                    survey_data += "</div>";
                    ...
                    if (question_type == "rating") {
                        ...
                        if (questions['maxsize'] == 1) {
                            survey_data += "             <option value='0'>select</option>";
                            survey_data += "             <option selected>1</option><option>2</option><option>3</option><option>4</option><option>5</option><option>6</option><option>7</option><option>8</option><option>9</option><option>10</option>";
                        } else if (questions['maxsize'] == 2) {
                            survey_data += "             <option value='0'>select</option>";
                            survey_data += "             <option>1</option><option selected>2</option><option>3</option><option>4</option><option>5</option><option>6</option><option>7</option><option>8</option><option>9</option><option>10</option>";
                        } else if (questions['maxsize'] == 3) {
                            survey_data += "             <option value='0'>select</option>";
                            survey_data += "             <option>1</option><option>2</option><option selected>3</option><option>4</option><option>5</option><option>6</option><option>7</option><option>8</option><option>9</option><option>10</option>";
                        } else if (questions['maxsize'] == 4) {
                            survey_data += "             <option value='0'>select</option>";
                            survey_data += "             <option>1</option><option>2</option><option>3</option><option selected>4</option><option>5</option><option>6</option><option>7</option><option>8</option><option>9</option><option>10</option>";
                        } else if (questions['maxsize'] == 5) {
                            survey_data += "             <option value='0'>select</option>";
                            survey_data += "             <option>1</option><option>2</option><option>3</option><option>4</option><option selected>5</option><option>6</option><option>7</option><option>8</option><option>9</option><option>10</option>";
                        } else if (questions['maxsize'] == 6) {
                            survey_data += "             <option value='0'>select</option>";
                            survey_data += "             <option>1</option><option>2</option><option>3</option><option>4</option><option>5</option><option selected>6</option><option>7</option><option>8</option><option>9</option><option>10</option>";
                        } else if (questions['maxsize'] == 7) {
                            survey_data += "             <option value='0'>select</option>";
                            survey_data += "             <option>1</option><option>2</option><option>3</option><option>4</option><option>5</option><option>6</option><option selected>7</option><option>8</option><option>9</option><option>10</option>";
                        } else if (questions['maxsize'] == 8) {
                            survey_data += "             <option value='0'>select</option>";
                            survey_data += "             <option>1</option><option>2</option><option>3</option><option>4</option><option>5</option><option>6</option><option>7</option><option selected>8</option><option>9</option><option>10</option>";
                        } else if (questions['maxsize'] == 9) {
                            survey_data += "             <option value='0'>select</option>";
                            survey_data += "             <option>1</option><option>2</option><option>3</option><option>4</option><option>5</option><option>6</option><option>7</option><option>8</option><option selected>9</option><option>10</option>";
                        } else if (questions['maxsize'] == 10) {
                            survey_data += "             <option value='0'>select</option>";
                            survey_data += "             <option>1</option><option>2</option><option>3</option><option>4</option><option>5</option><option>6</option><option>7</option><option>8</option><option>9</option><option selected>10</option>";
                        } else {
                            survey_data += "             <option value='0'>select</option>";
                            survey_data += "             <option>1</option><option>2</option><option>3</option><option>4</option><option selected>5</option><option>6</option><option>7</option><option>8</option><option>9</option><option>10</option>";
                        }
                    ...
                    if (question_type == "date-time") {
                        ...
                        if (questions['min'])
                        {
                            survey_data += '        <input value = "' + questions['min'] + '" name="startDate_' + questions['que_type'] + '_' + question_sequence + '" style = "width:85%;margin-top:5px;margin-left:3px;" name = "show_startdatepicker" id="show_startdatepicker" class = "show_datepicker datepicker" type = "text" data-type = "date" class = "ui-timepicker-input" placeholder = "Date" aria-label = "Start Date" disabled>';
                        } else {
                            survey_data += '        <input name="startDate_' + questions['que_type'] + '_' + question_sequence + '" style = "width:85%;margin-left:3px;margin-top:5px;" name = "show_startdatepicker" id="show_startdatepicker" class = "show_datepicker datepicker" type = "text" data-type = "date" class = "ui-timepicker-input" placeholder = "Date" aria-label = "Start Date" disabled>';
                        }
                        ...
                        survey_data += '        <span class = "add-on date" id="dateicon" data-icon = "calendar" style="margin-top:5px;"> <i class = "fa fa-calendar" > </i></span >';
                        survey_data += "       </div>";
                        survey_data += "       </div>";
                        //end date
                        ...
                        survey_data += '        <div class = "input-append date datetime" style="position:relative">';
                        if (questions['max']) {
                            survey_data += '        <input value = "' + questions['max'] + '" name="endDate_' + questions['que_type'] + '_' + question_sequence + '" style = "width:55%;margin-top:5px;margin-left:3px;" name = "show_enddatepicker" id="show_enddatepicker" class = "show_datepicker datepicker" type = "text" data-type = "date" class = "ui-timepicker-input" placeholder = "Date" aria-label = "Start Date" disabled>';
                        } else {
                            survey_data += '        <input name="endDate_' + questions['que_type'] + '_' + question_sequence + '" style = "width:55%;margin-left:3px;margin-top:5px;" name = "show_enddatepicker" id="show_enddatepicker" class = "show_datepicker datepicker" type = "text" data-type = "date" class = "ui-timepicker-input" placeholder = "Date" aria-label = "Start Date" disabled>';
                        }
                    ...
                    if (question_type == "matrix") {
                        ...
                        if (questions['advance_type'] == 'radio') {
                            survey_data += "      <input type='radio' style='margin-right: 0px;' value='radio' name='display_type_" + questions['que_type'] + "_" + question_sequence + "' checked='checked' class='inherit-width' disabled/><a href='javascript:void(0);' class='btn' style='margin-bottom-6px;margin-left-6px;'> <i class='fa fa-dot-circle-o'></i>&nbsp; Radio</a></i>&nbsp;&nbsp;";
                        } else if (questions['advance_type'] == 'checkbox') {
                            survey_data += "      <input type='radio' style='margin-right: 0px;' value='radio' name='display_type_" + questions['que_type'] + "_" + question_sequence + "' class='inherit-width' disabled/><a href='javascript:void(0);' class='btn' style='margin-bottom-6px;margin-left-6px;'> <i class='fa fa-dot-circle-o'></i>&nbsp; Radio</a></i>&nbsp;&nbsp;";
                        } else {
                            survey_data += "      <input type='radio' style='margin-right: 0px;' value='radio' name='display_type_" + questions['que_type'] + "_" + question_sequence + "' checked='checked' class='inherit-width' disabled/><a href='javascript:void(0);' class='btn' style='margin-bottom-6px;'> <i class='fa fa-dot-circle-o'></i>&nbsp; Radio</a></i>&nbsp;&nbsp;";
                        }

                        if (questions['advance_type'] == 'checkbox') {
                            survey_data += "      <input type='radio' style='margin-right: 0px;' value='checkbox' name='display_type_" + questions['que_type'] + "_" + question_sequence + "' checked='checked' class='inherit-width' disabled/><a href='javascript:void(0);' class='btn' style='margin-bottom-6px;margin-left-6px;'> <i class='fa fa-check-square-o'></i>&nbsp; Checkbox";
                        } else {
                            survey_data += "      <input type='radio' style='margin-right: 0px;' value='checkbox' name='display_type_" + questions['que_type'] + "_" + question_sequence + "' class='inherit-width' disabled/><a href='javascript:void(0);' class='btn' style='margin-bottom-6px;margin-left-6px;'> <i class='fa fa-check-square-o'></i>&nbsp; Checkbox";
                        }
                        survey_data += "                   </a>";
                        survey_data += "</div>";
                        survey_data += "</div>";
                    ...
                    if (question_type == "contact-information") {
                        ...
                        if ($.inArray("Name", requireFields) != -1) {
                            survey_data += "                    <input type='checkbox' name='Name_" + question_sequence + "' checked><span style='margin-top:1px;'>Name</span>";
                        } else {
                            survey_data += "                    <input type='checkbox' name='Name_" + question_sequence + "'><span style='margin-top:1px;'>Name</span>";
                        }
                        survey_data += "                </div>";
                        survey_data += "                <div class='span2'>";
                        if ($.inArray("Email", requireFields) != -1) {
                            survey_data += "                    <input type='checkbox' name='Email_" + question_sequence + "' checked><span style='margin-top:1px;'>Email Address</span>";
                        } else {
                            survey_data += "                    <input type='checkbox' name='Email_" + question_sequence + "'><span style='margin-top:1px;'>Email Address</span>";
                        }
                        survey_data += "                </div>";
                        survey_data += "                <div class='span2'>";
                        if ($.inArray("Company", requireFields) != -1) {
                            survey_data += "                    <input type='checkbox' name='Company_" + question_sequence + "' checked><span style='margin-top:1px;'>Company</span>";
                        } else {
                            survey_data += "                    <input type='checkbox' name='Company_" + question_sequence + "'><span style='margin-top:1px;'>Company</span>";
                        }
                        survey_data += "                </div>";
                        survey_data += "                <div class='span2'>";
                        if ($.inArray("Phone", requireFields) != -1) {
                            survey_data += "                    <input type='checkbox' name='Phone_" + question_sequence + "' checked><span style='margin-top:1px;'>Phone Number</span>";
                        } else {
                            survey_data += "                    <input type='checkbox' name='Phone_" + question_sequence + "'><span style='margin-top:1px;'>Phone Number</span>";
                        }
                        survey_data += "                </div>";
                        survey_data += "                <div class='span2'>";
                        if ($.inArray("Address", requireFields) != -1) {
                            survey_data += "                    <input type='checkbox' name='Address_" + question_sequence + "' checked><span style='margin-top:1px;'>Street1</span>";
                        } else {
                            survey_data += "                    <input type='checkbox' name='Address_" + question_sequence + "'><span style='margin-top:1px;'>Street1</span>";
                        }
                        survey_data += "                </div>";
                        ...
                        if ($.inArray("Address2", requireFields) != -1) {
                            survey_data += "                    <input type='checkbox' name='Address2_" + question_sequence + "' checked><span style='margin-top:1px;'>Street2</span>";
                        } else {
                            survey_data += "                    <input type='checkbox' name='Address2_" + question_sequence + "'><span style='margin-top:1px;'>Street2</span>";
                        }
                        survey_data += "                </div>";
                        survey_data += "                <div class='span2'>";
                        if ($.inArray("City", requireFields) != -1) {
                            survey_data += "                    <input type='checkbox' name='City_" + question_sequence + "' checked><span style='margin-top:1px;'>City/Town</span>";
                        } else {
                            survey_data += "                    <input type='checkbox' name='City_" + question_sequence + "'><span style='margin-top:1px;'>City/Town</span>";
                        }
                        survey_data += "                </div>";
                        survey_data += "                <div class='span2'>";
                        if ($.inArray("State", requireFields) != -1) {
                            survey_data += "                    <input type='checkbox' name='State_" + question_sequence + "' checked><span style='margin-top:1px;'>State/ Province</span>";
                        } else {
                            survey_data += "                    <input type='checkbox' name='State_" + question_sequence + "'><span style='margin-top:1px;'>State/ Province</span>";
                        }
                        survey_data += "                </div>";
                        survey_data += "                <div class='span2'>";
                        if ($.inArray("Zip", requireFields) != -1) {
                            survey_data += "                    <input type='checkbox' name='Zip_" + question_sequence + "' checked><span style='margin-top:1px;'>Zip/ Postal Code</span>";
                        } else {
                            survey_data += "                    <input type='checkbox' name='Zip_" + question_sequence + "'><span style='margin-top:1px;'>Zip/ Postal Code</span>";
                        }
                        survey_data += "                </div>";
                        survey_data += "                <div class='span2'>";
                        if ($.inArray("Country", requireFields) != -1) {
                            survey_data += "                    <input type='checkbox' name='Country_" + question_sequence + "' checked><span style='margin-top:1px;'>Country</span>";
                        } else {
                            survey_data += "                    <input type='checkbox' name='Country_" + question_sequence + "'><span style='margin-top:1px;'>Country</span>";
                        }
                        survey_data += "                </div>";
                        survey_data += "                </div>";
                        survey_data += "            </div>"; //row complete
                    ...
                    if (question_type == "commentbox") {
                        ...
                        survey_data += "          <input type='text' placeholder='Maximum Character Limit' name='size_" + questions['que_type'] + "_" + question_sequence + "'  class='inherit-width numericField' style='width:85%; margin-left:1px;'></div>";
                    ...
                    if (question_type == "scale") {
                        ...
                        survey_data += "        <input type='text' style='width:25%; margin-left:1px;' placeholder='Start Value' name='start_" + questions['que_type'] + "_" + question_sequence + "' value='" + start_value + "'  class='inherit-width numericField' disabled>&nbsp; -";
                        survey_data += "        <input type='text' style='width:25%; margin-left:1px;' placeholder='End Value' name='end_" + questions['que_type'] + "_" + question_sequence + "'  value='" + end_value + "' class='inherit-width numericField' disabled></div>";
                        survey_data += "      <div class='span1'>Step Value</div>";
                        survey_data += "      <div class='span5'><input type='text' style='width:55%; margin-left:1px;' placeholder='Step Value' name='step_" + questions['que_type'] + "_" + question_sequence + "' value='" + scale_slot + "'  class='inherit-width numericField' disabled></div>";
                        survey_data += "</div>";
                    }
                    if (question_type == "textbox") {
                        ...
                        survey_data += "          <a><select name='datatype_textbox_" + question_sequence + "' style='max-width:80%;' class='inherit-width datatype-textbox' disabled>";
                        survey_data += '          <option value="0">Select Data Type</option>';
                        ...
                        if (textbox_datatype == 'Char' || typeof textbox_datatype === 'undefined') {
                            ...
                            survey_data += "       <div class='span1'>Max Size</div><div class='span5'>";
                            survey_data += "          <input type='text' placeholder='Maximum Character Limit' value='" + questions['maxsize'] + "' name = 'size_textbox_" + question_sequence + "'  class = 'inherit-width numericField' style = 'width:85%; margin-left:1px;'disabled > ";
                            survey_data += "      </div>";
                            survey_data += "</div>";
                        }
                        if (textbox_datatype == 'Float') {
                            ...
                            survey_data += "           <input type='text' style='width:55%; margin-left: 1px;' value='" + questions['precision'] + "' placeholder='Precision' name='precision_textbox_" + question_sequence + "'  class='inherit-width numericField' disabled>";
                            survey_data += "       </div>";
                            survey_data += "</div>"; //question max and precision row closed
                        }
                        if (textbox_datatype == 'Integer' || textbox_datatype == 'Float') {
                            ...
                            survey_data += "        <input type='text' style='width:55%; margin-left:1px;' value='" + questions['min'] + "'  placeholder='Minimum value' name='min_textbox_" + question_sequence + "'  class='inherit-width numericField' disabled></div>";
                            survey_data += "      <div class='span1'>Max Value</div>";
                            survey_data += "      <div class='span5'><input type='text' style='width:55%; margin-left:1px;'  value='" + questions['max'] + "' placeholder='Maximum value' name='max_textbox_" + question_sequence + "'  class='inherit-width numericField' disabled></div>";
                            survey_data += "</div>";
                        }
                    }
                    ...
                    $.each(questions['answers'], function (aid, answer) {
                        $.each(answer, function (ans_id, ans_data) {
                            if (question_type == "netpromoterscore") {
                                if (ans_data['option'] == '0' || ans_data['option'] == '7' || ans_data['option'] == '9') {
                                    answerDetail[ans_id] = ans_data;
                                }
                            } else {
                                answerDetail[ans_id] = ans_data;
                    ...
                    $.each(questions['skip_logic'], function (answer_id, ans) {
                        if (typeof answerDetail[answer_id] != "undefined") {
                            survey_data += " <tr class='logicRow ' id='logicRow_" + answer_id + "'>";
                            var nps_answer = "";
                            if (answerDetail[answer_id]['option'] == "0") {
                                nps_answer = "0-6";
                            } else if (answerDetail[answer_id]['option'] == "7") {
                                nps_answer = "7-8";
                            } else if (answerDetail[answer_id]['option'] == "9") {
                                nps_answer = "9-10";
                            }
                            if (question_type == "netpromoterscore") {
                                survey_data += "       <td class='option_value logic-table-first-column' id='ans_option_" + answer_id + "'>" + nps_answer + "</td>";
                            } else {
                                survey_data += "       <td class='option_value logic-table-first-column' id='ans_option_" + answer_id + "'>" + answerDetail[answer_id]['option'] + "</td>";
                            }
                            survey_data += "       <td class='logic_actions_td'><select id='logic_actions_" + answer_id + "' class='logic_actions' name='logic_actions' >";
                ...
                survey_data += "<div class='general_options'><div class='row'>";
                survey_data += "       <div class='span1'>Type</div><div class='span4'>";
                if (question_type == "textbox") {
                    survey_data += '<i class="fa fa-file-text-o" style="font-size:13px;">&nbsp; TextBox </i> </div>';
                } else if (question_type == "commentbox") {
                    survey_data += '<i class="fa fa-comments-o" style="font-size:13px;">&nbsp; Comment TextBox</i> </div>';
                } else if (question_type == "richtextareabox") {
                    survey_data += '<i class="fa fa-comment" style="font-size:13px;">&nbsp; Rich TextBox</i> </div>';
                } else if (question_type == "multiselectlist") {
                    survey_data += '<i class="fa fa-list-ul" style="font-size:13px;">&nbsp; MultiSelect List </i> </div>';
                    survey_data += "      <div class='span2'>Enable Scoring </div>";
                    if (questions['enable_scoring'] == 'Yes')
                    {
                        survey_data += "      <div class='span2 enableScore'><input type='checkbox' name='enable_scoring_" + question_type + "_" + question_sequence + "' checked='checked' class='inherit-width' disabled/></div>";
                    } else {
                        survey_data += "      <div class='span1 enableScore'><input type='checkbox' name='enable_scoring_" + question_type + "_" + question_sequence + "' class='inherit-width' disabled/></div>";
                    }
                } else if (question_type == "check-box") {
                    survey_data += '<i class="fa  fa-check-square-o" style="font-size:13px;">&nbsp; CheckBox </i>  </div>';
                    survey_data += "      <div class='span2'>Enable Scoring </div>";
                    if (questions['enable_scoring'] == 'Yes')
                    {
                        survey_data += "      <div class='span2 enableScore'><input type='checkbox' name='enable_scoring_" + question_type + "_" + question_sequence + "' checked='checked' class='inherit-width' disabled/></div>";
                    ...
                    survey_data += "      <div class='span2'>Show Image Option? </div>";
                    if (questions['is_image_option'] == 'Yes')
                    {
                        survey_data += "      <div class='span2 isImageOption'><input type='checkbox' name='is_image_option_" + question_type + "_" + question_sequence + "' checked='checked' class='inherit-width' disabled/></div>";
                    } else {
                        survey_data += "      <div class='span1 isImageOption'><input type='checkbox' name='is_image_option_" + question_type + "_" + question_sequence + "' class='inherit-width' disabled/></div>";
                    }
//                survey_data += "        </div>";
                } else if (question_type == "boolean") {
                    survey_data += '<i class="fa  fa-check" style="font-size:13px;">&nbsp; Boolean </i></div>';
                    survey_data += "      <div class='span2'>Enable Scoring </div>";
                    if (questions['enable_scoring'] == 'Yes')
                    {
                        survey_data += "      <div class='span2 enableScore'><input type='checkbox' name='enable_scoring_" + question_type + "_" + question_sequence + "' checked='checked' class='inherit-width' disabled/></div>";
                    ...
                    survey_data += '<i class="fa fa-chevron-down" style="font-size:13px;">&nbsp; Dropdown List </i> </div>';
                    survey_data += "      <div class='span2'>Enable Scoring </div>";
                    if (questions['enable_scoring'] == 'Yes')
                    {
                        survey_data += "      <div class='span2 enableScore'><input type='checkbox' name='enable_scoring_" + question_type + "_" + question_sequence + "' checked='checked' class='inherit-width' disabled/></div>";
                    ...
                    survey_data += '<i class="fa fa-dot-circle-o" style="font-size:13px;">&nbsp; Radio Button </i> </div>';
                    survey_data += "      <div class='span2'>Enable Scoring </div>";
                    if (questions['enable_scoring'] == 'Yes')
                    {
                        survey_data += "      <div class='span2 enableScore'><input type='checkbox' name='enable_scoring_" + question_type + "_" + question_sequence + "' checked='checked' class='inherit-width' disabled/></div>";
                    ...
                    survey_data += '<i class="fa fa-list-alt" style="font-size:13px;">&nbsp; Contact Information </i> </div>';
                } else if (question_type == "rating") {
                    survey_data += '<i class="fa fa-star" style="font-size:13px;">&nbsp; Rating </i> </div>';
                } else if (question_type == "date-time") {
                    survey_data += '<i class="fa fa-calendar" style="font-size:13px;">&nbsp; DateTime </i> </div>';
                } else if (question_type == "image") {
                    survey_data += '<i class="fa fa-picture-o" style="font-size:13px;">&nbsp; Image </i> </div>';
                } else if (question_type == "video") {
                    survey_data += '<i class="fa fa-video-camera" style="font-size:13px;">&nbsp; Video </i> </div>';
                } else if (question_type == "scale") {
                    survey_data += '<i class="fa fa-arrows-h" style="font-size:13px;">&nbsp; Scale </i> </div>';
                } else if (question_type == "matrix") {
                    survey_data += '<i class="fa fa-th" style="font-size:13px;">&nbsp; Matrix / Grid </i> </div>';
                } else if (question_type == "doc-attachment") {
                    survey_data += '<i class="fa fa-paperclip" style="font-size:13px;">&nbsp; Attachment </i> </div>';
                } else if (question_type == "additional-text") {
                    survey_data += '<i class="fa fa-pencil-square-o" style="font-size:13px;">&nbsp; Additional Text </i> </div>';
                } else if (question_type == "netpromoterscore") {
                    survey_data += '<i class="fa fa-dashboard" style="font-size:13px;">&nbsp; NPS </i></div>';
                } else if (question_type == "emojis") {
                    survey_data += '<i class="fa fa-meh-o" style="font-size:13px;">&nbsp; Emojis </i> </div>';
                }
...
//                    survey_data += "&nbsp;<i class='fa fa-asterisk requiredSymbol'  style='color:red; font-size:7px; vertical-align: top; ' title='Required'></i>";
//                }
                    survey_data += "</div>";
                    if (questions['disabled_question'] == "Yes") {
                        survey_data += '<a rel="tooltip" title="" data-original-title="Disabled Question" class="disabled_questions_sybmol" rel="tooltip" style="display:inline-block;" disabled><i class="fa fa-lock" style="color:red;"></i></a>';
                    }
                    survey_data += "</div>";
                }
                        ...
                        $.each(questions['answers'], function (index, ans)
                        {
                            $.each(ans, function (key, answer)
                            {
                                ...
                                if (questions['is_image_option'] == 'Yes') {
                                    ...
                                    survey_data += '<input type="radio" name="radio_' + questions['que_id'] + '" style="margin-top:-1px;"/>' + answer['option'] + '<br/>';
                                }
                            });
                        });
                        //enable other option
                        ...
                        $.each(questions['answers'], function (index, ans)
                        {
                            $.each(ans, function (key, answer)
                            {
                                ...
                                if (questions['is_image_option'] == 'Yes') {
                                    if (questions['enable_scoring'] == 'Yes')
                                    {
                                        survey_data += "<div class='image_display'><img src='" + answer['radio_image'] + "'/><div><input type='checkbox' style='margin-top:-1px;'/>" + answer['option'] + "</div></div>   <input type='number' name='score_" + questions['que_type'] + "'  value='" + answer['weight'] + "' class='inherit-width score_weight' style='max-width:7%; margin-left: 10px;margin-top: 5px;' min='0' onpaste='return false; disabled'>";
                                    } else {
                                        survey_data += "<div class='image_display'><img src='" + answer['radio_image'] + "'/><div><input type='checkbox' style='margin-top:-1px;'/>" + answer['option'] + "</div></div>   <input type='number' name='score_" + questions['que_type'] + "'  value='" + answer['weight'] + "' class='inherit-width score_weight' style='max-width:7%; margin-left: 10px;margin-top: 5px;display:none;' min='0' onpaste='return false; disabled'>";
                                    }
//                                survey_data += '<div class="image_display"><img src="' + answer['radio_image'] + '"/><div><input type="checkbox" />' + answer['option'] + '</div></div>';
                                } else {
                                    if (questions['enable_scoring'] == 'Yes')
                                    {
                                        survey_data += "  <input type='checkbox' style='margin-top:-1px;'/>" + answer['option'] + "<input type='number' name='score_" + questions['que_type'] + "'  value='" + answer['weight'] + "' class='inherit-width score_weight' style='max-width:7%; margin-left: 10px;margin-top: 5px;' min='0' onpaste='return false;' disabled> <br/> ";
                                    } else {
                                        survey_data += "  <input type='checkbox' style='margin-top:-1px;'/>" + answer['option'] + "<input type='number' name='score_" + questions['que_type'] + "'  value='" + answer['weight'] + "' class='inherit-width score_weight' style='max-width:7%; margin-left: 10px;margin-top: 5px;display:none;' min='0' onpaste='return false;' disabled> <br/> ";
                                    }
//                                survey_data += '<input type="checkbox" />' + answer['option'] + '<br/>';
                        ...
                        if (questions['other_option'])
                        {
                            ...
                            if (other_option_label)
                            {
                                if (questions['enable_scoring'] == 'Yes')
                                {
                                    survey_data += "  <input type='checkbox' />" + other_option_label + "<input type='number' name='score_" + questions['que_type'] + "'  value='" + other_option_weight + "' class='inherit-width score_weight' style='max-width:7%; margin-left: 10px;margin-top: 5px;' min='0' onpaste='return false;' disabled> <br/> ";
                                } else {
                                    survey_data += "  <input type='checkbox' />" + other_option_label + "<input type='number' name='score_" + questions['que_type'] + "'  value='" + other_option_weight + "' class='inherit-width score_weight' style='max-width:7%; margin-left: 10px;margin-top: 5px;display:none;' min='0' onpaste='return false;' disabled> <br/> ";
                                }
//                            survey_data += '<input type="checkbox" />' + other_option_label + '<br/>';
                            }
                        }
                        survey_data += '</a></div></div>';
                    } else if (questions['que_type'] == 'contact-information') {
                        survey_data += '<a style="text-decoration:none; color:#333;">';
                        survey_data += '<div class="row">';
    ...
    detailview_data: function (page_id, survey_details, is_loading) {
        if (survey_details != 'null' && $('#data-page_' + page_sequence).length == 0) {
            var survey_data = '';
            if (this.$el != null) {
            ...
            self.$el.parents('.main-pane').find('.Survey_Pages').parents('.detail').css('display', 'block');
            ...
            if (is_loading == "true") {
                ...
                if (self.restrictEdit != 'true')
                {
                    $('[name=edit_button]').removeClass('disabled');
                    $('.SurveyPage').remove();
                }
            }
        }
            ...
            // allow to edit so save the edited record
                        ...
                        self.$el.parents('.main-pane').find('#' + question_class_id).each(function () {
                            ...
                            self.$el.parents('.main-pane').find('#' + question_class_id).find($(que_class)).each(function () {
                                ...
                                if ($(this).hasClass('question-section')) {
                                    ...
                                    //For Disabling the questions in survey form 7-08-2019 
                                    if ($(this).find('#section_title').parents('.row').find('.disable_section_que').css('display') == "none" && $(this).find('#section_title').parents('.row').find('.enable_section_que').css('display') == "inline-block") {
                                        survey_question_detail['disabled_question'] = "1";
                                    } else {
                                        survey_question_detail['disabled_question'] = "0";
                                    }
                                ...
                                // Questions
                                    ...
                                    $(data_page).find($('#' + que_section)).each(function () {
                                        ...
                                        //For Disabling the questions in survey form 7-08-2019                                         
                                        if (self.$el.parents('.main-pane').find('#' + que_section).find('.disable_que').css('display') == "none" && self.$el.parents('.main-pane').find('#' + que_section).find('.enable_que').css('display') == "inline-block") {
                                            survey_question_detail['disabled_question'] = "1";
                                        } else {
                                            survey_question_detail['disabled_question'] = "0";
                                        }
                                        ...
                                        if ($('#' + que_section).find('.piping_sequence').length != 0 && $('#' + que_section).find('.piping_sequence').val()) {
                                            var piping_sequence = $('#' + que_section).find('.piping_sequence').val();
                                        } else {
                                            var piping_sequence = question_id;
                                        }

                                        survey_question_detail['question_sequence'] = question_id;
                                        survey_question_detail['piping_sequence'] = piping_sequence;
                                        if (question_type == 'matrix') {
                                                    ...
                                                    { // check for radio button and checkbox dont allow null options
                                                        if (survey_question_detail['que_type'] == "radio-button" || survey_question_detail['que_type'] == "check-box") {
                                                            if (this.value.trim() != '') {
                                                                ...
                                                                var pipe_keyid = this.id;
                                                                answer_detail[this.id]['option'] = this.value.trim();
                                                                if (pipe_keyid == "") {
                                                                    answer_detail[this.id]['pipe_key'] = $(this.parentElement).find($('[name="piping_values_' + survey_question_detail['que_type'] + '"]')).val();
                                                                } else {
                                                                    answer_detail[this.id]['pipe_key'] = self.$el.parents('.main-pane').find('#' + pipe_keyid).find($('[name="piping_values_' + survey_question_detail['que_type'] + '"]')).val();
                                                                }
                                                        ...
                                                        if (survey_question_detail['que_type'] == "radio-button" || survey_question_detail['que_type'] == "check-box") {
                                                            if (this.value.trim() != '') {
                                                                ...
                                                                if (pipe_keyid == "") {
                                                                    ...
                                                                    answer_detail[this.id]['pipe_key'] = self.$el.parents('.main-pane').find('#' + pipe_keyid).parent().find($('[name="piping_values_' + survey_question_detail['que_type'] + '"]')).val();
                                                                }
                                                            ...
                                                            if (this.value.trim() != '') {
                                                                ...
                                                                if (survey_question_detail['que_type'] == "emojis") {
                    ...
                    App.api.call('create', url, {edited_survey_data: JSON.stringify(edited_survey_details), survey_id: self.record_id, remove_page_ids: JSON.stringify(self.remove_page_ids), remove_que_ids: JSON.stringify(self.remove_que_ids), remove_option_ids: JSON.stringify(self.remove_option_ids), type: type, individual_edit: self.individualEdit}, {
                        success: function (data) {
                            ...
                            if (self.model.isValid && error_status.length == 0 && self.model.editAllPages && self.isValid) {
                                self.surveypagesdetailView('', self.record_id, "true");
                ...
                // survey pages validation fail
                    ...
                    App.alert.show('error-msg', {
                        level: 'error',
                        messages: 'Please resolve errors on current survey page before proceeding.',
                        autoClose: true
                    });
                }
                ...
                self.$el.parents('.main-pane').find('.data-page').each(function () {
                    if (this.className.includes('error-custom')) {
                        App.alert.show('error-msg', {
                            level: 'error',
                            messages: 'Please select atleast one question for each page.',
                            autoClose: true
                        });
                    }
                });
    ...
    edit_button_clicked: function () {
        ...
        // edit mode enabled
            ...
            self.$el.parents('.main-pane').find('.get_share_link').hide();
            self.$el.parents('.main-pane').find('.open_link').hide();
            self.$el.parents('.main-pane').find('.copy_survey_link').hide();
    ...
    editview_data: function (page_id, isIndividualEdit, CopyFromTemplate, survey_details) {
            ...
            $.each(jQuery.parseJSON(survey_details), function (k, detail) {
                if (k != 'survey_theme' && k != 'enable_data_piping') {
                    ...
                    if (page_id != null && detail['page_number'] == page_id && isIndividualEdit) {
                        self.editviewPage_data(detail, page_sequence, self.submissionCount_edit);
                    }
                    // add all pages div to edit record
                        ...
                        self.editviewPage_data(detail, page_sequence, self.submissionCount_edit, CopyFromTemplate);
                    }
        ...
        if ((self.model && self.model.get('survey_type') != 'poll') || (self.context && !self.context.attributes.IsPOll))
        {
            if ($('.Survey_Pages').find('.SurveyPage').length == 0) {
     ...
     * @CopyFromTemplate set to 1 then page component prefill with given data
     */
    editviewPage_data: function (detail, page_sequence, submissionCount_edit, CopyFromTemplate) {
        var self = this;
        if ($('#page_' + page_sequence).length == 0) {
            ...
            var survey_data = '';
            if (page_sequence > 1 && submissionCount_edit != "true") {
                survey_data += "<div class = 'btn-group remove_closed' style='margin-top:5px;'><a class='btn remove_page' id='remove_page_" + page_sequence + "'><i data-action = \"loading\" class = \"fa fa-times\"  track = \"click:dashletToolbarCog\" > </i>&nbsp; Remove</a>\n\
                                    <a class='btn dropdown-toggle' data-toggle='dropdown'> <i class='fa fa-caret-down'></i></a>";
                ...
                survey_data += "<div class = 'btn-group remove_closed' style='margin-top:5px;display:none;'><a class='btn remove_page' id='remove_page_" + page_sequence + "'><i data-action = \"loading\" class = \"fa fa-times\"  track = \"click:dashletToolbarCog\" > </i>&nbsp; Remove</a>\n\
                                    <a class='btn dropdown-toggle' data-toggle='dropdown'> <i class='fa fa-caret-down'></i></a>";
                survey_data += '<ul class="dropdown-menu left setting-action" id="setting-action_' + page_sequence + '">';
                survey_data += '        <div>';
                //add page above the current page option
...
//            self.$el.parents('.main-pane').find('#page_' + page_sequence).find('#txt_page_title').val(detail['page_title']);
                var title = detail['page_title'];
                title = title.replace(/\&lt;/g, '<');
                title = title.replace(/\&gt;/g, '>');
                title = title.replace(/\&quot;/g, '"');
                self.$el.parents('.main-pane').find('#page_' + page_sequence).find('#txt_page_title').val(title);
            }
        }
    },
    /**
     ...
     * @CopyFromTemplate set to 1 then page component prefill with given data
     */
    editviewQuestion_data: function (questions, page_sequence, question_sequence, CopyFromTemplate, fromCloneQue) {
        var self = this;
        var show_remove_button = new Object();
        var que = new Object();
        var survey_data = '';
        if (CopyFromTemplate != 1) {
            var que_id_html = '<input type="hidden" class="que_id" value="' + questions['que_id'] + '"/><input type="hidden" class="piping_sequence" value="' + questions['piping_sequence'] + '"/>';
        } else {
            var que_id_html = '';
        }
        ...
        {
            if (questions['disabled_question'] == "Yes") {
                survey_data += '<div id="section_' + question_sequence + '" class=" question question-section sec_disabled" style="min-height:20px !important; border-radius: 10px 10px 0px 0px;">';
            } else {
            ...
            survey_data += '                         <span style="display:none;" ><a><i class="fa fa-exclamation-circle" title="Error. This field is required." style="color:red;font-size:12px;"></i></a></span>';
            survey_data += '                    </div>';
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                if (questions['disabled_question'] == "No") {
//                    survey_data += "  <a class='btn disable_section_que' id='que-disable_section' title='Disable Section Header'><i  id='disable_section_que_" + question_sequence + "' data-action = \"loading\" class = \"fa fa-unlock-alt disable_section\"  track = \"click:dashletToolbarCog\" ></i> </a> ";
                    survey_data += '  <div class="span1"><div class="btn disable_section_que" id="que-disable_section" title="Disable Section Header" style="margin-bottom:2px;"><i id="disable_section_que_' + question_sequence + '" class="fa fa-unlock-alt disable_section"></i></div>';
//                    survey_data += "  <a class='btn enable_section_que' id='que-enable_section' title='Enable Section Header' style='display:none;'><i  id='enable_section_que_" + question_sequence + "' data-action = \"loading\" class = \"fa fa-lock enable_section\"  track = \"click:dashletToolbarCog\" style=\"color:red;\"></i> </a> ";
                    survey_data += '  <div class="btn enable_section_que" id="que-enable_section" title="Enable Section Header" style="margin-bottom:2px;display:none;"><i id="enable_section_que_' + question_sequence + '" class="fa fa-lock enable_section" style="color:red;"></i></div></div>';
                } else if (questions['disabled_question'] == "Yes") {
//                    survey_data += "  <a class='btn disable_section_que sectionDisabled' id='que-disable_section' title='Disable Section Header' style='display:none;'><i  id='disable_section_que_" + question_sequence + "' data-action = \"loading\" class = \"fa fa-unlock-alt disable_section\"  track = \"click:dashletToolbarCog\" ></i> </a> ";
                    survey_data += '  <div class="span1"><div class="btn sectionDisabled disable_section_que" id="que-disable_section" title="Disable Section Header" style="margin-bottom:2px;display:none;"><i id="disable_section_que_' + question_sequence + '" class="fa fa-unlock-alt disable_section"></i></div>';
//                    survey_data += "  <a class='btn enable_section_que' id='que-enable_section' title='UnDisable Section Header'><i  id='enable_section_que_" + question_sequence + "' data-action = \"loading\" class = \"fa fa-lock enable_section\"  track = \"click:dashletToolbarCog\" style=\"color:red;\"></i> </a> ";
                    survey_data += '  <div class="btn enable_section_que" id="que-enable_section" title="Enable Section Header" style="margin-bottom:2px;"><i id="enable_section_que_' + question_sequence + '" class="fa fa-lock enable_section" style="color:red;"></i></div></div>';
                }
            } else {
                survey_data += '                    <div class="span1"><div class="btn remove-section" style="margin-bottom:2px;"><i class="fa fa-times"  title="Remove Section Header"></i></div></div>';
            }
        ...
        // if question
        else if ($('#' + questions['que_type'] + '_' + question_sequence).length == 0) {
            var survey_body = 'survey-body';
            ...
            if (questions['disabled_question'] == "Yes") {
                survey_data += '<div id="' + questions['que_type'] + '_' + question_sequence + '" class="que_' + question_sequence + ' question ' + survey_body + ' que_disabled">';
            } else {
            ...
            if (questions['que_type'] != 'additional-text')
            {
                ...
                if (questions['que_type'] != 'doc-attachment' && questions['que_type'] != 'richtextareabox' && questions['que_type'] != 'emojis' && questions['que_type'] != 'image' && questions['que_type'] != 'video')
                {
                    if (this.submissionCount_edit == "true") {
                        survey_data += '          <div class="dashlet-tab piping" style="pointer-events: none;">Piping</div>';
                    } else {
            ...
            survey_data += ' <a class="btn cloneQue" href="javascript:void(0);" title="Clone Question" id="clone_que_' + question_sequence + '">';
            survey_data += '    <i class="fa  fa-copy" style="font-size:12px;font-weight:bold">&nbsp;</i></a>';
            survey_data += ' <a class="btn showQuestionSection" href="javascript:void(0);" title="Add Section Header" id="add_section_header_' + question_sequence + '">';
            survey_data += '    <i class="fa  fa-bars" style="font-size:12px;">&nbsp;</i></a>';
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                if (questions['disabled_question'] == "No") {
                    survey_data += "  <a class='btn disable_que' id='que-disable' title='Disable Question'><i  id='disable_que_" + question_sequence + "' data-action = \"loading\" class = \"fa fa-unlock-alt\"  track = \"click:dashletToolbarCog\" ></i> </a> ";
                    survey_data += "  <a class='btn enable_que' id='que-enable' title='UnDisable Question' style='display:none;'><i  id='enable_que_" + question_sequence + "' data-action = \"loading\" class = \"fa fa-lock\"  track = \"click:dashletToolbarCog\" style=\"color:red;\"></i> </a> ";
                } else if (questions['disabled_question'] == "Yes") {
                    survey_data += "  <a class='btn disable_que questionDisabled' id='que-disable' title='Disable Question' style='display:none;'><i  id='disable_que_" + question_sequence + "' data-action = \"loading\" class = \"fa fa-unlock-alt\"  track = \"click:dashletToolbarCog\" ></i> </a> ";
                    survey_data += "  <a class='btn enable_que' id='que-enable' title='UnDisable Question'><i  id='enable_que_" + question_sequence + "' data-action = \"loading\" class = \"fa fa-lock\"  track = \"click:dashletToolbarCog\" style=\"color:red;\"></i> </a> ";
                }
            } else {
            ...
            survey_data += "</div>";
            survey_data += '    </div>';
            if (fromCloneQue == true) {
                $(survey_data).insertBefore(self.$el.parents('.main-pane').find('#page_' + page_sequence).find('#data-page_' + page_sequence).find('#placeholder'));
            } else {
                $(survey_data).appendTo(self.$el.parents('.main-pane').find('#page_' + page_sequence).find('#data-page_' + page_sequence));
            }
            survey_data = '';
            ...
            {
                if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                    survey_data += " &nbsp;</div>"; //question type span complete
                } else {
                ...
                {
                    if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                        survey_data += "      <div class='span1 isImageOption'><input type='checkbox' name='is_image_option_" + questions['que_type'] + "_" + question_sequence + "' checked='checked' class='inherit-width' disabled/></div>";
                    } else {
                        survey_data += "      <div class='span1 isImageOption'><input type='checkbox' name='is_image_option_" + questions['que_type'] + "_" + question_sequence + "' checked='checked' class='inherit-width'/></div>";
                    }
                    survey_data += "      <div class='span2 showOptionText' style='margin-top: 5px;margin-bottom: 4px;'>Show Option Text? </div>";
                    if (questions['show_option_text'] == 'Yes')
                    {
                        if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                            survey_data += "      <div class='span1 showOptionText'><input type='checkbox' name='show_option_text_" + questions['que_type'] + "_" + question_sequence + "' checked='checked' class='inherit-width' disabled/></div>";
                        } else {
                            survey_data += "      <div class='span1 showOptionText'><input type='checkbox' name='show_option_text_" + questions['que_type'] + "_" + question_sequence + "' checked='checked' class='inherit-width'/></div>";
                        }
                    } else {
                        if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                            survey_data += "      <div class='span1 showOptionText'><input type='checkbox' name='show_option_text_" + questions['que_type'] + "_" + question_sequence + "' class='inherit-width' disabled/></div>";
                        } else {
                            survey_data += "      <div class='span1 showOptionText'><input type='checkbox' name='show_option_text_" + questions['que_type'] + "_" + question_sequence + "' class='inherit-width'/></div>";
                        }
                    }

                } else {
                    if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                        survey_data += "      <div class='span1 isImageOption'><input type='checkbox' name='is_image_option_" + questions['que_type'] + "_" + question_sequence + "' class='inherit-width' disabled/></div>";
                        survey_data += "      <div class='span2 showOptionText' style='margin-top: 5px;margin-bottom: 4px;display:none;'>Show Option Text? </div>";
                        survey_data += "      <div class='span1 showOptionText' style='display:none;'><input type='checkbox' name='show_option_text_" + questions['que_type'] + "_" + question_sequence + "' class='inherit-width' disabled/></div>";
                    } else {
                        survey_data += "      <div class='span1 isImageOption'><input type='checkbox' name='is_image_option_" + questions['que_type'] + "_" + question_sequence + "' class='inherit-width'/></div>";
                        survey_data += "      <div class='span2 showOptionText' style='margin-top: 5px;margin-bottom: 4px;display:none;'>Show Option Text? </div>";
                        survey_data += "      <div class='span1 showOptionText' style='display:none;'><input type='checkbox' name='show_option_text_" + questions['que_type'] + "_" + question_sequence + "' class='inherit-width'/></div>";
                    }
                }
            }
            ...
            if ((question_type == 'multiselectlist' || question_type == 'dropdownlist' || questions['que_type'] == 'boolean') && (this.model && (this.model.get('survey_type') != 'poll') || (this.context && !this.context.attributes.IsPOll)))
            {
                ...
                if (questions['enable_scoring'] == 'Yes')
                {
                    ...
                    if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                        survey_data += "      <div class='span2 enableScore'><input type='checkbox' name='enable_scoring_" + question_type + "_" + question_sequence + "' class='inherit-width' disabled/></div>";
                    } else {
            ...
            if (question_type == 'radio-button' || question_type == 'check-box')
            {
                ...
                if (questions['is_image_option'] == 'Yes')
                {
                    ...
                    if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                        survey_data += "      <div class='span2 isImageOption'><input type='checkbox' name='is_image_option_" + question_type + "_" + question_sequence + "' class='inherit-width' disabled/></div>";
                    } else {
            ...
            if (questions['que_type'] == "image") {
                ...
                survey_data += '          <span style="display:none;" ><a><i class="fa fa-exclamation-circle" title="Error. This field is required." style="color:red;font-size:12px;"></i></a></span>';
                // Question Piping section
                if (questions['que_type'] == 'textbox' || questions['que_type'] == 'commentbox' || questions['que_type'] == 'radio-button' || questions['que_type'] == 'dropdownlist' || questions['que_type'] == 'check-box' || questions['que_type'] == 'multiselectlist' || questions['que_type'] == 'boolean' || questions['que_type'] == 'scale' || questions['que_type'] == 'rating' || questions['que_type'] == 'matrix' || questions['que_type'] == 'netpromoterscore' || questions['que_type'] == 'emojis' || questions['que_type'] == 'datetime' || questions['que_type'] == 'contact-information' || questions['que_type'] == 'doc-attachment') {
                    survey_data += '          <span class="que-piping-section"><a class="que-piping"><i class="fa fa-link" title="Question Piping"></i></a></span>';
                }
                survey_data += "         </div>";
                survey_data += "      </div>";
                survey_data += "</div>";
            ...
            if (questions['que_type'] != 'richtextareabox') {
                ...
                if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                    survey_data += "      <div class='span11'><input type='text' name='helptips_" + questions['que_type'] + "_" + question_sequence + "'  placeholder='" + placeholder + "' style='max-width:80%; margin-left:1px;' class='inherit-width' disabled/></div>";
                } else {
            ...
            if (questions['que_type'] == 'textbox')
            {
                survey_data += self.editviewTextBox(question_sequence, questions, fromCloneQue);
            }
            //Question datatype for commentbox
            else if (questions['que_type'] == 'commentbox')
            {
                survey_data += self.editviewCommentBox(question_sequence, questions, fromCloneQue);
            }
            //Question datatype for richtextareabox
            else if (questions['que_type'] == 'richtextareabox')
            {
                survey_data += self.editviewRichTextBox(question_sequence, questions, fromCloneQue);
            }
            //Question datatype for boolean
            else if (questions['que_type'] == 'boolean')
            {
                survey_data += self.editviewBoolean(question_sequence, questions, fromCloneQue);
            }
            //Question datatype for rating
            else if (questions['que_type'] == 'rating')
            {
                survey_data += self.editviewRating(question_sequence, questions, fromCloneQue);
            }
            //Question datatype for contact-information
            else if (questions['que_type'] == 'contact-information')
            {
                survey_data += self.editviewContactInformation(question_sequence, questions, fromCloneQue);
            }
            ...
            {
                survey_data += self.editviewMultiChoice(question_sequence, questions, fromCloneQue);
            }
            //Question datatype for date-time
            else if (questions['que_type'] == 'date-time')
            {
                survey_data += self.editviewDatetime(question_sequence, questions, fromCloneQue);
            }
            //Question datatype for video
            else if (questions['que_type'] == 'video')
            {
                survey_data += self.editviewVideo(question_sequence, questions, fromCloneQue);
            }
            //Question datatype for image
            else if (questions['que_type'] == 'image')
            {
                survey_data += self.editviewImage(question_sequence, questions, fromCloneQue);
            }
            //Question datatype for scale
            else if (questions['que_type'] == 'scale')
            {
                survey_data += self.editviewScale(question_sequence, questions, fromCloneQue);
            }
            //Question datatype for NPS
            else if (questions['que_type'] == 'netpromoterscore')
            {
                survey_data += self.editviewNetPromoterScore(question_sequence, questions, fromCloneQue);
            }
            //Question datatype for Emojis
            else if (questions['que_type'] == 'emojis')
            {
                survey_data += self.editviewEmojis(question_sequence, questions, fromCloneQue);
            }
            //Question datatype for matrix
            else if (questions['que_type'] == 'matrix')
            {
                survey_data += self.editviewMatrix(question_sequence, questions, fromCloneQue);
            }
            //Question datatype for doc-attachment
            else if (questions['que_type'] == 'doc-attachment')
            {
                survey_data += self.editviewDocAttachment(question_sequence, questions, fromCloneQue);
            }
            ...
            if (questions['que_type'] == 'multiselectlist' || questions['que_type'] == 'dropdownlist' || questions['que_type'] == 'radio-button' || questions['que_type'] == 'check-box' || questions['que_type'] == 'boolean' || questions['que_type'] == 'netpromoterscore' || questions['que_type'] == 'emojis')
            {
                // Show Logic Tab In Only Survey Module And Only Edit Mode.
                if (this.module == 'bc_survey' && (this.record_id != '' && this.record_id != null)) {
                    survey_data += self.getSkipLogiclayout(questions, page_sequence, fromCloneQue);
                }
            }
            ...
            if (questions['answers']) {
                self.editviewAnswer_data(questions, question_sequence, CopyFromTemplate, fromCloneQue);
            }
            ...
            if (questions['que_type'] == 'richtextareabox') {
                ...
                var que_title = question_title;
                que_title = que_title.replace(/\&lt;/g, '<');
                que_title = que_title.replace(/\&gt;/g, '>');
                que_title = que_title.replace(/\&quot;/g, '"');
                self.$el.parents('.main-pane').find("[name='question_" + questions['que_type'] + "_" + question_sequence + "']").val(que_title);
            }
    ...
    show_skip_logic_option: function (el) {
        ...
        if (que_id == '' || que_id == null || que_id == 'undefined') {
            app.alert.show('alert_for_logic', {
                level: 'info',
                messages: "You must save this survey before using skip logic.",
                autoClose: true
            });
        } else {
            ...
            $.each(optionsArr, function (item, val) {
                if ((que_type == 'netpromoterscore' && npsOptionsTrs.indexOf(val.firstElementChild.value) != '-1') || que_type != 'netpromoterscore') {
                    ...
                    if ((exist_logic_optionsTr == '' || exist_logic_optionsTr == null) && typeof optionLable != "undefined") {
            ...
            /* $.each(self.$el.parents('.main-pane').find('#' + page_divId).find('.skip_logic_options').find('#logicRows_table').find('.show_hide_question'), function () {
             ...
             }); */ // commented due to error in Dotb 7.8
    ...
    getSkipLogiclayout: function (questions, page_sequence, fromCloneQue) {
        var data = '';
        var self = this;
        ...
        $.each(questions['answers'], function (index, ans)
        {
            ...
            $.each(ans, function (key, answer)
            {
                if ((questions['que_type'] == 'boolean' && answer['option'] == 'Yes') || questions['que_type'] != 'boolean') {
                    data += self.addLogicOptionTr(questions, key, answer, page_sequence, fromCloneQue);
                }
            });
        });
        // If other option is added
        if (questions['other_option'])
        {
            ...
            {
                data += self.addLogicOptionTr(questions, key, answer, page_sequence, fromCloneQue, 1);
            });
        }
        data += " </tbody>";
        data += "</table>";
    ...
    addLogicOptionTr: function (questions, answer_Id, answer, page_sequence, fromCloneQue, isOther) {
        var self = this;
        var data = '';
        var npsOptionsTrs = ['0', '7', '9'];
        if ((questions['que_type'] == 'netpromoterscore' && npsOptionsTrs.indexOf(answer['option']) != '-1') || questions['que_type'] != 'netpromoterscore') {
            ...
            switch (logic_action) {
                case "redirect_to_page":
                    ...
                    page_dd_html = self.getDependendDDWithSelectedValForRedirectToPageAndShowHide('redirect_to_page', page_sequence, questions, answer_Id, fromCloneQue);
                    break;
                case "eop":
                ...
                case "show_hide_question":
                    ...
                    que_dd_html = self.getDependendDDWithSelectedValForRedirectToPageAndShowHide('show_hide_question', page_sequence, questions, answer_Id, fromCloneQue);
                    break;
                default:
                    break;
            }
            ...
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                data += "       <td class='logic_actions_td'><select id='logic_actions_" + answer_Id + "' class='logic_actions' name='logic_actions' disabled>";
            } else {
            ...
            data += "         </select></td>";
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                data += "    <td class='logic_action_targtes'><select id='redirect_to_page_dd_" + answer_Id + "' class='redirect_to_page_options logic-page-select' name='redirect_to_page_options' style='" + red_to_pageDD + "' disabled>";
            } else {
            ...
            data += page_dd_html;
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                data += "   </select><input type='text' id='redirect_to_url_" + answer_Id + "' name='redirect_to_url' class='redirect_to_url_options logic-url-box' style='margin-bottom:8px; " + red_to_urlDD + "' value='" + redirect_to_url_selectedVal + "' disabled> <span style='display:none;' class='error-tooltip add-on' data-container='body' rel='tooltip' title='' data-original-title='It should be in http://www.google.com format.'><i class='fa fa fa-exclamation-circle' style='color:red;'>&nbsp;</i></span><select id='show_hide_question_dd_" + answer_Id + "' class='show_hide_question' name='show_hide_question' style='" + showHide_DD + "' multiple disabled>";
            } else {
                data += "   </select><input type='text' id='redirect_to_url_" + answer_Id + "' name='redirect_to_url' class='redirect_to_url_options logic-url-box' style='margin-bottom:8px; " + red_to_urlDD + "' value='" + redirect_to_url_selectedVal + "'> <span style='display:none;' class='error-tooltip add-on' data-container='body' rel='tooltip' title='' data-original-title='It should be in http://www.google.com format.'><i class='fa fa fa-exclamation-circle' style='color:red;'>&nbsp;</i></span><select id='show_hide_question_dd_" + answer_Id + "' class='show_hide_question' name='show_hide_question' style='" + showHide_DD + "' multiple>";
            }
            data += que_dd_html;
            data += "   </select></td>"
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                data += "  <td class='clearTd'><span style='cursor: pointer;pointer-events: none;' class='clear_skip_logic'><a>Clear</a></span></td>"
            } else {
    ...
    getDependendDDWithSelectedValForRedirectToPageAndShowHide: function (sel_action, page_sequence, questions, answer_Id, fromCloneQue) {
        ...
        switch (sel_action) {
            ...
            case "show_hide_question":
                ...
                if (typeof selectedValArr == 'string') {
    ...
    reset_SkipLogic: function (el) {
        ...
        if (doClear) {
            app.alert.show('stop_confirmation', {
                level: 'confirmation',
                title: '',
                messages: 'Are you sure to remove logic ?',
                onConfirm: function () {
     ...
     * @CopyFromTemplate set to 1 then page component prefill with given data
     */
    editviewAnswer_data: function (questions, question_sequence, CopyFromTemplate, fromCloneQue) {
        var self = this;
        var survey_data = '';
        var show_remove_button = new Object();
        var que = new Object();
        ...
        if (questions['que_type'] == 'emojis') {
            ...
            $.each(questions['answers'], function (index, ans)
            {
                ...
                $.each(ans, function (key, answer)
                {
                    ...
                    if (CopyFromTemplate == 1) { // if copied from template then dont give option id
                        if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                            survey_data += '       <input type="' + inputType + '" ' + readonly + ' name="option_' + questions['que_type'] + '" placeholder="Option" class="inherit-width opt_' + key + '" style="margin-top:5px;max-width:30%;margin-left:1px;" disabled>';
                        } else {
                        ...
                        if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                            survey_data += '       <input type="' + inputType + '" ' + readonly + ' name="option_' + questions['que_type'] + '" id="' + key + '" placeholder="Option" class="inherit-width" style="margin-top:5px;max-width:30%;margin-left:1px;" disabled>';
                        } else {
                    ...
                    if (showInputFields) {
                        ...
                        {
                            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                                survey_data += "   <input type='number' name='score_" + questions['que_type'] + "'  value='" + value + "' class='inherit-width score_weight' style='max-width:7%; margin-left: 4px;margin-top: 5px;' min='0' onpaste='return false;' disabled>";
                            } else {
                                survey_data += "   <input type='number' name='score_" + questions['que_type'] + "'  value='" + value + "' class='inherit-width score_weight' style='max-width:7%; margin-left: 4px;margin-top: 5px;' min='0' onpaste='return false;'>";
                            }
                        } else {
                            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                                survey_data += "   <input type='number' name='score_" + questions['que_type'] + "'  value='" + value + "' class='inherit-width score_weight' style='max-width:7%; margin-left: 4px;margin-top: 5px;display:none;' min='0' onpaste='return false;' disabled>";
                            } else {
                                survey_data += "   <input type='number' name='score_" + questions['que_type'] + "'  value='" + value + "' class='inherit-width score_weight' style='max-width:7%; margin-left: 4px;margin-top: 5px;display:none;' min='0' onpaste='return false;'>";
                            }
                        }
                    }
                    survey_data += '</div></div>';
                    ...
                    if (CopyFromTemplate == '1')
                    {
                        var answer_name = answer['option'];
                        answer_name = answer_name.replace(/\&lt;/g, '<');
                        answer_name = answer_name.replace(/\&gt;/g, '>');
                        answer_name = answer_name.replace(/\&quot;/g, '"');
                        self.$el.parents('.main-pane').find('#' + questions['que_type'] + "_options_div_" + question_sequence).find('.opt_' + key).val(answer_name);
                    } else {
                        var answer_name = answer['option'];
                        answer_name = answer_name.replace(/\&lt;/g, '<');
                        answer_name = answer_name.replace(/\&gt;/g, '>');
                        answer_name = answer_name.replace(/\&quot;/g, '"');
                        self.$el.parents('.main-pane').find('#' + key).val(answer_name);
                    }
                    op_count++;
                });
            });
            ...
            $.each(questions['answers'], function (index, ans)
            {
                ...
                $.each(ans, function (key, answer)
                {
                    ...
                    if (CopyFromTemplate == 1) { // if copied from template then dont give option id
                        if (questions['que_type'] == "radio-button" || questions['que_type'] == "check-box") {
                            if (answer['radio_image'] && is_image_option) {
                                if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                                    survey_data += '<img src="' + answer['radio_image'] + '" height="30" width="30"/><a class="resetImageRadioUpload" style="padding-right: 56px;margin-left:3px;pointer-events: none;" ></a>';
                                } else {
                                    survey_data += '<img src="' + answer['radio_image'] + '" height="30" width="30"/><a class="resetImageRadioUpload" style="padding-right: 56px;margin-left:3px;">Remove</a>';
                                }
                            } else {
                        ...
                        if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                            survey_data += '       <input type="text" ' + readonly + ' name="option_' + questions['que_type'] + '" placeholder="Option" class="inherit-width opt_' + key + '" style="margin-top:5px;max-width:50%;margin-left:1px;" disabled><input type="hidden" name="piping_values_' + questions['que_type'] + '" id="piping_values_' + key + '" class="inherit-width piping_values">';
                        } else {
                            survey_data += '       <input type="text" ' + readonly + ' name="option_' + questions['que_type'] + '" placeholder="Option" class="inherit-width opt_' + key + '" style="margin-top:5px;max-width:50%;margin-left:1px;"><input type="hidden" name="piping_values_' + questions['que_type'] + '" id="piping_values_' + key + '" class="inherit-width piping_values">';
                        }
                    } else {
                        if (questions['que_type'] == "radio-button" || questions['que_type'] == "check-box") {
                            if (answer['radio_image'] && is_image_option) {
                        ...
                        if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                            survey_data += '       <input type="text" ' + readonly + ' name="option_' + questions['que_type'] + '" id="' + key + '" placeholder="Option" class="inherit-width" style="margin-top:5px;max-width:50%;margin-left:1px;" disabled><input type="hidden" name="piping_values_' + questions['que_type'] + '" id="piping_values_' + key + '" class="inherit-width piping_values">';
                        } else {
                            survey_data += '       <input type="text" ' + readonly + ' name="option_' + questions['que_type'] + '" id="' + key + '" placeholder="Option" class="inherit-width" style="margin-top:5px;max-width:50%;margin-left:1px;"><input type="hidden" name="piping_values_' + questions['que_type'] + '" id="piping_values_' + key + '" class="inherit-width piping_values">';
                        }
                    }
                    // set score weight
                    if (questions['enable_scoring'] == 'Yes' && typeof answer['weight'] != 'undefined')
                    ...
                    // Add Answer Piping Button
                    if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                        // do nothing
                    } else {
                        if (questions['que_type'] != "dropdownlist") {
                            survey_data += '          <span class="ans-piping-section"><a class="ans-piping"><i class="fa fa-link" title="Answer Piping"></i></a></span>';
                        }
                    }
                    //remove option button
                    if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                        survey_data += '       <a href="javascript:void(0);" class="btn  ' + questions['que_type'] + '_remove_option que_' + question_sequence + '" id="' + op_count + '" style="margin-left:2px; margin-top:5px;display:none;"><i class="fa fa-times" id="remove_option_' + op_count + '"></i></a>';
                    } else {
                    ...
                    {
                        var answer_name = answer['option'];
                        var pipe_key = answer['pipe_key'];
                        answer_name = answer_name.replace(/\&lt;/g, '<');
                        answer_name = answer_name.replace(/\&gt;/g, '>');
                        answer_name = answer_name.replace(/\&quot;/g, '"');
                        self.$el.parents('.main-pane').find('#' + questions['que_type'] + "_options_div_" + question_sequence).find('.opt_' + key).val(answer_name);
                        self.$el.parents('.main-pane').find('#' + questions['que_type'] + "_options_div_" + question_sequence).find('#piping_values_' + key).val(pipe_key);
                    } else {
                        var answer_name = answer['option'];
                        var pipe_key = answer['pipe_key'];
                        answer_name = answer_name.replace(/\&lt;/g, '<');
                        ...
                        answer_name = answer_name.replace(/\&quot;/g, '"');
                        self.$el.parents('.main-pane').find('#' + key).val(answer_name);
                        self.$el.parents('.main-pane').find('#piping_values_' + key).val(pipe_key);
                    }
                    op_count++;
                });
            });
            if (jQuery.isEmptyObject(questions["answers"])) {
                survey_data = '';
                survey_data += "               <div class='options' id='option_0'>";
                if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                    survey_data += "                 <input type='file' name='radioImage_" + questions['que_type'] + "' class='inherit-width radioImageUpload' style='display:none;width: 175px;margin-top:5px;max-width:50%; margin-left: 1px;'><span style='display:none;margin-left: -13px; margin-top: 6px; position: relative; z-index: 500; width: auto;' class='spanRadioUploadError'><a><i class='fa fa-exclamation-circle' title='Error. This field is required.' style='color:red;font-size:12px;'></i></a></span><input type='text' name='option_" + questions['que_type'] + "' placeholder='Option' class='inherit-width' style='max-width:50%;' disabled>";
                } else {
                    survey_data += "                 <input type='file' name='radioImage_" + questions['que_type'] + "' class='inherit-width radioImageUpload' style='display:none;width: 175px;margin-top:5px;max-width:50%; margin-left: 1px;'><span style='display:none;margin-left: -13px; margin-top: 6px; position: relative; z-index: 500; width: auto;' class='spanRadioUploadError'><a><i class='fa fa-exclamation-circle' title='Error. This field is required.' style='color:red;font-size:12px;'></i></a></span><input type='text' name='option_" + questions['que_type'] + "' placeholder='Option' class='inherit-width' style='max-width:50%;'>";
                }
                if (questions['enable_scoring'] == "Yes") {
                    if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                        survey_data += "                 <input type='number' name='score_" + questions['que_type'] + "'  value='1' class='inherit-width score_weight' style='max-width:7%;margin-left: 4px;margin-top: 5px;' min='0' onpaste='return false;' disabled>";
                    } else {
                        survey_data += "                 <input type='number' name='score_" + questions['que_type'] + "'  value='1' class='inherit-width score_weight' style='max-width:7%;margin-left: 4px;margin-top: 5px;' min='0' onpaste='return false;'>";
                    }
                } else {
                    if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                        survey_data += "                 <input type='number' name='score_" + questions['que_type'] + "'  value='1' class='inherit-width score_weight' style='max-width:7%; display:none;margin-left: 4px;margin-top: 5px;' min='0' onpaste='return false;' disabled>";
                    } else {
                        survey_data += "                 <input type='number' name='score_" + questions['que_type'] + "'  value='1' class='inherit-width score_weight' style='max-width:7%; display:none;margin-left: 4px;margin-top: 5px;' min='0' onpaste='return false;'>";
                    }
                }
                survey_data += "                 <a href='javascript:void(0);' class='btn  " + questions['que_type'] + "_remove_option que_" + question_sequence + "' id='0' style='display:none;margin-left:2px; margin-top:5px;'><i class='fa fa-times'></i></a>";
                survey_data += "                 <a href='javascript:void(0);' class='btn  " + questions['que_type'] + "_add_option que_" + question_sequence + "' style='display:none;' id='0'><i class='fa fa-plus' ></i></a>";
                survey_data += "               </div>";
                survey_data += "               <div class='options' id='option_1'>";
                if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                    survey_data += "                 <input type='file' name='radioImage_" + questions['que_type'] + "' class='inherit-width radioImageUpload' style='display:none;width: 175px;margin-top:5px;max-width:50%; margin-left: 1px;'><span style='display:none;margin-left: -13px; margin-top: 6px; position: relative; z-index: 500; width: auto;' class='spanRadioUploadError'><a><i class='fa fa-exclamation-circle' title='Error. This field is required.' style='color:red;font-size:12px;'></i></a></span><input type='text' name='option_" + questions['que_type'] + "' placeholder='Option' class='inherit-width' style='max-width:50%;margin-top:5px;' disabled>";
                } else {
                    survey_data += "                 <input type='file' name='radioImage_" + questions['que_type'] + "' class='inherit-width radioImageUpload' style='display:none;width: 175px;margin-top:5px;max-width:50%; margin-left: 1px;'><span style='display:none;margin-left: -13px; margin-top: 6px; position: relative; z-index: 500; width: auto;' class='spanRadioUploadError'><a><i class='fa fa-exclamation-circle' title='Error. This field is required.' style='color:red;font-size:12px;'></i></a></span><input type='text' name='option_" + questions['que_type'] + "' placeholder='Option' class='inherit-width' style='max-width:50%;margin-top:5px;'>";
                }
                if (questions['enable_scoring'] == "Yes") {
                    if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                        survey_data += "                 <input type='number' name='score_" + questions['que_type'] + "' value='2' class='inherit-width score_weight' style='width:7%;margin-left: 4px;margin-top: 5px;' min='0' onpaste='return false;' disabled>";
                    } else {
                        survey_data += "                 <input type='number' name='score_" + questions['que_type'] + "' value='2' class='inherit-width score_weight' style='width:7%;margin-left: 4px;margin-top: 5px;' min='0' onpaste='return false;'>";
                    }
                } else {
                    if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                        survey_data += "                 <input type='number' name='score_" + questions['que_type'] + "' value='2' class='inherit-width score_weight' style='width:7%;display:none;margin-left: 4px;margin-top: 5px;' min='0' onpaste='return false;' disabled>";
                    } else {
                        survey_data += "                 <input type='number' name='score_" + questions['que_type'] + "' value='2' class='inherit-width score_weight' style='width:7%;display:none;margin-left: 4px;margin-top: 5px;' min='0' onpaste='return false;'>";
                    }
                }
                survey_data += "                 <a href='javascript:void(0);' class='btn  " + questions['que_type'] + "_remove_option que_" + question_sequence + "' id='1' style='display:none;margin-left:2px; margin-top:5px;'><i class='fa fa-times'></i></a>";
                survey_data += "                 <a href='javascript:void(0);' class='btn  " + questions['que_type'] + "_add_option que_" + question_sequence + "' style='margin-left:2px; margin-top:5px;' id='1'><i class='fa fa-plus' ></i></a>";
                survey_data += "               </div>";
                $(survey_data).appendTo('#' + questions['que_type'] + "_options_div_" + question_sequence);
            }
        }
        ...
        if (questions['que_type'] != 'boolean' && questions['que_type'] != 'emojis')
        {
            ...
            if (questions['enable_otherOption'] == 'Yes' && (this.model && (this.model.get('survey_type') != 'poll' || (this.context && !this.context.attributes.IsPOll)) && !questions['sync_field']) && showInputFields)
            {
                ...
                {
                    if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                        survey_data += "      <input type='number' value='" + other_option_weight + "' style='margin-left: 4px;width:7%;' Placeholder='score' name='option_score_" + questions['que_type'] + "_" + question_sequence + "' class='inherit-width other_weight' min='0' onpaste='return false;' disabled/>";
                    } else {
                        survey_data += "      <input type='number' value='" + other_option_weight + "' style='margin-left: 4px;width:7%;' Placeholder='score' name='option_score_" + questions['que_type'] + "_" + question_sequence + "' class='inherit-width other_weight' min='0' onpaste='return false;'/>";
                    }
                } else {
                    if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                        survey_data += "      <input type='number' value='0' style='margin-left: 4px;width:7%;display:none;' Placeholder='score' name='option_score_" + questions['que_type'] + "_" + question_sequence + "' class='inherit-width other_weight' min='0' onpaste='return false;' disabled/>";
                    } else {
                        survey_data += "      <input type='number' value='0' style='margin-left: 4px;width:7%;display:none;' Placeholder='score' name='option_score_" + questions['que_type'] + "_" + question_sequence + "' class='inherit-width other_weight' min='0' onpaste='return false;'/>";
                    }
                }
                survey_data += "</div>";
                ...
                if (questions['que_type'] == "radio-button" || questions['que_type'] == "check-box") {
                    ...
                    if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                        survey_data += "      <div class='span1 otheroptiondiv'>Label </div><div class='span11 otheroptiondiv'><input placeholder='Other Option Label' style='width:50%;' value='Other' type='text'  name='label_otherOption_" + questions['que_type'] + "_" + question_sequence + "' class='inherit-width other_option_label' disabled/>";
                    } else {
                ...
                {
                    if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                        survey_data += "      <input type='number' value='0' style='margin-left: 4px;width:7%;' Placeholder='score' name='option_score_" + questions['que_type'] + "_" + question_sequence + "' class='inherit-width other_weight' min='0' onpaste='return false;' disabled/>";
                    } else {
                        survey_data += "      <input type='number' value='0' style='margin-left: 4px;width:7%;' Placeholder='score' name='option_score_" + questions['que_type'] + "_" + question_sequence + "' class='inherit-width other_weight' min='0' onpaste='return false;'/>";
                    }
                } else {
                    if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                        survey_data += "      <input type='number' value='0' style='margin-left: 4px;width:7%;display:none;' Placeholder='score' name='option_score_" + questions['que_type'] + "_" + question_sequence + "' class='inherit-width other_weight' min='0' onpaste='return false;' disabled/>";
    ...
    showOtherOptionRow: function (el) {
        if ($(el.currentTarget).prop('checked'))
        {
            ...
            $(el.currentTarget).parents('.general_options').find('.otheroptionRow').find('.other_option_label').val('Other');
    ...
    editviewRowsCols_data: function (questions, question_sequence, CopyFromTemplate) {
        ...
        if (questions["matrix_row"] == "{}") {
            //rows
            survey_data = '';
            survey_data += "               <div class='rows' id='row_0'>";
            survey_data += "                 <input type='text' name='row_matrix' placeholder='row label' class='inherit-width' style='max-width:50%; margin-left:1px;'>";
            survey_data += "                 <a href='javascript:void(0);' class='btn  matrix_remove_row que_" + question_sequence + "' id='0' style='display:none;margin-left:2px; margin-top:5px;'><i class='fa fa-times'></i></a>";
            survey_data += "                 <a href='javascript:void(0);' class='btn  matrix_add_row que_" + question_sequence + "' style='display:none;' id='0'><i class='fa fa-plus' ></i></a>";
            survey_data += "               </div>";
            survey_data += "               <div class='rows' id='row_1'>";
            survey_data += "                 <input type='text' name='row_matrix' placeholder='row label' class='inherit-width' style='max-width:50%;margin-top:5px; margin-left:1px;'>";
            survey_data += "                 <a href='javascript:void(0);' class='btn  matrix_remove_row que_" + question_sequence + "' id='1' style='display:none;margin-left:2px; margin-top:5px;'><i class='fa fa-times'></i></a>";
            survey_data += "                 <a href='javascript:void(0);' class='btn  matrix_add_row que_" + question_sequence + "' style='margin-left:2px; margin-top:5px;' id='1'><i class='fa fa-plus' ></i></a>";
            survey_data += "               </div>";
            $(survey_data).appendTo("#matrix_row_div_" + question_sequence);
        }
        ...
        if (questions["matrix_row"] == "{}") {
            //columns
            survey_data = '';
            survey_data += "               <div class='columns' id='column_0'>";
            survey_data += "                 <input type='text' name='column_matrix' placeholder='column label' class='inherit-width' style='max-width:50%; margin-left:1px;'>";
            survey_data += "                 <a href='javascript:void(0);' class='btn  matrix_remove_column que_" + question_sequence + "' id='0' style='display:none;margin-left:2px; margin-top:5px;'><i class='fa fa-times'></i></a>";
            survey_data += "                 <a href='javascript:void(0);' class='btn  matrix_add_column que_" + question_sequence + "' style='display:none;' id='0'><i class='fa fa-plus' ></i></a>";
            survey_data += "               </div>";
            survey_data += "               <div class='columns' id='column_1'>";
            survey_data += "                 <input type='text' name='column_matrix' placeholder='column label' class='inherit-width' style='max-width:50%;margin-top:5px; margin-left:1px;'>";
            survey_data += "                 <a href='javascript:void(0);' class='btn  matrix_remove_column que_" + question_sequence + "' id='1' style='display:none;margin-left:2px; margin-top:5px;'><i class='fa fa-times'></i></a>";
            survey_data += "                 <a href='javascript:void(0);' class='btn  matrix_add_column que_" + question_sequence + "' style='margin-left:2px; margin-top:5px;' id='1'><i class='fa fa-plus' ></i></a>";
            survey_data += "               </div>";
            $(survey_data).appendTo("#matrix_column_div_" + question_sequence);
     ...
     * @questions detail of questions
     */
    editviewTextBox: function (question_sequence, questions, fromCloneQue) {
        var self = this;
        var survey_data = '';
        //max allowed size & is required row start
        survey_data += '<div class="row">';
        ...
        if (questions['is_required'] == 'Yes') {
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "      <div class='span5'><input type='checkbox' name='is_required_" + questions['que_type'] + "_" + question_sequence + "' checked='checked' class='inherit-width' disabled/></div>";
            } else {
            ...
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "      <div class='span5'><input type='checkbox' name='is_required_" + questions['que_type'] + "_" + question_sequence + "' class='inherit-width' disabled/></div>";
            } else {
        ...
        if (questions['is_question_seperator'] == 'Yes') {
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "      <div class='span2'><input type='checkbox' name='is_question_seperator_" + questions['que_type'] + "_" + question_sequence + "' checked='checked' class='inherit-width' disabled/></div>";
            } else {
            ...
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "      <div class='span2'><input type='checkbox' name='is_question_seperator_" + questions['que_type'] + "_" + question_sequence + "' class='inherit-width' disabled/></div>";
            } else {
        ...
        survey_data += "       <div class='span1'>Max Size</div><div class='span5'>";
        if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
            survey_data += "          <input type='text' placeholder='Maximum Character Limit'  name='size_textbox_" + question_sequence + "'  class='inherit-width numericField' style='width:85%; margin-left:1px;' disabled>";
        } else {
            survey_data += "          <input type='text' placeholder='Maximum Character Limit'  name='size_textbox_" + question_sequence + "'  class='inherit-width numericField' style='width:85%; margin-left:1px;'>";
        }
        ...
        survey_data += "       <div class='span1'>Precision</div><div class='span5'>";
        if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
            survey_data += "           <input type='text' style='width:55%; margin-left: 1px;'  placeholder='Precision' name='precision_textbox_" + question_sequence + "'  class='inherit-width numericField' disabled>";
        } else {
        ...
        survey_data += "       <div class='span1'>Min Value</div><div class='span5'>";
        if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
            survey_data += "        <input type='text' style='width:55%; margin-left:1px;'  placeholder='Minimum value' name='min_textbox_" + question_sequence + "'  class='inherit-width numericField' disabled></div>";
        } else {
        ...
        survey_data += "      <div class='span1'>Max Value</div>";
        if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
            survey_data += "      <div class='span5'><input type='text' style='width:55%; margin-left:1px;'  placeholder='Maximum value' name='max_textbox_" + question_sequence + "'  class='inherit-width numericField' disabled></div>";
        } else {
...
//        {
            ...
            survey_data += "      <div class='span1 sync_field' style='" + is_hidden + "'>Sync Field </div>";
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "      <div class='span8 sync_field' style='" + is_hidden + "'><select class='sync_field_selection' name='sync_field_textbox_" + question_sequence + "' disabled>";
            } else {
                survey_data += "      <div class='span8 sync_field' style='" + is_hidden + "'><select class='sync_field_selection' name='sync_field_textbox_" + question_sequence + "'>";
            }
            survey_data += "            <option>Select Field</option>";
     ...
     * @questions detail of questions
     */
    editviewCommentBox: function (question_sequence, questions, fromCloneQue) {
        var self = this;
        var survey_data = '';
        // rows & cols allowed for question
        survey_data += "<div class='row'>";
        survey_data += "       <div class='span1'>Rows</div><div class='span5'>";
        if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
            survey_data += "        <input type='text' style='width:55%; margin-left:1px;' placeholder='Rows' name='rows_commentbox_" + question_sequence + "'  class='inherit-width numericField'disabled ></div>";
        } else {
        ...
        survey_data += "      <div class='span1'>Columns</div>";
        if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
            survey_data += "      <div class='span5'><input type='text' style='width:55%; margin-left:1px;' placeholder='Columns' name='cols_commentbox_" + question_sequence + "'  class='inherit-width numericField' disabled></div>";
        } else {
        ...
        survey_data += "       <div class='span1'>Max Size</div><div class='span5'>";
        if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
            survey_data += "          <input type='text' placeholder='Maximum Character Limit' name='size_" + questions['que_type'] + "_" + question_sequence + "'  class='inherit-width numericField' style='width:85%; margin-left:1px;' disabled></div>";
        } else {
            survey_data += "          <input type='text' placeholder='Maximum Character Limit' name='size_" + questions['que_type'] + "_" + question_sequence + "'  class='inherit-width numericField' style='width:85%; margin-left:1px;'></div>";
        }
...
//        {
            ...
            survey_data += "      <div class='span3 '>Disable Piping &nbsp;";
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "                         <input type='checkbox' style='margin-top:3px;' class='disable_piping_question' name='disable_piping_" + questions['que_type'] + "_" + question_sequence + "' " + is_checked + " disabled/> </div>";
            } else {
            ...
            survey_data += "      <div class='span1 sync_field' style='" + is_hidden + "'>Sync Field </div>";
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "      <div class='span8 sync_field' style='" + is_hidden + "'><select class='sync_field_selection' name='sync_field_" + questions['que_type'] + "_" + question_sequence + "' disabled>";
            } else {
     ...
     * @questions detail of questions
     */
    editviewRichTextBox: function (question_sequence, questions, fromCloneQue) {
        var self = this;
        var survey_data = '';
        survey_data += "<div class='row'>";
        // Add question seperator.
     ...
     * @questions detail of questions
     */
    editviewBoolean: function (question_sequence, questions, fromCloneQue) {
        var self = this;
        var survey_data = '';
        //max allowed size & is required row start
        survey_data += '<div class="row">';
        ...
        if (questions['is_required'] == 'Yes') {
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "      <div class='span2'><input type='checkbox' name='is_required_" + questions['que_type'] + "_" + question_sequence + "' checked='checked' class='inherit-width' disabled/></div>";
            } else {
            ...
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "      <div class='span2'><input type='checkbox' name='is_required_" + questions['que_type'] + "_" + question_sequence + "' class='inherit-width' disabled/></div>";
            } else {
...
//        {
            ...
            if (this.sync_module_fields_for_boolean && typeof this.sync_module_fields_for_boolean[self.model.get('sync_module')] != "undefined")
            {
                if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                    survey_data += " <select class='sync_field_selection boolean_sync_field' name='sync_field_" + questions['que_type'] + "_" + question_sequence + "' disabled>           <option>Select Field</option>";
                } else {
                ...
                $.each(this.sync_module_fields_for_boolean[self.model.get('sync_module')], function (key, field) {
                    // Piping tab contents
                    var is_checked = '';
                    if (questions['sync_field'] == key)
                    {
                ...
                if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                    survey_data += " <select class='sync_field_selection boolean_sync_field' style='display:none' name='sync_field_" + questions['que_type'] + "_" + question_sequence + "' disabled>           <option>Select Field</option>";
                } else {
     ...
     * @questions detail of questions
     */
    editviewRating: function (question_sequence, questions, fromCloneQue) {
        var self = this;
        var survey_data = '';
        //max allowed size & is required row start
        survey_data += '<div class="row">';
        ...
        survey_data += "       <div class='span1'>Star Numbers</div><div class='span5'>";
        if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
            survey_data += "          <select name='starNo_rating_" + question_sequence + "'  class='inherit-width' style='width:40%; margin-left:1px;' disabled>";
        } else {
            survey_data += "          <select name='starNo_rating_" + question_sequence + "'  class='inherit-width' style='width:40%; margin-left:1px;'>";
        }
        survey_data += "             <option value='0'>select</option>";
...
//        {
     ...
     * @questions detail of questions
     */
    editviewContactInformation: function (question_sequence, questions, fromCloneQue) {
        var self = this;
        var survey_data = '';
        //is required or not
        survey_data += '<div class="row">';
        ...
        if (questions['is_required'] == 'Yes') {
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "      <div class='span5'><input type='checkbox' name='is_required_" + questions['que_type'] + "_" + question_sequence + "' checked='checked' class='inherit-width is_required' disabled/></div>";
            } else {
            ...
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "      <div class='span5'><input type='checkbox' name='is_required_" + questions['que_type'] + "_" + question_sequence + "' class='inherit-width is_required' disabled/></div>";
            } else {
        ...
        if ($.inArray("Name", requireFields) != -1) {
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "                    <input type='checkbox' name='Name_" + question_sequence + "' checked disabled><span>Name</span>";
            } else {
                survey_data += "                    <input type='checkbox' name='Name_" + question_sequence + "' checked><span>Name</span>";
            }
        } else {
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "                    <input type='checkbox' name='Name_" + question_sequence + "' disabled><span>Name</span>";
            } else {
        ...
        if ($.inArray("Email", requireFields) != -1) {
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "                    <input type='checkbox' name='Email_" + question_sequence + "' checked disabled><span>Email Address</span>";
            } else {
                survey_data += "                    <input type='checkbox' name='Email_" + question_sequence + "' checked><span>Email Address</span>";
            }
        } else {
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "                    <input type='checkbox' name='Email_" + question_sequence + "' disabled><span>Email Address</span>";
            } else {
        ...
        if ($.inArray("Company", requireFields) != -1) {
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "                    <input type='checkbox' name='Company_" + question_sequence + "' checked disabled><span>Company</span>";
            } else {
                survey_data += "                    <input type='checkbox' name='Company_" + question_sequence + "' checked><span>Company</span>";
            }
        } else {
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "                    <input type='checkbox' name='Company_" + question_sequence + "' disabled><span>Company</span>";
            } else {
        ...
        if ($.inArray("Phone", requireFields) != -1) {
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "                    <input type='checkbox' name='Phone_" + question_sequence + "' checked disabled><span>Phone Number</span>";
            } else {
                survey_data += "                    <input type='checkbox' name='Phone_" + question_sequence + "' checked><span>Phone Number</span>";
            }
        } else {
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "                    <input type='checkbox' name='Phone_" + question_sequence + "' disabled><span>Phone Number</span>";
            } else {
        ...
        if ($.inArray("Address", requireFields) != -1) {
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "                    <input type='checkbox' name='Address_" + question_sequence + "' checked disabled><span>Street1</span>";
            } else {
                survey_data += "                    <input type='checkbox' name='Address_" + question_sequence + "' checked><span>Street1</span>";
            }
        } else {
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "                    <input type='checkbox' name='Address_" + question_sequence + "' disabled><span>Street1</span>";
            } else {
        ...
        if ($.inArray("Address2", requireFields) != -1) {
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "                    <input type='checkbox' name='Address2_" + question_sequence + "' checked disabled><span>Street2</span>";
            } else {
                survey_data += "                    <input type='checkbox' name='Address2_" + question_sequence + "' checked><span>Street2</span>";
            }
        } else {
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "                    <input type='checkbox' name='Address2_" + question_sequence + "' disabled><span>Street2</span>";
            } else {
        ...
        if ($.inArray("City", requireFields) != -1) {
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "                    <input type='checkbox' name='City_" + question_sequence + "' checked disabled><span>City/Town</span>";
            } else {
                survey_data += "                    <input type='checkbox' name='City_" + question_sequence + "' checked><span>City/Town</span>";
            }
        } else {
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "                    <input type='checkbox' name='City_" + question_sequence + "' disabled><span>City/Town</span>";
            } else {
        ...
        if ($.inArray("State", requireFields) != -1) {
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "                    <input type='checkbox' name='State_" + question_sequence + "' checked disabled><span>State/ Province</span>";
            } else {
                survey_data += "                    <input type='checkbox' name='State_" + question_sequence + "' checked><span>State/ Province</span>";
            }
        } else {
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "                    <input type='checkbox' name='State_" + question_sequence + "' disabled><span>State/ Province</span>";
            } else {
        ...
        if ($.inArray("Zip", requireFields) != -1) {
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "                    <input type='checkbox' name='Zip_" + question_sequence + "' checked disabled><span>Zip/ Postal Code</span>";
            } else {
                survey_data += "                    <input type='checkbox' name='Zip_" + question_sequence + "' checked><span>Zip/ Postal Code</span>";
            }
        } else {
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "                    <input type='checkbox' name='Zip_" + question_sequence + "' disabled><span>Zip/ Postal Code</span>";
            } else {
        ...
        if ($.inArray("Country", requireFields) != -1) {
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "                    <input type='checkbox' name='Country_" + question_sequence + "' checked disabled><span>Country</span>";
            } else {
                survey_data += "                    <input type='checkbox' name='Country_" + question_sequence + "' checked><span>Country</span>";
            }
        } else {
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "                    <input type='checkbox' name='Country_" + question_sequence + "' disabled><span>Country</span>";
            } else {
...
//        {
     ...
     * @questions detail of questions
     */
    editviewMultiChoice: function (question_sequence, questions, fromCloneQue) {
        var self = this;
        var survey_data = '';
        //is required or not
        survey_data += '<div class="row">';
        ...
        if (questions['que_type'] == 'multiselectlist' || questions['que_type'] == 'check-box')
        {
            ...
            survey_data += "      <div class='span1'>Selection Limit </div>";
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "<div class='span4'><select name='limit_min_" + questions['que_type'] + "_" + question_sequence + "' class='inherit-width selectionLimit' style='width:40%;' disabled>";
            } else {
                survey_data += "<div class='span4'><select name='limit_min_" + questions['que_type'] + "_" + question_sequence + "' class='inherit-width selectionLimit' style='width:40%;'>";
            }
            survey_data += "      <option value='0'>Select</option>";
...
//        {
     ...
     * @questions detail of questions
     */
    editviewDatetime: function (question_sequence, questions, fromCloneQue) {
        var self = this;
        var survey_data = '';
        //is required or not
        survey_data += '<div class="row">';
        ...
        if (questions['is_required'] == 'Yes') {
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "      <div class='span1'><input type='checkbox' name='is_required_" + questions['que_type'] + "_" + question_sequence + "' checked='checked' class='inherit-width' disabled/></div>";
            } else {
            ...
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "      <div class='span1'><input type='checkbox' name='is_required_" + questions['que_type'] + "_" + question_sequence + "' class='inherit-width' disabled/></div>";
            } else {
        ...
        //sorting
        survey_data += "      <div class='span1' style='margin-top: 5px;margin-bottom: 4px;'>Show Time</div>";
        if (questions['sync_field'])
        {
            var disabled = 'disabled';
        } else {
        ...
        if (questions['is_datetime'] == 'No') {
            ...
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "      <div class='span1'><input type='checkbox' " + disabled + "  name='is_datetime_" + questions['que_type'] + "_" + question_sequence + "' checked class='inherit-width' disabled/></div>";
            } else {
                survey_data += "      <div class='span1'><input type='checkbox' " + disabled + "  name='is_datetime_" + questions['que_type'] + "_" + question_sequence + "' checked class='inherit-width'/></div>";
            }
        }
        // Add question seperator.
        survey_data += "      <div class='span2' style='margin-top: 5px;margin-bottom: 4px;'>Add Question Separator?</div>";
        ...
        if (questions['allow_future_dates'] == 'No') {
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "      <div class='span2'><input type='checkbox' name='allow_future_dates_" + questions['que_type'] + "_" + question_sequence + "' class='inherit-width' disabled/></div>";
            } else {
            ...
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "      <div class='span2'><input type='checkbox' name='allow_future_dates_" + questions['que_type'] + "_" + question_sequence + "' checked='checked' class='inherit-width' disabled/></div>";
            } else {
        ...
        if (questions['min'])
        {
            ...
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                ...
                survey_data += '        <input name="startDate_' + questions['que_type'] + '_' + question_sequence + '" style = "width:85%;margin-left:3px;margin-top:5px;" name = "show_startdatepicker" id="show_startdatepicker" class = "show_datepicker datepicker" type = "text" data-type = "date" class = "ui-timepicker-input" placeholder = "Date" aria-label = "Start Date">';
            }
        }
        survey_data += '        <span name = "date_error" class = "error-tooltip add-on " style = "display:none;" data-container = "body" rel = "tooltip"  title = "Error. The date of this field must be after current Date." > <i class = "fa fa-exclamation-circle" > </i></span >';
        survey_data += '        <span class = "add-on date" id="dateicon" data-icon = "calendar" style="margin-top:5px;"> <i class = "fa fa-calendar" > </i></span >';
        ...
        if (questions['max']) {
            ...
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                ...
                survey_data += '        <input name="endDate_' + questions['que_type'] + '_' + question_sequence + '" style = "width:55%;margin-left:3px;margin-top:5px;" name = "show_enddatepicker" id="show_enddatepicker" class = "show_datepicker datepicker" type = "text" data-type = "date" class = "ui-timepicker-input" placeholder = "Date" aria-label = "Start Date">';
            }
        }
        survey_data += '        <span name = "date_error" class = "error-tooltip add-on " style = "display:none;" data-container = "body" rel = "tooltip"  title = "Error. The date of this field must be after Start Date." > <i class = "fa fa-exclamation-circle" > </i></span >';
        survey_data += '        <span class = "add-on date" id="dateicon" data-icon = "calendar" style="margin-top:5px;"> <i class = "fa fa-calendar" > </i></span >';
...
//        {
     ...
     * @questions detail of questions
     */
    editviewVideo: function (question_sequence, questions, fromCloneQue) {
        var self = this;
        var survey_data = '';
...
//        {
     ...
     * @questions detail of questions
     */
    editviewImage: function (question_sequence, questions, fromCloneQue) {
        var self = this;
        var survey_data = '';
        // Add question seperator.
        survey_data += '<div class="row">';
...
//        {
     ...
     * @questions detail of questions
     */
    editviewScale: function (question_sequence, questions, fromCloneQue) {
        var self = this;
        var survey_data = '';
        //max allowed size & is required row start
        var labels = questions['advance_type'] != null ? questions['advance_type'].split('-') : '';
...
//        {
     ...
     * @questions detail of questions
     */
    editviewNetPromoterScore: function (question_sequence, questions, fromCloneQue) {
        var self = this;
        var survey_data = '';
        //max allowed size & is required row start
        var labels = questions['advance_type'] != null ? questions['advance_type'].split('-') : '';
...
//        {
     ...
     * @questions detail of questions
     */
    editviewEmojis: function (question_sequence, questions, fromCloneQue) {
        var self = this;
        var survey_data = '';
        //is_required And Add Question Sep
        survey_data += '<div class="row">';
     ...
     * @questions detail of questions
     */
    editviewMatrix: function (question_sequence, questions, fromCloneQue) {
        var self = this;
        var survey_data = '';
        //is required or not
        survey_data += '<div class="row">';
        ...
        if (questions['advance_type'] == 'radio') {
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "      <input type='radio' style='margin-right: 0px;' value='radio' name='display_type_" + questions['que_type'] + "_" + question_sequence + "' checked='checked' class='inherit-width'/><a href='javascript:void(0);' class='btn' style='margin-bottom-6px;margin-left-6px;'> <i class='fa fa-dot-circle-o' disabled></i>&nbsp; Radio</a></i>&nbsp;&nbsp;";
            } else {
            ...
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "      <input type='radio' style='margin-right: 0px;' value='radio' name='display_type_" + questions['que_type'] + "_" + question_sequence + "' class='inherit-width'/><a href='javascript:void(0);' class='btn' style='margin-bottom-6px;margin-left-6px;'> <i class='fa fa-dot-circle-o' disabled></i>&nbsp; Radio</a></i>&nbsp;&nbsp;";
            } else {
            ...
            if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
                survey_data += "      <input type='radio' style='margin-right: 0px;' value='radio' name='display_type_" + questions['que_type'] + "_" + question_sequence + "' checked='checked' class='inherit-width'/><a href='javascript:void(0);' class='btn' style='margin-bottom-6px;' dsiabled> <i class='fa fa-dot-circle-o' disabled></i>&nbsp; Radio</a></i>&nbsp;&nbsp;";
            } else {
...
//        {
     ...
     * @questions detail of questions
     */
    editviewDocAttachment: function (question_sequence, questions, fromCloneQue) {
        var self = this;
        var survey_data = '';
        //max allowed size & is required row start
        survey_data += '<div class="row">';
        ...
        survey_data += "      <div class='span5'>";
        if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
            survey_data += "<select id='file_size_" + question_sequence + "' name='file_size_" + questions['que_type'] + "_" + question_sequence + "' disabled>";
        } else {
            survey_data += "<select id='file_size_" + question_sequence + "' name='file_size_" + questions['que_type'] + "_" + question_sequence + "'>";
        }
        $.each(file_sizeBytes, function (bytes, labelMB) {
        ...
        survey_data += "      <div class='span3' style='position: relative;'>";
        if (self.submissionCount_edit == "true" && (fromCloneQue == false || typeof fromCloneQue == "undefined")) {
            survey_data += "<select id='file_extension_" + question_sequence + "' name='file_extension_" + questions['que_type'] + "_" + question_sequence + "' multiple='true' disabled>";
        } else {
            survey_data += "<select id='file_extension_" + question_sequence + "' name='file_extension_" + questions['que_type'] + "_" + question_sequence + "' multiple='true'>";
        }
        var selectedExts = valid_fileext;
    ...
    disable_piping_question: function (el) {
        ...
        if ($(el.currentTarget).attr('checked') == 'checked')
        {
            ...
            if ($(el.currentTarget).parents('.question').find('.boolean_sync_field').length == 0)
            {
                ...
                {
                    survey_data += "   <input type='number' name='score_" + current_que_type + "'  value='1' class='inherit-width score_weight' style='max-width:7%; margin-left: 4px;margin-top: 5px;' min='0' onpaste='return false;'>";
                } else {
                    survey_data += "   <input type='number' name='score_" + current_que_type + "'  value='1' class='inherit-width score_weight' style='max-width:7%; margin-left: 4px;margin-top: 5px;display:none;' min='0' onpaste='return false;'>";
                }
                //remove option button
                ...
                if (enable_scoring == 'Yes')
                {
                    survey_data += "   <input type='number' name='score_" + current_que_type + "'  value='2' class='inherit-width score_weight' style='max-width:7%; margin-left: 4px;margin-top: 5px;' min='0' onpaste='return false;'>";
                } else {
                    survey_data += "   <input type='number' name='score_" + current_que_type + "'  value='2' class='inherit-width score_weight' style='max-width:7%; margin-left: 4px;margin-top: 5px;display:none;' min='0' onpaste='return false;'>";
                }
                //remove option button
    ...
    check_syn_field_type_convert_question_type: function (el) {
        ...
        if (duplicate_count > 1)
        {
            app.alert.show('error', {
                level: 'error',
                messages: 'Selected sync field is already synced with another survey question.',
                autoClose: true
            });
                                                ...
                                                if (data['options'])
                                                {
                                                    ...
                                                    $.each(data['options'], function (key, option) {
                                                        if (option)
                                                        {
                                                            ...
                                                            answer_key['option_' + count] = key;
                                                        }
                                                    });
                                                }
                                                self.confirmEditQueType(current_que_type, current_que_seq, data['correct_que_type'], question, is_required, helptips, display_boolean_label, question_id, answer_detail, [], sync_module, sync_field, answer_key);
    ...
    reset_sync_field: function (el) {
        ...
        app.alert.show('change_sync_module_confirmation', {
            ...
            onConfirm: function () {
                $.each($(el.currentTarget).parents('.main-pane').find('.question'), function () {
                    ...
                    if ($(this).find('.sync_field_selection').val() && $(this).find('.sync_field_selection').val() != 'Select Field') {
                        ...
                        if (muti_choice_que_type != 'boolean')
                        {
                            ...
                            data += "               </div>";
                            data += "               <div class='options' id='option_1'>";
                            data += "                 <input type='text' name='option_" + muti_choice_que_type + "' placeholder='Option' class='inherit-width' style='margin-left: 4px;max-width:50%;margin-top:5px;'>";
                            data += "                 <input type='number' name='score_" + muti_choice_que_type + "' value='2' class='inherit-width score_weight' style='max-width:7%; margin-left: 4px;margin-top: 5px;display:none;' min='0' onpaste='return false;'>";
                            data += "                 <a href='javascript:void(0);' class='btn  " + muti_choice_que_type + "_remove_option que_" + current_que_seq + "' id='1' style='display:none;margin-left:2px; margin-top:5px;'><i class='fa fa-times'></i></a>";
                            data += "                 <a href='javascript:void(0);' class='btn  " + muti_choice_que_type + "_add_option que_" + current_que_seq + "' style='margin-left:2px; margin-top:5px;' id='1'><i class='fa fa-plus' ></i></a>";
                            data += "               </div>";
                ...
                $(el.currentTarget).parents('.main-pane').find('[name=sync_module]').parent().find('#recent_sync_module').val(self.model.get('sync_module'));
                var sync_field_options = '<option>Select Field</option>';
                var sync_field_based_on_queType = self.sync_module_fields;

                $.each(sync_field_based_on_queType[self.model.get('sync_module')], function (key, field) {
                    // Piping tab contents
                ...
                if (typeof sync_field_based_on_queType[self.model.get('sync_module')] != "undefined")
                {
                    $.each(sync_field_based_on_queType[self.model.get('sync_module')], function (key, field) {
                        // Piping tab contents
                        sync_field_options += "        <option value='" + key + "' >" + field + "</option>";
    ...
    show_datepicker: function (el) {
        ...
        if ($(el.currentTarget).prop('id') != "dateicon") {
            $(el.currentTarget).datepicker(options).datepicker("show");
        } else {
            $('#' + $(el.currentTarget).parent().find('input').prop('id')).focus();
            $(el.currentTarget).parent().find('input').datepicker(options).datepicker("show");
        }
    ...
    show_ImageUploadType: function (el) {
        ...
        if (el.currentTarget.classList.contains("uploadsImage") == true)
        {
            ...
            if ($(el.currentTarget).parents('.general_options').find('.image-error-msg').length != 0) {
                $(el.currentTarget).parents('.general_options').find('.image-error-msg').remove();
                $(el.currentTarget).parents('.general_options').find('.uploadedImage').removeProp('src');
                $(el.currentTarget).parents('.general_options').find('.uploadSurveyImage').val('');
            }
            ...
            $(el.currentTarget).parents('.general_options').find('.uploadSurveyImage').removeClass('error-custom').parent().find('span').attr('style', 'display:none;');
        }
    },
    /**
    ...
    dropDownEditQueType: function (questions, question_sequence) {
        ...
        survey_data += '<select class="edit_que-type" style="width: 100%;" id="edit_que_type_' + questions['que_type'] + '_' + question_sequence + '" style="font-family: \'FontAwesome\', Helvetica; ">';
        ...
        survey_data += '>Matrix / Grid </i></option>';
    ...
    handleFileSelect: function (evt) {
        ...
        for (var i = 0, f; f = files[i]; i++) {
            ...
            if (!f.type.match('image.*')) {
                ...
                {
                    parent.find('.uploadSurveyImage').parent().append('<p class="image-error-msg" style="color:red; font-size:11px;">Please upload images of only jpeg, png or gif type.</p>');
                    if (parent.find('.uploadSurveyImage').parent().find('img').css('display') != "none") {
                        parent.find('.uploadSurveyImage').parent().find('img').css('display', 'none');
                    }
                }
                continue;
                ...
                if (parent.find('.uploadSurveyImage').parent().find('.image-error-msg').length == 0)
                {
                    parent.find('.uploadSurveyImage').parent().append('<p class="image-error-msg" style="color:red; font-size:11px;">Please upload file with size upto 500 KB.');
                    if (parent.find('.uploadSurveyImage').parent().find('img').css('display') != "none") {
                        parent.find('.uploadSurveyImage').parent().find('img').css('display', 'none');
                    }
                }
            ...
            reader.onload = (function (theFile) {
                return function (e) {
                    ...
                    if (theFile.type == 'image/png' || theFile.type == 'image/jpeg' || theFile.type == 'image/gif')
                    {
                        ...
                        self.image_content = '';
                        //parent.find('.uploadSurveyImage').parent().append('<p style="color:red; font-size:11px;">Please upload images of only jpeg, png or gif type.</p>')
                    }
                };
            })(f);
    ...
    handleFileSelectRadioUpload: function (evt) {
        ...
        for (var i = 0, f; f = files[i]; i++) {
            ...
            if (!f.type.match('image.*')) {
                ...
                currentFileUpload.parent().find('.spanRadioUploadError').find('i').attr('title', 'Please upload images of only jpeg, png or gif type.');
                ...
                currentFileUpload.parent().find('.spanRadioUploadError').find('i').attr('title', 'Please upload file with size upto 500 KB.');
    ...
    /**
     * function for disabling the question in survey Form 7th August 2019
     * @el current element
     */
    disabled_icon_clicked: function (el) {
        var self = this;
        //fetch id of remove icon clicked 
        var val = el.currentTarget.childNodes['0'].id;

        var not_disabledQuestionPerPage = $('#' + val).parents('.row').parent().parent().parent().find('.disable_que').length;
        var disabledQuestionPerPage = $('#' + val).parents('.row').parent().parent().parent().find('.questionDisabled').length;
        var TotalQuestionPerPage = $('#' + val).parents('.row').parent().parent().parent().find('.question.survey-body').length;
        if (!val) {
            val = el.currentTarget.parentElement.id;
        }

        if (val.search("disable_section_que_") >= 0) {
            //fetch id of question div to remove current question
            var que_id = '.que_' + val.split('_')[3];
            app.alert.show('disable_question', {
                level: 'confirmation',
                messages: "Are you sure to disable this section header?",
                onConfirm: function () {
                    $('#' + val).parent().addClass('sectionDisabled');
                    $('#' + val).parent().hide();
                    $('#' + val).parents('.row').find('.enable_section_que').show();
                },
                onCancel: function () {
                },
                autoClose: false
...
//                }
            ...
            if ((TotalQuestionPerPage - 1) == disabledQuestionPerPage) {
                alert("Atleast one question is required.");
            } else {
                //remove questions
                if (val.search("disable_que_") >= 0) {
                    //fetch id of question div to remove current question
                    var que_id = '.que_' + val.split('_')[2];
                    if (self.$el.parents('.main-pane').find(que_id).prev().hasClass('question-section'))
                    {
                        app.alert.show('disable_question', {
                            level: 'confirmation',
                            messages: "Disabling this question will also disable its section header. Are you sure to disable this question ?",
                            onConfirm: function () {
                                $('#' + val).parent().addClass('questionDisabled');
                                $('#' + val).parent().hide();
                                $('#' + val).parents(que_id).addClass('que_disabled');
                                $('#' + val).parents(que_id).prev().find('.disable_section_que').hide();
                                $('#' + val).parents('.row').find('.enable_que').show();
                                $('#' + val).parents('.row').find('.enable_que').find('.fa-lock').css('color', 'red');
                                $('#' + val).parents(que_id).prev().find('.enable_section_que').show();
                                $('#' + val).parents(que_id).prev().addClass('sec_disabled');
//                            var id = 'que_' + val.split('_')[2];
//                            var remove_que_id = self.$el.parents('.main-pane').find('.' + id).find('.que_id').val();
//                            if (remove_que_id) {
//                                self.remove_que_ids.push(remove_que_id);
...
//                            self.$el.parents('.main-pane').find(que_id).remove(); // remove question
                        ...
                        app.alert.show('disable_question', {
                            level: 'confirmation',
                            messages: "Are you sure to disable this question?",
                            onConfirm: function () {
                                $('#' + val).parent().addClass('questionDisabled');
                                $('#' + val).parent().hide();
                                $('#' + val).parents(que_id).addClass('que_disabled');
     ...
     * @el current element
     */
    enable_icon_clicked: function (el) {
        var self = this;
        //fetch id of remove icon clicked 
        var val = el.currentTarget.childNodes['0'].id;
        ...
        //enable question
        if (val.search("enable_que_") >= 0) {
            //fetch id of question div to remove current question
            var que_id = '.que_' + val.split('_')[2];
            if (self.$el.parents('.main-pane').find(que_id).prev().hasClass('question-section'))
            {
                app.alert.show('enable_question', {
                    level: 'confirmation',
                    messages: "Enabling this question will also enable its section header. Are you sure to enable this question ?",
                    onConfirm: function () {
                        $('#' + val).parent().hide();
                        $('#' + val).parents('.row').find('.questionDisabled').show();
                        $('#' + val).parents(que_id).prev().find('.disable_section_que').show();
                        $('#' + val).parents('.row').find('.questionDisabled').removeClass('questionDisabled');
                        $('#' + val).parents(que_id).prev().find('.enable_section_que').hide();
                        $('#' + val).parents(que_id).removeClass('que_disabled');
                        $('#' + val).parents(que_id).prev().removeClass('sec_disabled');
                    },
                    onCancel: function () {
                    },
                    autoClose: false
                ...
                app.alert.show('enable_question', {
                    level: 'confirmation',
                    messages: "Are you sure to enable this question?",
                    onConfirm: function () {
                        $('#' + val).parent().hide();
                        $('#' + val).parents('.row').find('.questionDisabled').show();
                        $('#' + val).parents(que_id).removeClass('que_disabled');
        ...
        } else if (val.search("enable_section_que_") >= 0) {
            //fetch id of question div to remove current question
            var val = el.currentTarget.childNodes['0'].id;
            var que_id = '#section_' + val.split('_')[3];
            var AfterSectionDisabledQuestionCount = self.$el.parents('.main-pane').find(que_id).nextAll('.que_disabled').length;
            var AfterSectionQuestionCount = self.$el.parents('.main-pane').find(que_id).nextAll('.question.survey-body').length;
            if (AfterSectionDisabledQuestionCount < AfterSectionQuestionCount)
            {
                app.alert.show('enable_question', {
                    level: 'confirmation',
                    messages: "Are you sure to enable this section Header?",
                    onConfirm: function () {
                        $('#' + val).parent().hide();
                        $('#' + val).parents('.row').find('.sectionDisabled').show();
                        $('#' + val).parents('.row').find('.sectionDisabled').removeClass('sectionDisabled');
                        $('#' + val).parents('.sec_disabled').removeClass('sec_disabled');
                    },
                    onCancel: function () {
                    },
                    autoClose: false
                ...
                alert("Section Header cannot be last.")
            }
        }
