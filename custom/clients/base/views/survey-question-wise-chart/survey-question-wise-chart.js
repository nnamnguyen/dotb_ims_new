 ...
 * @alias DOTB.App.view.views.BaseSavedReportsChartView
 * @extends View.View
 */
({
    plugins: ['Dashlet'],
    ...
    bindDataChange: function () {
        if (this.meta.config) {
            ...
            this.settings.on('change:saved_report_question', function (model) {
                var reportId = model.get('saved_report_id');
                var queId = model.get('saved_report_question');
                var options;
                if (_.isEmpty(reportId)) {
                    return;
                }
                ...
                this.getSavedReportById(reportId, options, queId);
            }, this);
    ...
    loadData: function (options) {
        ...
        var queId = this.settings.get('saved_report_question');
        var show_all_transactions = this.settings.get('show_all_transactions');
        ...
        if (!_.isEmpty(reportId)) {
            ...
            this.getSavedReportById(reportId, options, queId, show_all_transactions);
        }
    },
    ...
    setChartParams: function (serverData, update) {
        ...
        // persist the chart settings for use by DotbCharts
    ...
    chartComplete: function (chart, params, reportData, chartData) {
        ...
        // in dotbCharts for use in the Report module charts
        chart.seriesClick(_.bind(function (data, eo, chart, labels) {
            var state = DOTB.charts.buildChartState(eo, labels);
            if (!_.isFinite(state.seriesIndex)) {
                return;
            }

            if (params.chart_type === 'line chart') {
                params.groupLabel = DOTB.charts.extractSeriesLabel(state, data);
                params.seriesLabel = DOTB.charts.extractGroupLabel(state, labels);
            } else {
                params.seriesLabel = DOTB.charts.extractSeriesLabel(state, data);
                params.groupLabel = DOTB.charts.extractGroupLabel(state, labels);
            }
    ...
    _handleFilter: function (chart, params, state, reportData, chartData) {
        ...
        if (this.$el.parents('.drawer.active').length === 0) {
            enums = DOTB.charts.getEnums(reportData);
            groupDefs = DOTB.charts.getGrouping(reportData);
            drawerContext = {
                chartData: chartData,
                chartModule: module,
                chartState: state,
     ...
     */
    getSavedReportById: function (reportId, options, queId, show_all_transactions) {
        ...
        var module_name = '';
        if (this.model && this.model.attributes._module) {
            module_name = this.model.attributes._module;
        }
        if (dt) {
        ...
        var url = App.api.buildURL("bc_survey", "dashlet_survey_question_chart", "", {survey_id: reportId, que_id: queId, module_name: module_name, record_id: record_id, show_all_transactions: show_all_transactions});
 ...
 * serverData
 * 
 * {"reportData":{"label":"Survey Question wise chart","id":"3d565eaa-d1d9-11e8-afbf-408d5cdfda91","summary_columns":[{"name":"name","label":"Survey Question","table_key":"self"},{"name":"name","label":"Survey Answer","table_key":"bc_survey_submit_question:bc_survey_submit_question_bc_survey_answers"},{"name":"count","label":"Count","field_type":"","group_function":"count","table_key":"bc_survey_submit_question:bc_survey_submit_question_bc_survey_answers"}],"group_defs":[{"name":"name","label":"Name","table_key":"self","type":"name"},{"name":"name","label":"Name","table_key":"bc_survey_submit_question:bc_survey_submit_question_bc_survey_answers","type":"name"}],"filters_def":{"Filter_1":{"0":{"name":"survey_title","table_key":"self","qualifier_name":"equals","runtime":1,"input_name0":"Case Handler","input_name1":"on"},"1":{"name":"name","table_key":"bc_survey_submit_question:bc_survey_questions_bc_survey_submit_question_1","qualifier_name":"is","runtime":1,"input_name0":"bbb4c876-d112-11e8-a829-408d5cdfda91","input_name1":"Issue"},"operator":"AND"}},"base_module":"bc_survey_submit_question","full_table_list":{"self":{"value":"bc_survey_submit_question","module":"bc_survey_submit_question","label":"bc_survey_submit_question","dependents":[]},"bc_survey_submit_question:bc_survey_submit_question_bc_survey_answers":{"name":"bc_survey_submit_question  >  Survey Answers","parent":"self","link_def":{"name":"bc_survey_submit_question_bc_survey_answers","relationship_name":"bc_survey_submit_question_bc_survey_answers","bean_is_lhs":true,"link_type":"many","label":"Survey Answers","module":"bc_survey_answers","table_key":"bc_survey_submit_question:bc_survey_submit_question_bc_survey_answers"},"dependents":["group_by_row_2","display_summaries_row_group_by_row_2","display_cols_row_8","group_by_row_2","display_summaries_row_group_by_row_2","group_by_row_2","display_summaries_row_group_by_row_2","group_by_row_2","display_summaries_row_group_by_row_2","display_cols_row_3","group_by_row_2","display_summaries_row_group_by_row_2","display_cols_row_3","group_by_row_2","display_summaries_row_group_by_row_2","display_cols_row_4","group_by_row_2","display_summaries_row_group_by_row_2","display_cols_row_4","group_by_row_2","display_summaries_row_group_by_row_2","display_cols_row_4","group_by_row_2","display_summaries_row_group_by_row_2","display_cols_row_5","group_by_row_2","display_summaries_row_group_by_row_2","display_cols_row_5","group_by_row_2","display_summaries_row_group_by_row_2","display_cols_row_5","display_summaries_row_9"],"module":"bc_survey_answers","label":"Survey Answers"},"bc_survey_submit_question:bc_survey_questions_bc_survey_submit_question_1":{"name":"bc_survey_submit_question  >  Survey Questions","parent":"self","link_def":{"name":"bc_survey_questions_bc_survey_submit_question_1","relationship_name":"bc_survey_questions_bc_survey_submit_question_1","bean_is_lhs":false,"link_type":"one","label":"Survey Questions","module":"bc_survey_questions","table_key":"bc_survey_submit_question:bc_survey_questions_bc_survey_submit_question_1"},"dependents":["display_cols_row_8","display_cols_row_7","display_cols_row_3","display_cols_row_3","Filter.1_table_filter_row_2","Filter.1_table_filter_row_2","display_cols_row_3"],"module":"bc_survey_questions","label":"Survey Questions"},"bc_survey_submit_question:bc_survey_submit_question_bc_survey_submission":{"name":"bc_survey_submit_question  >  Survey Transactions","parent":"self","link_def":{"name":"bc_survey_submit_question_bc_survey_submission","relationship_name":"bc_survey_submit_question_bc_survey_submission","bean_is_lhs":false,"link_type":"one","label":"Survey Transactions","module":"bc_survey_submission","table_key":"bc_survey_submit_question:bc_survey_submit_question_bc_survey_submission"},"dependents":["display_cols_row_8","display_cols_row_8","display_cols_row_8"],"module":"bc_survey_submission","label":"Survey Transactions"}}},"chartData":{"properties":[{"title":"Total is 3","subtitle":"","type":"horizontal group by chart","legend":"on","labels":"value","print":"on","thousands":"","base_module":"bc_survey_submit_question","label":"Survey Question wise chart","allow_drillthru":"1","groupName":"Name","groupType":"string","seriesName":"Name","seriesType":"string","xDataType":"ordinal","yDataType":""}],"label":[""],"color":["#8c2b2b","#468c2b","#2b5d8c","#cd5200","#e6bf00","#7f3acd","#00a9b8","#572323","#004d00","#000087","#e48d30","#9fba09","#560066","#009f92","#b36262","#38795c","#3D3D99","#99623d","#998a3d","#994e78","#3d6899","#CC0000","#00CC00","#0000CC","#cc5200","#ccaa00","#6600cc","#005fcc"],"values":[{"label":"Issue","gvalue":"3","gvaluelabel":"3","values":[3],"valuelabels":["3"],"links":[""]}]}}
 */
