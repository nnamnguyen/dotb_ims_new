/**
 * The file used to set custom dashlet for Survey Send Report
 *
...
({
    ...
    className: 'survey-send-status-wrapper',
...
//        this.meta.label = this.meta.saved_report + ' : Send Report';
        // Call Quick Send Report in Dashlet
    ...
    bindDataChange: function () {
        ...
        if (this.meta.config) {
            this.settings.on('change:saved_report_id', function (model) {
                ...
                //this.meta.label = model.get('saved_report') + ' : Send Report';
                ...
                options = {
                    success: function (data) {
                        ...
                        label = $('[name="label"]');
                        if (label.length) {
                            label.val(model.get('saved_report') + ' : Send Status Report');
                        }
                    }
                };
...
//        this.displayNoData(!this.chart_loaded);
    ...
    processSurveyStatus: function (data) {
        ...
        var status2css = {
                ...
                stati = {"0": "Published", "1": "Unpublished", "2": "Total"},
                statusOptions = {"Published": "Published", "Unpublished": "Unpublished", "Total": "Total"};

        _.each(stati, function (status, index) {
            var name;
            if (status == "Published") {
                name = data['publishedSurveyList'];
            } else if (status == "Unpublished") {
                name = data['unpublishedSurveyList'];
            }
            if (!status2css[status]) {
                if (typeof data['chart']['data'][index] != "undefined") {
                    this.tabData.push({
                        index: index,
                        status: status,
                        statusLabel: statusOptions[status],
                        models: data['chart']['data'][index]['value'],
                        cssStyle: data['chart']['data'][index]['color'] ? data['chart']['data'][index]['color'] : "#30a7bc",
                        name: name
                    });
                }
            }
    ...
    loadTitle: function (options) {
        var self = this;
        var url = App.api.buildURL("bc_survey", "dashlet_survey_send_status_chart", "");
