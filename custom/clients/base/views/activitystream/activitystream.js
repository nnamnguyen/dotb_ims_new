 ...
 * @alias DOTB.App.view.views.BaseActivitystreamView
 * @extends View.View
 */
({
    events: {
    ...
    initialize: function(options) {
        ...
        var get_date = this.model.get("date_modified");
        get_date = get_date.slice(0,10);

        //gom nhom ngay
        var check_day=null;
        if(typeof(options) !='undefined' && typeof options.collections != "undefined" && typeof (options.collections.models) != 'undefined') {
            for (var i = 0; i < options.collections.models.length; i++) {
                if (options.collections.models[i].id == options.model.id) {
                    if (i == 0)
                        check_day = "0";
                    else if (options.collections.models[i - 1].attributes.date_modified.slice(0, 10) == get_date)
                        check_day = "1";
                    else check_day = "0";
                }
            }
        }
        this.model.set("check_day",check_day);
        this.model.set("get_date",moment(get_date).format("DD/MM/YYYY"));

        var _module = this.model.get('display_parent_type');
        var icon_module = app.config.module_icon[_module].src;
        this.model.set("icon_module",icon_module);

        var lastComment = this.model.get("last_comment");
        if(typeof lastComment.data.type != 'undefined'){
            if(lastComment.data.type = 'student'){
                lastComment.module = 'Contacts';
            }else{
                lastComment.module = 'Employees';
            }
        }else{
            lastComment.module = 'Employees';
        }
    ...
    fetchComments: function() {
        ...
        this.commentsCollection.fetch({
            ...
            success: function(collection) {
                _.each(collection.models, function (data) {
                    if(typeof data.attributes.data.type != 'undefined'){
                        if(data.attributes.data.type = 'student'){
                            data.attributes.module = 'Contacts';
                        }else{
                            data.attributes.module = 'Employees';
                        }
                    }else{
                        data.attributes.module = 'Employees';
                    }
    ...
    addComment: function (event) {
        ...
        payload.data = _.extend({type: 'admin'},this.getComment());
