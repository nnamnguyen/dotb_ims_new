...
<tr>
    ...
    <td>
        ...
        <input type='hidden' name='customTypeValidate' id='customTypeValidate' value='{$vardef.len|default:25}'
               ...
               onchange="if (parseInt(document.getElementById('field_len').value) < parseInt(document.getElementById('orig_len').value)) return confirm(DOTB.language.get('ModuleBuilder', 'LBL_CONFIRM_LOWER_LENGTH')); return true;">
        {/if}
        {literal}
            <script>
...
<tr>
    <td class='mbLBL'>{dotb_translate module="DynamicFields" label="LBL_HIGHLIGHTFIELD_BACKCOLOR"}:</td>
    <td>
        {if $hideLevel < 5}
...
<tr>
    <td class='mbLBL'>{dotb_translate module="DynamicFields" label="LBL_HIGHLIGHTFIELD_TEXTCOLOR"}:</td>
    <td>
        {if $hideLevel < 5}
