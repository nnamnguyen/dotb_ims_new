 ...
 * @alias DOTB.App.view.fields.BaseRelateField
...
({
    ...
    */
    ...
    setValue: function(models) {
        ...
        window.seft_model = this;
        // get unit price
        app.api.call("create", App.api.buildURL('get/unitPrice'), {product_template_id:this.model.attributes.product_template_id,unit_id:unit_id}, {
            success: function (data) {
                window.seft_model.model.set({'list_price':data});
                window.seft_model = undefined;
            },
            error:function () {
            }
        })
    ...
    openSelectDrawer: function() {
        ...
        if(this.module === 'Products' && this.name === 'unit_name' )
        {
            var filterOptions = new app.utils.FilterOptions().config({
                initial_filter_label: 'Unit in Group',
                initial_filter: 'group_unit_id',
                filter_populate: {
                    group_unit_id: this.model.get('group_unit_id')
                }
            }).format();
