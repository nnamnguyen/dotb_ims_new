                ...
                if(!empty($bean->voucher_id) && !empty($bean->payment_id)){
                    ...
                    if($bean->sponsor_number != $vou->name){
                        ...
                        if(!empty($bean->discount_id)){
                            $GLOBALS['db']->query("UPDATE j_payment_j_discount_1_c SET deleted=1, date_modified='{$GLOBALS['timedate']->nowDb()}' WHERE j_payment_j_discount_1j_payment_ida = '{$bean->payment_id}' AND j_payment_j_discount_1j_discount_idb = '{$bean->discount_id}'");
                            $bean->discount_id = '';
                        }
