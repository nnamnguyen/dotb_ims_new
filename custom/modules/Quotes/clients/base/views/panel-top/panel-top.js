...
({
    ...
    events: {
        ...
        // created order
        'click [name="create_order"]': 'createdOrder',
    },
    ...
    */
    ...
    createdOrder:function (event) {
        window.parent_quote = app.controller.context.attributes.model.attributes;
        window.parent_id = this.context.parent.get('modelId');
        window.parent_type = this.context.parent.get('module');
        window.parent_name = this.context.parent.get('model').get('name');
        window.location.replace('#Quotes/create');
    },
})
