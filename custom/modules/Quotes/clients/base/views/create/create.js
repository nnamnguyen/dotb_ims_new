...
({
    ...
    //add by TKT
    gobackduplicate_button: 'gobackduplicate_button',
    ...
    initialize: function (options) {
        ...
        //set default quote to
        if(window.parent_id != undefined)
        {
            this.model.attributes.parent_type = window.parent_type;

            this.model.attributes.billing_address_street = window.parent_quote.primary_address_street;
            this.model.attributes.billing_address_city = window.parent_quote.primary_address_city;
            this.model.attributes.billing_address_state = window.parent_quote.primary_address_state;
            this.model.attributes.billing_address_postalcode = window.parent_quote.primary_address_postalcode;
            this.model.attributes.billing_address_country = window.parent_quote.primary_address_country;
            this.model.setDefault({
                'parent_id': window.parent_id,
                'parent_name': window.parent_name,
            });
            this.model.relatedAttributes.parent_id = window.parent_id;
            this.model.relatedAttributes.parent_name = window.parent_name;
            if(window.parent_type == 'Contacts') {
                this.model.attributes.status = 'UnPaid';
            }
            else{
                this.model.attributes.status = 'Quotation';
            }
            window.parent_type = undefined;
            window.parent_id = undefined;
            window.parent_name = undefined;
            window.parent_quote = undefined;
        }
        //end

        // need to reset the default attributes because the plugin may have
        // calculated default values.
        this.on('dotblogic:initialize', function () {
    ...
    delegateButtonEvents: function () {
        ...
        //add by TKT
        this.context.on('button:' + this.gobackduplicate_button + ':click', this.goBackDuplicate, this);
    },

    //add by TKT
    goBackDuplicate: function () {
    ...
    save: function () {
        var discount_order ={};
        discount_order['discount'] = this.model.get('discount_detail');
        discount_order['sponsor'] = this.model.get('sponsor_detail');
        discount_order['loyalty'] = this.model.get('loyalty_detail');
        this.model.set({
            'quote_discount_detail': JSON.stringify(discount_order)
        })
        this.saveAndClose();
    },
    ...
    buildSuccessMessage: function (model) {
        ...
        //Add by Tuan Anh to redirect when convert target
        if(this.context.get('prospect_id') && this.context.get('prospect_id')!=''){
            var route = DOTB.App.router.buildRoute('Leads', model.get('id'));
            DOTB.App.router.redirect(route);
        }
        //End
