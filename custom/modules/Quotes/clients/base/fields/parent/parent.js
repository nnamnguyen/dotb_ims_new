 ...
 * @alias DOTB.App.view.fields.BaseParentField
...
({
    ...
    _render: function() {
        ...
        if(this.model.get('parent_id') != undefined || this.model.get('parent_id') != null ){
            window.seft = this;
            app.api.call("create", App.api.buildURL('get/loyalty'), {id:this.model.get('parent_id')}, {
                success: function (data) {
                    if(window.seft.model != null) {
    ...
    setValue: function(models) {
        ...
        _.each(models, _.bind(function(model) {
            ...
            if (app.acl.hasAccessToModel(this.action, this.model, this.name)) {
                ...
                if (!_.isUndefined(model.id)) {
                    ...
                    this.updateRelatedFields(model);
                    if (forceUpdate) {
                        this._updateField();
                    }
                }
        ...
        this.model.set({
            // 'order_sponsor_percent': 0,
            // 'order_sponsor_amount': 0,
            // 'sponsor_detail':undefined,
            // 'order_discount_percent': 0,
            // 'order_discount_amount': 0,
            // 'discount_detail':undefined,
            'order_loyalty_percent': 0,
            'order_loyalty_amount': 0,
            'loyalty_detail':undefined,

        })
        window.seft = this;
        app.api.call("create", App.api.buildURL('get/loyalty'), {id:models[0].id}, {
            success: function (data) {
                window.seft.model.set({
                    'loyalty':data.data.loyalty,
                    'loyalty_rate':data.data.loyalty_rate,
