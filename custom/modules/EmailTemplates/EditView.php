...
/**
 * The file used to customize emial template actions & layout setting
 *
 ...
 */
//register autoloader for allowing file function in onDemand
...
if ($has_campaign || $inboundEmail) {
    if (DotbAutoloader::fileExists('custom/modules/EmailTemplates/EditView.html')) {
        $xtpl = new XTemplate('custom/modules/EmailTemplates/EditView.html');
    } else {
        $xtpl = new XTemplate('modules/EmailTemplates/EditView.html');
    }
} else {
    if (DotbAutoloader::fileExists('custom/modules/EmailTemplates/EditViewMain.html')) {
...
$xtpl->assign("assign_user_select", DotbThemeRegistry::current()->getImage('id-ff-select', '', null, null, '.png', $mod_strings['LBL_SELECT']));
$xtpl->assign("assign_user_clear", DotbThemeRegistry::current()->getImage('id-ff-clear', '', null, null, '.gif', $mod_strings['LBL_ID_FF_CLEAR']));
//Assign qsd script
...
if (!DotbAutoloader::fileExists(dotb_cached('jsLanguage/') . $GLOBALS['current_language'] . '.js')) {
...
require_once('include/DotbFields/Fields/Teamset/DotbFieldTeamset.php');
...
if (true) {
    if (!isTouchScreen()) {
        require_once("include/DotbTinyMCE.php");
    ...
    if ($has_campaign) {
        ...
        //$xtpl->assign("CAMPAIGN_POPUP_JS", '<script type="text/javascript" src="include/javascript/dotb_3.js"></script>');
    } else {
    ...
    $storeSurveyID = (!empty($focus->survey_id)) ? $focus->survey_id : '';
    require_once('include/DotbQuery/DotbQuery.php');
    $query = new DotbQuery();
    $query->from(BeanFactory::getBean('bc_survey'));
    // Survey Status :: LoadedTech Customization added survey_status
    $query->select(array("id", "name", "survey_status","survey_submit_unique_id"));


    $runQuery = $query->execute();
    //  $selectSurvey = "SELECT id, name  FROM bc_survey where deleted = 0";
    //  $runQuery = $db->query($selectSurvey);
    // From Send Survey Email template creation request come then for requested data prefill the survey module & survey name
    if (isset($_REQUEST['survey_id'])) {

        //get survey Name from given survey id
        $selected_survey = $_REQUEST['survey_id'];
        $oSurvey = new bc_survey();
        $oSurvey->retrieve($selected_survey);
        $selected_survey_name = htmlspecialchars($oSurvey->name);
    }

    $surveyDDOption = "<option value=''>Select Survey</option>";
    foreach ($runQuery as $surveyresult) {
        $sel = '';
        // Survey Status :: LoadedTech Customization
        if ($surveyresult['survey_status'] == 'Active' || $storeSurveyID == $surveyresult['id']) {
            if ($storeSurveyID == $surveyresult['id']) {
                $sel = "selected=selected";
            }
            if(!empty($surveyresult['survey_submit_unique_id'])){
                $data_survey_submit_unique_id = 'data_survey_submit_unique_id=' . $surveyresult['survey_submit_unique_id'];
            }
            if (!empty($selected_survey) && $selected_survey == $surveyresult['id']) {
                $sel = "selected=selected";
                    $surveyDDOption .= "<option value='{$selected_survey}' $sel $data_survey_submit_unique_id>{$selected_survey_name}</option>";
            } else {
                $surveyDDOption .= "<option value='{$surveyresult['id']}' $sel $data_survey_submit_unique_id>{$surveyresult['name']}</option>";
            }
        }
        // Survey Status :: LoadedTech Customization END
    }
    $xtpl->assign("SURVEYLISTS", $surveyDDOption);
    // End 
    // Survey module type

    $dotb_survey_Url = $dotb_config['site_url'] . $survey_url_for_email;
    $xtpl->assign("SURVEYURL", $dotb_survey_Url);
    // End 
    /* Survey related code End */
    ///////////////////////////////////////
