 ...
 * @alias DOTB.App.view.views.BaseProductBundlesQuoteDataGroupListView
...
({
    ...
    /**
     * Track all the DotbLogic Contexts that we create for each record in bundle
     *
     * @type {Object}
     */
    dotbLogicContexts: {},
    ...
    initialize: function(options) {
        ...
        // for each item in the collection, setup DotbLogic
        ...
        _.each(collections, function(link) {
            ...
            if (collection) {
                this.setupDotbLogicForModelOrCollection(collection);
            }
        }, this);
    ...
    /**
     * Load and cache DotbLogic dependencies for a module
     *
     * @param {Data.Bean} model
     * @return {Array}
     * @private
     */
    _getDotbLogicDependenciesForModel: function(model) {
        ...
        if (_.isUndefined(this.moduleDependencies[module])) {
            ...
            // external from the Dotblogic plugin. Probably somewhere in the LumiaExpressionContext class...
     ...
     */
    setupDotbLogicForModelOrCollection: function(modelOrCollection) {
        var slContext;
        var isCollection = (modelOrCollection instanceof app.data.beanCollection);
        var dependencies = this._getDotbLogicDependenciesForModel(modelOrCollection);
        if (_.size(dependencies) > 0) {
            slContext = new DOTB.expressions.LumiaExpressionContext(
                this,
            ...
            this.dotbLogicContexts[id] = slContext;
        }
    },
    ...
    onCancelRowEdit: function(rowModel) {
        ...
        if (rowModel.isNew()) {
            ...
            if (!_.isUndefined(this.dotbLogicContexts[rowId])) {
                // cleanup any dotblogic contexts
                this.dotbLogicContexts[rowId].dispose();
            }
    ...
    onAddNewItemToGroup: function(linkName, prepopulateData) {
        ...
        var currencyId = quoteModel.get('currency_id') == undefined ? this.model.get('currency_id'):quoteModel.get('currency_id');
    ...
    _dispose: function() {
        ...
        _.each(this.dotbLogicContexts, function(slContext) {
            slContext.dispose();
        });
        this._super('_dispose');
        this.rowFields = null;
        this.dotbLogicContexts = {};
