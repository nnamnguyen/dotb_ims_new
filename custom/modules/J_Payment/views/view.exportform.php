<?php
    ...
    switch($payment->payment_type){
        case "Refund":
            $inputFileName = "custom/include/TemplateExcel/Form_Refund.xlsx";
            $outputFileName = 'custom/uploads/Refund_'.$payment->name.'-'.$section.'.xlsx';
            break;
        case "Delay":
            $inputFileName = "custom/include/TemplateExcel/Form_Delay.xlsx";
            $outputFileName = 'custom/uploads/Delay_'.$payment->name.'-'.$section.'.xlsx';
            break;
        case "Moving Out":
        case "Moving In":
            $inputFileName = "custom/include/TemplateExcel/Form_Moving_Center.xlsx";
            $outputFileName = 'custom/uploads/Moving_center_'.$payment->name.'-'.$section.'.xlsx';
            break;
        case "Transfer Out":
        case "Transfer In":
            $inputFileName = "custom/include/TemplateExcel/Form_Transfer.xlsx";
    ...
    //Write to Excel file
    writeData($payment, $objPHPExcel, $data);
    ...
    function writeData($payment, $objPHPExcel, $data){
        if($payment->payment_type == "Refund"){
