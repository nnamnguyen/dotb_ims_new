...
({
    ...
    renderColection:function () {
        var seft = this;
        var collection = [];
        var parent_id = this.context.get('parent_id');
        if(parent_id != undefined){
            this.collection.models.forEach(function (item) {
                if (item.get('parent_id') == parent_id && item.get('date_active') == "" && item.get('status') =='Available' && item.get('payment_type')=='Cashholder')
