...
({
    ...
    contextEvents: {
        ...
        'list:voidBalance:fire' : 'voidBalance',
    },
    ...
    voidBalance: function (evt) {
        var quoteId = this.context.parent.attributes.modelId;
        app.alert.show('message-id', {
            level: 'confirmation',
            messages: app.lang.get('LBL_CONFIRM_VOID_BALANCE','J_Balance'),
            autoClose: false,
            onConfirm: function(){
                ...
                app.api.call("create", App.api.buildURL('void/balanceQuote'), {model:evt.attributes,quote_id:quoteId}, {
                    success: function (data) {
                        if(data.success == 1) {
                            // app.controller.context.trigger('subpanel:reload', {links: ['j_balance_quotes_1', 'j_balance']});
                            // app.controller.context.reloadData({});
                            // app.alert.dismiss('process-id');
                            // app.alert.show('success-id', {
                            //     level: 'success',
