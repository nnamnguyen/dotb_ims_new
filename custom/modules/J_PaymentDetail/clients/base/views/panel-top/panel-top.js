...
({
    ...
    events: {
        ...
        'click [name="create_j_paymentdetail"]': 'createdReceipt',
    },
    ...
    */
    ...
    createdReceipt:function (evt) {
        var model =this.context.get('parentModel');
        var seft = this;
        if(model.get('parent_type') == 'Leads')
        {
            app.alert.show('message-id', {
                level: 'info',
                messages: app.lang.get('LBL_NOTIFICATION_CHOOOSE_QUOTES_LEAD', 'J_PaymentDetail'),
            });
        }
        else{
            var payment_amount = Number(model.get('unpaid_amount'))
            if(model.get('number_of_payment') ==='Monthly-plan'){
                var pay_plan = Number(model.get('total'))/Number(model.get('num_month_pay'));
                if(pay_plan < payment_amount)
                    payment_amount = pay_plan
            }
            if(this.parentModule == 'Accounts'){
                ...
                this.model.attributes.payment_type = 'Deposit';
            }
            else {
                this.model.setDefault({
                    'quote_id': model.get('id'),
                    'quote_name': model.get('name'),
                    'parent_id': model.get('parent_id'),
                    'parent_name': model.get('parent_name'),
                    'payment_amount': payment_amount,
                    'payment_type': 'Normal',
                });
                this.model.attributes.payment_amount = payment_amount;
                this.model.attributes.payment_type = 'Normal';
                this.model.attributes.quote_id = model.get('id');
            }
            app.drawer.open({
                layout: 'create',
                context: {
