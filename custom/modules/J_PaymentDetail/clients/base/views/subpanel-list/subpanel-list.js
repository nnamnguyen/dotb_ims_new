 ...
 * @alias DOTB.App.view.views.BaseSubpanelListView
 * @extends View.Views.Base.RecordlistView
 */
({
    extendsFrom: 'RecordlistView',
    fallbackFieldTemplate: 'list',
    plugins: ['ErrorDecoration', 'Editable', 'DotbLogic', 'Pagination',
    ...
    contextEvents: {
        ...
        // Added by Hieu Pham to register event unlink meeting
        'list:unlink_meeting:fire' : 'unlinkMeeting',
        "list:exportrow:fire": "exportRow",
    ...
    getVoidReceiptMessages: function(model) {
        var messages = {};
        var name = Handlebars.Utils.escapeExpression(app.utils.getRecordName(model)).trim();
        var context = app.lang.getModuleName(model.module).toLowerCase() + ' ' + name;

        messages.confirmation = app.utils.formatString(app.lang.get('NTC_VOID_RECEIPT_CONFIRMATION_FORMATTED'), [context]);
        messages.success = app.utils.formatString(app.lang.get('NTC_VOID_RECEIPT_SUCCESS'), [context]);
        return messages;
    },
    ...
    exportRow: function (model) {
        if(model.attributes.status == 'Cancelled'){
            ...
            window.open('index.php?module=Quotes&type=student&action=invoiceVoucher&record=' + model.id + '&dotb_body_only=true', '_blank');
