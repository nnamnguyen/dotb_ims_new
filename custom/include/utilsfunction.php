 ...
 */
require_once 'twilio-php-master/src/Twilio/Rest/Client.php';
/**
...
function CustomSendEmail($to = '', $subject = '', $body = '', $module_id = '', $module_type = '', $typeOfRec = 'to') {
    ...
    if ($to != '') {
        ...
        $getNotifyQry = "SELECT * FROM config WHERE category = 'notify' ";
        $resultNotify = $db->query($getNotifyQry);
        while ($rowNotify = $db->fetchByAssoc($resultNotify)) {
            if ($rowNotify['name'] == 'fromaddress') {
                $notify_fromaddress = $rowNotify['value'];
            } else if ($rowNotify['name'] == 'fromname') {
                $notify_fromname = $rowNotify['value'];
            }
        }
...
function getReportData($type = '', $survey_id = '', $name = '', $module_type = '', $status = '', $submission_start_date = '', $submission_end_date = '', $submission_type = '', $status_type = '', $sort = '', $sort_order = '', $page = '', $global_filter_by = '', $GF_QueLogic_Passed_Submissions = array()) {
    ...
    if ($type == 'status') {
        ...
        if ($status_type == 'openended') {
            ...
            $query->where()->equals('submission_type', 'Manual');
        } else if ($status_type == 'sms') {
            //submission by sms changes 26-11-2019
            $query->where()->equals('submission_type', 'SMS');
        } else if ($status_type == 'whatsapp') {
            //submission by whatsapp changes 06-02-2020
            $query->where()->equals('submission_type', 'WhatsApp');
        ...
        if (!empty($submission_start_date) && !empty($submission_end_date)) {
            ...
            if (empty($toDbStartSubmission)) {
                $toDbStartSubmission = $submission_start_date;
            }
            if (empty($toDbEndSubmission)) {
                $toDbEndSubmission = $submission_end_date;
            }
 ...
 * @param      bool - $isTakeSurvey :: Take survey changes 03-08-2019
 * @return     Array
 */
function sendSurveyEmailsModuleRecords($records = '', $module_name = '', $survey_id = '', $schedule_on_date = '', $schedule_on_time = '', $isSendNow = false, $beanFromLogicHook = '', $recipient_as = 'to', $origin_parent_type = '', $origin_parent_id = '', $email_automation = false, $isTakeSurvey = 0, $sendType = '', $fieldNameToSendSMSTo = array(), $toNumberFieldData = array()) {
    global $dotb_config, $db, $current_user, $timeDate;
    $schedule_date = "";
    ...
    foreach ($recipients[$module_name] as $rec_module_id) {
        ...
        //Take survey changes 03-08-2019
        if ((($email_automation == true && $is_send != 'already_sent') || $email_automation == false) && $isTakeSurvey != 1) {
            if ($sendType != "") {
                $mobile_numberlist = array();
                foreach ($fieldNameToSendSMSTo[$rec_module_id] as $field_name => $mobile_number) {
//                    foreach ($mobile_numberData as $field_name => $mobile_number) {
                    if ($field_name != "id") {
                        ...
                        if ($findSMSMobileNumber == "noOptOut") {
                            ...
                            if (isset($returnDataSurvey['sms_optOut']['records'][$rec_module_id])) {
                                if ($mobile_number == "") {
                                    $returnDataSurvey['sms_optOut']['records'][$rec_module_id]['mobile_number'] = $returnDataSurvey['sms_optOut']['records'][$rec_module_id]['mobile_number'] . ',' . $toNumberFieldData[$field_name] . ': no_number';
                                } else {
                                    $returnDataSurvey['sms_optOut']['records'][$rec_module_id]['mobile_number'] = $returnDataSurvey['sms_optOut']['records'][$rec_module_id]['mobile_number'] . ',' . $toNumberFieldData[$field_name] . ': ' . $mobile_number;
                                }
                            } else {
                                $sms_optOutCount++;
                                $returnDataSurvey['sms_optOut']['records'][$rec_module_id]['record_name'] = $record_name;
                                if ($mobile_number == "") {
                                    $returnDataSurvey['sms_optOut']['records'][$rec_module_id]['mobile_number'] = $toNumberFieldData[$field_name] . ": no_number";
                                } else {
                                    $returnDataSurvey['sms_optOut']['records'][$rec_module_id]['mobile_number'] = $toNumberFieldData[$field_name] . ": " . $mobile_number;
                                }
                            }
                        }
//                        }
            ...
            if (($focus->email_opt_out == 0 && $sendType == "") || (!empty($mobile_numberlist) && $sendType != "")) {
                /*
                 * Store survey data start
                 */
                $is_send = 'scheduled';
                if ($is_send == 'scheduled' && ($isSendNow || $bean_records_count <= 500)) {
//                if ($is_send == 'scheduled' && ($isSendNow)) {
                    ...
                    if ($sendType == "") {
                        ...
                        $survey_submission->to_be_sms_send_field = json_encode($mobile_numberlist);
                    ...
                    $tobe_Submission[] = array('module_id' => $rec_module_id, 'module_name' => $module_name, 'survey_id' => $survey_id, 'sub_type' => $subType, 'fieldNameToSendSMSTo' => $fieldNameToSendSMSTo);
                }
    ...
    if (!empty($tobe_Submission)) {
        ...
        $insert = "INSERT into survey_submission_pending_entries(module_name,module_id,survey_id,submission_type,to_be_sms_send_field) VALUES";

        foreach ($tobe_Submission as $k => $sub_data) {

            if ($counter_sub < 10) {
                // create new entry if not exists
                $insert .= "('{$sub_data['module_name']}','{$sub_data['module_id']}','{$sub_data['survey_id']}','{$sub_data['sub_type']}','{$sub_data['fieldNameToSendSMSTo']}'),";
                $counter_sub++;
            }
            if ($counter_sub == 10) {
        ...
        foreach ($insert_queries as $i => $query) {
            if ($query != 'INSERT into survey_submission_pending_entries(module_name,module_id,survey_id,submission_type,to_be_sms_send_field) VALUES') {
                $db->query($query);
            }
        }
    }
    $returnDataSurvey['mobile_numberlist'] = $mobile_numberlist;
    $returnDataSurvey['MailSentSuccessfullyFirstTime'] = $firsttimecount;
    $returnDataSurvey['sms_optOutCount'] = $sms_optOutCount;
...
function getAllExportData($type = '', $survey_id = '', $name = '', $module_type = '', $submission_type = '', $status = '', $submission_start_date = '', $submission_end_date = '') {
    ...
    foreach ($submtData as $sbmtId => $sbmtData) {
        ...
        $QuestionsFromSurvey = "SELECT
                            bc_survey_questions.id,
                            bc_survey_questions.name,
                            bc_survey_questions.disabled_question,
                            bc_survey_questions.piping_sequence,
        ...
        $runQuestionQuery = $db->query($QuestionsFromSurvey);
        while ($QuestionsFromSurveyData = $db->fetchByAssoc($runQuestionQuery)) {
            if (!array_key_exists($QuestionsFromSurveyData['id'], $submitDataArry)) {
                $submitDataArry[$QuestionsFromSurveyData['id']][$QuestionsFromSurveyData['name']] = 'N/A';
            }
        }
        ksort($submitDataArry);
 ...
 * @return     string
 */
function createContentForMailStatusPopup($customersSummaryData = '', $survey_id = '', $module_name = '', $total_seleted_records = '', $isSendNow = '', $record_name = '', $isSendSuccess = '', $isDetailView = '', $surveySingularModule = '', $sub_type = "") {
    ...
    $sms_optOut = (isset($customersSummaryData['sms_optOut']) && !empty($customersSummaryData['sms_optOut'])) ? $customersSummaryData['sms_optOut'] : array();
    $sms_optOutCount = (isset($customersSummaryData['sms_optOutCount']) && !empty($customersSummaryData['sms_optOutCount'])) ? $customersSummaryData['sms_optOutCount'] : array();
    ...
    if (!empty($sms_optOut['records'])) {
        $sms_optOutrecord = json_encode($sms_optOut['records']);
    }
    $html = '';
    $html .= "<html>
             <head>
    ...
    if (!empty($customersFirstTimeSuccSentArray) && $customersFirstTimeSuccSentArray > 0 && !$isSendNow) {
        $html .= "<table width='100%'>
                    ...
                    <tr class='title'>";
        if ($sub_type == "Email") {
            $html .= "<td colspan='3'><span>Your email will be delivered to <strong style='color: #008000'>{$customersFirstTimeSuccSentArray}</strong> {$surveySingularModule}(s) very soon.</td>";
        } elseif ($sub_type == "sms") {
            $html .= "<td colspan='3'><span>Your sms will be delivered to <strong style='color: #008000'>{$customersFirstTimeSuccSentArray}</strong> {$surveySingularModule}(s) very soon.</td>";
        } elseif ($sub_type == "whatsapp") {
            $html .= "<td colspan='3'><span>Your Whatsapp message will be delivered to <strong style='color: #008000'>{$customersFirstTimeSuccSentArray}</strong> {$surveySingularModule}(s) very soon.</td>";
        }
        $html .= "</tr>
                    </thead>
                  </table>";
    ...
    if (!empty($customersResponseSubmitted) && $customersResponseSubmitted > 0) {
        $html .= "<table width='100%'>
                    <thead>
                        <tr class='title'>";
        if ($sub_type == "Email") {
            $html .= "           <td colspan='4'><span>Your email has been already delivered to <strong style='color: #008000'>{$customersResponseSubmitted}</strong> {$surveySingularModule}(s) and also received their response.</span></td>";
        } else if ($sub_type == "sms") {
            $html .= "           <td colspan='4'><span>Your sms has been already delivered to <strong style='color: #008000'>{$customersResponseSubmitted}</strong> {$surveySingularModule}(s) and also received their response.</span></td>";
        } else if ($sub_type == "whatsapp") {
            $html .= "           <td colspan='4'><span>Your Whatsapp message has been already delivered to <strong style='color: #008000'>{$customersResponseSubmitted}</strong> {$surveySingularModule}(s) and also received their response.</span></td>";
        }
        $html .= "       </tr>
                    </thead>
                 </table>";
    ...
    if (!empty($allOptCustomers['count']) && $allOptCustomers['count'] > 0) {
        ...
        $html .= "           <td colspan='3'><span>Your email will not be send to <strong style='color: #008000'>{$allOptCustomers['count']}</strong> {$surveySingularModule}(s) due to opted-out email-address.</span>";
        $html .= "<strong>
    ...
    if (!empty($sms_optOutrecord)) {
        $html .= "<table width='100%'><thead><tr class='title'>";
        if ($sub_type == "sms") {
            $html .= "           <td colspan='3'><span>Your sms will not be send to <strong style='color: #008000'>{$sms_optOutCount}</strong> {$surveySingularModule}(s) due to no/incorrect number.</span>";
        } else if ($sub_type == "whatsapp") {
            $html .= "           <td colspan='3'><span>Your whatsapp message will not be send to <strong style='color: #008000'>{$sms_optOutCount}</strong> {$surveySingularModule}(s) due to no/incorrect number.</span>";
        }
        $html .= "<strong>
                                 <input class='btn btn-primary' type='button' name='ViewOptedOutResSMS' value='View' data-survey-id='{$survey_id}' data-survey-module='{$module_name}' data-opted-out-record='{$sms_optOutrecord}' onclick='' >
                       </strong>
                   </td>";
        $html .= "</tr></thead>
             </table>";
    ...
    if ($isSendNow && $isSendSuccess == 'send') {
        $html .= "<table width='99%'><thead><tr class='title'>";
        if ($sub_type == "Email") {
            $html .= "<td colspan='3'><span>Your email successfully delivered to <strong style='color: #008000'>{$record_name}</strong>.</span> ";
        } else if ($sub_type == "sms") {
            $html .= "<td colspan='3'><span>Your sms successfully delivered to <strong style='color: #008000'>{$record_name}</strong>.</span> ";
        } else if ($sub_type == "whatsapp") {
            $html .= "<td colspan='3'><span>Your Whatsapp message successfully delivered to <strong style='color: #008000'>{$record_name}</strong>.</span> ";
        }
        $html .= "</td>
               </tr></thead>
             </table>";
    } else if ($isSendNow && !empty($isSendSuccess)) {
        if ($isSendSuccess != "not_send") {
            $html .= "<table width='100%'><thead><tr class='title'>
            ...
            $html .= "<table width='100%'><thead><tr class='title'>
                   <td colspan='3'><span style='color :red;'>Message not send.</strong> Please check your whatsapp configuration settings.</span> 
                   </td>
               </tr></thead>
             </table>";
        }
...
function getOptOutEmailCustomers($moduleID = '', $moduleName = '') {
    require_once 'include/DotbEmailAddress/DotbEmailAddress.php';
    $DotbEmailAddress = new DotbEmailAddress();
    global $db;
    switch ($moduleName) {
        case "Accounts":
            $focus = new Account();
    ...
    $emailAddressDetail = $DotbEmailAddress->getAddressesByGUID($moduleID, $moduleName);
 ...
 * @return     array
 */
function getSMSTemplateBySurveyID($surveyID = '') {
    global $db;
    $selectsms_templateQ = "SELECT * FROM bc_survey_sms_template WHERE deleted = 0 AND sms_survey_linked = '{$surveyID}'";
    $sms_templateDetails = $db->query($selectsms_templateQ);
    $sms_templateData = $db->fetchByAssoc($sms_templateDetails);
    $smsTempId = $sms_templateData['id'];
    $smsTemplateID = (!empty($smsTempId)) ? $smsTempId : '';
    return array("smsTemplateID" => $smsTemplateID);
}
...
function getAnswerSubmissionCount($surveyID = '', $status_type = '', $global_filter = array(), $GF_QueLogic_Passed_Submissions = array(), $que_id = '') {
    ...
    switch ($status_type) {
        ...
        case 'sms':
            ...
            break;
        case 'whatsapp':
            //submission by whatsapp changes 26-11-2019
    ...
    while ($resultCountData = $db->fetchByAssoc($runQuery)) {
        //Take survey changes 03-08-2019
        $subGet = "SELECT bc_survey_submission.submission_type FROM bc_survey_submission WHERE id = '{$resultCountData['sub_id']}'  ";
        $resSubmission = $db->query($subGet);
        $submission_type = '';
        while ($row = $db->fetchByAssoc($resSubmission)) {
            $submission_type = $row['submission_type'];
        }

        if (($status_type == 'openended' && str_split($resultCountData['recId'], 8)[0] == 'Web Link') || ($status_type == 'email' && str_split($resultCountData['recId'], 8)[0] != 'Web Link') || ($status_type == 'manual' && $submissionTypeImplodeVal == 'Manual') || ($status_type == 'sms' && $submissionTypeImplodeVal == 'SMS') || ($status_type == 'whatsapp' && $submissionTypeImplodeVal == 'WhatsApp') || ($status_type == 'combined' || $status_type == '')) {
...
/**
 * get trend data for question wise submission
 *
 * @author     Biztech Consultancy
 * @param      string - $surveyID,$submission_type
 * @param      array - $accessbileSubmissionIds
 * @return array
 */
function getTrendQuestionWiseSubmissionData($surveyID = '', $submission_type = '', $accessbileSubmissionIds = array(), $takeDisabledQuestion = '') {
    global $db;
    $submissionTypeArray = array();
    switch ($submission_type) {
        ...
        case 'manual':
            ...
            break;
        case 'sms':
            //submission by SMS changes 29-11-2019
            $submissionTypeArray = array('SMS');
            break;
        case 'whatsapp':
            //submission by whatsapp changes 29-11-2019
            $submissionTypeArray = array('WhatsApp');
            break;
        case 'combined':
            //Take survey changes 03-08-2019
...
//        $groupByMssql = ',ss.id, subData.submit_answer_id '; // MSSQL support bugfix
    ...
    $q = "SELECT 
            ...
            GROUP BY ss.submission_date, subData.question_id, ss.id, subData.submit_answer_id
    ...
    if ($takeDisabledQuestion == 'false' && (string) $takeDisabledQuestion != "1") {
        $disabledQuestionRemovalQuery = " SELECT
                bc_survey_questions.id,
                bc_survey_questions.name
                FROM
                bc_survey_questions
              LEFT JOIN
                bc_survey_bc_survey_questions_c ON bc_survey_bc_survey_questions_c.bc_survey_bc_survey_questionsbc_survey_questions_idb = bc_survey_questions.id AND bc_survey_questions.deleted = 0 AND bc_survey_bc_survey_questions_c.deleted = 0
              WHERE
                bc_survey_bc_survey_questions_c.bc_survey_bc_survey_questionsbc_survey_ida = '{$surveyID}' 
                 and bc_survey_questions.disabled_question = 1";
        $rundisabledQuestionQ = $db->query($disabledQuestionRemovalQuery);
        while ($disabledQuestionData = $db->fetchByAssoc($rundisabledQuestionQ)) {
            unset($submittedDataArrayQuestion[$disabledQuestionData['id']]);
        }
    } else if ($takeDisabledQuestion == false) {
        $disabledQuestionRemovalQuery = " SELECT
                bc_survey_questions.id,
                bc_survey_questions.name
                FROM
...
//        $groupByMssql = ",subData.id ";
    ...
    $q = "SELECT
            ...
            GROUP BY subData.submission_date,subData.id 
...
/**
 * Get Date Range for Trend Report
 *
 * @author     Biztech Consultancy
 * @param      string - $first,$last,$step,$output_format
 * @return array
 */
 ...
 *
 * @author     Biztech Consultancy
 * @param      string - $surveyID,$status_type,$total_submitted_response
 * @param      array - $global_filter,$GF_QueLogic_Passed_Submissions
 * @return array
 */
 ...
 *
 * @author     Biztech Consultancy
 * @param      string - $surveyID,$moduleID,$questionID,$status_type,$submission_id
 * @param      array - $global_filter,$GF_QueLogic_Passed_Submissions
 * @return array
 */
function getAnswerSubmissionDataForMatrix($surveyID = '', $moduleID = '', $questionID = '', $status_type = '', $submission_id = '', $global_filter = array(), $GF_QueLogic_Passed_Submissions = array()) {
    ...
    while ($resultCountData = $db->fetchByAssoc($runQuery)) {
        ...
        if (in_array($resultCountData['sub_id'], $GF_QueLogic_Passed_Submissions) || empty($GF_QueLogic_Passed_Submissions)) {
            ...
            if (($status_type == 'openended' && str_split($resultCountData['survey_receiver_id'], 8)[0] == 'Web Link') || ($status_type == 'email' && str_split($resultCountData['survey_receiver_id'], 8)[0] != 'Web Link') || ($status_type == 'manual' && $submission_type == 'Manual') || ($status_type == 'sms' && $submission_type == 'SMS') || ($status_type == 'whatsapp' && $submission_type == 'WhatsApp') || ($status_type == 'combined' || $status_type == '')) {
 ...
 *
 * @author     Biztech Consultancy
 * @param      string - $surveyID,$que_id, $question_obj,$survey_type,$page
 * @param      array - $global_filter,$accesible_submissions
 * @return array
 */
function getQuestionWiseData($survey_id = '', $que_id = '', $question_obj = '', $survey_type = '', $page = '', $global_filter = array(), $accesible_submissions = array()) {
    ...
    } else if ($survey_type == 'manual') {
        //Take survey changes 03-08-2019
        $array_survey_type = array('Manual');
    } else if ($survey_type == 'sms') {
        //submission by SMS changes 26-11-2019
        $array_survey_type = array('SMS');
    } else if ($survey_type == 'whatsapp') {
        //submission by whatsapp changes 26-11-2019
        $array_survey_type = array('WhatsApp');
    } else {
        //Take survey changes 03-08-2019
        $array_survey_type = array('Email', 'Open Ended', 'Manual', 'SMS', 'WhatsApp');
    }
 ...
 *
 * @author     Biztech Consultancy
 * @param      string - $survey_id,$survey_type
 * @param      array - $accesible_submissionsAll
 * @return array
 */
            ...
            if ($count <= 0) { // for date enter first time
                ...
                if ($flag == 1) {
                    if (!array_key_exists($date, $inserted_dates)) { // check if date already not exist then insert date
                        //Take survey changes 03-08-2019
                        if ($status_type == 'manual') {
                            $result[] = array($date, (int) number_format($submitted), (int) 0);
                        } else {
...
function getHealthStatus() {
    ...
    //*********************************Script Status
    $script_statusOne = $admin->settings['SurveyPlugin_SurveyProductScriptOne'];
    $script_statusTwo = $admin->settings['SurveyPlugin_SurveyProductScriptTwo'];
    if ($script_statusOne == 1 && $script_statusTwo == 1) {
        $script_status = '<img src="themes/default/images/green_camp.gif" width="16" height="16" align="baseline" border="0" alt=""> <b style="color:green;">No script for execution for Survey Rocket Plugin.</b>';
    } else {
        $script_status = '<img src="themes/default/images/red_camp.gif" width="16" height="16" align="baseline" border="0" alt=""> <b style="color:red;">Please Execute the script.</b><a id="survey_script" href="index.php?module=Administration&amp;action=surveyscript" target="_self" class="tabDetailViewDL2Link">Click Here</a>';
    }
...
function getSubmittedAnswerByReciever($surveyId = '', $survey_recieverID = '', $submission_id = '') {
    global $db;
    $ques_subAnsArray = array();
    $ques_subAnsdataArray = array();
    $getSubAnsQuery = "SELECT question_id, submit_answer_id, submission_id FROM bc_survey_submit_answer_calculation
                       WHERE (survey_receiver_id = '{$survey_recieverID}' OR submission_id = '{$submission_id}')
 ...
 *
 * @author     Biztech Consultancy
 * @param      string - $survey_id,$submission_id
 * @param      array - $global_logic
 * @return array
 */
function getGlobalFilterByQuestionLogicSubmissions($survey_id = '', $submission_id = '', $global_logic = array()) {
    global $current_user, $db;
...
/**
 * Get Standard deviation Value after calculation
 *
 * @author     Biztech Consultancy
 * @param      string - $survey_id,$submission_id
 * @param      array - $global_logic
...
function getTrendTableHtmlStructure($rowDataArray) {
    ...
    $disabled_question = $rowDataArray['disabled_question'];
    $imgHtml = '<img src="' . trim($path_Img) . '" height="250px" width="500px">';
    if ($fromIndividualQuestion && $exportAS == 'image') {
        $imgHtml = '<img style="margin-left:90px;" src="' . trim($path_Img) . '">';
    }
    $trendHtml = '';
    $trendHtml .= '';
    $trendHtml .= '<table class="report_question_table" style="margin-left:15px;">
                    <tbody>
                       <tr class="question">';
    //changes regarding for disabled question in export table structure
    if ($disabled_question == 1) {
        $trendHtml .= '<td colspan="3">&nbsp;<span style="text-align:left;"><b>' . $qSeq . '.</b></span>&nbsp; <b>' . $quetitle . ' <span style="color:red;">(Disabled Question)</span></b></td>';
    } else {
        $trendHtml .= '<td colspan="3">&nbsp;<span style="text-align:left;"><b>' . $qSeq . '.</b></span>&nbsp; <b>' . $quetitle . '</b></td>';
    }
    $trendHtml .= '</tr>
                    </tbody>
                 </table>';
    ...
    if ($fromIndividualQuestion && $exportAS == 'image') {
        $trendHtml .= '<table  border="1" width="80%" style="margin-left:130px;">';
    } else {
        $trendHtml .= '<table  border="1" width="100%" style="margin-left:130px;">';
    }
    $trendHtml .= '               <tr class="thead">
                                      <td>
                                         <b>Date Range</b>
                                      </td>
                                      <td>
...
function getMatrixHtmlStructure($matrixDataArray = array()) {
    ...
    //changes regarding for disabled question in export table structure
    $disabled_question = $matrixDataArray['disabled_question'];
    ...
    $matrixhtml = '';
    $matrixhtml .= '<table class="report_question_table" style="margin-left:15px;">
                           <tbody>
                              <tr class="question">';
    //changes regarding for disabled question in export table structure
    if ($disabled_question == 1) {
        $matrixhtml .= '<td colspan="3">&nbsp;<span style=""><b>' . $qSeq . '.</b></span>&nbsp; <b>' . $quetitle . ' <span style="color:red">(Disabled Question)</span></b></td>';
    } else {
        $matrixhtml .= '<td colspan="3">&nbsp;<span style=""><b>' . $qSeq . '.</b></span>&nbsp; <b>' . $quetitle . '</b></td>';
    }
    $matrixhtml .= '                              </tr>
                           </tbody>
                        </table>';
    $matrixhtml .= '<p align="center"><b>Answered Persons :&nbsp;</b>' . $answeredCount . '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>Skipped Persons :&nbsp;</b>' . $skippedCount . '</p>
                           <span>' . $imgHtml . '</span><br><br>';
    if ($exportAS != 'image') {
        $matrixhtml .= '   <table class="" border="1" width="100%" style="">';
    } else {
        $matrixhtml .= '   <table class="" border="1" width="75%" style="margin-left:150px;">';
    }
...
function getChoiceTypeHtmlStructure($rowDataArray) {
    ...
    $imgHtml = '<img src="' . trim($path_matrixImg) . '" height="250px" width="500px">';
    if ($fromIndividualQuestion && $exportAS == 'image') {
        $imgHtml = '<img style="margin-left:80px;" src="' . trim($path_matrixImg) . '">';
    }
    $choiceHtml = '';
    $choiceHtml .= ' <table class="report_question_table" style="margin-left:15px;">
                           <tbody>
                              <tr class="question">';
    //changes regarding for disabled question in export table structure
    if ($disabled_question == 1) {
        $choiceHtml .= '<td colspan="3">&nbsp;<b><span style="">' . $qSeq . '.</span>&nbsp;' . $quetitle . ' <span style="color:red">(Disabled Question)</span></b></td>';
    } else {
        $choiceHtml .= '<td colspan="3">&nbsp;<b><span style="">' . $qSeq . '.</span>&nbsp;' . $quetitle . '</b></td>';
    }
    $choiceHtml .= '</tr>
                           </tbody>
                        </table>
    ...
    if ($exportAS != 'image') {
        ...
        $choiceHtml .= '   <table class="" border="1" width="82%" style="margin-left:130px;">';
    }
    $choiceHtml .= ' <tbody>
                                          <tr>';
...
function getNonChoiceTypeHtmlStructure($rowDataArray) {
    ...
    //changes regarding for disabled question in export table structure
    if ($disabled_question == 1) {
        $nonchoiceHtml .= '<td  >&nbsp;<b><span style="">' . $qSeq . '.</span>&nbsp;' . $quetitle . ' <span style="color:red">(Disabled Question)</span></b></td>';
    } else {
        $nonchoiceHtml .= '<td  >&nbsp;<b><span style="">' . $qSeq . '.</span>&nbsp;' . $quetitle . '</b></td>';
    }
    $nonchoiceHtml .= '</tr>
                           </tbody>
                        </table>
...
function getQuestionWiseTrendExportReportData($survey_id = '', $survey_type = '', $accesible_submissions = array(), $selectedRange = array(), $qID = '', $fromIndividualQuestion = false, $takeDisabledQuestion) {
    global $db;
    $trendsQReportDataMainArray = array();
    $trendsQReportDataArray = array();
    $trendsQReportDataMainArray = getTrendQuestionWiseSubmissionData($survey_id, $survey_type, $accesible_submissions, $takeDisabledQuestion);
...
function getQuestionWiseExportReportData($survey_id = '', $survey_type, $total_submitted_que, $GF_QueLogic_Passed_Submissions = array(), $questionID = '', $fromIndividualQuestion = false, $takeDisabledQuestion = false) {
    global $db;
...
//$choiceQType = array('netpromoterscore');
    ...
    $appnedDisabledQuesWhere = '';
    if ((string) $takeDisabledQuestion == 'false' && $fromIndividualQuestion != true) {
        $takeDisabledQuestion = false;
    } else if ((string) $takeDisabledQuestion == 'true' && $fromIndividualQuestion != true) {
        $takeDisabledQuestion = true;
    } else if ($fromIndividualQuestion == true) {
        $takeDisabledQuestion = true;
    }
    if (!$takeDisabledQuestion) {
        $appnedDisabledQuesWhere = " and bc_survey_questions.disabled_question != 1";
    }
    $q = "SELECT
                bc_survey_questions.id,
                bc_survey_questions.name,
              ...
              WHERE
                bc_survey_bc_survey_questions_c.bc_survey_bc_survey_questionsbc_survey_ida = '{$survey_id}'
                 {$appnedQuesWhere} {$appnedDisabledQuesWhere}
              ORDER BY
                bc_survey_pages.page_sequence,
    ...
    while ($data = $db->fetchByAssoc($runQ)) {
        ...
        $disabled_question = $data['disabled_question'];
        $queId = $data['id'];
        $qTitle = $data['name'];
...
//random color display for chart
            ...
            //changes regarding disabled question
            $tableStrucutureDataArray['disabled_question'] = $disabled_question;
...
function getExportReportData($type = '', $survey_id = '', $exportReport = '', $exportBy = '', $exportAs = '', $status_type = '', $selectedRange = array(), $GF_QueLogic_Passed_Submissions = array(), $takeDisabledQuestion = '') {
    global $db, $app_list_strings;
    require_once('include/DotbQuery/DotbQuery.php');
    if ($type == 'status') {
...
// check related survey
            ...
            $sName = $db->fetchByAssoc($runQ);
            $surveyName = $sName['name'];
            $trendsQReportDataArray = getTrendQuestionWiseSubmissionData($survey_id, $status_type, $GF_QueLogic_Passed_Submissions, $takeDisabledQuestion);
            $returnExportData = array();
            $trendDataReport = array();
            $i = 0;
            foreach ($trendsQReportDataArray as $qID => $trendReportData) {
...
function exportStatusReportDataAsPDF($report_type = '', $exportReport = '', $statusPieImgData = '', $statusLnImgData = '', $surveyName = '', $finalExportData = array()) {
    ...
    if ($report_type == 'status') {
        ...
        if ($exportReport == 'normal') {
            ...
            if (DotbAutoLoader::fileExists($path_pie) && DotbAutoLoader::fileExists($path_ln)) {
...
function exportQuestionTrendReportDataAsPDF($questionPDFData = array(), $survey_id = '', $survey_type = '', $GF_QueLogic_Passed_SubmissionsArr = array(), $selectedRangeVal = array(), $surveyName = '', $qID = '', $fromIndividualQuestion = false, $exportAS = '', $takeDisabledQuestion) {
    global $db;
    $removebaleFile = array();
    ...
    $retunrChartData = getQuestionWiseTrendExportReportData($survey_id, $survey_type, $GF_QueLogic_Passed_Submissions, $selectedRangeVal, $qID, $fromIndividualQuestion, $takeDisabledQuestion);
    $AnsweredAndSkippedPerson = getAnswerSubmissionAnsweredAndSkipped($survey_id, $survey_type, $total_submitted_que, '', $GF_QueLogic_Passed_Submissions);
    $rowDataArray = array();
    ...
    foreach ($retunrChartData as $qID => $tableStructureDataArray) {
        ...
        if (in_array($question_type, $choiceQType)) {
            ...
            //changes regarding for disabled question in export table structure
            $rowDataArray['disabled_question'] = $qDataArr['disabled_question'];
            if ($fromIndividualQuestion && $exportAS == 'image') {
                ...
                $pie_final_ext = $returnDataPie['final_ext'];
                $path_pie = "upload/$pieID" . $pie_final_ext;
                if (DotbAutoLoader::fileExists($path_pie)) {
                    $removebaleFile[] = $path_pie;
...
function exportQuestionNormalReportDataAsPDF($questionPDFData = array(), $survey_id = '', $survey_type = '', $GF_QueLogic_Passed_SubmissionsArr = array(), $surveyName = '', $qID = '', $fromIndividualQuestion = false, $exportAS = '', $takeDisabledQuestion = '') {
    $removebaleFile = array();
    $GF_QueLogic_Passed_Submissions = $GF_QueLogic_Passed_SubmissionsArr['accesible_submissions'];
    $total_submitted_que = $GF_QueLogic_Passed_SubmissionsArr['total_send_survey'];
    $pdfHtml = '';
    ...
    $textType = array('textbox', 'commentbox', 'date-time', 'contact-information');
    $matrixChoiceQType = array('matrix');
    $tableStructureDataMainArray = getQuestionWiseExportReportData($survey_id, $survey_type, $total_submitted_que, $GF_QueLogic_Passed_Submissions, $qID, $fromIndividualQuestion, $takeDisabledQuestion);
    $rowDataArray['surveyTitle'] = $surveyName;
    $rowDataArray['totalResponse'] = $total_submitted_que;
    ...
    foreach ($tableStructureDataMainArray as $qID => $tableStructureDataArray) {
        ...
        if (in_array($tableStructureDataArray['tableStrucutureDataArray']['qType'], $matrixChoiceQType)) {
            ...
            $rowDataArray['fromIndividualQuestion'] = $fromIndividualQuestion;
            //changes regarding for disabled question in export table structure
            $rowDataArray['disabled_question'] = $tableStructureDataArray['tableStrucutureDataArray']['disabled_question'];
            if ($fromIndividualQuestion && $exportAS == 'image') {
                $rowDataArray['path_Img'] = $chartImg;
            } else {
                $returnDataPie = uploadExportReportImages($chartImg);
...
function getAllQuestionExportData($report_type = '', $survey_id, $exportReport = '', $exportBy = '', $exportAs = '', $statustype = '', $selectedRange = array(), $returnData = array(), $takeDisabledQuestion = '') {
    global $db;
    $submtData = array();
    ...
    if ($report_type == 'question' && $exportAs == 'pdf' && $exportReport == 'normal') {
        ...
        $submtData = getExportReportData($report_type, $survey_id, $exportReport, $exportBy, $exportAs, $statustype, $selectedRange, $GF_QueLogic_Passed_Submissions, $takeDisabledQuestion);
...
// By Biztech. 
            foreach ($submtData as $sbmtId => $sbmtData) {
                $submitDataArry = $submitted_question_obj->custom_retrieve_by_string_fields(array('survey_ID' => $survey_id, 'submission_id' => $sbmtId, 'exportQuestionReportCSV' => 'exportQuestionReportCSV'));
                $disabled_questionQuery = '';
                if ($takeDisabledQuestion == 'false') {
                    $disabled_questionQuery = 'and bc_survey_questions.disabled_question != 1';
                }
                $QuestionsFromSurvey = "SELECT
                            bc_survey_questions.id,
                            bc_survey_questions.name,
                ...
                while ($QuestionsFromSurveyData = $db->fetchByAssoc($runQuestionQuery)) {
                    if (array_key_exists($QuestionsFromSurveyData['id'], $submitDataArry)) {
                        if ($takeDisabledQuestion == 'false') {
                            if ($QuestionsFromSurveyData['disabled_question'] == 1) {
                                unset($submitDataArry[$QuestionsFromSurveyData['id']]);
                            }
                        }
                    } else {
                        if ($takeDisabledQuestion == 'true') {
                            $submitDataArry[$QuestionsFromSurveyData['id']][$QuestionsFromSurveyData['name']] = 'N/A';
                        } else if ($takeDisabledQuestion == 'false') {
                            if ($QuestionsFromSurveyData['disabled_question'] != 1) {
                                $submitDataArry[$QuestionsFromSurveyData['id']][$QuestionsFromSurveyData['name']] = 'N/A';
...
// where condition
    ...
    } else if (isset($status_type) && $status_type == 'sms') {
        $query->where()->equals('bc_survey_submission.submission_type', 'SMS');
    } else if (isset($status_type) && $status_type == 'whatsapp') {
        $query->where()->equals('bc_survey_submission.submission_type', 'WhatsApp');
    } else if (isset($status_type) && $status_type == 'manual') {
        $query->where()->equals('bc_survey_submission.submission_type', 'Manual');
    } else {
...
/*
 * Get Survey Question chart Data based on selected question For Dashlet
 */

function getSurveyQuestionChartData($survey_id, $que_id, $module_name, $record_id, $show_all_transactions) {
    global $db, $current_user;
    if (!empty($module_name) && !empty($record_id) && $module_name != 'Dashboards') {
        $selQry = "SELECT bc_survey_submit_question.id AS id FROM bc_survey_submit_question  LEFT JOIN bc_survey_submission  ON bc_survey_submission.id = bc_survey_submit_question.submission_id WHERE bc_survey_submit_question.question_id = '$que_id' AND bc_survey_submit_question.deleted = 0  AND bc_survey_submit_question.reciepient_module='$module_name' AND bc_survey_submission.target_parent_id = '$record_id' ";
    } else {
        if ($show_all_transactions == 'true' || is_admin($current_user)) {
            $selQry = "SELECT * FROM bc_survey_submit_question WHERE question_id = '$que_id' AND deleted = 0 ";
        } else {
            $selQry = "SELECT bc_survey_submit_question.id AS id FROM bc_survey_submit_question LEFT JOIN bc_survey_submission  ON bc_survey_submission.id = bc_survey_submit_question.submission_id  WHERE bc_survey_submit_question.question_id = '$que_id' AND bc_survey_submission.deleted = 0 AND (bc_survey_submission.assigned_user_id = '{$current_user->id}' OR bc_survey_submission.assigned_user_id IS NULL) ";
        }
    }
    $resultSel = $db->query($selQry);
    ...
    $result = array();
    $answerList = array();
    while ($row = $db->fetchByAssoc($resultSel)) {
        $oSubmittedQuestion = BeanFactory::getBean('bc_survey_submit_question', $row['id']);
        $oAnswers = $oSubmittedQuestion->get_linked_beans('bc_survey_submit_question_bc_survey_answers', 'bc_survey_answers');

        foreach ($oAnswers as $ans) {
            if (!in_array($ans->answer_name, $answerList)) {
                // Multi Selection Question Type
                if ($oSubmittedQuestion->question_type == 'multiselectlist' || $oSubmittedQuestion->question_type == 'check-box') {
                    $answerListArray = explode(',', $ans->answer_name);
                    foreach ($answerListArray as $k => $answer) {
                        if (isset($answerList[$answer]) && !empty($answerList[$answer])) {
                    ...
                    // Rating
                    if ($oSubmittedQuestion->question_type == 'rating') {
                        if (isset($answerList[$ans->answer_name]) && !empty($answerList[$ans->answer_name])) {
                            $answerList[$ans->answer_name . ' Stars'] = $answerList[$ans->answer_name] + 1;
                        } else {
                            $answerList[$ans->answer_name . ' Stars'] = 1;
                        }
                    } else {
                        if (isset($answerList[$ans->answer_name]) && !empty($answerList[$ans->answer_name])) {
                            $answerList[$ans->answer_name] = $answerList[$ans->answer_name] + 1;
                        } else {
                            $answerList[$ans->answer_name] = 1;
                        }
                    }
                }
            }
    ...
    $result['label'] = array();
    $result['values'] = array();
    // Set values and label for Chart Data
    foreach ($answerList as $answer_name => $answer_count) {
        // Label
        $result['label'][] = "$answer_name";
        // Values
        $result['values'][] = array(
            'label' => array
                (
...
/*
 * Get Survey Response chart Data based on selected survey For Dashlet
 */

function getSurveyResponseChartData($survey_id, $accesible_submissions) {
    global $db, $current_user;

    $surveyResponse = array();
    //Total Responses - Shows the total number of responses for a particular survey.
    $totalResQuery = "SELECT
                    COUNT(submission.submission_date) AS totalResponse
                  FROM
    ...
    $totalResSel = $db->query($totalResQuery);
    $totalResData = $db->fetchByAssoc($totalResSel);
    $surveyResponse['totalResponse'] = $totalResData['totalResponse'];

    //Latest Response on - Shows date till which the last survey response has been collected.
    $lastSubmittedDateQuery = "SELECT
                   submission.submission_date AS lastSubmittedDate
                  FROM
                    bc_survey_submission AS submission
                  INNER JOIN
                    bc_survey_submission_bc_survey_c AS relation ON relation.bc_survey_submission_bc_surveybc_survey_submission_idb = submission.id
                  WHERE
                    relation.bc_survey_submission_bc_surveybc_survey_ida = '{$survey_id}' AND submission.deleted = 0 ORDER BY lastSubmittedDate DESC LIMIT 1 ";
    $lastSubmittedDateSel = $db->query($lastSubmittedDateQuery);
    $lastSubmittedDateData = $db->fetchByAssoc($lastSubmittedDateSel);
    $lastSubmittedDate = explode(' ', $lastSubmittedDateData['lastSubmittedDate']);

    //last submitted date in array
    $surveyResponse['lastSubmittedDate'] = $lastSubmittedDate[0];

    //Active for - This shows the number of days (from the first survey sent date till current day if active or else the day till it was active or open).

    $firstSubmittedDateQuery = "SELECT
                   submission.submission_date AS firstSubmittedDate
                  FROM
                    bc_survey_submission AS submission
                  INNER JOIN
                    bc_survey_submission_bc_survey_c AS relation ON relation.bc_survey_submission_bc_surveybc_survey_submission_idb = submission.id
                  WHERE
                    relation.bc_survey_submission_bc_surveybc_survey_ida = '{$survey_id}' AND submission.deleted = 0 AND submission.submission_date != 'NULL' ORDER BY firstSubmittedDate ASC LIMIT 1 ";
    $firstSubmittedDateSel = $db->query($firstSubmittedDateQuery);
    $firstSubmittedDateData = $db->fetchByAssoc($firstSubmittedDateSel);
    $firstSubmittedDateTimeData = $firstSubmittedDateData['firstSubmittedDate'];
    $firstSubmittedDate = explode(' ', $firstSubmittedDateTimeData);

    $todayDate = date("Y-m-d");

    //Is end date present in the selected survey
    $surveyEndDateQuery = "SELECT 
                   end_date 
                  FROM
                    bc_survey
                  WHERE
                    id = '{$survey_id}'";
    $surveyEndDateSel = $db->query($surveyEndDateQuery);
    $surveyEndDateData = $db->fetchByAssoc($surveyEndDateSel);
    $surveyEndDate = $surveyEndDateData['end_date'];

    if ($surveyEndDate != "") {
        if ($surveyEndDate > $todayDate) {
            $surveyActiveDays = time_range2($firstSubmittedDate[0], $todayDate);
        } else {
            $surveyActiveDays = time_range2($firstSubmittedDate[0], $surveyEndDate);
        }
    } else if ($surveyEndDate == "") {
        $firstDate = $firstSubmittedDate[0];
        $surveyActiveDays = time_range2($firstDate, $todayDate);
    }

    //survey Active in array
    $surveyResponse['activeFor'] = $surveyActiveDays;

    //Today’s Responses - Number of responses gathered for a survey.
    $todayResQuery = "SELECT
                    COUNT(submission.submission_date) AS SubmittedDate
                  FROM
                    bc_survey_submission AS submission
                  INNER JOIN
                    bc_survey_submission_bc_survey_c AS relation ON relation.bc_survey_submission_bc_surveybc_survey_submission_idb = submission.id
                  WHERE
                    relation.bc_survey_submission_bc_surveybc_survey_ida = '{$survey_id}' AND submission.deleted = 0 AND submission.submission_date LIKE '{$todayDate}%' ";
    $todayResSel = $db->query($todayResQuery);
    $todayResData = $db->fetchByAssoc($todayResSel);
    $todayResDataCount = $todayResData['SubmittedDate'];

    //todays Response Count in array
    $surveyResponse['todayResponseCount'] = $todayResDataCount;
    return $surveyResponse;
}

//Returns the time span between two dates in years  months and days for dashlet
function time_range2($start_date, $end_date) {
...
// Retrieve Answer re-labeled after Answer Piping
function get_actualAnswer_answer_piping_wise($survey_id, $que_piping_sequence, $submission_id) {
    global $db;
    $answer_name = '';
    // Get Question ID from given Piping Sequence
    $getAnsQry = "SELECT bc_survey_questions.id AS que_id, bc_survey_questions.question_type AS que_type FROM bc_survey_questions 
            LEFT JOIN bc_survey_bc_survey_questions_c 
            ON bc_survey_bc_survey_questions_c.bc_survey_bc_survey_questionsbc_survey_questions_idb = bc_survey_questions.id 
            WHERE bc_survey_questions.piping_sequence='$que_piping_sequence' AND bc_survey_bc_survey_questions_c.bc_survey_bc_survey_questionsbc_survey_ida = '$survey_id' AND bc_survey_bc_survey_questions_c.deleted = 0 ";
    $resultQry = $db->query($getAnsQry);

    // Get Answer of that question from given submission id
    if ($resultQry->num_rows != 0) {
        $que_id = $db->fetchByAssoc($resultQry)['que_id'];
        $que_type = $db->fetchByAssoc($resultQry)['que_type'];
        $qAns = "select submit_answer_id as subAnsID from bc_survey_submit_answer_calculation 
            where sent_survey_id = '{$survey_id}' and submission_id = '{$submission_id}' 
            and question_id = '{$que_id}'";
        $resultQry1 = $db->query($qAns);

        if ($resultQry1->num_rows != 0) {
            $answer_id = $db->fetchByAssoc($resultQry1)['subAnsID'];
            // Get Answer Name
            $getAnsQry2 = "SELECT answer_name FROM bc_survey_answers WHERE id='$answer_id' ";
            $resultQry2 = $db->query($getAnsQry2);
            if ($resultQry2->num_rows != 0) {
                $answer_name = $db->fetchByAssoc($resultQry2)['answer_name'];
            } else {
                $answer_name = $answer_id;
            }

            // Replace Piping string with correct answer selected
            $fullstring = $answer_name;
            ...
            for ($i = 0; $i < $occu; $i++) {
                ...
                $piping_wise_answer = get_actualAnswer_answer_piping_wise($survey_id, $current_piping_seq, $submission_id);
                $fullstring = str_replace('{{Q' . trim($current_piping_seq) . '}}', $piping_wise_answer, $fullstring);
            }
            $answer_name = $fullstring;
        }
    }
    return $answer_name;
}

// Replace Question/Answer Piping Text with Actual answer submitted
function ReplaceQAPipingText($que_type, $QorAText, $survey_id, $submission_id) {
    $QType_allowedforquestionpiping = array('radio-button', 'dropdownlist', 'check-box', 'multiselectlist', 'boolean', 'netpromoterscore', 'emojis', 'textbox', 'commentbox', 'scale', 'rating', 'date-time', 'matrix', 'contact-information', 'doc-attachment');
    $QType_appliedforanswerpiping = array('radio-button', 'dropdownlist', 'check-box', 'multiselectlist', 'emojis');

    if (in_array($que_type, $QType_allowedforquestionpiping)) {
        // Replace Piping string with Piping Answer
        $fullstring = $QorAText;
        $occu = substr_count($fullstring, "{{Q");
        $piping_seq = array();
        for ($i = 0; $i < $occu; $i++) {
 ...
 * @return array
 */
function comparisonChartData($newComparisonReportData) {
    global $db;
    //current question's question type
    $currentSurveyQuery = "SELECT question_type,maxsize FROM bc_survey_questions WHERE id = '{$newComparisonReportData['current_question_id_or_x_axis_question_id']}'";
    $currentSurveyDetails = $db->query($currentSurveyQuery);
    $currentSurveyData = $db->fetchByAssoc($currentSurveyDetails);
    $currentSurveyMaxSize = $currentSurveyData['maxsize'];
    if ($currentSurveyMaxSize == 0) {
        $currentSurveyMaxSize = 5;
    }
    $currentSurveyquestion_type = $currentSurveyData['question_type'];

    //current Survey's selected question total submitted submission
    $currentSurveyTotalSubmittedCountQuery = "SELECT
                COUNT(*) AS totalsubmittedCount
              FROM
                bc_survey_submit_answer_calculation
              WHERE
                sent_survey_id LIKE '{$newComparisonReportData['current_survey_id']}'  
                AND submit_answer_id != '' AND question_id = '{$newComparisonReportData['current_question_id_or_x_axis_question_id']}'";
    $currentSurveyTotalSubmittedCountData = $db->query($currentSurveyTotalSubmittedCountQuery);
    $currentSurTotalSubCount = $db->fetchByAssoc($currentSurveyTotalSubmittedCountData);
    $currentTotalCountSubmitted = $currentSurTotalSubCount['totalsubmittedCount'];
    if ($currentTotalCountSubmitted != 0) {
        $currentSurveySubPer = 100 / $currentTotalCountSubmitted;
    } else {
        $currentSurveySubPer = 0;
    }

    $currentSurveySubmittedCount = array();
    $currentTotalCountList = array();
    //Current survey's selected Question submission
    $currentSurveySubmittedCountQuery = "SELECT
                COUNT(*) AS totalCount,
                submit_answer_id
              FROM
                bc_survey_submit_answer_calculation
              ...
              GROUP BY
                submit_answer_id";
    $currentSurveySubmittedCountData = $db->query($currentSurveySubmittedCountQuery);
    $currentSurveySubmittedCount = $currentSurSubCountData['totalCount'];
    while ($currentSurSubCountData = $db->fetchByAssoc($currentSurveySubmittedCountData)) {
        $currentanswerid = explode(",", $currentSurSubCountData['submit_answer_id']);
        foreach ($currentanswerid as $sequence => $answerid) {
            if (array_key_exists($answerid, $currentSurveySubmittedCount)) {
                $currentTotalCountList[$answerid] = $currentSurSubCountData['totalCount'] + $currentTotalCountList[$answerid];
                $currentSurveySubmittedCount[$answerid] = ($currentSurSubCountData['totalCount'] * $currentSurveySubPer) + $currentSurveySubmittedCount[$answerid];
            } else {
                $currentTotalCountList[$answerid] = $currentSurSubCountData['totalCount'];
                $currentSurveySubmittedCount[$answerid] = $currentSurSubCountData['totalCount'] * $currentSurveySubPer;
            }
        }
    }

    //compared Survey's selected question total submitted submission
    $comparedSurveyTotalSubmittedCountQuery = "SELECT
                COUNT(*) AS totalsubmittedCount
              FROM
                bc_survey_submit_answer_calculation
              WHERE
                sent_survey_id LIKE '{$newComparisonReportData['compared_survey_id']}' 
                AND submit_answer_id != '' AND question_id = '{$newComparisonReportData['compared_question_id_or_y_axis_question_id']}'";
    $comparedSurveyTotalSubmittedCountData = $db->query($comparedSurveyTotalSubmittedCountQuery);
    $comparedSurTotalSubCount = $db->fetchByAssoc($comparedSurveyTotalSubmittedCountData);
    $comparedTotalCountSubmitted = $comparedSurTotalSubCount['totalsubmittedCount'];
    if ($comparedTotalCountSubmitted != 0) {
        $comparedSurveySubPer = 100 / $comparedTotalCountSubmitted;
    } else {
        $comparedSurveySubPer = 0;
    }

    $comparedSurveySubmittedCount = array();
    $comparedTotalCountList = array();
    //Compared survey's selected Question submission
    $comparedSurveySubmittedCountQuery = "SELECT
                COUNT(*) AS totalCount,
                submit_answer_id
              FROM
                bc_survey_submit_answer_calculation
              ...
              GROUP BY
                submit_answer_id";
    $comparedSurveySubmittedCountData = $db->query($comparedSurveySubmittedCountQuery);
    while ($comparedSurSubCountData = $db->fetchByAssoc($comparedSurveySubmittedCountData)) {
        $comparedanswerid = explode(",", $comparedSurSubCountData['submit_answer_id']);
        foreach ($comparedanswerid as $sequence => $answerid) {
            if (array_key_exists($answerid, $comparedSurveySubmittedCount)) {
//                ?$answerid
                $comparedTotalCountList[$answerid] = $comparedSurSubCountData['totalCount'] + $comparedTotalCountList[$answerid];
                $comparedSurveySubmittedCount[$answerid] = ($comparedSurSubCountData['totalCount'] * $comparedSurveySubPer) + $comparedSurveySubmittedCount[$answerid];
            } else {
                $comparedTotalCountList[$answerid] = $comparedSurSubCountData['totalCount'];
                $comparedSurveySubmittedCount[$answerid] = $comparedSurSubCountData['totalCount'] * $comparedSurveySubPer;
            }
        }
    }

    if ($currentSurveyquestion_type != 'rating') {
        //current survey answer list
        $currentSurveyAnswerList = array();
        $currentSurveyAnswerQuery = "SELECT
            bc_survey_answers.id,
            bc_survey_answers.name,
            bc_survey_answers.answer_sequence
          FROM
            bc_survey_answers
          LEFT JOIN
            bc_survey_answers_bc_survey_questions_c ON bc_survey_answers_bc_survey_questions_c.bc_survey_answers_bc_survey_questionsbc_survey_answers_idb = bc_survey_answers.id 
            AND bc_survey_answers.deleted = 0 
            AND bc_survey_answers_bc_survey_questions_c.deleted = 0
          WHERE
            bc_survey_answers_bc_survey_questions_c.bc_survey_answers_bc_survey_questionsbc_survey_questions_ida = '{$newComparisonReportData['current_question_id_or_x_axis_question_id']}' ORDER BY bc_survey_answers.answer_sequence ASC";
        $currentSurveyAnswerDetails = $db->query($currentSurveyAnswerQuery);
        while ($currentSurveyAnswerData = $db->fetchByAssoc($currentSurveyAnswerDetails)) {
            $currentSurveyAnswerList[$currentSurveyAnswerData['answer_sequence']] = array(
                "answer_name" => $currentSurveyAnswerData['name'],
                "answer_id" => $currentSurveyAnswerData['id'],
                "answer_count" => $currentTotalCountList[$currentSurveyAnswerData['id']]
            );
        }

        //compared survey answer list
        $comparedSurveyAnswerList = array();
        $comparedSurveyAnswerQuery = "SELECT
            bc_survey_answers.id,
            bc_survey_answers.name,
            bc_survey_answers.answer_sequence
          FROM
          ...
          LEFT JOIN
            ...
            AND bc_survey_answers_bc_survey_questions_c.deleted = 0
          WHERE
            bc_survey_answers_bc_survey_questions_c.bc_survey_answers_bc_survey_questionsbc_survey_questions_ida = '{$newComparisonReportData['compared_question_id_or_y_axis_question_id']}' ORDER BY bc_survey_answers.answer_sequence ASC";
        $comparedSurveyAnswerDetails = $db->query($comparedSurveyAnswerQuery);
        while ($comparedSurveyAnswerData = $db->fetchByAssoc($comparedSurveyAnswerDetails)) {
            $comparedSurveyAnswerList[$comparedSurveyAnswerData['answer_sequence']] = array(
                "answer_name" => $comparedSurveyAnswerData['name'],
                "answer_id" => $comparedSurveyAnswerData['id'],
                "answer_count" => $comparedTotalCountList[$comparedSurveyAnswerData['id']]
            );
        }
    } else {
        for ($i = 1; $i <= $currentSurveyMaxSize; $i++) {
            $currentSurveyAnswerList[$i] = array(
                "answer_name" => (string) $i,
                "answer_id" => (string) $i,
                "answer_count" => $currentTotalCountList[$i]
            );
            $comparedSurveyAnswerList[$i] = array(
                "answer_name" => (string) $i,
                "answer_id" => (string) $i,
                "answer_count" => $comparedTotalCountList[$i]
            );
        }
    }
    $totalSubQCurrent = "SELECT COUNT(*) as subCount FROM bc_survey_submit_answer_calculation WHERE sent_survey_id='{$newComparisonReportData['current_survey_id']}' AND question_id='{$newComparisonReportData['current_question_id_or_x_axis_question_id']}'";
    $totalSubDataCurrent = $db->query($totalSubQCurrent);
    $totalSubCountCurrent = $db->fetchByAssoc($totalSubDataCurrent);
    $totalSubCurrent = $totalSubCountCurrent['subCount'];

    $totalSubQCompared = "SELECT COUNT(*) as subCount FROM bc_survey_submit_answer_calculation WHERE sent_survey_id='{$newComparisonReportData['compared_survey_id']}' AND question_id='{$newComparisonReportData['compared_question_id_or_y_axis_question_id']}'";
    $totalSubDataCompared = $db->query($totalSubQCompared);
    $totalSubCountCompared = $db->fetchByAssoc($totalSubDataCompared);
    $totalSubCompared = $totalSubCountCompared['subCount'];

    $i = 1;
    $answerdropdown = array();
    foreach ($currentSurveyAnswerList as $answerSequence => $answers) {

        $answer = "Answer Option " . $answerSequence;
        $answerdropdown[$answer] = $answers['answer_name'] . " - " . $comparedSurveyAnswerList[$answerSequence]['answer_name'];
        $currentValue[$answer][] = array('Rows', $answers['answer_name'], array('role' => 'tooltip', 'p' => array('html' => 'true')), $comparedSurveyAnswerList[$answerSequence]['answer_name'], array('role' => 'tooltip', 'p' => array('html' => 'true')));
        $count = $currentSurveySubmittedCount[$answers['answer_id']];
        $countCompared = $comparedSurveySubmittedCount[$comparedSurveyAnswerList[$answerSequence]['answer_id']];
        $currentValue[$answer][] = array((string) $answer, (float) ($count), '<style>div.google-visualization-tooltip {margin-left:-55px;}</style><div><table><tr><td> <b>' . $answer . '</b> </td> </tr><tr><td> <b>' . $answers['answer_name'] . ': </b>' . number_format($count, 2, '.', '') . '%</td> </tr><tr><td> <b>Count : </b>' . $answers['answer_count'] . '</td></tr></table><div>', (float) ($countCompared), '<style>div.google-visualization-tooltip {margin-left:-55px;}</style><div><table><tr><td> <b>' . $answer . '</b></td> </tr><tr><td> <b>' . $comparedSurveyAnswerList[$answerSequence]['answer_name'] . ':</b>' . number_format($countCompared, 2, '.', '') . '% </td> </tr><tr><td> <b>Count :</b> ' . $comparedSurveyAnswerList[$answerSequence]['answer_count'] . '</td></tr></table><div>');
    }

    $array['current'] = $currentValue;
    $array['answerDropdown'] = $answerdropdown;
    $array['totalSubCompared'] = $totalSubCompared;
    $array['totalSubCurrent'] = $totalSubCurrent;
    $array['SubmittedCurrent'] = $currentTotalCountSubmitted;
    $array['SubmittedCompared'] = $comparedTotalCountSubmitted;
    return $array;
}

/**
 * Comparison Report Sort Data
 *
 * @author     Biztech Consultancy
 * @param      string - $survey_id,$submission_id
 * @param      array - $global_logic
 * @return array
 */
function getComparisonSearchData($survey_id = '', $report_type = '', $page = '', $sort_by = '', $sort_order = '', $searchby = '') {
    global $db;

    if ($sort_by != '' && $sort_order != '') {
        $searchQuery = "SELECT * FROM bc_survey_comparison_crosstab_report WHERE current_survey_id='{$survey_id}' AND report_type ='{$report_type}' AND deleted = 0 ORDER BY bc_survey_comparison_crosstab_report.{$sort_by} {$sort_order}";
    } else if ($searchby != '') {
        $searchQuery = "SELECT * FROM bc_survey_comparison_crosstab_report WHERE current_survey_id='{$survey_id}' AND report_type ='{$report_type}' AND deleted = 0 AND name LIKE '%{$searchby}%' ORDER BY bc_survey_comparison_crosstab_report.date_entered DESC";
    } else {
        $searchQuery = "SELECT * FROM bc_survey_comparison_crosstab_report WHERE current_survey_id='{$survey_id}' AND report_type ='{$report_type}' AND deleted = 0 ORDER BY bc_survey_comparison_crosstab_report.date_entered DESC";
    }
    $firstChartDataFlag = 0;
    $searchQueryDetails = $db->query($searchQuery);
    while ($searchQueryData = $db->fetchByAssoc($searchQueryDetails)) {
        $searchData[$searchQueryData['id']] = $searchQueryData;
        if ($firstChartDataFlag == 0) {
            $firstChartDataFlag = 1;
            $firstChartData = comparisonChartData($searchQueryData);
            $firstChartData['title'] = htmlspecialchars($searchQueryData['name']);
            $firstChartData['id'] = $searchQueryData['id'];
            $firstChartData['current_survey'] = $searchQueryData['current_survey_name'];
            $firstChartData['compare_survey'] = $searchQueryData['compared_survey_name'];
            $firstChartData['current_question'] = $searchQueryData['current_question_name_or_x_axis_question_name'];
            $firstChartData['compare_question'] = $searchQueryData['compared_question_name_or_y_axis_question_name'];
        }
    }
    return array('searchData' => $searchData, 'comparisonchartDetails' => $firstChartData);
}

/**
 * Crosstab Report Chart Data
 *
 * @author     Biztech Consultancy
 * @param      string - $survey_id,$submission_id
 * @param      array - $global_logic
 * @return array
 */
function crosstabChartData($newComparisonReportData) {
    global $db;

    $crossTabChartArray = array();

    //Find Y axis question type
    $y_axisSurveyQuery = "SELECT question_type,maxsize FROM bc_survey_questions WHERE id = '{$newComparisonReportData['compared_question_id_or_y_axis_question_id']}'";
    $y_axisSurveyDetails = $db->query($y_axisSurveyQuery);
    $y_axisSurveyData = $db->fetchByAssoc($y_axisSurveyDetails);
    $y_axisSurveyMaxSize = $y_axisSurveyData['maxsize'];
    if ($y_axisSurveyMaxSize == 0) {
        $y_axisSurveyMaxSize = 5;
    }
    $y_axisSurveyquestion_type = $y_axisSurveyData['question_type'];

    if ($y_axisSurveyquestion_type != "rating") {
        //Y-axis question's answers
        $secondQueSubQuery = "SELECT
            bc_survey_answers.id,
            bc_survey_answers.name,
            bc_survey_answers.answer_sequence
          FROM
          ...
          WHERE
            bc_survey_answers_bc_survey_questions_c.bc_survey_answers_bc_survey_questionsbc_survey_questions_ida = '{$newComparisonReportData['compared_question_id_or_y_axis_question_id']}' ORDER BY bc_survey_answers.answer_sequence ASC";
        $secondQueSubDetails = $db->query($secondQueSubQuery);
        $i = 1;
        while ($secondQueSubData = $db->fetchByAssoc($secondQueSubDetails)) {
            if (!isset($crossTabChartArray[0][0])) {
                $crossTabChartArray[0][0] = "Rows";
                $crossTabChartArray[0][$i] = $secondQueSubData['name'];
                $i++;
            } else {
                $crossTabChartArray[0][$i] = $secondQueSubData['name'];
                $i++;
            }
            $answerList[$secondQueSubData['id']] = $secondQueSubData['name'];
        }
    } else {
        for ($i = 1; $i <= $y_axisSurveyMaxSize; $i++) {
            if (!isset($crossTabChartArray[0][0])) {
                $crossTabChartArray[0][0] = "Rows";
                $crossTabChartArray[0][$i] = (string) $i;
            } else {
                $crossTabChartArray[0][$i] = (string) $i;
            }
            $answerList[$i] = $i;
    ...
    //Find Y axis question type
    $x_axisSurveyQuery = "SELECT question_type,maxsize FROM bc_survey_questions WHERE id = '{$newComparisonReportData['current_question_id_or_x_axis_question_id']}'";
    $x_axisSurveyDetails = $db->query($x_axisSurveyQuery);
    $x_axisSurveyData = $db->fetchByAssoc($x_axisSurveyDetails);
    $x_axisSurveyMaxSize = $x_axisSurveyData['maxsize'];
    $x_axisSurveyquestion_type = $x_axisSurveyData['question_type'];
    if ($x_axisSurveyMaxSize == 0 && $x_axisSurveyquestion_type == "rating") {
        $x_axisSurveyMaxSize = 5;
    }

    if ($x_axisSurveyquestion_type != "rating") {
        //X-axis question's answers
        $firstQueAnsQuery = "SELECT
            bc_survey_answers.id,
            bc_survey_answers.name,
            bc_survey_answers.answer_sequence
          FROM
          ...
          WHERE
            bc_survey_answers_bc_survey_questions_c.bc_survey_answers_bc_survey_questionsbc_survey_questions_ida = '{$newComparisonReportData['current_question_id_or_x_axis_question_id']}' ORDER BY bc_survey_answers.answer_sequence ASC";
        $firstQueAnsDetails = $db->query($firstQueAnsQuery);
        $j = 1;
        while ($firstQueAnsData = $db->fetchByAssoc($firstQueAnsDetails)) {
            for ($k = 0; $k < count($crossTabChartArray[0]); $k++) {
                if ($k == 0) {
                    $crossTabChartArray[$j][$k] = $firstQueAnsData['name'];
                } else {
                    $crossTabChartArray[$j][$k] = 0;
                }
            }
            $j++;
            $answerList[$firstQueAnsData['id']] = $firstQueAnsData['name'];
            $answer_sequence[$firstQueAnsData['name']] = $firstQueAnsData['answer_sequence'];
        }
        $x_axisSurveyMaxSize = count($answer_sequence);
    } else {
        $j = 1;
        for ($i = 1; $i <= $x_axisSurveyMaxSize; $i++) {
            for ($k = 0; $k < count($crossTabChartArray[0]); $k++) {
                if ($k == 0) {
                    $crossTabChartArray[$i][$k] = $i;
                } else {
                    $crossTabChartArray[$i][$k] = 0;
                }
            }
            $answerList[$i] = $i;
            $answer_sequence[$i] = $i;
    ...
    //X-axis question's submitted answer
    $xAxisQueSubQuery = "SELECT
                    *
                  FROM
                    bc_survey_submit_answer_calculation
                  WHERE
                    sent_survey_id = '{$newComparisonReportData['current_survey_id']}' AND question_id = '{$newComparisonReportData['current_question_id_or_x_axis_question_id']}' AND submit_answer_id != ''";
    $xAxisQueSubDetails = $db->query($xAxisQueSubQuery);
    while ($xAxisQueSubData = $db->fetchByAssoc($xAxisQueSubDetails)) {
        $crosstabanswerid = explode(",", $xAxisQueSubData['submit_answer_id']);
        foreach ($crosstabanswerid as $sequence => $answerid) {
            if ($xAxisQueSubData['answer_type'] != rating) {
                $crosstabSubAnsXAxis[$xAxisQueSubData['submission_id']][$sequence] = $answerList[$answerid];
            } else {
                $crosstabSubAnsXAxis[$xAxisQueSubData['submission_id']][$sequence] = $answerid;
            }
        }
    }
    //y-axis question's submitted answer
    $yAxisQueSubQuery = "SELECT
                    *
                  FROM
                    bc_survey_submit_answer_calculation
                  WHERE
                    sent_survey_id = '{$newComparisonReportData['current_survey_id']}' AND question_id = '{$newComparisonReportData['compared_question_id_or_y_axis_question_id']}' AND submit_answer_id != ''";
    $yAxisQueSubDetails = $db->query($yAxisQueSubQuery);
    while ($yAxisQueSubData = $db->fetchByAssoc($yAxisQueSubDetails)) {
        $crosstabanswerid = explode(",", $yAxisQueSubData['submit_answer_id']);
        foreach ($crosstabanswerid as $sequence => $answerid) {
            if ($xAxisQueSubData['answer_type'] != "rating") {
                if ($answerid != "") {
                    $crosstabSubAnsYAxis[$yAxisQueSubData['submission_id']][$sequence] = $answerList[$answerid];
                }
            } else {
                $crosstabSubAnsYAxis[$yAxisQueSubData['submission_id']][$sequence] = $answerid;
            }
        }
    }

    foreach ($crosstabSubAnsXAxis as $sub_id => $subAnsData) {
        if (isset($crosstabSubAnsYAxis[$sub_id])) {
            foreach ($subAnsData as $seq => $answer) {
                foreach ($crosstabSubAnsYAxis[$sub_id] as $sequence => $secQueAnswer) {
                    $firstQuekey = $answer_sequence[$answer];
                    $secondQuekey = array_search($secQueAnswer, $crossTabChartArray[0]);
                    if (isset($crossTabChartArray[$firstQuekey][$secondQuekey])) {
                        $crossTabChartArray[$firstQuekey][$secondQuekey] = $crossTabChartArray[$firstQuekey][$secondQuekey] + 1;
                    }
                }
            }
        }
    ...
    foreach ($crossTabChartArray as $rows => $columnDetails) {
        if ($rows != 0) {
            $totalValue = 0;
            foreach ($columnDetails as $column => $columData) {
                if ($column != 0) {
                    if ($totalValue == 0) {
                        $totalValue = $crossTabChartArray[$rows][$column];
                    } else {
                        $totalValue = $totalValue + $crossTabChartArray[$rows][$column];
                    }
                }
            }
            $totalValueArray[$rows] = $totalValue;
            if ($totalValue != 0) {
                $totalPercentage[$rows] = 100 / $totalValue;
            } else {
                $totalPercentage[$rows] = 0;
            }
        }
    }

    $crossTabTableArray = $crossTabChartArray;
    foreach ($totalPercentage as $perSeq => $pecentage) {
        foreach ($crossTabChartArray[$perSeq] as $key => $finalPercentage) {
            if ($key != 0) {
//                $crossTabChartArray[$perSeq][$key] = $finalPercentage * $pecentage;
                $percent = (int) ($finalPercentage * $pecentage);
                if ($percent == ($finalPercentage * $pecentage)) {
                    $crossTabChartArray[$perSeq][$key] = $finalPercentage * $pecentage;
                } else {
                    $crossTabChartArray[$perSeq][$key] = (float) (number_format($finalPercentage * $pecentage, 2));
                }
//                $crossTabChartArray[$perSeq][$key] = number_format((float)$percent, 2);
                $crossTabTableChartarray[$perSeq][$key] = (number_format((float) $finalPercentage * $pecentage, 2, '.', ''));
            }
        }
    }
    $crossChartArray = array();
    foreach ($crossTabChartArray as $crossTabChartArrayKey => $crossTabChartArrayValue) {
        if ($crossTabChartArrayKey == 0) {
            foreach ($crossTabChartArrayValue as $key => $val) {
                if ($key == 0) {
                    $crossChartArray[$crossTabChartArrayKey][$key] = $val;
                } else {
                    $countone = count($crossChartArray[$crossTabChartArrayKey]);
                    $crossChartArray[$crossTabChartArrayKey][$countone] = $val;
                    $counttwo = count($crossChartArray[$crossTabChartArrayKey]);
                    $crossChartArray[$crossTabChartArrayKey][$counttwo] = array('role' => 'tooltip', 'p' => array('html' => 'true'));
                }
            }
        } else {
            foreach ($crossTabChartArrayValue as $key => $val) {
                if ($key == 0) {
                    ...
                    $crossChartArray[$crossTabChartArrayKey][$countone] = $val;
                    $counttwo = count($crossChartArray[$crossTabChartArrayKey]);
                    $crossChartArray[$crossTabChartArrayKey][$counttwo] = "<style>div.google-visualization-tooltip {margin-left:-55px;}</style><div><table><tr><td> <b> " . $crossTabTableArray[$crossTabChartArrayKey][0] . " </b> </td> </tr><tr><td> <b>" . $crossTabTableArray[0][$key] . ": " . $crossTabChartArray[$crossTabChartArrayKey][$key] . "%</b></td> </tr><tr><td> <b>Count : " . $crossTabTableArray[$crossTabChartArrayKey][$key] . "</b></td></tr></table><div>";
                }
            }
        }
    }
//    $array['current'][0] = $crossTabChartArray;
    $array['current'][0] = $crossChartArray;
    $array['crossTabTableArray'] = $crossTabTableArray;
    $array['crossTabTableChartarray'] = $crossTabTableChartarray;
    $array['x_axisSurveyMaxSize'] = (int) $x_axisSurveyMaxSize;
    $arrayLength = count($array['crossTabTableArray']);
    $multiDimenarrayLength = count($array['crossTabTableArray'][0]);
    for ($i = 0; $i < $arrayLength; $i++) {
        if ($i == 0) {
            $array['crossTabTableArray'][$i][$multiDimenarrayLength] = 'Response Count';
        } else {
            $array['crossTabTableArray'][$i][$multiDimenarrayLength] = $totalValueArray[$i];
        }
    }
    $array['multiDimenarrayLength'] = $multiDimenarrayLength;

    return $array;
}

/**
 * Crosstab Report Sort Data
 *
 * @author     Biztech Consultancy
 * @param      string - $survey_id,$report_type,$page,$sort_by,$sort_order,$searchby
 * @param      array - $global_logic
 * @return array
 */
function getCrosstabSearchData($survey_id = '', $report_type = '', $page = '', $sort_by = '', $sort_order = '', $searchby = '') {
    global $db;
 ...
 *
 * @author     Biztech Consultancy
 * @param      array - $survey_sms
 * @return array
 */
function sendSMS($survey_sms = array()) {
// Your Account Sid and Auth Token from twilio.com/user/account
    $sid = $survey_sms['survey_sms_sid'];
    $token = $survey_sms['survey_sms_token'];
    $client = new Twilio\Rest\Client($sid, $token);
    $fromPhoneNumber = $survey_sms['survey_sms_fromPhoneNumber'];
    $to_send_message_number = json_decode($survey_sms['fields'], true);
    $record_id = $survey_sms['record_id'];
    $response_data = array();
    foreach ($to_send_message_number as $field_name => $mobile_number) {
        try {
            $from_no = $fromPhoneNumber;
            $message = $survey_sms['message'];
            ...
            $carrier_type = $number->carrier['type'];
            if ($carrier_type != 'mobile') {
                $response_data['success'] = 0;
                $response_data['msg'] = 'Invalid Mobile number.';
            } else {
                ...
                $toNoWithoutCountryCode = (strpos($data->to, '+1') !== -1) ? str_replace('+1', '', $data->to) : $data->to;
                $sender = $data->from;
                $receiver = $toNoWithoutCountryCode;
                $sid = $data->sid;
                $status = $data->status;
                if ($status == 'queued') {
                    $status = 'sent';
                }
                $GLOBALS['log']->fatal('This is the result : $statusThis is the result : $status ', print_r($status, 1));
                $response_data['success'] = 'send';
            }
        } catch (Exception $ex) {
            $response_data['success'] = 'not_send';
        }
    }
...
/**
 * send survey link through whatsapp
 *
 * @author     Biztech Consultancy
 * @param      array - $survey_sms
 * @return array
 */
function sendWhatsApp($survey_sms = array(), $submission_id = "", $survey_url = "") {
    global $db;
// Your Account Sid and Auth Token from twilio.com/user/account
    $sid = $survey_sms['survey_sms_sid'];
    $token = $survey_sms['survey_sms_token'];
    ...
    foreach ($to_send_message_number as $field_name => $mobile_number) {
        try {
            ...
            $wpmobile_no = "whatsapp:" . $mobile_no;
            $wpfrom_no = "whatsapp:" . $from_no;
            $number = $client->lookups
                    ->phoneNumbers($mobile_no)
                    ->fetch(
                    array("type" => "carrier")
            ...
            if ($carrier_type != 'mobile') {
                ...
                $sid = $data->sid;
                $status = $data->status;
                $GLOBALS['log']->fatal('This is the result : $data', print_r($data, 1));
                if ($status == 'queued') {
                    $status = 'sent';
                }
                $response_data['success'] = 'send';
            }
            $select = "SELECT whatsapp_send_response_sid FROM bc_survey_submission WHERE id = '{$submission_id}' AND deleted = 0 ";
            $result = $db->query($select);
            $rowSelect = $db->fetchByAssoc($result);
            if ($rowSelect['whatsapp_send_response_sid'] != "" && $rowSelect['whatsapp_send_response_sid'] != null) {
                $WPSId = $rowSelect['whatsapp_send_response_sid'] . "," . $sid;
                $update = "UPDATE bc_survey_submission SET whatsapp_send_response_sid = '{$WPSId}' WHERE id = '{$submission_id}' AND deleted = 0 ";
                $db->query($update);
            } else {
                $update = "UPDATE bc_survey_submission SET whatsapp_send_response_sid = '{$sid}' WHERE id = '{$submission_id}' AND deleted = 0 ";
                $db->query($update);
            }
        } catch (Exception $ex) {
            $response_data['success'] = 'not_send';
    ...
    $GLOBALS['log']->fatal('This is the result : $response_data', print_r($response_data, 1));
    return $response_data;
}

/**
 * findSMSMobileNumber
 * To find that to number in which they want to send sms is mobile number or not.
 * @author     Biztech Consultancy
 * @param      string - $survey_id,$report_type,$page,$sort_by,$sort_order,$searchby
 * @param      array - $global_logic
 * @return array
 */
function findSMSMobileNumber($mobile_number = "") {
    if ($mobile_number == "") {
        $response_data = "optOut";
    } else {
        $custom_Sms_Settings = new Administration();
        $custom_Sms_Settings->retrieveSettings('SurveySms');
        $sid = $custom_Sms_Settings->settings['SurveySms_survey_sms_sid'];
        $token = $custom_Sms_Settings->settings['SurveySms_survey_sms_token'];
        $client = new Twilio\Rest\Client($sid, $token);
        $response_data = array();
        $phoneNo = $mobile_number;
        $mobile_no = $phoneNo;
...
/**
 * genrateQRCode
 * To QR code for survey send through email or whatsApp
 * @author     Biztech Consultancy
 * @param      string - $survey_url
 * @param      string - $response_data
 * @return array
 */
function genrateQRCode($survey_url = "") {
    require_once 'custom/biz/classes/qrGenerator.php';
    $ex1 = new QRGenerator($survey_url);
    $response_data = $ex1->generate();
    return $response_data;
}
