...
//                                html += "<span data-action='answerHistory'><span class='btn btn-info pull-right answerHistory' data-que_id='" + que_id + "' onclick='getAnswerHistory(this)' style='margin-right: 3px;' title='Answer History'><i class='fa fa-history'></i></span></span><span style='float:right;font-weight:bold;background-color: #DDDDDD; border-radius: 4px; height: 18px; padding:5px;'>  " + answers['obtained_que_score'] + " / " + answers['base_que_score'] + " </span></p>";
                                if (answers['disabled_question'] == 1) {
                                    ...
                                    html += "<span data-action='answerHistory'><span class='btn btn-info pull-right answerHistory' data-que_id='" + que_id + "' onclick='getAnswerHistory(this)' style='margin-right: 3px;' title='Answer History'><i><img src='custom/include/images/History.png'  height='21px' width='21px'></i></span></span><span style='float:right;font-weight:bold;background-color: #DDDDDD; border-radius: 4px; height: 18px; padding:5px;'>  " + answers['obtained_que_score'] + " / " + answers['base_que_score'] + " </span></p>";
                                }
                            } else {
//                                html += "<span data-action='answerHistory'><span class='btn btn-info pull-right answerHistory' data-que_id='" + que_id + "' onclick='getAnswerHistory(this)' title='Answer History'><i class='fa fa-history'></i></span></span></p>";
                                if (answers['disabled_question'] == 1) {
                                    ...
                                    html += "<span data-action='answerHistory'><span class='btn btn-info pull-right answerHistory' data-que_id='" + que_id + "' onclick='getAnswerHistory(this)' title='Answer History'><i><img src='custom/include/images/History.png'  height='21px' width='21px'></i></span></span></p>";
                                }
                            }
...
function surveySliderValidationOnNextPrevClick(type, que_id, is_required, min, max, maxsize, precision, advance_type, is_datetime, is_sort, scale_slot, limit_min, lang_detail) {
    ...
    if ($('#' + que_id + '_div').parent('div').css('display') !== 'none') {
        ...
        switch (type) {
            case 'multiselectlist':
                ...
                if (value_selected == 'is_other_option' && !($('.' + que_id + '_other').val().trim()))
                {
                    $('#require_msg_' + que_id).remove();
                    if ($('#' + que_id + '_div').parent('div').children('h3').children('span:nth-child(2)').html() == undefined) {
            ...
            case 'check-box':
                ...
                // check other option is null or not
                if (isOtherSelected && !($('.' + que_id + '_other').val().trim()))
                {
                    $('#require_msg_' + que_id).remove();
                    if ($('#' + que_id + '_div').parent('div').children('h3').children('span:nth-child(2)').html() == undefined) {
    ...
    if (validate == false && type != 'contact-information') {
...
function textboxValiadation(que_id, is_required, min, max, maxsize, precision, datatype, reg, lang_detail) {
    ...
    // min limit msg
    var limit_min_msg = 'Value can not be less than ' + min + '.';
    ...
    // max limit msg
    var limit_max_msg = 'Value can not be more than' + min + '.';
...
function contactInformationValidation(que_id, is_required, requireFields, lang_detail) {
    ...
    } else if (is_required == 1 && (requireFields != null || requireFields != '') && typeof requireFields != "undefined") {
        ...
        $.each(requireFields, function (key, field) {
            if (field != null && field != '') {
                if ($('.' + que_id + '_' + field.toLowerCase()).val() == '' || $('.' + que_id + '_' + field.toLowerCase()).val() == null || ($('.' + que_id + '_' + field.toLowerCase()).val() && $('.' + que_id + '_' + field.toLowerCase()).val().trim() == '')) {
                    if ($('#' + que_id + '_div').parent('div').children('h3').find('.validation-tooltip').length == 0 || $('#' + que_id + '_div').parent('div').children('h3').find('.validation-tooltip').css('display') == 'none') {
                        $('#' + que_id + '_div').parent('div').children('h3').append('<div class="validation-tooltip" style=\'color:red;display: inline-table;\' id=\'require_msg_' + que_id + '_combine\'>  ' + req_msg + '</div>');
                        $('.' + que_id + '_' + field.toLowerCase()).focus();
                    }
                    flag = false;
                }
            }
...
function MatrixValidation(que_id, is_required, lang_detail) {
    // required msg
    var matrix_required_msg = ' This question requires one answer per row.';
...
function openHelpTipsPopUpSurvey(el, helpCommentText) {
    ...
    var theme_name = $('#survey_theme').val();
    var topParent = $(el).parents();
    var topParentLength = $(el).parents().length;
    var screenheight = $(topParent[topParentLength - 1]).css('height');
    var screenwidth = $(topParent[topParentLength - 1]).css('width');
    var Splitscreenwidth = screenwidth.split('px');
    var maxwidth = "767px";
    var Splitmaxwidth = maxwidth.split('px');
    if (parseInt(Splitscreenwidth[0]) <= parseInt(Splitmaxwidth[0]) && parseInt(Splitscreenwidth) >= 0) {
       
        $('.customClassForTooltip').css('top', offset_Question.top - 10);
        
        if (parseInt(Splitscreenwidth) >= 425) {
            $('.customClassForTooltip').css('left', (offset_Question.left + 40));
        } else {
            var left = (offset_Question.left + 40);
            $('.customClassForTooltip').css('left', left);
        }
...
function validateBody() {
    ...
    if (check_form('EditView')) {
        ...
        var qr_code_status = window.tinyMCE.get('body_text')['contentDocument'].getElementById('qr_code'); // survey link element
        var link_edit_status = window.tinyMCE.get('body_text')['contentDocument'].getElementById('dotb_text_survey_link'); // survey link element
        var qr_code_edit_status = window.tinyMCE.get('body_text')['contentDocument'].getElementById('dotb_text_qr_code'); // survey link element
        var survey = $('#survey_id').val();

        if (survey != '') {
            if ((link_status == null || link_status == '') && (qr_code_status == null || qr_code_status == '')) { //survey link not added to email body
                if ((link_edit_status == null || link_edit_status == '') && (qr_code_edit_status == null || qr_code_edit_status == '')) { // survey link not added to email body ( Edit mode)
    ...
    if (flag) { // if survey link added then submit form
        return DOTB.ajaxUI.submitForm(_form);
    }
    return false;
}
...
function insertSurveyUrlLinkEmailTemplate() {
    ...
    var data_survey_submit_unique_id = $('#survey_id').find('option:selected').attr('data_survey_submit_unique_id');

    // var surveyUrl = '<a id="survey_link" href="' + surveyUrlPart + "SURVEY_PARAMS" + '">click here</a>';
    var surveyUrl = '<a id="survey_link" href="' + surveyUrlPart + "/survey_submission.php?q=" + data_survey_submit_unique_id + '">click here</a>';
...
// insert survey link to email body
function insertSurveyQRCodeEmailTemplate() {
    var surveyUrlPart = document.getElementById('survey_url_link').value;
    var surveyID = document.getElementById('survey_id').value;
    var surveyUrl = '<span id="qr_code" href="' + surveyUrlPart + "SURVEY_PARAMS" + '">QR code: <br/> <img id="qr_code_img" src="' + surveyUrlPart + '"></span>';
    var qr_code_status = window.tinyMCE.get('body_text')['contentDocument'].getElementById('qr_code'); // survey link element
    var qr_code_edit_status = window.tinyMCE.get('body_text')['contentDocument'].getElementById('dotb_text_qr_code'); // survey link element
    ...
    if (surveyID == '') {
        ...
        app.api.call('GET', url, {}, {
            success: function (result) {
                var current_record = $('[name=record]').val();
                if ($(qr_code_status).html() != "click here" && $(qr_code_edit_status).html() != "click here" && (current_record == result || result == ''))
                {
                    var inst = tinyMCE.getInstanceById("body_text");
                    if (inst)
                        inst.getWin().focus();
                    inst.execCommand('mceInsertRawHTML', false, surveyUrl);
                } else if ($(qr_code_status).html() != "click here" && $(qr_code_edit_status).html() != "click here" && current_record != result && result != '')
                {
                    $('#survey_insert_btn_msg').text('Email template has already created for this survey.').css({
                        'font-size': '12px',
                        'font-weight': 'normal',
...
function ApplyGlogalFilter() {
    ...
    if (active_report != 'individual') {
        ...
        var GF_Filter_By = $('[name=global_filter_selection]').val();
    }
...
function executeExportReportData(el, survey_id) {
    ...
    var takeDisabledQuestion = $(el).parent().parent().find('#exportDisabledQuestion').prop('checked');
    ...
    if (exportAs == 'pdf' && reportType == 'question') {
        ...
        App.api.call('create', url, {survey_id: survey_id, status_type: exportfrom, JsonGfData: JsonGfData, exportReport: exportReport, selectedRangeVal: selectedRangeVal, takeDisabledQuestion: takeDisabledQuestion}, {
            success: function (qchartData) {
                generateChartsImg(exportby, qchartData, JsonGfData, exportfrom, reportType, selectedRangeVal, statusPieImgData, statusLnImgData, exportReport, '', '', takeDisabledQuestion);
            }
        });
    } else {
        ...
        $('#generatePDFData').find('input[name=takeDisabledQuestion]').val(takeDisabledQuestion);
        $('#generatePDFData').submit();
        $('#export_loader').hide();
        close_export_report_div(this);
    }
}


function generateChartsImg(type, qchartData, JsonGfData, exportfrom, reportType, selectedRangeVal, statusPieImgData, statusLnImgData, exportReport, fromIndividualQuestion, qID, takeDisabledQuestion) {
    var questionPDFData = {};
            ...
            google.load('visualization', '1', {packages: ['corechart'], 'callback': function ()
                {
                    ...
                    if (execution == maxexelength) {
                        if (typeof fromIndividualQuestion !== 'undefined' && fromIndividualQuestion === true) {
                            ...
                            //changes regarding for disabled question in export table structure
                            $('#questionWiseImageExport').find('input[name=takeDisabledQuestion]').val(takeDisabledQuestion);
...
// close global filter function
        ...
        html += "<input type='hidden' name='takeDisabledQuestion' value=''>";
        ...
        html += "       </div>";
        html += "   </div>";
        html += "   <div class='row' style='margin-left:2px;" + displayOpt + "'>";
        html += "   <div class='span4'>";
        html += "   <input id='exportDisabledQuestion' type='checkbox'/>";
        html += "   <span class='label-field'>Export disabled questions</span>";
        html += "   </div>";
        html += "   </div>";
...
function getAnswerHistory(el) {
    ...
    if ($(el).attr('data-submission_id')) {
        var sub_id = $(el).attr('data-submission_id');
    } else {
...
//Close survey comparison pop up
function close_comparison_report_div(el) {
    $("#comparison_report_pop_up_main_div").fadeOut(function () {
        $("#comparison_report_pop_up_main_div").hide();
    });
}
//Close survey comparison pop up
function close_crosstab_report_div(el) {
    $("#crosstab_report_pop_up_main_div").fadeOut(function () {
        $("#crosstab_report_pop_up_main_div").hide();
    });
}
