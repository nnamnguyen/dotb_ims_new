    ...
    class Code{
        function addCode(&$bean, $event, $arguments)
        {
            if(empty($bean->fetched_row)){
                ...
                $sql = "SELECT name, code_separator, code_field, custom_table, add_date, date_format, zero_padding, first_num FROM c_configid WHERE custom_table = '{$table}' ORDER BY date_entered DESC LIMIT 1;";
                $res = $GLOBALS['db']->query($sql);

                // Compare last prefix and new prefix - If there're different -> Create new code
                ...
                if((!isset($bean->fetched_row[$r['code_field']]) || $bean->fetched_row[$r['code_field']]=='') && isset($pattern) ) {
                    ...
                    $query = "SELECT ".$r['code_field']." FROM " . $r['custom_table'] ." WHERE ( ". $r['code_field'] . " <> '' AND ". $r['code_field'] . " IS NOT NULL) ORDER BY RIGHT(" . $r['code_field'] . ", ".$r['zero_padding'].") DESC LIMIT 1";
