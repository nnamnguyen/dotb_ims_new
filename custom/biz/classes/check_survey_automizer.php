...
require_once 'custom/biz/function/get_survey_automizer_fields.php';

class check_survey_automizer_class {
    /*
     * this function is used for checking automizer is set for currently saving module or not
     */
    ...
    function check_survey_automizer_method($bean, $event, $arguments) {
        ...
        if (in_array($current_saving_module, $allowed_modules)) {
            ...
            $query->where()->equals('target_module', $current_saving_module);
            $query->where()->equals('execution_occurs', 'when_record_saved');
            $query->where()->equals('status', 'active');
            ...
            $GLOBALS['log']->debug("This is the list of applicable automizer : " . print_r($listApplicableAutomizer, 1));
            ...
            foreach ($listApplicableAutomizer as $k => $automizerId) {
                ...
                foreach ($oAutomizer->bc_survey_automizer_bc_automizer_actions->getBeans() as $act) {
                    ...
                    if ($oSurvey->survey_status == 'Active') {
                        ...
                        $GLOBALS['log']->debug("This is the condition result o automizer  $automizerId : " . print_r($result, 1));
                        if ($result != NULL && !in_array(false, $result)) {
                            $GLOBALS['log']->debug("This is the no false : " . print_r('', 1));
    ...
    function check_valid_survey_automizer_conditions($automizerId, $isNew, $bean) {
        ...
        // not applicable
            ...
            $GLOBALS['log']->debug("This is not applicable to send survey from automizer");
        }
        if ($isApplicable) {
            ...
            foreach ($conditionList as $key => $condition) {
                ...
                $GLOBALS['log']->debug("This is the condition related module : " . print_r($conditionRelatedModule, 1));
                ...
                if ($isRelatedModuleCondition && !empty($conditionRelatedModule)) {
                    ...
                    foreach ($bean->field_defs as $k => $field) {
                        // $GLOBALS['log']->debug("This is the field : " . print_r($field['module'], 1));
                    ...
                    $GLOBALS['log']->debug("This is the relationship name : " . print_r($relationship_name, 1));
                    ...
                    if ($bean->load_relationship($relationship_name)) {
                        ...
                        if ($getRelatedBean) {
                            ...
                            foreach ($getRelatedBean as $relatedClass) {
                                $GLOBALS['log']->debug("This is the related bean record for condition checking : " . print_r($relatedClass->id, 1));
                                ...
                                if ($condition->filter_by == 'any_related' && $result_flag) {
                                    ...
                                    $GLOBALS['log']->debug("This is the result any related true : " . print_r($result_array_related_true, 1));
                                    break;
                                } else {
                                    $result_array_related[] = $result_flag;
                                    $GLOBALS['log']->debug("This is the result any related false 1 : " . print_r($result_array_related, 1));
                                }
                            }
                        }
                        // If no any related record
                        else {
                            $GLOBALS['log']->debug("This is the relationship bean is null 1: " . print_r($getRelatedBean, 1));
                            $reassign_array = array(false);
                            $result_array_related = array();
                            $result_array_related = $reassign_array;
                            $GLOBALS['log']->debug("This is the result any related false 2 : " . print_r($result_array_related, 1));
                            break;
                        }
                    }
                    // if no any relationship found for given relationship name
                    else {
                        $GLOBALS['log']->debug("This is the relationship bean is null 2: " . print_r($getRelatedBean, 1));
                        $reassign_array = array(false);
                        $result_array_related = array();
                        $result_array_related = $reassign_array;
                        $GLOBALS['log']->debug("This is the result any related false 3 : " . print_r($result_array_related, 1));
                        break;
                    }
                    $GLOBALS['log']->debug("This is the related bean : $conditionRelatedModule : - Relationship name : " . print_r($relationship_name, 1));
                }
            }
        }
        ...
        $GLOBALS['log']->debug("This is the result array related -------------------------: " . print_r($result_array_related, 1));
        $result_array = array_merge($result_array, $result_array_related);

        $GLOBALS['log']->debug("This is the condition result in self function: " . print_r($result_array, 1));
    ...
    function checkTargetModuleConditions($condition, $automizerId, $bean, $execution_type) {
        ...
        if ($condition->value_type == 'Value' || $condition->value_type == 'Date') {
            $result = $this->checkValueType_value_or_field($condition, $automizerId, $bean, $condition->value_type, $execution_type);
        }
        // Check value for is null
        else if ($condition->condition_operator == 'is_null') {
            $field = $condition->condition_field;
            $GLOBALS['log']->debug("This is the checking of condition : " . print_r($bean->$field . ' is....check for is empty', 1));
            if (empty($bean->$field)) {
                return true;
            }
        }
        // Check value for Any Change
            ...
            $GLOBALS['log']->debug("This is the checking of condition : " . print_r($bean->$condition_field . ' is....check for Any Change', 1));
    ...
    function checkValueType_value_or_field($condition, $automizerId, $bean, $value_type, $execution_type) {
        ...
        // Check value for Not Equals to
            ...
            $GLOBALS['log']->debug("This is the checking of condition : " . print_r($tobe_compare_value . ' is....check for Not_Equal_To' . $compare_value, 1));
        ...
        // Check value forGreater Than
            ...
            $GLOBALS['log']->debug("This is the checking of condition : " . print_r($tobe_compare_value . ' is....check for Greater_Than' . $compare_value, 1));
        ...
        // Check value for Less than
            ...
            $GLOBALS['log']->debug("This is the checking of condition : " . print_r($tobe_compare_value . ' is....check for Less_Than' . $compare_value, 1));
            if ((int) $tobe_compare_value < (int) $compare_value) {
                return true;
            }
        }
        // Check value for Greater than equals to
            ...
            $GLOBALS['log']->debug("This is the checking of condition : " . print_r($tobe_compare_value . ' is....check for Greater_Than_or_Equal_To' . $compare_value, 1));
            if ((int) $tobe_compare_value >= (int) $compare_value) {
                return true;
            }
        }
        // Check value for Less than equals to
            ...
            $GLOBALS['log']->debug("This is the checking of condition : " . print_r($tobe_compare_value . ' is....check for Less_Than_or_Equal_To' . $compare_value, 1));
            if ((int) $tobe_compare_value <= (int) $compare_value) {
                return true;
            }
        }
        // Check value for Contais given string
            ...
            $GLOBALS['log']->debug("This is the checking of condition : " . print_r($tobe_compare_value . ' is....check for Contains' . $compare_value, 1));
        ...
        // Check value for Starts with given string
            ...
            $GLOBALS['log']->debug("This is the checking of condition : " . print_r($tobe_compare_value . ' is....check for Starts with' . $condition->compare_value, 1));
        ...
        // Check value for Ends with given string
            ...
            $GLOBALS['log']->debug("This is the checking of condition : " . print_r($tobe_compare_value . ' is....check for Ends with' . $condition->compare_value, 1));
    ...
    function checkValueType_multi($condition, $automizerId, $bean, $execution_type) {
        ...
        if ($condition->condition_operator == 'Equal_To') {
            ...
            $GLOBALS['log']->debug("This is the checking of condition : " . print_r($bean->$field . ' is....check for ' . $compare_value, 1));
        ...
        // Check value for Not Equals to
            ...
            $GLOBALS['log']->debug("This is the checking of condition : " . print_r($bean->$field . ' is....check for Not_Equal_To' . $compare_value, 1));
            if (!in_array($bean->$field, $compare_value)) {
                return true;
            }
        }
        ...
        foreach ($oAutomizer->bc_survey_automizer_bc_automizer_actions->getBeans() as $act) {
            ...
            if ($oSurvey->survey_status == 'Active') {
                ...
                if ($act->recipient_type == 'target_module' && $act->recipient_module == 'email1') {
                    ...
                    $GLOBALS['log']->debug("This is the action.................................. : " . print_r($email_address, 1));
                    if (!empty($bean->id)) {
                        $module_id = $bean->id;
                    } else {
                    ...
                    $GLOBALS['log']->debug("This is the origin module id : " . print_r($module_id, 1));
                    $GLOBALS['log']->debug("This is the origin module name : " . print_r($bean->module_name, 1));
                    $this->survey_submission_entry($module_id, $bean, $survey_id, $execution_type, $act, $module_id, $bean->module_name, $bean);
                }
                // Send survey to target module email field **************************************************
                    ...
                    if (in_array($RelatedTo, $allowed_modules)) {
                        ...
                        $this->survey_submission_entry($module_id, $RelateBean, $survey_id, $execution_type, $act, $bean->id, $bean->module_name, $bean);
                    } else {
                        $GLOBALS['log']->debug("Related module $RelatedTo not match to send survey : " . print_r('', 1));
                    }
                } else if ($act->recipient_type == 'target_module' && !empty($act->recipient_module)) {
                    $recipient_record = $act->recipient_module;
                    $survey_id = $act->survey_id;

                    //Get Related module name from given field name
                    if (!empty($bean->field_defs[$recipient_record]['module'])) {
                        $recipient_record = $bean->field_defs[$recipient_record]['module'];
                    } else if (!empty($bean->$recipient_record->relationship->def['lhs_module']) && $bean->$recipient_record->relationship->def['lhs_module'] != $bean->module_name) {
                        $recipient_record = $bean->$recipient_record->relationship->def['lhs_module'];
                    } else if (!empty($bean->$recipient_record->relationship->def['rhs_module'])) {
                        $recipient_record = $bean->$recipient_record->relationship->def['rhs_module'];
                    }

                    $RelatedTo = $recipient_record;

                    // get related field id to retrieve that record
                    if (!empty($bean->field_defs[$act->recipient_module]['id_name'])) {
                        $idField = $bean->field_defs[$act->recipient_module]['id_name'];
                    }
                    $RelatedToId = $bean->$idField;
                    ...
                    if (in_array($RelatedTo, $allowed_modules)) {
                        $RelateBean = BeanFactory::getBean($RelatedTo, $RelatedToId);
                        $email_address = $RelateBean->email1;
                        $GLOBALS['log']->debug("This is the action taregt parent.................................. : " . print_r($email_address, 1));
                        if (!empty($RelatedToId)) {
                            $module_id = $RelatedToId;
                        } else {
                            $module_id = create_guid();
                ...
                // Send survey to related module *************************************************
                    ...
                    if ($bean->load_relationship($relationship_name)) {
                        ...
                        if ($getRelatedBean) {
                            ...
                            foreach ($getRelatedBean as $relatedClass) {
                                ...
                                if (empty($act->filter_by) || $act->filter_by == 'all_related') {
                                    ...
                                    $GLOBALS['log']->debug("This is the related action.................................. : " . print_r($email_address, 1));
                                    if (!empty($relatedClass->id)) {
                                        $module_id = $relatedClass->id;
                                    }
                                    $this->survey_submission_entry($module_id, $relatedClass, $survey_id, $execution_type, $act, $bean->id, $bean->module_name, $bean);
                                }
                                // Send survey to those records whose conditions matches with action
                                    ...
                                    if (($operator == 'Equal_To' && $relatedClass->$rel_field == $value) || ($operator == 'Not_Equal_To' && $relatedClass->$rel_field != $value)) {
                                        ...
                                        $GLOBALS['log']->debug("This is the action recipient condition not match for $relatedClass->id condition check for $rel_field match with $value : " . print_r('', 1));
                                    }
                                }
                            }
                        }
                        ...
                        $GLOBALS['log']->debug("This is the else part of relationship : " . print_r('', 1));
                        $survey_id = $act->survey_id;
                        $allowed_modules = array('Accounts', 'Contacts', 'Leads', 'Prospects');
                        ...
                        foreach ($beanFields as $field) {
                            if ($field['name'] == $act->recipient_module) {
                                if (in_array($field['module'], $allowed_modules)) {
                                    ...
                                    $GLOBALS['log']->debug("This is the relate module {$field['module']} and field id {$relatedId} : " . print_r('', 1));
                                    $this->survey_submission_entry($relatedId, $relatedBean, $survey_id, $execution_type, $act, $bean->id, $bean->module_name, $bean);
                                }
                            }
                        }
                    }
    ...
    function survey_submission_entry($module_id, $bean, $survey_id, $execution_type, $act, $Origin_module_id, $Origin_module_name, &$SubmissionBean) {
        ...
        $GLOBALS['log']->debug("This is the survey submission entry : " . print_r($dataArray, 1));
        $sub_unique_id = $dataArray['email_survey_sumission_unique_id'];
        if ($SubmissionBean->module_dir == 'bc_survey_submission' && $SubmissionBean->id == $dataArray['submission_id']) {
            $resend = $SubmissionBean->resend;
            $resubmit = $SubmissionBean->resubmit;
        } else {
        ...
        if ($dataArray['is_send'] == 'already_sent' && $dataArray['status'] == 'Submitted' && $resend == 0 && $resubmit == 0 && $execution_type == 'when_record_saved') {
            $this->send_survey($module_id, $module_name, $survey_id, $bean, $dataArray, 1, $act, $SubmissionBean);
        }
        ...
        if ($dataArray['is_send'] == 'scheduled' && $execution_type == 'when_record_saved') {
            $this->send_survey($module_id, $module_name, $survey_id, $bean, $dataArray, '', $act, $SubmissionBean, $sub_unique_id);
        }
    }

    /*
     * this function is used send survey when record saved
     * 
     * @params
     * $module_id - target module id
     ...
     * $survey_id - survey id
     * $bean - current bean
     * $rec_module_detail - submission detail
     * $resend - resend flag
     * $act - action bean
     */

    function send_survey($module_id, $module_name, $survey_id, $bean, $rec_module_detail, $resend, $act, $SubmissionBean, $sub_unique_id) {
        global $dotb_config;
        ...
        $moduleDetail = "&module_name=" . $module_name . "&module_id=" . $module_id;
        ...
        // retrieve submission
        if ($SubmissionBean->module_dir == 'bc_survey_submission' && $SubmissionBean->id == $rec_module_detail['submission_id']) {
            // Submission retrieve of current saving bean
        } else {
            $survey_submission = new bc_survey_submission();
...
//            $pure_data = $oSurvey->id . '&ctype=' . $module_name . '&cid=' . $module_id . '&sub_id=' . $survey_submission->id;
            $uid = $sub_unique_id;
            $pure_data = $uid;
...
//            $template_data['body_html'] = str_replace($replacing_url, $new_url, $template_data['body_html']);
            ...
            $sendMail = CustomSendEmail($to_Email, $emailSubject, $emailBody, $module_id, $module_name, $act->recipient_email_field);
            /*
             * Store survey data
             */

            $GLOBALS['log']->debug("Send sent mail status : " . print_r($sendMail, 1));
            if (trim($sendMail) == 'send') {
                $gmtdatetime = TimeDate::getInstance()->nowDb();
                // Submission retrieve of current saving bean
                if ($SubmissionBean->module_dir == 'bc_survey_submission' && $SubmissionBean->id == $rec_module_detail['submission_id']) {
                    $SubmissionBean->last_send_on = $gmtdatetime;
                    $SubmissionBean->mail_status = 'sent successfully';
                    $SubmissionBean->survey_send = 1;
                    $SubmissionBean->schedule_on = $gmtdatetime;
                    if ($resend == 1) {
                        // resend survey if survey is already send to recipient and has already submitted the respective survey
                        $resend_count = $SubmissionBean->resend_counter;
                        $resend_count++;
                        $SubmissionBean->resend_counter = $resend_count;  // update counter
                        $SubmissionBean->resubmit = 1;
                    } else {
                        $SubmissionBean->resubmit = 0;
                    }
                } else {
                    ...
                    if ($resend == 1) {
                        // resend survey if survey is already send to recipient and has already submitted the respective survey
                        $resend_count = $survey_submission->resend_counter;
                        $resend_count++;
                        $survey_submission->resend_counter = $resend_count;  // update counter
                        $survey_submission->resubmit = 1;
                        $survey_submission->resend = 1;
                    } else {
                ...
                // Submission retrieve of current saving bean
                if ($SubmissionBean->module_dir == 'bc_survey_submission' && $SubmissionBean->id == $rec_module_detail['submission_id']) {
                    $SubmissionBean->mail_status = $sendMail;
                } else {
                ...
                $GLOBALS['log']->debug("Mail Send Status From Automizer : " . print_r($sendMail, 1));
            }
        } else {
            // Submission retrieve of current saving bean
            if ($SubmissionBean->module_dir == 'bc_survey_submission' && $SubmissionBean->id == $rec_module_detail['submission_id']) {
                $SubmissionBean->mail_status = 'Survey Sending Failed due to Email Template does not exists for given survey.';
            } else {
            ...
            $GLOBALS['log']->debug("Email Template does not exists for survey : " . print_r($survey_id, 1));
        }
    }
