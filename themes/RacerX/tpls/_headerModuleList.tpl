...
dotb_theme_gm_current = '{$currentGroupTab}';
Set_Cookie('dotb_theme_gm_current','{$currentGroupTab}',30,'/','','');
</script>
...
{/php}
  ...
  {/if}
	...
	{if $name == $MODULE_TAB}
		<li class="current {$homeClass}">{dotb_link id="moduleTab_$tabGroupName$name" module=$name data=$module label=$homeImageLabel title=$title class="sf-with-ul"}
	{else}
		<li class="{$homeClass}">{dotb_link id="moduleTab_$tabGroupName$name" module=$name data=$module label=$homeImageLabel title=$title class="sf-with-ul"}
	{/if}
			...
			<div class="megawrapper">
				<div class="megacolumn">
					...
					{foreach from=$shortcutTopMenu.$name item=shortcut_item}
					  ...
					  {else}
		                {if $module == "Calendar"}
					  		<li><a id="{$shortcut_item.LABEL|replace:' ':''}{$module}{$tabGroupName}" href="{dotb_ajax_url url=$shortcut_item.URL}">{$shortcut_item.LABEL}</a></li>
					  	{else}
		                	<li><a id="{$shortcut_item.LABEL|replace:' ':''}{$tabGroupName}" href="{dotb_ajax_url url=$shortcut_item.URL}">{$shortcut_item.LABEL}</a></li>
					  	{/if}
					  {/if}
					{/foreach}
					</ul>
			...
			<div class="megawrapper">
				<div class="megacolumn">
					<div class="megacolumn-content">
							...
							<li {if $smarty.foreach.moduleList.index > 4}class="moreOverflow"{/if}>{dotb_link id="moduleTab_$tabGroupName$module" module="$module" data="$name" class="sf-with-ul"}
							...
							<li class="moduleMenuOverFlowMore" id="moduleMenuOverFlowMore{$currentGroupTab}"><a href="javascript: DOTB.themes.toggleMenuOverFlow('moduleTabMore{$currentGroupTab}','more');">{$APP.LBL_SHOW_MORE} <div class="showMoreArrow"></div></a></li>
							<li class="moduleMenuOverFlowLess" id="moduleMenuOverFlowLess{$currentGroupTab}"><a href="javascript: DOTB.themes.toggleMenuOverFlow('moduleTabMore{$currentGroupTab}','less');">{$APP.LBL_SHOW_LESS} <div class="showLessArrow"></div></a></li>
							{/if}
							</ul>		
					 		...
					 		<li><a href="#" class="group sf-with-ul" title="{$tabGroupName}">{$APP.LBL_FILTER_MENU_BY}</a>
								...
								<ul class="sf-menu filter-menu">
						          {foreach from=$groupTabs item=module key=group name=groupList}
				                      ...
				                      {/php}
						          <li {if $tabGroupName eq $group}class="selected"{/if}><a href="javascript:(DOTB.themes.dotb_theme_gm_switch('{$group}', '{$group_id}') && false)" class="{if $tabGroupName eq $group}selected{/if}">{$group}</a></li>
						          {/foreach}
								</ul>
				   				</li>
				        	</ul>
