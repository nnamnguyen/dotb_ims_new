 ...
 */
class DotbWebServiceImplv3 extends DotbWebServiceImpl {

    public function __construct()
    {
        self::$helperObject = new DotbWebServiceUtilv3();
    }
    ...
    function get_module_fields_md5($session, $module_name){
        ...
        $GLOBALS['log']->info('Begin: DotbWebServiceImpl->get_module_fields_md5(v3) for module: '. print_r($module_name, true));
    ...
    {
        $GLOBALS['log']->info('Begin: DotbWebServiceImpl->get_last_viewed');
        $error = new SoapError();
    	if (!self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', '', '', '', $error))
    	{
    		$error->set_error('invalid_login');
    		$GLOBALS['log']->info('End: DotbWebServiceImpl->get_last_viewed');
    		return;
    	} // if
    	...
    	foreach ($module_names as $module )
    	{
            ...
            {
                $GLOBALS['log']->debug("DotbWebServiceImpl->get_last_viewed: NO ACCESS to $module");
                continue;
            }
    ...
    {
        $GLOBALS['log']->info('Begin: DotbWebServiceImpl->get_upcoming_activities');
        $error = new SoapError();
    	if (!self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', '', '', '', $error))
    	{
    		$error->set_error('invalid_login');
    		$GLOBALS['log']->info('End: DotbWebServiceImpl->get_upcoming_activities');
    		return;
    	} // if
...
DotbWebServiceImplv3::$helperObject = new DotbWebServiceUtilv3();
