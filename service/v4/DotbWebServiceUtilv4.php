...
class DotbWebServiceUtilv4 extends DotbWebServiceUtilv3_1
{
   ...
   /**
	...
	/**
	 * Processes the filter_fields attribute to use with DotbBean::create_new_list_query()
	 *
	 * @param object $value DotbBean
	 * @param array $fields
	 * @return array
	 */
    protected function filter_fields_for_query(DotbBean $value, array $fields)
    {
	...
	function get_mobile_login_data(&$nameValueArray)
	{
        ...
        $nameValueArray['dotb_flavor'] = $dotb_flavor;
        $nameValueArray['dotb_version'] = $dotb_version;
	}
	...
	function get_subpanel_defs($module, $type)
	{
	    ...
	    {
	        case 'wireless':
                $defs = DotbAutoLoader::existingCustomOne('modules/'.$module.'/metadata/wireless.subpaneldefs.php');
                if($defs) {
                    require $defs;
                }

                //If an Ext/WirelessLayoutdefs/wireless.subpaneldefs.ext.php file exists, then also load it as well
                $defs = DotbAutoLoader::loadExtension("wireless_subpanels", $module);
                if($defs) {
                    require $defs;
                }
	            break;
	        ...
	        case 'default':
	        default:
	            $defs = DotbAutoLoader::loadWithMetafiles($module, 'subpaneldefs');
	            if($defs) {
	            	require $defs;
	            }
	            $defs = DotbAutoLoader::loadExtension("layoutdefs", $module);
	            if($defs) {
	            	require $defs;
	            }
     ...
     * @param DotbBean $seed bean containing the fields
