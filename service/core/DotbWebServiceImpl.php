...
DotbWebServiceImpl::$helperObject = new SoapHelperWebServices();



class DotbWebServiceImpl{

	public static $helperObject = null;

/**
 * Retrieve a single DotbBean based on ID.
 *
 * @param String $session -- Session ID returned by a previous call to login.
 * @param String $module_name -- The name of the module to return records from.  This name should be the name the module was developed under (changing a tab name is studio does not affect the name that should be passed into this method)..
 * @param String $id -- The DotbBean's ID value.
...
function get_entry($session, $module_name, $id,$select_fields, $link_name_to_fields_array){
	$GLOBALS['log']->info('Begin: DotbWebServiceImpl->get_entry');
	return self::get_entries($session, $module_name, array($id), $select_fields, $link_name_to_fields_array);
	$GLOBALS['log']->info('end: DotbWebServiceImpl->get_entry');
}
...
function get_entries($session, $module_name, $ids, $select_fields, $link_name_to_fields_array){
	$GLOBALS['log']->info('Begin: DotbWebServiceImpl->get_entries');
	$error = new SoapError();
	...
	if (!self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', $module_name, 'read', 'no_access', $error)) {
		$GLOBALS['log']->info('End: DotbWebServiceImpl->get_entries');
		return;
	} // if
 ...
 * @param array $name_value_list -- The keys of the array are the DotbBean attributes, the values of the array are the values the attributes should have.
...
function set_relationship($session, $module_name, $module_id, $link_field_name, $related_ids, $name_value_list, $delete){
	$GLOBALS['log']->info('Begin: DotbWebServiceImpl->set_relationship');
	$error = new SoapError();
	if (!self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', '', '', '', $error)) {
		$GLOBALS['log']->info('End: DotbWebServiceImpl->set_relationship');
		return;
	} // if
 ...
 * @param array $name_value_lists -- Array of Array. The keys of the inner array are the DotbBean attributes, the values of the inner array are the values the attributes should have.
...
function set_relationships($session, $module_names, $module_ids, $link_field_names, $related_ids, $name_value_lists, $delete_array) {
	$GLOBALS['log']->info('Begin: DotbWebServiceImpl->set_relationships');
	$error = new SoapError();
	if (!self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', '', '', '', $error)) {
		$GLOBALS['log']->info('End: DotbWebServiceImpl->set_relationships');
		return;
	} // if
...
function get_relationships($session, $module_name, $module_id, $link_field_name, $related_module_query, $related_fields, $related_module_link_name_to_fields_array, $deleted){
	...
	$GLOBALS['log']->info('Begin: DotbWebServiceImpl->get_relationships');
	$error = new SoapError();
	if (!self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', $module_name, 'read', 'no_access', $error)) {
		$GLOBALS['log']->info('End: DotbWebServiceImpl->get_relationships');
		return;
	} // if
...
/**
 * Update or create a single DotbBean.
 *
 * @param String $session -- Session ID returned by a previous call to login.
 * @param String $module_name -- The name of the module to return records from.  This name should be the name the module was developed under (changing a tab name is studio does not affect the name that should be passed into this method)..
 * @param Array $name_value_list -- The keys of the array are the DotbBean attributes, the values of the array are the values the attributes should have.
...
function set_entry($session,$module_name, $name_value_list){
	...
	$GLOBALS['log']->info('Begin: DotbWebServiceImpl->set_entry');
    if (self::$helperObject->isLogLevelDebug()) {
	...
	if (!self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', $module_name, 'write', 'no_access', $error)) {
		$GLOBALS['log']->info('End: DotbWebServiceImpl->set_entry');
		return;
	} // if
	$seed = BeanFactory::newBean($module_name);
    ...
    } catch (DotbApiExceptionNotAuthorized $ex) {
        $GLOBALS['log']->info('End: DotbWebServiceImpl->set_entry');
...
/**
 * Update or create a list of DotbBeans
 *
 * @param String $session -- Session ID returned by a previous call to login.
 * @param String $module_name -- The name of the module to return records from.  This name should be the name the module was developed under (changing a tab name is studio does not affect the name that should be passed into this method)..
 * @param Array $name_value_lists -- Array of Bean specific Arrays where the keys of the array are the DotbBean attributes, the values of the array are the values the attributes should have.
...
function set_entries($session,$module_name, $name_value_lists){
	$GLOBALS['log']->info('Begin: DotbWebServiceImpl->set_entries');
    if (self::$helperObject->isLogLevelDebug()) {
		$GLOBALS['log']->debug('SoapHelperWebServices->set_entries - input data is ' . var_export($name_value_lists, true));
    } // if
	$error = new SoapError();
	if (!self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', $module_name, 'write', 'no_access', $error)) {
		$GLOBALS['log']->info('End: DotbWebServiceImpl->set_entries');
		return;
	} // if
 ...
 */
    ...
    {
	$GLOBALS['log']->info('Begin: DotbWebServiceImpl->login');
	global $dotb_config;
	$error = new SoapError();
	...
	if($success){
		...
		$GLOBALS['log']->info('End: DotbWebServiceImpl->login - succesful login');
	...
	self::$helperObject->setFaultObject($error);
	$GLOBALS['log']->info('End: DotbWebServiceImpl->login - failed login');
}
...
function logout($session){
	...
	$GLOBALS['log']->info('Begin: DotbWebServiceImpl->logout');
	$error = new SoapError();
	LogicHook::initialize();
	if (!self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', '', '', '', $error)) {
		$GLOBALS['logic_hook']->call_custom_logic('Users', 'after_logout');
		$GLOBALS['log']->info('End: DotbWebServiceImpl->logout');
		return;
	} // if
 ...
 * @return Array - flavor - String - Retrieve the specific flavor of dotb.
 * 				 - version - String - Retrieve the version number of Dotb that the server is running.
 * 				 - gmt_time - String - Return the current time on the server in the format 'Y-m-d H:i:s'. This time is in GMT.
 * @exception 'SoapFault' -- The SOAP error, if any
 */
function get_server_info(){
	$GLOBALS['log']->info('Begin: DotbWebServiceImpl->get_server_info');
	global $dotb_flavor;
	require_once('dotb_version.php');

	$admin = Administration::getSettings('info');
	$dotb_version = '';
	if(isset($admin->settings['info_dotb_version'])){
		$dotb_version = $admin->settings['info_dotb_version'];
	}else{
		$dotb_version = '1.0';
	}

	$GLOBALS['log']->info('End: DotbWebServiceImpl->get_server_info');
	return array('flavor' => $dotb_flavor, 'version' => $dotb_version, 'gmt_time' => TimeDate::getInstance()->nowDb());
} // fn
...
function get_user_id($session){
	$GLOBALS['log']->info('Begin: DotbWebServiceImpl->get_user_id');
	$error = new SoapError();
	if (!self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', '', '', '', $error)) {
		return;
	} // if
	global $current_user;
	$GLOBALS['log']->info('End: DotbWebServiceImpl->get_user_id');
...
function get_module_fields($session, $module_name, $fields = array()){
	$GLOBALS['log']->info('Begin: DotbWebServiceImpl->get_module_fields for ' . $module_name);
	$error = new SoapError();
	$module_fields = array();

	if (!self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', $module_name, 'read', 'no_access', $error)) {
		$GLOBALS['log']->error('End: DotbWebServiceImpl->get_module_fields FAILED on checkSessionAndModuleAccess for ' . $module_name);
		return;
	} // if
	...
	if($seed->ACLAccess('ListView', true) || $seed->ACLAccess('DetailView', true) || 	$seed->ACLAccess('EditView', true) ) {
        ...
        $GLOBALS['log']->info('End: DotbWebServiceImpl->get_module_fields SUCCESS for ' . $module_name);
        return $return;
    }
    $error->set_error('no_access');
	self::$helperObject->setFaultObject($error);
    $GLOBALS['log']->error('End: DotbWebServiceImpl->get_module_fields FAILED NO ACCESS to ListView, DetailView or EditView for ' . $module_name);
}
...
function seamless_login($session){
	$GLOBALS['log']->info('Begin: DotbWebServiceImpl->seamless_login');
	if(!self::$helperObject->validate_authenticated($session)){
		return 0;
	}
	$_SESSION['seamless_login'] = true;
	$GLOBALS['log']->info('End: DotbWebServiceImpl->seamless_login');
	return 1;
}
...
function set_note_attachment($session, $note) {
	$GLOBALS['log']->info('Begin: DotbWebServiceImpl->set_note_attachment');
	$error = new SoapError();
	$module_name = '';
	$module_access = '';
	$module_id = '';
	...
	if (!self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', $module_name, $module_access, 'no_access', $error)) {
		$GLOBALS['log']->info('End: DotbWebServiceImpl->set_note_attachment');
		return;
	} // if
...
function get_note_attachment($session,$id) {
	$GLOBALS['log']->info('Begin: DotbWebServiceImpl->get_note_attachment');
	$error = new SoapError();
	if (!self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', '', '', '', $error)) {
		$GLOBALS['log']->info('End: DotbWebServiceImpl->get_note_attachment');
		return;
	} // if
...
function set_document_revision($session, $document_revision) {
	$GLOBALS['log']->info('Begin: DotbWebServiceImpl->set_document_revision');
	$error = new SoapError();
	if (!self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', '', '', '', $error)) {
		$GLOBALS['log']->info('End: DotbWebServiceImpl->set_document_revision');
		return;
	} // if
...
function get_document_revision($session, $id) {
	$GLOBALS['log']->info('Begin: DotbWebServiceImpl->get_document_revision');
    global $dotb_config;

    $error = new SoapError();
	if (!self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', '', '', '', $error)) {
		$GLOBALS['log']->info('End: DotbWebServiceImpl->get_document_revision');
		return;
	} // if
...
function search_by_module($session, $search_string, $modules, $offset, $max_results){
    ...
    if(!file_exists($cachedfile = dotb_cached('modules/unified_search_modules.php'))) {
        $usa->buildCache();
    }
	...
	if(!empty($search_string) && isset($search_string)) {
    	...
    	foreach($modules_to_search as $name => $beanName) {
			...
			if ($beanName != "User"
				...
				require_once DotbAutoLoader::loadWithMetafiles($seed->module_dir, 'listviewdefs');
...
function get_available_modules($session){
	$GLOBALS['log']->info('Begin: DotbWebServiceImpl->get_available_modules');

	$error = new SoapError();
	if (!self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', '', '', '', $error)) {
		$error->set_error('invalid_login');
		$GLOBALS['log']->info('End: DotbWebServiceImpl->get_available_modules');
		return;
	} // if
...
function get_user_team_id($session){
	$GLOBALS['log']->info('Begin: DotbWebServiceImpl->get_user_team_id');

	$error = new SoapError();
	if (!self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', '', '', '', $error)) {
		$error->set_error('invalid_login');
		$GLOBALS['log']->info('End: DotbWebServiceImpl->get_user_team_id');
		return;
	} // if
	global $current_user;
	$GLOBALS['log']->info('End: DotbWebServiceImpl->get_user_team_id');
...
/**
*   Once we have successfuly done a mail merge on a campaign, we need to notify Dotb of the targets
...
function set_campaign_merge($session,$targets, $campaign_id){
	$GLOBALS['log']->info('Begin: DotbWebServiceImpl->set_campaign_merge');

	$error = new SoapError();
	if (!self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', '', '', '', $error)) {
		$error->set_error('invalid_login');
		$GLOBALS['log']->info('End: DotbWebServiceImpl->set_campaign_merge');
		return;
	} // if
...
function get_entries_count($session, $module_name, $query, $deleted) {
	$GLOBALS['log']->info('Begin: DotbWebServiceImpl->get_entries_count');

	$error = new SoapError();
	if (!self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', $module_name, 'list', 'no_access', $error)) {
		$GLOBALS['log']->info('End: DotbWebServiceImpl->get_entries_count');
		return;
	} // if
...
function get_report_entries($session, $ids, $select_fields ){
	$GLOBALS['log']->info('Begin: DotbWebServiceImpl->get_report_entries');
	$error = new SoapError();
	...
	if (!self::$helperObject->checkSessionAndModuleAccess($session, 'invalid_session', $module_name, 'read', 'no_access', $error)) {
		$GLOBALS['log']->info('End: DotbWebServiceImpl->get_report_entries');
		return;
	} // if
