...
function validate_user($user_name, $password){
	...
	global $server, $current_user, $dotb_config;
	...
	function get_user_module_list($user){
		...
		global $moduleList;
		$modules = array_flip(DotbACL::filterModuleList($moduleList, 'access', true)); // module names end up as keys
		global $modInvisList;
		...
		foreach($modules as $key=>$val) {
            if(!DotbACL::checkAccess($key, 'edit', array("owner_override" => true))) {
 ...
 * @param Array $link_module_fields -- The keys of the array are the DotbBean attributes, the values of the array are the values the attributes should have.
...
*/
	...
	function login_success($name_value_list = array()){
		...
		global $current_language, $dotb_config, $app_strings, $app_list_strings;
		...
		if (is_array($name_value_list) && !empty($name_value_list)) {
			foreach($name_value_list as $key => $value) {
				if (isset($value['name']) && ($value['name'] == 'language')) {
					...
					$supportedLanguages = $dotb_config['languages'];
	...
	function isLogLevelDebug() {
		if (isset($GLOBALS['dotb_config']['logger'])) {
			if (isset($GLOBALS['dotb_config']['logger']['level'])) {
				return ($GLOBALS['dotb_config']['logger']['level'] == 'debug');
			} // if
		}
		return false;
	} // fn
