 ...
 * @api
 */
class DotbRestRSS extends DotbRest
{
	/**
	...
	{
	    global $app_strings, $dotb_version, $dotb_flavor;
...
<channel>
    ...
    <link>{$GLOBALS['dotb_config']['site_url']}</link>
    <description>{$count} {$app_strings['LBL_RSS_RECORDS_FOUND']}</description>
    <pubDate>{$date}</pubDate>
    <generator>DotBCRM $dotb_version $dotb_flavor</generator>
...
EORSS;
    ...
    protected function generateItem($item)
    {
        ...
        $url = $GLOBALS['dotb_config']['site_url']  . htmlentities('/index.php?module=' . $item['module_name']. '&action=DetailView&record=' . $item['id']);
